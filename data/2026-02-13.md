...
TL;DR: DD-MDN is a probabilistic human trajectory forecasting model that excels in accuracy, uncertainty modeling, and robustness to short observations, outperforming prior methods on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address gaps in prior work, particularly the lack of focus on uncertainty modeling, calibration, and forecasts from short observation periods, which are crucial for applications like autonomous driving and human-robot interaction.

Method: The method involves an end-to-end probabilistic model combining a few-shot denoising diffusion backbone with a dual mixture density network to learn self-calibrated residence areas and probability-ranked anchor paths.

Result: Experiments on ETH/UCY, SDD, inD, and IMPTC datasets show DD-MDN outperforms existing methods in accuracy, robustness to short observations, and uncertainty modeling.

Conclusion: The paper concludes that DD-MDN achieves state-of-the-art accuracy, robustness, and reliable uncertainty modeling in human trajectory forecasting, making it suitable for critical downstream tasks like path planning and collision avoidance.
...
TL;DR: ABot-M0 is a framework for building general-purpose embodied agents by unifying data, optimizing training, and introducing Action Manifold Learning, achieving improved efficiency and stability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the challenges of fragmented data, inconsistent representations, and misaligned training objectives in robotics, aiming to enable general-purpose embodied intelligence across diverse platforms.

Method: The method involves constructing the UniACT-dataset, unified pre-training, and introducing Action Manifold Learning (AML) with a DiT backbone. It also includes a dual-stream mechanism for modular perception.

Result: Results show improved knowledge transfer, generalization, action prediction efficiency, and policy stability, with modular perception enhancing spatial understanding.

Conclusion: The paper concludes that ABot-M0, through its systematic data curation, unified pre-training, and Action Manifold Learning, successfully addresses the challenge of building general-purpose embodied agents across diverse hardware, demonstrating improved efficiency, stability, and generalization.
...
TL;DR: The paper proposes an automated deep learning model for classifying tea leaf diseases using the teaLeafBD dataset, achieving high accuracy with EfficientNetB3 (93%) and DenseNet201 (91%).


<details>
  <summary>Details</summary>
Motivation: Manual detection of tea leaf diseases is time-consuming and error-prone, necessitating an automated solution to improve efficiency and accuracy.

Method: The study uses a pipeline involving data preprocessing, adversarial training, augmentation, and model training with DenseNet201 and EfficientNetB3, along with Explainable AI strategies like Grad-CAM.

Result: EfficientNetB3 achieved 93% accuracy, and DenseNet201 reached 91%, demonstrating robust performance even with noisy inputs.

Conclusion: The proposed deep learning approach effectively classifies tea leaf diseases, offering a practical solution for agricultural management.
...
TL;DR: Active-Zero introduces active exploration for vision-language models, using three agents to autonomously improve learning, achieving notable accuracy gains over passive self-play methods.


<details>
  <summary>Details</summary>
Motivation: Existing self-play methods for vision-language models rely on passive interaction with static datasets, leading to inefficiency and dependence on initial data. Active-Zero addresses this by enabling active exploration of visual environments.

Method: Active-Zero employs three co-evolving agents: a Searcher for tailored image retrieval, a Questioner for calibrated reasoning tasks, and a Solver refined through accuracy rewards, forming a closed-loop self-scaffolding auto-curricula.

Result: Active-Zero improves reasoning task accuracy by 5.7% (53.97 average) and general understanding by 3.9% (59.77 average) on Qwen2.5-VL-7B-Instruct across 12 benchmarks, outperforming baselines.

Conclusion: Active-Zero demonstrates that active exploration is crucial for scalable and adaptive self-evolving vision-language systems, achieving significant improvements over existing self-play methods.
...
TL;DR: ReTracing is a multi-agent performance art using AI and robotics to explore human-machine interaction, revealing biases in generative systems through choreographed movements.


<details>
  <summary>Details</summary>
Motivation: The project aims to examine how artificial intelligence shapes, constrains, and produces bodily movement, drawing from science-fiction to explore human-machine interaction.

Method: The project uses large language models to generate paired prompts from science-fiction excerpts, which are then transformed into choreographic guides for a human performer and motor commands for a robot. The performance is captured and reconstructed into 3D point clouds and motion trails.

Result: ReTracing creates a digital archive of motion traces, revealing socio-cultural biases encoded in generative systems through choreographed movements.

Conclusion: ReTracing serves as a novel approach to reveal how generative systems encode socio-cultural biases through choreographed movements, addressing the critical question of what it means to be human among AIs that move, think, and leave traces.
...
TL;DR: VidLMs fail basic tests on video content, temporal sequence, and motion. REVEAL benchmark exposes these weaknesses, offering a tool for future research.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess whether VidLMs robustly account for video content, temporal sequence, and motion, given their increasing use in video understanding tasks. The study aims to identify and address fundamental weaknesses in these models.

Method: The research employs REVEAL, a diagnostic benchmark with five stress tests (temporal expectation bias, language-only shortcuts, video sycophancy, camera motion sensitivity, and spatiotemporal occlusion) to evaluate VidLMs. Leading open- and closed-source VidLMs are tested against human performance.

Result: Results show that VidLMs often fail to handle reversed scenes, neglect video content, agree with false claims, struggle with camera motion, and fail to aggregate temporal information under spatiotemporal occlusion. Humans outperform VidLMs in these tasks.

Conclusion: The study concludes that current Video-Language Models (VidLMs) lack robustness in handling video content, temporal sequence, and motion, as evidenced by their failures in controlled stress tests. The REVEAL benchmark and data pipeline are introduced to facilitate future research and improvements in VidLMs.
...
TL;DR: A novel pipeline for generating synthetic images from high-quality 3D models is introduced, enabling repeatable, quantifiable experiments for comparing ground-truth and reconstructed estimations.


<details>
  <summary>Details</summary>
Motivation: The need for a quantifiable and repeatable method to evaluate photogrammetry techniques, as current approaches are largely qualitative.

Method: A pipeline that generates synthetic images from high-quality 3D models and programmatically generated camera poses.

Result: Enables comparison of ground-truth knowledge (camera parameters, objects) against reconstructed estimations.

Conclusion: The proposed pipeline provides a robust framework for quantitative evaluation of photogrammetry techniques, addressing limitations of qualitative assessments.
...
TL;DR: The paper introduces SYNC loss, combining ad-hoc and post-hoc methods for better selective prediction in DNNs, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Current selective prediction methods are either ad-hoc (e.g., SelectiveNet) or post-hoc (e.g., softmax response), but the selective prior from post-hoc methods is underutilized during training.

Method: The SYNC loss integrates softmax response into SelectiveNet's training process, leveraging the selective prior during training.

Result: The method improves generalization and outperforms previous works on datasets like CIFAR-100, ImageNet-100, and Stanford Cars.

Conclusion: The proposed SYNC loss, which integrates ad-hoc and post-hoc methods, enhances selective prediction capabilities and sets new benchmarks for state-of-the-art performance.
...
TL;DR: A novel framework integrates dense depth priors into VINS-Mono for robust pose estimation in low-texture environments, achieving real-time performance on edge devices with significant accuracy improvements.


<details>
  <summary>Details</summary>
Motivation: Traditional VIO systems struggle in low-texture environments due to sparse visual features, necessitating complementary dense depth estimation.

Method: Proposes a framework enforcing affine-invariant depth consistency and ordinal constraints, filtering unstable artifacts via variance-based gating, while adhering to edge device computational limits.

Result: Reduces Absolute Trajectory Error (ATE) by up to 28.3% on TartanGround and M3ED datasets, preventing divergence in challenging scenarios.

Conclusion: The method successfully bridges the gap between dense depth estimation and real-time edge deployment, enhancing VIO robustness in low-texture environments.
...
TL;DR: The paper introduces StreamSR, a dataset for real-time super-resolution in streaming, and EfRLFN, an efficient model with novel design choices, showing improved performance and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing methods and datasets for real-time super-resolution do not adequately address the challenges of compressed video content in streaming media.

Method: The authors introduce StreamSR, a dataset from YouTube, and propose EfRLFN, a model with Efficient Channel Attention and a hyperbolic tangent activation function. They also design a composite loss function for better training.

Result: EfRLFN outperforms 11 state-of-the-art models, and fine-tuning other models on StreamSR leads to significant performance gains.

Conclusion: The study provides a relevant dataset, an efficient model, and demonstrates improved performance for real-time super-resolution in streaming.
...
TL;DR: ArtContext uses a novel corpus and a domain-specific CLIP model (PaintingCLIP) to annotate artworks with contextual information from art history articles, outperforming CLIP and generalizing to other humanities fields.


<details>
  <summary>Details</summary>
Motivation: Identifying and consolidating information from various art history articles about specific artworks is challenging. ArtContext aims to address this by providing a systematic way to annotate artworks with contextual information.

Method: The method involves a corpus collection pipeline and a bespoke CLIP model adapted using Low-Rank Adaptation (LoRA). The resulting model, PaintingCLIP, is weakly supervised by the collected corpus.

Result: PaintingCLIP outperforms the original CLIP model in providing context for artworks, demonstrating the effectiveness of the proposed pipeline.

Conclusion: The proposed ArtContext pipeline, leveraging a novel corpus collection method and a domain-specific CLIP model (PaintingCLIP), effectively annotates artworks with contextual information from art history articles and Wikidata. The approach is generalizable to other humanities domains.
...
TL;DR: Latent Forcing modifies latent diffusion models to efficiently generate high-quality images by jointly processing latents and pixels with tuned noise schedules, achieving state-of-the-art results on ImageNet.


<details>
  <summary>Details</summary>
Motivation: Latent diffusion models lose end-to-end benefits by discarding information during encoding, requiring separate decoders, and modeling auxiliary distributions. Latent Forcing addresses these limitations.

Method: Latent Forcing jointly processes latents and pixels with separately tuned noise schedules, allowing latents to serve as intermediate computation scratchpads before generating high-frequency pixel features.

Result: Latent Forcing achieves a new state-of-the-art for diffusion transformer-based pixel generation on ImageNet at the given compute scale.

Conclusion: The proposed Latent Forcing method efficiently combines latent and pixel processing, improving image generation quality and addressing limitations of traditional latent diffusion models.
...
TL;DR: A neural implicit function trained on high-resolution CTA data improves cardiac shape reconstruction from CMRI, achieving high accuracy for RV and MYO.


<details>
  <summary>Details</summary>
Motivation: The anisotropic nature of SAX CMRI limits cardiac shape analysis, prompting the use of high-resolution CTA data to overcome this limitation.

Method: A single neural implicit function is trained using near-isotropic, high-resolution CTA data to jointly represent cardiac shapes from CMRI at any resolution. The method is evaluated for RV and MYO reconstruction, with performance assessed using 4CH slice extraction and comparison to reference CMRI segmentations.

Result: The method achieved Dice similarity coefficients of 0.91 ± 0.07 (RV) and 0.75 ± 0.13 (MYO), and Hausdorff distances of 6.21 ± 3.97 mm (RV) and 7.53 ± 5.13 mm (MYO), demonstrating accurate and anatomically plausible shape reconstruction.

Conclusion: The proposed method successfully leverages high-resolution CTA data to train a neural implicit function for accurate and smooth cardiac shape reconstruction from CMRI, improving cardiac shape analysis.
...
TL;DR: Ctrl&Shift is a diffusion-based framework for object-level manipulation in images/videos, achieving background preservation, geometric consistency, and user control without explicit 3D modeling.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to jointly achieve background preservation, geometric consistency, and user-controllable transformations, with geometry-based approaches requiring explicit 3D reconstruction and diffusion-based methods lacking fine-grained control.

Method: Ctrl&Shift decomposes manipulation into object removal and reference-guided inpainting under camera pose control, using a unified diffusion process. It employs a multi-task, multi-stage training strategy to separate background, identity, and pose signals.

Result: The framework achieves state-of-the-art results in fidelity, viewpoint consistency, and controllability, validated through extensive experiments.

Conclusion: Ctrl&Shift is the first framework to unify fine-grained geometric control and real-world generalization for object manipulation without explicit 3D modeling.
...
TL;DR: The paper introduces a ULF DTI sequence and two algorithms (Bayesian bias correction and DiffSR) to improve imaging quality. Results show successful recovery of white matter information and potential for Alzheimer's disease classification. Code is publicly released.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of portable, ultra-low-field (ULF) magnetic resonance imaging, such as coarse resolutions and low signal-to-noise ratios, particularly in diffusion tensor imaging (DTI). The goal is to improve the accuracy and reliability of ULF DTI scans for neuroimaging applications.

Method: The paper introduces a nine-direction, single-shell ULF DTI sequence, along with a Bayesian bias field correction algorithm and a convolutional neural network-based superresolution algorithm (DiffSR). These methods are tested through synthetic downsampling experiments and real, matched ULF and high-field DTI scans.

Result: The results demonstrate that the proposed algorithms can recover microstructural and volumetric white matter information at ULF. Additionally, DiffSR improves Alzheimer's disease classification in synthetically degraded scans and enhances agreement between DTI metrics compared to un-degraded scans.

Conclusion: The paper concludes by highlighting the successful recovery of microstructural and volumetric white matter information at ultra-low-field (ULF) using the introduced algorithms. It also emphasizes the potential of DiffSR in improving Alzheimer's disease classification and the public release of the code to advance ULF reconstruction and DTI sequence harmonization.
...
TL;DR: DBTANet improves Semantic Change Detection by combining global and local features, temporal modeling, and boundary awareness, achieving top performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for Semantic Change Detection (SCD) suffer from blurred boundaries and inadequate temporal modeling, limiting accuracy.

Method: The paper proposes DBTANet, a dual-branch framework with a Siamese encoder (frozen SAM branch for global context and ResNet34 for local details), a Bidirectional Temporal Awareness Module (BTAM), and a Gaussian-smoothed Projection Module (GSPM).

Result: Extensive experiments on public benchmarks show DBTANet outperforms existing methods, effectively addressing the identified issues.

Conclusion: DBTANet achieves state-of-the-art performance in Semantic Change Detection by integrating global semantics, local details, temporal reasoning, and boundary awareness.
...
TL;DR: ARFC is a flexible feature compression framework using a single model for any ratio, with ARC, MoS, and ERGC modules. It outperforms existing methods and even uncompressed features in some cases.


<details>
  <summary>Details</summary>
Motivation: Existing feature compression methods lack flexibility and generalization, requiring retraining for new compression ratios. The goal is to develop a single-model solution supporting any compression ratio.

Method: The proposed ARFC framework includes the Arbitrary Ratio Compressor (ARC) for flexible compression via next-token prediction, along with the Mixture of Solutions (MoS) module and Entity Relation Graph Constraint (ERGC) to enhance feature quality.

Result: ARFC consistently outperforms existing approaches across various tasks (cross-modal retrieval, image classification, image retrieval) and datasets, sometimes exceeding uncompressed feature performance.

Conclusion: The ARFC framework demonstrates effectiveness and versatility, outperforming existing methods and even surpassing uncompressed features in some cases. It is validated for practical, resource-constrained scenarios.
...
TL;DR: ImagineAgent, a framework combining cognitive reasoning and generative imagination, addresses cross-modal hallucinations and ambiguity in OV-HOI, achieving SOTA results with 20% less training data.


<details>
  <summary>Details</summary>
Motivation: The limitations of Multimodal Large Language Models in Open-Vocabulary Human-Object Interaction (OV-HOI), such as cross-modal hallucinations and occlusion-induced ambiguity, drive the need for a more robust solution.

Method: The framework constructs cognitive maps to model relationships between entities and actions, dynamically invoking tools like retrieval augmentation, image cropping, and diffusion models for cross-modal alignment.

Result: ImagineAgent outperforms existing methods, achieving superior performance with significantly less training data.

Conclusion: ImagineAgent achieves state-of-the-art performance on SWIG-HOI and HICO-DET datasets, demonstrating robustness and efficiency with only 20% of the training data required by existing methods.
...
TL;DR: VasoMIM, a vascular anatomy-aware SSL framework, and XA-170K dataset address the scarcity of annotated X-ray angiogram data, achieving top performance in downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for X-ray angiogram analysis are limited by scarce annotated data. Large-scale self-supervised learning (SSL) remains unexplored due to the lack of effective frameworks and datasets.

Method: The VasoMIM framework integrates domain-specific anatomical knowledge through an anatomy-guided masking strategy and an anatomical consistency loss. It also introduces XA-170K, a large-scale X-ray angiogram dataset.

Result: VasoMIM achieves state-of-the-art performance on four downstream tasks across six datasets, showcasing its robust transferability.

Conclusion: VasoMIM demonstrates superior transferability and state-of-the-art performance in X-ray angiogram analysis tasks, highlighting its potential as a foundation model. The framework and dataset (XA-170K) will be publicly available.
...
TL;DR: MFdiff, a supervise-assisted multi-modality fusion diffusion model, restores high-quality PET images from low-dose inputs using MR data, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To reduce radiation exposure in PET imaging without compromising image quality, leveraging MR images for PET restoration while overcoming multi-modality fusion inconsistencies and OOD data challenges.

Method: The method involves a multi-modality feature fusion module to optimize fusion features and a diffusion model for iterative SPET image generation, supported by a two-stage supervise-assisted learning strategy.

Result: Experiments show MFdiff restores high-quality SPET images and surpasses state-of-the-art methods in both qualitative and quantitative performance.

Conclusion: The proposed MFdiff model effectively restores high-quality SPET images from multi-modality inputs and outperforms existing methods, addressing challenges in multi-modality fusion and OOD data mismatch.
...
TL;DR: The paper introduces a generative compression framework for image denoising, combining entropy-coded latent representations with perceptual measures to balance distortion and realism. It proposes two methods: a WGAN-based denoiser and a diffusion-based approach, backed by theoretical guarantees and validated on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Distortion-driven denoising methods often over-smooth images, especially under strong noise or distribution shifts. The paper aims to preserve structural details and perceptual realism by leveraging generative models and perceptual metrics.

Method: The framework uses entropy-coded latent representations for low-complexity structure and generative decoders (WGAN and diffusion models) to recover textures via LPIPS loss and Wasserstein distance. It includes theoretical analysis of reconstruction error and decoding error probability.

Result: Experiments show consistent perceptual improvements while maintaining competitive distortion performance on synthetic and real-noise benchmarks.

Conclusion: The proposed generative compression framework effectively balances rate-distortion-perception trade-offs, offering improved perceptual quality without sacrificing distortion performance.
...
TL;DR: LUVE is a latent-cascaded UHR video generation framework using dual frequency experts to address challenges in motion modeling, semantic planning, and detail synthesis.


<details>
  <summary>Details</summary>
Motivation: Ultra-high-resolution (UHR) video generation is challenging due to difficulties in motion modeling, semantic planning, and detail synthesis.

Method: LUVE uses a three-stage architecture: low-resolution motion generation, video latent upsampling, and high-resolution content refinement with dual frequency experts.

Result: LUVE achieves superior photorealism and content fidelity in UHR video generation.

Conclusion: The framework effectively addresses UHR video generation challenges, validated by extensive experiments and ablation studies.
...
TL;DR: FlowAdapt, a PEFT framework for V2X, uses optimal transport to address redundancy and semantic erosion, achieving top performance with 1% trainable parameters.


<details>
  <summary>Details</summary>
Motivation: The challenges of performance degradation and training instability when applying Parameter-Efficient Fine-Tuning (PEFT) to multi-agent settings in V2X collaborative perception.

Method: FlowAdapt, a parameter-efficient framework based on optimal transport theory, includes Wasserstein Greedy Sampling for redundant sample filtering and Progressive Knowledge Transfer to mitigate semantic degradation.

Result: FlowAdapt outperforms existing methods, demonstrating significant improvements in performance and stability with minimal trainable parameters.

Conclusion: FlowAdapt effectively bridges domain gaps in V2X collaborative perception with superior sample efficiency and generalization, achieving state-of-the-art performance using only 1% of trainable parameters.
...
TL;DR: The paper proposes an LLM-based framework (LLM-DRS) for vision-based SHM, automating damage assessment and report generation to enhance post-disaster resilience.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of previous AI-aided SHM methods, which only provide discrete outputs like damage labels and coordinates, requiring additional analysis by engineers. The rise of LLMs in 2022 offered new opportunities to enhance SHM by automating report generation.

Method: The proposed LLM-based Disaster Reconnaissance Summarization (LLM-DRS) framework involves a standard reconnaissance plan, processing text-based metadata and image-based vision data into a unified format, extracting key attributes using Deep Convolutional Neural Networks, and generating summary reports via an LLM.

Result: Results indicate that the LLM-DRS framework effectively integrates vision data and metadata, enabling the generation of comprehensive summary reports for structures or regions, thereby improving post-disaster reconnaissance efficiency.

Conclusion: The study concludes that integrating Large Language Models (LLMs) into vision-based Structural Health Monitoring (SHM) shows promising potential for improving resilience in the built environment, especially for rapid post-disaster reconnaissance.
...
TL;DR: PLOT-CT uses Voronoi decomposition on pre-log sinograms to improve LDCT reconstruction, outperforming traditional methods by 2.36dB PSNR.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of severe noise and compromised data fidelity in LDCT reconstruction by exploiting structural information in pre-log measurements, avoiding the noise amplification caused by logarithmic transformation.

Method: The method involves Voronoi decomposition of pre-log sinograms to disentangle data into distinct components embedded in separate latent spaces, enhancing feature learning and reconstruction accuracy.

Result: PLOT-CT achieves a 2.36dB PSNR improvement over traditional methods at the 1e4 incident photon level in the pre-log domain.

Conclusion: PLOT-CT significantly improves LDCT reconstruction by leveraging pre-log domain information and Voronoi decomposition, achieving superior performance over traditional methods.
...
TL;DR: PLESS enhances pseudo-labels in scribble-supervised medical image segmentation by hierarchically refining labels, improving accuracy on cardiac MRI datasets.


<details>
  <summary>Details</summary>
Motivation: Scribble annotations reduce labeling costs but suffer from noisy and incomplete supervision. Existing pseudo-label methods are limited by label quality.

Method: PLESS hierarchically partitions images into coherent regions to propagate scribble information and refine pseudo-labels. It is model-agnostic and integrates into existing methods.

Result: PLESS consistently improves segmentation accuracy on ACDC and MSCMRseg datasets across four scribble-supervised algorithms.

Conclusion: PLESS enhances pseudo-label reliability and spatial consistency, improving segmentation accuracy across scribble-supervised algorithms, as demonstrated on cardiac MRI datasets.
...
TL;DR: ScalSelect is a scalable, training-free method for multimodal data selection in VIT, achieving high performance with minimal data by focusing on instruction-relevant features and linear-time complexity.


<details>
  <summary>Details</summary>
Motivation: Large-scale VIT training is inefficient due to data redundancy, and existing data selection methods are costly or limited in scalability.

Method: ScalSelect constructs instruction-relevant sample representations and identifies key samples by approximating the dominant subspace of the dataset, avoiding pairwise comparisons.

Result: ScalSelect achieves over 97.5% performance of full-data training with only 16% of the data and sometimes outperforms full-data training.

Conclusion: ScalSelect offers an efficient, scalable solution for data selection in VIT, improving training efficiency without external models or datasets.
...
TL;DR: A novel surface reconstruction method using Poisson's equation as a proxy PDE, leveraging Green's functions and superposition for improved high-frequency detail approximation.


<details>
  <summary>Details</summary>
Motivation: The paper aims to improve surface reconstruction by encoding it as a solution to Poisson's equation, inspired by its connection to physics (e.g., electrostatic potential).

Method: The approach uses Green's functions to derive a closed-form solution for Poisson's equation and exploits the linearity of the PDE to superpose solutions for shape reconstruction.

Result: The method demonstrates enhanced performance in capturing high-frequency details, even with limited shape priors.

Conclusion: The proposed technique offers a promising alternative to traditional SDF-based methods, particularly for detailed surface reconstruction.
...
TL;DR: The paper explores adversarial robustness in ResNet-based models for brain tumor classification, finding BrainNeXt most robust to black-box attacks but highlighting vulnerabilities in other models and data preprocessing trade-offs.


<details>
  <summary>Details</summary>
Motivation: The study addresses the underexplored challenge of adversarial robustness in deep learning models for brain tumor classification, especially in clinical MRI scenarios.

Method: The authors evaluate ResNet-based architectures (BrainNet, BrainNeXt, DilationNet) against FGSM and PGD attacks across three preprocessing configurations: full-sized augmented, shrunk augmented, and shrunk non-augmented MRI datasets.

Result: BrainNeXt shows the highest robustness to black-box attacks but weaker transferable adversarial samples. BrainNet and Dilation models are more vulnerable, especially under PGD. Shrunk and non-augmented data reduce resilience despite high untampered accuracy.

Conclusion: The findings emphasize the need to jointly assess classification performance and adversarial robustness for reliable real-world deployment in brain MRI analysis.
...
TL;DR: GR-Diffusion integrates GR and diffusion models for superior 3D PET reconstruction, outperforming state-of-the-art methods in detail preservation and image quality.


<details>
  <summary>Details</summary>
Motivation: PET reconstruction faces challenges like noise amplification and detail loss due to sparse sampling and ill-posed inverse problems. GR-Diffusion aims to address these by leveraging GR's geometric priors and diffusion models' generative capabilities.

Method: The GR-Diffusion framework combines the 3D discrete Gaussian representation (GR) with diffusion models. GR generates a reference PET image, which guides the diffusion process hierarchically—fine-grained for local details and coarse-grained for global consistency.

Result: Experiments on UDPET and Clinical datasets demonstrate GR-Diffusion's superiority in enhancing image quality and preserving physiological details across varying dose levels.

Conclusion: GR-Diffusion significantly improves 3D whole-body PET reconstruction by integrating geometric priors from GR and the generative power of diffusion models, outperforming existing methods in image quality and detail preservation.
