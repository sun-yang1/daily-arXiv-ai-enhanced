...
TL;DR: A new data fusion framework, MoDM, uses second-order moments from distinct orientation distributions to uniquely and accurately reconstruct molecular structures, enhancing cryo-EM imaging quality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the reconstruction of three-dimensional molecular structures from noisy tomographic projection images by utilizing multiple datasets under different experimental conditions.

Method: The paper introduces the method of double moments (MoDM), a convex-relaxation-based algorithm that reconstructs molecular structures from two instances of the second-order moment of projection images obtained under distinct orientation distributions (one uniform, the other non-uniform and unknown).

Result: The results demonstrate that the MoDM framework accurately recovers molecular structures using only second-order statistics, proving that these moments generically uniquely determine the underlying structure (up to a global rotation and reflection).

Conclusion: The paper concludes that leveraging dataset diversity by collecting and modeling multiple datasets under different experimental conditions can significantly enhance reconstruction quality in computational imaging tasks.
...
TL;DR: ASAG improves diffusion models by disrupting attention alignments via optimal transport, enhancing quality and controllability without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing guidance methods like CFG lack a principled foundation and rely on heuristic perturbations, prompting the need for a more systematic approach.

Method: ASAG reinterprets attention scores via optimal transport and injects an adversarial cost within self-attention layers to reduce pixel-wise similarity between queries and keys.

Result: ASAG consistently improves text-to-image diffusion, enhances controllability and fidelity in downstream applications, and is lightweight and plug-and-play.

Conclusion: ASAG, a novel method based on optimal transport and adversarial cost injection, improves diffusion model performance by disrupting misleading attention alignments, enhancing sample quality and controllability without retraining.
...
TL;DR: TrackStudio is a user-friendly, modular toolkit for markerless motion tracking, validated for reliability and accessibility across diverse settings.


<details>
  <summary>Details</summary>
Motivation: There is a gap in accessible, integrated solutions for markerless motion tracking that cater to non-experts across diverse settings.

Method: TrackStudio combines established open-source tools into a modular, GUI-based pipeline, offering automatic 2D and 3D tracking, calibration, preprocessing, feature extraction, and visualization.

Result: Validation tests showed stable and consistent tracking, with average inter-frame correlations exceeding 0.98 and low triangulation errors (<13.6mm for hand tracking). The pipeline can also be extended to other body and face regions.

Conclusion: TrackStudio provides a practical and accessible solution for markerless motion tracking, enabling reliable performance without requiring specialist expertise.
...
### [4] [Predicting Coronary Artery Calcium Severity based on Non-Contrast Cardiac CT images using Deep Learning](https://arxiv.org/abs/2511.07695)
*Lachlan Nguyen,Aidan Cousins,Arcot Sowmya,Hugh Dixson,Sonit Singh*

Main category: cs.CV

TL;DR: A CNN model was developed to automate CAC scoring in cardiac CT images, achieving high accuracy and agreement with current semiautomatic methods, demonstrating potential for clinical use.


<details>
  <summary>Details</summary>
Motivation: Cardiovascular disease is a leading cause of mortality, and current CAC scoring methods are time-intensive. The study aims to automate and improve the efficiency of CAC scoring using deep learning.

Method: A deep learning CNN model was developed to classify calcium scores in cardiac, non-contrast CT images into six clinical categories. The dataset included 68 patient scans, divided into training, validation, and test sets, with semiautomatic CAC scores as reference labels.

Result: The model achieved high performance with a Cohen's kappa of 0.962, overall accuracy of 96.5%, and misclassified only 32 cases, mostly overestimating CAC scores. It showed good generalizability to test data.

Conclusion: The study demonstrates the viability of a CNN model for accurately classifying CAC scores into six clinical categories, showing high agreement with current semiautomatic practices and good generalizability.
...
### [5] [FlowFeat: Pixel-Dense Embedding of Motion Profiles](https://arxiv.org/abs/2511.07696)
*Nikita Araslanov,Anna Sonnweber,Daniel Cremers*

Main category: cs.CV

TL;DR: FlowFeat introduces a high-resolution, multi-task feature representation using a novel distillation technique for apparent motion, enhancing state-of-the-art encoders in dense prediction tasks.


<details>
  <summary>Details</summary>
Motivation: Current networks like transformers produce low-resolution feature grids, which are suboptimal for dense prediction tasks.

Method: FlowFeat uses a novel distillation technique to embed motion profiles, leveraging optical flow networks and diverse video data for self-supervised training.

Result: FlowFeat significantly improves five state-of-the-art encoders and upsampling strategies across three dense tasks, with low computational cost and robustness to inaccurate flow estimation.

Conclusion: FlowFeat advances reliable and versatile dense image representations, offering high spatial detail and temporal consistency.
...
TL;DR: A unified method for fine-grained image-text alignment improves robustness and interpretability by addressing limitations in token significance assessment and uncertainty modeling, achieving state-of-the-art results on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Existing approaches lack robust intra-modal mechanisms for assessing token significance and fail to model fine-grained uncertainty, leading to poor generalization and oversimplified cross-modal relationships.

Method: The method leverages modality-specific biases to identify salient features and represents region features as a mixture of Gaussian distributions to capture fine-grained uncertainty.

Result: The approach outperforms existing methods across various backbone architectures, demonstrating improved performance in fine-grained alignment tasks.

Conclusion: The proposed unified approach, incorporating significance-aware and granularity-aware modeling along with region-level uncertainty modeling, achieves state-of-the-art performance on Flickr30K and MS-COCO datasets, enhancing robustness and interpretability in fine-grained image-text alignment.
...
TL;DR: UltraGS is a Gaussian Splatting framework for ultrasound imaging, improving novel view synthesis with depth-aware splatting and a physics-based rendering function, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: The limited field of view in ultrasound imaging complicates novel view synthesis, prompting the need for an optimized framework like UltraGS.

Method: The paper introduces UltraGS, a Gaussian Splatting framework with a depth-aware strategy and SH-DARS, a lightweight rendering function combining spherical harmonics and ultrasound-specific wave physics.

Result: UltraGS outperforms existing methods, achieving PSNR up to 29.55, SSIM up to 0.89, and MSE as low as 0.002, with real-time synthesis at 64.69 fps.

Conclusion: UltraGS achieves state-of-the-art performance in novel view synthesis for ultrasound imaging, with high PSNR, SSIM, and low MSE, while enabling real-time synthesis. The framework and dataset are open-sourced for community use.
...
TL;DR: VectorSynth is a diffusion-based framework for satellite image synthesis using polygonal annotations, enabling fine-grained edits and outperforming prior methods in realism and fidelity.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the limitations of prior text- or layout-conditioned models by enabling fine-grained, spatially grounded edits in satellite image synthesis.

Method: VectorSynth uses a diffusion-based framework with a vision language alignment module to produce pixel-level embeddings from polygon semantics, guiding conditional image generation.

Result: The framework outperforms prior methods in semantic fidelity and structural realism, with the trained vision language model showing fine-grained spatial grounding.

Conclusion: VectorSynth demonstrates strong improvements in semantic fidelity and structural realism over prior methods, with fine-grained spatial grounding. The framework supports interactive workflows for rapid simulations and edits.
...
TL;DR: Auto-US integrates ultrasound video data with clinical text, achieving 86.73% accuracy in classification and generating diagnostic suggestions validated by clinicians.


<details>
  <summary>Details</summary>
Motivation: Existing AI-assisted ultrasound diagnosis lacks dataset diversity, diagnostic performance, and clinical applicability.

Method: Proposed Auto-US, an intelligent agent combining ultrasound videos and clinical text, supported by the CUV Dataset (495 videos) and CTU-Net for classification.

Result: CTU-Net achieved 86.73% accuracy, and Auto-US generated clinically validated diagnostic suggestions with scores exceeding 3/5.

Conclusion: Auto-US demonstrates effectiveness and clinical potential for real-world ultrasound applications.
...
### [10] [Filtered-ViT: A Robust Defense Against Multiple Adversarial Patch Attacks](https://arxiv.org/abs/2511.07755)
*Aja Khanal,Ahmed Faid,Apurva Narayan*

Main category: cs.CV

TL;DR: Filtered-ViT, a new vision transformer with SMART-VMF, outperforms existing defenses against adversarial and natural patch-like disruptions, achieving high accuracy in synthetic and real-world tests.


<details>
  <summary>Details</summary>
Motivation: Deep learning vision systems are vulnerable to adversarial patches, especially in safety-critical domains like healthcare, where existing defenses fail against multiple localized disruptions.

Method: Filtered-ViT integrates SMART Vector Median Filtering (SMART-VMF), a spatially adaptive, multi-scale, robustness-aware mechanism that selectively suppresses corrupted regions while preserving semantic detail.

Result: On ImageNet with LaVAN multi-patch attacks, Filtered-ViT achieves 79.8% clean accuracy and 46.3% robust accuracy under four simultaneous 1% patches, outperforming existing defenses. It also performs well in real-world scenarios, mitigating natural artifacts without degrading diagnostic content.

Conclusion: Filtered-ViT is the first transformer to demonstrate unified robustness against both adversarial and naturally occurring patch-like disruptions, making it a reliable solution for high-stakes environments like healthcare.
...
TL;DR: SCBCH introduces a novel framework for cross-modal hashing, addressing label noise and semantic overlaps, and achieves superior retrieval performance.


<details>
  <summary>Details</summary>
Motivation: Existing cross-modal hashing methods rely on fully annotated datasets and overlook label noise and partial semantic overlaps in multi-label data, limiting their robustness.

Method: The framework includes two modules: Cross-modal Semantic-Consistent Classification (CSCC) for handling noisy labels and Bidirectional Soft Contrastive Hashing (BSCH) for adaptive contrastive learning based on semantic overlap.

Result: SCBCH outperforms state-of-the-art approaches on four cross-modal retrieval benchmarks, especially under noisy multi-label conditions.

Conclusion: The proposed SCBCH framework effectively addresses the challenges of label noise and partial semantic overlaps in cross-modal hashing, demonstrating superior performance and robustness compared to existing methods.
...
### [12] [Cancer-Net PCa-MultiSeg: Multimodal Enhancement of Prostate Cancer Lesion Segmentation Using Synthetic Correlated Diffusion Imaging](https://arxiv.org/abs/2511.07816)
*Jarett Dewbury,Chi-en Amy Tai,Alexander Wong*

Main category: cs.CV

TL;DR: CDI$^s$ improves prostate cancer lesion segmentation, enhancing performance in 94% of cases without extra scan time or architectural changes.


<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches for prostate cancer lesion segmentation achieve limited performance, with Dice scores of 0.32 or lower in large patient cohorts.

Method: A comprehensive evaluation of six state-of-the-art segmentation architectures using 200 patients with co-registered CDI$^s$, DWI, and ADC sequences.

Result: CDI$^s$ integration enhances or preserves segmentation performance in 94% of configurations, with up to 72.5% relative improvement over baseline modalities. CDI$^s$ + DWI is the safest enhancement pathway.

Conclusion: The study validates CDI$^s$ as a practical and effective enhancement for prostate cancer lesion segmentation, with significant improvements in performance and no additional scan time or architectural changes required.
...
### [13] [CloudMamba: Grouped Selective State Spaces for Point Cloud Analysis](https://arxiv.org/abs/2511.07823)
*Kanglin Qu,Pan Gao,Qun Dai,Zhanzhi Ye,Rui Ye,Yuanhao Sun*

Main category: cs.CV

TL;DR: CloudMamba improves point cloud analysis by addressing serialization, geometric perception, and overfitting, achieving top results with less complexity.


<details>
  <summary>Details</summary>
Motivation: Existing Mamba-based point cloud analysis suffers from imperfect serialization, lack of high-level geometric perception, and overfitting in the selective state space model (S6).

Method: The paper introduces sequence expanding and merging for stable point cloud serialization, chainedMamba for high-level geometric perception, and GS6 to mitigate overfitting in S6.

Result: Experiments show CloudMamba outperforms existing methods with significantly lower complexity.

Conclusion: CloudMamba achieves state-of-the-art results in point cloud analysis with reduced complexity, addressing challenges like imperfect serialization, insufficient geometric perception, and overfitting.
...
### [14] [Visual Bridge: Universal Visual Perception Representations Generating](https://arxiv.org/abs/2511.07877)
*Yilin Gao,Shuguang Dou,Junzhou Li,Zhiheng Yu,Yin Li,Dongsheng Jiang,Shugong Xu*

Main category: cs.CV

TL;DR: A universal visual perception framework using flow matching outperforms single-task models, achieving competitive results across multiple tasks and settings.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of the "single-task-single-model" paradigm in diffusion models and the cross-domain generalization ability of large language models, aiming to enhance generalizability and scalability in multi-task scenarios.

Method: The approach formulates the process as a universal flow-matching problem from image patch tokens to task-specific representations, leveraging a self-supervised foundation model and a multi-scale, circular task embedding mechanism.

Result: Extensive experiments show the model achieves competitive performance in zero-shot and fine-tuned settings, with ablation studies validating its robustness, scalability, and generalization.

Conclusion: The proposed universal visual perception framework demonstrates competitive performance across multiple tasks, outperforming prior generalist and specialist models, and lays a foundation for future research in universal vision modeling.
...
### [15] [Generating Sketches in a Hierarchical Auto-Regressive Process for Flexible Sketch Drawing Manipulation at Stroke-Level](https://arxiv.org/abs/2511.07889)
*Sicong Zang,Shuhui Gao,Zhijun Fang*

Main category: cs.CV

TL;DR: A hierarchical auto-regressive method for flexible, real-time manipulation of sketches during generation by adjusting stroke embeddings.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable more flexible manipulation of sketch drawing during generation, as existing methods require all conditions to be set before generation starts, limiting real-time adjustments.

Method: The method involves a three-staged hierarchy for generating each stroke: predicting a stroke embedding, anchoring the stroke on the canvas, and translating the embedding to drawing actions. This process is auto-regressive, considering recently generated strokes and their positions.

Result: The approach successfully allows for real-time manipulation of sketches during generation by adjusting stroke embeddings, providing a more flexible and dynamic sketching process.

Conclusion: The proposed hierarchical auto-regressive sketch generating process allows for flexible manipulation of stroke-level sketch drawing during generation by adjusting editable stroke embeddings, overcoming the limitation of no further manipulation during the generation process.
...
### [16] [Exploring the Underwater World Segmentation without Extra Training](https://arxiv.org/abs/2511.07923)
*Bingyu Li,Tao Huo,Da Zhang,Zhiyuan Zhao,Junyu Gao,Xuelong Li*

Main category: cs.CV

TL;DR: The paper introduces AquaOV255, a large-scale underwater segmentation dataset, and UOVSBench, a benchmark for open-vocabulary underwater segmentation. It also proposes Earth2Ocean, a training-free framework for transferring terrestrial vision-language models to underwater domains, achieving notable performance improvements.


<details>
  <summary>Details</summary>
Motivation: Existing datasets and models for segmentation are limited to terrestrial scenes, leaving a gap for marine biodiversity monitoring. The paper aims to address this by creating a comprehensive underwater dataset and benchmark.

Method: The authors introduce AquaOV255 (a dataset) and UOVSBench (a benchmark). They also propose Earth2Ocean, a framework with two components: GMG for refining visual features and CSA for enhancing text embeddings.

Result: Earth2Ocean achieves significant performance improvements on the UOVSBench benchmark while maintaining efficient inference.

Conclusion: The paper successfully bridges the gap in underwater segmentation by providing a dataset, benchmark, and a novel framework, demonstrating effective transfer of terrestrial models to underwater domains.
...
### [17] [HD$^2$-SSC: High-Dimension High-Density Semantic Scene Completion for Autonomous Driving](https://arxiv.org/abs/2511.07925)
*Zhiwen Yang,Yuxin Peng*

Main category: cs.CV

TL;DR: The paper introduces HD²-SSC, a framework for 3D semantic scene completion in autonomous driving, addressing dimension and density gaps in existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing SSC methods struggle with the dimension gap between 2D inputs and 3D outputs, and the density gap between sparse annotations and real-world dense occupancy.

Method: The HD²-SSC framework includes a High-dimension Semantic Decoupling module to expand 2D features and a High-density Occupancy Refinement module to enhance voxel density.

Result: Experiments on SemanticKITTI and SSCBench-KITTI-360 datasets demonstrate the framework's effectiveness.

Conclusion: HD²-SSC successfully bridges the gaps in 3D semantic scene completion, improving scene understanding for autonomous driving.
...
### [18] [An Image-Based Path Planning Algorithm Using a UAV Equipped with Stereo Vision](https://arxiv.org/abs/2511.07928)
*Selim Ahmet Iz,Mustafa Unel*

Main category: cs.CV

TL;DR: A novel image-based path planning algorithm using UAV-generated disparity maps and computer vision outperforms A* and PRM in handling terrain depth for safer paths.


<details>
  <summary>Details</summary>
Motivation: Traditional path planning algorithms struggle with distinguishing terrain depth (e.g., craters and hills) in 2D images, which impacts path safety. The proposed method addresses this limitation by leveraging disparity maps and advanced computer vision.

Method: The algorithm uses computer vision techniques, including edge, line, and corner detection, along with stereo depth reconstruction, to generate a disparity map from UAV-captured images. ArUco marker pose estimation and circle detection are used to identify initial and desired points.

Result: Comparative analysis in V-REP simulations and a laboratory setup shows the algorithm's effectiveness, outperforming A* and PRM in scenarios where terrain depth is critical.

Conclusion: The proposed image-based path planning algorithm demonstrates promising results in both virtual and physical environments, proving its effectiveness compared to deterministic and probabilistic algorithms like A* and PRM.
...
### [19] [Federated CLIP for Resource-Efficient Heterogeneous Medical Image Classification](https://arxiv.org/abs/2511.07929)
*Yihang Wu,Ahmad Chaddad*

Main category: cs.CV

TL;DR: FedMedCLIP is a novel FL approach for medical image classification using CLIP, FAM, and masked MLP, achieving high performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: To address privacy concerns and challenges like data heterogeneity and resource costs in federated learning for medical imaging, especially with vision language models.

Method: The method involves a masked feature adaptation module (FAM) for communication efficiency, frozen CLIP encoders for computational savings, a masked MLP for local task adaptation, and KL divergence-based distillation for mutual learning. Model compression and ensemble predictions are also utilized.

Result: FedMedCLIP achieves 8% higher performance than the second-best baseline on ISIC2019 and is 120x faster than FedAVG.

Conclusion: The proposed FedMedCLIP approach demonstrates superior performance and efficiency in medical image classification, outperforming baselines while significantly reducing resource costs.
...
### [20] [Laytrol: Preserving Pretrained Knowledge in Layout Control for Multimodal Diffusion Transformers](https://arxiv.org/abs/2511.07934)
*Sida Huang,Siqi Huang,Ping Luo,Hongyuan Zhang*

Main category: cs.CV

TL;DR: The paper introduces the Layout Control (Laytrol) Network and the Layout Synthesis (LaySyn) dataset to improve spatial controllability in text-to-image generation, addressing issues like low visual quality and stylistic inconsistency.


<details>
  <summary>Details</summary>
Motivation: Enhancing spatial controllability in text-to-image generation is a key challenge, especially for layout-to-image tasks where existing methods often degrade visual quality and stylistic consistency.

Method: The proposed method involves the LaySyn dataset to mitigate distribution shift and the Laytrol Network, which inherits parameters from MM-DiT. A dedicated initialization scheme ensures stable control conditions.

Result: Qualitative and quantitative experiments show the method's effectiveness in generating spatially consistent and high-quality images.

Conclusion: The Laytrol Network and LaySyn dataset successfully address the challenges of layout-to-image generation, preserving pretrained knowledge and improving output quality.
...
### [21] [DiffRegCD: Integrated Registration and Change Detection with Diffusion Features](https://arxiv.org/abs/2511.07935)
*Seyedehnanita Madani,Rama Chellappa,Vishal M. Patel*

Main category: cs.CV

TL;DR: DiffRegCD unifies dense registration and change detection, using diffusion features and classification-based correspondence to outperform existing methods under challenging real-world conditions.


<details>
  <summary>Details</summary>
Motivation: Real-world imagery often suffers from parallax, viewpoint shifts, and long temporal gaps, causing severe misalignment. Existing methods struggle with large displacements and rely on limited techniques like regression-only flow or synthetic perturbations.

Method: DiffRegCD integrates dense registration and change detection into a single model, reformulating correspondence estimation as a Gaussian smoothed classification task. It leverages frozen multi-scale features from a pretrained denoising diffusion model and uses controlled affine perturbations for supervision.

Result: DiffRegCD consistently surpasses recent baselines on aerial and ground-level datasets (LEVIR-CD, DSIFN-CD, WHU-CD, SYSU-CD, VL-CMU-CD), demonstrating reliability under wide temporal and geometric variations.

Conclusion: DiffRegCD establishes diffusion features and classification-based correspondence as a robust foundation for unified change detection, outperforming recent baselines under wide temporal and geometric variations.
...
### [22] [Is It Truly Necessary to Process and Fit Minutes-Long Reference Videos for Personalized Talking Face Generation?](https://arxiv.org/abs/2511.07940)
*Rui-Qing Sun,Ang Li,Zhijing Wu,Tian Lan,Qianyu Lu,Xingshan Yao,Chen Xu,Xian-Ling Mao*

Main category: cs.CV

TL;DR: ISExplore identifies short, informative video segments for TFG, boosting efficiency by 5x while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Current TFG methods require lengthy reference videos, which are computationally expensive. The study questions this necessity and explores the potential of shorter, high-quality segments.

Method: The ISExplore strategy automatically selects a 5-second reference video segment based on audio feature diversity, lip movement amplitude, and number of camera views.

Result: ISExplore increases data processing and training speed by over 5x for NeRF and 3DGS methods without compromising output fidelity.

Conclusion: The ISExplore strategy significantly improves the efficiency of TFG methods by identifying and utilizing short, informative video segments, reducing computational burden while maintaining high-quality output.
...
### [23] [Libra-MIL: Multimodal Prototypes Stereoscopic Infused with Task-specific Language Priors for Few-shot Whole Slide Image Classification](https://arxiv.org/abs/2511.07941)
*Zhenfeng Zhuang,Fangyu Zhou,Liansheng Wang*

Main category: cs.CV

TL;DR: The paper proposes a Multimodal Prototype-based Multi-Instance Learning method to enhance LLM-based computational pathology by addressing bias in instance-level descriptions and improving cross-modal synergy through bidirectional interaction.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in the bias of LLM-generated instance-level descriptions for pathological tasks and the limitations of unidirectional guidance in existing vision-language MIL methods.

Method: The approach involves constructing task-specific pathological entity prototypes, using a frozen LLM for text prototypes, and employing the Stereoscopic Optimal Transport algorithm for fusion.

Result: Experiments on three cancer datasets show superior generalization and explainability of the proposed method.

Conclusion: The method effectively improves feature generalizability and interpretability in computational pathology by leveraging bidirectional multimodal interaction.
...
### [24] [ReIDMamba: Learning Discriminative Features with Visual State Space Model for Person Re-Identification](https://arxiv.org/abs/2511.07948)
*Hongyang Gu,Qisong Yang,Lei Pu,Siming Han,Yao Ding*

Main category: cs.CV

TL;DR: ReIDMamba, a Mamba-based framework for person re-identification, introduces multi-granularity features and ranking-aware triplet regularization to enhance discriminative and robust feature learning, achieving state-of-the-art performance with fewer parameters and faster inference.


<details>
  <summary>Details</summary>
Motivation: Transformer-based methods in person ReID face scalability issues due to quadratic memory and computational demands. ReIDMamba aims to address this by leveraging Mamba's efficiency while improving feature robustness.

Method: The framework includes a Mamba-based baseline with multiple class tokens, a multi-granularity feature extractor (MGFE) for fine-grained features, and ranking-aware triplet regularization (RATR) to reduce redundancy and enhance diversity.
...
Conclusion: ReIDMamba successfully integrates Mamba into ReID, offering a scalable, efficient, and high-performing solution for robust feature extraction.
...
### [25] [Burst Image Quality Assessment: A New Benchmark and Unified Framework for Multiple Downstream Tasks](https://arxiv.org/abs/2511.07958)
*Xiaoye Liang,Lai Jiang,Minglang Qiao,Yichen Guo,Yue Zhang,Xin Deng,Shengxi Li,Yufan Liu,Mai Xu*

Main category: cs.CV

TL;DR: Proposes BuIQA to assess burst image quality, introduces a benchmark dataset, and achieves superior performance in downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Redundancy in burst images increases storage and transmission demands while reducing downstream task efficiency, necessitating a quality assessment method.

Method: A task-driven prompt generation network with heterogeneous knowledge distillation and a task-aware quality assessment network are introduced.

Result: The approach outperforms state-of-the-art methods across 10 downstream scenarios and achieves a 0.33 dB PSNR improvement in denoising and super-resolution tasks.

Conclusion: The proposed Burst Image Quality Assessment (BuIQA) framework effectively addresses redundancy in burst images, improving downstream task efficiency and performance.
...
### [26] [Multi-Modal Assistance for Unsupervised Domain Adaptation on Point Cloud 3D Object Detection](https://arxiv.org/abs/2511.07966)
*Shenao Zhao,Pengpeng Liang,Zhoufan Yang*

Main category: cs.CV

TL;DR: MMAssist improves 3D UDA by using image and text features to align 3D features between domains, enhancing pseudo labels with 2D detection, and outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Little attention has been paid to the usefulness of image data in 3D UDA, despite the common practice of collecting point clouds and images together.

Method: Aligns 3D features using image and text features as bridges, fuses features for prediction, and enhances pseudo labels with 2D detection.

Result: Achieves promising performance in three domain adaptation tasks on popular 3D object detection datasets.

Conclusion: MMAssist demonstrates the effectiveness of multi-modal assistance in improving 3D UDA performance.
...
### [27] [Morphing Through Time: Diffusion-Based Bridging of Temporal Gaps for Robust Alignment in Change Detection](https://arxiv.org/abs/2511.07976)
*Seyedehanita Madani,Vishal M. Patel*

Main category: cs.CV

TL;DR: A modular pipeline improves robustness in remote sensing change detection by integrating diffusion-based morphing and flow refinement, outperforming existing methods on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Spatial misalignment in bi-temporal images due to seasonal or multi-year gaps limits the robustness of existing models, which rely on precise co-registration.

Method: The framework integrates diffusion-based semantic morphing, dense registration, and residual flow refinement, using a diffusion module to synthesize intermediate frames and a lightweight U-Net for flow refinement.

Result: Experiments on LEVIR-CD, WHU-CD, and DSIFN-CD show improved registration accuracy and change detection performance across various backbones.

Conclusion: The proposed modular pipeline enhances spatial and temporal robustness in remote sensing change detection without modifying existing networks, demonstrating consistent improvements in registration accuracy and change detection across multiple datasets.
...
### [28] [DANCE: Density-agnostic and Class-aware Network for Point Cloud Completion](https://arxiv.org/abs/2511.07978)
*Da-Yeong Kim,Yeong-Jun Cho*

Main category: cs.CV

TL;DR: DANCE is a novel framework for point cloud completion that handles variable sparsity and limited supervision, outperforming existing methods in accuracy and robustness.
...
Method: DANCE uses ray-based sampling to generate candidate points, refines their positions with a transformer decoder, and predicts opacity scores. It includes a lightweight classification head for semantic guidance without external image supervision.

Result: DANCE outperforms state-of-the-art methods on PCN and MVP benchmarks in accuracy and structural consistency, while being robust to varying input densities and noise.

Conclusion: DANCE provides a robust and accurate solution for point cloud completion, addressing limitations of existing methods and excelling in real-world scenarios.
...
TL;DR: Specialized vision-language models for fracture detection in chest X-rays outperform general-purpose models, improving accuracy in describing rare pathologies.


<details>
  <summary>Details</summary>
Motivation: General radiology report generation models often miss rare but clinically important pathologies like fractures, creating a need for specialized solutions.

Method: Specialized fracture-specific vision-language models were trained using encoders from MAIRA-2 and CheXagent.

Result: The models significantly improved accuracy in generating fracture descriptions compared to general-purpose models, with analysis revealing strengths and limitations by fracture type, location, and age.

Conclusion: The best-performing fracture-reporting model is publicly released to advance research in accurate reporting of rare pathologies.
...
TL;DR: A semantic-guided framework (CSF-Net) improves large-mask image inpainting by leveraging pretrained AC models for structure-aware candidates and fusing them with contextual features.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of large-mask image inpainting where essential content is missing and contextual cues are limited.

Method: Uses a pretrained Amodal Completion model to generate semantic priors and introduces CSF-Net, a transformer-based fusion framework, to integrate these priors with contextual features.

Result: CSF-Net enhances inpainting quality, reduces object hallucination, and improves visual realism and semantic alignment, as shown on Places365 and COCOA datasets.

Conclusion: CSF-Net is effective for large-mask inpainting, seamlessly integrates with existing models, and consistently improves performance.
...
### [34] [Invisible Triggers, Visible Threats! Road-Style Adversarial Creation Attack for Visual 3D Detection in Autonomous Driving](https://arxiv.org/abs/2511.08015)
...
### [36] [Multi-modal Deepfake Detection and Localization with FPN-Transformer](https://arxiv.org/abs/2511.08031)
...
### [39] [ProSona: Prompt-Guided Personalization for Multi-Expert Medical Image Segmentation](https://arxiv.org/abs/2511.08046)
...
### [42] [I2E: Real-Time Image-to-Event Conversion for High-Performance Spiking Neural Networks](https://arxiv.org/abs/2511.08065)
...
### [48] [LatentPrintFormer: A Hybrid CNN-Transformer with Spatial Attention for Latent Fingerprint identification](https://arxiv.org/abs/2511.08119)
...
### [55] [Multi-Granularity Mutual Refinement Network for Zero-Shot Learning](https://arxiv.org/abs/2511.08163)
...
### [62] [UCDSC: Open Set UnCertainty aware Deep Simplex Classifier for Medical Image Datasets](https://arxiv.org/abs/2511.08196)
...
### [63] [Twist and Compute: The Cost of Pose in 3D Generative Diffusion](https://arxiv.org/abs/2511.08203)
...
### [64] [Evaluating Gemini LLM in Food Image-Based Recipe and Nutrition Description with EfficientNet-B4 Visual Backbone](https://arxiv.org/abs/2511.08215)
*Rizal Khoirul Anam*
...
### [76] [SynWeather: Weather Observation Data Synthesis across Multiple Regions and Variables via a General Diffusion Transformer](https://arxiv.org/abs/2511.08291)
...
### [77] [SkelSplat: Robust Multi-view 3D Human Pose Estimation with Differentiable Gaussian Rendering](https://arxiv.org/abs/2511.08294)
...
### [78] [NeuSpring: Neural Spring Fields for Reconstruction and Simulation of Deformable Objects from Videos](https://arxiv.org/abs/2511.08310)
...
### [83] [VideoChain: A Transformer-Based Framework for Multi-hop Video Question Generation](https://arxiv.org/abs/2511.08348)
...
### [84] [Extreme Model Compression with Structured Sparsity at Low Precision](https://arxiv.org/abs/2511.08360)
...
### [85] [Retrospective motion correction in MRI using disentangled embeddings](https://arxiv.org/abs/2511.08365)
...
### [86] [A Circular Argument : Does RoPE need to be Equivariant for Vision?](https://arxiv.org/abs/2511.08368)
...
### [87] [Text-based Aerial-Ground Person Retrieval](https://arxiv.org/abs/2511.08369)
...
### [88] [RAPTR: Radar-based 3D Pose Estimation using Transformer](https://arxiv.org/abs/2511.08387)
...
### [89] [Anatomy-VLM: A Fine-grained Vision-Language Model for Medical Interpretation](https://arxiv.org/abs/2511.08402)
...
### [90] [OmniAID: Decoupling Semantic and Artifacts for Universal AI-Generated Image Detection in the Wild](https://arxiv.org/abs/2511.08423)
...
### [91] [Cross-pyramid consistency regularization for semi-supervised medical image segmentation](https://arxiv.org/abs/2511.08435)
...
### [92] [Contrastive Integrated Gradients: A Feature Attribution-Based Method for Explaining Whole Slide Image Classification](https://arxiv.org/abs/2511.08464)
...
### [93] [Generalizable Blood Cell Detection via Unified Dataset and Faster R-CNN](https://arxiv.org/abs/2511.08465)
*Siddharth Sahay*
...
### [94] [Compression then Matching: An Efficient Pre-training Paradigm for Multimodal Embedding](https://arxiv.org/abs/2511.08480)
...
### [95] [Fast Multi-Organ Fine Segmentation in CT Images with Hierarchical Sparse Sampling and Residual Transformer](https://arxiv.org/abs/2511.08509)
...
### [96] [CleverBirds: A Multiple-Choice Benchmark for Fine-grained Human Knowledge Tracing](https://arxiv.org/abs/2511.08512)
...
### [97] [UniVA: Universal Video Agent towards Open-Source Next-Generation Video Generalist](https://arxiv.org/abs/2511.08521)
...
### [98] [Large Sign Language Models: Toward 3D American Sign Language Translation](https://arxiv.org/abs/2511.08535)
...
### [99] [3D4D: An Interactive, Editable, 4D World Model via 3D Video Generation](https://arxiv.org/abs/2511.08536)
...
### [100] [RePose-NeRF: Robust Radiance Fields for Mesh Reconstruction under Noisy Camera Poses](https://arxiv.org/abs/2511.08545)
...
### [101] [Vision Transformer Based User Equipment Positioning](https://arxiv.org/abs/2511.08549)
...
### [102] [SENCA-st: Integrating Spatial Transcriptomics and Histopathology with Cross Attention Shared Encoder for Region Identification in Cancer Pathology](https://arxiv.org/abs/2511.08573)
