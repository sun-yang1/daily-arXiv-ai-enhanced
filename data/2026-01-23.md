...
TL;DR: Three methods enhance defect segmentation in culverts/sewer pipes: preprocessing techniques, a novel architecture (FORTRESS), and few-shot learning, addressing data scarcity while improving performance.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the critical role of culverts and sewer pipes in drainage systems, where failure poses risks to public safety and the environment. Data collection and annotation are challenging due to domain-specific requirements, making large datasets infeasible.

Method: The methods include preprocessing strategies (data augmentation and dynamic label injection), FORTRESS (a novel architecture combining depthwise separable convolutions, adaptive KAN, and multi-scale attention), and few-shot semantic segmentation using bidirectional prototypical networks with attention mechanisms.

Result: Preprocessing strategies improve segmentation performance (IoU and F1 score), FORTRESS achieves state-of-the-art results with reduced computational costs, and few-shot learning models achieve satisfactory metrics despite limited data.

Conclusion: The thesis successfully proposes three methods—preprocessing strategies, the FORTRESS architecture, and few-shot semantic segmentation—to improve defect segmentation in culverts and sewer pipes, addressing data scarcity challenges effectively.
...
TL;DR: MLLMs underperform classical face recognition systems in heterogeneous face recognition tasks, highlighting the need for rigorous evaluation.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the growing interest in MLLMs for biometric applications, particularly their potential in HFR tasks where enrollment and probe images come from different sensing modalities.

Method: The study systematically evaluates state-of-the-art MLLMs for HFR across various cross-modality scenarios (VIS-NIR, VIS-SWIR, VIS-THERMAL) using biometric protocols and metrics like Acquire Rate, EER, and TAR.

Result: Results show substantial performance gaps between MLLMs and traditional face recognition systems, especially in challenging cross-spectral conditions.

Conclusion: The paper concludes that current Multimodal Large Language Models (MLLMs) have significant performance gaps compared to classical face recognition systems in heterogeneous face recognition (HFR) tasks, especially under cross-spectral conditions. It emphasizes the need for rigorous biometric evaluation before deploying MLLMs in face recognition systems.
...
TL;DR: CURE enhances medical report generation by improving visual-textual alignment through error-aware curriculum learning.


<details>
  <summary>Details</summary>
Motivation: Existing medical vision-language models misalign textual findings with visual evidence, leading to unreliable or weakly grounded predictions.

Method: CURE fine-tunes a multimodal instructional model on phrase grounding, grounded report generation, and anatomy-grounded report generation using public datasets, dynamically adjusting sampling based on model performance.

Result: CURE improves grounding accuracy by +0.37 IoU, boosts report quality by +0.188 CXRFEScore, and reduces hallucinations by 18.6%.

Conclusion: CURE is a data-efficient framework that enhances grounding accuracy and report reliability in medical vision-language models.
...
TL;DR: DuFal enhances sparse-view CT reconstruction by combining frequency and spatial processing, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of recovering fine-grained anatomical details in sparse-view CT reconstructions, where conventional CNN-based methods struggle with high-frequency information.

Method: Integrates frequency-domain and spatial-domain processing via a dual-path architecture with High-Local Factorized Fourier Neural Operator and Spectral-Channel Factorization.

Result: Significant improvements on LUNA16 and ToothFairy datasets, especially in extremely sparse-view settings.

Conclusion: DuFal outperforms existing methods in preserving high-frequency anatomical features under sparse-view CT settings.
...
TL;DR: A deviation-guided prompt learning framework improves few-normal shot anomaly detection by integrating vision-language models with statistical deviation-based scoring.


<details>
  <summary>Details</summary>
Motivation: Addresses the weak discriminability of prompts and lack of principled scoring mechanisms in few-normal shot anomaly detection (FNSAD) using vision-language models like CLIP.

Method: Introduces a deviation-guided prompt learning framework with learnable context vectors, anomaly-specific suffix tokens, and a deviation loss with Top-K MIL for patch-level anomaly scoring.

Result: Superior pixel-level detection performance on MVTecAD and VISA benchmarks, validated through ablation studies.

Conclusion: The proposed deviation-guided prompt learning framework outperforms existing methods like PromptAD on benchmarks MVTecAD and VISA, demonstrating improved anomaly detection and localization.
...
TL;DR: A sensor-physics grounded NeRF framework improves HDR novel view synthesis from blurry LDR images by addressing mismatches and using event data.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to account for sensor-physics mismatches in LDR blurry images, leading to suboptimal HDR and deblurring results.

Method: A unified sensor-physics grounded NeRF framework is introduced, featuring HDR domain radiance representation, pixel-wise RGB mapping, and event mapping fields optimized jointly with NeRF.

Result: Experiments show superior deblurring and HDR novel view synthesis outcomes compared to existing approaches.

Conclusion: The proposed framework achieves state-of-the-art results in HDR novel view synthesis by addressing sensor-physics mismatches and leveraging event data for enhanced 3D representation.
...
TL;DR: Replacing U-Net with Vision Transformer reduces bias in chest X-ray classifiers without compromising diagnostic accuracy.


<details>
  <summary>Details</summary>
Motivation: Bias in chest X-ray classifiers due to sex- and age-related shortcuts leads to underdiagnosis of minority subgroups, prompting the need for better attribute neutralizers.

Method: The study trained a DeiT-S neutralizer on the ChestX-ray14 dataset, evaluating edited images across eleven edit-intensity levels using an independent AI judge for attribute leakage and a ConvNet for disease prediction.

Result: At moderate edit levels, the ViT neutralizer reduces sex-recognition AUC by ~10 points compared to the original U-Net encoder while keeping diagnostic accuracy within five percentage points of the baseline.

Conclusion: The study demonstrates that substituting a Vision Transformer backbone in the Attribute-Neutral Framework can reduce demographic attribute leakage while maintaining diagnostic accuracy, offering a practical approach to fairer chest X-ray AI.
...
TL;DR: LASAGNA introduces a framework for generating images with composable layers, offering better controllability and realism than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing image generation models lack controllability and consistency when editing specific elements, and layered representations often fail to maintain coherent compositing or realistic visual effects.

Method: LASAGNA proposes a unified framework for generating images with composable layers—photorealistic backgrounds and transparent foregrounds—learned from diverse conditioning inputs (text prompts, masks, etc.).

Result: LASAGNA outperforms prior work by generating high-quality, consistent layers with realistic effects, supported by the new LASAGNA-48K dataset and LASAGNABENCH benchmark.

Conclusion: LASAGNA demonstrates significant improvements in generating consistent and coherent image layers, enabling diverse post-editing applications while preserving visual effects. The release of LASAGNA-48K and LASAGNABENCH aims to foster open research.
...
TL;DR: Novel dual-stream delta encoder improves hand pose estimation in occluded scenarios by leveraging dorsal skin deformation, reducing errors by 18%.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of frequent finger occlusions in egocentric hand pose estimation, which is vital due to the proliferation of XR devices.

Method: A dual-stream delta encoder that contrasts features from a dynamic hand with a baseline relaxed position, leveraging dorsal hand skin deformation.

Result: Reduces Mean Per Joint Angle Error (MPJAE) by 18% in self-occluded scenarios compared to state-of-the-art techniques, improving reliability in downstream tasks.

Conclusion: The proposed method enhances hand pose estimation in occluded scenarios and enables new interaction paradigms, such as detecting isometric force, while minimizing model size.
...
TL;DR: VIOLA adapts MLLMs to novel video domains efficiently by combining minimal expert supervision with unlabeled data, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of adapting MLLMs to specialized video domains with scarce labeled data, avoiding reliance on large annotated pools.

Method: The framework combines density-uncertainty-weighted sampling for efficient annotation and confidence-aware retrieval and prompting to utilize unlabeled data without noise.

Result: VIOLA achieves robust adaptation across nine benchmarks using four MLLMs, significantly outperforming baselines with minimal annotation costs.

Conclusion: VIOLA effectively bridges the gap in adapting MLLMs to novel video domains with minimal expert supervision, outperforming baselines in low-resource settings.
...
TL;DR: DDPMs for K-pop faces show high visual quality but poor semantic controllability, with a new metric (RCA) revealing trade-offs.


<details>
  <summary>Details</summary>
Motivation: Evaluate the semantic controllability of Denoising Diffusion Probabilistic Models (DDPMs) for fine-grained, single-domain tasks, where standard metrics like FID and Inception Score often fail.

Method: The study investigates Class-Conditional DDPMs for K-pop idol face generation, proposing a calibrated metric (Relative Classification Accuracy) to evaluate generative performance against an oracle classifier's baseline.

Result: The model achieves high visual quality (FID 8.93) but suffers from severe semantic mode collapse (RCA 0.27), attributed to resolution constraints and intra-gender ambiguity.

Conclusion: The framework provides a rigorous standard for verifying identity consistency in conditional generative models, highlighting the trade-off between visual quality and semantic controllability.
...
TL;DR: RSM-CoDG improves cross-subject EEG emotion recognition by combining region-aware spatial modeling, multi-scale temporal analysis, and collaborative domain generalization, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Cross-subject EEG-based emotion recognition is challenged by inter-subject variability and high complexity of neural representations. Existing approaches lack unified frameworks for spatial, temporal, and generalization alignment.

Method: A Region-aware Spatiotemporal Modeling framework with Collaborative Domain Generalization (RSM-CoDG) is proposed, incorporating neuroscience priors for spatial representations, multi-scale temporal modeling, and collaborative domain generalization to reduce subject-specific bias.

Result: Extensive experiments on SEED datasets show RSM-CoDG's superior performance over competing methods.

Conclusion: : RSM-CoDG consistently outperforms existing methods in cross-subject EEG-based emotion recognition, demonstrating improved robustness and generalization to unknown individuals.
...
TL;DR: Proposes an automated CoT data generation and RL-enhanced framework for deepfake detection, addressing annotation challenges and achieving SOTA-competitive results.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the lack of explainable outputs in deepfake detection methods and the challenges of high-quality dataset annotation. The study also explores the potential of reinforcement learning (RL) to enhance performance and cross-domain generalization in this task.

Method: The authors introduce an automated Chain-of-Thought (CoT) data generation framework based on Self-Blended Images and an RL-enhanced deepfake detection framework, including a tailored reward mechanism and feedback-driven synthetic data generation.

Result: Extensive experiments show the effectiveness of the CoT data construction pipeline, reward mechanism, and synthetic data generation. The method achieves competitive performance with SOTA approaches across multiple cross-dataset benchmarks.

Conclusion: The proposed method demonstrates competitive performance with SOTA approaches in deepfake detection, leveraging automated CoT data generation and RL enhancement to address annotation challenges and improve cross-domain generalization.
...
TL;DR: The paper introduces a continual panoptic perception model for multimodal, multi-task learning, using cross-modal encoders and knowledge distillation to combat forgetting and alignment issues, showing strong results in fine-grained tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to extend continual learning to multimodal and multi-task scenarios, addressing issues like catastrophic forgetting and semantic obfuscation in incremental training.

Method: The method involves an end-to-end continual panoptic perception model featuring a collaborative cross-modal encoder, contrastive feature distillation, instance distillation, cross-modal consistency constraints, and asymmetric pseudo-labeling.

Result: Extensive experiments show the proposed model's superiority, especially in fine-grained continual learning tasks, without needing exemplar replay.

Conclusion: The paper concludes that the proposed CPP model, with its collaborative cross-modal encoder and knowledge inheritance module, effectively addresses catastrophic forgetting and semantic obfuscation in multi-task continual learning, demonstrating superior performance in fine-grained tasks.
...
TL;DR: SuperOcc introduces a superquadric-based 3D occupancy prediction framework with enhanced temporal modeling, geometric expressiveness, and efficiency, achieving top results on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing methods overlook sparsity in real-world driving scenes and suffer from insufficient temporal modeling, trade-offs between sparsity and expressiveness, and inefficient splatting.

Method: SuperOcc incorporates cohesive temporal modeling, multi-superquadric decoding, and efficient superquadric-to-voxel splatting.

Result: SuperOcc outperforms on SurroundOcc and Occ3D benchmarks, demonstrating improved performance and efficiency.

Conclusion: SuperOcc achieves state-of-the-art performance in 3D occupancy prediction while maintaining superior efficiency, addressing key limitations of existing superquadric frameworks.
...
TL;DR: Event-VStream enhances real-time video understanding by detecting events and using memory, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: To address challenges in VLMs like redundant frame processing and rapid forgetting of past context in long video streams.

Method: Event-VStream integrates motion, semantic, and predictive cues to detect state transitions, triggering language generation at event boundaries and consolidating embeddings into a memory bank.

Result: Improves by +10.4 points on OVOBench-Realtime, matches Flash-VStream-7B performance with a general-purpose backbone, and maintains ~70% GPT-5 win rate on 2-hour streams.

Conclusion: Event-VStream demonstrates competitive performance in real-time long video understanding, outperforming baselines and maintaining efficiency.
...
TL;DR: Skywork UniPic 3.0 excels in multi-image composition, surpassing competitors with innovative training and efficient inference.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in multi-image composition, particularly for Human-Object Interaction (HOI) tasks, by improving consistency and quality.

Method: A unified multimodal framework integrating single-image editing and multi-image composition, with a novel training paradigm and efficient inference techniques.

Result: Achieves state-of-the-art performance in benchmarks, with efficient inference (12.5x speedup) and high-quality output.

Conclusion: Skywork UniPic 3.0 outperforms Nano-Banana and Seedream 4.0 in multi-image composition tasks, validated by benchmarks, and its resources are publicly available.
...
TL;DR: Cr-GAN synthesizes diverse SAR samples with limited data, boosting few-shot recognition accuracy and outperforming baselines with minimal parameters.


<details>
  <summary>Details</summary>
Motivation: Few-shot recognition in SAR imagery is hindered by extreme data scarcity, and conventional GANs require abundant data, creating a paradox. Cr-GAN resolves this by enabling high-fidelity sample synthesis with limited data.

Method: The proposed Cr-GAN introduces a dual-branch discriminator, channel-wise feature interpolation, and dual-domain cycle consistency to enable stable GAN training under severe data limitations.

Result: Cr-GAN achieves highly competitive accuracies of 71.21% and 51.64% on MSTAR and SRSDD datasets in the 8-shot setting, outperforming baselines while using fewer parameters.

Conclusion: The Cr-GAN framework effectively addresses the data scarcity challenge in few-shot SAR recognition by synthesizing diverse, high-fidelity samples, significantly outperforming baselines with minimal parameters.
...
TL;DR: MGRAL introduces a mAP-guided reinforced active learning method for object detection, outperforming existing approaches on standard benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current AL strategies for object detection do not directly correlate data informativeness with downstream task performance like mAP, leading to suboptimal results.

Method: MGRAL employs a reinforcement learning-based sampling agent that optimizes selection using policy gradient with mAP improvement as reward, and uses unsupervised fast look-up tables for mAP estimation.

Result: MGRAL achieves the highest AL curve with convincing visualizations on PASCAL VOC and COCO benchmarks.

Conclusion: MGRAL establishes a new paradigm in reinforcement learning-driven active object detection, demonstrating superior performance on PASCAL VOC and COCO benchmarks.
...
TL;DR: Improved LULC segmentation and water detection using SAR data via lightweight refinements, addressing common SAR issues without added complexity.


<details>
  <summary>Details</summary>
Motivation: To address common SAR dense-prediction issues like boundary over-smoothing, missed thin structures, and rare-class degradation under long-tailed labels.

Method: Three lightweight refinements are introduced: (i) high-resolution feature injection, (ii) a progressive refine-up head, and (iii) an α-scale factor for class reweighting.

Result: The model shows consistent improvements on the Japan-wide ALOS-2 LULC benchmark, particularly for under-represented classes, and enhances water detection metrics.

Conclusion: The proposed model improves national-scale LULC semantic segmentation and water detection, especially for under-represented classes, without increasing pipeline complexity.
...
TL;DR: VLMs outperform traditional methods in fashion attribute prediction but struggle with attribute applicability detection. Efficient models offer cost-effective deployment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the underexplored systematic evaluation of VLMs on multi-attribute fashion tasks, particularly focusing on conditional attributes like "outer fabric" which may not always be applicable.

Method: The paper introduces a three-tier evaluation framework to decompose the challenge of conditional fashion attributes: (1) overall task performance, (2) attribute applicability detection, and (3) fine-grained classification. It benchmarks nine VLMs against classifiers trained on Fashion-CLIP embeddings.

Result: Key findings include: (1) zero-shot VLMs achieve 64.0% macro-F1, (2) VLMs excel in fine-grained classification (70.8% F1) but struggle with applicability detection (34.1% NA-F1), and (3) efficient models achieve over 90% of flagship performance at lower cost.

Conclusion: The paper concludes that while VLMs significantly outperform traditional methods in fine-grained classification, their performance in attribute applicability detection remains a bottleneck. Efficient models offer a practical deployment solution, achieving near-flagship performance at lower costs.
...
TL;DR: VideoThinker, a Video Large Language Model, uses synthetic tool interaction trajectories to enhance long-form video understanding, outperforming existing models by leveraging adaptive temporal and spatial exploration.


<details>
  <summary>Details</summary>
Motivation: Current models for long-form video understanding rely on static reasoning, leading to information loss and weak temporal localization. Agentic tools like retrieval and zoom can help but require strong comprehension, creating a circular dependency.

Method: VideoThinker converts videos into captions, uses an agentic language model to generate tool use sequences in caption space, and grounds these back to video frames, creating a synthetic dataset.

Result: VideoThinker outperforms caption-only agents and video baselines in long-video benchmarks, showing the effectiveness of synthetic tool-augmented data.

Conclusion: VideoThinker demonstrates that synthetic agentic data and adaptive reasoning tools significantly improve long-form video understanding.
...
TL;DR: FAIR-ESI is a novel framework for electrophysiological source imaging (ESI) that adaptively refines features across spectral, temporal, and patch-wise views, improving brain disorder diagnosis.


<details>
  <summary>Details</summary>
Motivation: Accurate feature selection and refinement are critical for precise ESI, but existing methods (model-based optimization and deep learning) face challenges in this area.

Method: FAIR-ESI refines features using FFT-based spectral refinement, weighted temporal refinement, and self-attention-based patch-wise refinement.

Result: Experiments on simulation and clinical datasets show FAIR-ESI's effectiveness in advancing brain disorder diagnosis.

Conclusion: FAIR-ESI offers a promising approach for ESI, with potential to enhance brain disorder diagnosis and provide new insights into brain function.
...
TL;DR: ARDIS is a novel framework for deep image steganography that allows arbitrary resolution secret images to be hidden and recovered without detail loss, outperforming current methods.


<details>
  <summary>Details</summary>
Motivation: Current deep image steganography methods require secret and cover images to have the same resolution, leading to detail loss and inability to recover original resolution.

Method: ARDIS uses a Frequency Decoupling Architecture for hiding and a Latent-Guided Implicit Reconstructor for recovery, along with Implicit Resolution Coding for blind recovery.
...
Conclusion: ARDIS successfully addresses resolution mismatch issues in deep image steganography, enabling faithful recovery of arbitrary resolution secret images.
...
TL;DR: ES-mHC enhances interpretability in HSIC by modeling spectrum interactions with structured matrices, revealing internal dynamics and spatial patterns.


<details>
  <summary>Details</summary>
Motivation: To address the opacity and lack of interpretability in deep learning models for hyperspectral image classification (HSIC), which limits understanding of internal decision mechanisms.

Method: The paper introduces ES-mHC, a hyper-connection framework that models interactions among electromagnetic spectrum groupings using structured, directional matrices, separating feature representation from interaction structure.

Result: The learned hyper-connection matrices show coherent spatial patterns and asymmetric interactions, and increasing the expansion rate accelerates structured interaction patterns.

Conclusion: ES-mHC transforms HSIC into a structurally transparent, partially white-box learning process by exposing internal information flow and providing mechanistic insights.
...
TL;DR: FeTal-SAM adapts SAM for fetal brain MRI segmentation, enabling flexible, atlas-based prompts to avoid retraining, with strong performance for well-contrasted structures.


<details>
  <summary>Details</summary>
Motivation: Traditional deep learning methods require large annotated datasets for fixed labels, making them inflexible when needs change. FeTal-SAM aims to address this by allowing adaptable segmentation without retraining and providing insight into segmentation drivers.

Method: The method integrates atlas-based prompts and foundation-model principles. Multi-atlas registration generates spatially aligned label templates as dense prompts, alongside a bounding-box prompt, for SAM's segmentation decoder. This enables binary segmentation per structure, which is fused to reconstruct full 3D segmentation volumes.

Result: FeTal-SAM achieves robust performance across gestational ages, with Dice scores comparable to state-of-the-art baselines for well-contrasted structures. It maintains flexibility for user-specified anatomy but shows slightly lower accuracy for low-contrast structures.

Conclusion: FeTal-SAM presents a promising advancement in fetal brain MRI segmentation by addressing the inflexibility of traditional deep learning methods. It leverages atlas-based prompts and foundation-model principles to enable adaptable segmentation without exhaustive retraining, despite slightly lower accuracy for subtle structures.
...
TL;DR: LL-GaussianMap introduces an unsupervised framework using 2DGS for low-light image enhancement, preserving edges and reducing artifacts without paired data, achieving superior results with minimal storage.


<details>
  <summary>Details</summary>
Motivation: Existing low-light image enhancement methods neglect intrinsic geometric structural priors. The paper aims to bridge the gap by incorporating 2DGS, known for its structural fitting capabilities, into unsupervised low-light enhancement.

Method: The method involves two stages: high-fidelity structural reconstruction using 2DGS and data-driven enhancement dictionary coefficients rendered via Gaussian splatting. It formulates enhancement as a gain map generation process guided by 2DGS primitives.

Result: LL-GaussianMap outperforms conventional methods, achieving high-quality enhancement with low storage footprint, validated through experiments.

Conclusion: LL-GaussianMap demonstrates the effectiveness of explicit Gaussian representations (2DGS) in low-light image enhancement, achieving superior performance with minimal storage requirements while preserving edges and suppressing artifacts.
...
TL;DR: LL-GaussianImage is a zero-shot unsupervised framework for low-light enhancement directly within the 2DGS compressed domain, avoiding decompression-recompression pipelines. It uses semantic-guided Mixture-of-Experts, multi-objective loss, and two-stage optimization for high-quality results.


<details>
  <summary>Details</summary>
Motivation: Existing low-light enhancement methods require decompression of 2DGS-compressed images, leading to inefficiency and degradation. LL-GaussianImage aims to enhance images directly in the compressed domain.

Method: The framework employs semantic-guided Mixture-of-Experts for dynamic transformations, a multi-objective loss system for smoothness and fidelity, and a two-stage optimization process.

Result: LL-GaussianImage achieves high-quality low-light enhancement while maintaining compression ratios, validated by experiments.

Conclusion: The framework successfully demonstrates the feasibility and superiority of direct processing in the compressed representation domain.
