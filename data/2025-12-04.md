...
TL;DR: A self-supervised symbolic auto-encoder for diagram understanding that encodes diagrams into geometric primitives and their relationships, using hierarchical rewards and stabilization mechanisms to achieve state-of-the-art performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Symbolic computer vision requires different learning paradigms from pixel-based models to achieve interpretable understanding through logical rules and structured representations.

Method: Proposes a symbolic auto-encoder with hierarchical process reward modeling for step-level parsing consistency and stabilization mechanisms to balance exploration-exploitation in reinforcement learning.

Result: Achieves 98.2% MSE reduction in geometric reconstruction, surpasses GPT-4o by 0.6% with 7B model on chart reconstruction, +13% on MathGlance perception, and +3% on MathVerse/GeoQA reasoning benchmarks.

Conclusion: The neuro-symbolic system effectively integrates neural reasoning with symbolic interpretability, demonstrating superior performance across reconstruction, perception, and reasoning tasks.
...
TL;DR: Proposes a drainage node framework for handling noisy labels, ambiguous classes, and outlier rejection in deep learning, achieving state-of-the-art results on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Address challenges of noisy labels, class ambiguity, and need to reject out-of-distribution/corrupted samples in modern deep learning.

Method: Adds a drainage node at network output to reallocate probability mass toward uncertainty while maintaining end-to-end training and differentiability.

Result: Achieves up to 9% accuracy improvement in high-noise regimes on CIFAR-10/100 with instance-dependent/asymmetric noise; matches/surpasses SOTA on real-world datasets.

Conclusion: Drainage node provides effective uncertainty handling, denoising, and stable decision boundaries, with broader applications beyond classification.
...
TL;DR: AI-based facial image restoration (3D reconstruction, 2D frontalization, feature enhancement) generally degrades recognition accuracy, but selective combination of CFR-GAN and CodeFormer improves it.


<details>
  <summary>Details</summary>
Motivation: Biometric facial recognition models perform poorly on real-world images with quality issues, non-frontal poses, and occlusions.

Method: Used a model-agnostic forensic pipeline to test three restoration approaches: NextFace (3D reconstruction), CFR-GAN (2D frontalization), and CodeFormer (feature enhancement).

Result: Naive application of restoration techniques degrades accuracy; selective use of CFR-GAN with CodeFormer yields meaningful improvements.

Conclusion: Targeted, selective restoration (combining frontalization and enhancement) can mitigate real-world challenges in facial recognition, but broad application is counterproductive.
...
TL;DR: Flux4D: Unsupervised 4D reconstruction framework using 3D Gaussians for dynamic scenes, enabling fast, scalable learning without annotations


<details>
  <summary>Details</summary>
Motivation: Address limitations of existing differentiable rendering methods (NeRF, 3DGS) which require motion annotations and suffer from scalability issues

Method: Predicts 3D Gaussians and motion dynamics using only photometric losses and 'as static as possible' regularization, trained across multiple scenes

Result: Outperforms existing methods on outdoor driving datasets in scalability, generalization, and reconstruction quality

Conclusion: Flux4D provides efficient, annotation-free dynamic scene reconstruction that generalizes well to unseen environments
...
TL;DR: This paper explores using multimodal LLMs (GPT-4o and GPT-5) for zero-shot object counting with only text prompts, achieving results competitive with state-of-the-art methods on standard datasets.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot counting methods require annotated data or visual exemplars. LLMs possess strong reasoning abilities, suggesting potential for unsupervised counting.

Method: Leverage the visual capabilities of GPT-4o and GPT-5 to perform object counting using only textual prompts in a zero-shot manner.

Result: Evaluation on FSC-147 and CARPK datasets shows performance comparable to state-of-the-art zero-shot methods, sometimes surpassing them.

Conclusion: Multimodal LLMs can effectively perform zero-shot object counting using text prompts alone, offering a promising unsupervised alternative to existing methods.
...
TL;DR: LLM-based method for inferring material composition from 3D point clouds in zero-shot manner, achieving high plausibility on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Existing 3D shape datasets and models focus only on geometry, ignoring material properties that determine object appearance.

Method: Two-stage LLM approach: first predicts object semantics, then assigns materials to geometric segments based on inferred semantics, both stages zero-shot without task-specific training.

Result: Achieves high semantic and material plausibility across 1,000 shapes from Fusion/ABS and ShapeNet, evaluated using LLM-as-a-Judge in DeepEval.
...
TL;DR: A noise synthesis method using single noisy/dark images per ISO setting that achieves state-of-the-art denoising performance without large datasets.


<details>
  <summary>Details</summary>
Motivation: Low-light images are very noisy but learning-based denoisers require large paired datasets which are difficult to collect.

Method: Combines Poisson distribution for signal-dependent noise with Fourier-domain spectral sampling for signal-independent noise, requiring only one noisy image and one dark frame per ISO.

Result: Generates realistic noise while maintaining spatial/statistical properties, leading to SOTA performance on multiple benchmarks.

Conclusion: Provides practical, accurate noise synthesis without parametric models or large datasets, enabling effective low-light denoising.
...
TL;DR: PixPerfect is a pixel-level refinement framework that solves pixel inconsistencies in Latent Diffusion Models for local image editing, using discriminative pixel space, artifact simulation, and direct refinement to achieve seamless results across diverse tasks.


<details>
  <summary>Details</summary>
Motivation: Latent Diffusion Models suffer from pixel-level inconsistencies like chromatic shifts and texture mismatches in local editing tasks. Existing solutions fail to fully eliminate artifacts and lack generalization across different latent representations.

Method: PixPerfect employs three key components: (1) differentiable discriminative pixel space for amplifying/suppressing color-texture discrepancies, (2) comprehensive artifact simulation pipeline for realistic training exposure, and (3) direct pixel-space refinement scheme for broad applicability.

Result: Extensive experiments on inpainting, object removal, and insertion benchmarks show PixPerfect significantly improves perceptual fidelity and editing performance.

Conclusion: PixPerfect establishes a new standard for robust and high-fidelity localized image editing, demonstrating effectiveness across diverse LDM architectures and tasks.
...
TL;DR: Evaluation of deep learning models for wildfire classification and fire radiative power estimation using MASTER sensor data, proposing a two-stage pipeline called PyroFocus for real-time edge deployment.


<details>
  <summary>Details</summary>
Motivation: Increasing wildfire frequency and severity requires low-latency, computationally efficient onboard detection methods that can distinguish fire conditions and estimate intensity using multispectral/hyperspectral thermal imagers despite high data dimensionality and limited resources.

Method: Systematic evaluation of deep learning architectures (CNNs and Transformers) for multi-class fire classification, plus PyroFocus - a two-stage pipeline performing classification followed by FRP regression/segmentation to reduce inference time and computational cost.

Result: The two-stage pipeline achieves strong trade-offs between speed and accuracy, enabling efficient onboard deployment.

Conclusion: PyroFocus demonstrates significant potential for real-time edge deployment in future wildfire monitoring missions due to its optimized balance of performance and computational efficiency.
...
TL;DR: H²U3D is a large-scale 3D VQA dataset for house-level scenes, and SpatialReasoner is an active perception framework that achieves SOTA performance with efficient image usage.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models struggle with spatial reasoning in large-scale environments (beyond room-scale).

Method: Proposes SpatialReasoner framework with two-stage training: supervised cold start followed by RL with adaptive exploration rewards. Uses automated annotation pipeline for hierarchical visual representations.

Result: SpatialReasoner outperforms GPT-4o and Gemini-2.5-Pro on H²U3D, achieving superior results with only 3-4 images versus 16+ for baselines.

Conclusion: The coarse-to-fine active exploration paradigm enables effective large-scale 3D spatial reasoning with high efficiency.
...
TL;DR: NavMapFusion is a diffusion-based framework that fuses coarse navigation maps with real-time sensor data to create accurate HD maps for autonomous driving, showing 21.4% improvement on nuScenes benchmark.


<details>
  <summary>Details</summary>
Motivation: HD maps are essential but static, while the real world changes constantly. Available navigation maps are too low-resolution for direct use but can serve as priors for online map construction.

Method: Uses diffusion model that iteratively denoises map representations conditioned on both high-fidelity sensor data and low-fidelity navigation maps. Discrepancies between prior maps and sensor data are treated as noise in the diffusion process.

Result: 21.4% relative improvement on 100m perception range on nuScenes benchmark when using OpenStreetMap data as prior. Shows even stronger improvements at larger ranges while maintaining real-time performance.

Conclusion: Diffusion-based map fusion provides robust framework for creating accurate, up-to-date environment representations by combining low-fidelity priors with high-fidelity sensor data, enabling safer autonomous driving.
...
TL;DR: Proposes Step-by-Step Layered Design Generation using SLEDGE model for iterative design refinement with atomic changes guided by instructions, introducing new benchmark.


<details>
  <summary>Details</summary>
Motivation: Traditional design generation approaches treat synthesis as single-step, underestimating complexity of creative process requiring progressive refinement.

Method: SLEDGE uses multi-modal LLMs to model design updates as atomic layered changes grounded in sequential instructions.

Result: Exhaustive experiments show efficacy compared to state-of-the-art approaches in new benchmark setup.

Conclusion: Introduces pragmatic research direction for layered design generation, hoping to attract community attention.
...
TL;DR: ProtoEFNet introduces interpretable prototype learning for continuous ejection fraction prediction using cardiac ultrasound videos, achieving comparable accuracy to black-box models with clinically meaningful explanations.


<details>
  <summary>Details</summary>
Motivation: Traditional EF estimation requires manual expertise and current deep learning models lack transparency, reducing clinical trust. Post-hoc explanations don't guide internal reasoning.

Method: Video-based prototype learning model with dynamic spatiotemporal prototypes and Prototype Angular Separation (PAS) loss for discriminative representations across EF spectrum.

Result: Achieves accuracy comparable to non-interpretable models on EchonetDynamic dataset, with PAS loss improving F1 score from 77.67±2.68 to 79.64±2.10.

Conclusion: ProtoEFNet provides accurate EF prediction with clinically relevant interpretability, addressing transparency limitations of current deep learning approaches.
...
TL;DR: HalluGen is a diffusion-based framework that generates controllable hallucinations in safety-critical image restoration tasks, creating a large annotated dataset for systematic evaluation of hallucination detection and mitigation methods.


<details>
  <summary>Details</summary>
Motivation: Generative models in image restoration produce hallucinations that are problematic in safety-critical domains like medical imaging, but evaluating these hallucinations is challenging due to costly and subjective labeling requirements.

Method: The paper introduces HalluGen, a diffusion-based framework that synthesizes realistic hallucinations with controllable type, location, and severity to create perceptually realistic but semantically incorrect outputs.

Result: HalluGen produced a dataset of 4,350 annotated images from 1,450 brain MR images, demonstrated through applications including benchmarking metrics (developing SHAFE) and training hallucination detectors that generalize to real restoration failures.

Conclusion: HalluGen and its open dataset provide the first scalable foundation for evaluating hallucinations in safety-critical image restoration, addressing the circular dependency problem in hallucination assessment.
...
TL;DR: Hierarchical attention models outperform 2D/3D CNNs and ViTs for detecting subtle anomalies in 3D medical imaging, offering 21-23% higher sensitivity/specificity by aligning receptive fields with intermediate spatial scales of subclinical abnormalities.


<details>
  <summary>Details</summary>
Motivation: Detecting weak, spatially distributed anomalies in volumetric medical imaging is challenging because 2D/3D CNNs impose excessive locality while ViTs diffuse global attention too broadly, leaving optimal inductive biases for sparse pattern recognition unresolved.

Method: Comparative analysis of 16 deep learning architectures (2D/3D CNNs, hybrids, ViTs) for subclinical keratoconus detection from 3D OCT volumes, using mechanistic analyses like attention-distance measurements and representational similarity.

Result: Hierarchical attention models achieve 21-23% higher sensitivity and specificity in sparse anomaly detection, with mechanistic evidence showing their receptive fields align better with intermediate spatial scales of subclinical abnormalities.

Conclusion: Hierarchical attention provides a principled, parameter-efficient inductive bias for early pathological change detection in 3D medical imaging, establishing design guidance for future volumetric anomaly detection systems.
...
TL;DR: SeeU learns continuous 4D dynamics from 2D images/videos to generate unseen visual content through a 2D→4D→2D framework.


<details>
  <summary>Details</summary>
Motivation: Current visual understanding operates on 2D projections, which is suboptimal. Modeling the continuous 4D world (3D space + time) can improve performance.

Method: Reconstruct 4D from 2D frames, learn continuous dynamics with low-rank representation and physical constraints, then reproject to 2D for generation.

Result: Achieves continuous and physically-consistent novel visual generation for tasks like unseen temporal/spatial generation and video editing.

Conclusion: Modeling in 4D enables superior visual content generation compared to direct 2D approaches.
...
TL;DR: Deep learning system for lung cancer classification from CT scans achieves 98% accuracy using DenseNet169 and SVM models, with enhanced interpretability through Grad-CAM and SHAP.


<details>
  <summary>Details</summary>
Motivation: Manual CT scan interpretation for lung cancer diagnosis is time-consuming and error-prone, necessitating an automated deep learning solution.

Method: Used DenseNet169 with SE blocks and FPN for feature extraction, Focal Loss for handling class imbalance, and MobileNetV2 with SVM for classification. Integrated Grad-CAM and SHAP for interpretability.

Result: Both DenseNet169 and SVM models achieved 98% accuracy on the IQOTHNCCD dataset with Normal, Benign, Malignant classes.

Conclusion: The system demonstrates high accuracy and interpretability, showing potential for real-world clinical use in lung cancer diagnosis.
...
TL;DR: FireSentry introduces a high-resolution wildfire dataset and FiReDiff model for precise fire spread prediction using infrared video forecasting and mask segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing wildfire prediction methods use coarse data that limits localized modeling; need for fine-grained prediction to improve emergency response.

Method: Created FireSentry dataset with sub-meter/second UAV data, then developed FiReDiff which predicts infrared video sequences first, then segments fire masks.

Result: FiReDiff achieved SOTA gains vs generative models: 39.2% PSNR, 36.1% SSIM, 50.0% LPIPS, 29.4% FVD for video; 3.3% AUPRC, 59.1% F1, 42.9% IoU, 62.5% MSE for masks.

Conclusion: FireSentry dataset and FiReDiff paradigm significantly advance fine-grained wildfire forecasting and dynamic disaster simulation capabilities.
...
TL;DR: ShelfGaussian is an open-vocabulary 3D scene understanding framework using Gaussian representations supervised by vision foundation models, achieving state-of-the-art zero-shot performance on semantic occupancy prediction.


<details>
  <summary>Details</summary>
Motivation: Current Gaussian-based methods either use closed-set semantic supervision (ignoring rendering ability) or open-set 2D self-supervision (leading to degraded geometry and camera-only limitations). The authors aim to fully exploit Gaussian potential by combining multi-modal sensing and joint 2D-3D supervision.

Method: Proposes Multi-Modal Gaussian Transformer allowing Gaussians to query features from diverse sensors, and Shelf-Supervised Learning Paradigm that optimizes Gaussians with VFM features jointly at both 2D image and 3D scene levels.

Result: Achieves state-of-the-art zero-shot semantic occupancy prediction performance on Occ3D-nuScenes benchmark and demonstrates strong in-the-wild performance on unmanned ground vehicles across diverse urban scenarios.

Conclusion: ShelfGaussian successfully bridges the gap between closed-set and open-set Gaussian methods, demonstrating superior open-vocabulary 3D scene understanding capabilities through effective multi-modal integration and foundation model supervision.
...
TL;DR: MOS framework bridges optical-SAR modality gap for ship re-identification using consistent representation learning and cross-modal data generation.


<details>
  <summary>Details</summary>
Motivation: Modality gap between optical and SAR imagery hinders accurate cross-modal ship re-identification, which is crucial for maritime surveillance.

Method: Proposes MOS with two components: Modality-Consistent Representation Learning (denoising SAR images + class-wise alignment loss) and Cross-modal Data Generation and Feature fusion (using diffusion model to synthesize samples).

Result: Achieves state-of-the-art performance on HOSS dataset with R1 accuracy improvements of +3.0% (ALL to ALL), +6.2% (Optical to SAR), and +16.4% (SAR to Optical).

Conclusion: MOS effectively mitigates modality gap and enables robust cross-modal ship re-identification, with code and models to be released.
...
TL;DR: The paper introduces Video Difference Captioning (ViDiC), a new task and benchmark (ViDiC-1K) for evaluating Multimodal Large Language Models' ability to describe fine-grained similarities and differences between video pairs.


<details>
  <summary>Details</summary>
Motivation: Existing Image Difference Captioning approaches fail to capture temporal changes like motion continuity and event evolution in dynamic scenes. There's a need to evaluate models on comparative perception of video pairs.

Method: Created ViDiC-1K dataset with 1,000 curated video pairs annotated with 4,000+ checklist items across 7 categories. Proposed a dual-checklist evaluation framework using LLM-as-a-Judge to separately measure similarity and difference accuracy.

Result: Experiments on 19 multimodal models revealed significant performance gaps in comparative description and difference perception abilities.

Conclusion: ViDiC-1K serves as a challenging benchmark to advance video understanding, edit awareness, and comparative reasoning in multimodal intelligence.
...
TL;DR: YOLOA is a real-time affordance detection model that jointly handles object detection and affordance learning using a lightweight detector with LLM Adapter, achieving state-of-the-art accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: Existing affordance detection methods either focus only on "how" objects can be used or treat object detection and affordance learning as separate tasks without effective interaction and real-time capability.

Method: YOLOA uses a lightweight detector with object detection and affordance learning branches refined by an LLM Adapter, which generates accurate class priors, box offsets, and affordance gates during training.

Result: YOLOA achieves 52.8 / 73.1 mAP on ADG-Det / IIT-Heat benchmarks and real-time performance up to 89.77 FPS (846.24 FPS for lightweight variant), showing excellent accuracy-efficiency trade-off.

Conclusion: YOLOA successfully addresses the "what-where-how" challenge in affordance detection by jointly handling object detection and affordance learning with real-time capability and state-of-the-art performance.
...
TL;DR: DM3D is a deformable Mamba architecture using offset-guided Gaussian sequencing for structure-adaptive point cloud serialization, achieving SOTA results in classification, few-shot learning, and segmentation.


<details>
  <summary>Details</summary>
Motivation: State Space Models (SSMs) show strong potential for long-sequence modeling but conflict with point clouds' irregular nature due to reliance on input order. Existing predefined serialization strategies cannot adjust to diverse geometric structures.

Method: Proposes DM3D with an offset-guided Gaussian sequencing mechanism unifying local resampling and global reordering. Includes Gaussian-based KNN Resampling (GKR) for adaptive neighborhood reorganization and Gaussian-based Differentiable Reordering (GDR) for end-to-end serialization optimization. A Tri-Path Frequency Fusion module enhances feature complementarity.

Result: Extensive experiments on benchmark datasets show DM3D achieves state-of-the-art performance in classification, few-shot learning, and part segmentation.

Conclusion: Adaptive serialization effectively unlocks the potential of SSMs for point cloud understanding, with DM3D demonstrating superior performance across multiple tasks.
...
TL;DR: First systematic zero-shot evaluation of 8 stereo depth estimation methods for forestry UAV operations, revealing foundation models excel in structured scenes while iterative methods show better cross-domain robustness.


<details>
  <summary>Details</summary>
Motivation: Need robust depth estimation with cross-domain generalization for autonomous UAV forestry operations, as existing evaluations focus on urban/indoor scenarios, leaving a gap for vegetation-dense environments.

Method: Trained 8 stereo methods exclusively on Scene Flow and evaluated without fine-tuning on 4 standard benchmarks plus a novel 5,313-pair Canterbury forestry dataset captured with ZED Mini camera.

Result: Foundation models excel on structured scenes while iterative methods maintain cross-domain robustness. RAFT-Stereo showed catastrophic failure on ETH3D but normal performance on KITTI. DEFOM identified as optimal baseline for forestry.

Conclusion: DEFOM is the optimal gold-standard baseline for vegetation depth estimation, exhibiting superior depth smoothness, occlusion handling, and cross-domain consistency in forestry environments.
...
TL;DR: A label-efficient framework using frozen diffusion model features for hyperspectral image classification with sparse annotations, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral imaging suffers from low spatial resolution and limited labeled data, creating challenges for land cover classification.

Method: Extracts spatial features from early denoising timesteps of a pretrained diffusion model and integrates them with spectral data using a lightweight FiLM-based fusion module.

Result: Outperforms state-of-the-art approaches on two hyperspectral datasets using only sparse training labels, with ablation studies confirming the benefits of diffusion features and spectral fusion.

Conclusion: Pretrained diffusion models enable domain-agnostic, label-efficient representation learning suitable for remote sensing and scientific imaging tasks.
...
TL;DR: A novel vision-language pretraining framework combining multi-agent data generation and ontology-based knowledge enhancement achieves SOTA zero-shot performance on medical imaging tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLP methods struggle with noisy web data and complex unstructured medical texts in medical image analysis.

Method: Proposes MAGEN system for data generation/verification and O-MAKE framework for decomposing long texts into knowledge aspects with fine-grained alignment.

Result: Achieves state-of-the-art zero-shot performance on disease classification and cross-modal retrieval across eight dermatology datasets.

Conclusion: The framework effectively addresses data quality and text complexity challenges, with code and augmented dataset Derm1M-AgentAug to be released publicly.
...
TL;DR: LM-CartSeg is an automated pipeline for knee MRI segmentation and radiomics analysis, using dual nnU-Net models with geometric post-processing to generate robust ROIs for cartilage/bone compartments.


<details>
  <summary>Details</summary>
Motivation: Manual ROI delineation in knee MRI radiomics lacks robustness and quality control, hindering multi-center studies. There's a need for automatic, anatomically meaningful segmentation that captures both cartilage and subchondral bone.

Method: Two 3D nnU-Net models trained on different datasets (SKM-TEA and OAIZIB-CM) with zero-shot prediction fusion. Geometric post-processing includes connected-component cleaning, 10mm subchondral bone band construction, and PCA/k-means based tibial compartmentalisation. Evaluation on test sets with QC using volume/thickness signatures.

Result: Post-processing improved segmentation accuracy (ASSD: 2.63→0.36mm, HD95: 25.2→3.35mm, DSC: 0.91). Zero-shot DSC 0.80 on external dataset. Geometric L/M compartment rule showed stability across datasets. Only 6-12% of radiomic features correlated with size parameters.

Conclusion: LM-CartSeg provides automatic, quality-controlled ROIs and radiomic features containing discriminative information beyond simple morphometry, establishing a practical foundation for multi-center knee OA radiomics studies.
...
TL;DR: Unsupervised learning of structured 3D keypoints from point clouds for generative shape reconstruction using diffusion models.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised keypoint methods are not designed for unconditional generative settings, limiting their use in modern 3D generative pipelines.

Method: Learning spatially structured 3D keypoints from point cloud data that condition an Elucidated Diffusion Model (EDM) for shape reconstruction.

Result: Achieves 6 percentage-point improvement in keypoint consistency compared to prior approaches, with keypoints showing repeatable spatial structure and smooth interpolation.

Conclusion: The framework successfully bridges the gap between unsupervised keypoint learning and generative modeling, providing compact and interpretable 3D representations.
...
TL;DR: GalaxyDiT is a training-free method that accelerates video generation by 1.87-2.37x with minimal quality loss (0.72-0.97% drop) through guidance alignment and proxy selection.


<details>
  <summary>Details</summary>
Motivation: Diffusion models for video generation require intensive computation due to iterative steps and classifier-free guidance doubling compute, hindering broader adoption.

Method: Uses guidance alignment and systematic proxy selection via rank-order correlation analysis to identify optimal computational reuse across model families and scales.

Result: Achieves 1.87x speedup on Wan2.1-1.3B and 2.37x on Wan2.1-14B with only 0.97% and 0.72% VBench-2.0 quality drop; maintains superior fidelity exceeding prior methods by 5-10 dB PSNR.

Conclusion: GalaxyDiT enables efficient video generation acceleration without training, making diffusion models more practical for downstream applications.
...
TL;DR: A video generation method using depth prediction and geometric losses to improve 3D consistency.


<details>
  <summary>Details</summary>
Motivation: Existing video generation methods lack explicit 3D modeling, causing temporal inconsistencies and artifacts.

Method: Augment latent diffusion models with per-frame depth prediction and a multi-view geometric loss.

Result: Produces more stable and geometrically consistent videos than baselines.

Conclusion: Bridges appearance generation with 3D structure modeling for better results.
