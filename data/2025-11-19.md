...
TL;DR: nuCarla: A synthetic BEV perception dataset for closed-loop autonomous driving research, compatible with nuScenes format.


<details>
  <summary>Details</summary>
Motivation: Existing real-world datasets are non-interactive, limiting closed-loop E2E autonomous driving development.

Method: Created nuScenes-style dataset using CARLA simulator with balanced classes and compatibility for model transfer.

Result: Provides large-scale data and high-performance BEV backbones enabling seamless closed-loop testing.

Conclusion: nuCarla accelerates reliable E2E autonomous driving research by bridging the simulation-to-reality gap.
...
TL;DR: Proposes a two-component framework to address overconfidence in Open Set Recognition by using uncertainty estimation and improved unknown detection.


<details>
  <summary>Details</summary>
Motivation: Inter-class overlap in feature space causes models to assign high confidence to unknown samples similar to known classes, leading to misclassification and undermining OSR performance.

Method: Framework includes a perturbation-based uncertainty estimation module that applies parameter perturbations to quantify uncertainty, and a two-stage unknown detection module with distinct classifiers that uses this uncertainty.

Result: Experimental results on three public datasets demonstrate superior performance compared to existing OSR methods.

Conclusion: The framework effectively mitigates overconfidence caused by inter-class overlap, enhancing discrimination between known and unknown classes in OSR.
...
TL;DR: A novel object-aware temporal modeling approach for few-shot video object detection that uses selective feature propagation to achieve temporal consistency without complex object tube proposals, showing significant performance gains across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Traditional video object detection methods require extensive training data and suffer from temporal inconsistencies due to occlusion and appearance variations. Few-shot settings need efficient approaches that avoid computationally expensive region proposal methods.

Method: Uses a filtering mechanism to selectively propagate high-confidence object features across frames, combined with few-shot trained detection and classification heads for focused feature progression.

Result: Achieves AP improvements of 3.7% (FSVOD-500), 5.3% (FSYTV-40), 4.3% (VidOR), and 4.5% (VidVRD) in 5-shot setting, with consistent gains in 1-shot, 3-shot, and 10-shot configurations.

Conclusion: The proposed method effectively addresses temporal consistency and generalization challenges in few-shot video object detection, demonstrating robust performance without dependency on explicit object tube proposals.
...
TL;DR: This paper proposes FusionFM, a unified flow matching framework for multi-modal image fusion that improves sampling efficiency and structural consistency while addressing the lack of ground truth data through pseudo-label selection and refinement.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal image fusion methods require task-specific models (high training costs) and generative approaches suffer from slow inference due to complex sampling trajectories.

Method: Formulates fusion as direct probabilistic transport using flow matching, collects fusion results from SOTA models as priors with task-aware selection, and introduces Fusion Refiner module for pseudo-label enhancement. Uses elastic weight consolidation and experience replay for multi-task scenarios.

Result: Achieves competitive performance across diverse fusion tasks with significant sampling efficiency improvements and lightweight model design.

Conclusion: The proposed FusionFM framework provides an efficient and scalable solution for multi-modal image fusion that balances performance and computational efficiency.
...
TL;DR: Two-stage framework using diffusion model (Mapfusion) for crash detection without trajectory data, generating realistic road segment maps from noisy inputs and comparing them to monitored maps.


<details>
  <summary>Details</summary>
Motivation: Overcome limitations of trajectory acquisition and vehicle tracking in real-time crash detection by using road segment maps instead.

Method: Mapfusion diffusion model adds noise to road segment maps then denoises using temporal embeddings; ControlNet incorporates background context. Second stage compares monitored maps to generated ones.

Result: Mapfusion generates realistic road segment evolution maps robust to sampling intervals; experiments show accurate crash detection on real-world data.

Conclusion: The two-stage trajectory-free framework effectively detects crashes using road segment maps, enhancing proactive safety management.
...
TL;DR: Proposes ADATG, an adaptive two-grid foundation model with Hilbert encoding for seismic data that separates and processes high/low frequencies using an adaptive training strategy.


<details>
  <summary>Details</summary>
Motivation: Standard vision transformers fail to capture both high- and low-frequency seismic information efficiently due to sequential tokenization ignoring intrinsic seismic patterns.

Method: Uses spectrum decomposition to separate frequency components, hierarchical Hilbert encoding for effective representation, and adaptive training that starts with coarse features then refines to fine details.

Result: Extensive experiments demonstrate the method's effectiveness and efficiency in pretraining visual seismic foundation models.

Conclusion: Highlights the importance of data encoding and training strategies tailored to seismic data's frequency characteristics for improved foundation model performance.
...
TL;DR: Proposes a language-free dementia screening method using facial micro-dynamics (blinks, mouth/jaw motions, gaze, head adjustments) extracted from short videos, achieving high performance without speech or text.


<details>
  <summary>Details</summary>
Motivation: Existing dementia screening relies on speech or scripted interviews, limiting scalability and requiring language/transcription. This work enables passive, scalable screening from unscripted videos across devices/cultures.

Method: Stabilize facial signals, convert micro-movements into interpretable time series, smooth them, and summarize into clip-level statistics (activity mix per window) for lightweight classifier training.

Result: On YT DemTalk dataset (300 clips), achieved AUROC 0.953, AP 0.961, F1 0.851, accuracy 0.857. Gaze lability and mouth/jaw dynamics were most informative.

Conclusion: Facial temporal micro-dynamics alone are sufficient for effective dementia screening, offering a scalable, language-agnostic alternative to traditional methods.
...
TL;DR: New benchmark Gen-ViRe assesses video models' Chain-of-Frames reasoning across 6 cognitive dimensions, revealing gap between visual quality and true reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing video generation benchmarks focus on fidelity/alignment but cannot evaluate Chain-of-Frames reasoning abilities needed for world simulation.

Method: Created Gen-ViRe framework with 6 cognitive dimensions and 24 subtasks, using multi-source data curation, minimal prompting, and hybrid VLM-assisted evaluation.

Result: Experiments on state-of-the-art systems show substantial discrepancies between visual quality and actual reasoning depth.

Conclusion: Gen-ViRe provides first quantitative assessment tool to advance development of genuine world simulators.
...
TL;DR: This paper proposes ranking-based losses (RSPose) to address shortcomings in heatmap-based human pose estimation, replacing traditional MSE loss with metric-aligned losses that improve joint localization, heatmap balance, and correlation with mAP evaluation.


<details>
  <summary>Details</summary>
Motivation: Traditional heatmap-based pose estimation suffers from three key problems: MSE loss penalizes all pixel deviations equally without focusing on peak localization, heatmaps are spatially and class-wise imbalanced, and there's a discrepancy between loss functions and mAP evaluation metrics.

Method: The authors propose ranking-based losses that are theoretically and empirically superior to MSE and KL-Divergence losses. The method works with both 1D and 2D heatmaps and aligns loss functions with the mAP evaluation metric.

Result: RSPose achieves state-of-the-art performance with 79.9 mAP on COCO-val using ViTPose-H, and improves SimCC Resnet-50 by 1.5 AP to 73.6 mAP. The method enhances correlation between confidence scores and localization quality, leading to better NMS and mAP performance.

Conclusion: The proposed ranking-based losses effectively address the limitations of traditional heatmap losses, achieving superior performance across multiple datasets and demonstrating that aligning loss functions with evaluation metrics is crucial for advancing human pose estimation.
...
TL;DR: We introduce Collision Sound Source Segmentation (CS3), a new task that segments objects causing collision sounds in videos using audio input, and propose a weakly-supervised method with foundation models that outperforms baselines by 3x–4.7x mIoU on two benchmarks.


<details>
  <summary>Details</summary>
Motivation: Humans can recognize object properties from interaction sounds, inspiring the need to computationally segment collision sound sources in cluttered egocentric videos where visual cues are limited.

Method: A weakly-supervised approach using foundation models (CLIP and SAM2) and egocentric cues (e.g., objects in hands) to segment sound sources without full supervision.

Result: The method achieves 3x and 4.7x higher mIoU than baselines on the introduced EPIC-CS3 and Ego4D-CS3 benchmarks.

Conclusion: The CS3 task is viable and our audio-visual segmentation method effectively leverages weak supervision and egocentric cues to outperform existing approaches.
...
TL;DR: Proposes Geometric Representation Regression (GRR), an absolute pose regression method that predicts disentangled 3D geometric representations (ray directions and pointmap) from images and recovers the pose with a solver, outperforming prior methods on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Standard APR models act as black boxes, memorizing training views instead of understanding 3D geometry, leading to poor generalizability. The inverse of novel view synthesis offers a geometrically-grounded alternative.

Method: Reformulates APR as GRR by regressing two disentangled geometric representations from the image: ray bundle directions (for rotation) and a pointmap (for translation), then solves for the 6-DoF pose deterministically.

Result: Achieves state-of-the-art performance on 7-Scenes and Cambridge Landmarks datasets, with improved generalization due to the geometric prior and decoupled rotation/translation prediction.

Conclusion: Modeling the inverse rendering process through GRR provides a more robust path for absolute pose estimation, enhancing performance by embedding geometric understanding.
...
TL;DR: New dataset and hierarchical AI model H-CNN-ViT improves bladder cancer recurrence prediction using multi-sequence MRI, achieving 78.6% AUC.


<details>
  <summary>Details</summary>
Motivation: Bladder cancer has high recurrence rates but MRI interpretation is difficult due to post-surgical changes; lack of dedicated datasets hinders AI development.

Method: Introduces a curated multi-sequence MRI dataset and proposes H-CNN-ViT model with hierarchical gated attention for balanced CNN/ViT feature fusion from multiple modalities.

Result: H-CNN-ViT achieves 78.6% AUC on their dataset, outperforming state-of-the-art models.

Conclusion: The work provides a valuable benchmark dataset and demonstrates effective AI architecture for bladder cancer recurrence prediction, with model publicly available.
...
TL;DR: QwenCLIP replaces CLIP's limited text encoder with an LLM-based embedding module to handle long radiology reports, improving medical image-text alignment.


<details>
  <summary>Details</summary>
Motivation: CLIP's text encoder is limited to 77 tokens, insufficient for long radiology reports. Domain-specific adaptations like PubMedBERT still have limited input length and shallow semantics.

Method: Proposes QwenCLIP framework using LLM-based embedding module (e.g., Qwen3-Embedding) with learnable prompts for enhanced cross-modal alignment, leveraging extended context windows.

Result: QwenCLIP captures comprehensive medical semantics from long clinical text, substantially improving medical image-text alignment and downstream performance on radiology benchmarks.

Conclusion: QwenCLIP addresses CLIP's limitations for medical applications by leveraging LLMs' extended context and richer representations, with publicly available code.
...
TL;DR: Novel hybrid model combining EfficientNet and VGG19 with custom Pseudo-Newton Boosting and Sparsity-Induced Feature Reduction layers for improved lumbar spine degeneration classification from DICOM images, achieving 88.1% accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional transfer learning methods for medical image classification often miss detailed anatomical features and suffer from redundancy in high-dimensional data, limiting performance in lumbar spine degeneration analysis.

Method: Hybrid architecture integrating EfficientNet and VGG19 with two custom layers: Pseudo-Newton Boosting (smart feature weight variations) and Sparsity-Induced Feature Reduction (removes redundancy). Includes specific preprocessing pipeline.

Result: Significant performance improvement over baseline EfficientNet: precision 0.9, recall 0.861, F1 score 0.88, loss 0.18, accuracy 88.1%.

Conclusion: The proposed multi-tiered framework overcomes traditional transfer learning limitations in medical imaging, providing lean yet robust feature representations that advance automated diagnostic tools for lumbar spine pathology.
...
TL;DR: This paper proposes using vision-language models (VLMs) as semantic enhancers rather than direct decision-makers in autonomous driving, improving performance and interpretability through enriched scene descriptions and multi-modal fusion.


<details>
  <summary>Details</summary>
Motivation: Existing VLM approaches in autonomous driving rely on handcrafted prompts and show inconsistent performance, lacking robustness and generalization for real-world scenarios.

Method: The method involves using VLMs to generate structured, linguistically rich scene descriptions from visual inputs, then integrating these with a multi-modal architecture that fuses visual and linguistic features, plus a post-hoc refinement module for reliability.

Result: Extensive experiments on two autonomous driving benchmarks show state-of-the-art performance, with enhanced decision accuracy and interpretable explanations.

Conclusion: The approach offers a promising direction for reliable and interpretable autonomous driving systems by effectively leveraging VLMs as semantic enhancers.
...
TL;DR: This paper investigates power law scaling laws for medical image segmentation across multiple anatomical tasks and modalities, showing performance improvements with larger datasets. It introduces a novel deformation-based augmentation method that enhances data efficiency and accelerates convergence beyond standard scaling trends.


<details>
  <summary>Details</summary>
Motivation: Medical anatomical segmentation scaling relationships remain underexplored despite their importance. The motivation is to understand how data scaling laws apply in this domain and leverage topological isomorphism to improve efficiency through deformation-guided augmentation.

Method: Analyzed scaling laws across 15 semantic tasks and 4 imaging modalities. Evaluated existing augmentation methods (random elastic and registration-guided deformation) and proposed a novel approach generating diffeomorphic mappings from geodesic subspace via image registration.

Result: Larger datasets consistently improved segmentation performance following power law scaling. Both registered and generated deformation augmentations significantly boosted data utilization efficiency. The proposed generated deformation method achieved superior performance and faster convergence than standard scaling.

Conclusion: The work provides insights into segmentation scalability in medical imaging, demonstrating that topology-aware deformation augmentation can enhance model efficiency substantially, reducing annotation and computational costs while exceeding conventional scaling trends.
...
TL;DR: Uni-Hema is a unified multi-task model for digital hematopathology that performs detection, classification, segmentation, and reasoning across multiple diseases, outperforming single-task models.


<details>
  <summary>Details</summary>
Motivation: Existing hematopathology models lack unified multi-task, multi-modal reasoning capabilities across different disease categories.

Method: Built on Hema-Former, a multimodal module integrating visual and textual representations, using 46 datasets with 700K+ images and 21K QA pairs.

Result: Achieves comparable or superior performance to single-task models while providing interpretable, single-cell level insights.

Conclusion: Establishes a new standard for multi-task digital hematopathology with publicly available code.
...
TL;DR: A weakly supervised pipeline using Vision Language Models for detecting ephemeral gullies in agricultural fields from remote sensing images, reducing manual labeling needs while achieving strong performance.


<details>
  <summary>Details</summary>
Motivation: Ephemeral gullies are hard to automatically detect due to short temporal cycles and scarcity of accurate labeled data, limiting machine learning approaches to zero-shot methods that are difficult to implement.

Method: Uses remote sensing and VLMs to minimize manual labeling by leveraging VLM pretraining knowledge and a teacher-student model where the teacher learns from VLM-generated noisy labels, and the student learns via weak supervision with a noise-aware loss function.

Result: The approach shows superior performance compared to VLMs and the label model alone when using weak supervision to train the student model.

Conclusion: The pipeline effectively reduces labeling effort and improves detection accuracy, supported by a novel dataset of over 18,000 high-resolution images for future research.
...
TL;DR: This paper introduces a motion vector framework to evaluate temporal fidelity in generative videos, identifying motion discrepancies and enhancing classification accuracy.


<details>
  <summary>Details</summary>
Motivation: Current video evaluation metrics focus on spatial quality but lack sensitivity to temporal realism, creating a gap in assessing motion authenticity in generated videos.

Method: Using motion vectors from compressed video codecs (H.264/HEVC), the authors compute statistical divergences (KL, JS, Wasserstein) between real and generated videos and explore MV-RGB fusion techniques.

Result: Experiments reveal motion artifacts in state-of-the-art generators; MV integration improves classification accuracy (up to 99.0% for real vs. generated discrimination) across various models.

Conclusion: Compressed-domain motion vectors effectively diagnose motion defects and enhance temporal reasoning in video analysis, offering a scalable, model-agnostic evaluation tool.
...
TL;DR: SAE-MCVT is a scalable real-time multi-camera vehicle tracking framework that balances accuracy, efficiency, and scalability for city-scale ITS deployment using edge computing and self-supervised camera link modeling.


<details>
  <summary>Details</summary>
Motivation: Existing MCVT approaches focus heavily on accuracy but neglect real-time performance and scalability, which are critical for practical large-scale urban deployments.

Method: Uses edge devices for local processing (object detection, tracking, geo-mapping, feature extraction) transmitting only lightweight metadata to a central workstation that performs cross-camera association using learned spatiotemporal constraints.

Result: Achieves real-time operation on 2K 15 FPS streams with 61.2 IDF1 score on RoundaboutHD dataset.

Conclusion: SAE-MCVT is the first MCVT framework demonstrating both scalability and real-time performance suitable for city-scale intelligent transportation systems.
...
TL;DR: Evaluation of multi-modal LLMs' understanding of road safety concepts using schematic traffic sign images reveals significant gaps in safety reasoning compared to human learning.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles must adhere to road safety norms, requiring AI systems to accurately interpret safety concepts. Current multi-modal LLMs' capability in this domain is unclear.

Method: Curated a pilot dataset of traffic sign and road-safety images from school textbooks, evaluated multi-modal LLMs in a zero-shot setting.

Result: Models demonstrated poor performance in safety reasoning, highlighting substantial discrepancies between human understanding and AI interpretation.

Conclusion: Significant gaps exist in multi-modal LLMs' road safety comprehension, necessitating further research to bridge these differences for reliable autonomous systems.
...
TL;DR: Curriculum-based Relative Policy Optimization (CuRPO) improves Visual Grounding by structured training using CoT reasoning metrics, outperforming existing methods with efficiency gains.


<details>
  <summary>Details</summary>
Motivation: Traditional CoT prompting with RL fine-tuning degrades Visual Grounding performance for complex queries; dataset size increase doesn't always help due to data complexity.

Method: CuRPO uses CoT length and gIoU rewards as complexity indicators to progressively train models from simple to hard examples.

Result: CuRPO achieves up to +12.52 mAP improvement on RefCOCO, showing robustness and efficiency in few-shot learning scenarios.

Conclusion: CuRPO effectively addresses CoT limitations in Visual Grounding, offering a scalable and efficient training strategy for complex tasks.
...
TL;DR: Cluster-based frame selection prevents data leakage by grouping similar frames before dataset splitting, ensuring balanced and representative partitions.


<details>
  <summary>Details</summary>
Motivation: To address information leakage in video-derived datasets where similar frames may appear across training, validation, and test sets, leading to overestimated model performance.

Method: Cluster visually similar frames and then split them into training, validation, and test sets to ensure each partition contains distinct frame groups.

Result: Produces more representative, balanced, and reliable dataset splits that reduce data leakage.

Conclusion: The cluster-based strategy effectively mitigates information leakage, improving the validity of evaluations on video frame datasets.
...
TL;DR: Pretraining vision transformers on procedurally-generated data (no visual/semantic content) improves data efficiency, convergence speed, and ImageNet-1k accuracy by 1.7% with just 1% procedurally data budget.


<details>
  <summary>Details</summary>
Motivation: Transformers show versatility across domains, suggesting beneficial inductive biases exist. The goal is to instill generic biases in ViTs via pretraining on non-visual procedural data to enhance learning efficiency.

Method: Generate data using simple algorithms (e.g., formal grammars) with no relation to natural/synthetic images. Pretrain ViTs in a warm-up phase bypassing visual patch embeddings to internalize abstract computational priors, then follow with standard image-based training.

Result: Warm-up with procedural data significantly improves data efficiency and convergence. On ImageNet-1k, 1% procedural data improves accuracy by 1.7%, equivalent to using 28% of ImageNet-1k data in performance gain.

Conclusion: Pretraining on procedurally-generated data offers a promising strategy for data-efficient and domain-agnostic ViT pretraining, leveraging abstract priors to boost performance.
...
TL;DR: This paper introduces a method for cell image segmentation using continuous scalar fields derived from PDE solutions, trained efficiently with a simple tensor input to achieve robust results with low computational demands.


<details>
  <summary>Details</summary>
Motivation: To create a robust and efficient cell segmentation method that minimizes the impact of outliers in training data and reduces computational requirements, making it suitable for edge computing.

Method: A continuous scalar field is learned via a U-Net by minimizing field residuals of Poisson PDE or heat equation steady-state solutions, segmented using the watershed method without regularization.

Result: The approach delivers competitive segmentation accuracy on public datasets, with simplified implementation, faster training/inference, lower energy use, and small memory footprint.

Conclusion: The geometrically insightful scalar field approach provides an effective, efficient solution for cell segmentation, balancing performance with practical computational benefits.
...
TL;DR: EchoAgent uses LLM-controlled vision tools for automated, guideline-based cardiac ultrasound analysis, achieving accurate and transparent video-level interpretation.


<details>
  <summary>Details</summary>
Motivation: Current deep learning models lack video-level reasoning and guideline-based measurement support for echocardiography.

Method: Orchestrates specialized vision tools under LLM control for temporal localization, spatial measurement, and clinical interpretation, including a measurement-feasibility predictor.

Result: Achieves accurate, interpretable results with visual evidence and guideline grounding despite spatiotemporal complexity.

Conclusion: Demonstrates feasibility of agentic, guideline-aligned reasoning for cardiac ultrasound, setting a new direction for trustworthy AI.
...
TL;DR: CrossTrainer presents a transferable video representation for skill assessment that discovers universal skill-attributes and uses a multimodal language model to generate feedback across different sports, achieving significant gains over state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Current skill assessment models are specialized for individual sports and suffer from high cost and scarcity of expert-level supervision, limiting their application across the long tail of sports.

Method: The approach discovers cross-sport skill-attributes (e.g., balance, control, hand positioning), then trains a multimodal language model to generate actionable feedback and proficiency levels for novel videos.

Result: The model achieves up to 60% relative gains over state-of-the-art methods in both cross-sport (transfer) and intra-sport (in-domain) settings across multiple datasets.

Conclusion: By abstracting shared behaviors indicative of human skill, CrossTrainer generalizes better than existing techniques and enriches multimodal large language models for broader skill assessment applications.
...
TL;DR: CD-DPE: A dual-prompt expert network using convolutional dictionary feature decoupling for multi-contrast MRI super-resolution, separating cross/intra-contrast features and using frequency/adaptive routing prompts for superior fusion and generalization.


<details>
  <summary>Details</summary>
Motivation: Multi-contrast MRI super-resolution struggles with effective utilization of reference image textures due to modality disparities, leading to suboptimal feature integration and reconstruction quality.

Method: Introduces iterative convolutional dictionary feature decoupling (CD-FDM) to separate features, and dual-prompt feature fusion expert module (DP-FFEM) using frequency and adaptive routing prompts for guided feature integration.

Result: Outperforms state-of-the-art methods on public datasets in reconstructing fine details and shows strong generalization on unseen datasets.

Conclusion: CD-DPE effectively addresses contrast disparities and enhances reconstruction quality through feature decoupling and intelligent fusion, demonstrating robust performance and generalizability.
...
TL;DR: RISE introduces a novel benchmark and system for indoor scene understanding using single static mmWave radar, achieving superior layout reconstruction (16 cm Chamfer Distance) and first-ever mmWave-based object detection (58% IoU) by leveraging multipath reflections.


<details>
  <summary>Details</summary>
Motivation: Optical sensors (RGB/LiDAR) face occlusion and privacy issues indoors, while mmWave radar preserves privacy but has low resolution. Multipath reflections, usually seen as noise, actually contain valuable geometric information for scene understanding.

Method: Proposes Bi-Angular Multipath Enhancement to model Angle-of-Arrival and Angle-of-Departure for recovering ghost reflections, and a simulation-to-reality Hierarchical Diffusion framework to transform radar data into layout and object detection.

Result: On a new benchmark with 50,000 frames from 100 indoor trajectories, RISE reduces Chamfer Distance by 60% to 16 cm for layout reconstruction and achieves 58% IoU for object detection, surpassing state-of-the-art methods.

Conclusion: RISE establishes a new foundation for privacy-preserving, geometry-aware indoor scene understanding using single static radar, effectively turning multipath noise into useful signals for accurate spatial analysis.
...
TL;DR: A 2.5D context-aware classifier improves MRI plane orientation detection accuracy to 99.49%, reducing errors by 60% compared to 2D methods, and enhances brain tumor detection by 1% when integrated.


<details>
  <summary>Details</summary>
Motivation: Automated systems struggle to identify anatomical planes in MRI slices without metadata, causing issues in analysis, dataset merging, and diagnostic accuracy.

Method: Developed a 2.5D model using multi-slice information from 3D sequences and static 2D images to resolve ambiguity and improve feature learning.

Result: The 2.5D method achieved 99.49% accuracy (vs. 98.74% for 2D), reduced errors by 60%, and improved brain tumor detection accuracy from 97.0% to 98.0% when used with a gated uncertainty-based strategy.

Conclusion: 2.5D context is crucial for robust plane orientation detection, and integrating this metadata via a gated strategy significantly enhances diagnostic performance, with the model provided as open-source.
