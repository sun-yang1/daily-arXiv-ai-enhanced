...
TL;DR: iFlyBot-VLA is a Vision-Language-Action model trained with a dual-level action representation framework and mixed training strategy, achieving superior performance in manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: To enhance 3D perceptual and reasoning capabilities in Vision-Language Models (VLMs) for robotic manipulation tasks by aligning language, vision, and action representations.

Method: A dual-level action representation framework supervises VLMs and action experts, combining latent actions (high-level intentions) and structured discrete action tokens (low-level dynamics). Mixed training includes robot trajectory data and QA datasets.

Result: Superior performance on the LIBERO Franka benchmark and competitive success rates in real-world manipulation tasks.

Conclusion: iFlyBot-VLA effectively bridges vision, language, and action for robotic manipulation, with plans to open-source part of the dataset for community research.
...
TL;DR: Domain-specific pretraining in fetal ultrasound outperforms generic foundation models, improving discrimination of anatomically similar structures by up to 20%.


<details>
  <summary>Details</summary>
Motivation: To evaluate foundation models in fetal ultrasound imaging under low inter-class variability conditions, focusing on anatomically similar structures like fetal brain standard planes (TT, TV, TC).

Method: The study curated and aggregated publicly available fetal ultrasound datasets into FetalUS-188K, pretrained DINOv3 in a self-supervised manner, and evaluated features through standardized adaptation protocols (linear probing and full fine-tuning) under two initialization schemes.

Result: Models pretrained on fetal ultrasound data outperformed those initialized on natural images, with up to 20% improvement in weighted F1-score. Domain-adaptive pretraining preserved subtle cues for distinguishing planes like TV.

Conclusion: Generic foundation models struggle with low inter-class variability, while domain-specific pretraining is crucial for robust and reliable representations in fetal brain ultrasound imaging.
...
TL;DR: GFMs like Clay offer small to moderate improvements in flood mapping accuracy over traditional models, with lower computational costs. Clay excels in performance, efficiency, and detail retention, making it a strong choice for practical applications.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of systematic comparison between GFMs and traditional models in flood inundation mapping, guiding end-users in model selection by evaluating performance across different sensors and data availability scenarios.

Method: The study evaluates three GFMs (Prithvi 2.0, Clay V1.5, DOFA) and a variant (UViT) against traditional models (TransNorm, U-Net, Attention U-Net) using datasets from PlanetScope, Sentinel-1, and Sentinel-2. Performance is measured through metrics like mIoU and computational time, with additional validation via leave-one-region-out cross-validation and few-shot experiments.

Result: Results show competitive performance among GFMs, with Clay outperforming others on PlanetScope and Sentinel-2, and Prithvi leading on Sentinel-1. Clay also demonstrates better generalization in cross-validation and few-shot experiments, along with faster computational times due to its smaller model size.

Conclusion: The study concludes that Geo-Foundational Models (GFMs) like Clay, Prithvi, and DOFA offer small to moderate improvements in flood mapping accuracy compared to traditional models like U-Net, with lower computational costs and labeling effort. Clay, in particular, stands out for its superior performance, efficiency, and ability to retain fine details.
...
TL;DR: A learning-based framework for image reconstruction from incomplete measurements, leveraging multiple invariances to match fully supervised performance with less data.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of conventional supervised and self-supervised methods for image reconstruction, especially with fixed, deterministic sampling patterns.

Method: Exploiting multiple invariances of the image distribution to achieve reconstruction performance comparable to fully supervised approaches.

Result: Competitive or superior results in optical-resolution image upsampling for photoacoustic microscopy (PAM) with reduced ground truth data.

Conclusion: The proposed method effectively bridges the gap between fully supervised and self-supervised approaches, offering high performance with less data.
...
TL;DR: The paper benchmarks LMMs for PHI detection in medical imaging, showing superior OCR performance but mixed overall accuracy gains, with recommendations for deployment.


<details>
  <summary>Details</summary>
Motivation: To improve PHI detection in medical imaging by leveraging advancements in Large Multimodal Models (LMMs) for better text extraction and semantic analysis.

Method: Systematic benchmarking of three LMMs (GPT-4o, Gemini 2.5 Flash, Qwen 2.5 7B) using two pipeline configurations: text analysis alone and OCR + semantic analysis.

Result: LMMs outperform traditional OCR models in efficacy (lower WER and CER) but show inconsistent overall PHI detection accuracy improvements, with notable gains in complex cases.

Conclusion: LMMs offer superior OCR performance but require tailored deployment strategies; the study provides practical recommendations for LMM selection and infrastructure.
...
TL;DR: The paper introduces 	extit{StrengthSense}, an open IMU dataset for strength-demanding activities, validated against video data, to aid in human activity recognition and health monitoring.


<details>
  <summary>Details</summary>
Motivation: The motivation behind the paper is the lack of comprehensive datasets capturing strength-demanding activities, which are crucial for monitoring muscular strength, endurance, and power.

Method: The dataset was collected from 29 healthy subjects using 10 IMUs placed on limbs and the torso, with annotations based on video recordings. The paper details data collection, pre-processing, and technical validation, including a comparative analysis of IMU-estimated and video-extracted joint angles.

Result: The result is the introduction of 	extit{StrengthSense}, an open dataset capturing 11 strength-demanding activities and 2 non-strength demanding activities, validated for accuracy and reliability.

Conclusion: The paper concludes that the 	extit{StrengthSense} dataset is a valuable resource for advancing human activity recognition algorithms and health monitoring applications, validated through comparative analysis of IMU and video-extracted joint angles.
...
TL;DR: A pipeline automates text-VQA dataset synthesis using OCR, ROI detection, and question generation, producing 72K QA pairs from 44K images.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the tedious and challenging human annotation process for text-VQA datasets by leveraging foundation models and mature OCR systems.

Method: The method integrates OCR detection and recognition, ROI detection, caption generation, and question generation into a cohesive pipeline.

Result: The pipeline produces around 72K QA pairs based on 44K images, demonstrating scalability and faithfulness in QA pair synthesis.

Conclusion: The proposed pipeline successfully automates the synthesis and validation of large-scale text-VQA datasets, addressing the challenges of human annotation.
...
TL;DR: A markerless AR pipeline on HoloLens 2 achieved ~3-4 mm median error in live surgeries, validated by preclinical and clinical trials, improving markerless AR guidance for small or low-curvature anatomies.


<details>
  <summary>Details</summary>
Motivation: To develop and clinically evaluate a depth-only, markerless AR registration pipeline for accurate alignment on small or low-curvature anatomies in real-life surgical settings.

Method: On HoloLens 2, the pipeline aligns Articulated Hand Tracking (AHAT) depth to CT-derived skin meshes via depth-bias correction, human-in-the-loop initialization, and global and local registration. Validation involved comparing AR-traced distances to CT ground truth on leg and foot models.

Result: Preclinical validation showed tight agreement between AR-traced and CT distances (leg: median |Δd| 0.78 mm, RMSE 0.97 mm; feet: 0.80 mm, 1.20 mm). Clinically, per-point error had a median of 3.9 mm, with anatomy-specific medians of 3.2 mm (feet), 4.3 mm (ear), and 5.3 mm (lower leg).

Conclusion: A depth-only, markerless AR pipeline on HMDs achieved ~3-4 mm median error across feet, ear, and lower leg in live surgical settings without fiducials, approaching typical clinical error thresholds for moderate-risk tasks. Human-guided initialization plus global-to-local registration enabled accurate alignment on small or low-curvature targets, improving the clinical readiness of markerless AR guidance.
...
TL;DR: An automated pipeline combining instance segmentation and chamber-ordering reconstructs 3D growth trajectories of foraminifera, reducing manual effort and maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Automated tracing of chamber growth in planktonic foraminifera is underexplored, with current methods relying on manual segmentation, which is time-consuming and subjective.

Method: The proposed pipeline integrates instance segmentation with a chamber-ordering algorithm to automatically reconstruct 3D growth trajectories from high-resolution computed tomography scans. Multiple instance segmentation methods are evaluated for their impact on growth-order reconstruction accuracy.

Result: The pipeline significantly reduces manual effort while maintaining biologically meaningful accuracy. The chamber-ordering algorithm remains robust even under partial segmentation.

Conclusion: The study presents the first fully automated and reproducible pipeline for digital foraminiferal growth analysis, enabling large-scale, data-driven ecological studies.
...
TL;DR: A PCA and RPCA-based framework for efficient and accurate pavement crack width measurement, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Accurate crack width measurement is crucial for assessing pavement structural integrity, but challenges arise from complex crack morphology and the need for rapid measurements from arbitrary pixel locations.

Method: The methodology involves three stages: initial crack segmentation, determination of the primary orientation axis using PCA, and extraction of the Main Propagation Axis (MPA) for irregular cracks using RPCA.

Result: Comprehensive evaluations on three datasets show the framework achieves high computational efficiency and measurement accuracy.

Conclusion: The proposed cascaded framework integrating PCA and RPCA demonstrates superior performance in computational efficiency and measurement accuracy for pavement crack width quantification, outperforming existing state-of-the-art techniques.
...
TL;DR: An autobiasing system using CNNs dynamically adjusts event camera biases to reduce flicker, improving face detection performance in diverse lighting.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address flicker effects caused by rapid light intensity variations, which degrade event camera performance in various environments.

Method: The method involves using a Convolutional Neural Network (CNN) to detect flicker in spatial space and dynamically adjust event camera biases to minimize its impact.

Result: Results show enhanced YOLO confidence metrics for face detection, increased frames capturing detected faces, and a 38.2% (well-lit) and 53.6% (low-light) reduction in average gradient indicating flicker presence.

Conclusion: The paper concludes that the proposed autobiasing system significantly improves event camera performance by mitigating flicker effects across diverse lighting conditions and frequencies.
...
TL;DR: A three-stage pipeline for GVQA introduces a trigger moment for grounding and tracking, achieving a HOTA score of 0.4968, a major improvement over prior results.


<details>
  <summary>Details</summary>
Motivation: The GVQA task requires robust multimodal models for complex reasoning over video content, visual grounding of answers, and temporal tracking of referenced objects.

Method: The approach decomposes the GVQA task into a three-stage pipeline: Video Reasoning & QA, Spatio-temporal Grounding, and Tracking, with the introduction of a trigger moment derived from the CORTEX prompt.

Result: The framework achieves a HOTA score of 0.4968, demonstrating significant improvement over the previous year's winning score of 0.2704.

Conclusion: The proposed framework significantly improves performance on the GVQA task, achieving a HOTA score of 0.4968, a notable advancement over the previous year's score of 0.2704.
...
TL;DR: MM-UNet, a novel deep learning architecture, improves retinal vessel segmentation by enhancing branching perception and boundary awareness, achieving higher accuracy than existing methods.


<details>
  <summary>Details</summary>
Motivation: Retinal vessel segmentation is crucial for clinical diagnosis, but the thin and branching structures of vasculature pose challenges for precision and robustness.

Method: MM-UNet introduces Morph Mamba Convolution layers for better branching perception and Reverse Selective State Guidance modules for improved boundary awareness.

Result: MM-UNet outperforms existing methods with F1-score gains of 1.64% on DRIVE and 1.25% on STARE datasets.

Conclusion: MM-UNet is an effective and advanced solution for retinal vessel segmentation, with publicly available code for further use.
...
TL;DR: A generative model synthesizes realistic PET images from MRI and blood biomarkers, improving Alzheimer's diagnosis by reducing reliance on costly Abeta-PET scans.


<details>
  <summary>Details</summary>
Motivation: The high cost and limited accessibility of amyloid-beta positron emission tomography (Abeta-PET) for Alzheimer's disease diagnosis motivated the exploration of predicting Abeta-PET spatial patterns from BBMs and MRI scans.

Method: A language-enhanced generative model, utilizing a large language model (LLM) and multimodal information fusion, was developed to synthesize PET images from blood-based biomarkers (BBMs) and MRI scans. The synthetic images were evaluated for quality, diagnostic consistency, and clinical applicability.

Result: Synthetic PET images closely resemble real PET scans (SSIM = 0.920 +/- 0.003, Pearson's r = 0.955 +/- 0.007) and show high diagnostic agreement (accuracy = 0.80). The synthetic PET-based model (AUC = 0.78) outperforms T1-based (AUC = 0.68) and BBM-based (AUC = 0.73) models, with further improvement when combined with BBMs (AUC = 0.79).

Conclusion: The language-enhanced generative model successfully synthesizes realistic PET images, improving the diagnostic workflow for Alzheimer's disease by leveraging MRI and blood-based biomarkers.
...
TL;DR: A novel object-centric 3D reconstruction framework using SAM-2 and background masking improves strawberry plant phenotyping by providing accurate, non-destructive trait analysis.


<details>
  <summary>Details</summary>
Motivation: Traditional plant phenotyping methods are time-consuming and destructive, while current 3DGS applications in agriculture include noisy background elements, complicating analysis. The goal is to achieve accurate, non-destructive reconstructions for trait analysis.

Method: The method involves a preprocessing pipeline using SAM-2 and alpha channel background masking for clean reconstructions, followed by DBSCAN clustering and PCA for trait estimation.

Result: The framework produces background-free reconstructions, enabling automatic estimation of plant traits like height and canopy width, with superior accuracy and efficiency compared to conventional pipelines.

Conclusion: The proposed object-centric 3D reconstruction framework, incorporating SAM-2 and alpha channel background masking, offers a scalable and non-destructive solution for strawberry plant phenotyping, outperforming conventional methods in accuracy and efficiency.
...
TL;DR: The paper introduces autoStrain, an automated deep learning pipeline for segmental longitudinal strain (SLS) estimation in TEE, comparing two DL methods (TeeFlow and TeeTracker). TeeTracker outperformed TeeFlow in accuracy, validated by synthetic and clinical data, improving cardiac function assessment.


<details>
  <summary>Details</summary>
Motivation: Current strain estimation techniques are manual and resource-intensive, limiting efficiency for monitoring myocardial ischemia.

Method: The study uses deep learning (TeeFlow and TeeTracker) on synthetic TEE data (synTEE) for automated SLS estimation, validated with clinical data.

Result: TeeTracker achieved 0.65 mm mean error on synthetic data and aligned with clinical references (1.09% mean difference). Simulated ischemia improved model accuracy.

Conclusion: AI-driven motion estimation with TEE enhances precision and efficiency in cardiac function assessment, validated by synthetic and clinical data.
...
TL;DR: Foundation models enhance mobile sparse sensing, improving accuracy in geometry-aware image warping and 3D reconstruction, but challenges remain for integration.


<details>
  <summary>Details</summary>
Motivation: Mobile sensing systems struggle with balancing sensing quality and efficiency due to computational and power constraints. Existing sparse sensing methods often compromise accuracy.

Method: The research evaluates foundation models using real-world mobile AR data, focusing on geometry-aware image warping and 3D scene reconstruction.

Result: Foundation models significantly improve geometry-aware image warping and demonstrate scalability and leading performance in 3D scene reconstruction.

Conclusion: The study highlights the potential of foundation models in improving mobile sparse sensing, particularly in geometry-aware image warping and 3D scene reconstruction, while also identifying open challenges for integration.
...
TL;DR: A new multimodal fusion framework for Alzheimer's disease diagnosis improves performance by preserving shared and modality-specific features and aligning latent distributions between MRI and PET.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal fusion approaches for Alzheimer's disease diagnosis often overlook modality-specific features and suffer from biased representations due to distributional differences between MRI and PET.

Method: The framework includes a learnable parameter representation (LPR) block to handle missing modality information, shared and modality-independent encoders, and a consistency-guided mechanism to align latent distributions across MRI and PET modalities.
...
Conclusion: The proposed Collaborative Attention and Consistent-Guided Fusion framework effectively addresses the limitations of existing multimodal neuroimaging fusion methods by preserving both shared and modality-specific features, leading to superior diagnostic performance for Alzheimer's disease.
...
TL;DR: A latent feature alignment method reduces the domain gap in endoscopic depth estimation, outperforming current methods.


<details>
  <summary>Details</summary>
Motivation: Supervised learning of depth from real endoscopic images is limited due to the difficulty of obtaining absolute depth maps, and existing domain adaptation methods often leave a residual domain gap.

Method: The method employs adversarial learning and directional feature consistency to align latent features between translated synthetic and real endoscopic frames, focusing on depth estimation itself.

Result: The approach achieves superior performance on both absolute and relative depth metrics, consistently improving results across various backbones and pretrained weights.

Conclusion: The proposed latent feature alignment method significantly improves absolute depth estimation in endoscopic videos by reducing the domain gap between synthetic and real images, outperforming state-of-the-art methods.
...
TL;DR: The paper introduces HTSC-CIF, a hierarchical task decomposition framework addressing three key challenges in Medical Report Generation (MRG): domain knowledge, text-visual alignment, and cross-modal biases, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: The need to reduce radiologists' burden by automating report generation from radiological images, while overcoming challenges like insufficient domain knowledge, poor text-visual alignment, and cross-modal biases.

Method: Proposes HTSC-CIF, a hierarchical framework: 1) Low-level aligns medical entities with spatial locations; 2) Mid-level uses Prefix Language Modeling and Masked Image Modeling for cross-modal alignment; 3) High-level employs cross-modal causal intervention to reduce biases.

Result: HTSC-CIF significantly outperforms state-of-the-art MRG methods in experiments.

Conclusion: The HTSC-CIF framework effectively addresses the three main challenges in MRG, demonstrating superior performance and potential for practical application.
...
TL;DR: Euler angles, despite their shortcomings, are viable for Normalizing Flows models in probabilistic 3D pose estimation, offering practical advantages.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the need for probabilistic pose outputs in scenarios where pose ambiguity arises due to sensor constraints, projection limitations, or object symmetries.

Method: The paper explores the use of Euler angles as a parameterization for Normalizing Flows models in the context of 3D pose estimation.

Result: The results suggest that Euler angles can lead to useful models for pose estimation, even when compared to more complex parameterizations.

Conclusion: The paper concludes that Euler angles, despite their limitations, can be effectively used as a basis for Normalizing Flows models in pose estimation, offering practical benefits over more complex parameterizations.
...
TL;DR: SAIL-RL is a reinforcement learning framework that improves reasoning in multimodal large language models by teaching them adaptive thinking strategies, outperforming existing methods and reducing hallucinations.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack sound reasoning supervision and uniform strategies, leading to overthinking or underthinking. SAIL-RL aims to address these limitations.

Method: SAIL-RL uses a dual reward system: Thinking Reward for reasoning quality and Judging Reward for adaptive strategy selection.

Result: SAIL-RL enhances reasoning and multimodal understanding, outperforms benchmarks, and reduces hallucinations, competing with models like GPT-4o.

Conclusion: SAIL-RL is a principled framework for building reliable and adaptive multimodal large language models.
...
TL;DR: A GNN-based method for HME recognition uses BLSTM for initial graph formation and a GNN to refine the Symbol Label Graph, achieving promising results.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the recognition of Handwritten Mathematical Expressions (HMEs) by modeling them as graphs to capture spatial dependencies between symbols.

Method: The method involves using a deep BLSTM network for symbol segmentation, recognition, and spatial relation classification to form an initial primitive graph. A 2D-CFG parser generates possible spatial relations, and a GNN-based link prediction model refines the graph by removing unnecessary connections.

Result: Experimental results show the approach's effectiveness in HME structure recognition.

Conclusion: The proposed GNN-based approach effectively recognizes Handwritten Mathematical Expressions (HMEs) by refining the Symbol Label Graph, demonstrating promising performance in structure recognition.
...
TL;DR: Cycle-Sync is a robust framework for camera pose estimation, using modified MPLS and cycle consistency, outperforming existing methods without bundle adjustment.


<details>
  <summary>Details</summary>
Motivation: The paper aims to provide a robust and global framework for estimating camera poses (rotations and locations) without relying on bundle adjustment.

Method: Cycle-Sync adapts message-passing least squares (MPLS) for camera location estimation, emphasizing cycle-consistent information, redefining cycle consistencies, and incorporating a Welsch-type robust loss. It also includes a plug-and-play outlier rejection module and integrates cycle consistency into MPLS for rotation synchronization.

Result: Cycle-Sync achieves the strongest known deterministic exact-recovery guarantee for camera location estimation, demonstrating that cycle consistency alone suffices for the lowest sample complexity currently known.

Conclusion: Cycle-Sync outperforms leading pose estimators, including full structure-from-motion pipelines with bundle adjustment, as demonstrated by experiments on synthetic and real datasets.
...
TL;DR: GAFD-CC improves OOD detection by decoupling features and fusing them with logit-based confidence, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing post-hoc OOD detection methods overlook the correlation between features and logits, limiting their effectiveness.

Method: GAFD-CC refines decision boundaries by decoupling features into positively and negatively correlated types, then fusing them with multi-scale logit-based confidence.

Result: Extensive experiments show GAFD-CC's superior performance on large-scale benchmarks.

Conclusion: GAFD-CC demonstrates competitive performance and strong generalization ability in OOD detection, outperforming state-of-the-art methods.
...
TL;DR: The paper introduces M3PD, a dual-view mobile photoplethysmography dataset, and F3Mamba, a model improving heart-rate accuracy by fusing facial and fingertip views.


<details>
  <summary>Details</summary>
Motivation: Current portable physiological monitoring methods are limited by accessibility and reliability issues, especially for cardiovascular patients. Video-based photoplethysmography on smartphones faces challenges like motion artifacts and lighting variations.

Method: The authors introduce the M3PD dataset, the first dual-view mobile photoplethysmography dataset, and propose F3Mamba, a model that fuses facial and fingertip views using Mamba-based temporal modeling.

Result: F3Mamba reduces heart-rate error by 21.9 to 30.2 percent over single-view baselines and improves robustness in real-world scenarios.

Conclusion: The M3PD dataset and F3Mamba model address key limitations in portable physiological monitoring, offering a more reliable and accessible solution for cardiovascular patients.
...
TL;DR: CoCoVa introduces continuous cross-modal reasoning for VLMs, outperforming larger models and capturing interpretable reasoning patterns.


<details>
  <summary>Details</summary>
Motivation: Contemporary Vision-Language Models (VLMs) are limited by discrete linguistic tokens, restricting their ability to capture the rich, high-dimensional nature of visual perception. CoCoVa aims to address this gap.

Method: CoCoVa employs an iterative reasoning cycle with a Latent Q-Former (LQ-Former) for cross-modal fusion, a token selection mechanism for attentional focus, and a multi-task objective combining contrastive learning and diffusion-based reconstruction.

Result: CoCoVa improves accuracy and token efficiency, competing with or surpassing larger models (7B-9B) on benchmarks. Qualitative analysis shows interpretable reasoning patterns in the latent space.

Conclusion: CoCoVa demonstrates the potential to bridge the gap between discrete language processing and continuous visual understanding, achieving competitive or superior performance compared to larger models.
...
TL;DR: RxnCaption framework transforms chemical reaction diagram parsing into an image captioning task using LVLMs, achieving state-of-the-art results with the BIVP strategy and RxnCaption-11k dataset.


<details>
  <summary>Details</summary>
Motivation: Existing chemical reaction data in papers are image-based and not machine-readable, limiting AI research in chemistry.

Method: Proposes RxnCaption framework using LVLMs and BIVP strategy (MolYOLO for molecular detection) to reformulate parsing as an image captioning problem.

Result: RxnCaption-VL achieves state-of-the-art performance; RxnCaption-11k dataset is significantly larger and balanced.

Conclusion: The framework, dataset, and models advance structured information extraction in chemistry and enable broader AI applications.
...
TL;DR: The paper introduces a self-supervised method for moving object segmentation in sparse and noisy radar point clouds, using contrastive learning and limited annotated data to improve efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Radar sensors provide direct Doppler velocity measurements for single-scan moving object segmentation, but their sparse and noisy data makes annotation tedious and costly.

Method: A two-step approach: contrastive self-supervised representation learning followed by supervised fine-tuning with a novel clustering-based contrastive loss and dynamic points removal for cluster refinement.

Result: The method improves label efficiency and boosts state-of-the-art performance through self-supervised pretraining.

Conclusion: The proposed approach effectively addresses the challenges of radar data annotation and enhances moving object segmentation performance.
...
TL;DR: A grouping-based hybrid color correction algorithm for color point clouds is proposed, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Color consistency correction for color point clouds is crucial for 3D rendering and compression, but most prior work focused on color images.

Method: The algorithm estimates overlapping rates, partitions target points into groups (Gcl, Gmod, Gdist), and applies KBI, JKHE, or HE methods for color correction.

Result: Tested on 1086 color point cloud pairs, the algorithm demonstrates superior performance.

Conclusion: The proposed method effectively corrects color consistency in point clouds, validated by extensive testing.
