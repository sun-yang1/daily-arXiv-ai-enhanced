...
TL;DR: The paper focuses on detecting cyberbullying in GIFs using a deep learning model (VGG16) on a dataset collected from Twitter, achieving 97% accuracy.


<details>
  <summary>Details</summary>
Motivation: Cyberbullying is a growing issue on social media, with limited research on detecting it in GIFs/stickers.

Method: Collected a dataset of 4100 GIFs from Twitter using hashtags, applied the VGG16 deep learning model for detection.

Result: Achieved 97% accuracy in detecting cyberbullying in GIFs.

Conclusion: The study provides a valuable GIF dataset for future research in cyberbullying detection.
...
TL;DR: The paper proposes a deep learning model using 4-band satellite imagery to monitor fire damage in Sudan's conflict zones, showing effectiveness in five case studies with minimal delay.


<details>
  <summary>Details</summary>
Motivation: The ongoing war in Sudan necessitates rapid monitoring and analysis of conflict-related damages.

Method: Uses 4-band imagery from Planet Labs with a deep learning model for near real-time monitoring.

Result: The automated method captures active fires and charred areas more accurately than a baseline, with 8-band imagery offering only marginal gains.

Conclusion: The approach effectively monitors fire damage in conflict zones with minimal delay, demonstrating practical utility.
...
TL;DR: LivingSwap is a video reference-guided face swapping model that uses keyframes for identity injection and temporal stitching, achieving high fidelity and consistency in long video sequences.


<details>
  <summary>Details</summary>
Motivation: To enhance fidelity and temporal coherence in video face swapping by leveraging visual attributes from source videos, inspired by reference-guided image editing.

Method: Employs keyframes as conditioning signals for identity injection and combines them with video reference guidance for temporal stitching.

Result: Achieves state-of-the-art results in seamlessly integrating target identity with source video attributes, reducing manual effort.

Conclusion: LivingSwap demonstrates effective video face swapping with high fidelity and temporal consistency, validated by extensive experiments.
...
TL;DR: The paper introduces a hierarchical framework for semantic segmentation in dental imaging, improving accuracy and clinical plausibility by embedding anatomical structures into the model.


<details>
  <summary>Details</summary>
Motivation: Existing hierarchy-aware segmentation methods provide weak supervision. The paper aims to enhance this by explicitly embedding anatomical hierarchy into segmentation models.

Method: The framework uses a recurrent, level-wise prediction scheme with restrictive output heads and top-down feature conditioning, coupled with Feature-wise Linear Modulation for child class features.

Result: The hierarchical variants consistently improve IoU, Dice, and recall, especially for fine-grained anatomies, though with increased false positives.

Conclusion: Explicit hierarchical structuring enhances performance and clinical plausibility in dental imaging, particularly in low-data regimes.
...
TL;DR: The paper introduces FRIEDA, a benchmark for cartographic reasoning in LVLMs, revealing significant performance gaps between models and humans.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of evaluation for cartographic reasoning, a critical skill for tasks like disaster response and urban planning, by creating a rigorous benchmark that tests multi-step inference and cross-map grounding.

Method: The paper introduces FRIEDA, a benchmark for evaluating complex cartographic reasoning in LVLMs, sourcing real map images and categorizing spatial relations into topological, metric, and directional. It evaluates eleven LVLMs under direct and contextual settings.

Result: Results show that even the best models achieve only 38.20% and 37.20% accuracy, far below human performance of 84.87%, revealing a gap in multi-step cartographic reasoning.

Conclusion: The paper concludes that current LVLMs, including top models like Gemini-2.5-Pro and GPT-5-Think, significantly underperform humans in cartographic reasoning, highlighting the need for improved spatial intelligence benchmarks like FRIEDA.
...
TL;DR: The paper introduces SSplain, a method for generating smooth and sparse explanations for neural networks in medical diagnosis, specifically for Retinopathy of Prematurity (ROP) classification from fundus images. It outperforms existing methods in accuracy and smoothness.


<details>
  <summary>Details</summary>
Motivation: Current explainer methods fail to preserve image structures like smoothness and sparsity, which are crucial for clinicians to trust and understand model outputs.

Method: SSplain enforces smoothness and sparsity through an optimization problem solved via ADMM, generating pixel-wise explanations.

Result: SSplain outperforms common explainers in accuracy and smoothness, and identifies features consistent with clinical discriminative factors for ROP.

Conclusion: SSplain provides realistic explanations for black-box models in medical diagnosis, enhancing clinician trust and understanding, and generalizes well to other datasets.
...
TL;DR: A unified model for sign language translation and alignment, achieving top results on BSL and ASL datasets.


<details>
  <summary>Details</summary>
Motivation: To develop a unified model for sign language understanding, enabling conversion of signing videos into spoken language text and temporal alignment with subtitles for practical communication, corpus construction, and education.

Method: The approach includes a lightweight visual backbone, a Sliding Perceiver mapping network, and a multi-task scalable training strategy.

Result: State-of-the-art performance on BOBSL (BSL) and robust generalization on How2Sign (ASL).

Conclusion: The model achieves state-of-the-art results on BOBSL (BSL) for both SLT and SSA, and demonstrates robust zero-shot generalization and fine-tuned performance on How2Sign (ASL).
...
TL;DR: The paper proposes frequency-domain masking for deepfake detection, enhancing accuracy and scalability while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to achieve robust generalization to new and unseen deepfakes while minimizing computational overhead for large-scale screening.

Method: The method introduces random masking and geometric transformations, with a focus on frequency masking for its superior generalization properties.

Result: The method achieves state-of-the-art generalization on GAN- and diffusion-generated image datasets and exhibits consistent robustness under structured pruning.

Conclusion: The paper concludes that frequency masking enhances deepfake detection accuracy and maintains performance under model pruning, offering a scalable and resource-conscious solution.
...
TL;DR: M2A introduces a simple CTTA approach using random masking and achieves strong performance on corrupted datasets.


<details>
  <summary>Details</summary>
Motivation: To determine if simple random masking can suffice for test-time adaptation under strong corruption, avoiding complex designs.

Method: Generates masked views (spatial/frequency) and adapts using mask consistency loss and entropy minimization loss.

Result: M2A (Spatial) achieves 8.3%/19.8%/39.2% mean error on CIFAR10C/CIFAR100C/ImageNetC, outperforming baselines.

Conclusion: Simple random masking with consistency and entropy objectives is effective for test-time adaptation.
...
TL;DR: The paper evaluates various change detection models for deforestation monitoring in the Amazon, incorporating modern architectures and preprocessing techniques to improve effectiveness.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations in existing deforestation detection models, such as lack of modern architectures and methodological standardization.

Method: Comparative analysis of multitemporal satellite images using fully convolutional models and Transformer-based networks, with various pre- and post-processing techniques.

Result: Achieved an F1-score of 80.41%, demonstrating improved model effectiveness through combined strategies.

Conclusion: The study highlights the potential of advanced models and techniques in enhancing deforestation monitoring, contributing to global conservation efforts.
...
TL;DR: CVP enhances 3D spatial reasoning by mimicking human central and peripheral vision, achieving top performance in benchmarks.


<details>
  <summary>Details</summary>
Motivation: Addresses the limitation of existing approaches that lack explicit, high-level structural understanding for spatial reasoning.

Method: Introduces target-affinity token and allocentric grid to enhance spatial reasoning in a Large Multimodal Model-based architecture.

Result: CVP outperforms existing methods across various 3D scene understanding benchmarks.

Conclusion: The CVP framework achieves state-of-the-art performance in 3D scene understanding by integrating central and peripheral vision-inspired components.
...
TL;DR: Fourier-RWKV is a novel dehazing framework with linear complexity, combining spatial, frequency, and semantic perceptions for efficient and effective haze removal.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of real-time deployment for Transformer-based methods in non-uniform haze conditions by reducing quadratic computational complexity.

Method: The model integrates three distinct perceptual states: Spatial-form Perception (DQ-Shift), Frequency-domain Perception (Fourier Mix block), and Semantic-relation Perception (SBM with DSK-Fusion).

Result: Extensive experiments demonstrate superior performance and reduced computational overhead.

Conclusion: Fourier-RWKV delivers state-of-the-art performance across diverse haze scenarios while significantly reducing computational overhead, establishing a favorable trade-off between restoration quality and practical efficiency.
...
TL;DR: Study explores the trade-off between model accuracy and human-like behavior in monocular depth estimation, revealing distinct patterns despite shared biases.


<details>
  <summary>Details</summary>
Motivation: Aligning model representations with human perception to enhance robustness and interpretability in monocular depth estimation.

Method: Systematic investigation of 69 monocular depth estimators using the KITTI dataset, with affine fitting to decompose prediction errors.

Result: Found shared biases between humans and DNNs but distinct trade-offs between accuracy and human similarity.

Conclusion: Highlights the need for multifaceted, human-centric evaluations beyond traditional accuracy metrics.
...
TL;DR: GeoLoom is a novel framework for text-to-diagram generation in geometric domains, combining autoformalization and Monte Carlo optimization for high structural fidelity.


<details>
  <summary>Details</summary>
Motivation: The challenge of high-quality geometric diagram generation demands strict spatial accuracy and offers well-defined constraints to guide generation.

Method: GeoLoom comprises an autoformalization module that translates natural language into GeoLingua and a coordinate solver using Monte Carlo optimization.

Result: Empirical results show GeoLoom significantly outperforms state-of-the-art baselines in structural fidelity.

Conclusion: GeoLoom provides a principled foundation for interpretable and scalable diagram generation, outperforming state-of-the-art baselines in structural fidelity.
...
TL;DR: Proposes an on-device framework for Animal Re-ID, scaling MobileNetV2 for low-resolution inputs, achieving competitive accuracy with significantly reduced model size.


<details>
  <summary>Details</summary>
Motivation: Enable practical Animal Re-ID on low-power edge devices (MCUs) for wildlife monitoring and livestock management.

Method: Systematically scales a CNN-based MobileNetV2 backbone for low-resolution inputs and introduces data-efficient fine-tuning.

Result: Compact model achieves competitive retrieval accuracy, reducing size by over two orders of magnitude, with minimal accuracy drop in deployment.

Conclusion: Demonstrates practical, adaptable Animal Re-ID on MCU-class devices, enabling scalable deployment in real field environments.
...
TL;DR: Blur2Sharp integrates 3D-aware neural rendering and diffusion models to generate sharp, consistent novel-view human avatars from a single reference, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of creating lifelike human avatars with realistic pose variation and viewpoint flexibility, overcoming issues of geometric inconsistency and blurry outputs in current approaches.

Method: The framework integrates 3D-aware neural rendering and diffusion models, employing a dual-conditioning architecture with Human NeRF for geometrically coherent renderings and a diffusion model for refinement. Hierarchical feature fusion enhances visual quality.

Result: The method generates sharp, geometrically consistent novel-view images from a single reference view, preserving fine-grained details and structural fidelity.

Conclusion: Blur2Sharp consistently surpasses state-of-the-art techniques in novel pose and view generation tasks, excelling in challenging scenarios like loose clothing and occlusions.
...
TL;DR: The paper introduces Visual Knowledge Base (VKB) and VisKnow framework for structured multi-modal object understanding, demonstrated via AnimalKB, enhancing tasks like zero-shot recognition and VQA.


<details>
  <summary>Details</summary>
Motivation: To address the lack of systematically organized multi-modal data for comprehensive object understanding.

Method: Proposes VisKnow framework integrating text and image-source knowledge with region annotations via expert design and large-scale models.

Result: Constructed AnimalKB with 406 animal categories, 22K textual triplets, 420K images, improving tasks like zero-shot recognition and VQA.

Conclusion: Automated construction of visual knowledge bases like AnimalKB advances visual understanding and practical applications.
...
TL;DR: Explores prompt tuning methods in 3D object detection, proposing a Scene-Oriented Prompt Pool (SOP²) and demonstrating its effectiveness.


<details>
  <summary>Details</summary>
Motivation: To investigate if LLMs' prompt tuning techniques can be adapted to 3D object detection, using the Waymo dataset as a foundation.

Method: Examines prompt tokens and prompt generators, then proposes SOP² for 3D object detection.

Result: Shows effectiveness of prompt pools in 3D object detection.

Conclusion: Aims to inspire deeper exploration of prompts' potential in the 3D field.
...
TL;DR: Proposes three VVC profiles for feature compression in split inference, balancing efficiency and speed.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency of traditional video codecs for abstract, sparse, and task-specific intermediate features in split inference systems.

Method: The study involves a tool-level analysis of VVC for feature compression under the MPEG-AI FCM standard, evaluating impact on compression efficiency and downstream task accuracy.

Result: Fast profile: 2.96% BD-Rate gain, 21.8% encoding time reduction. Faster: 1.85% BD-Rate gain, 51.5% speedup. Fastest: 95.6% encoding time reduction, 1.71% BD-Rate loss.

Conclusion: The paper proposes three lightweight VVC profiles (Fast, Faster, Fastest) for compressing intermediate features in split inference systems, achieving significant improvements in encoding time with minimal BD-Rate loss.
...
TL;DR: MM-CoT benchmarks visual grounding and logical coherence in multimodal models, exposing gaps in current reasoning capabilities.


<details>
  <summary>Details</summary>
Motivation: To assess whether reasoning in multimodal models is genuinely grounded in visual evidence and logically coherent, addressing the neglect of verification in existing benchmarks.

Method: Introduction of MM-CoT, a diagnostic benchmark probing visual grounding and logical coherence in Chain-of-Thought reasoning for multimodal models.

Result: Leading vision-language models struggle on MM-CoT, revealing a discrepancy between generative fluency and true reasoning fidelity.

Conclusion: MM-CoT provides a foundation for developing models that reason faithfully and coherently within the visual world, addressing the gap in evaluating visual grounding and logical coherence.
...
TL;DR: Proposes a normal-guided sparse depth sampling strategy for depth completion, improving accuracy and realism by leveraging PCA-based surface normal estimation.


<details>
  <summary>Details</summary>
Motivation: Addresses the unrealistic generation of sparse depth in current pipelines by considering geometry-dependent reliability.

Method: Uses PCA-based surface normal estimation on RGB-D point clouds to compute per-pixel depth reliability, guiding sparse depth sampling.

Result: Improves accuracy, reduces artifacts, and produces more realistic training conditions on NYU Depth v2.

Conclusion: The proposed method enhances depth completion by better reflecting real sensor behavior.
...
TL;DR: FastBEV++ is a framework that balances high performance and deployment efficiency in BEV perception, using a novel view transformation and depth-aware fusion.


<details>
  <summary>Details</summary>
Motivation: To reconcile the tension between state-of-the-art performance and on-vehicle deployment tractability in camera-only Bird's-Eye-View perception.

Method: The framework introduces a novel view transformation paradigm and integrates an end-to-end, depth-aware fusion mechanism.

Result: FastBEV++ achieves 0.359 NDS on the nuScenes benchmark and exceeds 134 FPS on automotive-grade hardware.

Conclusion: FastBEV++ presents a mature and scalable design philosophy for production autonomous systems, achieving high performance and deployment efficiency.
...
TL;DR: HTC-VLM introduces a hybrid framework for efficient and high-fidelity vision-language modeling, balancing semantics and appearance with significant compression.


<details>
  <summary>Details</summary>
Motivation: To address the quadratic computational costs and memory strain from feeding hundreds of visual patch tokens into LLMs, without losing high-level semantics or fine-grained details.

Method: HTC-VLM uses a hybrid framework with dual channels (continuous for fine-grained details and discrete for symbolic anchors) fused into a 580-token hybrid sequence and compressed into a single voco token.

Result: Achieves 87.2% performance retention across seven benchmarks, outperforming the leading continuous baseline at 81.0% with a 580-to-1 compression ratio.

Conclusion: HTC-VLM resolves the efficiency-fidelity dilemma in VLMs through a minimalist hybrid design, achieving high performance retention with significant compression.
...
TL;DR: A hybrid Residual-SwinCA-Net framework outperforms existing methods in breast lesion segmentation, achieving high accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges in extracting locally correlated and robust features, learning global dependencies, enhancing tissue continuity, ultrasound noise suppressions, and maintaining morphological integrity of malignant lesion contours.

Method: A novel deep hybrid Residual-SwinCA-Net segmentation framework incorporating residual CNN modules, Swin Transformer blocks, Laplacian-of-Gaussian regional operator, boundary-oriented operator, contraction strategy, Multi-Scale Channel Attention and Squeezing (MSCAS) module, and Pixel-Attention module.

Result: The framework achieved 99.29% mean accuracy, 98.74% IoU, and 0.9041 Dice for breast lesion segmentation on the BUSI dataset.

Conclusion: The proposed Residual-SwinCA-Net framework improves the BUSI lesion diagnostic performance and strengthens timely clinical decision-making.
...
TL;DR: FTKD enhances online 3D object detection by distilling future knowledge from offline models, improving accuracy and velocity estimation.


<details>
  <summary>Details</summary>
Motivation: To enable online models to effectively learn future knowledge from offline models, addressing the limitations of existing KD methods.

Method: Future Temporal Knowledge Distillation (FTKD) with future-aware feature reconstruction and future-guided logit distillation.

Result: Achieves up to 1.3 mAP and 1.3 NDS gains on nuScenes dataset.

Conclusion: FTKD achieves significant improvements in 3D object detection and velocity estimation on the nuScenes dataset without added inference cost.
...
TL;DR: A novel Query-aware Hub Prototype (QHP) learning method is proposed to address prototype bias in few-shot 3D point cloud semantic segmentation by modeling semantic correlations between support and query sets.


<details>
  <summary>Details</summary>
Motivation: Existing methods generate prototypes solely from the support set, leading to prototype bias and degraded performance due to overfitting and distribution shifts.

Method: The QHP method includes a Hub Prototype Generation (HPG) module for constructing query-relevant prototypes and a Prototype Distribution Optimization (PDO) module to refine prototypes.

Result: QHP achieves substantial performance gains over state-of-the-art methods on S3DIS and ScanNet datasets.

Conclusion: The proposed QHP method effectively narrows the semantic gap between prototypes and query sets, improving segmentation performance in FS-3DSeg.
...
TL;DR: SFP combines spatial and frequency priors for robust scene recovery, outperforming existing methods in real-world conditions.


<details>
  <summary>Details</summary>
Motivation: Existing methods are limited by reliance on single priors or synthetic data, prompting the need for a more generalized approach to handle real-world degradations.

Method: The method leverages spatial priors for transmission map estimation and frequency priors for adaptive enhancement, combined with a weighted fusion strategy.

Result: Extensive evaluations confirm the effectiveness and superiority of SFP in various degradation scenarios.

Conclusion: The proposed Spatial and Frequency Priors (SFP) method effectively integrates spatial and frequency domain insights for robust scene recovery, demonstrating superiority across diverse degradation conditions.
...
TL;DR: RLCNet is a deep learning framework for online calibration of multimodal sensors in autonomous vehicles, showing robust performance and practical deployment potential.


<details>
  <summary>Details</summary>
Motivation: Addressing challenges like mechanical vibrations and sensor drift in dynamic environments for reliable perception in autonomous vehicles.

Method: RLCNet, a novel end-to-end trainable deep learning framework incorporating weighted moving average and outlier rejection for dynamic adjustment of calibration parameters.

Result: Superior accuracy and robustness compared to existing methods, validated on real-world datasets.

Conclusion: RLCNet demonstrates robust performance and practical deployment potential for online calibration of LiDAR, RADAR, and camera sensors in autonomous vehicles.
...
TL;DR: EgoX framework generates egocentric videos from exocentric inputs using diffusion models and geometry-guided attention.


<details>
  <summary>Details</summary>
Motivation: Translating exocentric to egocentric videos is challenging due to pose variations and minimal view overlap.

Method: Uses pretrained video diffusion models with LoRA adaptation and a unified conditioning strategy.

Result: Achieves coherent and realistic egocentric video generation with scalability and robustness.

Conclusion: EgoX effectively bridges the gap between exocentric and egocentric perspectives with high fidelity.
...
TL;DR: PAVAS integrates physical reasoning into V2A generation, improving audio realism and outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: Most V2A models are appearance-driven, lacking consideration of physical factors shaping real-world sounds.

Method: Uses a Physics-Driven Audio Adapter (Phy-Adapter) with object-level physical parameters estimated by a Physical Parameter Estimator (PPE).

Result: PAVAS achieves superior performance in both quantitative and qualitative evaluations.

Conclusion: PAVAS outperforms existing V2A models by incorporating physical reasoning, producing more plausible and coherent audio.
