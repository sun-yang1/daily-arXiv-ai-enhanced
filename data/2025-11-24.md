...
TL;DR: This paper uses persistent homology to objectively analyze and differentiate artistic styles between artists and AI-generated art.


<details>
  <summary>Details</summary>
Motivation: Traditional art style identification relies on subjective human expertise, but mathematical tools can provide more structured and objective analysis.

Method: The authors apply persistent homology, a topological data analysis method, to quantify artistic style differences.

Result: Persistent homology can statistically differentiate between artists from different/same artistic movements and distinguish real artist works from AI-generated imitations.

Conclusion: Topological data analysis offers objective, interpretable insights for art classification, enhancing traditional art analysis methods.
...
TL;DR: A method using StyleGAN2 to generate diverse macaque facial expressions, overcoming data limitations through data augmentation, sample selection, and loss refinement, outperforming models using only original images.


<details>
  <summary>Details</summary>
Motivation: Limited training data quantity and variation for animal faces, especially facial expressions across individuals, makes generative AI challenging for species like macaques important in neuroscience and evolution research.

Method: StyleGAN2 enhanced with: 1) data augmentation via motion transfer to animate still images, 2) sample selection using latent representations for varied training data, 3) loss function refinement for subtle movement accuracy.

Result: Generates diverse facial expressions for multiple macaques, outperforming models using original images only; enables style-based image editing with parameters corresponding to facial movements.

Conclusion: Effectively disentangles motion components as style parameters, providing a valuable tool for macaque facial expression research with broader implications for animal face generation.
...
TL;DR: First large-scale dataset (PairHuman) and method (DHumanDiff) for high-quality dual-person portrait customization with enhanced facial consistency and personalized scene generation.


<details>
  <summary>Details</summary>
Motivation: Lack of benchmark dataset hinders progress in personalized dual-person portrait generation for applications like emotional memory preservation and wedding photography.

Method: Create PairHuman dataset with 100K+ images and rich metadata; develop DHumanDiff baseline with enhanced facial consistency and balanced person/scene generation.

Result: Produces highly customized portraits with superior visual quality tailored to human preferences.

Conclusion: PairHuman dataset and DHumanDiff method effectively address dual-person portrait generation challenges and advance the field.
...
TL;DR: An unsupervised autoencoder with CDF 97 wavelet transform effectively denoises mixed Gaussian-Poisson noise in neutron images, outperforming traditional methods like BM3D.


<details>
  <summary>Details</summary>
Motivation: Neutron images for ICF analysis are degraded by overlapping Gaussian and Poisson noise, which conventional filtering methods struggle to remove without losing details.

Method: Implementation of an unsupervised autoencoder using a Cohen-Daubechies-Feauveau (CDF 97) wavelet transform in the latent space for denoising.

Result: The network successfully denoises data, showing lower reconstruction error and better edge preservation compared to BM3D and other non-ML filters.

Conclusion: This ML approach is a promising advancement for noise reduction in neutron imaging and 3D reconstruction analysis of ICF experiments.
...
TL;DR: SAM 3 is a unified model for promptable object segmentation in images/videos, achieving double the accuracy of existing systems through a novel architecture and large-scale dataset.


<details>
  <summary>Details</summary>
Motivation: To create a more accurate and flexible system for detecting, segmenting, and tracking objects using various concept prompts (noun phrases, exemplars, or both).

Method: Uses Promptable Concept Segmentation (PCS) with an image-level detector and memory-based video tracker sharing a backbone, plus a presence head to decouple recognition and localization. Built with a scalable data engine producing 4M unique concept labels.

Result: SAM 3 doubles accuracy of existing systems in image/video PCS and improves previous SAM capabilities on segmentation tasks.

Conclusion: SAM 3 advances promptable concept segmentation significantly and is open-sourced along with the SA-Co benchmark.
...
TL;DR: SaFeR-CLIP is a fine-tuning method that redirects unsafe concepts to their closest safe alternatives, improving safety while maintaining model performance better than previous methods.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning vision-language models for safety often causes significant performance drops due to rigid alignment strategies that disrupt semantic structure.

Method: Proposes proximity-aware alignment that redirects unsafe concepts to semantically closest safe alternatives, implemented in the SaFeR-CLIP framework with minimal intervention principle.

Result: Recovers up to 8.0% in zero-shot accuracy over prior methods while maintaining robust safety; introduces NSFW-Caps benchmark for evaluation.

Conclusion: Respecting the geometry of pretrained representations is key to achieving safety without sacrificing performance.
...
TL;DR: Three-stage framework generates multi-view consistent SVGs from single SVG input using 3D lifting, spatial memory extension from SAM2, and path optimization.


<details>
  <summary>Details</summary>
Motivation: Multi-view consistent SVG generation from single-view input remains challenging despite SVG's importance in modern design workflows.

Method: 1) Rasterized input lifted to 3D and rendered under target camera poses 2) SAM2's temporal memory extended to spatial domain for part-level correspondences 3) Path consolidation and structural optimization during raster-to-vector conversion

Result: Produces SVGs with geometric/color consistency across views, reduced redundancy, preserved boundaries/semantics, and fine structural details.

Conclusion: Bridges generative modeling and structured vector representation, enabling scalable multi-view SVG generation for applications like asset creation and semantic vector editing.
...
TL;DR: Mesh RAG is a training-free plug-and-play framework that enhances autoregressive mesh generation by using retrieval-based component integration, improving quality, speed, and enabling incremental editing without retraining.


<details>
  <summary>Details</summary>
Motivation: Autoregressive mesh generation models face a quality-speed trade-off due to sequential dependency, making generation slow and incremental editing difficult.

Method: The approach leverages point cloud segmentation, spatial transformation, and point cloud registration to retrieve, generate, and integrate mesh components, decoupling sequential dependencies.

Result: Mesh RAG significantly enhances mesh quality, accelerates generation speed compared to sequential methods, and enables incremental editing across various models.

Conclusion: Mesh RAG overcomes limitations of autoregressive models by providing a flexible, efficient framework that improves performance without requiring model retraining.
...
TL;DR: WorldGen is an AI system that automatically generates interactive 3D worlds from text descriptions, combining LLMs, procedural generation, and diffusion models to create ready-to-use virtual environments.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between creative intent and functional virtual spaces by enabling automatic 3D world creation without requiring manual modeling or specialized 3D expertise.

Method: Combines LLM-driven scene layout reasoning, procedural generation, diffusion-based 3D generation, and object-aware scene decomposition in a modular architecture.

Result: Produces traversable, fully textured 3D environments that are geometrically consistent, visually rich, and efficient for real-time rendering in standard game engines.

Conclusion: WorldGen represents significant progress toward accessible, generative world-building at scale, advancing 3D generative AI for gaming, simulation, and immersive applications.
...
TL;DR: Introduces REO-Instruct, the first unified benchmark combining descriptive tasks and regression tasks for Earth Observation with vision-language models, using forest ecology scenarios to bridge qualitative understanding and quantitative prediction.


<details>
  <summary>Details</summary>
Motivation: Vision language models show strong perceptual abilities but lack application and benchmarks for scientific regression tasks in Earth Observation, motivating the need for a dataset that aligns multimodal perception with measurable biophysical variables.

Method: Development of REO-Instruct benchmark using a hybrid human-AI pipeline to integrate co-registered Sentinel-2 and ALOS-2 imagery with structured textual annotations, focusing on forest ecological logic chains involving human activity, land-cover, patch counting, and biomass regression.

Result: Evaluation of generic vision language models on REO-Instruct reveals challenges in numeric reasoning, indicating a significant gap for scientific VLMs to handle quantitative prediction tasks effectively.

Conclusion: REO-Instruct provides a standardized foundation to advance next-generation geospatial models for both descriptive and scientific inference, addressing key limitations in current EO datasets and model capabilities.
...
TL;DR: BOP-ASK is a new large-scale dataset addressing VLMs' weaknesses in fine-grained spatial reasoning, featuring 150k images and 33M QA pairs across six tasks for training and benchmarking.


<details>
  <summary>Details</summary>
Motivation: Existing spatial reasoning benchmarks for VLMs focus on high-level relationships but lack fine-grained understanding needed for real-world applications like 3D localization and object interactions.

Method: The dataset is generated using 6D object poses from BOP datasets, annotated with grasp poses, object relationships, trajectories, and spatial details.

Result: VLMs trained on BOP-ASK outperform baselines, showing improved capabilities in precise pose estimation, trajectory planning, and spatial reasoning in cluttered scenes.

Conclusion: BOP-ASK fills a critical gap in VLM evaluation and training for fine-grained spatial tasks, with public release of datasets and pipeline to advance research.
...
TL;DR: Parts-Mamba, a hybrid GCN-Mamba model, improves skeleton action recognition by capturing distant joint context, addressing performance drops from incomplete skeletons due to occlusions or missing frames.


<details>
  <summary>Details</summary>
Motivation: Existing GCN models struggle with incomplete skeletons caused by occlusions or poor data capture, as they rely heavily on local context which is disrupted when parts are missing.

Method: Propose Parts-Mamba, combining GCN with Mamba to capture part-specific information via parts-specific scanning and preserve non-neighboring joint context through a parts-body fusion module.

Result: Evaluated on NTU RGB+D 60 and 120 datasets under occlusion settings, achieving up to 12.9% accuracy improvement over existing models.

Conclusion: Parts-Mamba effectively enhances robustness in skeleton action recognition by maintaining contextual information from distant joints, outperforming current methods in imperfect data scenarios.
...
TL;DR: The paper extends Gromov-Wasserstein distance to Joint Gromov-Wasserstein (JGW) for multiple-to-multiple object matching, achieving better performance and efficiency than existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional GW distance is limited to pairwise matching of single objects, which restricts its use in applications requiring multiple-object matching.

Method: Introduces the JGW objective, extending GW framework with non-negative dissimilarity measure for partially isomorphic distributions, using Optimal Transport algorithms with entropic regularization.

Result: JGW shows superior accuracy and computational efficiency compared to other GW variants, with successful applications on synthetic and real-world datasets.

Conclusion: JGW provides an effective solution for complex multiple shape matching problems, with promising applications in computer graphics and structural biology.
...
TL;DR: REPA enhances inverse problem solving by aligning diffusion/flow model representations with pretrained visual encoders like DINOv2, improving reconstruction quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: Alignment between generative models and self-supervised encoders improves convergence and sample quality; extending this to inverse problems could enhance reconstruction fidelity where ground-truth is unavailable.

Method: Apply representation alignment (REPA) at inference to guide reconstruction by matching model representations with approximate target features from DINOv2.

Result: Theoretical analysis links REPA to divergence measures and representation steering; integrations show consistent improvements in super-resolution, inpainting, deblurring tasks with efficiency gains.

Conclusion: REPA provides a general, effective inductive bias for inverse problems, boosting perceptual realism and reducing computational steps without solver performance loss.
...
TL;DR: Proposes NFGlassNet for glass surface detection using flash/no-flash image pairs by exploiting reflection differences due to illumination variations.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on boundary or reflection cues but fail to use intrinsic glass properties. Authors observe that illumination differences across glass surfaces cause reflection dynamics, which can be leveraged for better detection.

Method: NFGlassNet uses a Reflection Contrast Mining Module (RCMM) to extract reflections and a Reflection Guided Attention Module (RGAM) to fuse reflection and glass surface features. Trained on a new dataset of 3.3K flash/no-flash image pairs.

Result: Extensive experiments show the method outperforms state-of-the-art techniques.

Conclusion: The approach effectively utilizes reflection dynamics for accurate glass surface detection, with code, model, and dataset to be released upon acceptance.
...
TL;DR: Introduces R-AVST dataset for audio-visual spatio-temporal reasoning and AVST-Zero model using RL, advancing complex video understanding beyond simple scenarios.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs focus on simple video scenarios but lack handling of complex real-world audio-visual events requiring spatio-temporal reasoning.

Method: Created R-AVST dataset via LLM-based object extraction, automatic annotation, and manual inspection; proposed AVST-Zero model using reinforcement learning with multi-dimensional rewards.

Result: R-AVST contains 5K videos with 27K objects and 8K QA pairs; AVST-Zero shows competitive performance in audio-visual reasoning tasks.

Conclusion: R-AVST is the first dataset for real-world audio-visual spatio-temporal reasoning, and AVST-Zero provides a novel approach for future challenges in this domain.
...
TL;DR: Introduces Warm Diffusion, a model combining noise and blurring to improve image generation by leveraging both high-frequency detail preservation and structural correlations.


<details>
  <summary>Details</summary>
Motivation: Traditional hot diffusion relies only on noise, missing image structure correlations, while cold diffusion uses only blurring, ignoring noise's role in data manifold shaping, causing performance issues.

Method: Proposes Warm Diffusion with Blur-Noise Mixture Diffusion Model (BNMD) to jointly control blur and noise, using a divide-and-conquer strategy that separates denoising and deblurring based on spectral dependencies.

Result: Extensive experiments validate the approach's effectiveness in image generation benchmarks, showing improved performance by balancing blur and noise.

Conclusion: Warm Diffusion successfully integrates the strengths of both paradigms, offering a unified solution that enhances generative modeling by optimizing the blur-to-noise trade-off.
...
TL;DR: Q-Real is a fine-grained dataset for evaluating realism and plausibility in AI-generated images, with annotations for 3,088 images, designed to enhance MLLM evaluation and training.


<details>
  <summary>Details</summary>
Motivation: Current AI-generated image quality assessments provide only single scores, which are too coarse to guide model improvement effectively, lacking evaluation along critical dimensions like realism and plausibility.

Method: Constructed Q-Real dataset with 3,088 AI-generated images, annotated with entity locations, judgment questions, and attributions for realism and plausibility; developed Q-Real Bench for MLLM evaluation and a fine-tuning framework.

Result: Experimental results confirm the dataset's high quality and significance, showing its effectiveness in benchmarking and improving MLLMs for fine-grained image evaluation.

Conclusion: Q-Real provides a comprehensive benchmark for fine-grained assessment, facilitating targeted improvements in generative models and advancing unified generation-understanding capabilities.
...
TL;DR: UniModel is a unified diffusion framework that handles both visual understanding and generation as pixel-to-pixel translations using painted text images in a shared visual space.


<details>
  <summary>Details</summary>
Motivation: To achieve unification along three axes: model, tasks, and representations - eliminating modality discrepancies by mapping both text and images into a shared visual space for fully vision-native multimodal learning.

Method: A single Unified Diffusion Transformer trained with rectified flow in pixel space, where text is rendered as painted text images and all inputs/outputs are RGB pixels. Lightweight task embeddings specify the direction of transformation.
...
TL;DR: DeltaDeno is a zero-shot anomaly generation method that uses diffusion model comparisons with minimal prompts to create realistic defects without training or real anomaly samples.


<details>
  <summary>Details</summary>
Motivation: Existing anomaly generation methods require fine-tuning with scarce anomaly samples, which contradicts the motivation for generation and risks overfitting to category priors.

Method: DeltaDeno contrasts two diffusion branches using a minimal prompt pair under a shared schedule, accumulates denoising deltas into a localization map for latent inpainting, and uses token-level prompt refinement and spatial attention bias for stability.

Result: Experiments show DeltaDeno achieves strong generation quality, realism, and improves downstream detection performance on public datasets.

Conclusion: DeltaDeno provides an effective training-free approach for zero-shot anomaly generation, addressing prior limitations and enhancing practical applications.
...
TL;DR: DGAF-VSR is a novel diffusion-based video super-resolution method that improves alignment and compensation using feature-domain processing and upscaled warping, achieving better perceptual quality, fidelity, and temporal consistency than existing methods.
...
Method: Proposes DGAF-VSR with an Optical Guided Warping Module (OGWM) to maintain high-frequency details in aligned features and a Feature-wise Temporal Condition Module (FTCM) for dense guidance in the feature domain, leveraging feature-domain compensation and upscaled resolution warping.

Result: Outperforms state-of-the-art methods with 35.82% DISTS reduction (perceptual quality), 0.20 dB PSNR gain (fidelity), and 30.37% tLPIPS reduction (temporal consistency) on synthetic and real-world datasets.

Conclusion: DGAF-VSR effectively addresses limitations in DM-based VSR by optimizing alignment and compensation in the feature domain, achieving superior performance in perceptual quality, fidelity, and temporal consistency.
...
TL;DR: A deep learning framework integrating semantic and shape awareness for accurate tooth segmentation in CBCT images, particularly addressing interdental adhesion challenges.
...
Method: Proposes a deep learning framework with target-tooth-centroid prompted multi-label learning to model semantic relationships between teeth and a tooth-shape-aware learning mechanism to enforce morphological constraints. Components are unified via multi-task learning.

Result: Extensive evaluations on internal and external datasets demonstrate significant outperformance over existing methods.
...
TL;DR: The paper introduces OmniGround, a new benchmark for Spatio-Temporal Video Grounding to address limitations in existing models, and proposes PG-TAF, a training-free framework that significantly improves performance.


<details>
  <summary>Details</summary>
Motivation: Current models for Spatio-Temporal Video Grounding struggle with real-world scenes due to limited benchmark scope, leading to category bias, oversimplified reasoning, and poor linguistic robustness.

Method: The authors introduce the OmniGround benchmark with 3,475 videos and complex queries, created using a Forward-Backward-Refinement annotation pipeline. They also propose PG-TAF, a training-free two-stage framework that decomposes STVG into temporal grounding and spatio-temporal propagation.

Result: Evaluations show a 10.4% performance drop on complex scenes. PG-TAF achieves 25.6% and 35.6% improvements in m_tIoU and m_vIoU on OmniGround, with consistent gains across benchmarks.

Conclusion: The proposed benchmark and framework effectively address limitations in STVG, demonstrating significant advancements in handling diverse objects and complex queries.
...
TL;DR: MultiPriv is the first benchmark designed to evaluate individual-level privacy reasoning in Vision-Language Models (VLMs), addressing risks of inferring and linking distributed information to construct profiles. It includes a bilingual dataset with synthetic profiles and nine tasks, revealing significant privacy risks in VLMs that current benchmarks and safety measures fail to address.


<details>
  <summary>Details</summary>
Motivation: Current privacy benchmarks for VLMs are insufficient as they focus on attribute perception but overlook the critical risk of privacy reasoning, where models can link scattered information to identify individuals.

Method: The authors propose the Privacy Perception and Reasoning (PPR) framework and create a bilingual multimodal dataset with synthetic individual profiles where identifiers like faces and names are linked to sensitive attributes. This supports nine tasks evaluating attributes from detection to cross-image re-identification.

Result: Evaluation of over 50 VLMs shows significant unmeasured reasoning-based privacy risks, perception metrics are poor predictors of these risks, and existing safety alignments are ineffective against reasoning attacks.

Conclusion: MultiPriv highlights systemic vulnerabilities in VLMs and provides a framework for developing more privacy-preserving models, underscoring the need for better evaluation methods.
...
TL;DR: Novel implicit neural representation framework jointly models dynamic MRI images and motion fields using physics-informed constraints, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Dynamic MRI reconstruction is challenged by limited sampling and motion artifacts. Conventional methods rely on inaccurate pre-estimated optical flow, especially under undersampling.

Method: Uses two implicit neural representations - one for spatiotemporal image content, one for optical flow - coupled via optical flow equation as physics regularization, plus data consistency with k-space measurements.

Result: Superior performance on cardiac MRI datasets compared to state-of-the-art methods, achieving better reconstruction quality, accurate motion estimation, and improved temporal fidelity.

Conclusion: Implicit joint modeling with flow-regularized constraints shows strong potential for advancing dynamic MRI reconstruction.
...
TL;DR: Proposes FingerCap for generating fine-grained finger-level hand motion captions from videos, introduces FingerCap-40K dataset and HandJudge evaluation, and develops FiGOP method to address temporal sparsity in Video-MLLMs.


<details>
  <summary>Details</summary>
Motivation: Understanding detailed human hand motion is crucial for visual perception, embodied intelligence, and communication, but current methods lack fine finger-level semantic capture.

Method: FiGOP (Finger Group-of-Pictures) pairs RGB keyframes with subsequent hand keypoints, using a lightweight temporal encoder to integrate motion embeddings with RGB features to recover fine temporal cues.

Result: Experiments show existing Video-MLLMs struggle with finger-level reasoning, while FiGOP-augmented models achieve consistent gains in HandJudge and human evaluations on FingerCap-40K.

Conclusion: FiGOP effectively enhances fine-grained hand motion captioning by addressing temporal sparsity, demonstrating superiority over baseline models in capturing detailed finger semantics.
...
TL;DR: A novel two-branch framework for point-supervised facial expression spotting that uses soft pseudo-labeling and intensity modeling to reduce annotation costs while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing facial expression spotting methods require expensive temporal boundary annotations, creating a need for more efficient training approaches using only single timestamp labels per instance.

Method: Proposes a Gaussian-based instance-adaptive intensity modeling module for soft pseudo-labeling and a two-branch framework with class-agnostic intensity detection and class-aware apex classification branches.

Result: Extensive experiments on SAMM-LV, CAS(ME)^2, and CAS(ME)^3 datasets demonstrate the framework's effectiveness in spotting facial expressions with minimal annotation requirements.

Conclusion: The proposed point-supervised approach successfully reduces annotation burden while maintaining competitive performance, offering a practical solution for real-world facial expression analysis applications.
...
TL;DR: Neighbor GRPO is a new alignment method that avoids SDE conversion by using initial noise perturbations and distance-based optimization, achieving better efficiency and quality than SDE-based GRPO.


<details>
  <summary>Details</summary>
Motivation: SDE-based GRPO has issues with inefficient credit assignment and incompatibility with high-order solvers when applied to flow matching models.

Method: Generate diverse trajectories by perturbing ODE initial noise, optimize using softmax distance-based surrogate leaping policy, with symmetric anchor sampling and group-wise quasi-norm reweighting.

Result: Significantly outperforms SDE-based GRPO in training cost, convergence speed, and generation quality.

Conclusion: Neighbor GRPO provides an effective alternative to SDE conversion, preserving ODE advantages while improving alignment performance.
...
TL;DR: MatPedia is a foundation model using a joint RGB-PBR representation to enable unified material synthesis for tasks like text-to-material generation, outperforming existing methods in quality and diversity.


<details>
  <summary>Details</summary>
Motivation: Current PBR material creation is labor-intensive, and generative models lack a unified representation that bridges natural image appearance with PBR properties, leading to fragmented pipelines.

Method: MatPedia encodes materials into two interdependent latents (RGB appearance and PBR maps) as a 5-frame sequence, using video diffusion architectures to capture correlations and transfer priors from RGB models.

Result: Trained on MatHybrid-410K dataset, MatPedia achieves native 1024x1024 synthesis that substantially surpasses existing approaches in quality and diversity.

Conclusion: The joint representation enables a unified framework for multiple material tasks within a single architecture, advancing photorealistic graphics.
...
TL;DR: A dual branch network extracts blur and noise embeddings for blind super-resolution, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing super-resolution methods perform poorly when the actual image degradation differs from the assumed fixed degradation.

Method: Proposes unsupervised Dual Branch Degradation Extractor Network to predict blur and noise embeddings separately, then adapts SR network accordingly.

Result: Extensive experiments show the method achieves state-of-the-art performance on multiple benchmarks for blind super-resolution.

Conclusion: The approach effectively handles blind super-resolution by explicitly modeling blur and noise degradations through separate embeddings.
