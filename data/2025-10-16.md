...
TL;DR: SimULi is a real-time neural rendering method for arbitrary camera models and LiDAR data, improving speed, cross-sensor consistency, and fidelity over existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing neural rendering methods are limited by low rendering speeds, support for only pinhole camera models, and cross-sensor inconsistencies, which hinder their application in autonomous robot testing.

Method: The method extends 3DGUT with LiDAR support through automated tiling for spinning LiDAR models and ray-based culling. It also introduces a factorized 3D Gaussian representation and anchoring strategy to reduce cross-sensor inconsistencies.

Result: SimULi renders 10-20x faster than ray tracing approaches and 1.5-10x faster than prior rasterization-based methods. It also reduces mean camera and depth error by up to 40% and matches or exceeds state-of-the-art fidelity on benchmark datasets.

Conclusion: SimULi successfully addresses the limitations of existing neural rendering methods by enabling real-time rendering of arbitrary camera models and LiDAR data, while also improving cross-sensor consistency and rendering speed.
...
TL;DR: The paper introduces SurgFUTR, a state-change learning framework for surgical future prediction, improving accuracy and generalizability across diverse procedures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of current surgical AI research, which focuses on understanding present events rather than predicting future ones, lacks unified approaches for short- and long-term predictions, and struggles with generalization across different surgical contexts.

Method: The method involves a teacher-student architecture where video clips are compressed into state representations using Sinkhorn-Knopp clustering. The teacher learns from current and future clips, while the student predicts future states from current videos alone, guided by the Action Dynamics (ActDyn) module.

Result: Experiments across four datasets and three procedures demonstrate consistent improvements in prediction accuracy. Cross-procedure transfer validates the generalizability of the approach.

Conclusion: The paper concludes that reframing surgical future prediction as state-change learning, implemented through the SurgFUTR framework, significantly improves prediction accuracy and generalizability across diverse surgical contexts and procedures.
...
TL;DR: TMPS, a metric learning framework, improves plant disease diagnosis robustness by leveraging limited target domain samples, outperforming existing methods with significant F1 score gains.


<details>
  <summary>Details</summary>
Motivation: Deep learning-based plant disease diagnosis systems often fail to generalize to images taken under different conditions from the training environment due to limited training data diversity and domain gaps.

Method: Target-Aware Metric Learning with Prioritized Sampling (TMPS), a metric learning-based framework that utilizes limited labeled samples from the target domain to enhance model generalization.

Result: TMPS achieves average macro F1 score improvements of 7.3 and 3.6 points over models trained with combined source and target samples and fine-tuned models, respectively, and 18.7 and 17.1 points over baseline and conventional metric learning.

Conclusion: The proposed TMPS framework significantly improves diagnostic robustness in plant disease diagnosis by effectively leveraging limited labeled samples from the target domain, outperforming existing methods.
...
TL;DR: ViZer is a framework for zero-label learning in image captioning, improving VLMs without labeled data by aligning vision-language features.


<details>
  <summary>Details</summary>
Motivation: The reliance on labeled datasets limits scalability and underutilizes unlabeled data, prompting the need for zero-label learning solutions.

Method: ViZer aligns vision and language representation features during training, eliminating the need for text labels or full retraining.

Result: ViZer produces more grounded and descriptive captions, outperforming baselines in qualitative evaluations.

Conclusion: ViZer offers a practical approach for zero-label adaptation in vision-language tasks, enhancing VLMs without labeled data.
...
TL;DR: FetalMind, a tailored AI system for fetal ultrasound, uses SED and a large dataset to improve report generation and diagnosis, outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: Existing medical vision-language models underperform in fetal ultrasound due to challenges like multi-view reasoning, diverse diseases, and image variability. FetalMind aims to bridge this gap.

Method: The paper introduces Salient Epistemic Disentanglement (SED), which uses an expert-curated bipartite graph and reinforcement learning to decouple view-disease associations and align inference with clinical practice.

Result: FetalMind outperforms baselines with +14% average gains and +61.2% higher accuracy on critical conditions, trained on the FetalSigma-1M dataset.

Conclusion: FetalMind demonstrates superior performance in fetal ultrasound tasks, achieving significant gains in accuracy and efficiency while aligning with clinical workflows.
...
TL;DR: CADE 2.5 enhances SD/SDXL models with ZeResFDG for better guidance and QSilk for stabilization, improving sharpness and robustness without retraining.


<details>
  <summary>Details</summary>
Motivation: To improve the performance of SD/SDXL latent diffusion models by enhancing sharpness, prompt adherence, and artifact control without requiring retraining.

Method: The paper introduces ZeResFDG, a module combining frequency-decoupled guidance, energy rescaling, and zero-projection, alongside a spectral EMA for adaptive sampling. It also includes QSilk Micrograin Stabilizer for inference-time stabilization.

Result: ZeResFDG improves model outputs across samplers, and QSilk stabilizes high-resolution results with minimal overhead.

Conclusion: CADE 2.5, with its ZeResFDG module and QSilk Micrograin Stabilizer, enhances sharpness, prompt adherence, and artifact control in SD/SDXL latent diffusion models without retraining, while maintaining robustness and natural high-frequency micro-texture.
...
TL;DR: SCOPE dynamically selects specialized encoders for VLMs, reducing compute costs while outperforming brute-force aggregation.


<details>
  <summary>Details</summary>
Motivation: Naively stacking multiple vision encoders in VLMs leads to diminishing returns and high inference costs. The goal is to improve efficiency and performance by dynamically selecting specialized encoders per image-text pair.

Method: SCOPE uses a Mixture-of-Encoders (MoEnc) framework with dynamic encoder selection via instance-level routing. A lightweight router employs cross-attention between text prompts and shared visual features to choose the optimal encoder. Training involves dual entropy regularization with auxiliary losses.

Result: SCOPE with one shared and one routed encoder outperforms models using all four extra encoders simultaneously, reducing compute by 24-49%.

Conclusion: SCOPE demonstrates that intelligent encoder selection outperforms brute-force aggregation in multi-encoder VLMs, reducing compute costs while maintaining performance.
...
TL;DR: The paper introduces SVAG, a task for detecting, tracking, and temporally localizing objects in videos based on natural language descriptions, and proposes SVAGFormer and SVAGEval as solutions, showing current models' limitations in complex scenes.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in jointly detecting and tracking multiple objects based on their actions while grounding them temporally, which is overlooked by current methods focusing on coarse-grained action recognition or generic object tracking.

Method: The authors introduce SVAGFormer, a baseline framework adapting state-of-the-art vision-language models for joint spatial and temporal grounding, and SVAGEval, a standardized evaluation toolkit.

Result: Empirical results demonstrate poor performance of existing models on SVAG, particularly in dense or complex scenes.

Conclusion: The paper concludes that existing models struggle with the SVAG task, especially in dense or complex scenes, highlighting the necessity for advanced reasoning in fine-grained object-action interactions in long videos.
...
TL;DR: SeqBench is a new benchmark for evaluating narrative coherence in T2V generation, using a DTG-based metric to address gaps in current models.


<details>
  <summary>Details</summary>
Motivation: Existing T2V benchmarks focus on visual quality but lack evaluation of narrative coherence over extended sequences, creating a gap in assessing logical progression through multiple events.

Method: The paper introduces SeqBench, a benchmark with 320 prompts and 2,560 human-annotated videos, and a Dynamic Temporal Graphs (DTG)-based metric for evaluating long-range dependencies and temporal ordering.

Result: The DTG-based metric correlates strongly with human annotations, revealing limitations in current T2V models, such as inconsistent object states, implausible results, and timing issues.

Conclusion: SeqBench provides a systematic framework for evaluating narrative coherence in T2V generation, highlighting limitations in current models and offering insights for future improvements.
...
TL;DR: SceneAdapt is a framework that integrates scene awareness into text-conditioned motion models by leveraging disjoint datasets through two adaptation stages: inbetweening and scene-aware inbetweening.


<details>
  <summary>Details</summary>
Motivation: Existing motion generation approaches lack integration of motion semantics and scene-awareness due to the challenge of creating large-scale datasets with both.

Method: SceneAdapt uses motion inbetweening as a proxy task to bridge datasets, employing keyframing layers and a scene-conditioning layer with cross-attention.

Result: SceneAdapt successfully injects scene awareness into text-to-motion models, with experimental results validating its effectiveness.

Conclusion: The framework demonstrates a novel way to combine motion semantics and scene-awareness, with potential for broader applications.
...
TL;DR: A hybrid Mamba-based model outperforms traditional deep learning methods in ECG classification, achieving higher AUPRC and AUROC scores, supporting early diagnosis and telemedicine.


<details>
  <summary>Details</summary>
Motivation: Traditional deep learning models like residual networks and transformers have limited performance with long sequential ECG signals, prompting the need for more efficient alternatives like state space models.

Method: A hybrid framework combining convolutional feature extraction with Mamba, a selective state space model, is introduced. The model is built upon Vision Mamba, a bidirectional variant for enhanced temporal dependency representation.

Result: The proposed model achieved higher AUPRC and AUROC scores than existing methods on twelve-lead electrocardiograms, as validated on PhysioNet Challenges 2020 and 2021 datasets.

Conclusion: The study demonstrates the potential of Mamba-based architectures, specifically the One Dimensional Convolutional Neural Network Electrocardiogram Mamba, to advance reliable ECG classification, supporting early diagnosis and personalized treatment.
...
TL;DR: XFactor is a geometry-free self-supervised model for novel view synthesis, achieving transferable pose representations without 3D biases, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: The key criterion for true NVS is transferability of pose representations across scenes, which prior self-supervised NVS models fail to achieve.

Method: XFactor combines pair-wise pose estimation with a simple augmentation scheme to disentangle camera pose from scene content, enabling geometric reasoning without 3D biases.

Result: XFactor achieves transferability with unconstrained latent pose variables and outperforms prior pose-free NVS transformers, as demonstrated by a new transferability metric and large-scale experiments.

Conclusion: XFactor is the first geometry-free self-supervised model capable of true novel view synthesis (NVS), achieving transferability without 3D inductive biases or explicit pose parameterization. Its latent poses correlate highly with real-world poses, outperforming prior pose-free NVS transformers.
...
TL;DR: A new direction-aware gradient loss improves edge fidelity in image fusion by preserving directional information across scales, outperforming traditional methods without changing model architectures.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based approaches collapse gradients to their magnitude, removing directional information and leading to ambiguous supervision and suboptimal edge fidelity.

Method: The paper introduces a direction-aware, multi-scale gradient loss that supervises horizontal and vertical gradient components separately while preserving their sign across scales.

Result: Experiments on open-source models and public benchmarks confirm the effectiveness of the approach in producing sharper, better-aligned edges and richer texture preservation.

Conclusion: The proposed direction-aware, multi-scale gradient loss significantly improves edge fidelity and texture preservation in infrared and visible image fusion without altering model architectures or training protocols.
...
TL;DR: A novel unsupervised domain adaptation framework improves hippocampus segmentation in MRI by addressing domain shifts through style harmonization and bidirectional deformable image registration, outperforming baselines by up to 15% in Dice score.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for medical image segmentation struggle with domain shifts (style and content variations) across datasets, especially in cross-domain hippocampus segmentation.

Method: Combines z-normalisation for style harmonisation with a bidirectional deformable image registration (DIR) strategy, jointly trained with segmentation and discriminator networks to align source images to the target domain.

Result: Outperforms existing baselines, achieving up to 15% relative improvement in Dice score, particularly in scenarios with significant content shift.

Conclusion: The framework effectively addresses domain shifts, enabling accurate hippocampus segmentation across diverse populations.
...
TL;DR: The paper introduces a method to quantify counting hallucinations in diffusion models, using a new dataset and evaluation protocol, and highlights the limitations of current metrics like FID.


<details>
  <summary>Details</summary>
Motivation: The lack of feasible methodologies for quantifying hallucinations in diffusion probabilistic models (DPMs) hinders progress in addressing this challenge and obscures pathways for designing next-generation generative models under factual constraints.

Method: The authors construct a dataset suite (CountHalluSet) with well-defined counting criteria and develop a standardized evaluation protocol to quantify counting hallucinations. They also analyze the correlation between sampling conditions in DPMs and common evaluation metrics like FID.

Result: The study reveals that common evaluation metrics like FID fail to consistently capture counting hallucinations. It also examines how different sampling conditions in DPMs affect hallucination levels.

Conclusion: This work aims to systematically quantify hallucinations in diffusion models, particularly counting hallucinations, and provides insights for future research in hallucination phenomena in image generation.
...
TL;DR: Edit-Your-Interest is a lightweight, text-driven video editing method that improves efficiency and visual fidelity by using spatio-temporal feature memory and propagation techniques.


<details>
  <summary>Details</summary>
Motivation: Existing video editing methods suffer from high computational overhead, memory consumption, and visual fidelity issues like temporal inconsistencies and artifacts.

Method: The method introduces a Spatio-Temporal Feature Memory bank (SFM) for efficient caching, Feature Most-Similar Propagation (FMP) for temporal consistency, and an SFM update algorithm for long-term relevance. Cross-attention maps are used for fine-grained control.

Result: Extensive experiments show that Edit-Your-Interest achieves better efficiency and visual fidelity compared to existing methods.

Conclusion: Edit-Your-Interest outperforms state-of-the-art methods in efficiency and visual fidelity, proving its superior effectiveness and practicality.
...
TL;DR: The paper introduces EgoSocial, a dataset for benchmarking AI intervention in social interactions, and EgoSoD, a method to improve AI's social awareness and intervention timing.


<details>
  <summary>Details</summary>
Motivation: Current AI lacks social awareness, leading to disruptive responses in conversations. The need for AI to understand human social dynamics from an egocentric perspective is growing.

Method: The authors created EgoSocial, a dataset with 13,500 video-question pairs, and proposed EgoSoD, an end-to-end method integrating multimodal cues into a social thinking graph.

Result: EgoSoD significantly improved intervention timing and social interaction performance for AI models like Phi-4 and Gemini 2.5 Pro.

Conclusion: The work addresses a critical gap in AI's social awareness, offering a dataset and method to enhance intervention timing in social interactions.
...
TL;DR: DriveCritic introduces a dataset and VLM-based evaluator to improve context-aware benchmarking of autonomous driving planners, outperforming existing metrics in human preference alignment.


<details>
  <summary>Details</summary>
Motivation: Current metrics like EPDMS lack context awareness in nuanced scenarios, making it difficult to benchmark autonomous driving planners in alignment with human judgment.

Method: The DriveCritic framework includes a curated dataset of challenging scenarios annotated with human preferences and a Vision-Language Model (VLM) evaluator fine-tuned via a two-stage supervised and reinforcement learning pipeline.
...
Conclusion: DriveCritic provides a more reliable, human-aligned foundation for evaluating autonomous driving systems by outperforming existing metrics in context awareness and human preference matching.
...
TL;DR: VPreg is a new diffeomorphic image registration method that improves accuracy and transformation quality, outperforming existing methods in brain scan registrations.


<details>
  <summary>Details</summary>
Motivation: To enhance registration accuracy and ensure high-quality transformations, particularly for neuroimaging workflows, by addressing limitations of conventional methods.

Method: VPreg uses a grid generation approach called Variational Principle (VP) to create non-folding grids with specific Jacobian determinant and curl, ensuring diffeomorphic transformations.

Result: VPreg outperforms ANTs-SyN, Freesurfer-Easyreg, and FSL-Fnirt in Dice scores, transformation regularity, and inverse map accuracy for brain scans from the OASIS-1 dataset.

Conclusion: VPreg is a superior method for diffeomorphic image registration, offering improved accuracy, transformation quality, and inverse map consistency compared to state-of-the-art techniques.
...
TL;DR: A novel method (OS-HGAdapter) uses LLM and hypergraph adapters to bridge text-image entropy gaps, achieving significant retrieval improvements on Flickr30K and MS-COCO.


<details>
  <summary>Details</summary>
Motivation: The challenge lies in the inherent entropy difference between text and images, leading to imbalanced retrieval performance. The goal is to leverage LLM's open semantic knowledge to bridge this gap and improve alignment.

Method: The method involves a two-step process: 1) using a new prompt template with LLM to enhance text modality polysemy, and 2) employing a hypergraph adapter to correct matching errors and reduce noise in the embedding space.

Result: OS-HGAdapter achieves 16.8% (text-to-image) and 40.1% (image-to-text) retrieval gains over existing methods, setting new benchmarks in semantic alignment.

Conclusion: The proposed Open Semantic Hypergraph Adapter (OS-HGAdapter) significantly improves cross-modal retrieval performance, achieving state-of-the-art results on Flickr30K and MS-COCO benchmarks.
...
TL;DR: 3D CNNs and LSTMs are compared for ASL recognition; 3D CNNs are more accurate but slower, while LSTMs are faster but less accurate. Hybrid models suggest context-dependent architecture choices are key.


<details>
  <summary>Details</summary>
Motivation: The research aims to compare the performance of 3D CNNs and LSTMs for real-time ASL recognition, addressing the trade-offs between accuracy and computational efficiency in edge computing environments.

Method: The study evaluates 3D CNNs and LSTM networks on a dataset of 1,200 ASL signs across 50 classes, comparing accuracy, computational efficiency, and latency. A hybrid 3D CNN-LSTM model is also tested.

Result: 3D CNNs achieve 92.4% accuracy but require more processing time, while LSTMs maintain 86.7% accuracy with lower resource consumption. The hybrid model shows decent performance.

Conclusion: The study concludes that context-dependent architecture selection is crucial for practical implementation of ASL recognition systems, balancing recognition precision and real-time operational requirements.
...
TL;DR: The paper introduces a novel perceptual design for steganography, improving capacity (100 to 500 bits) and accuracy (1 failure bit out of 2000), while maintaining high visual quality.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve existing capacity limits and accuracy in steganography for visual mediums like metadata and watermarking.

Method: The authors trained models using efficient latent representations and foveated rendering to enhance steganographic performance.

Result: The results show a capacity increase from 100 to 500 bits, better accuracy (1 failure bit out of 2000 at 200K test bits), and high visual quality (31.47 dB PSNR and 0.13 LPIPS).

Conclusion: The paper demonstrates the effectiveness of a novel perceptual design in steganography, achieving high visual quality and improved capacity limits.
...
TL;DR: The paper proposes DP-TTA, a method to improve TEM signal denoising by leveraging intrinsic physical characteristics for test-time adaptation, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning-based TEM denoising models fail to generalize across different geographical regions due to varying noise characteristics.

Method: The proposed DP-TTA uses dictionary learning to encode intrinsic TEM signal characteristics (e.g., exponential decay) as prior knowledge, guiding dynamic parameter adjustment during testing via self-supervised losses.

Result: DP-TTA achieves superior denoising performance compared to existing TEM and TTA methods.

Conclusion: The method effectively adapts pre-trained models to new environments, enhancing denoising accuracy by leveraging consistent physical properties of TEM signals.
...
TL;DR: The paper proposes STT-GS, a novel edge Gaussian splatting strategy, using FDC and PTTM for efficient sampling and JCSPC for resource optimization, outperforming benchmarks in GS quality and communication efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional edge resource management methods are inapplicable for maximizing Gaussian splatting (GS) qualities in scene reconstruction. The paper addresses this gap by focusing on heterogeneous view contributions of clients and resolving the causality dilemma in evaluating the GS-oriented objective function.

Method: The paper introduces a sample-then-transmit EGS (STT-GS) strategy, utilizing feature-domain clustering (FDC) and pilot transmission time minimization (PTTM) for efficient sampling. A joint client selection and power control (JCSPC) framework is developed, solved via the penalty alternating majorization minimization (PAMM) algorithm.

Result: Experiments show the proposed scheme outperforms benchmarks, accurately predicting GS-oriented objectives with low sampling ratios (e.g., 10%) and achieving a tradeoff between view contributions and communication costs.

Conclusion: The proposed STT-GS strategy, incorporating FDC and PTTM, along with the JCSPC framework, significantly outperforms existing benchmarks by accurately predicting GS-oriented objectives with low sampling ratios and balancing view contributions with communication costs.
...
TL;DR: CIGOcc improves 3D occupancy prediction by fusing multi-level features from 2D images and using SAM knowledge, achieving top results on SemanticKITTI without extra training costs.


<details>
  <summary>Details</summary>
Motivation: Existing methods for camera-based occupancy prediction focus on structural modifications, leaving the rich diversity of features in 2D images underutilized. This paper explores representation fusion to improve performance.

Method: CIGOcc is a two-stage occupancy prediction framework that extracts segmentation, graphics, and depth features from 2D images and uses a deformable multi-level fusion mechanism to combine these features. It also incorporates knowledge distilled from SAM.

Result: CIGOcc achieves state-of-the-art performance on the SemanticKITTI benchmark.

Conclusion: CIGOcc achieves state-of-the-art performance on the SemanticKITTI benchmark by leveraging multi-level representation fusion and knowledge distillation from SAM, without increasing training costs.
...
TL;DR: Paper Copilot addresses peer-review challenges by archiving reviews, providing an open dataset, and analyzing ICLR reviews to improve transparency and reliability.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges in the peer-review system, such as heavy workloads, inconsistent standards, and lack of transparency, which hinder the quality and reliability of reviews.

Method: The authors introduce Paper Copilot, a system that archives peer reviews across computer-science venues, providing an open dataset for large-scale analysis. They conduct an empirical analysis of ICLR reviews over multiple years.

Result: The results include the creation of a durable digital archive of peer reviews, an open dataset for research, and empirical insights into the evolution of peer review practices.

Conclusion: The paper concludes by highlighting the potential of Paper Copilot to improve the peer-review system through transparency, reproducibility, and evidence-based insights, aiming for a more robust and reliable process.
...
TL;DR: MimicParts enhances stylized 3D human motion generation by capturing regional style differences and dynamically adapting to speech rhythm and emotion, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current methods oversimplify stylistic diversity or ignore regional motion style differences, limiting motion realism. Additionally, they often overlook the dynamic adaptation of motion styles to speech rhythm and emotion.

Method: The proposed MimicParts framework divides the body into regions to encode localized motion styles and uses a part-aware attention block to dynamically adapt motion styles to speech rhythm and emotion cues.

Result: Experimental results demonstrate that MimicParts outperforms existing methods in generating natural and expressive 3D human motion sequences.

Conclusion: MimicParts successfully addresses the limitations of current methods by introducing part-aware style injection and denoising, resulting in more natural and expressive 3D human motion sequences.
...
TL;DR: The survey clarifies Visual Prompting (VP) and Visual Prompt Tuning (VPT) by unifying them under Prompt-based Adaptation (PA), categorizes methods, and explores applications across domains, providing a roadmap for future research.


<details>
  <summary>Details</summary>
Motivation: The motivation is to clarify the blurred conceptual boundaries between VP and VPT, which are often used interchangeably, and to provide a systematic distinction between these techniques and their applications.

Method: The survey revisits the designs of Visual Prompting (VP) and Visual Prompt Tuning (VPT) from first principles, conceptualizing them within a unified framework termed Prompt-based Adaptation (PA). It categorizes existing methods into learnable, generative, and non-learnable prompts, and organizes them by injection granularity (pixel-level and token-level).

Result: The survey examines PA's integrations across diverse domains (medical imaging, 3D point clouds, vision-language tasks), its role in test-time adaptation and trustworthy AI, and summarizes current benchmarks.

Conclusion: The survey provides a comprehensive roadmap for understanding and exploring Prompt-based Adaptation (PA), highlighting its distinct characteristics and applications across diverse domains. It also identifies key challenges and future directions for PA-related research.
...
TL;DR: A sample-centric multi-task learning framework improves defect inspection by jointly optimizing sample-level classification and pixel-level localization, addressing challenges like defect sparsity and low contrast.


<details>
  <summary>Details</summary>
Motivation: Existing models achieve strong pixel-overlap metrics but lack stability at the sample level due to foreground-background imbalance, defect sparsity, and low contrast, hindering deployment.

Method: A shared-encoder architecture jointly learns sample-level defect classification and pixel-level mask localization, with sample-level supervision modulating feature distribution and boosting recall for small or low-contrast defects.

Result: Experiments on benchmark datasets show substantial improvements in sample-level decision reliability and defect localization completeness.

Conclusion: The proposed sample-centric multi-task learning framework significantly improves the reliability of sample-level decisions and defect localization completeness, addressing the mismatch between optimization objectives and QC decision granularity.
...
TL;DR: The paper addresses the affirmative bias in VLMs by introducing CoVAND, a negation dataset pipeline, and NegToMe, a token merging module, improving negation understanding in detection tasks.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art vision-language models (VLMs) suffer from a critical failure in understanding negation, particularly in described object detection (DOD) tasks, referred to as affirmative bias. This limitation hinders their real-world applicability.

Method: The authors propose two contributions: (1) CoVAND, a dataset pipeline for generating high-quality negation data, and (2) NegToMe, a lightweight text token merging module that addresses the architectural cause of affirmative bias. NegToMe is integrated with a parameter-efficient LoRA fine-tuning approach.

Result: The method significantly improves performance on negation benchmarks, reducing the false positive rate and boosting NMS-AP by up to +10.8 points on OVDEval. It also generalizes to state-of-the-art VLMs.

Conclusion: The paper marks a crucial step forward in addressing negation understanding for real-world detection applications, demonstrating significant improvements in performance on challenging negation benchmarks.
