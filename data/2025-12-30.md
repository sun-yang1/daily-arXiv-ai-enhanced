...
TL;DR: The paper systematically analyzes how motion and appearance are encoded in video diffusion models, identifying distinct timestep regimes for each. This simplifies motion customization and enhances existing methods without extra components.


<details>
  <summary>Details</summary>
Motivation: Despite the widespread use of the heuristic that early timesteps shape motion and layout while later ones refine appearance, this behavior lacks systematic characterization. The study aims to understand and quantify this phenomenon to improve motion customization in video diffusion models.

Method: The researchers proxy motion encoding by analyzing the trade-off between appearance editing and motion preservation when injecting new conditions over specified timestep ranges. They conduct a large-scale quantitative study to characterize this proxy and factor motion from appearance.

Result: The study identifies distinct motion-dominant and appearance-dominant regimes along the denoising trajectory, establishing a boundary in timestep space. This enables simplified motion customization by restricting training and inference to the motion-dominant regime, achieving strong motion transfer without additional modules or objectives.

Conclusion: The study transforms the empirical heuristic of motion and appearance encoding in video diffusion models into a principled understanding, demonstrating that motion and appearance can be disentangled by focusing on specific timestep regimes. This insight simplifies motion customization and enhances existing methods.
...
TL;DR: A hybrid deep learning system combining 3D CNN and LSTM for real-time ASL recognition, achieving high accuracy and deployed on AWS and edge devices.


<details>
  <summary>Details</summary>
Motivation: Address communication barriers for 70+ million deaf and hard-of-hearing individuals by enabling real-time ASL recognition.

Method: Hybrid architecture using 3D CNN for spatial-temporal feature extraction and LSTM for sequential dependency modeling, trained on WLASL, ASL-LEX, and expert-annotated datasets.

Result: Achieves F1-scores of 0.71–0.99 across sign classes, deployed on AWS and OAK-D cameras for real-time inference.

Conclusion: The system demonstrates robust performance and practical deployment potential for accessibility applications.
...
TL;DR: AI-enhanced LLE model improves medical billing and transcription accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: To revolutionize the handling of high-dimensional medical data by integrating AI with Locally Linear Embedding (LLE).

Method: Introduces an innovative AI-enhanced LLE model tailored for medical billing and transcription.

Result: Significant improvement in data processing accuracy and operational efficiency.

Conclusion: Sets a foundation for future research into broader healthcare applications of AI-enhanced LLE.
...
TL;DR: VISTA is a modular framework that decouples perception from reasoning to improve visual question-answering robustness, reducing reliance on spurious correlations and enhancing performance across benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address the issue of end-to-end Vision-language Models (VLMs) exploiting spurious correlations instead of causal visual evidence, especially when fine-tuned, leading to shortcut-prone behavior.

Method: VISTA decouples perception from reasoning using an explicit information bottleneck. It employs a frozen VLM sensor for short, objective perception queries and a text-only LLM reasoner for question decomposition, query planning, and aggregation of visual facts in natural language. The framework is trained with reinforcement learning (GRPO) using 641 curated multi-step questions.

Result: VISTA improves robustness to spurious correlations (+16.29% with Qwen-2.5-VL-7B and +6.77% with Llama-3.2-Vision-11B on SpuriVerse) while remaining competitive on MMVP and SeedBench. It also transfers robustly across unseen VLM sensors and recovers from perception failures.

Conclusion: VISTA significantly improves robustness to real-world spurious correlations and enhances visual reasoning by decoupling perception from reasoning, demonstrating better performance and transferability across different VLM sensors.
...
TL;DR: SAMM2D improves aneurysm detection with a dual-encoder framework, showing unaugmented models outperform augmented ones, achieving high sensitivity and cost savings.


<details>
  <summary>Details</summary>
Motivation: The challenge of aneurysm detection lies in subtle morphology, class imbalance, and scarcity of annotated data. The study aims to improve detection performance and overturn the assumption that more augmentation is always beneficial in low-data medical settings.

Method: SAMM2D, a dual-encoder framework, is introduced for aneurysm detection. The method includes an unaugmented baseline model and evaluates six augmentation regimes. The model uses ImageNet-pretrained features and calibrates decision thresholds for optimal performance.

Result: SAMM2D achieves an AUC of 0.686, a 32% improvement over the clinical baseline. The unaugmented baseline outperforms augmented variants by 1.75--2.23 percentage points (p < 0.01). The model reaches 95% sensitivity, surpassing radiologist performance, and projects significant cost savings. Grad-CAM visualizations show 85% true positives focus on relevant vascular regions.

Conclusion: Strong pretraining is more beneficial than complex augmentation in medical imaging. SAMM2D's performance suggests a shift in future workflows towards leveraging robust pretrained features rather than extensive augmentation.
...
TL;DR: HookMIL is a context-aware and computationally efficient MIL framework for whole-slide image analysis, leveraging multimodal hook tokens for improved performance.


<details>
  <summary>Details</summary>
Motivation: Traditional MIL approaches lose contextual information, and transformer-based variants suffer from quadratic complexity and redundancy.

Method: HookMIL uses learnable hook tokens initialized from key-patch features, text embeddings, and spatial features, with bidirectional attention and a Hook Diversity Loss.

Result: HookMIL achieves state-of-the-art performance on four pathology datasets with enhanced efficiency and interpretability.

Conclusion: HookMIL addresses limitations of traditional and transformer-based MIL, offering a robust solution for computational pathology.
...
TL;DR: Tiny-YOLOSAM combines YOLOv12 with TinySAM for efficient segmentation, improving coverage and speed.


<details>
  <summary>Details</summary>
Motivation: To address the computational expense of SAM and the inefficiency of TinySAM's 'segment-everything' mode.

Method: Uses YOLOv12 for box prompts and sparse point prompts for uncovered regions.

Result: Improves AR to 77.1% and mIoU to 67.8%, reducing runtime to 10.39s/image.

Conclusion: Detector-guided prompting with sparse sampling is effective for full-scene segmentation.
...
TL;DR: The paper proposes a multimodal explainability model using a VLM with few-shot learning to enhance DR diagnostics by analyzing lesion distributions and generating Grad-CAM heatmaps for improved interpretability and accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need for early detection of Diabetic Retinopathy (DR) to prevent vision loss, coupled with the limitations of current AI models that lack interpretability and rely on single imaging modalities. Physicians require models that explain classifications beyond just lesion locations, and a quantitative-detection system in natural language would be more versatile.

Method: The paper introduces a novel multimodal explainability model utilizing a Vision-Language Model (VLM) with few-shot learning. The model analyzes lesion distributions within retinal quadrants in fundus images and generates paired Grad-CAM heatmaps for both OCT and fundus images, visually highlighting regions contributing to DR severity classification.

Result: The model was tested on a dataset of 3,000 fundus images and 1,000 OCT images. It successfully generated Grad-CAM heatmaps, showcasing individual neuron weights across both imaging modalities, thereby improving interpretability and effectiveness in DR severity classification.

Conclusion: The paper concludes that the proposed multimodal explainability model, which mimics an ophthalmologist's reasoning through a VLM with few-shot learning, effectively addresses the limitations of current DR diagnostic tools. It provides a practical and comprehensive solution for improving patient outcomes by enhancing interpretability and accuracy in DR severity classification.
...
TL;DR: TCFormer is a lightweight, weakly-supervised transformer-based crowd counting framework that achieves high accuracy with minimal parameters, suitable for edge devices.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of labor-intensive annotations and computationally intensive backbones in crowd counting, enabling scalability and deployment in resource-constrained environments.

Method: The paper proposes TCFormer, a tiny, ultra-lightweight, weakly-supervised transformer-based framework with a Learnable Density-Weighted Averaging module and a density-level classification loss.

Result: Extensive experiments on benchmarks (ShanghaiTech A/B, UCF-QNRF, NWPU) demonstrate competitive performance with only 5 million parameters.

Conclusion: TCFormer achieves a superior trade-off between parameter efficiency and counting accuracy, making it a viable solution for crowd counting in resource-constrained environments.
...
TL;DR: A custom CNN achieves 96% accuracy in classifying malaria-infected blood cells, outperforming traditional methods and established deep learning models, with added interpretability via Explainable AI techniques.


<details>
  <summary>Details</summary>
Motivation: Traditional malaria diagnostic methods (e.g., microscopic blood smear analysis) are low in sensitivity, depend on expert judgment, and may lack resources in remote settings. This study aims to overcome these limitations by leveraging deep learning for automated and accurate diagnosis.

Method: The study employs a custom Convolutional Neural Network (CNN) to automatically classify blood cell images as parasitized or uninfected. It also compares this custom CNN with established architectures like ResNet50, VGG16, MobileNetV2, and DenseNet121. Explainable AI techniques (SHAP, LIME, Saliency Maps) are used for model interpretability.

Result: The custom CNN achieves 96% accuracy, with precision and recall scores exceeding 0.95 for both parasitized and uninfected classes. The model outperforms or matches the performance of established deep learning architectures.

Conclusion: The study concludes that the proposed deep learning-based approach, utilizing a custom CNN, provides a quick, accurate, and interpretable solution for malaria diagnosis, particularly in resource-limited settings.
...
TL;DR: Signal-SGN++ integrates SNNs and GCNs for energy-efficient action recognition, using adaptive topology-aware mechanisms and multi-resolution feature fusion, achieving superior accuracy-efficiency trade-offs.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between the energy efficiency of Spiking Neural Networks (SNNs) and the strong modeling capability of Graph Convolutional Networks (GCNs) for action recognition, the paper proposes a topology-aware spiking graph framework.

Method: The network employs a backbone of 1D Spiking Graph Convolution (1D-SGC) and Frequency Spiking Convolution (FSC) for joint spatiotemporal and spectral feature extraction. It includes a Topology-Shift Self-Attention (TSSA) mechanism for adaptive attention routing and an auxiliary Multi-Scale Wavelet Transform Fusion (MWTF) branch for multi-resolution temporal-frequency representations.

Result: Signal-SGN++ outperforms existing SNN-based methods and achieves competitive results against state-of-the-art GCNs under substantially reduced energy consumption.

Conclusion: Signal-SGN++ achieves superior accuracy-efficiency trade-offs, outperforming existing SNN-based methods and matching state-of-the-art GCNs with significantly reduced energy consumption.
...
TL;DR: VLM-PAR, a vision-language framework, improves pedestrian attribute recognition by addressing class imbalance and domain shifts, achieving state-of-the-art performance on PA100K, PETA, and Market-1501 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Pedestrian Attribute Recognition (PAR) faces challenges like class imbalance, attribute dependencies, and domain shifts, necessitating a robust solution.

Method: The paper introduces VLM-PAR, a modular framework using frozen SigLIP 2 multilingual encoders and cross-attention fusion for refining visual features.

Result: VLM-PAR achieves significant accuracy improvements on PA100K, PETA, and Market-1501 benchmarks, setting a new state-of-the-art.

Conclusion: The results highlight the effectiveness of integrating vision-language pretraining with cross-modal refinement to tackle PAR challenges.
...
TL;DR: ViSignVQA is a new Vietnamese dataset for signboard VQA, integrating OCR and language models to improve performance, with a multi-agent framework achieving 75.98% accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation was to address the underexplored area of understanding signboard text in natural scenes, particularly for low-resource languages like Vietnamese, and to provide a benchmark dataset for OCR-integrated VQA models.

Method: The authors introduced ViSignVQA, a dataset with 10,762 images and 25,573 question-answer pairs, and adapted state-of-the-art VQA models by integrating a Vietnamese OCR model (SwinTextSpotter) and a pretrained language model (ViT5). They also proposed a multi-agent VQA framework combining perception and reasoning agents with GPT-4.

Result: The OCR-enhanced context improved F1-scores by up to 209%, and the multi-agent VQA framework achieved 75.98% accuracy via majority voting.

Conclusion: The study presents ViSignVQA, the first large-scale Vietnamese dataset for signboard-oriented VQA, highlighting the importance of domain-specific resources for low-resource languages. It underscores the effectiveness of OCR-enhanced context and introduces a multi-agent VQA framework with promising results.
...
TL;DR: The paper introduces Semantic Abstraction, a framework using 2D VLMs' relevancy maps as abstract object representations to learn 3D localization and completion for hidden objects, improving efficiency over random search.


<details>
  <summary>Details</summary>
Motivation: To enable household robots to efficiently locate hidden or partially occluded objects by leveraging historical data and semantic abstraction.

Method: Uses 2D VLMs' relevancy activations as abstract object representations to localize and complete 3D structures for hidden objects, utilizing historical placement data for efficiency.

Result: The model accurately identifies the complete 3D location of hidden objects faster than random search, enhancing robot efficiency.

Conclusion: Semantic Abstraction extends to hidden object localization, providing robots with time-saving skills for practical applications like finding lost items.
...
TL;DR: VideoScaffold is a dynamic framework for streaming video understanding, featuring Elastic-Scale Event Segmentation (EES) and Hierarchical Event Consolidation (HEC) to adaptively refine event boundaries and aggregate segments. It achieves state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing static strategies for video understanding are optimized for offline settings and produce fragmented or over-compressed outputs when applied to continuous video streams.

Method: VideoScaffold introduces EES for dynamic event boundary refinement and HEC for hierarchical segment aggregation, enabling smooth transition from fine-grained to abstract reasoning.

Result: VideoScaffold achieves state-of-the-art performance in both offline and streaming video understanding benchmarks.

Conclusion: VideoScaffold provides a modular and plug-and-play framework that extends image-based MLLMs to continuous video comprehension effectively.
...
TL;DR: The paper introduces a KAN-enhanced FPN-Stem architecture to address performance bottlenecks in Vision Transformers (ViT) for dense prediction tasks, achieving a +2.0 AP improvement on COCO.


<details>
  <summary>Details</summary>
Motivation: Current ViT front-end designs, like ViTPose, suffer from simplistic patchification leading to irreversible information loss and poor multi-scale handling.

Method: Proposes a KAN-based convolutional layer to replace the linear smoothing step in FPN, leveraging non-linear modeling to rectify fusion artifacts.

Result: Achieves a +2.0 AP boost over ViTPose-S on COCO, demonstrating the effectiveness of KAN-FPN-Stem.

Conclusion: The bottleneck in ViT performance lies in feature fusion quality, not refinement, and KAN provides a viable solution.
...
TL;DR: MiG-DM, a meta-information guided model, improves low-dose PET imaging by integrating patient-specific data and cross-domain processing, outperforming current methods.


<details>
  <summary>Details</summary>
Motivation: To address challenges in low-dose PET imaging, such as noise interference and reduced contrast, by leveraging patient-specific meta-information and projection-domain physics knowledge.

Method: The study introduces a meta-information guided cross-domain synergistic diffusion model (MiG-DM) that uses a meta-information encoding module and a cross-domain architecture combining projection and image-domain processing.

Result: MiG-DM demonstrates superior performance in enhancing PET image quality and preserving physiological details on both public and clinical datasets.

Conclusion: The MiG-DM model successfully integrates meta-information and cross-domain priors to enhance low-dose PET imaging, outperforming existing methods in quality and detail preservation.
...
TL;DR: A hybrid knowledge distillation method creates a lightweight CNN for smart agriculture, achieving near-teacher accuracy with much lower computational cost and model size, validated on rice seeds and plant disease datasets.


<details>
  <summary>Details</summary>
Motivation: The challenge of deploying deep learning models on resource-constrained edge devices in smart agriculture, due to the trade-off between computational efficiency and recognition accuracy, motivates the development of a lightweight yet high-performance model.

Method: The study introduces a hybrid knowledge distillation framework involving a customized student model with inverted residual blocks and dense connectivity, trained under the guidance of a ResNet18 teacher network. The training employs a multi-objective strategy integrating hard-label supervision, feature-level distillation, response-level distillation, and self-distillation.

Result: The distilled student model achieves 98.56% accuracy on rice seed variety classification (98.65% for the teacher), with 0.68 GFLOPs and ~1.07M parameters, reducing computational cost by 2.7x and model size by >10x compared to ResNet18. It also outperforms or matches pretrained models like DenseNet121 and ViT in accuracy while being significantly smaller.

Conclusion: The proposed hybrid knowledge distillation framework successfully develops a lightweight yet high-performance convolutional neural network, achieving near-teacher-level accuracy with significantly reduced computational cost and model size. This demonstrates its robustness, efficiency, and strong deployment potential for resource-constrained smart agriculture systems.
...
TL;DR: An adaptive framework fusing RGB and LWIR video streams improves robotic vision in varying light conditions, outperforming baseline models.


<details>
  <summary>Details</summary>
Motivation: Traditional RGB detection struggles in low-light, and thermal systems lack color/texture info. The paper aims to enhance autonomous robotic vision.

Method: Trained 33 YOLO models on 22,000 images across light levels, fused RGB/LWIR at 11 ratios, and dynamically selected optimal models.

Result: Best models achieved 92.8% (full-light) and 92.0% (dim-light) confidence, surpassing baselines. No-light fusion reached 71.0%.

Conclusion: RGB-LWIR fusion significantly boosts detection confidence and reliability in all lighting, advancing robotic vision.
...
TL;DR: The paper introduces a method using a Human Perception Embedding teacher to guide text-to-image diffusion models, improving geometric control and semantic alignment by 80%.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models often fail to adhere to strict geometric constraints due to a semantic gap between human perception and generative models. The research investigates whether geometric understanding can be introduced without specialized training by leveraging lightweight discriminators as external guidance signals.

Method: The approach involves using a Human Perception Embedding (HPE) teacher trained on the THINGS triplet dataset to inject gradients into the latent diffusion process, separating geometry and style controllably. Three architectures—Stable Diffusion v1.5, SiT-XL/2, and PixArt-Σ—were evaluated.

Result: The guided generation improved semantic alignment by approximately 80% compared to unguided baselines, demonstrating successful zero-shot transfer of complex 3D shapes onto conflicting materials.

Conclusion: The study demonstrates that small teacher models can effectively guide large generative systems, enhancing geometric control and expanding the creative possibilities of text-to-image synthesis.
...
TL;DR: GeCo is a geometry-grounded metric for detecting geometric deformation and occlusion-inconsistency artifacts in static scenes, providing dense consistency maps. It benchmarks video generation models and serves as a training-free guidance loss to reduce artifacts.


<details>
  <summary>Details</summary>
Motivation: To address the need for interpretable and dense consistency maps for detecting geometric deformation and occlusion-inconsistency artifacts in static scenes.

Method: Fuses residual motion and depth priors to produce interpretable, dense consistency maps.

Result: GeCo effectively benchmarks video generation models and reduces deformation artifacts when used as a training-free guidance loss.

Conclusion: GeCo provides a robust solution for detecting and mitigating geometric deformation and occlusion-inconsistency artifacts in video generation.
...
TL;DR: AI models excel in structured tasks but struggle with multimodal clinical reasoning, limiting their current role to supportive, text-based tasks in healthcare. Future breakthroughs in multimodal integration are needed for broader clinical competence.


<details>
  <summary>Details</summary>
Motivation: The rapid integration of foundation models into clinical practice and public health necessitates a rigorous evaluation of their true clinical reasoning capabilities beyond narrow examination success. Current benchmarks fail to capture the integrated, multimodal reasoning essential for real-world patient care.

Method: The study developed the Bones and Joints (B&J) Benchmark, a comprehensive evaluation framework comprising 1,245 questions derived from real-world patient cases in orthopedics and sports medicine. It assessed models across 7 tasks mirroring the clinical reasoning pathway, including knowledge recall, text and image interpretation, diagnosis generation, treatment planning, and rationale provision. Eleven vision-language models (VLMs) and six large language models (LLMs) were evaluated against expert-derived ground truth.

Result: The results showed a pronounced performance gap between task types. State-of-the-art models achieved high accuracy (exceeding 90%) on structured multiple-choice questions but declined markedly on open-ended tasks requiring multimodal integration (accuracy scarcely reached 60%). VLMs exhibited substantial limitations in interpreting medical images and frequent text-driven hallucinations. Models fine-tuned for medical applications showed no consistent advantage over general-purpose counterparts.

Conclusion: Current artificial intelligence models are not yet clinically competent for complex, multimodal reasoning. Their safe deployment should currently be limited to supportive, text-based roles. Future advancement in core clinical tasks awaits fundamental breakthroughs in multimodal integration and visual understanding.
...
TL;DR: Fetal-Gauge is the first large-scale benchmark for evaluating VLMs in fetal ultrasound tasks, revealing a significant performance gap and highlighting the need for domain-adapted architectures.


<details>
  <summary>Details</summary>
Motivation: The growing demand for prenatal ultrasound imaging has intensified a global shortage of trained sonographers, creating barriers to essential fetal health monitoring. Deep learning, particularly Vision-Language Models (VLMs), has the potential to enhance sonographers' efficiency and support training. However, no standardized benchmark exists to evaluate VLMs in fetal ultrasound imaging due to its challenging nature, operator dependency, and limited public datasets.

Method: The benchmark comprises over 42,000 images and 93,000 question-answer pairs, spanning various fetal ultrasound tasks. Several state-of-the-art VLMs, including general-purpose and medical-specific models, are systematically evaluated.

Result: The best-performing model achieves only 55% accuracy, far below clinical requirements, revealing critical limitations of current VLMs in fetal ultrasound interpretation.

Conclusion: Fetal-Gauge establishes a rigorous foundation for advancing multimodal deep learning in prenatal care and provides a pathway toward addressing global healthcare accessibility challenges.
...
TL;DR: Uni4D is a unified framework for 3D retrieval and 4D generation, using multi-modal alignment to improve semantic understanding and practical applications.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of large-scale open vocabulary 3D retrieval and controlled 4D generation by improving semantic alignment across text, 3D models, and image modalities.

Method: The framework employs a 3D text multi-head attention and search model, along with three components: precise text-to-3D retrieval, multi-view 3D-to-image alignment, and image-to-text alignment for generating temporally consistent 4D assets.

Result: Experimental results show that Uni4D achieves high-quality 3D retrieval and controllable 4D generation.

Conclusion: Uni4D successfully advances dynamic multimodal understanding and practical applications through its unified framework for 3D retrieval and 4D generation.
...
TL;DR: SirenPose introduces a novel loss function combining sinusoidal representation networks with geometric priors for improved 3D scene reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with motion modeling accuracy and spatiotemporal consistency in fast-moving and multi-target scenes.

Method: SirenPose uses physics-inspired constraints and expands the training dataset to 600,000 annotated instances.

Result: Models trained with SirenPose show significant improvements in spatiotemporal consistency metrics.

Conclusion: SirenPose outperforms prior methods in handling rapid motion and complex scene changes.
...
TL;DR: A low-cost, single-camera driver monitoring system achieves real-time behavior recognition (16-25 FPS) on edge devices, covering 17 behaviors and enabling reliable alerts for human-centered vehicle intelligence.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the need for low-latency, cost-effective, and power-efficient in-cabin driver monitoring systems that can recognize distraction- and drowsiness-related behaviors reliably.

Method: The method involves a single-camera pipeline with a compact per-frame vision model, confounder-aware label design, and a temporal decision head for sustained and confident predictions. It covers 17 behavior classes and is optimized for deployment on Raspberry Pi 5 and Google Coral Edge TPU.

Result: The system achieves about 16 FPS on Raspberry Pi 5 and about 25 FPS on Coral Edge TPU, with per-frame latency under 60 ms, enabling real-time monitoring and stable alert generation.

Conclusion: The paper concludes that the proposed driver monitoring system is effective for real-time behavior recognition on low-cost edge platforms, enabling reliable alerts and serving as a foundation for human-centered vehicle intelligence.
...
TL;DR: An attack-aware deepfake and image-forensics detector with robust performance, calibrated probabilities, and transparent evidence under realistic conditions.


<details>
  <summary>Details</summary>
Motivation: To create a detector that is robust against attacks, provides well-calibrated probabilities, and offers transparent evidence in real-world deployment scenarios.

Method: Combines red-team training with randomized test-time defense in a two-stream architecture, using a pretrained backbone for semantic content and forensic residuals, fused via a lightweight residual adapter. Includes a Feature Pyramid Network style head for tamper heatmaps under weak supervision.

Result: Demonstrates near-perfect ranking across attacks, low calibration error, minimal abstention risk, and controlled degradation under regrain.

Conclusion: Establishes a modular, data-efficient, and practically deployable baseline for attack-aware detection with calibrated probabilities and actionable heatmaps.
...
TL;DR: PortionNet is a cross-modal framework for food nutrition estimation from RGB images, mimicking point cloud features without depth sensors, achieving top performance on MetaFood3D and generalizing well on SimpleFood45.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of accurate food nutrition estimation from single images without relying on depth-sensors, which are inaccessible on most smartphones.

Method: The paper proposes PortionNet, a cross-modal knowledge distillation framework that learns geometric features from point clouds during training but only requires RGB images at inference. It uses a dual-mode training strategy with a lightweight adapter network to mimic point cloud representations.

Result: PortionNet achieves state-of-the-art performance on MetaFood3D for volume and energy estimation and shows strong generalization in cross-dataset evaluation on SimpleFood45.

Conclusion: PortionNet demonstrates strong generalization in energy estimation and outperforms previous methods in volume and energy estimation, making it a practical solution for food nutrition estimation from single RGB images.
...
TL;DR: MoFu is a framework for multi-subject video generation that addresses scale inconsistency and permutation sensitivity using SMO and Fourier Fusion, achieving superior results in scale preservation and visual quality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the challenges of scale inconsistency and permutation sensitivity in multi-subject video generation, which currently lead to unnatural results and subject distortion.

Method: The proposed method, MoFu, includes Scale-Aware Modulation (SMO) for handling scale inconsistency by extracting implicit scale cues from prompts, and Fourier Fusion for addressing permutation sensitivity by processing reference features' frequency information. A Scale-Permutation Stability Loss is also introduced to ensure consistent and invariant generation.

Result: Extensive experiments show that MoFu significantly outperforms existing methods in preserving natural scale, subject fidelity, and overall visual quality, as validated by a dedicated benchmark.

Conclusion: The paper concludes that MoFu effectively addresses scale inconsistency and permutation sensitivity in multi-subject video generation, outperforming existing methods in maintaining natural scale, subject fidelity, and visual quality.
...
TL;DR: VideoZoomer is a novel agentic framework enabling MLLMs to dynamically adjust visual focus for long video understanding, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Overcoming limitations of uniform frame sampling in MLLMs for long video understanding, which often misses critical evidence.

Method: A two-stage training strategy: supervised fine-tuning on distilled exemplars and reflection trajectories, followed by reinforcement learning.

Result: The 7B model shows diverse reasoning patterns, strong performance in benchmarks, and surpasses open-source and proprietary systems.

Conclusion: VideoZoomer enhances MLLMs' long video understanding with dynamic focus, achieving superior efficiency and performance.
