...
TL;DR: The paper benchmarks five open-vocabulary object detection (OVD) models on aerial imagery, revealing severe domain transfer issues and semantic confusion as primary bottlenecks.


<details>
  <summary>Details</summary>
Motivation: To evaluate the transferability of OVD models to aerial imagery, which remains unexplored, and identify challenges in zero-shot recognition.

Method: Systematic benchmark of five OVD models on the LAE-80C dataset under strict zero-shot conditions, using Global, Oracle, and Single-Category inference modes to isolate semantic confusion.

Result: Best model (OWLv2) achieves only 27.6% F1-score with 69% false positives; reducing vocabulary size improves performance 15x, showing semantic confusion as the main issue. Prompt engineering fails to help.

Conclusion: Domain transfer for OVD in aerial imagery is challenging, with semantic confusion as the primary bottleneck, necessitating domain-adaptive approaches.
...
TL;DR: A new VQA benchmark for scientific charts is proposed, addressing the lack of datasets where chart marks don't directly correspond to underlying data. Synthetic histograms and ground truth data are used to create a dataset for future research.


<details>
  <summary>Details</summary>
Motivation: Current VQA datasets for charts often assume a direct link between chart marks and data, missing the complexity of real-world scientific charts where data is transformed.

Method: Surveyed existing VQA datasets, generated synthetic histogram charts with ground truth data, and tested with humans and a large reasoning model.

Result: A new open-source dataset was created, including figures, underlying data, and annotations for future research.

Conclusion: The paper advocates for a specialized VQA benchmark for scientific charts to better reflect real-world data transformations and reasoning challenges.
...
TL;DR: VLMs struggle with 3D spatial reasoning, particularly in relative camera pose estimation, despite excelling in 2D tasks. New benchmarks (VRRPI-Bench and VRRPI-Diag) highlight these limitations, showing VLMs' poor performance compared to geometric baselines and humans.


<details>
  <summary>Details</summary>
Motivation: The gap in VLMs' understanding of 3D spatial structure compared to their strong 2D perception and semantic reasoning capabilities.

Method: Introducing VRRPI-Bench and VRRPI-Diag benchmarks derived from egocentric videos to evaluate VLMs on relative camera pose estimation (RCPE).

Result: VLMs fail to generalize beyond shallow 2D heuristics, especially for depth changes and roll transformations. They also struggle with multi-image reasoning, performing inconsistently.

Conclusion: VLMs have significant limitations in 3D and multi-view spatial reasoning, as revealed by their poor performance on RCPE tasks compared to geometric baselines and humans.
...
TL;DR: The paper explores how positional embeddings in vision transformers act as geometric priors, influencing spatial structure and multi-view consistency, supported by experiments on 14 ViT models.
...
Method: Introduces token-level diagnostics to measure multi-view geometric consistency and conducts experiments on 14 foundation ViT models.

Result: Positional embeddings function as geometric priors, shaping spatial structure and influencing multi-view geometry and spatial reasoning.

Conclusion: Positional embeddings are a causal mechanism governing spatial structure in ViT representations, clarifying their role beyond simple token indexing.
...
TL;DR: Single-level VQ-VAEs, when optimized for codebook usage and matched in representational budget, can achieve reconstruction fidelity comparable to hierarchical VQ-VAEs, challenging the need for hierarchy in high-quality reconstructions.


<details>
  <summary>Details</summary>
Motivation: To investigate whether a single-level VQ-VAE can match the reconstruction fidelity of hierarchical models when codebook collapse is mitigated and representational budgets are matched.

Method: Comparison of a two-level hierarchical VQ-VAE and a capacity-matched single-level model on high-resolution ImageNet images, with interventions like codebook initialization, periodic reset of inactive vectors, and hyperparameter tuning.

Result: Single-level VQ-VAEs achieve similar reconstruction fidelity to hierarchical models when codebook collapse is reduced and representational budgets are matched.

Conclusion: Hierarchical quantization is not inherently superior for high-quality reconstructions; single-level VQ-VAEs can perform equally well under optimized conditions.
...
TL;DR: VMonarch introduces a structured Monarch matrix-based attention mechanism for Video DiTs, reducing FLOPs by 17.5x and speeding up computation by 5x while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: The quadratic complexity of attention in Video DiTs limits context scalability, prompting the need for efficient sparse attention solutions.

Method: VMonarch uses Monarch matrices for sub-quadratic attention, adapts spatio-temporal factorization, introduces recomputation for stability, and integrates an online entropy algorithm with FlashAttention.

Result: VMonarch matches or surpasses full attention quality on VBench, reduces FLOPs by 17.5x, and speeds up attention computation by 5x for long videos.

Conclusion: VMonarch effectively addresses the attention bottleneck in Video DiTs, offering scalable and efficient sparse attention with superior performance.
...
TL;DR: C2R (Coarse-to-Real) is a generative rendering framework that synthesizes realistic urban crowd videos from coarse 3D simulations using a neural renderer guided by text prompts, overcoming data limitations with a mixed CG-real training strategy.


<details>
  <summary>Details</summary>
Motivation: Traditional rendering pipelines struggle with scalability and realism for dynamic scenes, prompting the need for a more efficient and controllable solution.

Method: C2R uses coarse 3D renderings to control scene layout and motion, while a neural renderer generates realistic details. It employs a two-phase mixed CG-real training strategy to bridge the gap between simulations and real videos.

Result: The system produces temporally consistent, controllable, and realistic urban scene videos from minimal 3D input, generalizing across diverse CG and game inputs.

Conclusion: C2R offers a scalable and realistic solution for generating urban crowd videos, with potential applications in gaming, simulations, and visual effects.
...
TL;DR: FlexMap adapts to variable camera setups without retraining, using a geometry-aware model to outperform existing HD map methods.


<details>
  <summary>Details</summary>
Motivation: Current HD map construction methods are fragile to sensor failures or varying camera configurations, limiting practical deployment.

Method: Uses a geometry-aware foundation model with cross-frame attention to implicitly encode 3D scene understanding, featuring a spatial-temporal enhancement module and a camera-aware decoder with latent camera tokens.

Result: FlexMap maintains robustness to missing views and sensor variations, outperforming existing methods across multiple configurations.

Conclusion: FlexMap outperforms existing HD map construction methods by adapting to variable camera configurations without architectural changes or retraining, ensuring robustness to sensor variations.
...
TL;DR: A jailbreak framework using Chain-of-Thought prompting and ReAct-driven adaptive noising to bypass safety filters in vision-language models, improving attack success rates while maintaining output naturalness.


<details>
  <summary>Details</summary>
Motivation: Vision-language models (VLMs) are sensitive to prompt variations, revealing vulnerabilities in safety alignment. This work aims to exploit these vulnerabilities to bypass safety filters.

Method: The framework combines post-training Chain-of-Thought (CoT) prompting for stealthy prompts and a ReAct-driven adaptive noising mechanism to iteratively perturb input images based on model feedback.

Result: The dual-strategy significantly improves attack success rates (ASR) while maintaining naturalness in both text and visual domains.

Conclusion: The proposed approach effectively bypasses safety filters in VLMs, highlighting the need for more robust safety alignment methods.
...
TL;DR: A probabilistic multi-view markerless motion capture (MMMC) method was evaluated for reliability and calibration, showing accurate confidence intervals and strong correlation between predicted uncertainty and observed errors.


<details>
  <summary>Details</summary>
Motivation: To enhance clinical trust and implementation of video-based human movement analysis by ensuring accurate and reliable confidence intervals for MMMC systems.

Method: Utilized variational inference to estimate joint angle posterior distributions, validated against an instrumented walkway and marker-based motion capture, and measured calibration using Expected Calibration Error (ECE).

Result: Demonstrated reliable calibration with ECE values < 0.1, median step/stride length errors of ~16 mm/~12 mm, and kinematic errors of 1.5-3.8 degrees. Predicted uncertainty strongly correlated with observed errors.

Conclusion: The probabilistic MMMC model effectively quantifies epistemic uncertainty, enabling identification of unreliable outputs without ground-truth instrumentation.
...
TL;DR: The paper introduces a Self-Validation Framework to reduce object hallucination in LVLMs by verifying object existence in captions, achieving notable performance gains.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the critical issue of object hallucination in LVLMs, which stems from their over-reliance on language priors, and to provide a deeper understanding and solution for this problem.

Method: The method involves Language-Prior-Free Verification and a training-free Self-Validation Framework that validates object existence in candidate captions and selects or aggregates them to reduce hallucination.

Result: The framework achieves significant improvements, such as a 65.6% enhancement on the CHAIRI metric with LLaVA-v1.5-7B, outperforming previous state-of-the-art methods.

Conclusion: The paper concludes that the proposed Self-Validation Framework effectively mitigates object hallucination in LVLMs by leveraging their inherent potential, surpassing previous methods.
...
TL;DR: ScribbleSense uses MLLMs and image generation to clarify scribble-based texture editing, achieving top performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for interactive 3D model texture editing are limited in handling coarse-grained scribble-based interactions, often leading to ambiguous editing intentions and unclear target semantic locations.

Method: The method combines MLLMs to predict editing intent from scribbles and uses globally generated images to extract local texture details, anchoring local semantics.

Result: Experimental results show that ScribbleSense outperforms existing methods, providing clearer semantic intent and improved texture editing.

Conclusion: ScribbleSense effectively leverages multimodal large language models (MLLMs) and image generation models to address ambiguities in scribble-based texture editing, achieving state-of-the-art performance.
...
TL;DR: The paper introduces a guidance scheme using a representation alignment projector to mitigate semantic drift in diffusion transformers, improving semantic coherence and visual fidelity in image synthesis.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address semantic drift in early denoising stages of diffusion transformers, which leads to inconsistent alignment despite identical conditioning.

Method: The method involves a representation alignment projector that injects predicted representations into intermediate sampling steps, acting as a semantic anchor without altering the model architecture.

Result: Experiments show significant improvements, such as reducing FID scores (e.g., REPA-XL/2 from 5.9 to 3.3) and outperforming existing guidance methods like representative guidance.

Conclusion: The proposed approach enhances semantic preservation and image consistency, proving effective when combined with classifier-free guidance.
...
TL;DR: HAVC improves MLLMs' fine-grained reasoning by selectively refining attention heads for better visual grounding, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with fine-grained reasoning due to low-resolution inputs and noisy attention aggregation, limiting their performance in VQA.

Method: HAVC filters and refines attention heads using OCR-based diagnostics, spatial entropy, and gradient sensitivity to produce a Visual Cropping Guidance Map for precise subimage cropping.

Result: HAVC consistently outperforms state-of-the-art cropping methods, achieving better localization and visual grounding in fine-grained VQA tasks.

Conclusion: HAVC offers a simple yet effective strategy to enhance MLLMs' precision in visual reasoning tasks.
...
TL;DR: PromptMAD is a cross-modal prompting framework for unsupervised visual anomaly detection, leveraging CLIP-encoded text prompts and Focal loss to improve detection of subtle anomalies. It achieves state-of-the-art performance on MVTec-AD.


<details>
  <summary>Details</summary>
Motivation: The challenges in multi-class visual anomaly detection include diverse object categories, scarce anomalous examples, and camouflaged defects.

Method: The framework integrates semantic guidance via vision-language alignment, uses CLIP-encoded text prompts, and incorporates Focal loss for class imbalance. It also includes a supervised segmentor with multi-scale features and Transformer-based attention.

Result: Achieves 98.35% mean AUC and 66.54% AP on MVTec-AD, demonstrating state-of-the-art pixel-level performance.

Conclusion: PromptMAD effectively addresses challenges in anomaly detection, offering high-resolution anomaly maps and efficient performance across diverse categories.
...
TL;DR: MirrorTalk uses a diffusion model and style encoder to create personalized talking faces with better lip-sync and style preservation.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with disentangling speaker-specific style and semantic content, limiting faithful persona transfer.

Method: The framework uses a conditional diffusion model with a Semantically-Disentangled Style Encoder (SDSE) and a hierarchical modulation strategy.

Result: MirrorTalk improves lip-sync accuracy and personalization preservation compared to state-of-the-art methods.

Conclusion: MirrorTalk successfully synthesizes personalized talking faces with high lip-sync accuracy and expressive dynamics, outperforming existing methods.
...
TL;DR: DreamVAR, a VAR-based framework for subject-driven image synthesis, pre-fills subject features before predicting image tokens, outperforming diffusion models in appearance preservation.


<details>
  <summary>Details</summary>
Motivation: The potential of VAR models in subject-driven image generation is underexplored despite their unified architecture and efficient inference. DreamVAR aims to address this gap.

Method: DreamVAR employs a VAR model with next-scale prediction, pre-filling subject feature sequences before predicting target image tokens. It uses reinforcement learning to enhance semantic alignment and subject consistency.

Result: Extensive experiments show DreamVAR outperforms leading diffusion-based methods in appearance preservation.

Conclusion: DreamVAR demonstrates superior appearance preservation in subject-driven image synthesis compared to leading diffusion-based methods, leveraging a novel VAR framework with next-scale prediction and reinforcement learning.
...
TL;DR: The paper introduces CoVA, a new task for composed video retrieval that includes both visual and auditory variations, and proposes AVT, a fusion method for integrating video, audio, and text features.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for composed video retrieval (CoVR) ignore audio differences despite visual similarity, limiting their applicability.

Method: The authors introduce AV-Comp, a benchmark with video pairs and textual queries describing cross-modal changes, and propose AVT Compositional Fusion to align queries with the most relevant modality.

Result: AVT outperforms traditional unimodal fusion methods, establishing a strong baseline for the CoVA task.

Conclusion: The paper addresses the gap in composed video retrieval by incorporating auditory variations and provides a robust benchmark and fusion method for future research.
...
TL;DR: The paper introduces the DNA framework for forgery detection, leveraging pre-trained models' latent capabilities without retraining. It uses a coarse-to-fine approach to identify forgery-discriminative units and achieves superior performance with minimal data.


<details>
  <summary>Details</summary>
Motivation: Current forgery detection methods rely on resource-intensive fine-tuning, but the paper argues that pre-trained models already possess intrinsic detection capabilities.

Method: The DNA framework employs feature decoupling, attention analysis, and a triadic fusion scoring metric to isolate forgery-discriminative units (FDUs). It also introduces the HIFI-Gen benchmark for evaluation.

Result: DNA achieves superior detection performance under few-shot conditions and shows robustness across architectures and unseen generative models.

Conclusion: The study demonstrates that activating latent neurons in pre-trained models is more effective than extensive fine-tuning for forgery detection.
...
TL;DR: Depth images outperform point clouds for BCS prediction in dairy cattle, except for hindquarter data, where performance is similar. Point clouds are more sensitive to noise.


<details>
  <summary>Details</summary>
Motivation: Conventional visual scoring of body condition score (BCS) in dairy cattle is subjective and labor-intensive. Computer vision approaches, particularly depth images and point clouds, offer potential improvements but lack direct comparisons.

Method: The study compared top-view depth image and point cloud data for BCS prediction under four settings: unsegmented raw data, segmented full-body data, segmented hindquarter data, and handcrafted feature data. Prediction models were evaluated using data from 1,020 dairy cows with cow-level cross-validation.

Result: Depth image-based models achieved higher accuracy than point cloud-based models for unsegmented and segmented full-body data, while performance was comparable for segmented hindquarter data. Both approaches showed reduced accuracy with handcrafted features. Point clouds were more sensitive to noise and model architecture.

Conclusion: Three-dimensional point clouds do not consistently outperform depth images for BCS prediction in dairy cattle under the evaluated conditions.
...
TL;DR: SHED is a novel encoder-decoder architecture for dense prediction that enforces geometric prior through segmentation, improving structural consistency and cross-domain generalization.


<details>
  <summary>Details</summary>
Motivation: Existing dense prediction methods treat scenes as independent pixel-wise predictions, leading to structural inconsistencies. SHED addresses this by incorporating segmentation to enforce geometric prior.

Method: SHED uses a bidirectional hierarchical reasoning approach where segment tokens are pooled in the encoder and unpooled in the decoder. The model is supervised only at the final output, allowing segment hierarchy to emerge without explicit segmentation supervision.

Result: SHED improves depth boundary sharpness, segment coherence, and cross-domain generalization. It also enhances 3D reconstruction quality and reveals interpretable part-level structures.

Conclusion: SHED's hierarchy-aware decoder captures global 3D scene layouts effectively, outperforming conventional pixel-wise methods in semantic segmentation and 3D perception tasks.
...
TL;DR: A hybrid cross-device localization pipeline combining geometric and neural methods achieved high recall and accuracy in the CroCoDL 2025 Challenge.


<details>
  <summary>Details</summary>
Motivation: To improve recall and accuracy in cross-device localization tasks by integrating complementary geometric and neural approaches.

Method: Combines a shared retrieval encoder with two localization branches: a geometric branch (feature fusion and PnP) and a neural branch (MapAnything). Uses neural-guided pruning and depth-conditioned refinement.

Result: Achieved a score of 92.62 (R@0.5m, 5°) in the challenge, showing significant improvements in recall and accuracy.

Conclusion: The hybrid approach effectively enhances localization performance by leveraging both classical and neural methods.
...
TL;DR: The paper introduces MA-PaPSP, a memory-augmented method for selective prediction in visual language tasks, addressing instability and calibration issues in existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing selective prediction methods focus on closed-set tasks, leaving a gap for open-set and unbounded vocabulary tasks in visual language models.

Method: Proposes MA-PaPSP, which augments PaPSP with a retrieval dataset and contrastive normalization to stabilize embeddings and improve score calibration.

Result: MA-PaPSP outperforms PaPSP and other baselines in selective captioning, image-text matching, and fine-grained classification.

Conclusion: MA-PaPSP effectively addresses key challenges in selective prediction for visual language models, offering a robust and training-free solution.
...
TL;DR: DELNet is a continual learning framework for weather image restoration that avoids retraining by using a judging valve and dynamic expert library, outperforming existing methods with significant PSNR gains.


<details>
  <summary>Details</summary>
Motivation: Existing all-in-one weather image restoration methods depend on pre-collected data and require retraining for unseen degradations, leading to high costs.

Method: DELNet integrates a judging valve to measure task similarity and a dynamic expert library for knowledge transfer and task-specific feature capture.

Result: DELNet surpasses state-of-the-art continual learning methods, achieving PSNR gains of 16%, 11%, and 12% on OTS, Rain100H, and Snow100K datasets.

Conclusion: DELNet demonstrates effectiveness, robustness, and efficiency in weather image restoration, reducing retraining costs and enabling practical deployment.
...
TL;DR: The paper introduces Spatiotemporal-Semantic Contrastive Decoding to mitigate hallucinations in Video Large Language Models by disrupting and contrasting spatiotemporal and semantic features.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reducing video hallucinations are heuristic and lack precision in addressing root causes and fine-grained correlations, limiting robustness and generalization.

Method: Proposes a novel decoding strategy that constructs negative features by disrupting spatiotemporal consistency and semantic associations, then uses contrastive decoding to suppress hallucinations.

Result: The method effectively reduces hallucinations while maintaining the model's video understanding and reasoning capabilities.

Conclusion: The proposed strategy offers a more precise and robust solution to mitigate hallucinations in Video Large Language Models.
...
TL;DR: PhoStream is a new benchmark for evaluating multimodal large language models (MLLMs) in real-world mobile streaming scenarios, revealing their limitations in timing responses correctly.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for MLLMs focus on offline tasks or short videos, lacking evaluation for continuous real-world streaming scenarios like mobile assistants.

Method: The paper introduces PhoStream, a benchmark with 5,572 open-ended QA pairs from 578 videos, built using an Automated Generative Pipeline and evaluated with an Online Inference Pipeline and LLM-as-a-Judge.

Result: Models perform well on Instant and Backward tasks (e.g., Gemini 3 Pro scores over 80) but poorly on Forward tasks (16.40), showing difficulty in timing responses.

Conclusion: Current MLLMs struggle with deciding when to respond in streaming scenarios, highlighting a key limitation beyond content generation.
...
TL;DR: MIFOMO, a foundation model for cross-domain few-shot HSI classification, uses coalescent projection, mixup domain adaptation, and label smoothing to outperform prior methods by up to 14%.


<details>
  <summary>Details</summary>
Motivation: Existing methods for CDFSL in HSI classification rely on unrealistic data augmentation and suffer from overfitting. The paper aims to address these issues by utilizing a foundation model with strong generalization capabilities.

Method: MIFOMO leverages a pre-trained remote sensing foundation model, introduces coalescent projection for quick adaptation, mixup domain adaptation for domain discrepancy, and label smoothing for noisy pseudo-labels.

Result: MIFOMO achieves superior performance, outperforming prior methods by up to 14%.

Conclusion: The paper concludes that MIFOMO outperforms prior methods by up to 14%, demonstrating its effectiveness in cross-domain few-shot learning for HSI classification. The model's open-source availability ensures reproducibility and further research.
...
TL;DR: FOTBCD is a large-scale building change detection dataset covering diverse environments across France, designed for benchmarking under geographic domain shift. It includes binary and instance-level annotated subsets, showing improved cross-domain generalization.


<details>
  <summary>Details</summary>
Motivation: Existing building change detection datasets are geographically limited, constraining their utility for evaluating models under diverse conditions. FOTBCD addresses this by spanning multiple regions in France.

Method: The dataset is derived from French orthophotos and topographic data, covering 28 departments. It includes binary and instance-level annotations, with training and evaluation sets geographically disjoint.

Result: FOTBCD demonstrates improved cross-domain generalization compared to existing datasets like LEVIR-CD+ and WHU-CD, validated through empirical benchmarks.

Conclusion: FOTBCD provides a geographically diverse and high-quality dataset for building change detection, enhancing model evaluation and generalization.
...
TL;DR: TTSA3R is a training-free framework for 3D reconstruction that addresses memory forgetting by combining temporal and spatial adaptive updates, improving long-term stability.


<details>
  <summary>Details</summary>
Motivation: Streaming recurrent models for 3D reconstruction suffer from catastrophic memory forgetting, and existing methods lack consideration of temporal and spatial consistency.

Method: TTSA3R uses a Temporal Adaptive Update Module and a Spatial Contextual Update Module to regulate updates based on state evolution and spatial alignment.

Result: TTSA3R shows only 15% error increase on extended sequences vs. 200% in baselines, enhancing long-term reconstruction stability.

Conclusion: TTSA3R effectively improves 3D reconstruction stability by leveraging temporal and spatial adaptive updates, outperforming existing methods.
...
TL;DR: UniGeo is a unified 3D indoor detection framework that improves geometric feature modeling and representation in sparse point clouds, outperforming previous methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of existing methods in modeling geometric relationships and feature distribution in sparse point cloud scenes.

Method: Introduces a geometry-aware learning module and a dynamic channel gating mechanism to enhance geometric features and optimize point cloud representation.

Result: Demonstrates superior performance on six indoor scene datasets.

Conclusion: UniGeo effectively improves 3D object detection by enhancing geometric feature modeling and representation in sparse point clouds.
