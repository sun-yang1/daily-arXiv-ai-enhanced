...
TL;DR: Using frequency-domain analysis with 2D DFT and ResNet50 to detect StyleGAN-generated images, achieving 92.8% accuracy and 0.95 AUC.


<details>
  <summary>Details</summary>
Motivation: The rapid rise of photorealistic GAN-generated images poses challenges for image forensics and industrial systems requiring content authenticity.

Method: Apply 2D Discrete Fourier Transform to images to detect subtle periodic artifacts, then train a ResNet50 neural network on the transformed images to differentiate real from synthetic ones.

Result: The frequency-domain model achieves 92.8% accuracy and 0.95 AUC, significantly outperforming equivalent models trained on raw spatial-domain images.

Conclusion: GAN-generated images have unique frequency-domain signatures, and combining signal processing with deep learning enhances digital forensics and industrial AI trustworthiness.
...
TL;DR: ViTs paired with contrastive learning improve 3D shape representation, achieving 90.6% accuracy on ModelNet10, reducing the need for labeled data and overcoming CNN limitations.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of CNNs in recognizing 3D objects from 2D images, particularly their inability to capture crucial shape relationships and the need for large labeled datasets.

Method: The method involves pairing ViT-based architectures with both supervised and self-supervised contrastive learning objectives to capture global shape semantics and refine local features.

Result: The results show promising performance, with supervised contrastive losses achieving 90.6% accuracy on ModelNet10, demonstrating the effectiveness of ViTs and contrastive learning in 3D shape understanding.

Conclusion: The paper concludes that combining Vision Transformers (ViTs) with contrastive learning objectives effectively addresses the challenges of 3D shape representation learning, achieving high accuracy (e.g., 90.6% on ModelNet10) and reducing reliance on extensive labeled data.
...
TL;DR: FutrTrack is a camera-LiDAR multi-object tracking framework using transformer-based refinement and fusion, achieving strong performance on benchmarks with reduced identity switches.


<details>
  <summary>Details</summary>
Motivation: The paper aims to enhance 3D multi-object tracking by combining camera and LiDAR data without relying on explicit motion models, addressing challenges like occlusion and viewpoint changes.

Method: FutrTrack introduces a transformer-based smoother and a fusion-driven tracker, leveraging multimodal two-stage transformer refinement and tracking. It integrates bounding boxes with BEV fusion features from cameras and LiDAR, using geometric and semantic cues for robust re-identification.

Result: FutrTrack achieves an aMOTA of 74.7 on the nuScenes test set, demonstrating significant benefits from multimodal sensor features compared to single-sensor approaches.

Conclusion: FutrTrack provides an efficient framework for improving transformer-based trackers, achieving strong performance on 3D MOT benchmarks with reduced identity switches and competitive accuracy, even with limited data and no pretraining.
...
TL;DR: Online Label Smoothing (OLS) dynamically adjusts soft labels during training, improving accuracy and calibration in medical image classification.


<details>
  <summary>Details</summary>
Motivation: Deep learning models in medical image classification often produce overconfident predictions, undermining reliability. Traditional label smoothing fails to account for class relationships.

Method: The study explores Online Label Smoothing (OLS), a dynamic approach that adjusts soft labels during training based on the model's prediction patterns. It evaluates OLS on the RadImageNet dataset using ResNet-50, MobileNetV2, and VGG-19 architectures.

Result: OLS improves Top-1 and Top-5 accuracy compared to standard methods and enhances feature embedding compactness and separation.

Conclusion: OLS is a practical and effective solution for improving predictive performance and calibration in medical image classification, making AI systems more trustworthy.
...
TL;DR: A detection framework for fisheye cameras that uses pre/post-processing and ensemble methods to handle radial distortion, achieving 8th place in AI City Challenge Track 4.


<details>
  <summary>Details</summary>
Motivation: Fisheye cameras capture wide fields of view but suffer from strong radial distortion and nonuniform resolution, particularly near image boundaries, which degrades object detection performance.

Method: Employs a pre and post-processing pipeline to enhance detection consistency across distorted regions, trains multiple state-of-the-art detection models on fisheye imagery, and combines their outputs through ensemble strategy.

Result: Achieved F1 score of 0.6366 on the 2025 AI City Challenge Track 4, ranking 8th out of 62 teams.

Conclusion: The framework effectively addresses fisheye imagery challenges, demonstrating robust detection performance despite severe distortion near image boundaries.
...
TL;DR: Real-time render-aware filtering method for 3D Gaussian Splatting that reduces visual noise when viewing models outside training viewpoints by targeting anisotropic orientation instabilities using sensitivity scores from intermediate gradients.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting models exhibit substantial visual noise when viewed from camera positions outside the training data distribution due to uncertain density, color, and geometry predictions in extrapolated regions.

Method: Proposes a novel real-time render-aware filtering method that leverages sensitivity scores derived from intermediate gradients to explicitly target instabilities caused by anisotropic orientations rather than isotropic variance.

Result: Experimental evaluation shows substantial improvements in visual quality, realism, and consistency compared to existing NeRF-based approaches like BayesRays, with seamless real-time integration into existing 3DGS rendering pipelines.

Conclusion: The method effectively addresses generative uncertainty in 3D reconstruction systems, enabling high visual fidelity during free navigation outside original training viewpoints without requiring extensive retraining or fine-tuning.
...
TL;DR: BrainPuzzle is a hybrid two-stage framework combining physics-based modeling with machine learning to achieve accurate speed-of-sound mapping for quantitative transcranial ultrasound brain imaging, overcoming limitations of traditional FWI and purely data-driven methods.


<details>
  <summary>Details</summary>
Motivation: Ultrasound brain imaging faces challenges due to skull-brain sound speed differences and poor probe coupling. Traditional FWI methods suffer from weak signals and incomplete coverage, while purely data-driven approaches fail to model complex wave propagation through bone, leading to quantitatively biased results.

Method: Two-stage hybrid framework: 1) Reverse time migration produces migration fragments from multi-angle acquisitions; 2) Transformer-based super-resolution encoder-decoder with graph-based attention unit fuses fragments into coherent SoS images. Uses partial-array acquisition with movable low-count transducers.

Result: Experiments on synthetic datasets show BrainPuzzle achieves superior SoS reconstruction accuracy and image completeness compared to existing methods.
...
TL;DR: The paper evaluates cultural bias in generative image models, focusing on both text-to-image (T2I) and image-to-image (I2I) systems, revealing biases toward Global-North depictions and superficial cultural edits.


<details>
  <summary>Details</summary>
Motivation: Cultural misrepresentation in generative image models is underexplored, especially in I2I systems, prompting a standardized evaluation across countries and eras.

Method: The study uses open models with fixed settings, combining automatic metrics, a culture-aware VQA, and expert human judgments to audit T2I and I2I systems.

Result: Findings include default Global-North biases, erosion of cultural fidelity in I2I edits, and superficial cultural cues in I2I models.

Conclusion: The paper provides a reproducible benchmark for diagnosing cultural bias in generative models, highlighting the unreliability of culture-sensitive edits in current systems.
...
TL;DR: FIBAR is a simple filter-based method for reconstructing intensity images from event camera data that runs efficiently on CPU, offering an alternative to GPU-based neural network approaches.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reconstructing intensity images from event cameras typically rely on neural networks that require GPUs, creating a need for simpler, more efficient CPU-based approaches.

Method: Uses temporal IIR filter to integrate intensity changes from events, detects stale pixels with a novel algorithm, and applies Gaussian filtering to reduce noise based on the assumption that absence of events indicates low image gradient.

Result: FIBAR runs at 42-140 million events/s on a laptop CPU, produces noisier reconstructions with ghost images compared to neural networks, but is sufficient for tasks like fiducial marker detection.

Conclusion: While FIBAR produces lower quality reconstructions than neural network methods, it provides a practical CPU-based alternative for applications where computational efficiency is prioritized over image quality.
...
TL;DR: TBTLRR - a novel transformed bilateral tensor low-rank representation model that learns adaptive unitary transforms for better image clustering with improved noise robustness.
...
Method: Proposes TBTLRR model with data-adaptive tensor nuclear norm using learned unitary transforms, bilateral structure for local correlations, and ℓ₁/₂-norm + Frobenius norm regularization for complex noise handling. Uses ADMM-based optimization algorithm.
...
TL;DR: Endoshare is a cross-platform app for standardizing and de-identifying surgical videos, developed with user feedback, achieving high usability and strong recommendations, though further validation is needed.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of heterogeneous recording formats and privacy concerns in video-based surgical assessment, aiming to improve surgical training, research, and quality improvement.

Method: The development followed the software development life cycle with iterative, user-centered feedback, including internal and external surveys to assess usability and adoption.

Result: Endoshare achieved high usability scores (4.68 +/- 0.40/5 and 4.03 +/- 0.51/5 initially, improving to 4.38 +/- 0.48/5 after refinement) and strong recommendations (9.20 +/- 0.79/10) from surgeons. Processing time varied with mode, video duration, and hardware.

Conclusion: Endoshare offers a transparent and user-friendly solution for standardized, privacy-preserving surgical video management, though further compliance certification and interoperability validation are needed for broader deployment.
...
TL;DR: The paper introduces Attentive Convolution (ATConv), which enhances Conv with principles from SA (adaptive routing and lateral inhibition). ATConv outperforms SA in vision tasks, enabling efficient models like AttNet (84.4% accuracy on ImageNet-1K) and improving diffusion-based image generation.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the quadratic complexity of SA being a bottleneck for practical applications, while Conv offers linear complexity but lacks the expressivity of SA. The paper aims to bridge this gap by enhancing Conv with principles derived from SA.

Method: The method involves re-examining CNN design principles to identify why SA outperforms Conv. The authors propose ATConv, which integrates adaptive routing and lateral inhibition into Conv, and validate its performance through experiments on vision tasks and diffusion-based image generation.

Result: Results show that ATConv consistently outperforms SA mechanisms in vision tasks. AttNet, built on ATConv, achieves 84.4% ImageNet-1K Top-1 accuracy with only 27M parameters. In diffusion-based image generation, replacing SA with ATConv improves performance (e.g., reducing FID by 0.15).

Conclusion: The paper concludes that the proposed Attentive Convolution (ATConv) effectively bridges the performance gap between Convolutions (Conv) and Self-Attention (SA) by incorporating adaptive routing and lateral inhibition principles. ATConv, with its simple $3\times3$ kernels, outperforms SA mechanisms in vision tasks and enables the creation of efficient CNN models like AttNet.
...
TL;DR: StableSketcher enhances diffusion models for hand-drawn sketch generation using fine-tuned autoencoders and reinforcement learning, outperforming baselines. SketchDUO, a new dataset, addresses prior dataset limitations.


<details>
  <summary>Details</summary>
Motivation: Challenges in synthesizing pixel-based human-drawn sketches, a form of abstract expression, persist despite advancements in diffusion models.

Method: The framework fine-tunes a variational autoencoder for better latent decoding and integrates a reinforcement learning reward function based on visual question answering to enhance text-image alignment.

Result: StableSketcher achieves improved stylistic fidelity and better alignment with prompts compared to baseline models.

Conclusion: StableSketcher successfully generates hand-drawn sketches with high prompt fidelity and stylistic accuracy, outperforming the Stable Diffusion baseline. The introduction of SketchDUO addresses dataset limitations by providing instance-level sketches with captions and QA pairs.
...
TL;DR: The paper introduces BIOCAP, a biological foundation model trained with synthetic captions, improving species classification and text-image retrieval by aligning images and captions with latent biological traits.


<details>
  <summary>Details</summary>
Motivation: The work addresses the challenge of obtaining faithful, instance-specific captions at scale for biological multimodal foundation models, aiming to align images and captions with the latent morphospace of species.

Method: The authors generate synthetic captions using multimodal large language models (MLLMs), guided by Wikipedia-derived visual information and taxon-tailored format examples, to train BIOCAP (BIOCLIP with Captions).

Result: BIOCAP achieves strong performance in species classification and text-image retrieval, highlighting the effectiveness of synthetic captions.

Conclusion: The study demonstrates the value of descriptive captions in enhancing biological foundation models, showing improved performance in species classification and text-image retrieval.
...
TL;DR: A novel system combining deep learning and physics-based tracking for detecting and tracking fast-moving small objects in 3D space, outperforming Kalman filter trackers by up to 70% in accuracy.


<details>
  <summary>Details</summary>
Motivation: The challenge of detecting and tracking fast-moving tiny objects is underexplored, despite advancements in general object detection and tracking.

Method: Combines deep learning-based detection with a physics-based tracking algorithm, integrating kinematics motion equations and an outlier detection module.

Result: The system achieves up to 70% less Average Displacement Error compared to Kalman filter-based trackers on a custom racquetball dataset.

Conclusion: The system demonstrates the effectiveness of integrating physics-based models with deep learning for real-time 3D detection and tracking, with applications in autonomous robotics.
...
TL;DR: A novel view synthesis method called inverse image-based rendering that generates light fields from single images using neural rendering with cross-attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Traditional light field acquisition requires computational costs or specialized hardware like bulky camera setups. The paper aims to broaden light field applicability by enabling generation from single images.

Method: Proposes inverse image-based rendering - a neural rendering pipeline that reconstructs light flows from image pixels. Uses cross-attention to compute relationships among source rays and predict target ray colors. Iteratively generates novel views while updating occluded contents.

Result: The method works well on various challenging datasets without retraining or finetuning after training on synthetic data, and outperforms state-of-the-art novel view synthesis methods.

Conclusion: Inverse image-based rendering successfully generates light fields from single images, demonstrating effectiveness across diverse datasets and superior performance compared to existing methods.
...
TL;DR: LogitGap is a novel post-hoc OOD detection method that exploits the relationship between maximum logit and remaining logits to improve ID/OOD separability, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing post-hoc OOD detection methods often underexploit the rich information in model's logits space, particularly the relationship between maximum logit and other logits.

Method: Proposes LogitGap which explicitly uses the gap between maximum logit and remaining logits. Introduces a training-free strategy to automatically identify the most informative logits subset for scoring.
...
Conclusion: LogitGap effectively leverages logit space information for OOD detection, providing both theoretical analysis and empirical evidence of its superiority over existing methods.
...
TL;DR: PartNeXt is a new dataset with 23,000 textured 3D models and fine-grained part labels, addressing limitations of existing datasets like PartNet. It improves performance in part segmentation and introduces a new benchmark for 3D-LLMs.


<details>
  <summary>Details</summary>
Motivation: Existing datasets like PartNet are limited by untextured geometries and expert-dependent annotations, hindering scalability and usability.

Method: PartNeXt introduces a dataset with high-quality, textured 3D models and fine-grained, hierarchical part labels across 50 categories. It benchmarks tasks like part segmentation and 3D part-centric question answering.

Result: PartNeXt outperforms PartNet in tasks like part segmentation and reveals gaps in 3D-LLMs for part grounding. Training Point-SAM on PartNeXt shows significant improvements.

Conclusion: PartNeXt advances structured 3D understanding by offering scalable annotation, texture-aware labels, and multi-task evaluation, opening new research avenues.
...
TL;DR: The paper introduces an 8D pose estimation method for articulated bicycles and cyclists from a single RGB image, improving upon 6D pose methods by estimating steering and pedal angles for better travel direction prediction.


<details>
  <summary>Details</summary>
Motivation: Accurate pose estimation of cyclists is critical for safety in autonomous driving, but existing 6D pose methods fail to account for articulated bicycle parts (steering/pedals), which affect travel direction.

Method: The proposed model jointly estimates 8D pose (including steering and pedal angles) and 3D keypoints, using a mix of synthetic and real image data for training.

Result: The method achieves competitive results compared to state-of-the-art 6D pose estimators, demonstrating improved accuracy in estimating fine-grained bicycle pose and travel direction.

Conclusion: The 8D pose estimation method enhances cyclist safety by providing more detailed pose information, outperforming rigid 6D pose estimators in capturing articulated bicycle movements.
...
TL;DR: A novel approach for Compositional Zero-Shot Learning that uses multimodal knowledge accumulation from unsupervised data to adapt prototypes at test time, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing CZSL methods suffer from performance degradation due to distribution shift caused by unseen attribute-object compositions at test time.

Method: Proposes accumulating comprehensive knowledge from textual and visual modalities using unsupervised data to update multimodal prototypes, with adaptive update weights and dynamic priority queue for high-confidence images. Uses multimodal collaborative representation learning to align textual and visual prototypes.

Result: Achieves state-of-the-art performance on four benchmark datasets under both closed-world and open-world settings.

Conclusion: The proposed approach effectively overcomes distribution shift challenges in CZSL by flexibly adapting to test-time shifts through multimodal knowledge accumulation and prototype alignment.
...
TL;DR: IB-GAN is a new GAN-based unsupervised model for disentangled representation learning that uses Information Bottleneck framework to optimize GAN, achieving competitive disentanglement scores and better sample quality than existing methods.


<details>
  <summary>Details</summary>
Motivation: To improve disentangled representation learning in GANs by applying the Information Bottleneck framework to constrain mutual information between input and generated output, addressing limitations of existing approaches like InfoGAN.

Method: IB-GAN leverages an intermediate stochastic layer in the generator to constrain mutual information between input and output. The architecture is similar to InfoGAN but uses IB framework for optimization, with the intermediate layer serving as a learnable latent distribution trained jointly with the generator in end-to-end fashion.

Result: IB-GAN achieves competitive disentanglement scores to state-of-the-art β-VAEs and outperforms InfoGAN on dSprites and Color-dSprites datasets. It also generates better visual quality and diversity of samples (measured by FID score) on CelebA and 3D Chairs datasets compared to β-VAEs and InfoGAN.

Conclusion: The Information Bottleneck framework can be effectively applied to GAN optimization for disentangled representation learning, resulting in improved interpretability, competitive disentanglement performance, and superior sample generation quality compared to existing methods.
...
TL;DR: PPMStereo introduces a Pick-and-Play Memory module for efficient long-range temporal consistency in stereo video depth estimation, achieving state-of-the-art performance with reduced computational costs.


<details>
  <summary>Details</summary>
Motivation: Temporally consistent depth estimation from stereo video is critical for real-world applications like augmented reality, but remains challenging due to the difficulty in modeling long-term temporal consistency efficiently. Previous methods face a fundamental trade-off between limited temporal modeling and high computational costs.

Method: Proposes PPMStereo with a Pick-and-Play Memory (PPM) module inspired by human decision-making. PPM consists of a 'pick' process that identifies the most relevant frames and a 'play' process that weights selected frames adaptively for spatio-temporal aggregation, maintaining a compact yet informative memory buffer.

Result: Extensive experiments show state-of-the-art performance in both accuracy and temporal consistency. Notably achieves 0.62/1.11 TEPE on Sintel clean/final (17.3% & 9.02% improvements over BiDAStereo) with fewer computational costs.

Conclusion: PPMStereo effectively addresses the temporal consistency challenge in stereo depth estimation through an efficient memory-based approach, demonstrating significant improvements over existing methods while maintaining computational efficiency.
...
TL;DR: The paper evaluates text-to-video (T2V) and image-to-video (I2V) models as implicit simulators of pedestrian dynamics, revealing effective priors for multi-agent behavior but identifying areas for improvement.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for video generation models focus on individual subjects, leaving the plausibility of multi-agent dynamics unverified.

Method: The study proposes an evaluation protocol for T2V and I2V models, using start frames from datasets for I2V and a prompt suite for T2V, along with a method to reconstruct 2D bird's-eye view trajectories.

Result: Leading models show effective priors for plausible multi-agent behavior but exhibit failure modes like merging and disappearing people.

Conclusion: The study highlights the potential of video generation models as world simulators while identifying specific areas for future improvement.
...
TL;DR: SPAN introduces continuous regression for suspicious intention analysis, outperforming discrete methods with improved detection, explainability, and practical utility in security.


<details>
  <summary>Details</summary>
Motivation: Existing discrete classification methods fail to capture the continuous and evolving nature of suspicious intentions, limiting early intervention and explainability in video surveillance.

Method: The Suspicion Progression Analysis Network (SPAN) shifts from discrete classification to continuous regression, incorporating Temporal Point Process (TPP) theory, Suspicion Coefficient Modulation, and Concept-Anchored Mapping to model suspicion progression.

Result: SPAN reduces MSE by 19.8%, improves average mAP by 1.78%, and achieves a 2.74% mAP gain in low-frequency cases, demonstrating superior performance in capturing subtle behavioral changes.

Conclusion: SPAN's continuous suspicion modeling approach outperforms discrete classification methods, enabling earlier detection, proactive intervention, and enhanced explainability in security applications.
...
TL;DR: The paper analyzes 54 brain MRI datasets, uncovering imbalances and heterogeneity, and shows that preprocessing alone can't remove dataset biases, advocating for adaptive strategies in foundation model development.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the scarcity of systematic assessments of scale, diversity, and consistency in brain MRI datasets, which are critical for developing foundation models.

Method: The research involves analyzing 54 publicly accessible brain MRI datasets, characterizing modality composition, disease coverage, and dataset scale at the dataset level. At the image level, it quantifies voxel spacing, orientation, and intensity distributions. It also evaluates preprocessing variability and conducts a feature-space case study using a 3D DenseNet121.

Result: The study reveals strong imbalances between healthy and clinical populations, substantial heterogeneity in image-level characteristics, and residual covariate shift after preprocessing, indicating that harmonization alone cannot eliminate inter-dataset bias.

Conclusion: The study highlights the persistent variability and biases in public brain MRI datasets despite preprocessing efforts, underscoring the necessity for preprocessing-aware and domain-adaptive strategies to develop generalizable foundation models for brain MRI.
...
TL;DR: RAPO++ is a cross-stage prompt optimization framework for text-to-video (T2V) generation, enhancing prompts through retrieval, iterative refinement, and LLM fine-tuning, significantly improving video quality without modifying the generative model.


<details>
  <summary>Details</summary>
Motivation: User-provided prompts in T2V generation are often short and misaligned with training data, limiting the potential of diffusion-based models.

Method: RAPO++ unifies three stages: retrieval-augmented prompt optimization (RAPO), sample-specific prompt optimization (SSPO), and LLM fine-tuning, leveraging multi-source feedback and training-data alignment.

Result: RAPO++ achieves significant improvements in semantic alignment, compositional reasoning, temporal stability, and physical plausibility across multiple benchmarks.

Conclusion: RAPO++ is a model-agnostic, cost-efficient, and scalable solution that sets a new standard for prompt optimization in T2V generation.
...
TL;DR: FlowCycle introduces a target-aware intermediate state for text-based image editing, optimizing it via a cycle-consistent process to achieve better quality and consistency than current methods.


<details>
  <summary>Details</summary>
Motivation: Current methods construct intermediate states in a target-agnostic manner, leading to limited editability or inconsistency when modifications deviate from the source. The paper argues for a target-aware intermediate state to address this issue.

Method: FlowCycle is an inversion-free and flow-based editing framework that parameterizes corruption with learnable noises and optimizes them through a cycle-consistent process.

Result: Extensive ablations demonstrate that FlowCycle outperforms existing methods in editing quality and consistency.

Conclusion: FlowCycle achieves superior editing quality and consistency over state-of-the-art methods by introducing a target-aware intermediate state and optimizing it through a cycle-consistent process.
...
TL;DR: CURL, a self-supervised learning framework, improves fetal movement detection using contrastive learning, achieving 78.01% sensitivity and 81.60% AUROC.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the subjectivity and limited accuracy of traditional fetal movement detection methods like maternal perception and cardiotocography (CTG).

Method: The paper proposes Contrastive Ultrasound Video Representation Learning (CURL), a self-supervised learning framework using dual-contrastive loss (spatial and temporal) and a task-specific sampling strategy for effective separation of movement and non-movement segments.

Result: CURL achieves a sensitivity of 78.01% and an AUROC of 81.60% on an in-house dataset of 92 subjects with 30-minute ultrasound sessions.

Conclusion: The paper concludes that CURL demonstrates potential for reliable and objective fetal movement analysis, paving the way for improved prenatal monitoring and clinical decision-making.
...
TL;DR: EditInfinity adapts VQ-based generative models for text-driven image editing by leveraging exact intermediate quantized representations to achieve precise image inversion, outperforming diffusion-based methods.
...
Method: Proposes EditInfinity which adapts Infinity (a binary-quantized generative model) with an efficient image inversion mechanism integrating text prompting rectification and style preservation, plus a holistic smoothing strategy.
...
Conclusion: Leveraging VQ-based models' exact intermediate representations enables more effective supervision for precise image inversion, leading to better text-driven image editing with high fidelity and semantic alignment.
...
TL;DR: The paper investigates the root cause of hallucinations in LVLMs, proposing a framework to detect and suppress them, validating the role of context in longer responses.


<details>
  <summary>Details</summary>
Motivation: To understand whether hallucinations in LVLMs are due to response length or deeper mechanisms like context reliance, and to improve mitigation.

Method: Proposes an 'induce-detect-suppress' framework: inducing hallucinations via designed contexts, detecting high-risk cases early, and suppressing them during decoding.

Result: Consistent, significant improvements across benchmarks, validating the framework and the hypothesis on context's role.

Conclusion: The study provides insights into hallucinations in LVLMs, emphasizing context's role and offering a practical framework for mitigation.
