...
TL;DR: The paper introduces CMER-Bench, large-scale datasets MER-17M and CMER-3M, and CMERNet, a model that excels in complex mathematical expression recognition.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of robust recognition of complex mathematical expressions, as current methods perform poorly on them due to the lack of diverse and complex training samples.

Method: The paper introduces CMER-Bench for evaluation, proposes large-scale datasets MER-17M and CMER-3M, and develops a novel expression tokenizer and Structured Mathematical Language representation. CMERNet is then built upon an encoder-decoder architecture.

Result: Experimental results show that CMERNet outperforms existing MER models and MLLMs on CMER-Bench, demonstrating its effectiveness in handling complex expressions.

Conclusion: The paper concludes that CMERNet, a specialized model trained on CMER-3M, significantly outperforms existing MER models and MLLMs on complex mathematical expression recognition, despite having only 125 million parameters.
...
TL;DR: The paper explores controllable image production in journalism, addressing AIGC's ethical and technical challenges through experiments and a human-in-the-loop pipeline, recommending evaluation metrics for human-AI collaboration.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges posed by opaque AIGC tools in journalism, including misinformation, authenticity, semantic fidelity, and interpretability issues, which create ethical, sociotechnical, and trust dilemmas.

Method: The paper conducts two experiments: (1) testing cross-platform adaptability via standardized prompts across three scenes, and (2) building a human-in-the-loop modular pipeline combining high-precision segmentation, semantic alignment, and style regulating tools. The pipeline includes CLIP-based semantic scoring, NSFW/OCR/YOLO filtering, and verifiable content credentials.

Result: The results highlight disparities in semantic alignment, cultural specificity, and visual realism due to training-corpus bias and platform-level filtering. The proposed pipeline ensures editorial fidelity and traceable deployment.

Conclusion: The paper concludes by proposing a human-AI collaboration mechanism for AIGC-assisted image production in special coverage, emphasizing the importance of evaluating Character Identity Stability (CIS), Cultural Expression Accuracy (CEA), and User-Public Appropriateness (U-PA) to address ethical and sociotechnical challenges.
...
TL;DR: A hybrid model (MobileCoAtNet) for endoscopic images achieves high accuracy, but LLMs struggle with stable clinical reasoning, highlighting the need for safer systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap between what a medical image classifier sees and the type of reasoning clinicians expect, as current models either lack explainability (image classifiers) or produce unstable explanations (LLMs).

Method: The study introduces a hybrid model, MobileCoAtNet, designed for endoscopic images, achieving high accuracy across eight stomach-related classes. The model's outputs drive reasoning by several LLMs, evaluated against two expert-verified benchmarks covering causes, symptoms, treatment, lifestyle, and follow-up care.

Result: MobileCoAtNet achieves high accuracy in classification, but the LLMs' reasoning remains unstable and falls short of human-level stability. Strong classification improves explanation quality, but LLMs still change their reasoning with varying prompts.

Conclusion: The study concludes that while combining deep learning (DL) with large language models (LLMs) can generate useful clinical narratives, current LLMs are still unreliable for high-stakes medical decisions. The framework highlights their limitations and suggests a path toward safer reasoning systems.
...
TL;DR: The paper investigates the limitations of multimodal large language models (MLLMs) in biomedical decision-making tasks, proposing strategies to improve performance.


<details>
  <summary>Details</summary>
Motivation: Despite advancements in MLLMs, they struggle with basic Medical Decision Making (MDM) tasks, prompting the need for improvement.

Method: The study uses two datasets: Alzheimer's disease classification and MIMIC-CXR chest radiograph classification, testing text-only, vision-only, and multimodal approaches.

Result: Text-only reasoning outperforms vision-only or multimodal settings. Three strategies are explored to mitigate this limitation.

Conclusion: Current MLLMs lack grounded visual understanding, highlighting the need for improved multimodal decision-making in healthcare.
...
TL;DR: STAR introduces a task-progressive unified multimodal learning scheme, achieving SOTA results by decomposing learning into stages and using stacked AR modules.


<details>
  <summary>Details</summary>
Motivation: Achieving a unified target for multimodal understanding and generation is challenging due to optimization conflicts and performance trade-offs. The goal is to enhance generative performance while preserving existing comprehension capabilities.

Method: STAR decomposes multimodal learning into multiple stages: understanding, generation, and editing. It freezes the parameters of the fundamental autoregressive (AR) model and progressively stacks isomorphic AR modules, avoiding cross-task interference while expanding the model's capabilities. It also introduces a high-capacity VQ and an implicit reasoning mechanism.

Result: STAR demonstrates superior performance on benchmarks: GenEval (0.91), DPG-Bench (87.44), and ImgEdit (4.34).

Conclusion: STAR achieves state-of-the-art performance on GenEval (0.91), DPG-Bench (87.44), and ImgEdit (4.34), validating its efficacy for unified multimodal learning.
...
TL;DR: Deep learning models (SRDRN and UNet) with added temporal modules improve spatial downscaling of tropospheric ozone data, enhancing performance and convergence speed.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of coarse spatial resolution in satellite data of atmospheric pollutants, which restricts their utility in local-scale environmental analysis and decision-making.

Method: The study extends two deep learning architectures, SRDRN and UNet, with lightweight temporal modules (using sinusoidal or RBF encoding) to fuse temporal features with spatial representations for spatial downscaling of tropospheric ozone.

Result: The results indicate that the proposed time-aware extensions significantly improve downscaling performance and convergence speed compared to their baseline counterparts.

Conclusion: The study concludes that incorporating lightweight temporal modules into deep learning architectures like SRDRN and UNet significantly enhances the spatial downscaling performance of tropospheric ozone data without substantially increasing computational complexity.
...
TL;DR: Proposes a compact representation decoupling geometry and appearance, using surfels and neural fields, reducing primitives and memory usage significantly while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Addresses the inefficiency of Gaussian splatting in novel view synthesis by reducing the number of primitives and memory usage.

Method: Uses surfels for geometry and a combination of a global neural field and per-primitive colors for appearance.

Result: Achieves comparable perceptual quality with 9.7x fewer primitives and 5.5x less memory on outdoor scenes, and 31x fewer primitives and 3.7x less memory on indoor scenes.

Conclusion: The proposed representation is more efficient, rendering twice as fast as existing methods while improving visual quality.
...
TL;DR: VajraV1 introduces architectural improvements over YOLO models, achieving top mAP scores in real-time object detection while maintaining speed.


<details>
  <summary>Details</summary>
Motivation: The paper aims to improve upon existing YOLO-based detectors by introducing architectural enhancements for better accuracy and speed.

Method: VajraV1 combines effective design choices from prior YOLO models to enhance architecture.

Result: VajraV1 outperforms YOLOv12 and YOLOv13 models in mAP across all sizes (Nano, Small, Medium, Large, Xlarge) on the COCO validation set.

Conclusion: VajraV1 sets a new benchmark in real-time object detection by achieving state-of-the-art accuracy across various model sizes while maintaining competitive inference speeds.
...
TL;DR: MoLingo is a text-to-motion model that uses a semantically aligned latent space and cross-attention text conditioning to generate realistic human motion, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve text-to-motion models by addressing two key questions: how to build a semantically aligned latent space for effective diffusion and how to best inject text conditioning for accurate motion-description alignment.

Method: The paper proposes a semantic-aligned motion encoder trained with frame-level text labels to create a diffusion-friendly latent space. It also compares single-token conditioning with multi-token cross-attention, finding the latter superior for motion realism and alignment.

Result: MoLingo sets a new state of the art in human motion generation, as demonstrated by standard metrics and user studies.

Conclusion: MoLingo introduces a text-to-motion model that achieves state-of-the-art performance in human motion generation by focusing on semantically aligned latent spaces and effective text conditioning through cross-attention. The model's code and models will be released for further research.
...
TL;DR: Telescopic Adapters introduce a novel PEFT framework for medical VLSMs, dynamically scaling adapter capacity across transformer layers to improve efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Existing PEFT methods use uniform adapter dimensions, leading to suboptimal parameter allocation and reduced adaptation efficiency in medical imaging domains.

Method: Employ depth-aware scaling to progressively increase adapter capacity from shallow to deep transformer layers, integrating lightweight bottleneck modules within CLIPSeg's encoders.

Result: Achieves superior performance across five medical datasets with only 613k trainable parameters (244x fewer than end-to-end fine-tuning).

Conclusion: Telescopic Adapters establish a new paradigm for efficient medical VLSM fine-tuning, enabling deployment in resource-constrained clinical environments while maintaining accuracy.
...
TL;DR: CFHA is a three-stage diffusion-based framework designed to bridge the domain gap between synthetic and real images for UAV-based human detection, improving detection accuracy by up to 14.1% mAP50.


<details>
  <summary>Details</summary>
Motivation: The domain gap between synthetic and real images hinders the effective application of models trained on synthetic data. CFHA addresses this by transforming synthetic data to better match real-world distributions.

Method: CFHA uses three modules: Global Style Transfer, Local Refinement, and Hallucination Removal, to align synthetic images with real-world data while preserving labels.

Result: The method achieves a significant improvement in detection accuracy (up to +14.1 mAP50) on UAV Sim2Real benchmarks.

Conclusion: CFHA effectively narrows the domain gap between synthetic and real data, enhancing UAV-based human detection performance.
...
TL;DR: The paper introduces SAGE, a system for any-horizon video reasoning, trained using synthetic data and RL post-training, achieving significant performance gains on long and open-ended video tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inefficiency of current state-of-the-art video reasoning models, which process all frames in a single turn, akin to watching entire long videos, and lack the flexibility of human-like any-horizon reasoning.

Method: The method involves: 1) Introducing SAGE, a multi-turn reasoning system for long videos. 2) Developing a synthetic data generation pipeline using Gemini-2.5-Flash to train the orchestrator, SAGE-MM. 3) Proposing an RL post-training recipe to enhance any-horizon reasoning. 4) Curating SAGE-Bench for evaluation.

Result: The results show notable improvements: up to 6.1% on open-ended video reasoning tasks and 8.2% on videos longer than 10 minutes, validating the effectiveness of the proposed system and methods.

Conclusion: The paper concludes that the proposed SAGE system, along with its components (SAGE-MM and RL post-training recipe), effectively addresses the challenge of any-horizon video reasoning, demonstrating significant performance improvements on both general and long-duration video tasks.
...
TL;DR: Route-DETR improves DETR by adaptive routing in self-attention, reducing redundant queries and achieving better performance on COCO and Cityscapes.


<details>
  <summary>Details</summary>
Motivation: DETR suffers from inefficient query competition where multiple queries converge to similar positions, leading to redundant computations. Route-DETR aims to mitigate this by distinguishing between competing and complementary queries.

Method: The method involves dual routing mechanisms (suppressor and delegator routes) using inter-query similarity, confidence scores, and geometry, implemented via learnable low-rank attention biases. A dual-branch training strategy is employed to incorporate routing biases during training without additional inference cost.

Result: Route-DETR achieves a +1.7% mAP gain over DINO on ResNet-50 and reaches 57.6% mAP on Swin-L, surpassing prior state-of-the-art models.

Conclusion: Route-DETR introduces adaptive pairwise routing in decoder self-attention layers to address inefficient query competition in DETR, achieving state-of-the-art performance on COCO and Cityscapes.
...
TL;DR: Proposes KLO-Net, a dynamic K-Nearest Neighbor attention U-Net with Cross Stage Partial encoder for efficient prostate gland segmentation from MRI scans, addressing computational load and memory footprint.


<details>
  <summary>Details</summary>
Motivation: Real-time deployment of prostate MRI segmentation is bottlenecked by computational load and memory footprint, with existing deep learning approaches challenged by anatomical variability.

Method: Introduces KLO-Net, featuring a dynamic K-NN attention mechanism and CSP blocks to adaptively determine attention connections and reduce memory consumption.

Result: Comprehensive experiments on PROMISE12 and PROSTATEx datasets validate the model's computational efficiency and segmentation quality.

Conclusion: KLO-Net effectively bridges the efficiency gap in prostate gland segmentation while maintaining reliable accuracy.
...
TL;DR: A standard UNet is surprisingly effective for SVBRDF prediction, achieving high coherence and accuracy in texture atlases, outperforming more complex designs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of multiview incoherence in SVBRDF predictions and explore the opportunities provided by generated RGB images for improving appearance modeling pipelines.

Method: The paper compares neural architectures and conditions for SVBRDF prediction, leveraging generated RGB images and their conditioning modalities to enhance estimation accuracy.

Result: The study finds that a standard UNet performs comparably to more complex architectures in terms of accuracy and coherence for SVBRDF prediction.

Conclusion: The paper concludes that a standard UNet architecture is competitive with more complex designs for SVBRDF prediction, achieving high accuracy and coherence in texture atlases.
...
TL;DR: The paper introduces 'unbranding,' a task to remove trademarks and subtle brand features while maintaining semantic coherence, and proposes a VLM-based metric for evaluation.


<details>
  <summary>Details</summary>
Motivation: Addressing the gap in prior work that fails to handle specific brand identifiers, especially beyond explicit logos.

Method: Introduces unbranding and constructs a benchmark dataset, using a VLM-based question-answering framework for evaluation.

Result: Confirms unbranding as a distinct problem requiring specialized techniques, validated by the VLM metric.

Conclusion: The study highlights the urgency of unbranding with newer models synthesizing brand identifiers more readily, proposing a novel approach and evaluation framework.
...
TL;DR: A pipeline for generating diverse, high-quality synthetic training data improves marine obstacle segmentation by addressing dataset limitations and environmental challenges.


<details>
  <summary>Details</summary>
Motivation: Marine obstacle detection faces challenges like degraded image quality and limited training data diversity due to factors like sun glitter, fog, and repetitive datasets.

Method: A quality-driven and diversity-aware sample expansion pipeline that combines a class-aware style bank and an adaptive annealing sampler to generate training data at inference time.

Result: The proposed framework improves segmentation performance across marine obstacle benchmarks and enhances visual variation in rare and texture-sensitive classes.

Conclusion: Augmenting training data with controlled synthetic samples consistently improves segmentation performance and increases visual variation in rare and texture-sensitive classes.
...
TL;DR: The paper demonstrates how XAI can diagnose generalization failure in medical imaging models, showing UMamba's attention mechanism failed due to dataset bias.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the critical barrier of domain shift in deep learning models for medical imaging, which hinders clinical deployment. The study aims to move beyond simple performance metrics to understand why models fail catastrophically on external datasets.

Method: The paper employs a two-phase approach: first, quantifying the domain gap between source and target datasets (RSNA CTA Aneurysm and TopCoW Circle of Willis CT) in terms of Z-resolution and background noise. Second, using Seg-XRes-CAM to analyze the model's attention mechanism, measuring overlap between attention maps, ground truth segmentations, and prediction masks.

Result: The model's Dice score dropped from 0.8604 (Source) to 0.2902 (Target). Analysis showed the model's attention mechanism abandoned true anatomical features in the Target domain, focusing on spurious correlations instead.

Conclusion: The study concludes that Explainable AI (XAI) is a powerful diagnostic tool for identifying dataset bias in emerging architectures, specifically highlighting the failure of State-Space Models (SSMs) like UMamba to generalize due to spurious correlations.
...
TL;DR: FocalComm is a novel collaborative perception framework focusing on hard-instance-oriented features for improved pedestrian detection in autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Existing CP approaches optimize for vehicle detection but underperform on smaller, safety-critical objects like pedestrians, leading to potential catastrophic failures.

Method: FocalComm introduces a learnable progressive hard instance mining (HIM) module and a query-based feature-level fusion technique for dynamic feature weighting.

Result: FocalComm outperforms state-of-the-art methods on V2X-Real and DAIR-V2X datasets, showing strong performance in pedestrian detection.

Conclusion: FocalComm enhances collaborative perception by focusing on hard-instance features, improving safety for vulnerable road users.
...
TL;DR: A framework using the Shape Atlas adapts 2D diffusion models for 3D shape completion, overcoming data scarcity and modality gaps, producing high-quality results with practical applications.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of 3D diffusion models, such as the scarcity of high-quality 3D datasets and the modality gap between 3D inputs and 2D latent spaces, by utilizing the generative power of pretrained 2D diffusion models.

Method: The method introduces the Shape Atlas, a compact 2D representation of 3D geometry, to leverage pretrained 2D diffusion models for 3D shape completion from incomplete point clouds. This aligns modalities between 3D inputs and 2D latent spaces.

Result: The framework produces high-quality, detail-preserving shape completions, validated on the PCN and ShapeNet-55 datasets, and shows practical utility in downstream applications like creating artist-created meshes.

Conclusion: The framework successfully adapts 2D diffusion models for 3D shape completion by introducing the Shape Atlas, enabling high-quality, detail-preserving completions and demonstrating practical applications in creating artist-created meshes.
...
TL;DR: Sparse-LaViDa accelerates MDM sampling by dynamically truncating unnecessary masked tokens, achieving 2x speedup without quality loss.


<details>
  <summary>Details</summary>
Motivation: MDMs suffer from slow inference due to redundant masked token processing.

Method: Introduces dynamic token truncation, register tokens for quality preservation, and a specialized attention mask for training-inference consistency.

Result: Achieves up to 2x speedup in tasks like text-to-image generation and image editing.

Conclusion: Sparse-LaViDa effectively balances speed and quality in MDM-based tasks.
...
TL;DR: KFS-Bench is the first benchmark for key frame sampling in long video QA, featuring multi-scene annotations to evaluate sampling strategies directly. It improves QA accuracy by balancing sampling diversity and question-frame similarity.


<details>
  <summary>Details</summary>
Motivation: Prior works indirectly assess frame selection quality via QA accuracy, lacking direct evaluation of sampling strategies. KFS-Bench addresses this gap.

Method: The benchmark includes ground-truth annotations of disjoint scenes per question, enabling direct analysis of sampling approaches. A novel sampling quality metric and adaptive balanced sampling method are proposed.

Result: The adaptive balanced sampling method improves both key frame sampling and QA performance.

Conclusion: KFS-Bench provides a robust framework for evaluating and improving key frame sampling in long video QA, with demonstrated effectiveness in enhancing QA accuracy.
...
TL;DR: The paper reviews deep learning's role in improving scene understanding for autonomous robots, covering object detection, segmentation, and 3D reconstruction, and highlights future research directions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore how deep learning techniques can enhance scene understanding in autonomous robots, overcoming limitations of traditional methods and improving real-time performance in dynamic environments.

Method: The paper reviews deep learning applications in scene understanding, focusing on innovations in object detection, semantic and instance segmentation, depth estimation, 3D reconstruction, and visual SLAM. It highlights how these techniques address limitations of traditional geometric models.

Result: The review demonstrates that deep learning techniques significantly improve depth perception, semantic reasoning, and integration of perception modules in unstructured environments, leading to better decision-making, navigation, and interaction.

Conclusion: The paper concludes by outlining existing challenges and future research directions to advance learning-based scene understanding in autonomous robots, emphasizing the need for further improvements in dynamic and unstructured environments.
...
TL;DR: CITab, a novel SSL framework, leverages semantic-aware tabular modeling and a P-MoLin module to enable scalable and transferrable multi-modal learning, achieving superior performance in Alzheimer's diagnosis.


<details>
  <summary>Details</summary>
Motivation: Existing SSL methods for image-tabular representation learning are limited by rigid tabular modeling mechanisms, hindering transferrable knowledge learning across diverse cohorts.

Method: The paper proposes CITab, a self-supervised learning framework that integrates column headers as semantic cues for tabular modeling and introduces a prototype-guided mixture-of-linear layer (P-MoLin) module to handle tabular data heterogeneity.

Result: CITab outperforms state-of-the-art approaches in Alzheimer's disease diagnosis, as validated on three publicly available data cohorts with 4,461 subjects.

Conclusion: CITab, a novel SSL framework, effectively addresses the limitations of existing methods by enabling cross-tabular multi-modal learning, demonstrating superior performance in Alzheimer's disease diagnosis across diverse cohorts.
...
TL;DR: A learning-based structured light decoding framework improves 3D imaging robustness by matching features in neural space rather than pixel domain, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Traditional structured light methods struggle with occlusions, fine details, and non-Lambertian surfaces due to pixel-domain matching.

Method: Proposes a neural feature matching approach with cost volumes in feature space and a depth refinement module leveraging monocular depth priors.

Result: Achieves superior performance over commercial systems and passive stereo RGB-based methods, generalizing well to real-world scenarios.

Conclusion: The framework demonstrates significant improvements in robustness and detail recovery, validated by synthetic and real-world experiments.
...
TL;DR: A novel neural RGB-D SLAM system using Scene Coordinate Regression (SCR) achieves real-time performance, efficient 3D mapping, and works reliably in dynamic settings.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the use of SCR for neural SLAM, as SCR networks provide efficient, low-memory 3D map representations, fast relocalization, and inherent privacy preservation, making them suitable for neural implicit SLAM.

Method: The method involves using Scene Coordinate Regression (SCR) as the core implicit map representation in a neural SLAM pipeline. They introduce a novel SCR architecture tailored for real-time SLAM and integrate it into a live pipeline, supporting both sparse and dense features.

Result: The system achieves competitive performance on synthetic and real-world benchmarks, demonstrating real-time capabilities and reliability in dynamic environments.

Conclusion: The paper concludes that their novel SCR-based neural RGB-D SLAM system achieves strict real-time performance, offers efficient and low-memory 3D map representations, and operates reliably in dynamic environments.
...
TL;DR: The paper introduces ASAP Textured Gaussians, using adaptive sampling and anisotropic parameterization to improve efficiency in textured 3D Gaussian rendering, reducing memory usage while maintaining high quality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the memory efficiency challenges introduced by texture parameterizations in 3D Gaussian Splatting, which often lead to inefficient sampling and over-parameterization.

Method: The method involves two strategies: adaptive sampling based on Gaussian density distribution and error-driven anisotropic parameterization to allocate texture resources efficiently.

Result: The result is a significant improvement in the quality-efficiency tradeoff, enabling high-fidelity rendering with reduced texture parameters.

Conclusion: The paper concludes that the proposed ASAP Textured Gaussians, through adaptive sampling and anisotropic parameterization, effectively addresses the inefficiencies in existing textured Gaussian methods, achieving high-fidelity rendering with fewer texture parameters.
...
TL;DR: ChartAgent uses Tool-Integrated Reasoning and a modular tool library to enhance chart understanding, especially when annotations are sparse, providing transparent and reproducible results.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs for chart understanding rely heavily on textual annotations and degrade without key numerals. ChartAgent addresses this by mimicking human cognition and ensuring verifiable, reproducible analysis.

Method: The framework employs Tool-Integrated Reasoning (TIR) and a modular tool library (e.g., keyelement detection, OCR) to systematically parse charts. It standardizes intermediate outputs into an Evidence Package for transparency.

Result: Experiments demonstrate ChartAgent's improved robustness in sparse annotation scenarios, offering a trustworthy and extensible solution for chart understanding.

Conclusion: ChartAgent, based on Tool-Integrated Reasoning (TIR), significantly enhances chart understanding by decomposing analysis into observable steps and using an extensible tool library. It improves robustness in sparse annotation settings and provides traceable, reproducible results.
...
TL;DR: OmniDrive-R1 introduces an end-to-end VLM framework for autonomous driving, addressing reliability failures in VLMs through an interleaved Multi-modal Chain-of-Thought (iMCoT) mechanism and Reinforcement-driven visual grounding.


<details>
  <summary>Details</summary>
Motivation: The deployment of VLMs in safety-critical domains like autonomous driving is hindered by reliability failures, particularly object hallucination, due to ungrounded, text-based Chain-of-Thought reasoning.

Method: OmniDrive-R1 uses an iMCoT mechanism and Clip-GRPO algorithm for Reinforcement-driven visual grounding, enabling fine-grained analysis without dense labels.

Result: The model improves reasoning scores from 51.77% to 80.35% and final answer accuracy from 37.81% to 73.62% compared to Qwen2.5VL-7B.

Conclusion: OmniDrive-R1 significantly enhances reliability and performance in autonomous driving applications by unifying perception and reasoning through innovative grounding techniques.
...
TL;DR: SELECT is a multi-modal method for detecting label errors in scene text datasets, using SSLC for realistic training and outperforming existing approaches.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of detecting label errors in real-world scene text datasets, particularly variable-length labels and character-level errors, which existing methods struggle with.

Method: SELECT uses an image-text encoder and a character-level tokenizer to address variable-length sequence labels, label misalignment, and character-level errors. It incorporates SSLC to simulate real-world label errors during training.

Result: SELECT outperforms existing methods in accuracy and practical utility, successfully detecting label errors and improving STR accuracy on real-world datasets.

Conclusion: SELECT is an effective method for detecting label errors in scene text datasets, leveraging multi-modal training and SSLC to improve accuracy and practical utility.
