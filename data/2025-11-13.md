...
TL;DR: AI-based automation of gas plant digitization achieves high accuracy in extracting design data and plant topology, overcoming data variety challenges.


<details>
  <summary>Details</summary>
Motivation: The motivation is to automate the digitization of SNAM's gas plant infrastructure to streamline daily operations, addressing challenges like data variety and lack of standardization.

Method: The method involves using OCR, Vision LLM, Object Detection, Relational Reasoning, and optimization algorithms to process P&ID documents in PDF format. A new Transformer architecture is introduced to enhance Scene Graph Generation for analyzing complex plant component relations.

Result: The solution achieves 91% accuracy in extracting textual design data and 93% accuracy in identifying plant components, with an 80% accuracy in extracting hierarchical structures.

Conclusion: The paper concludes that the synergistic use of AI-based technologies, including OCR, Vision LLM, Object Detection, and a novel Transformer architecture, successfully automates the digitization of gas plant structures with high accuracy (91% for textual data and 93% for component identification).
...
TL;DR: A new evaluation framework detects and quantifies lip leakage in talking face generation, using three test setups and derived metrics, offering a reliable benchmark.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of lip leakage in inpainting-based talking face generation, where generated lips are influenced by the reference image rather than the driving audio, a problem undetectable by standard metrics.

Method: The framework employs three test setups (silent-input generation, mismatched audio-video pairing, and matched audio-video synthesis) and introduces derived metrics like lip-sync discrepancy and silent-audio-based lip-sync scores.

Result: The proposed methodology effectively analyzes and quantifies lip leakage, providing insights into how identity reference selections impact leakage.

Conclusion: The paper concludes by proposing a model-agnostic evaluation methodology to quantify lip leakage in talking face generation, offering a reliable benchmark for future research.
...
TL;DR: MATRIX introduces a dataset and deep learning framework for multi-drone pedestrian tracking in dynamic environments, outperforming static methods with robust accuracy and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing multi-drone surveillance systems struggle with dynamic camera positions and complex occlusions, limiting their effectiveness in real-world scenarios.

Method: The paper introduces a novel deep learning framework for multi-view detection and tracking, incorporating real-time camera calibration, feature-based image registration, and multi-view feature fusion in bird's-eye-view (BEV) representation.

Result: The proposed framework achieves ~90% detection and tracking accuracy in complex environments and maintains ~80% trajectory tracking under challenging conditions. Transfer learning and camera dropout experiments highlight its robustness and generalization.

Conclusion: The MATRIX dataset and framework set a benchmark for dynamic multi-view surveillance systems, demonstrating robust performance in complex environments and strong generalization capabilities.
...
TL;DR: A topology-driven multi-subspace fusion framework on the Grassmannian manifold enhances geometric representation learning by dynamically integrating multiple subspaces, achieving state-of-the-art performance in various tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on static single-subspace representations, missing the dynamic interplay of multiple subspaces needed for complex geometric structures.

Method: Proposes an adaptive multi-subspace modelling mechanism and a multi-subspace interaction block, leveraging topological convergence analysis and Fréchet mean optimisation.

Result: Demonstrates superior performance in 3D action recognition, EEG classification, and graph tasks.

Conclusion: The framework advances geometric deep learning by adapting multi-channel interaction principles to non-Euclidean domains, improving discriminability and interpretability.
...
TL;DR: Time-to-Move (TTM) is a training-free framework for motion- and appearance-controlled video generation using I2V diffusion models, leveraging crude animations as motion cues and dual-clock denoising for precise control.


<details>
  <summary>Details</summary>
Motivation: Existing methods for motion-conditioned video synthesis require costly fine-tuning, limiting flexibility and accessibility. TTM aims to provide precise motion and appearance control without additional training.

Method: TTM uses crude reference animations (e.g., cut-and-drag or depth-based reprojection) as motion cues, adapting SDEdit's approach to videos. It employs dual-clock denoising for region-dependent alignment and preserves appearance with image conditioning.

Result: TTM matches or exceeds training-based baselines in realism and motion control, while enabling pixel-level appearance control beyond text-only prompting.

Conclusion: TTM offers a lightweight, plug-and-play solution for motion- and appearance-controlled video generation, compatible with any backbone and requiring no extra training.
...
TL;DR: A novel CAD framework with a unified memory bank improves flexibility and scalability, outperforming existing methods in anomaly detection across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Existing embedding-based CAD methods require class-specific sub-memory banks for each task, limiting flexibility and scalability. The goal is to overcome this limitation by introducing a unified memory bank.

Method: The framework uses a fixed-size coreset to incrementally update embeddings, allowing continuous knowledge acquisition without task-specific memory fragmentation. Anomaly scores are computed via nearest-neighbor matching during inference.

Result: The method achieves state-of-the-art performance, with average image-level AUROC scores of 0.972 (MVTec AD) and 0.891 (Visa), and 100% accuracy on a real-world electronic paper dataset.

Conclusion: The proposed CAD framework with a unified memory bank outperforms existing methods, achieving high accuracy and robustness in anomaly detection across multiple datasets, including real-world scenarios.
...
TL;DR: A diffusion-based denoising and time-aware actor-critic framework improves accident anticipation in autonomous driving, achieving robust and timely predictions under noisy conditions.


<details>
  <summary>Details</summary>
Motivation: The challenges of noisy sensory inputs and the need for timely yet reliable predictions in accident anticipation motivate the development of a unified solution.

Method: The framework integrates diffusion-based denoising with a time-aware actor-critic model. The diffusion module refines noisy inputs, while the actor-critic model optimizes alert timing using long-horizon reasoning and time-weighted rewards.

Result: Experiments on benchmark datasets (DAD, CCD, A3D) show superior accuracy, improved mean time-to-accident, and robustness under noise. Qualitative analyses confirm earlier, stable, and human-aligned predictions.

Conclusion: The proposed framework demonstrates significant improvements in accident anticipation, achieving state-of-the-art accuracy and robustness under noisy conditions, making it suitable for real-world autonomous driving applications.
...
TL;DR: RS-Net scores object pair importance in videos, improving relation prediction in DSGG without changing model architecture, and outperforms existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing DSGG methods lack guidance for non-related object pairs, making it hard to identify meaningful relations during inference. RS-Net addresses this by scoring contextual importance.

Method: RS-Net uses a spatial context encoder with learnable tokens and a temporal encoder to score object pair importance, integrating these scores into a triplet scoring mechanism.

Result: Experiments on Action Genome show RS-Net improves Recall and Precision, especially in mean Recall, outperforming state-of-the-art methods.

Conclusion: RS-Net enhances relation prediction in DSGG by scoring contextual importance of object pairs, improving Recall and Precision without architectural changes to existing models. It maintains efficiency despite added parameters.
...
TL;DR: A lightweight Anonymizing Adapter Module (AAM) reduces privacy leakage in video foundation models by 35% while maintaining utility, addressing limitations of current methods and mitigating gender bias.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address privacy leakage in video foundation models, where current methods require retraining and are task-specific, making them unsuitable for foundational models.

Method: The method involves a lightweight AAM that operates in the latent space, employing three training objectives: clip-level self-supervised privacy, co-training for utility retention, and latent consistency for generalization.

Result: Results show a 35% reduction in privacy leakage with near-baseline utility performance across tasks like Action Recognition, Temporal Action Detection, and Anomaly Detection. The method also mitigates gender bias.

Conclusion: The paper concludes that the Anonymizing Adapter Module (AAM) effectively reduces privacy leakage by 35% while maintaining utility across various downstream tasks. It also mitigates gender bias in action recognition models.
...
TL;DR: The paper shows that compute, not data, is the bottleneck in scaling autoregressive next-pixel prediction, predicting feasibility of pixel-by-pixel image modeling within five years due to rapid compute growth.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore the scaling properties of autoregressive next-pixel prediction, an under-explored framework for unified vision models, and understand its optimal scaling strategies for different tasks.

Method: The paper trains a family of Transformers using IsoFlops profiles across compute budgets up to 7e19 FLOPs, evaluating next-pixel prediction, ImageNet classification accuracy, and generation quality via Fr'echet Distance.

Result: Key findings include: (1) Optimal scaling is task-dependent, with generation requiring faster data growth than classification. (2) Higher resolutions demand faster model size growth than data size. (3) Compute is the main bottleneck, not data.

Conclusion: The study concludes that the primary bottleneck in scaling autoregressive next-pixel prediction is compute, not training data. With compute growing four to five times annually, pixel-by-pixel modeling of images could become feasible within five years.
...
TL;DR: The paper introduces diffusion-finetuning techniques (LoRA, DreamBooth) and clustering to generate balanced training data, outperforming vanilla Stable Diffusion and matching SOTA debiasing methods, especially in severe bias cases.


<details>
  <summary>Details</summary>
Motivation: Image classification systems often inherit biases from uneven group representation in training data. Existing methods using Stable Diffusion struggle to preserve the original data distribution, prompting the need for more accurate techniques.

Method: The study explores diffusion-finetuning techniques (LoRA and DreamBooth) to generate balanced training data. It introduces clustering within groups to train DreamBooth models per cluster, addressing intra-group variations. The generated data is used for pretraining, followed by fine-tuning on real data.

Result: Experiments show the finetuning approaches outperform vanilla Stable Diffusion and achieve results comparable to SOTA debiasing techniques like Group-DRO, with superior performance as dataset bias severity increases.

Conclusion: The proposed diffusion-finetuning techniques, including LoRA and DreamBooth, along with clustering-based DreamBooth models, effectively generate balanced training data. This approach outperforms vanilla Stable Diffusion and matches or surpasses SOTA debiasing methods like Group-DRO, especially in cases of severe dataset bias.
...
TL;DR: WiCV@CVPR 2025 highlighted its role in advancing diversity in computer vision through papers, mentorship, and networking, supported by sponsors and grants.


<details>
  <summary>Details</summary>
Motivation: The goal was to document the impact and evolution of WiCV to support future editions and similar diversity-focused initiatives.

Method: The report provided an overview of the workshop program, participation statistics, mentorship outcomes, and historical trends.

Result: The workshop featured 14 accepted papers, 80 mentees matched with 37 mentors, over 100 onsite participants, and $44,000 USD in support.

Conclusion: The WiCV@CVPR 2025 workshop successfully documented its impact and evolution, serving as a reference for future initiatives promoting diversity, equity, and inclusion in AI and computer vision.
...
TL;DR: PoseKAN improves 3D human pose estimation by using adaptive graph learning to overcome GCN limitations, achieving better performance in occlusions and depth ambiguities.


<details>
  <summary>Details</summary>
Motivation: GCN-based methods for 3D human pose estimation are limited by their local receptive field and spectral bias, which hinder their ability to capture long-range dependencies and high-frequency details.

Method: The paper introduces PoseKAN, an adaptive graph Kolmogorov-Arnold Network (KAN) framework, which employs learnable functions on graph edges for data-driven feature transformations. It includes multi-hop feature aggregation, residual PoseKAN blocks, and global response normalization.

Result: Extensive experiments show that PoseKAN outperforms state-of-the-art methods in handling occlusions and depth ambiguities.

Conclusion: PoseKAN demonstrates competitive performance in 3D human pose estimation by addressing the limitations of GCNs through adaptive graph learning and multi-hop feature aggregation.
...
TL;DR: SIFT-Graph integrates SIFT keypoints and GAT to create a robust, structure-aware defense against adversarial attacks, maintaining high clean accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the vulnerability of deep vision models to adversarial attacks due to their reliance on pixel-level representations, highlighting the need for inherently robust visual features.

Method: The method combines Scale-Invariant Feature Transform (SIFT) keypoints with a Graph Attention Network (GAT) to extract robust, scale and rotation invariant features, which are then fused with traditional vision models like Vision Transformers and CNNs.

Result: Preliminary results show improved robustness against gradient-based white box adversarial attacks with only a marginal drop in clean accuracy.

Conclusion: The SIFT-Graph framework successfully enhances the robustness of traditional vision models by integrating structurally meaningful features, demonstrating resilience against adversarial attacks with minimal impact on clean accuracy.
...
TL;DR: DT-NVS is a 3D diffusion model using transformer architecture for generating novel views from single images, trained on real-world videos with image-only losses.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion methods are limited to small camera movements or unnatural object scenes, restricting real-world applications. This paper addresses generating novel views for everyday scenes from single views.

Method: Proposes DT-NVS with transformer backbone, architectural improvements for 3D translation, novel camera conditioning for unaligned datasets, and a training paradigm swapping reference frame roles.

Result: Shows improvements over state-of-the-art 3D diffusion and deterministic methods in generalized novel view synthesis, generating diverse outputs.

Conclusion: The approach effectively handles real-world scenes, advancing single-image novel view synthesis beyond previous constraints.
...
TL;DR: This paper proposes Shadow-informed Pose Feature (SiPF) and Rotation-invariant Attention Convolution (RIAttnConv) to address the loss of global pose information in rotation-invariant 3D point cloud learning, solving the Wing-tip feature collapse problem and achieving superior performance on classification and segmentation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing rotation-invariant methods for 3D point clouds lose global pose information due to limited receptive fields, causing Wing-tip feature collapse where symmetric components like airplane wings become indistinguishable.

Method: Introduces SiPF, which augments local rotation-invariant descriptors with a global 'shadow' reference point from learned rotation. Proposes RIAttnConv operator to integrate SiPFs into feature aggregation. Includes a Bingham distribution-based module to dynamically learn optimal global rotation for shadow consistency.

Result: Extensive experiments show the approach substantially outperforms existing rotation-invariant methods on 3D classification and part segmentation benchmarks, especially for fine-grained spatial discrimination under arbitrary rotations.

Conclusion: The SiPF and RIAttnConv framework successfully preserves global pose awareness while maintaining rotation invariance, effectively solving the Wing-tip feature collapse problem in 3D point cloud analysis.
...
TL;DR: SAS-SSM introduces a skeleton structure-aware approach with spatiotemporal convolution and stride-based scanning for efficient 3D human pose estimation, achieving competitive performance with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Existing SSM-based methods flatten 2D pose sequences using manual scans, disrupting spatial structure and entangling spatiotemporal features, which hinders capturing complex pose dependencies.

Method: Uses structure-aware spatiotemporal convolution to capture local joint interactions, followed by a stride-based scan strategy to build multi-scale global structural representations while maintaining linear complexity.

Result: SasMamba model achieves competitive 3D pose estimation performance with significantly fewer parameters compared to hybrid models.

Conclusion: SAS-SSM effectively models local and global pose information efficiently, addressing limitations of previous SSM approaches in preserving spatial structure.
...
TL;DR: This paper proposes MFAVBs, a Vision Transformer-based architecture for image clustering that explicitly fuses features from positive pairs through multiple fusion-augmentation blocks, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing contrastive learning networks with implicit encoder interactions fail to fully exploit complementarity and similarity of positive pairs for clustering feature extraction.

Method: Uses shared-weight ViTs for initial feature extraction, followed by multiple fusing-augmenting ViT blocks (MFAVBs) for explicit feature fusion and augmentation, with CLIP-based preprocessing and dual-level projection for loss calculation.

Result: Experiments on seven public datasets show superior clustering performance compared to state-of-the-art techniques.

Conclusion: MFAVBs effectively enhances contrastive clustering by explicit feature fusion and multiple augmentation cycles, demonstrating significant performance improvements.
...
TL;DR: Deep learning models (EfficientNet) classify GBM histopathological regions effectively on training data but struggle with generalization to new data.


<details>
  <summary>Details</summary>
Motivation: Automated identification of histological sub-regions in GBM could enhance morphological understanding and improve clinical diagnostics.

Method: A four-step deep learning approach using EfficientNet architectures (B0-B4) was designed and evaluated on the BraTS-Path 2024 dataset.

Result: EfficientNet-B1 and B4 achieved high F1 scores (0.98) on training data but lower scores (0.546 and 0.517) on validation and testing data, highlighting generalization challenges.

Conclusion: The study demonstrates the potential of deep learning for classifying histopathological sub-regions in GBM, though challenges in generalization to new data remain.
...
TL;DR: VisNet and its biologically inspired variants improve object recognition accuracy by leveraging Hebbian learning and invariant feature capture, validated across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: The challenge of reproducing human-like object recognition efficiency in artificial systems drives the study, aiming to improve recognition accuracy using biologically inspired models.

Method: The research investigates VisNet and its enhanced variants, incorporating radial basis function neurons, Mahalanobis distance-based learning, and retinal-like preprocessing. It leverages Hebbian learning and temporal continuity to build invariant representations.

Result: Enhanced VisNet variants show substantial improvement in recognition accuracy across datasets like MNIST, CIFAR10, and custom symmetric object sets compared to the baseline model.

Conclusion: The study highlights the adaptability and biological relevance of VisNet-inspired architectures, providing a robust and interpretable framework for visual recognition in neuroscience and AI.
...
TL;DR: The paper introduces Asymmetric Cross-modal Knowledge Distillation (ACKD) for weakly connected modalities, proposes the SemBridge framework to address knowledge transmission challenges, and demonstrates superior performance on remote sensing datasets.


<details>
  <summary>Details</summary>
Motivation: The limited availability of paired modalities in real-world scenarios restricts Symmetric Cross-modal Knowledge Distillation (SCKD), prompting the need for a more flexible approach under weak semantic consistency.

Method: The paper proposes the SemBridge framework, which includes a Student-Friendly Matching module for personalized knowledge transfer and a Semantic-aware Knowledge Alignment module for optimal transport path selection using Lagrangian optimization.

Result: The framework outperforms 7 existing methods on 6 model architectures across various datasets, particularly in remote sensing scene classification.

Conclusion: ACKD and the SemBridge framework effectively address knowledge transfer challenges in weakly connected modalities, achieving state-of-the-art performance.
...
TL;DR: A framework fuses visual predictions with LLM structural priors for semi-supervised document layout understanding, achieving high accuracy with minimal labels and offering privacy and semantic benefits.


<details>
  <summary>Details</summary>
Motivation: Document layout understanding is data-intensive, and the paper aims to enhance semi-supervised detection by leveraging LLM structural priors.

Method: The method combines visual predictions from a teacher detector with structural priors from text-pretrained LLMs using inverse-variance fusion to generate refined pseudo-labels.

Result: The framework achieves 88.2±0.3 AP with a lightweight backbone (5% labels) and 89.7±0.4 AP with LayoutLMv3, surpassing standard semi-supervised learning and matching UDOP. Key findings include improved gating, privacy-preserving deployment, and semantic disambiguation.

Conclusion: The framework demonstrates that LLM structural priors complement both lightweight and pretrained architectures, offering privacy-preserving deployment and targeted semantic disambiguation.
...
TL;DR: A novel Consistency Change Detection Framework (CCDF) with Cycle and Semantic Consistency modules improves unsupervised remote sensing change detection by addressing generator overfitting and enhancing detail reconstruction.


<details>
  <summary>Details</summary>
Motivation: Unsupervised remote sensing change detection often suffers from poor performance due to generator overfitting in existing methods.

Method: The framework includes a Cycle Consistency (CC) module to reduce generator overfitting and a Semantic Consistency (SC) module for detail reconstruction.

Result: Extensive experiments show that CCDF outperforms other state-of-the-art approaches.

Conclusion: The proposed Consistency Change Detection Framework (CCDF) effectively addresses the overfitting issue in generator-based reconstruction and improves detail reconstruction, outperforming state-of-the-art methods.
...
TL;DR: HitoMi-Cam, a spectral-based person detection method, outperforms CNNs in unpredictable shape scenarios, achieving 93.5% AP and 23.2 fps on edge devices, making it viable for real-time disaster rescue.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the shape dependency of CNN-based object detection, which degrades performance for postures not included in training data. The study aims to address this limitation for real-world applications like disaster rescue.

Method: The paper introduces HitoMi-Cam, a lightweight and shape-agnostic person detection method using spectral reflectance properties of clothing. It was implemented on a GPU-less edge device to evaluate real-time performance.

Result: HitoMi-Cam achieved a processing speed of 23.2 fps (253x190 pixels) and an average precision (AP) of 93.5% in a search and rescue scenario, outperforming CNN models (best AP of 53.8%). False positives were minimal.

Conclusion: The study concludes that HitoMi-Cam, a spectral-based person detection method, is a viable complementary tool to CNN-based detectors, especially in scenarios with unpredictable shapes like disaster rescue. It demonstrates practical viability on resource-constrained edge devices.
...
TL;DR: The paper introduces Negative Entity Suppression (NES) to improve cross-domain generalization in zero-shot image captioning by filtering and suppressing irrelevant entities from retrieved captions.


<details>
  <summary>Details</summary>
Motivation: Text-only training for zero-shot image captioning suffers from poor cross-domain generalization and hallucination when encountering novel visual environments. Retrieval-based methods can worsen hallucination by including irrelevant entities.

Method: NES integrates three stages: using synthetic images for consistent retrieval, filtering negative entities, and applying attention-level suppression to minimize hallucination.

Result: NES achieves state-of-the-art results in zero-shot image captioning, improving cross-domain transfer and reducing hallucination while maintaining in-domain performance.

Conclusion: NES effectively addresses hallucination and cross-domain challenges in zero-shot image captioning, offering a robust solution with competitive performance.
...
TL;DR: SPEED-Q is a novel framework for quantizing small-scale VLMs, addressing sensitivity discrepancies and training instability, achieving superior accuracy on edge devices.


<details>
  <summary>Details</summary>
Motivation: Deploying VLMs on edge devices requires efficient quantization due to resource constraints, but existing methods lack exploration of aggressive quantization for small-scale VLMs.

Method: SPEED-Q introduces a staged sensitivity adaptive mechanism and a distillation-enhanced quantization strategy to address discrepancies in quantization sensitivity and training instability.

Result: SPEED-Q achieves up to 6x higher accuracy than existing methods under 2-bit settings and outperforms prior on-device VLMs in both 2-bit and 4-bit settings.

Conclusion: SPEED-Q is the first framework designed for quantizing small-scale billion-parameter VLMs to low bits, achieving superior accuracy and performance compared to existing methods.
...
TL;DR: The paper introduces Diff-FCHM, a novel collaborative compression method prioritizing machine-vision-oriented compression over human-vision pipelines, achieving superior performance for both human and machine visions.


<details>
  <summary>Details</summary>
Motivation: Existing collaborative compression methods, based on human-vision pipelines, are inefficient for machine-vision tasks due to complexity and high bit-rates. Machine vision requires less information, focusing on core regions, unlike human vision.

Method: The proposed Diff-FCHM method uses machine-vision-oriented compression as the basis, progressively aggregating semantics for machine tasks and restoring high-fidelity details for human vision using diffusion priors. A plug-and-play variable bit-rate strategy is also developed.

Result: Experiments show Diff-FCHM outperforms existing methods in both machine-vision and human-vision compression with significant margins.

Conclusion: Diff-FCHM successfully addresses the inefficiencies of human-vision-based collaborative compression by prioritizing machine-vision needs, achieving superior results for both human and machine tasks.
...
TL;DR: The paper proposes a Hierarchical Distillation framework combining trajectory and distribution-based methods for high-fidelity single-step diffusion models, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: The inference latency of diffusion models hinders real-time applications. Existing methods (trajectory-based and distribution-based) have trade-offs: trajectory-based methods lose high-frequency details, while distribution-based methods suffer from mode collapse and unstable training. The paper aims to unify these paradigms for better performance.

Method: The HD framework leverages trajectory distillation to create a structural "sketch" for initialization, followed by distribution-based refinement. The Adaptive Weighted Discriminator (AWD) is introduced to dynamically focus on local imperfections for efficient detail refinement.

Result: The single-step model achieves an FID of 2.26 on ImageNet 256×256, rivaling its 250-step teacher. It also performs well on the high-resolution MJHQ benchmark, showcasing generalizability.

Conclusion: The paper introduces the Hierarchical Distillation (HD) framework, which synergistically combines trajectory-based and distribution-based distillation methods to achieve high-fidelity, single-step diffusion models. The approach demonstrates state-of-the-art performance across diverse tasks, proving its generalizability and robustness.
...
TL;DR: NAMEA improves adversarial transferability by integrating gradients from non-attention areas in ensemble models, outperforming state-of-the-art attacks by significant margins.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the unsatisfactory performance of existing ensemble attacks when transferring across heterogeneous architectures, due to divergent gradient update directions.

Method: The method involves decoupling gradients from attention and non-attention areas of ensemble models and merging them using meta-learning. This leverages the observation that non-attention areas in one model type (e.g., ViTs) may be focus areas in another (e.g., CNNs).

Result: Empirical results on ImageNet show NAMEA outperforms AdaEA and SMER by 15.0% and 9.6%, respectively.

Conclusion: The paper concludes that NAMEA, a novel ensemble attack integrating gradients from non-attention areas, significantly improves adversarial transferability across heterogeneous model architectures, outperforming state-of-the-art methods by 15.0% and 9.6%.
...
TL;DR: BRHVC improves NBVC by harmonizing bi-directional references with BMC and BCF, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: NBVC's hierarchical coding complicates temporal prediction, causing unbalanced contributions from reference frames. BRHVC aims to optimize reference information utilization.

Method: The proposed BRHVC method includes Bi-directional Motion Converge (BMC) for accurate motion compensation and Bi-directional Contextual Fusion (BCF) for modeling reference context weights.

Result: BRHVC surpasses previous NVC methods and traditional coding (VTM-RA) on HEVC datasets.

Conclusion: BRHVC, with its BMC and BCF components, effectively harmonizes bi-directional references in NBVC, outperforming previous state-of-the-art NVC methods and even traditional coding like VTM-RA.
