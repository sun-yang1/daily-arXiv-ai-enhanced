...
TL;DR: SIDeR decouples identity from facial images for privacy, using diffusion models to generate anonymous yet machine-recognizable faces, with high attack success and restoration quality.


<details>
  <summary>Details</summary>
Motivation: To protect privacy by decoupling identity information from visual representations in facial recognition systems.

Method: SIDeR decomposes facial images into identity and appearance components, using semantic-guided recomposition in a diffusion model with momentum-driven perturbation optimization.

Result: Achieves 99% attack success rate in black-box scenarios and 41.28% better PSNR-based restoration quality than baselines.

Conclusion: SIDeR effectively decouples identity from visual representation, achieving high privacy protection with a 99% attack success rate and superior restoration quality.
...
TL;DR: UniTrack is a universal graph-theoretic loss function for MOT, improving performance without architectural changes, achieving significant reductions in identity switches and boosting IDF1 scores.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of prior graph-based MOT methods that require architectural redesigns, UniTrack aims to provide a universal training objective that enhances tracking performance without modifying existing systems.

Method: UniTrack introduces a plug-and-play graph-theoretic loss function that integrates detection accuracy, identity preservation, and spatiotemporal consistency into a single end-to-end trainable loss function, enabling seamless integration with existing MOT systems.

Result: UniTrack achieves a 53% reduction in identity switches and a 12% IDF1 improvement across benchmarks, with GTR showing a peak performance gain of 9.7% MOTA on SportsMOT.

Conclusion: UniTrack significantly enhances multi-object tracking (MOT) performance by optimizing tracking-specific objectives through a unified differentiable learning approach, demonstrating consistent improvements across diverse models and benchmarks.
...
TL;DR: A training framework enhances visual conditioning in VLA models by optimizing action-visual alignment, improving performance without extra data or architecture changes.


<details>
  <summary>Details</summary>
Motivation: Extending pretrained VLMs to action space can cause vision-action misalignment, leading to unreliable outputs. Observations show successful rollouts depend more on visual input than failed ones.

Method: The approach aligns action prediction with visual input via preference optimization on a surrogate task and transfers enhanced alignment through latent-space distillation during supervised finetuning.

Result: The method improves visual conditioning and task performance for discrete OpenVLA and yields gains in continuous OpenVLA-OFT settings.

Conclusion: The proposed training framework strengthens visual conditioning in VLA models, improving both visual alignment and task performance without architectural changes or additional data.
...
TL;DR: TL;DR: The paper reviews methods to improve food portion size estimation from 2D images, highlighting the use of auxiliary inputs and deep learning for better accuracy in dietary assessment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve dietary assessment accuracy by overcoming the limitation of estimating 3D food sizes from 2D images, which is vital for chronic disease and obesity prevention.

Method: The paper examines different strategies for portion estimation, including the use of auxiliary inputs like depth maps, multi-view inputs, template matching, and deep learning techniques with monocular or combined image inputs.

Result: The exploration reveals that combining deep learning with auxiliary inputs can enhance the precision of portion estimation from image inputs.

Conclusion: The paper concludes that while image-based dietary assessment is crucial for health monitoring, accurately estimating food portion sizes from 2D images remains a challenge. Various strategies, including auxiliary inputs and deep learning, are explored to address this limitation.
...
TL;DR: The paper introduces Visual Concept Ranking (VCR) to audit large multimodal models (LMMs) for biases in medical tasks, focusing on skin lesion classification and chest radiographs.


<details>
  <summary>Details</summary>
Motivation: To ensure reliability of LMMs in healthcare by identifying and addressing performance gaps across demographic subgroups.

Method: Developed VCR to uncover visual concept dependencies in LMMs, validated with manual interventions.

Result: VCR successfully identified biases in LMMs, revealing unexpected performance gaps in medical tasks.

Conclusion: VCR is an effective auditing tool for uncovering and addressing biases in LMMs, enhancing their reliability in healthcare.
...
TL;DR: CLEAR-HPV enhances interpretability in HPV histopathology by discovering morphologic concepts and reducing feature dimensions, maintaining predictive accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the limited morphologic interpretability of attention-based MIL in HPV-related whole-slide histopathology.

Method: Introduces CLEAR-HPV, a concept-level explainable attention-guided representation framework that restructures the MIL latent space for concept discovery without labeled concepts during training.

Result: CLEAR-HPV discovers morphologic concepts (e.g., keratinizing, basaloid, stromal), generates spatial concept maps, and reduces feature space to 10 interpretable concepts, generalizing across multiple datasets.

Conclusion: CLEAR-HPV provides a compact, interpretable framework for HPV-related histopathology analysis, preserving predictive power while enhancing morphologic interpretability.
...
TL;DR: ARGaze introduces an autoregressive transformer model for online egocentric gaze estimation, leveraging temporal continuity and indirect cues to outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve online egocentric gaze estimation, which lacks explicit head or eye signals, by leveraging temporal continuity and indirect cues like hand-object interactions and scene content.

Method: ARGaze uses a transformer decoder to predict gaze sequentially, conditioning on current visual features and a fixed-length Gaze Context Window of recent gaze estimates. This autoregressive approach enforces causality and enables efficient streaming inference.

Result: ARGaze achieves state-of-the-art performance in online egocentric gaze estimation across multiple benchmarks, demonstrating the effectiveness of autoregressive modeling with bounded gaze history.

Conclusion: The paper concludes by highlighting the state-of-the-art performance of ARGaze in online egocentric gaze estimation, validated through extensive ablations. The authors commit to releasing the source code and pre-trained models.
...
TL;DR: AirGlove improves vision-based hand tracking for gloved hands by generalizing learned representations to new glove designs, outperforming current methods.


<details>
  <summary>Details</summary>
Motivation: Existing bare-hand models perform poorly on gloved hands due to appearance gaps, and sensor-based tracking is restrictive due to accuracy issues.

Method: AirGlove leverages existing gloves to generalize learned glove representations towards new gloves with limited data.

Result: AirGlove shows substantial performance boosts over existing schemes when tested with multiple sensing gloves.

Conclusion: AirGlove effectively generalizes hand pose models to new glove designs, achieving significant performance improvements over existing methods.
...
TL;DR: SHaSaM uses submodular hard-sample mining to enhance fairness in deep neural networks, improving fairness and accuracy without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks often inherit biases from annotated data, leading to unfair predictions, especially with sensitive attributes like race, age, and gender. Existing methods worsen unfairness due to data imbalance.

Method: The two-stage approach includes SHaSaM-MINE for submodular subset selection to mitigate data imbalance and SHaSaM-LEARN with combinatorial loss functions to minimize sensitive attribute influence.

Result: SHaSaM improves fairness (Equalized Odds) by up to 2.7 points and accuracy by 3.5% on CelebA and UTKFace datasets, outperforming existing methods in fewer epochs.

Conclusion: SHaSaM effectively addresses fairness and performance trade-offs in deep neural networks by leveraging submodular hard-sample mining and combinatorial loss functions, achieving state-of-the-art results.
...
TL;DR: A diffusion-based image-to-image pipeline is developed to reverse underwater degradations in photography, achieving high perceptual consistency with a small model trained on a limited dataset.


<details>
  <summary>Details</summary>
Motivation: Underwater photography suffers from reduced contrast, blur, and color distortions, requiring heavy post-processing to correct.

Method: A synthetic corruption pipeline is used to train a diffusion-based model to reverse underwater degradations.

Result: The model achieves high perceptual consistency and generalization in synthesizing 512x768 images with ~11M parameters trained on ~2.5k images.

Conclusion: The proposed method effectively reverses underwater distortions with a compact model, demonstrating strong performance on a small dataset.
...
TL;DR: ShapePuri is a new defense framework using geometric guidance and appearance debiasing to improve robustness against adversarial attacks, achieving high accuracy without extra costs.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks are vulnerable to adversarial attacks, and existing defense methods like adversarial training and purification suffer from high computational costs and information loss.

Method: ShapePuri combines a Shape Encoding Module (SEM) for geometric guidance via Signed Distance Functions (SDF) and a Global Appearance Debiasing (GAD) module to mitigate appearance bias through stochastic transformations.

Result: ShapePuri achieves 84.06% clean accuracy and 81.64% robust accuracy under the AutoAttack protocol, surpassing the 80% threshold.

Conclusion: ShapePuri introduces a scalable and efficient adversarial defense framework that aligns model representations with stable structural invariants, achieving high clean and robust accuracy without additional computational costs.
...
TL;DR: PoseGaussian is a pose-guided Gaussian Splatting framework for high-fidelity human novel view synthesis, leveraging pose for depth refinement and temporal consistency, achieving real-time rendering and superior performance.


<details>
  <summary>Details</summary>
Motivation: Address challenges in dynamic human scenes like articulated motion and severe self-occlusion by embedding pose signals into both geometric and temporal stages for improved robustness and generalization.

Method: The framework integrates human body pose as a structural prior and temporal cue, refining depth estimation and enhancing temporal consistency through a differentiable, end-to-end trainable pipeline.

Result: Achieves state-of-the-art performance with PSNR 30.86, SSIM 0.979, and LPIPS 0.028 on ZJU-MoCap, THuman2.0, and in-house datasets.

Conclusion: PoseGaussian achieves high-fidelity human novel view synthesis with real-time rendering at 100 FPS, demonstrating superior performance in perceptual quality and structural accuracy.
...
TL;DR: Proposes using video generative models as reward models (\modelname) to better capture temporal dynamics, achieving superior performance with fewer annotations.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of current Vision-Language Models (VLMs) in capturing subtle temporal dynamics in video generative models, prompting the need for a temporally-aware reward model.

Method: The method involves transforming video generation models into energy-based models (EBMs) to discriminate video quality, using contrastive objectives and synthetic negative videos created via latent-space perturbations like temporal slicing, feature swapping, and frame shuffling.

Result: \modelname achieves state-of-the-art performance on GenAI-Bench and MonteBench, requiring only 30K human annotations—6× to 65× fewer than VLM-based methods.

Conclusion: The paper concludes that repurposing video generative models as reward models, through the proposed Generative-Transformer-based Self-Supervised Video Judge (\modelname), significantly outperforms existing VLM-based approaches with far fewer human annotations.
...
TL;DR: A training-free framework integrates explicit and implicit representations to enhance ultra-low bitrate compression, outperforming current methods.


<details>
  <summary>Details</summary>
Motivation: Address the deterioration of neural codecs at ultra-low bitrates and the tradeoff between semantic faithfulness and perceptual realism in existing methods.

Method: A unified framework combining explicit high-level semantics with implicit fine-grained details via a diffusion model and reverse-channel coding, plus a plug-in encoder for tradeoff control.

Result: Outperforms existing methods, surpassing DiffC by significant margins (29.92%, 19.33%, 20.89%) in DISTS BD-Rate across datasets.

Conclusion: The proposed framework successfully bridges the gap between semantic faithfulness and perceptual realism by integrating explicit and implicit representations, achieving state-of-the-art performance.
...
TL;DR: E.M.Ground improves Temporal Video Grounding with semantic aggregation, noise reduction, and multi-grained features, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for Temporal Video Grounding rely on exact timestamps and fail to capture semantic continuity, leading to ambiguities.

Method: E.M.Ground introduces an <event> token for semantic aggregation, Savitzky-Golay smoothing for noise reduction, and multi-grained frame feature aggregation for enhanced reliability.

Result: E.M.Ground achieves significant performance improvements over existing Vid-LLMs on benchmark datasets.

Conclusion: E.M.Ground outperforms state-of-the-art Vid-LLMs in Temporal Video Grounding by addressing semantic continuity and noise issues, demonstrating superior accuracy.
...
TL;DR: MPA improves cross-domain few-shot segmentation by progressively adapting few-shot capability through data augmentation and multi-view prediction, outperforming existing methods by 7%.


<details>
  <summary>Details</summary>
Motivation: Existing methods have constrained performance due to limited target samples and weak few-shot capability in target domains, hindered by substantial domain gaps.

Method: Multi-view Progressive Adaptation (MPA) with Hybrid Progressive Augmentation (data perspective) and Dual-chain Multi-view Prediction (strategy perspective).

Result: MPA achieves robust and accurate adaptation to target domains, outperforming state-of-the-art methods by +7.0%.

Conclusion: MPA effectively adapts few-shot capability to target domains, outperforming state-of-the-art methods by a large margin (+7.0%).
...
TL;DR: CPS improves cross-domain few-shot segmentation by adaptively sparsifying dense points, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The success of SAM in promptable segmentation inspired its use in few-shot segmentation, but dense point matching underperforms in cross-domain settings due to large domain shifts disrupting point-image interactions.

Method: The paper proposes Conditional Point Sparsification (CPS), which leverages ground-truth masks from reference images to adaptively sparsify dense matched points, improving SAM's performance in cross-domain scenarios.

Result: Extensive experiments show that CPS outperforms existing training-free SAM-based methods across diverse CD-FSS datasets.

Conclusion: CPS, a training-free approach, effectively addresses the challenges of cross-domain few-shot segmentation by adaptively sparsifying dense matched points, leading to more accurate segmentation results.
...
TL;DR: A novel computer vision method improves die casting defect detection, reducing errors by 20-28.2% and achieving high AUROC scores without needing anomalous training data.


<details>
  <summary>Details</summary>
Motivation: Surface defects in die casting hinder quality control, prompting the exploration of automated defect detection using computer vision.

Method: The approach combines local neighbor-aware patch features with a normalizing flow model and introduces an adapter module to bridge the gap between generic pretrained features and industrial images.

Result: The method reduces error rates by 20% on MVTec AD (AUROC: 99.28%) and by 28.2% on VisA (AUROC: 96.48%), with 95.77% accuracy on a proprietary dataset without anomalous training samples.

Conclusion: The paper demonstrates the potential of combining computer vision and deep learning to enhance defect detection in die casting, achieving significant improvements over state-of-the-art methods.
...
TL;DR: A cost-effective active label cleaning method improves EDD detection by denoising crowdsourced data, achieving near-expert performance with significantly lower annotation costs.


<details>
  <summary>Details</summary>
Motivation: Automated detection of electron dense deposits (EDD) in glomerular disease is limited by scarce high-quality labeled data, and crowdsourcing introduces label noise.

Method: An active label cleaning method is introduced, utilizing active learning to select valuable noisy samples for expert re-annotation and a Label Selection Module to grade noise and select samples.

Result: The method achieves 67.18% AP50 on a private dataset, an 18.83% improvement over noisy labels, reaching 95.79% of full expert annotation performance while reducing costs by 73.30%.

Conclusion: The proposed method offers a practical and cost-effective solution for developing reliable medical AI by efficiently denoising crowdsourced datasets, achieving high performance with reduced expert annotation costs.
...
TL;DR: ReGLA combines convolutions and gated attention for efficient, accurate high-resolution image processing, outperforming competitors.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of balancing accuracy and latency in lightweight Transformer-based models for high-resolution images.
...
Result: Achieves 80.85% Top-1 accuracy on ImageNet-1K (224px) with 4.98 ms latency (512px), outperforming iFormer models in downstream tasks.

Conclusion: ReGLA is a state-of-the-art solution for high-resolution visual applications, excelling in accuracy and latency.
...
TL;DR: Proposes prototype fine-tuning over feature extractor updates for FSCIL, using dual-calibration to dynamically adjust prototypes, achieving better performance with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of traditional methods that rely on static class prototypes, which suffer from representation bias, and the insufficient adaptability of recent prompt-based tuning methods under extreme data scarcity.

Method: The proposed method involves a dual-calibration prototype fine-tuning framework, which includes class-specific and task-aware offsets to dynamically adjust prototypes within a static feature space.

Result: The results show that the proposed framework significantly improves discriminative capacity for incremental classes, outperforming existing methods across multiple benchmarks.

Conclusion: The paper concludes that fine-tuning prototypes rather than the feature extractor is more effective for few-shot class-incremental learning, as demonstrated by superior performance across benchmarks with minimal parameter updates.
...
TL;DR: Magic-MM-Embedding introduces an efficient MLLM with visual token compression and multi-stage training, achieving state-of-the-art performance in multimodal retrieval.


<details>
  <summary>Details</summary>
Motivation: To address the high computational cost of MLLMs in universal multimodal retrieval while maintaining high performance.

Method: Magic-MM-Embedding leverages a highly efficient MLLM architecture with visual token compression and a multi-stage progressive training strategy.

Result: The model outperforms existing methods in performance and efficiency.
...
TL;DR: Fast-SAM3D accelerates SAM3D by addressing pipeline heterogeneity with three dynamic mechanisms, achieving 2.67× speedup without significant quality loss.


<details>
  <summary>Details</summary>
Motivation: The deployment of SAM3D is hindered by prohibitive inference latency due to neglected multi-level heterogeneity in the pipeline.

Method: The framework integrates three heterogeneity-aware mechanisms: Modality-Aware Step Caching, Joint Spatiotemporal Token Carving, and Spectral-Aware Token Aggregation.

Result: Fast-SAM3D delivers up to 2.67× end-to-end speedup with negligible fidelity loss.

Conclusion: Fast-SAM3D establishes a new Pareto frontier for efficient single-view 3D generation by dynamically aligning computation with generation complexity, achieving up to 2.67× speedup with negligible fidelity loss.
...
TL;DR: FlashBlock improves block diffusion efficiency by reusing stable attention outputs, boosting throughput and reducing computation.


<details>
  <summary>Details</summary>
Motivation: Addresses the substantial overhead from repeatedly computing attention over a growing KV cache in block diffusion.

Method: Proposes FlashBlock, a cached block-external attention mechanism that reuses stable attention output.

Result: Achieves up to 1.44× higher token throughput and up to 1.6× reduction in attention time with negligible impact on generation quality.

Conclusion: FlashBlock effectively reduces attention computation and KV cache access in block diffusion, enhancing efficiency without compromising generation quality.
...
TL;DR: Wid3R is a neural network for wide field-of-view 3D reconstruction, outperforming prior methods with a novel distortion-aware approach.


<details>
  <summary>Details</summary>
Motivation: Prior methods are limited by assumptions of rectified or pinhole camera images, restricting their real-world applicability with fisheye or panoramic cameras.

Method: Wid3R leverages a ray representation with spherical harmonics and a novel camera model token within the network, enabling distortion-aware 3D reconstruction.

Result: Wid3R achieves improvements of up to +77.33 on Stanford2D3D and supports feed-forward 3D reconstruction directly from 360 imagery.

Conclusion: Wid3R demonstrates strong zero-shot robustness and outperforms prior methods, achieving significant improvements in 3D reconstruction from wide field-of-view imagery.
...
TL;DR: MTPano is a multi-task panoramic foundation model that leverages perspective priors and a novel architecture to overcome domain gaps and task interference, achieving top performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: The scarcity of high-resolution, multi-task annotations and the challenges of adapting perspective foundation models to panoramic domains (due to geometric distortions and coordinate discrepancies) motivate the need for a robust panoramic multi-task model.

Method: The paper proposes MTPano, a multi-task panoramic foundation model trained via a label-free pipeline. It leverages perspective dense priors to generate pseudo-labels, categorizes tasks into rotation-invariant and rotation-variant groups, and introduces the Panoramic Dual BridgeNet with geometry-aware modulation layers. ERP token mixers and auxiliary tasks further enhance cross-task learning.

Result: MTPano delivers competitive results against task-specific panoramic specialist models and achieves state-of-the-art performance on multiple benchmarks.

Conclusion: MTPano achieves state-of-the-art performance on multiple benchmarks and competes effectively with task-specific panoramic models, demonstrating its robustness as a multi-task foundation model for panoramic scene understanding.
...
TL;DR: PAIR introduces a framework for concept erasure that uses unsafe-safe pairs to preserve semantic consistency while removing undesirable content, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing concept erasure methods focus on removing unsafe content but fail to provide safe alternatives, leading to inconsistencies in structural and semantic fidelity.

Method: The PAIR framework leverages unsafe-safe pairs for semantic realignment and Fisher-weighted initialization for DoRA, enabling fine-grained erasure without compromising generation quality.

Result: PAIR outperforms state-of-the-art baselines, achieving effective concept erasure while maintaining semantic coherence and generation quality.

Conclusion: The PAIR framework effectively addresses the limitations of existing concept erasure methods by introducing a novel approach that preserves structural and semantic consistency while removing undesirable concepts.
...
TL;DR: APEX improves OOD detection by addressing flaws in prototype-based methods with adaptive prototypes and quality-aware scoring.


<details>
  <summary>Details</summary>
Motivation: Existing prototype-based OOD detection methods suffer from Static Homogeneity Assumption and Learning-Inference Disconnect, limiting model capacity and performance.

Method: APEX employs a Two-Stage Repair process with an Adaptive Prototype Manifold (APM) and Posterior-Aware OOD Scoring (PAOS) mechanism to optimize feature representation and prototype quality.

Result: APEX outperforms existing methods, achieving state-of-the-art performance on benchmarks like CIFAR-100.

Conclusion: APEX introduces a novel OOD detection framework that addresses fundamental flaws in existing prototype-based methods, achieving state-of-the-art performance on benchmarks like CIFAR-100.
...
TL;DR: HIVE introduces a "slow thinking" framework for MLLMs by integrating hierarchical visual cues into latent reasoning, improving robustness and reducing reliance on textual rationales.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs rely on inefficient, verbose, and hallucination-prone "fast thinking" paradigms like end-to-end generation or explicit CoT. This work aims to enable robust reasoning within a latent space by seamlessly integrating multimodal signals.

Method: The proposed method recursively extends transformer blocks to create an internal loop for iterative reasoning refinement, injectively grounding the process with hierarchical visual cues from global to fine-grained regional details.

Result: Extensive evaluations show that HIVE improves complex scene understanding through hierarchical visual cue integration and effective test-time scaling with vision knowledge.

Conclusion: The HIVE framework successfully integrates hierarchical visual cues into the latent space of MLLMs, enabling more robust and grounded reasoning without relying on verbose textual rationales.
