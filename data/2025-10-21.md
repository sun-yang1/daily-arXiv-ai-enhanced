...
TL;DR: ESCA introduces SGClip, a CLIP-based model for scene graphs, trained without human labels, improving MLLMs' performance in embodied tasks.


<details>
  <summary>Details</summary>
Motivation: Current training pipelines for MLLMs lack fine-grained alignment between pixel-level visual content and textual semantics, limiting their effectiveness as general-purpose embodied agents.

Method: The proposed ESCA framework includes SGClip, a CLIP-based, open-domain, and promptable model for generating scene graphs. SGClip is trained on 87K+ open-domain videos using a neurosymbolic learning pipeline that leverages model-driven self-supervision and structured reasoning, eliminating the need for human-labeled annotations.

Result: SGClip excels in scene graph generation and action localization benchmarks. ESCA with SGClip consistently enhances both open-source and commercial MLLMs, achieving top performance in embodied environments.

Conclusion: ESCA with SGClip significantly improves multi-modal large language models (MLLMs) by reducing perception errors and enabling open-source models to outperform proprietary baselines, achieving state-of-the-art performance in embodied environments.
...
TL;DR: CrossRay3D improves sparse multi-modal detection with better token representation, achieving SOTA performance and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing sparse detectors overlook token representation quality, leading to sub-optimal foreground quality and limited performance.

Method: Proposes Sparse Selector (SS) with Ray-Aware Supervision (RAS) and Class-Balanced Supervision, along with Ray Positional Encoding (Ray PE).

Result: Outperforms other sparse multi-modal detectors, running 1.84× faster than leading methods.

Conclusion: CrossRay3D achieves state-of-the-art performance with 72.4 mAP and 74.7 NDS, demonstrating robustness even with missing LiDAR or camera data.
...
TL;DR: The paper proposes a pipeline using CCTV streams for multi-defect detection with YOLO and a vision language model (VLM) for structured summaries, aiming to improve infrastructure maintenance in smart cities.


<details>
  <summary>Details</summary>
Motivation: Manual inspection of infrastructure defects is costly and hazardous, while existing automatic systems lack comprehensiveness and structured outputs for maintenance crews.

Method: The method combines YOLO for defect detection and a VLM for generating structured action plans in JSON format, including descriptions, tools, and repair plans.

Result: The system accurately identifies diverse defects and produces coherent summaries, validated on public datasets and CCTV clips.

Conclusion: The paper highlights the system's potential for city-wide deployment while acknowledging challenges in scaling.
...
TL;DR: IAD-GPT leverages MLLMs for Industrial Anomaly Detection, combining text semantics with image data for detailed anomaly descriptions and multi-turn dialogues.


<details>
  <summary>Details</summary>
Motivation: Traditional IAD methods lack multi-turn dialogues and detailed anomaly descriptions, while existing large model-based methods underutilize their potential in anomaly detection.

Method: IAD-GPT integrates text and image data, using an Abnormal Prompt Generator (APG) and Text-Guided Enhancer to improve visual grounding. A Multi-Mask Fusion module enhances pixel-level anomaly perception.

Result: State-of-the-art performance on MVTec-AD and VisA datasets for self-supervised and few-shot anomaly detection and segmentation.

Conclusion: IAD-GPT effectively combines MLLMs with visual data for advanced anomaly detection, offering detailed descriptions and improved accuracy.
...
TL;DR: AI-assisted structured reporting (AI-SR) outperforms free-text and structured reporting in diagnostic accuracy and efficiency, while also improving user experience.


<details>
  <summary>Details</summary>
Motivation: To assess the impact of structured reporting and AI on radiologists' interaction with imaging studies, focusing on image analysis behavior, diagnostic accuracy, efficiency, and user experience.

Method: A prospective study evaluated three reporting modes (free-text, structured reporting, and AI-assisted structured reporting) using a customized viewer and eye-tracking system. Outcomes included diagnostic accuracy, reporting time, eye-tracking metrics, and user experience. Statistical analysis used generalized linear mixed models with Bonferroni post-hoc tests.

Result: Diagnostic accuracy was higher in AI-SR (κ = 0.71) compared to FT (κ = 0.58) and SR (κ = 0.60). Reporting times decreased significantly with SR and AI-SR. Eye-tracking metrics showed reduced saccade counts and fixation durations with SR and AI-SR. AI-SR was the preferred mode.

Conclusion: Structured reporting (SR) improves efficiency by guiding visual attention toward the image, and AI-prefilled SR (AI-SR) further enhances diagnostic accuracy and user satisfaction.
...
TL;DR: A framework (IFEF) and data augmentation method (BWA) are introduced to analyze and mitigate intersectional biases in image classification, improving accuracy for underrepresented groups by up to 24% and reducing fairness disparities by 35%.


<details>
  <summary>Details</summary>
Motivation: Machine learning models trained on imbalanced datasets often exhibit intersectional biases, leading to systematic errors in predictions.

Method: The paper introduces the Intersectional Fairness Evaluation Framework (IFEF) for bias analysis and Bias-Weighted Augmentation (BWA) for mitigation, adapting transformation intensities based on subgroup statistics.

Result: BWA improves accuracy for underrepresented class-environment intersections by up to 24 percentage points and reduces fairness metric disparities by 35%, with statistically significant improvements (p < 0.05).
...
TL;DR: A differentiable quantization method with convergence proof supports n-bit quantization, achieving near-full-precision accuracy (weight-only) and SOTA accuracy (weight+activation) with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Previous quantization methods were non-differentiable or manually set derivatives, limiting learning ability. Additionally, prior work often avoided activation quantization or suffered accuracy drops. This work addresses these gaps.

Method: The approach uses a differentiable quantization method with proof of convergence, supporting n-bit quantization (including logarithmic forms like $2^n$). It is tested on image classification (ImageNet, ResNet18) with shift bit quantization.

Result: Weight-only quantization achieves <1% accuracy drop vs. full precision. Weight and activation quantization matches SOTA accuracy in 15 epochs, with slightly higher CPU instructions but no need for high-precision multiplication.

Conclusion: The proposed quantization method is differentiable, converges to the optimal neural network, and supports n-bit quantization. It achieves near-full-precision accuracy with weight-only quantization and comparable SOTA accuracy with weight and activation quantization, with minimal inference overhead.
...
TL;DR: StripRFNet, a novel deep neural network, enhances road damage detection with three specialized modules, achieving state-of-the-art accuracy and real-time efficiency.


<details>
  <summary>Details</summary>
Motivation: Accurate detection of road surface damage is challenging due to diverse damage shapes, difficulty in capturing slender cracks, and high error rates in small-scale damage recognition. Addressing these issues is crucial for traffic safety and sustainable urban development.

Method: StripRFNet comprises three modules: (1) a Shape Perception Module (SPM) for shape discrimination, (2) a Strip Receptive Field Module (SRFM) for capturing slender cracks, and (3) a Small-Scale Enhancement Module (SSEM) for improving small-object detection.

Result: StripRFNet outperforms existing methods, improving F1-score, mAP50, and mAP50:95 by 4.4, 2.9, and 3.4 percentage points on the Chinese subset, and achieving the highest F1-score of 80.33% on the full dataset.

Conclusion: StripRFNet achieves state-of-the-art accuracy and real-time efficiency, making it a promising tool for intelligent road maintenance and sustainable infrastructure management.
...
TL;DR: ObjectTransforms improves vision-based object detection by reducing uncertainty through object-specific transformations during training and inference, enhancing accuracy and robustness.


<details>
  <summary>Details</summary>
Motivation: Vision-based object detectors are vulnerable to uncertainty from data bias and distributional shifts, which is critical for safety in autonomous driving.

Method: ObjectTransforms uses color space perturbations and diffusion models during training, and applies object perturbations at inference to quantify uncertainty.

Result: Experiments show accuracy improvements and uncertainty reduction, with higher uncertainty for false positives than true positives.

Conclusion: ObjectTransforms is a lightweight, effective method for reducing and quantifying uncertainty in vision-based perception.
...
TL;DR: A2PD is an open egocentric dataset captured with Aria Gen 2 glasses, featuring five scenarios and supporting tools for research.


<details>
  <summary>Details</summary>
Motivation: To provide timely access to a comprehensive egocentric multimodal dataset for research, showcasing the device's perception capabilities.

Method: The dataset is captured using Aria Gen 2 glasses, featuring raw sensor data and output from machine perception algorithms across five primary scenarios.

Result: The dataset includes diverse scenarios and demonstrates robust performance across users and conditions.

Conclusion: The A2PD dataset is publicly available and supported by open-source tools, making it a valuable resource for research in egocentric multimodal data.
...
TL;DR: A training-free method using universal guidance transfers appearance to 3D assets, outperforming baselines and introducing a GPT-based evaluation system for robust assessment.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of state-of-the-art methods in handling significant geometric differences between input and appearance objects, and the failure of direct 3D generative models to produce appealing results.

Method: The method involves interacting with the sampling process of a pretrained rectified flow model by periodically adding guidance, modeled as a differentiable loss function. Two types of guidance are experimented with: part-aware losses for appearance and self-similarity.

Result: The results demonstrate successful texture and geometric detail transfer, with the method outperforming baselines both qualitatively and quantitatively. A GPT-based evaluation system is introduced to address the shortcomings of traditional metrics.

Conclusion: The paper concludes that the proposed training-free method, inspired by universal guidance, effectively transfers texture and geometric details to 3D assets, outperforming baselines. It also highlights the inadequacy of traditional metrics for evaluation and introduces a GPT-based system for robust assessment.
...
TL;DR: Deep learning automates thrombectomy tasks, improving efficiency and safety, with promising results in classification and regression.


<details>
  <summary>Details</summary>
Motivation: To automate critical aspects of thrombectomy, improving efficiency and safety in ischemic stroke treatment.

Method: A self-supervised framework using a regression-based pretext task to classify skeletal landmarks.

Result: The model outperforms existing methods in regression and classification tasks, with the positional pretext task significantly boosting downstream classification performance.

Conclusion: The proposed self-supervised framework enhances thrombectomy efficiency and safety, with future work aimed at fully autonomous C-arm control.
...
TL;DR: DuetMatch, a dual-branch semi-supervised framework for medical image segmentation, improves stability and performance by asynchronously optimizing encoder and decoder branches, introducing novel regularization and label refinement techniques.


<details>
  <summary>Details</summary>
Motivation: The limited availability of annotated medical imaging data and the challenges of joint optimization in teacher-student frameworks motivate the need for a more stable and effective semi-supervised approach.

Method: DuetMatch uses asynchronous optimization of encoder and decoder branches, Decoupled Dropout Perturbation for regularization, Pair-wise CutMix Cross-Guidance for diversity, and Consistency Matching to refine noisy pseudo-labels.

Result: DuetMatch outperforms state-of-the-art methods on benchmark brain MRI segmentation datasets (ISLES2022 and BraTS), demonstrating robustness in diverse semi-supervised scenarios.

Conclusion: DuetMatch effectively addresses challenges in semi-supervised medical image segmentation, offering improved stability, performance, and robustness through its innovative dual-branch framework and regularization techniques.
...
TL;DR: A pipeline for autonomous C-arm navigation to anatomical landmarks using X-ray images, incorporating uncertainty prediction and conformal prediction for reliability.


<details>
  <summary>Details</summary>
Motivation: Manual C-arm positioning in fluoroscopy-guided interventions increases radiation exposure and procedural delays, necessitating an automated solution.

Method: The pipeline predicts 3D displacement vectors from arbitrary X-ray images, uses probabilistic loss and skeletal pose regularization, and calibrates uncertainties with conformal prediction.

Result: Strong localization accuracy and well-calibrated prediction bounds were achieved on a synthetic X-ray dataset.

Conclusion: The pipeline shows promise for safe and reliable autonomous C-arm systems, with code publicly available.
...
TL;DR: The paper introduces a cost-saving formula for automatic pre-filtering in image quality assessment (IQA) to reduce manual review costs in deep generative models.


<details>
  <summary>Details</summary>
Motivation: Manual IQA in deep generative models is slow and expensive due to low yield of high-quality images.

Method: Proposes a formula to estimate cost savings based on precision and pass yield of an IQA engine, demonstrated with an AutoML solution.

Result: Achieved a significant cost saving of 51.61% in a background inpainting use case.

Conclusion: Automatic pre-filtering can effectively reduce costs in production pipelines using generated images.
...
TL;DR: The paper introduces PRISM, a model that projects fMRI signals into a structured text space for visual stimuli reconstruction, outperforming existing methods with an 8% reduction in perceptual loss.


<details>
  <summary>Details</summary>
Motivation: Understanding how the brain encodes visual information is a central challenge in neuroscience and machine learning, particularly in reconstructing images from fMRI signals.

Method: The method involves transforming fMRI signals into a latent space (structured text space) and using a generative model (PRISM) with an object-centric diffusion module and an attribute-relationship search module.

Result: PRISM achieves up to an 8% reduction in perceptual loss compared to existing methods.

Conclusion: The results emphasize the importance of using structured text as an intermediate space to bridge fMRI signals and image reconstruction.
...
TL;DR: The paper proposes a data-centric AI approach for tropical agriculture mapping, reviewing 25 strategies and recommending 9 practical methods to overcome challenges like data scarcity and variability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the unique challenges of mapping agriculture in tropical areas, including lack of high-quality annotated data, high labeling costs, data variability, and regional generalization issues.

Method: The paper reviews and prioritizes techniques such as confident learning, core-set selection, data augmentation, and active learning, advocating a Data-Centric Artificial Intelligence (DCAI) perspective.

Result: The paper highlights the readiness and suitability of 25 distinct strategies for large-scale agricultural mapping pipelines, with a focus on tropical contexts.

Conclusion: The paper concludes by proposing a practical pipeline using the 9 most mature and straightforward methods for large-scale tropical agricultural mapping, emphasizing the effectiveness of a data-centric approach in addressing the unique challenges of tropical agriculture.
...
TL;DR: StretchySnake, a flexible training method for video SSMs, improves adaptability to varying spatio-temporal resolutions, outperforming transformers and SSMs by up to 28% in action recognition tasks.


<details>
  <summary>Details</summary>
Motivation: Current training methods for video understanding are tailored towards transformers and fail to fully leverage the unique attributes of SSMs, leading to spatio-temporal inflexibility and degraded performance on unseen resolutions.

Method: A flexible training method that samples videos at varying temporal and spatial resolutions during training and dynamically interpolates model weights to accommodate any spatio-temporal scale.
...
Conclusion: The proposed method, StretchySnake, provides a simple drop-in training recipe that enhances the robustness, resolution-agnostic nature, and efficiency of video SSMs across diverse action recognition scenarios, outperforming transformer and SSM baselines by up to 28%.
...
TL;DR: The paper introduces VM-BeautyNet, a heterogeneous ensemble combining Vision Transformer and Mamba-based Vision models for Facial Beauty Prediction, achieving state-of-the-art results on SCUT-FBP5500.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning models struggle to capture holistic facial features for beauty prediction, while Vision Transformers face quadratic complexity issues.

Method: Proposes VM-BeautyNet, fusing Vision Transformer (global features) and Mamba-based Vision model (linear complexity for long-range dependencies).

Result: Achieves Pearson Correlation of 0.9212, MAE of 0.2085, and RMSE of 0.2698 on SCUT-FBP5500.

Conclusion: VM-BeautyNet offers a powerful, interpretable paradigm for computational aesthetics, confirmed by Grad-CAM visualizations.
...
TL;DR: A CNN and hardware system were developed for early OCSCC detection, showing that higher-resolution images improve accuracy, with diminishing returns.


<details>
  <summary>Details</summary>
Motivation: Early detection of OCSCC is challenging due to subtle early symptoms and hidden development areas. CNNs offer a promising solution for accurate and early diagnosis.

Method: A CNN was trained on 4293 images (benign, malignant, and negative samples) and evaluated using precision, recall, and mAP. Testing involved images at 5 resolutions, and hardware was designed for image capture.

Result: Higher image resolution led to more accurate predictions, following a logarithmic scale. The hardware and application enhanced the CNN's practical usability.

Conclusion: The study successfully demonstrated that higher-resolution images improve the accuracy of OCSCC detection using CNNs, though with diminishing returns. The developed hardware and application provide a practical solution for early detection.
...
TL;DR: Embody 3D is a large-scale multimodal dataset with 500 hours of 3D motion data from 439 participants, featuring diverse human activities and interactions, tracked motion, annotations, and audio.


<details>
  <summary>Details</summary>
Motivation: The motivation behind Embody 3D is to address the need for a large-scale, multimodal dataset that captures a wide range of human motions and interactions, enabling advancements in research related to human behavior, motion analysis, and virtual avatars.

Method: The dataset was collected using a multi-camera setup in a controlled environment, capturing 500 hours of 3D motion data from 439 participants. It includes tracked human motion (body and hands), text annotations, and individual audio tracks.

Result: The dataset comprises over 54 million frames of 3D motion data, featuring diverse activities such as prompted motions, hand gestures, locomotion, and multi-person interactions like conversations and collaborative tasks.

Conclusion: The Embody 3D dataset provides a comprehensive and diverse collection of 3D motion data, including single-person and multi-person interactions, with detailed annotations and audio tracks, making it a valuable resource for research in human motion and behavior.
...
TL;DR: The paper introduces a proactive scene decomposition and reconstruction method using human-object interactions to dynamically refine scene understanding, achieving accurate and photorealistic results.


<details>
  <summary>Details</summary>
Motivation: Human behaviors provide rich cues for scene dynamics, but static object-level reconstruction methods often miss these dynamic aspects. The paper aims to leverage human-object interactions to address ambiguities in reconstruction.

Method: The proposed system integrates tasks like camera and object pose estimation, instance decomposition, and online map updating. It uses Gaussian splatting for photorealistic and efficient rendering, refining the process iteratively through observed interactions.

Result: The method achieves accurate and consistent dynamic scene modeling, validated in real-world scenarios with promising advantages.

Conclusion: The approach offers a flexible, progressive alternative to conventional reconstruction methods, effectively utilizing human-object interactions for improved scene understanding.
...
TL;DR: Cerberus introduces a two-stage system for efficient real-time video anomaly detection, leveraging motion mask prompting and rule-based deviation to achieve high speed and accuracy.


<details>
  <summary>Details</summary>
Motivation: The high computational cost and unstable visual grounding performance of Vision-Language Models (VLMs) hinder their real-time deployment for video anomaly detection.

Method: Cerberus employs a two-stage cascaded system with motion mask prompting and rule-based deviation detection, combining lightweight filtering and fine-grained VLM reasoning.

Result: Cerberus achieves 57.68 fps on an NVIDIA L40S GPU (151.79× speedup) and 97.2% accuracy, comparable to state-of-the-art VLM-based methods.

Conclusion: Cerberus is established as a practical solution for real-time video anomaly detection, achieving high accuracy and significant speedup compared to state-of-the-art VLM-based methods.
...
TL;DR: OpenLVLM-MIA is a benchmark addressing biases in membership inference attacks (MIA) on large vision-language models (LVLMs), showing prior high success rates stem from dataset biases, not true membership detection.


<details>
  <summary>Details</summary>
Motivation: Prior MIA evaluations on LVLMs often suffer from distributional biases, leading to misleading high success rates. OpenLVLM-MIA aims to provide an unbiased benchmark to accurately assess MIA performance.

Method: A controlled benchmark of 6,000 images with balanced member/non-member distributions and ground-truth labels across three training stages was introduced.

Result: State-of-the-art MIA methods performed no better than random chance under unbiased conditions in OpenLVLM-MIA.

Conclusion: OpenLVLM-MIA reveals the limitations of current MIA research on LVLMs and supports the development of better privacy-preserving techniques by providing a transparent, unbiased benchmark.
...
TL;DR: Stroke2Sketch is a training-free framework for style-guided sketch generation, using cross-image stroke attention and adaptive techniques to transfer stroke attributes while preserving content and structure.


<details>
  <summary>Details</summary>
Motivation: The need to transfer stroke attributes (e.g., line thickness, deformation, texture sparsity) while preserving semantic structure and content fidelity in sketch generation guided by reference styles.

Method: The paper proposes Stroke2Sketch, a training-free framework with cross-image stroke attention embedded in self-attention layers for fine-grained semantic correspondences and stroke attribute transfer. It also includes adaptive contrast enhancement and semantic-focused attention.

Result: The method adaptively integrates reference stroke characteristics into content images while maintaining structural integrity, producing sketches that closely resemble handcrafted results.

Conclusion: Stroke2Sketch effectively synthesizes stylistically faithful sketches resembling handcrafted results, outperforming existing methods in expressive stroke control and semantic coherence.
...
TL;DR: The paper studies scaling laws in deepfake detection using the ScaleDF dataset, revealing power-law scaling similar to LLMs and enabling performance forecasting.


<details>
  <summary>Details</summary>
Motivation: To systematically study scaling laws for deepfake detection, addressing the lack of large-scale datasets and understanding how performance scales with data diversity and volume.

Method: The research constructs ScaleDF, the largest dataset for deepfake detection, with 5.8 million real images from 51 domains and 8.8 million fake images from 102 methods. Performance is analyzed against scaling factors like real domains, deepfake methods, and training images.

Result: Power-law scaling is observed: detection error decays predictably with more real domains or deepfake methods. This allows forecasting additional data needs for target performance.

Conclusion: The study highlights the power-law scaling in deepfake detection, similar to LLMs, and suggests a data-centric approach to counter evolving deepfake technology. It also explores the role of pre-training and data augmentations, while acknowledging scaling limitations.
...
TL;DR: Scale-DiT introduces hierarchical local attention with low-resolution global guidance for efficient ultra-high-resolution image synthesis, outperforming existing methods without requiring native 4K training data.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models struggle with ultra-high-resolution (e.g., 4K) image generation due to quadratic attention complexity and lack of native 4K training data.

Method: Scale-DiT uses hierarchical local attention with low-resolution global guidance, divides high-resolution latents into fixed-size windows, and employs LoRA adaptation for consistency. It optimizes inference with Hilbert curve token reordering and fused-kernel skipping.

Result: Scale-DiT achieves 2× faster inference, lower memory usage, and scales to 4K resolution without additional training data, outperforming benchmarks (FID, IS, CLIP Score) and qualitative comparisons.
...
TL;DR: DiffusionX is a cloud-edge framework that speeds up image generation by using on-device previews and cloud refinements, reducing latency and cloud workload while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: The computational intensity and iterative prompt refinement in diffusion models increase latency and cloud resource burden, necessitating a more efficient solution.

Method: The framework combines a lightweight on-device diffusion model for rapid previews with a high-capacity cloud model for final refinements, aided by a dynamic noise level predictor to optimize computation.

Result: DiffusionX reduces average generation time by 15.8% compared to Stable Diffusion v1.5 and is only 0.9% slower than Tiny-SD, with significantly better image quality.

Conclusion: DiffusionX successfully balances efficiency and image quality by leveraging a cloud-edge collaborative framework, reducing generation time and cloud workload while maintaining high-quality outputs.
...
TL;DR: TokenAR introduces a token-level enhancement mechanism to improve identity consistency in multi-reference image generation, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive models struggle with decoupling reference identities in conditional image generation.

Method: TokenAR uses Token Index Embedding, Instruct Token Injection, and identity-token disentanglement (ITD) to enhance token representations.

Result: The framework improves identity consistency and background reconstruction, validated by experiments on the InstructAR Dataset.

Conclusion: TokenAR advances multi-reference image generation, with code and datasets made publicly available.
