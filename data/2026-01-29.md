...
Motivation: The rise of diet-related chronic diseases like obesity and diabetes requires accurate food intake monitoring. Current AI-driven dietary assessment struggles with estimating portion sizes from monocular images due to scale ambiguity in 3D reconstruction methods, limiting their use in precision nutrition.

Method: Proposes a method that recovers true-to-scale 3D reconstructed objects from monocular images by leveraging rich visual features from models trained on large-scale datasets to estimate object scale, enabling conversion of single-view 3D reconstructions into physically meaningful models.

Result: Extensive experiments on two publicly available datasets show the method consistently outperforms existing techniques, achieving nearly 30% reduction in mean absolute volume-estimation error.

Conclusion: The method bridges 3D computer vision and digital health by enabling accurate food portion estimation from monocular images, showing strong potential to enhance precision nutrition applications.
...
TL;DR: DiSa introduces a saliency-aware foreground-background disentangled framework to address CLIP's foreground bias and limited spatial localization in open-vocabulary semantic segmentation, achieving state-of-the-art performance on six benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models like CLIP suffer from foreground bias (ignoring background regions) and limited spatial localization (blurred object boundaries) when adapted to semantic segmentation tasks.

Method: Proposes DiSa framework with two key modules: Saliency-aware Disentanglement Module (SDM) that separately models foreground and background features using saliency cues, and Hierarchical Refinement Module (HRM) that leverages pixel-wise spatial contexts for multi-level feature refinement.

Result: Extensive experiments on six benchmarks demonstrate that DiSa consistently outperforms state-of-the-art methods.

Conclusion: DiSa effectively addresses the limitations of VLMs in semantic segmentation by explicitly disentangling foreground and background features and incorporating hierarchical refinement, achieving superior performance across multiple benchmarks.
...
TL;DR: SSMAE is a semi-supervised framework that combines masked image reconstruction with classification using pseudo-labels, achieving better performance than supervised ViT and fine-tuned MAE on CIFAR datasets, especially with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Training Vision Transformers effectively when labeled data is scarce but unlabeled data is abundant remains a challenge. The paper aims to develop a method that leverages both labeled and unlabeled data efficiently for ViT training.

Method: SSMAE jointly optimizes masked image reconstruction and classification using both unlabeled and labeled samples. It introduces a validation-driven gating mechanism that activates pseudo-labeling only after the model achieves reliable, high-confidence predictions consistent across both weakly and strongly augmented views of the same image, reducing confirmation bias.

Result: On CIFAR-10 and CIFAR-100, SSMAE consistently outperforms supervised ViT and fine-tuned MAE, with the largest gains in low-label regimes (+9.24% over ViT on CIFAR-10 with 10% labels).

Conclusion: The timing of when pseudo-labels are introduced is as important as how they are generated for data-efficient transformer training. SSMAE demonstrates effective semi-supervised learning for Vision Transformers.
...
TL;DR: The paper introduces Sparse CLIP, integrating sparsity directly into training to achieve interpretable and performant representations, challenging the trade-off between interpretability and accuracy.


<details>
  <summary>Details</summary>
Motivation: CLIP's dense and opaque latent representations pose interpretability challenges, and post-hoc approaches like Sparse Autoencoders often degrade performance and lose multimodal capabilities. The authors aim to address these limitations.

Method: The authors propose integrating sparsity directly into CLIP training, resulting in Sparse CLIP representations. This method preserves performance, enhances interpretability, and retains multimodal capabilities.

Result: Sparse CLIP representations outperform post-hoc methods, maintaining strong downstream performance, superior interpretability, and multimodal capabilities. They also enable semantic concept alignment and reveal cross-modal training dynamics.

Conclusion: The paper challenges the conventional trade-off between interpretability and performance in CLIP models, demonstrating that sparsity can be integrated directly into training to achieve both goals. This approach offers a promising design principle for future models.
...
TL;DR: This paper shifts focus from developing new segmentation algorithms to systematically evaluating and standardizing publicly available datasets for nuclei instance segmentation in H&E-stained images, creating unified training and test sets for improved benchmarking.


<details>
  <summary>Details</summary>
Motivation: Most research in nuclei instance segmentation focuses on algorithm development while using limited, arbitrarily selected public datasets. The authors identified a need to systematically evaluate and standardize the datasets themselves rather than creating new segmentation models.

Method: Conducted extensive literature review to identify manually annotated public datasets, standardized them into unified format, systematically evaluated datasets using two state-of-the-art segmentation models (CNN-based and hybrid CNN/vision transformer), and created unified test (NucFuse-test) and training (NucFuse-train) sets by merging images from multiple datasets.

Result: Provided a comprehensive benchmark by evaluating and ranking datasets, performing analyses, generating fused datasets, conducting external validation, and making implementation publicly available for fair cross-dataset evaluation.

Conclusion: The work establishes a new benchmark for training, testing, and evaluating nuclei instance segmentation models on H&E-stained histological images, addressing the need for standardized dataset evaluation in this field.
...
TL;DR: SAP, a training-free pruning method, reduces index vectors by over 90% while maintaining retrieval fidelity, outperforming traditional pruning by focusing on middle-layer structural anchors.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of existing training-free pruning solutions, which underperform in high-compression scenarios and rely on query-dependent visual token importance. The paper questions the feasibility of such methods and seeks a more effective approach.

Method: The paper proposes Structural Anchor Pruning (SAP), a training-free pruning method that identifies key visual patches from middle layers. It also introduces the Oracle Score Retention (OSR) protocol to evaluate layer-wise information's impact on compression efficiency.

Result: Evaluations on the ViDoRe benchmark show that SAP reduces index vectors by over 90% while maintaining robust retrieval fidelity. The OSR-based analysis reveals that semantic structural anchor patches persist in middle layers.

Conclusion: The paper concludes that Structural Anchor Pruning (SAP) effectively reduces index vector size by over 90% while maintaining retrieval fidelity, offering a scalable solution for Visual RAG. It also highlights the persistence of semantic structural anchor patches in middle layers, unlike traditional pruning methods.
...
TL;DR: The paper introduces a structured token pruning strategy for diffusion-based multimodal models like LLaDA-V, reducing computational costs by up to 65% while maintaining 95% task performance.


<details>
  <summary>Details</summary>
Motivation: The bidirectional attention and iterative denoising in LLaDA-V cause high computational overhead, with delayed semantic alignment in middle-to-late layers.

Method: Proposes a structured token pruning strategy, targeting middle-to-late layers in the first denoising step to align with LLaDA-V's attention patterns.

Result: Achieves up to 65% reduction in computational cost while preserving 95% of task performance across benchmarks.

Conclusion: The framework offers efficient inference for LLaDA-V and demonstrates the potential of vision-aware pruning in diffusion-based models.
...
TL;DR: TeleStyle is a lightweight DiT-based model for image and video stylization that uses Curriculum Continual Learning to handle content-style entanglement, achieving SOTA performance in style similarity, content consistency, and aesthetic quality.


<details>
  <summary>Details</summary>
Motivation: Content-preserving style transfer remains challenging for Diffusion Transformers due to the inherent entanglement of content and style features in their internal representations.

Method: Built on Qwen-Image-Edit, TeleStyle uses a Curriculum Continual Learning framework trained on hybrid datasets of curated and synthetic triplets, plus a video-to-video module for temporal consistency.

Result: TeleStyle achieves state-of-the-art performance across three core evaluation metrics: style similarity, content consistency, and aesthetic quality.

Conclusion: TeleStyle effectively addresses content-style entanglement in DiTs and generalizes to unseen styles while maintaining precise content fidelity, with code and models publicly available.
...
TL;DR: Automated classification of marine biofouling severity using computer vision models and LLMs shows complementary strengths, with hybrid methods offering promising scalable assessment.


<details>
  <summary>Details</summary>
Motivation: Marine biofouling on vessel hulls poses major ecological, economic, and biosecurity risks, but traditional diver inspections are hazardous and limited in scalability.

Method: Evaluated convolutional neural networks, transformer-based segmentation, and zero-shot LLMs on expert-labelled dataset from New Zealand Ministry for Primary Industries. Used structured prompts and retrieval for LLMs.

Result: Computer vision models showed high accuracy at extreme LoF categories but struggled with intermediate levels due to dataset imbalance and image framing. LLMs achieved competitive performance without training and provided interpretable outputs.

Conclusion: Hybrid methods integrating segmentation coverage with LLM reasoning offer a promising pathway toward scalable and interpretable biofouling assessment.
...
TL;DR: DenseGRPO addresses the sparse reward problem in GRPO-based text-to-image generation by introducing dense rewards that evaluate each denoising step's contribution, along with a reward-aware exploration scheme to calibrate the stochasticity injection in SDE samplers.


<details>
  <summary>Details</summary>
Motivation: Existing GRPO-based approaches suffer from the sparse reward problem where terminal rewards are applied to all intermediate denoising steps, creating a mismatch between global feedback signals and fine-grained step contributions.

Method: Proposes two key components: (1) step-wise reward gain prediction as dense rewards using an ODE-based approach on intermediate clean images, and (2) a reward-aware scheme that adaptively adjusts timestep-specific stochasticity injection in SDE samplers to calibrate exploration space.

Result: Extensive experiments on multiple standard benchmarks demonstrate the effectiveness of DenseGRPO and highlight the critical role of valid dense rewards in flow matching model alignment.

Conclusion: DenseGRPO successfully addresses the sparse reward problem in text-to-image generation alignment, providing a more effective framework for human preference alignment through fine-grained step evaluation and calibrated exploration.
...
TL;DR: FPL (Feature Projection Learning) is a simple and efficient method that adapts CLIP models to downstream tasks by transforming classification into a feature projection problem, achieving superior accuracy with fewer parameters and faster training.


<details>
  <summary>Details</summary>
Motivation: Existing methods for adapting Vision-Language Pre-trained models like CLIP to downstream tasks suffer from limited performance, excessive parameters, or long training times, hindering their effectiveness.

Method: FPL uses a projection model to project class prototype features into query image feature space and reconstructs the query image feature map, using negative average squared reconstruction error as the class score. It combines predictions from the projection model with original CLIP outputs.
...
Conclusion: FPL provides an efficient and effective solution for adapting CLIP models to downstream tasks, addressing key limitations of existing approaches while achieving better performance.
...
TL;DR: PAE improves Visual Prompt Tuning by addressing unstable training dynamics through frequency-aware initialization, coordinated layer updates via shared Koopman operator, and Lyapunov-based regularization for stable evolution.


<details>
  <summary>Details</summary>
Motivation: Existing Visual Prompt Tuning (VPT) methods suffer from unstable training dynamics with gradient oscillations, where shallow-layer prompts stagnate early while deeper-layer prompts oscillate with high variance, leading to cross-layer mismatch that slows convergence and degrades performance.

Method: Proposes Prompt-Agnostic Evolution (PAE) with three key components: 1) Frequency-aware initialization that uncovers and propagates frequency shortcut patterns for task-aware prompt initialization, 2) Shared Koopman operator for coordinated layer updates instead of uncoordinated layer-specific updates, and 3) Lyapunov-inspired regularization to constrain error amplification during evolution.

Result: PAE achieves average 1.41× convergence speedup and improves accuracy by 1-3% on 25 datasets across multiple downstream tasks. It is prompt-agnostic, lightweight, and integrates seamlessly with diverse VPT variants without requiring backbone modification or inference-time changes.

Conclusion: PAE effectively addresses the unstable training dynamics in Visual Prompt Tuning by modeling prompt evolution, resulting in faster convergence and better performance while maintaining compatibility with existing VPT methods.
...
TL;DR: BLenDeR is a diffusion sampling method that uses set-theory operations to enhance intra-class diversity in Deep Metric Learning, achieving state-of-the-art performance improvements on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: Deep Generative Models can generate synthetic data to augment authentic data in Deep Metric Learning, but existing approaches lack controlled synthesis of diverse attribute combinations within classes.

Method: BLenDeR leverages union and intersection operations on denoising residuals to encourage attribute presence across multiple prompts and extract common directions via principal component analysis, enabling controlled synthesis of diverse intra-class attributes.

Result: BLenDeR consistently outperforms state-of-the-art baselines, achieving 3.7% increase in Recall@1 on CUB-200 and 1.8% increase on Cars-196 under standard experimental settings.

Conclusion: The proposed diffusion sampling method effectively addresses limitations of existing generative approaches by providing controllable intra-class diversity enhancement, leading to improved Deep Metric Learning performance.
...
TL;DR: The paper introduces the Reversible Efficient Diffusion (RED) model, a supervised training framework for multi-modal image fusion that avoids detail loss and computational inefficiency issues in diffusion models.


<details>
  <summary>Details</summary>
Motivation: Diffusion models often lose fine details in image fusion due to noise errors from the Markov process, and explicit supervision in training introduces computational inefficiency.

Method: The proposed Reversible Efficient Diffusion (RED) model incorporates explicit supervision while avoiding distribution estimation, maintaining generative capabilities.

Result: The RED model effectively preserves fine details and high visual fidelity in fused images without computational inefficiency.

Conclusion: The RED model successfully addresses the limitations of diffusion models in image fusion, offering a balanced solution for detail preservation and computational efficiency.
...
TL;DR: LVLMs-Saliency introduces a gradient-aware framework to detect and mitigate hallucinations in large vision-language models by combining attention weights with input gradients, improving reliability without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for detecting hallucinations in LVLMs rely only on forward-pass attention patterns, missing gradient-based signals that reveal token influence, leading to unreliable distinctions between hallucinated and grounded outputs.

Method: The paper proposes LVLMs-Saliency, a framework that fuses attention weights with input gradients to quantify visual grounding strength. It also introduces Saliency-Guided Rejection Sampling (SGRS) and Local Coherence Reinforcement (LocoRE) to mitigate hallucinations during inference.

Result: Experiments show the method significantly reduces hallucination rates while maintaining fluency and task performance across multiple LVLMs.

Conclusion: LVLMs-Saliency offers a robust, interpretable solution for enhancing model reliability by addressing contextual memory retention issues in LVLMs.
...
TL;DR: A novel source-free domain adaptation method using multiview augmentation and latent space consistency to learn domain-invariant features directly from the target domain, achieving high accuracy across datasets.


<details>
  <summary>Details</summary>
Motivation: Existing domain adaptation methods are computationally expensive, requiring source domain data, adversarial training, or complex pseudo-labeling techniques. This paper aims to simplify the process by learning directly from the target domain.

Method: The method uses multiview augmentation and latent space consistency to learn transferable representations from the target domain, eliminating the need for source-target alignment or pseudo-label refinement. It includes a ConvNeXt-based encoder and a combined classification and consistency loss function.

Result: The model achieves average classification accuracies of 90.72%, 84%, and 97.12% on Office-31, Office-Home, and Office-Caltech datasets, respectively, outperforming existing methods by +1.23%, +7.26%, and +1.77%.

Conclusion: The proposed source-free domain adaptation method effectively learns domain-invariant features directly from the target domain, offering a simpler and more efficient alternative to existing approaches.
...
TL;DR: The paper introduces GenVID, a dataset for evaluating video generation artifacts, and DVAR, a framework for fine-grained artifact detection, improving accuracy and content filtering.


<details>
  <summary>Details</summary>
Motivation: Existing video evaluation methods lack detailed localization and categorization of artifacts, necessitating a comprehensive protocol for assessing video quality.

Method: The authors define a taxonomy of 10 artifact categories and introduce GenVID, a dataset of 80k annotated videos. They develop DVAR, a framework for dense artifact recognition.

Result: Experiments show significant improvement in artifact detection accuracy and effective filtering of low-quality content.

Conclusion: The proposed protocol and framework enhance the evaluation of generated videos by addressing specific artifact categories and improving detection precision.
...
TL;DR: C-SAM is a framework that combines sharpness-aware minimization with compression-aware training by perturbing pruning masks instead of parameters, achieving better robustness than SAM while maintaining model compactness.


<details>
  <summary>Details</summary>
Motivation: SAM improves DNN robustness but doesn't account for pruning needs. Pruning SAM-trained models can undermine robustness, and applying SAM after pruning is constrained by early pruning decisions.

Method: C-SAM shifts sharpness-aware learning from parameter perturbations to mask perturbations, explicitly perturbing pruning masks during training to promote flatter loss landscapes with respect to model structure.

Result: C-SAM achieves up to 42% higher certified robustness than baselines while maintaining task accuracy comparable to unpruned models on CelebA-HQ, Flowers-102, and CIFAR-10-C across various architectures.

Conclusion: C-SAM effectively bridges the gap between model robustness and compression requirements by enabling discovery of pruning patterns that optimize both compactness and robustness to input variations.
...
TL;DR: A dual domain learning strategy combining NA and WA CT data improves bladder segmentation performance under covariate shift, achieving high Dice and IoU scores with minimal WA data.


<details>
  <summary>Details</summary>
Motivation: Addressing performance degradation in deep learning models for medical image segmentation due to covariate shift, particularly in bladder segmentation for brachytherapy.

Method: Proposes a dual domain learning strategy integrating NA and WA CT data, using a curated dataset and multiple deep learning architectures.

Result: Doping 10-30% WA data into NA training sets achieves performance comparable to models trained exclusively on WA data, with Dice scores up to 0.94 and IoU scores up to 0.92.

Conclusion: Integrating distribution-shifted datasets enhances robustness and clinical reliability, overcoming data scarcity in brachytherapy treatment planning.
...
TL;DR: A physically structured framework for single-image mass estimation uses 3D geometry and material semantics to improve accuracy, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Mass estimation from visual input is challenging due to the unobservable nature of geometric volume and material-dependent density from RGB appearance, making the problem ill-posed.

Method: The method recovers 3D geometry via monocular depth estimation for volume and uses a vision-language model for coarse material semantics to guide density-related reasoning. These representations are fused through an instance-adaptive gating mechanism, with separate regression heads for volume- and density-related factors under mass-only supervision.

Result: Experiments on image2mass and ABO-500 datasets demonstrate consistent outperformance over state-of-the-art methods.

Conclusion: The proposed physically structured framework for single-image mass estimation effectively addresses the ambiguity in mass prediction by aligning visual cues with physical factors, outperforming state-of-the-art methods.
...
TL;DR: A new deep learning model, SLDM, enhances low-dose iodinated contrast media CT images by integrating structural and semantic information, improving accuracy and reducing risks.


<details>
  <summary>Details</summary>
Motivation: Overdose of iodinated contrast media (ICM) in CT scans can cause kidney damage and allergic reactions. Existing methods struggle with incomplete image pairs and structural recognition.

Method: Proposes SLDM, which uses structural priors and semantic supervision to ensure consistency and accuracy in enhancing low-dose ICM images. Includes a subtraction angiography module for better contrast.

Result: Qualitative and quantitative analyses show SLDM effectively reconstructs angiographic images from low-dose ICM CT scans.

Conclusion: SLDM successfully addresses limitations of existing methods, offering a safer and more accurate approach for low-dose ICM CT enhancement.
...
TL;DR: TPGDiff is a unified image restoration model that uses triple priors (degradation, structural, semantic) in diffusion models to address multiple degradation types while preserving spatial structures and generating reliable content in severely degraded regions.


<details>
  <summary>Details</summary>
Motivation: Existing all-in-one image restoration methods struggle with content reconstruction in severely degraded areas and often disrupt spatial structures when integrating semantic information into shallow diffusion layers.

Method: Triple-Prior Guided Diffusion (TPGDiff) network that incorporates degradation priors throughout diffusion, structural priors in shallow layers for fine details, and semantic priors in deep layers via a distillation-driven extractor for robust high-level guidance.

Result: Extensive experiments show TPGDiff achieves superior performance and generalization across diverse restoration scenarios on both single- and multi-degradation benchmarks.

Conclusion: TPGDiff effectively addresses unified image restoration by hierarchically integrating complementary priors, enabling better content reconstruction while preserving spatial structures across various degradation types.
...
TL;DR: OSDEnhancer introduces a one-step diffusion framework with specialized experts and a bidirectional VAE decoder for robust STVSR, achieving state-of-the-art results in real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: Current STVSR methods struggle with complex real-world degradations and lack robustness in achieving high reconstruction fidelity and temporal consistency. The goal is to develop a framework that addresses these challenges effectively.

Method: The method involves a linear pre-interpolation strategy for initializing spatiotemporal structures, training TR-SE MoE for specialized temporal and spatial learning, and using a bidirectional deformable VAE decoder for recurrent spatiotemporal aggregation.

Result: Experiments show that OSDEnhancer outperforms existing methods, maintaining superior generalization in real-world scenarios.

Conclusion: OSDEnhancer, the proposed framework, achieves state-of-the-art performance in real-world STVSR by combining a one-step diffusion process with specialized expert pathways and a bidirectional deformable VAE decoder, ensuring high reconstruction fidelity and temporal coherence.
...
<details>
  <summary>Details</summary>
Motivation: Current channel-dependent models overfit channel ordering and hamper adaptation, while channel-independent models neglect inter-channel dependencies. There's a need for a method that can handle structural and distributional co-drift without retraining.

Method: CPiRi combines spatio-temporal decoupling architecture with permutation-invariant regularization: frozen pretrained temporal encoder extracts temporal features, lightweight spatial module learns inter-channel relations, and channel shuffling enforces permutation invariance during training.

Result: State-of-the-art results on multiple benchmarks. CPiRi remains stable with channel order shuffling and exhibits strong inductive generalization to unseen channels even when trained on only half of the channels, while maintaining practical efficiency on large-scale datasets.

Conclusion: CPiRi provides an effective solution for multivariate time series forecasting that handles channel permutation and structural changes without retraining, demonstrating strong generalization capabilities and practical efficiency.
...
TL;DR: The paper introduces GVGS, a method that combines visibility-aware multi-view geometric consistency and progressive quadtree-calibrated monocular depth constraints to improve surface reconstruction accuracy in 3D Gaussian Splatting.


<details>
  <summary>Details</summary>
Motivation: Current methods for surface reconstruction in 3D Gaussian Splatting face challenges: multi-view geometric constraints become unreliable under large geometric discrepancies, while monocular depth priors suffer from scale ambiguity and local inconsistency, leading to inaccurate Gaussian depth supervision.

Method: 1) Gaussian visibility-aware multi-view geometric consistency constraint that aggregates visibility of shared Gaussian primitives across views for more stable geometric supervision. 2) Progressive quadtree-calibrated monocular depth constraint that performs block-wise affine calibration from coarse to fine spatial scales to mitigate scale ambiguity while preserving surface details.

Result: Extensive experiments on DTU and TNT datasets demonstrate consistent improvements in geometric accuracy over prior Gaussian-based and implicit surface reconstruction methods.

Conclusion: The proposed GVGS method effectively addresses the limitations of existing approaches by combining visibility-aware multi-view constraints and calibrated monocular priors, achieving superior surface reconstruction quality in 3D Gaussian Splatting.
...
TL;DR: TopoOT, a topology-aware optimal transport framework, leverages TDA for robust anomaly segmentation, achieving SOTA results by aligning persistence diagrams and using stability-aware pseudo-labels.


<details>
  <summary>Details</summary>
Motivation: Deep topological data analysis (TDA) is well-suited for anomaly segmentation due to its ability to capture structural invariants like connectivity and cycles across scales, unlike threshold-based methods that are brittle under distribution shift.

Method: TopoOT integrates multi-filtration persistence diagrams with test-time adaptation, using Optimal Transport Chaining to align persistence diagrams across thresholds and filtrations. This yields stability scores that supervise a lightweight head trained online with OT-consistency and contrastive objectives.
...
Conclusion: TopoOT achieves state-of-the-art performance in anomaly segmentation, outperforming existing methods by significant margins on both 2D and 3D benchmarks.
...
TL;DR: MMSF is a multimodal framework for computational pathology that integrates whole slide images and clinical data using graph feature extraction, clinical embedding, feature fusion, and Mamba-based MIL encoding, achieving significant improvements in accuracy, AUC, and survival prediction.


<details>
  <summary>Details</summary>
Motivation: Multimodal evidence is critical in computational pathology but integrating heterogeneous signals from gigapixel whole slide images and patient-level clinical descriptors is challenging due to distinct feature spaces with different statistics and scales.

Method: MMSF uses a multitask multimodal supervised framework with: 1) graph feature extraction module for tissue topology, 2) clinical data embedding module for patient attributes, 3) feature fusion module aligning modality-shared and modality-specific representations, and 4) Mamba-based MIL encoder with multitask prediction heads.

Result: Experiments show 2.1-6.6% accuracy and 2.2-6.9% AUC improvements on CAMELYON16 and TCGA-NSCLC, and 7.1-9.8% C-index improvements on five TCGA survival cohorts compared to unimodal methods, plus 5.6-7.1% improvements over multimodal alternatives.

Conclusion: MMSF effectively addresses the challenge of integrating multimodal pathology data by explicitly decomposing and fusing cross-modal information, demonstrating superior performance across multiple datasets and metrics compared to both unimodal and existing multimodal approaches.
...
TL;DR: PalmBridge is a plug-and-play feature-space alignment framework that uses vector quantization to improve palmprint recognition under domain shifts by learning representative vectors and blending them with original features.


<details>
  <summary>Details</summary>
Motivation: Real-world palmprint recognition performance degrades due to feature distribution shifts from heterogeneous deployment conditions. Most deep models assume closed stationary distributions, leading to overfitting to dataset-specific textures rather than learning domain-invariant representations.

Method: PalmBridge learns a compact set of representative vectors directly from training features. During enrollment and verification, each feature vector is mapped to its nearest representative vector under minimum-distance criterion, then blended with the original vector. The framework uses joint optimization with task supervision, feature-consistency objective, and orthogonality regularization.
...
Conclusion: PalmBridge effectively addresses domain shift issues in palmprint recognition by suppressing nuisance variation while retaining discriminative identity cues through feature-space alignment, demonstrating robust performance across different deployment conditions.
...
TL;DR: SpatialGenEval is a new benchmark for evaluating text-to-image models' spatial intelligence using 1,230 long, information-dense prompts across 25 real-world scenes. The paper also introduces SpatialT2I dataset for fine-tuning, showing consistent performance gains.


<details>
  <summary>Details</summary>
Motivation: Current T2I models fail in handling complex spatial relationships (spatial perception, reasoning, interaction), and existing benchmarks overlook these aspects due to short or information-sparse prompt design.

Method: Introduced SpatialGenEval benchmark with 1,230 long prompts covering 10 spatial sub-domains and 25 real-world scenes. Also created SpatialT2I dataset with 15,400 text-image pairs for fine-tuning foundation models like Stable Diffusion-XL, Uniworld-V1, and OmniGen2.

Result: Evaluation of 21 state-of-the-art models revealed higher-order spatial reasoning as a primary bottleneck. Fine-tuning on SpatialT2I dataset yielded consistent performance gains (+4.2%, +5.7%, +4.4%) and more realistic spatial relations.

Conclusion: Spatial intelligence remains a key challenge for T2I models, and the proposed information-dense benchmark and dataset provide a data-centric paradigm to improve spatial reasoning capabilities in text-to-image generation.
...
TL;DR: CURVE is a causality-inspired framework that uses variational uncertainty modeling and structural regularization to suppress environment-specific relations in scene graphs, improving out-of-distribution generalization through prototype-conditioned debiasing.


<details>
  <summary>Details</summary>
Motivation: Scene graphs often overfit to spurious correlations, which hinders out-of-distribution generalization. The paper aims to address this limitation by developing a framework that can suppress high-variance, environment-specific relations.

Method: The proposed CURVE framework integrates variational uncertainty modeling with uncertainty-guided structural regularization. It applies prototype-conditioned debiasing to disentangle invariant interaction dynamics from environment-dependent variations, promoting a sparse and domain-stable topology.

Result: Empirical evaluation shows CURVE's effectiveness in zero-shot transfer and low-data sim-to-real adaptation. The framework demonstrates ability to learn domain-stable sparse topologies and provide reliable uncertainty estimates for risk prediction under distribution shifts.

Conclusion: CURVE successfully addresses the overfitting problem in scene graphs by suppressing environment-specific relations through causality-inspired uncertainty modeling, enabling better out-of-distribution generalization and reliable uncertainty estimation under distribution shifts.
