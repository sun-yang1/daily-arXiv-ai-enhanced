...
TL;DR: FD-RL improves OCR performance for formatted text by using entropy-based filtration and format decoupled rewards, achieving a record score on OmniDocBench.


<details>
  <summary>Details</summary>
Motivation: Advanced OCR models exhibit high entropy in formatted text, indicating struggles with output uncertainty, suggesting reasoning over diverse reading pathways could improve performance.

Method: Proposes format decoupled reinforcement learning (FD-RL), leveraging entropy-based data filtration and format decoupled rewards for targeted optimization.

Result: FD-RL sets a new record on OmniDocBench with an average score of 90.41.

Conclusion: FD-RL achieves a new record on OmniDocBench with an average score of 90.41, demonstrating its effectiveness through comprehensive ablation studies.
...
TL;DR: Analysis of robustness and fairness in Vision-Language Models (VLMs) and generative image models under input transformations, focusing on rotation and distributional shifts. Proposed mitigation strategies improve robustness and reduce bias.


<details>
  <summary>Details</summary>
Motivation: To explore the insufficiently investigated robustness and fairness of VLMs and generative models under transformations like rotation and distributional shifts.

Method: Investigation of bias propagation and robustness degradation, followed by proposing mitigation strategies involving data augmentation, representation alignment, and model-level regularization.

Result: Proposed methods significantly improve robustness and reduce bias amplification without sacrificing performance.

Conclusion: The study highlights limitations in current multimodal systems and provides practical techniques for more reliable and fair AI models.
...
TL;DR: R$^2$BD introduces a fast, unified framework for detecting fake images by simulating multiple generative models and using single-step inference, improving efficiency and generalization over existing methods.


<details>
  <summary>Details</summary>
Motivation: Current reconstruction-based methods for AIGC image detection are inefficient due to multi-step inversion and limited generalization to non-diffusion generative models like GANs. The goal is to improve efficiency and broaden detection scope.

Method: The R$^2$BD framework includes two key components: (1) G-LDM, a unified reconstruction model simulating VAEs, GANs, and diffusion models, and (2) a residual bias calculation module for single-step inference.

Result: R$^2$BD is 22× faster than existing methods and achieves superior detection accuracy, outperforming state-of-the-art methods by an average of 13.87% in cross-dataset evaluations.

Conclusion: The proposed R$^2$BD framework significantly improves efficiency and generalization in AIGC image detection by introducing a unified reconstruction model (G-LDM) and a single-step residual bias calculation module. It outperforms existing methods in both speed and accuracy, demonstrating strong cross-dataset generalization.
...
TL;DR: The paper proposes a Cross-Modal Residual Fusion Network (CRFN) for audio-visual embodied navigation, addressing challenges in multimodal fusion and cross-domain scenarios.


<details>
  <summary>Details</summary>
Motivation: The key challenge lies in effectively modeling interactions between heterogeneous features during multimodal fusion to avoid single-modality dominance or information degradation.

Method: CRFN introduces bidirectional residual interactions between audio and visual streams for complementary modeling and fine-grained alignment while maintaining representation independence.

Result: CRFN outperforms state-of-the-art fusion baselines on Replica and Matterport3D datasets and shows stronger cross-domain generalization.

Conclusion: The study reveals differentiated modality dependence across datasets, offering new insights into cross-modal collaboration mechanisms for embodied agents.
...
TL;DR: ForensicFormer is a hierarchical multi-scale framework for cross-domain forgery detection, achieving 86.8% accuracy across diverse test sets and outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: The need for effective detection of AI-generated imagery and sophisticated editing tools that traditional forensic methods cannot handle.

Method: Uses a hierarchical approach combining low-level artifact detection, mid-level boundary analysis, and high-level semantic reasoning via cross-attention transformers.

Result: Maintains 86.8% average accuracy across seven test sets, with robustness to JPEG compression (83% accuracy at Q=70) and pixel-level forgery localization (0.76 F1-score).

Conclusion: ForensicFormer bridges classical image forensics and modern deep learning, providing a practical solution for real-world deployment.
...
TL;DR: SAR-Net introduces a unified framework for image registration under domain shift by disentangling scene and appearance, achieving significant improvements in performance and real-time efficiency.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the ill-posed problem of image registration under systematic intensity differences between source and target images, where conventional methods fail due to violated brightness constancy assumptions.

Method: SAR-Net decomposes images into domain-invariant scene representations and domain-specific appearance codes, enabling registration via re-rendering instead of direct intensity matching.

Result: SAR-Net achieves 0.885 SSIM and 0.979 NCC, a 3.1x improvement over baselines, with real-time performance (77 fps). Ablation studies confirm the necessity of scene consistency and domain alignment losses.

Conclusion: SAR-Net successfully addresses the challenge of image registration under domain shift by disentangling scene and appearance representations, validated by strong empirical results and theoretical proofs.
...
TL;DR: The paper surveys the role of foundation models in embodied AI, proposing the Semantic Lifecycle framework to unify semantic knowledge evolution, and highlights future research directions.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of leveraging multi-source and multi-stage semantic information in embodied AI, especially as agents face increasingly complex environments and open-ended tasks.

Method: The study employs a survey approach, analyzing recent advances in foundation models (FMs) and their impact on embodied AI. It introduces the Semantic Lifecycle framework to unify the evolution of semantic knowledge.

Result: The result is the proposal of the Semantic Lifecycle framework, which provides a holistic perspective on the continuous flow and maintenance of semantic knowledge in embodied AI.

Conclusion: The paper concludes by summarizing existing challenges and outlining promising directions for future research in embodied AI, particularly focusing on the Semantic Lifecycle framework.
...
TL;DR: A novel framework injects semantic intent into MoE routing, mitigating task interference and improving performance in unified image tasks.


<details>
  <summary>Details</summary>
Motivation: To address the issue of task interference in dense diffusion transformers architectures, where shared parameter space compromises conflicting objectives.

Method: Hierarchical Task Semantic Annotation scheme and Predictive Alignment Regularization are introduced to align routing decisions with task semantics.

Result: The model outperforms dense baselines in fidelity and quality, with experts developing clear specializations.

Conclusion: The proposed framework effectively mitigates task interference in unified image generation and editing models by introducing semantic intent into MoE routing. It outperforms dense baselines in fidelity and quality, with experts developing clear and semantically correlated specializations.
...
TL;DR: Compression of geospatial foundation models using manifold-constrained optimization (DLRT) for resource-constrained edge devices, outperforming traditional methods like LoRA.


<details>
  <summary>Details</summary>
Motivation: To enable deployment of large geospatial foundation models on edge devices by reducing parameter counts without significant accuracy loss.

Method: Leverages manifold-constrained optimization framework (DLRT) during transfer learning to compress large vision transformer-based models.

Result: Achieves substantial parameter reduction with minimal accuracy loss, outperforming low-rank methods like LoRA.

Conclusion: The proposed method enables high-performing, on-device geospatial models by effectively balancing compression and accuracy.
...
TL;DR: An adaptive CV framework for TPL quality control uses LDA-based novelty detection, few-shot incremental learning, and domain adaptation, achieving high accuracy with minimal data.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of static computer vision models in dynamic manufacturing environments, which cannot detect new defect classes, update efficiently from scarce data, or adapt to new part geometries.

Method: The method involves a framework built on a scale-robust backbone model, integrating three key methodologies: (1) a statistical hypothesis testing framework based on LDA for novelty detection, (2) a rehearsal-based strategy for few-shot incremental learning, and (3) a few-shot DANN for domain adaptation.

Result: The framework achieved 99-100% accuracy in novelty detection, 92% accuracy in integrating new classes with only 20 samples, and 96.19% accuracy in domain adaptation with just 5 shots.

Conclusion: The paper concludes that the proposed adaptive CV framework effectively addresses the limitations of static models in dynamic manufacturing environments by enabling novelty detection, few-shot incremental learning, and domain adaptation with high accuracy and data efficiency.
...
TL;DR: UAMSA-UNet improves brain tumor segmentation by combining multi-scale attention with a learned smoothing prior, achieving better accuracy and efficiency over existing methods.


<details>
  <summary>Details</summary>
Motivation: Brain tumor segmentation is crucial for diagnosis and treatment planning, but many CNN and U-Net based approaches produce noisy boundaries in regions of tumor infiltration.

Method: UAMSA-UNet leverages Monte Carlo Dropout to learn a data-driven smoothing prior over its predictions, while fusing multi-scale features and attention maps to capture fine details and global context. It uses a smoothing-regularized loss that augments binary cross-entropy with a variance penalty.

Result: On BraTS2023, UAMSA-UNet improves Dice Similarity Coefficient by up to 3.3% and mean IoU by up to 2.7% over U-Net; on BraTS2024, it delivers up to 4.5% Dice and 4.0% IoU gains over the best baseline. It also reduces FLOPs by 42.5% relative to U-Net++ while maintaining higher accuracy.

Conclusion: UAMSA-UNet demonstrates superior segmentation quality and computational efficiency by combining multi-scale attention with a learned smoothing prior, and provides a flexible foundation for future integration with transformer-based modules.
...
TL;DR: Thermo-LIO combines thermal imaging and LiDAR for enhanced Structural Health Monitoring, improving defect detection and coverage in large-scale structures.


<details>
  <summary>Details</summary>
Motivation: Traditional thermography is limited in assessing complex geometries and subsurface defects, necessitating a more advanced solution.

Method: Develops a multimodal fusion method for thermal imaging and LiDAR, integrating it with LiDAR-Inertial Odometry (LIO) for full coverage.

Result: Experimental validations on a bridge and hall building show Thermo-LIO detects thermal anomalies and defects more accurately than traditional methods.

Conclusion: Thermo-LIO enhances diagnostic precision, enables real-time processing, and expands inspection coverage, advancing SHM methodologies.
...
TL;DR: SAM 2.1 is adapted with pose-guided segmentation and PoseMaskRefine to handle occlusion better, improving robustness and accuracy while retaining generalization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address SAM's limitations under occlusion, where keypoints may be partially or fully invisible, and to improve human segmentation accuracy and robustness.

Method: The method involves fine-tuning SAM 2.1 using PoseMaskRefine, incorporating high-visibility pose keypoints into the iterative correction process. Inference is simplified by selecting only the three keypoints with the highest visibility.

Result: The results show improved accuracy and robustness in occlusion scenarios, with the model capable of accurate mask prediction from as few as a single keypoint.

Conclusion: The adaptation of SAM 2.1 with pose-guided segmentation and the PoseMaskRefine strategy improves robustness and accuracy in occlusion scenarios while retaining the model's generalization capabilities.
...
TL;DR: A new method using focus prediction and segmentation improves accuracy in classifying crystal agglomeration from microscopic images, outperforming baseline models.


<details>
  <summary>Details</summary>
Motivation: The challenge of accurately analyzing crystal agglomeration from 2D microscopic images due to overlapping crystals appearing connected even when at different depth layers, and the limitations of traditional focus measures, motivated the development of a more accurate method.

Method: The method involves quantifying camera focus using an instance camera focus prediction network to classify focus levels, followed by combining this with an instance segmentation model for agglomeration classification.

Result: The method achieved higher agglomeration classification and segmentation accuracy than baseline models on datasets of ammonium perchlorate and sugar crystals.

Conclusion: The proposed method, combining an instance camera focus prediction network with an instance segmentation model, significantly improves the accuracy of agglomeration classification and segmentation in crystal analysis compared to baseline models.
...
TL;DR: Investigation of visual attention mechanisms in signal detection tasks using simulated tomographic breast images, focusing on lesion detection performance and gaze metrics.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of image and signal properties on visual attention during signal detection, particularly in medical imaging to reduce misdiagnosis.

Method: Used simulated tomographic breast images (DBT) with inserted lesions of varying densities and structures. Conducted an observer study with six participants, collecting eye-gaze data to analyze visual attention.

Result: Detection performance is constrained by perceptual stages, with decision failures being the primary error source. Signal detectability depends on target morphology and background complexity, with visual attention varying by lesion type.

Conclusion: The study highlights the interplay between local signal features and global noise in medical imaging, suggesting that visual attention mechanisms are critical for improving diagnostic accuracy.
...
TL;DR: The paper explores blockwise self-supervised learning (BWSSL) for masked video transformers, comparing it to end-to-end backpropagation in terms of learning dynamics and representation development.


<details>
  <summary>Details</summary>
Motivation: The study is motivated by the underexplored application of BWSSL to masked video modeling and the sparse analyses comparing BWSSL and end-to-end training.

Method: The authors apply blockwise learning to a masked autoencoding video vision transformer by partitioning the encoder into blocks, each optimized with a local masked reconstruction loss.

Result: Training converges and yields representations close to matched end-to-end baselines under linear-probe and retrieval proxies. Blockwise training exposes higher-level structure earlier and induces token-level shifts consistent with stronger early mixing.

Conclusion: The findings highlight late-block saturation and interface formation as contributors to the remaining gap between BWSSL and end-to-end training.
...
TL;DR: STDTrack enhances lightweight object tracking by integrating spatiotemporal dependencies, achieving top performance with real-time efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing lightweight trackers use sparse sampling during training, limiting spatiotemporal information exploration and causing a performance gap with high-performance trackers. STDTrack aims to address this limitation.

Method: STDTrack introduces dense video sampling, a temporally propagating spatiotemporal token, and the Multi-frame Information Fusion Module (MFIFM) to maximize spatiotemporal information utilization. It also includes a multi-scale prediction head for dynamic adaptation to object sizes.

Result: STDTrack achieves state-of-the-art results on six benchmarks, rivaling high-performance non-real-time trackers like MixFormer on GOT-10k, while operating at 192 FPS (GPU) and 41 FPS (CPU).

Conclusion: STDTrack bridges the gap between lightweight and high-performance trackers by integrating reliable spatiotemporal dependencies, achieving state-of-the-art results while maintaining real-time efficiency.
...
TL;DR: This paper introduces a method for localizing rovers in planetary environments using cross-view-localising deep neural networks, semantic segmentation, and synthetic data to overcome the scarcity of real training data. The approach achieves accurate position estimation using particle filters.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to address the challenge of accurate localization for planetary robotics, which is crucial for the autonomy of future missions. The scarcity of real space data with ground-truth labels for training machine learning models is a key limitation that the paper aims to overcome.

Method: The method involves using cross-view-localising dual-encoder deep neural networks to localize rovers in an aerial map. The approach leverages semantic segmentation with vision foundation models and synthetic data to bridge the domain gap to real images. Particle filters are then used for state estimation based on sequences of ground-view images.

Result: The results demonstrate that the proposed method allows for accurate position estimation of rovers over both simple and complex trajectories, using sequences of ground-view images. The inclusion of a new cross-view dataset of real-world rover trajectories and synthetic image pairs further validates the approach.

Conclusion: The paper concludes that the proposed method of using cross-view-localising dual-encoder deep neural networks, combined with semantic segmentation and synthetic data, effectively addresses the scarcity of real space data for training. This approach enables accurate position estimation for rovers in planetary environments, as demonstrated by the successful application of particle filters for state estimation.
...
TL;DR: Proposes WEFT, a dynamic wavelet expert-guided fine-tuning paradigm for efficient large-scale model adaptation in ORSI segmentation, outperforming 21 SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods face challenges with large-scale models due to high computational costs and GPU memory consumption.

Method: Introduces a wavelet expert extractor and expert-guided conditional adapter for dynamic feature regulation and efficient fine-tuning.

Result: Achieves superior performance on three ORSI datasets and optimal results in camouflage, natural, and medical scenarios.

Conclusion: WEFT effectively adapts large-scale models to ORSI segmentation with fewer trainable parameters, demonstrating broad applicability.
...
TL;DR: SAM-Aug leverages SAM’s geometry-aware segmentation to improve few-shot land cover mapping via unsupervised mask priors and RegionSmoothLoss, outperforming baselines without extra labels.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenge of few-shot semantic segmentation in time-series remote sensing images, especially in regions with scarce or costly labeled data. Existing models degrade under limited supervision, limiting real-world applicability.

Method: The proposed SAM-Aug framework constructs cloud-free composite images from temporal sequences and applies SAM in an unsupervised manner to generate geometry-aware mask priors. These priors are integrated into training via RegionSmoothLoss, which enforces consistency within SAM-derived regions across temporal frames.

Result: SAM-Aug achieves a mean test mIoU of 36.21% under a 5% labeled setting, outperforming the state-of-the-art baseline by +2.33 percentage points (6.89% relative improvement). On the most favorable split (seed=42), it reaches 40.28% mIoU, an 11.2% relative gain.

Conclusion: The paper concludes that SAM-Aug, leveraging the Segment Anything Model (SAM), significantly improves few-shot land cover mapping by integrating geometry-aware mask priors and enforcing prediction consistency through RegionSmoothLoss. The method outperforms state-of-the-art baselines without additional labeled data, demonstrating the potential of foundation models like SAM as scalable solutions for remote sensing.
...
TL;DR: Proposes slow4fast-VLN, a dynamic fast-slow reasoning framework for generalized Vision-Language Navigation in unseen environments, enhancing adaptability and performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of traditional Vision-Language Navigation (VLN) methods, which operate under a close-set assumption and struggle with unseen environments and instructions. The real world is open and diverse, requiring generalized navigation abilities.

Method: The proposed method is slow4fast-VLN, a dynamic interactive fast-slow reasoning framework. The fast-reasoning module handles real-time navigation actions, while the slow-reasoning module analyzes accumulated experiences to enhance generalization.

Result: The framework enables continuous adaptation and efficient navigation in unseen scenarios by leveraging fast-slow interaction and memory-based experience extraction.

Conclusion: The slow4fast-VLN framework successfully addresses the challenges of open-world navigation by integrating dynamic fast-slow reasoning, outperforming traditional close-set methods.
...
TL;DR: A novel end-to-end structure-aware multimodal reasoning framework (Qwen3-VL + CMRM) for license plate recognition addresses degradation challenges by integrating explicit structural priors, outperforming traditional and general VLM methods.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the misalignment between pixel-level restoration objectives and semantic recognition goals in traditional two-stage LPR methods, leading to artifacts and errors. General VLMs lack explicit structural modeling for license plate sequences.

Method: The method introduces an end-to-end framework based on Qwen3-VL, featuring the CMRM with learnable Character Slot Queries. These queries retrieve fine-grained evidence for character positions via cross-attention and modulate visual tokens for autoregressive generation. LoRA fine-tuning ensures domain adaptation while retaining generalization.

Result: Extensive experiments on synthetic and real-world degraded datasets show the method significantly outperforms existing restoration-recognition combinations and general VLMs, validating its effectiveness.

Conclusion: The proposed structure-aware multimodal reasoning framework, leveraging Qwen3-VL and the Character-Aware Multimodal Reasoning Module (CMRM), effectively addresses the challenges of license plate recognition under severe degradations. It outperforms traditional methods and general VLMs by integrating explicit structural priors and fine-grained evidence retrieval.
...
TL;DR: LPCANet, a lightweight network for rail defect detection, outperforms existing methods with efficient computational performance and high accuracy, validated on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: This paper addresses the limitations of current vision-based rail defect detection methods, including high computational complexity, excessive parameter counts, and suboptimal accuracy.

Method: The architecture integrates MobileNetv2 as a backbone for RGB feature extraction with a lightweight pyramid module (LPM) for depth processing, coupled with a cross-attention mechanism (CAM) for multimodal fusion and a spatial feature extractor (SFE) for enhanced structural analysis.

Result: Comprehensive evaluations on three unsupervised RGB-D rail datasets demonstrate that LPCANet achieves state-of-the-art performance with only 9.90 million parameters, 2.50 G FLOPs, and 162.60 fps inference speed. Compared to 18 existing methods, LPCANet shows significant improvements, including +1.48% in $S_α$, +0.86% in IOU, and +1.77% in MAE over the best-performing baseline.

Conclusion: The proposed LPCANet framework effectively bridges traditional and deep learning approaches, offering substantial practical value for industrial defect inspection. Future work will focus on further model compression for real-time deployment.
...
TL;DR: CenterPolar introduces a novel SDG-DML framework with centrifugal expansion and centripetal constraint phases to generalize across unseen domains and categories, outperforming existing methods on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: The dual challenge of category and domain shifts in SDG-DML limits real-world applications, and existing methods fail to simulate broad domain shifts due to proxy-based expansion clustering samples near class proxies.

Method: CenterPolar employs two collaborative class-centric polarization phases: Class-Centric Centrifugal Expansion ($C^3E$) to generalize to unseen domains and Class-Centric Centripetal Constraint ($C^4$) to consolidate domain-invariant class information.

Result: Extensive experiments on datasets like CUB-200-2011 Ext., Cars196 Ext., DomainNet, PACS, and Office-Home show CenterPolar's superiority over state-of-the-art methods.

Conclusion: The proposed CenterPolar framework effectively addresses the limitations of existing SDG-DML methods by dynamically expanding and constraining domain distributions, demonstrating superior performance on multiple datasets.
...
TL;DR: SkinFlow introduces a novel framework for dermatology diagnosis by optimizing visual information transmission efficiency, outperforming larger models.


<details>
  <summary>Details</summary>
Motivation: Addressing the 'diffuse attention' issue in LVLMs for dermatology, where subtle lesions are hard to distinguish from noise.

Method: Utilizes a Virtual-Width Dynamic Vision Encoder and a two-stage Reinforcement Learning strategy for alignment and reconstruction.

Result: Achieves +12.06% Top-1 and +28.57% Top-6 accuracy gains on Fitzpatrick17k benchmark.

Conclusion: Optimizing geometric capacity and information flow is more effective than raw parameter scaling for diagnostic precision.
...
TL;DR: SSVP introduces a synergistic approach combining hierarchical visual-semantic fusion and dynamic prompting to enhance zero-shot anomaly detection.


<details>
  <summary>Details</summary>
Motivation: Existing ZSAD methods are limited by single visual backbones, failing to balance global semantics and fine-grained details.

Method: Proposes SSVP with HSVS for multi-scale integration and VCPG for dynamic prompt generation, alongside VTAM for calibration.

Result: Achieves 93.0% Image-AUROC and 92.2% Pixel-AUROC on MVTec-AD, outperforming other zero-shot methods.

Conclusion: SSVP effectively bridges the gap in ZSAD by leveraging synergistic visual-semantic fusion, setting a new benchmark.
...
TL;DR: Model-based training approaches, especially adversarial training and data augmentation using learned nuisance models, outperform baselines in robustness to natural corruptions, with trade-offs in computational cost.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of robustness to natural corruptions in deep learning, especially in safety-sensitive domains like traffic sign recognition, by leveraging learned nuisance models for more resilient models.

Method: The paper explores model-based training approaches that use a learned nuisance variation model to generate realistic corruptions. It also introduces hybrid strategies combining random coverage with adversarial refinement in nuisance space. The methods are evaluated on the CURE-TSR dataset with Snow and Rain corruptions.

Result: Model-based methods consistently outperform baseline methods (Vanilla, Adversarial Training, and AugMix). Model-based adversarial training shows the strongest robustness but with higher computational cost, while model-based data augmentation achieves comparable robustness with less complexity.

Conclusion: The study concludes that model-based training approaches, particularly model-based adversarial training, provide the strongest robustness across various corruptions, albeit with higher computational costs. Model-based data augmentation offers a balance between robustness and computational efficiency.
...
TL;DR: Study evaluates five GenAI platforms for architectural image accuracy, finding limited performance (mean 42%) and common inaccuracies, advocating for labeling and cautious use.


<details>
  <summary>Details</summary>
Motivation: The motivation was to assess the capability of generative AI text-to-image systems to produce accurate architectural imagery, given the historical and rule-bound nature of the field, and to identify common inaccuracies.

Method: The study evaluated five GenAI platforms using 30 architectural prompts, generating four images per prompt (600 images total). Two architectural historians independently scored each image for accuracy against predefined criteria, with disagreements resolved by consensus. Performance was summarized as the number of accurate images per four-image set.

Result: Results showed limited accuracy across platforms (mean 42%), with Common prompts being 2.7-fold more accurate than Rare prompts. All-correct outcomes were similar across platforms, but all-incorrect outcomes varied significantly. Recurring inaccuracies included over-embellishment, confusion between medieval styles and revivals, and misrepresentation of descriptive prompts.

Conclusion: The study concludes that current GenAI text-to-image systems have limited accuracy in reproducing architectural imagery, with performance varying across platforms and prompt types. It advocates for visible labeling of synthetic content, provenance standards for training datasets, and cautious use in educational settings.
...
TL;DR: N EIoU YOLOv9 introduces a lightweight detection framework with a novel loss function, improving accuracy for agricultural disease detection and maintaining efficiency for edge deployment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance detection performance for small and low-contrast targets in agricultural disease imagery, addressing the limitations of standard bounding box regression losses.

Method: The framework introduces a lightweight detection model using the N EIoU loss, which reshapes localization gradients via non-monotonic focusing and geometric decoupling. It is integrated into the YOLOv9t architecture and evaluated on a dataset of rice leaf images.

Result: The N EIoU YOLOv9 achieves a mean Average Precision of 90.3%, a 4.3% improvement over the baseline, with an inference time of 156 milliseconds per frame on an Android device.

Conclusion: The proposed N EIoU YOLOv9 framework, with its innovative loss function, effectively balances accuracy and computational efficiency, making it suitable for edge-based agricultural monitoring systems.
...
TL;DR: A deployment-oriented framework using knowledge distillation creates compact, efficient student models from high-capacity segmentation models, maintaining accuracy and compatibility for on-premises clinical use.


<details>
  <summary>Details</summary>
Motivation: The need to deploy medical image segmentation models in resource-constrained on-premises clinical environments, where computational resources are fixed and cloud-based inference may not be feasible due to governance and security policies.

Method: The framework uses knowledge distillation to translate a high-performing segmentation model into a scalable family of compact student models, preserving architectural compatibility with existing clinical systems.

Result: Under a 94% parameter reduction, the distilled student model retains 98.7% of the teacher model's segmentation accuracy and achieves a 67% reduction in CPU inference latency.

Conclusion: The study successfully demonstrates that knowledge distillation can effectively convert high-capacity segmentation models into compact, deployment-ready versions without compromising accuracy or compatibility with existing clinical systems.
