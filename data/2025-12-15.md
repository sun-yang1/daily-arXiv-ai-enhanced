...
TL;DR: Novel text-guided methods improve fairness in facial gender classification by leveraging image captions and multimodal fusion, reducing bias without demographic labels.


<details>
  <summary>Details</summary>
Motivation: Addressing demographic bias in AI, specifically in facial gender classification, by integrating semantic information from text to enhance fairness and accuracy.

Method: Utilizes Image Text Matching (ITM) guidance and Image Text fusion to align images and texts for improved multimodal representations and fairness.

Result: Experiments show reduced bias and higher accuracy across gender and racial groups compared to existing methods, without needing demographic labels.

Conclusion: The text-guided approach offers an interpretable, effective solution for mitigating bias in facial analysis algorithms, applicable broadly without demographic data.
...
TL;DR: SoccerMaster is a unified model for soccer visual understanding that outperforms task-specific models, with data, code, and model to be publicly available.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the domain-specific complexity and unique challenges of soccer understanding by proposing a unified model instead of relying on isolated, task-specific expert models.

Method: The method involves developing SoccerMaster, a soccer-specific vision foundation model, and an automated data curation pipeline to generate scalable spatial annotations. It integrates these with existing datasets to construct SoccerFactory.

Result: The results show that SoccerMaster consistently outperforms task-specific expert models across diverse downstream tasks.

Conclusion: The paper concludes that SoccerMaster, a unified model for soccer visual understanding, outperforms task-specific expert models across diverse tasks. The data, code, and model will be publicly available.
...
TL;DR: Knowledge distillation trains CNN models for TB classification, reducing spurious correlations and localizing abnormalities without bounding-box annotations, with the student model outperforming the teacher.


<details>
  <summary>Details</summary>
Motivation: TB is a leading cause of mortality, especially in resource-limited countries. While CXR is cost-effective, it requires expert interpretation, which is often unavailable. Machine learning models for TB classification often rely on spurious correlations and fail to generalize, and creating large datasets with high-quality annotations is resource-intensive.

Method: The study used a teacher-student framework with ResNet50 architecture, trained on the TBX11k dataset, to achieve a high mIOU score of 0.2428.

Result: The proposed method achieved a 0.2428 mIOU score, with the student model consistently outperforming the teacher model, demonstrating improved robustness.

Conclusion: The study successfully repurposed knowledge distillation to train CNN models for TB classification, reducing spurious correlations and localizing TB-related abnormalities without needing bounding-box annotations. The student model outperformed the teacher model, showing improved robustness and potential for clinical deployment.
...
TL;DR: The paper introduces Synthetic Vasculature Reasoning (SVR) to generate synthetic OCTA images and detailed reasoning texts, addressing the scarcity of labeled data in specialized domains like medical imaging. It curates OCTA-100K-SVR, a large-scale dataset, and demonstrates that a VLM trained on this dataset achieves high zero-shot accuracy and improves explanation quality.


<details>
  <summary>Details</summary>
Motivation: The lack of large-scale, detailed text descriptions for specialized medical imaging modalities like OCTA hinders the training of Vision-Language Models (VLMs) for interpretable diagnosis.

Method: The authors propose SVR, a framework to synthesize realistic retinal vasculature images with DR features and corresponding granular reasoning texts. They curate OCTA-100K-SVR, a dataset of 100,000 synthetic image-text pairs.

Result: A general-purpose VLM (Qwen3-VL-8b) trained on OCTA-100K-SVR achieves 89.67% zero-shot accuracy on real OCTA images, outperforming supervised baselines. Human expert evaluation confirms improved explanation quality and pathology localization.

Conclusion: SVR effectively addresses the data scarcity problem in specialized medical domains, enabling high-performance VLMs for interpretable diagnosis and enhancing clinical utility.
...
TL;DR: VDAWorld uses a VLM to create abstract, simulatable representations from images, enabling high-quality dynamic simulations and addressing key limitations of generative video models.


<details>
  <summary>Details</summary>
Motivation: Generative video models face limitations such as violating physical and logical rules, lacking interactivity, and being opaque black boxes. These challenges hinder their suitability for building structured, queryable worlds.

Method: The authors propose VDAWorld, a framework where a Vision-Language Model (VLM) acts as an intelligent agent to distill image-caption pairs into abstract representations optimized for simulation. The VLM autonomously constructs grounded scene representations and selects compatible physics simulators.

Result: VDAWorld can infer latent dynamics from static scenes to predict plausible future states. Experiments demonstrate its ability to produce high-quality simulations across diverse dynamic scenarios.

Conclusion: VDAWorld combines intelligent abstraction and adaptive simulation to create a versatile world model, overcoming the limitations of traditional generative video models.
...
TL;DR: Survey on the evolution of urban dynamics monitoring, focusing on event-based cameras and their advantages, challenges, and potential multi-sensor fusion applications.


<details>
  <summary>Details</summary>
Motivation: To explore better practices for understanding city dynamics by leveraging advanced technologies like event-based cameras.

Method: Analysis of event-based cameras, their applications, advantages, challenges, and machine learning integration.

Result: Event-based cameras offer unique capabilities for urban dynamics monitoring, especially in low-light conditions, and their fusion with other sensors can enhance performance.

Conclusion: Event-based cameras, especially when combined with other sensors, present a promising approach for studying urban dynamics while addressing privacy and technical challenges.
...
TL;DR: VLM-IRIS adapts VLMs for infrared data, enabling zero-shot industrial monitoring without retraining.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of conventional vision systems in low-light or enclosed manufacturing environments by leveraging infrared cameras and zero-shot learning frameworks.

Method: The method involves preprocessing infrared images into RGB-compatible inputs for CLIP-based encoders, using magma representation and centroid prompt ensembling.

Result: The results demonstrate high accuracy in zero-shot workpiece presence detection on a 3D printer bed using thermal imaging, validating the framework's effectiveness.

Conclusion: The paper concludes that VLM-IRIS effectively adapts vision-language models to infrared data, enabling zero-shot predictions for industrial applications without the need for labeled datasets or model retraining.
...
TL;DR: VGent introduces a modular encoder-decoder architecture for visual grounding, achieving state-of-the-art performance with fast inference.


<details>
  <summary>Details</summary>
Motivation: Current visual grounding models either suffer from slow auto-regressive decoding and hallucinations or compromise the LLM's reasoning ability by re-aligning it with vision features. VGent aims to address these issues by disentangling reasoning and prediction.

Method: VGent employs a modular encoder-decoder architecture, using a frozen MLLM as the encoder for reasoning and a decoder for bounding box prediction. It introduces QuadThinker for multi-target reasoning, mask-aware labels for ambiguity resolution, and global target recognition.

Result: VGent outperforms prior methods with +20.6% F1 improvement, +8.2% gIoU, and +5.8% cIoU under visual reference challenges.

Conclusion: VGent achieves state-of-the-art performance with significant improvements in F1, gIoU, and cIoU metrics, while maintaining fast inference latency.
...
TL;DR: Intelligent fusion of pathology foundation models improves cancer grading and staging performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: To address the limited understanding of the complementarity, redundancy, and biological interpretation of features in pathology FMs, the study proposes an information-driven fusion strategy for integrating multiple FMs into a unified representation.

Method: The study evaluated three FM fusion schemes at tile and slide levels: majority-vote ensembling, naive feature concatenation, and intelligent fusion based on correlation-guided pruning of redundant features. Diagnostic H&E whole-slide images from kidney, prostate, and rectal cancers were used.

Result: Intelligent fusion of tile-level embeddings yielded consistent gains in classification performance across all three cancers compared to the best single FMs and naive fusion. Attention maps showed concentrated focus on tumor regions and reduced spurious focus on benign regions.

Conclusion: The study demonstrates that intelligent, correlation-guided fusion of pathology foundation models (FMs) enhances predictive performance and interpretability in computational pathology tasks, providing compact and task-tailored representations.
...
TL;DR: LEGO is a three-stage framework for adapting pre-trained image restoration models to real-world, out-of-distribution degradations without paired data, using a generative oracle to create pseudo-ground-truths and mixed-supervision fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Pre-trained models often fail on real-world, out-of-distribution degradations due to domain gaps, and traditional adaptation methods require complex changes.

Method: LEGO involves: 1) initial restorations from the pre-trained model, 2) refinement using a generative oracle for pseudo-ground-truths, and 3) fine-tuning with mixed-supervision (in-distribution data + pseudo-pairs).

Result: LEGO effectively bridges the domain gap, improving performance on diverse real-world benchmarks without sacrificing original robustness or requiring architectural changes.

Conclusion: LEGO provides a practical solution for post-training domain adaptation in image restoration, addressing the challenge of unpaired out-of-distribution data.
...
TL;DR: Fast-FoundationStereo combines knowledge distillation, architecture search, and pruning to achieve real-time zero-shot stereo matching, outperforming prior methods in speed and accuracy.


<details>
  <summary>Details</summary>
Motivation: Stereo foundation models achieve strong zero-shot generalization but are computationally prohibitive for real-time applications. Efficient stereo architectures sacrifice robustness for speed and require costly per-domain fine-tuning.

Method: The paper presents Fast-FoundationStereo, a family of architectures employing a divide-and-conquer acceleration strategy with three components: (1) knowledge distillation to compress the hybrid backbone into a single efficient student; (2) blockwise neural architecture search for discovering optimal cost filtering designs under latency budgets; and (3) structured pruning for eliminating redundancy in the iterative refinement module. Additionally, an automatic pseudo-labeling pipeline curates 1.4M in-the-wild stereo pairs to supplement synthetic training data.

Result: The resulting model runs over 10x faster than FoundationStereo while closely matching its zero-shot accuracy, establishing a new state-of-the-art among real-time methods.

Conclusion: Fast-FoundationStereo bridges the gap between zero-shot generalization and real-time performance, achieving both without sacrificing robustness.
...
TL;DR: ItemizedCLIP is a framework for learning visual representations from itemized text supervision, improving zero-shot performance and interpretability across domains like medical imaging and remote sensing.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of learning from itemized text annotations in non-object-centric visual domains, where standard multi-caption supervision is insufficient.

Method: ItemizedCLIP uses a cross-attention module and tailored objectives to enforce item independence and representation completeness.

Result: Substantial improvements in zero-shot performance and fine-grained interpretability across multiple domains.

Conclusion: ItemizedCLIP provides semantically grounded, item-differentiable, complete, and visually interpretable representations.
...
TL;DR: Reproduction and analysis of Monkey VLM (Li et al. 2023b) confirms tiling recovers local details but highlights deviations based on task type and tile granularity, with extensions exploring global context.


<details>
  <summary>Details</summary>
Motivation: Addressing reproducibility issues in complex multimodal models by transparently replicating and analyzing Monkey VLM's high-resolution image understanding approach.

Method: Replicated the tiling strategy using open checkpoints and reimplemented the training pipeline, then extended the work by investigating global context inclusion.

Result: Confirmed tiling recovers local details but found deviations dependent on task type and tile granularity. Extended insights for future high-resolution multimodal modeling.

Conclusion: Tiling is effective for local detail recovery, but results vary by task and granularity. Global context inclusion offers practical insights, highlighting the need for further research in high-resolution multimodal models.
...
TL;DR: LGSCV: A lightweight 3D GS compression method using Morton scans, PCA, and MiniPLAS for efficient video codec integration, achieving 20% RD gain and 50% faster encoding.


<details>
  <summary>Details</summary>
Motivation: To address the computational expense and time-consuming nature of current GS compression methods, limiting their application on lightweight devices.

Method: A two-stage Morton scan for blockwise 2D map generation, PCA for SH dimensionality reduction, and MiniPLAS for primitive permutation within block sizes.

Result: Significant improvement in RD performance, especially at medium and low bitrates, and reduced encoding time.

Conclusion: The proposed LGSCV method achieves over 20% RD gain compared with state-of-the-art methods, reduces 2D map generation time to approximately 1 second, and cuts encoding time by 50%.
...
TL;DR: A solution for the BinEgo-360 Challenge at ICCV 2025, focusing on temporal action localization in multi-perspective and multi-modal video settings, using an extended Temporal Shift Module and multi-task learning.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of temporal action localization in diverse video settings (panoramic, third-person, egocentric) with fine-grained annotations.

Method: Extended Temporal Shift Module (TSM) with a background class and fixed-length non-overlapping intervals, multi-task learning for scene classification and TAL, and weighted ensemble strategy.

Result: Ranked first in both initial and extended rounds of the competition.

Conclusion: The combination of multi-task learning, an efficient backbone, and ensemble learning proves effective for temporal action localization.
...
TL;DR: CADKnitter is a framework for generating complementary CAD parts using geometry-guided diffusion sampling, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Traditional CAD modeling is time-intensive and requires expertise. Existing methods focus on single-part generation, which lacks practicality for real-world applications requiring multi-part assemblies under constraints.

Method: Proposes CADKnitter, a compositional CAD generation framework with geometry-guided diffusion sampling, and introduces the KnitCAD dataset with 310,000 samples.

Result: CADKnitter outperforms state-of-the-art baselines by a clear margin.

Conclusion: CADKnitter advances CAD generation by enabling multi-part assemblies under semantic and geometric constraints, demonstrating superior performance.
...
TL;DR: AutoRefiner enhances AR-VDMs by refining noise along denoising paths, improving fidelity without parameter updates.


<details>
  <summary>Details</summary>
Motivation: AR-VDMs show promise but need better sample fidelity. Existing methods like inference-time alignment are impractical for AR-VDMs.

Method: Proposes AutoRefiner with pathwise noise refinement and reflective KV-cache.

Result: AutoRefiner effectively improves sample fidelity in AR-VDMs.

Conclusion: AutoRefiner is a practical solution for enhancing AR-VDMs without computational overhead.
...
TL;DR: SmokeBench evaluates MLLMs on wildfire smoke detection, revealing limitations in early-stage localization despite some success in classification.


<details>
  <summary>Details</summary>
Motivation: Wildfire smoke is visually confounded with clouds, making early-stage detection challenging. This work aims to evaluate MLLMs' ability to recognize and localize wildfire smoke in images.

Method: The benchmark, SmokeBench, evaluates MLLMs on four tasks: smoke classification, tile-based smoke localization, grid-based smoke localization, and smoke detection. Models tested include Idefics2, Qwen2.5-VL, InternVL3, Unified-IO 2, Grounding DINO, GPT-4o, and Gemini-2.5 Pro.

Result: Results show that while some models can classify smoke when it covers a large area, all struggle with accurate localization, especially in early stages. Smoke volume strongly correlates with performance, whereas contrast plays a minor role.

Conclusion: The findings highlight critical limitations of current MLLMs for safety-critical wildfire monitoring and underscore the need for improved methods for early-stage smoke localization.
...
TL;DR: A generative forecaster using autoregressive flow matching in VFM feature space outperforms deterministic regression, offering scalable and accurate world modeling.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of deterministic regression averaging over plausible futures, which undermines forecast accuracy by failing to capture uncertainty.

Method: The method involves autoregressive flow matching in VFM feature space, encoding features into a compact latent space suitable for diffusion, and decoding into interpretable output modalities.

Result: The results show that the proposed method produces sharper and more accurate predictions than regression across all modalities, including semantic segmentation, depth, surface normals, and RGB.

Conclusion: The paper concludes that stochastic conditional generation of VFM features provides a scalable and promising foundation for future world models, outperforming deterministic regression in accuracy and sharpness across multiple modalities.
...
TL;DR: FutureX enhances end-to-end autonomous driving planners by using Chain of Thought (CoT) for future scene reasoning and trajectory refinement, improving motion planning quality and reducing collisions.


<details>
  <summary>Details</summary>
Motivation: Exclusive reliance on current scene data in dynamic traffic environments leads to suboptimal motion planning due to evolving interactions between the ego vehicle and its surroundings.

Method: FutureX employs a CoT-driven pipeline with an Auto-think Switch to decide between Thinking mode (future scene latent reasoning via Latent World Model and Summarizer Module) and Instant mode (forward pass for simple scenes).

Result: FutureX improves motion planning rationality and reduces collisions without efficiency loss, achieving a 6.2 PDMS improvement for TransFuser on NAVSIM.

Conclusion: FutureX demonstrates significant performance gains in autonomous driving by integrating future scene reasoning, making it a promising advancement for end-to-end planners.
...
TL;DR: REST is a real-time, end-to-end streaming audio-driven talking head generation framework using diffusion models, addressing slow inference speeds and non-autoregressive constraints.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of slow inference speeds and non-autoregressive paradigms in diffusion-based talking head generation (THG) models.

Method: Proposes a compact video latent space via high spatiotemporal VAE compression, an ID-Context Cache mechanism for autoregressive streaming, and Asynchronous Streaming Distillation (ASD) for training.

Result: REST outperforms state-of-the-art methods in generation speed and overall performance.

Conclusion: REST bridges the gap between autoregressive and diffusion-based approaches, offering substantial value for real-time THG applications.
...
TL;DR: RoomPilot is a unified framework for generating controllable and interactive indoor scenes using multi-modal inputs, overcoming limitations of existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing methods for indoor scene generation are either limited in input modalities or rely on stochastic processes, reducing controllability.

Method: RoomPilot introduces an Indoor Domain-Specific Language (IDSL) to parse diverse inputs (text or CAD plans) into structured scenes, leveraging interaction-annotated assets.

Result: The framework demonstrates strong multi-modal understanding, fine-grained controllability, and superior physical consistency and visual fidelity.

Conclusion: RoomPilot marks a significant advancement in general-purpose controllable 3D indoor scene generation.
...
TL;DR: WildCap introduces a hybrid inverse rendering framework for high-quality facial appearance capture from smartphone videos in uncontrolled lighting, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of existing methods that require controllable lighting, which increases capture cost and limits usability. The goal is to achieve high-quality facial appearance capture from readily available smartphone videos recorded in uncontrolled environments.

Method: The method involves a hybrid inverse rendering framework: first using SwitchLight to convert images into constrained conditions, then applying model-based inverse rendering. A texel grid lighting model is introduced to handle non-physical artifacts, and optimization jointly samples a diffusion prior for reflectance maps while optimizing lighting.

Result: WildCap achieves significantly better results than prior methods in the same capture setup, effectively reducing the quality gap between in-the-wild and controllable recordings.

Conclusion: The paper concludes that WildCap significantly improves the quality of facial appearance capture from in-the-wild smartphone videos, closing the gap with controllable lighting setups. The code will be made available for further research and application.
...
TL;DR: A novel Cross-modal Prompting method improves incomplete multi-modal emotion recognition by enhancing modality features and boosting accuracy.


<details>
  <summary>Details</summary>
Motivation: To overcome the performance gap and modality under-optimization problem in incomplete multi-modal emotion recognition, exacerbated by missing data.

Method: The method includes a progressive prompt generation module with a dynamic gradient modulator, cross-modal knowledge propagation, and a coordinator for dynamic re-weighting of modality outputs.

Result: Extensive experiments on 4 datasets with 7 state-of-the-art methods under different missing rates validate the method's effectiveness.

Conclusion: The proposed Cross-modal Prompting (ComP) method effectively addresses the challenges of incomplete multi-modal emotion recognition by enhancing modality-specific features and improving recognition accuracy through coherent information and dynamic re-weighting.
...
TL;DR: PersonaLive is a diffusion-based framework for real-time portrait animation, focusing on latency reduction and efficiency via multi-stage training, hybrid signals, and autoregressive streaming.


<details>
  <summary>Details</summary>
Motivation: Existing models prioritize visual quality over latency, limiting live streaming applications.

Method: Uses hybrid implicit signals (facial representations, 3D keypoints), appearance distillation, and autoregressive micro-chunk streaming with sliding training.

Result: Achieves 7-22x speedup over prior models with SOTA performance.

Conclusion: PersonaLive enables efficient, low-latency portrait animation for live streaming.
...
TL;DR: The paper explores Reformer architecture as a computationally efficient alternative to Vision Transformers (ViTs) for vision tasks, comparing performance on CIFAR-10, ImageNet-100, and high-resolution medical datasets.


<details>
  <summary>Details</summary>
Motivation: Standard ViTs are computationally expensive due to quadratic self-attention scaling, limiting practicality for high-resolution inputs and resource-constrained settings.

Method: Proposes a Reformer-based vision model combining patch-based tokenization with locality-sensitive hashing (LSH) attention to reduce time complexity from O(n²) to O(n log n).

Result: Reformer achieves higher accuracy on CIFAR-10 but underperforms ViT in practical efficiency and computation time for larger/high-resolution datasets.

Conclusion: LSH-based attention's theoretical advantages don't translate to practical gains for typical high-resolution images, suggesting further optimization is needed.
...
TL;DR: The study evaluates Sentinel-2 imagery for monitoring serrated tussock, an invasive grass, achieving comparable accuracy to aerial imagery but at a lower cost.


<details>
  <summary>Details</summary>
Motivation: Invasive species like serrated tussock disrupt ecosystems and agriculture, but current monitoring methods are costly or unscalable.

Method: Eleven models using Sentinel-2 data (spectral bands, texture features, vegetation indices, seasonal data) were tested with a random forest classifier.

Result: Best Sentinel-2 model achieved 68% accuracy (OA) and 0.55 Kappa, slightly outperforming aerial imagery (67% OA, 0.52 Kappa).

Conclusion: Sentinel-2 offers a cost-effective, scalable alternative for landscape-scale invasive species monitoring.
...
TL;DR: FilmWeaver is a novel framework for generating consistent, multi-shot videos of arbitrary length by addressing inter-shot consistency and intra-shot coherence through a dual-level cache mechanism.


<details>
  <summary>Details</summary>
Motivation: Current video generation models struggle with multi-shot videos, particularly in maintaining character and background consistency across shots and flexibly generating videos of arbitrary length and shot count.

Method: FilmWeaver employs an autoregressive diffusion paradigm and a dual-level cache mechanism (shot memory for inter-shot consistency and temporal memory for intra-shot coherence) to achieve flexible, multi-shot video generation.

Result: The framework surpasses existing approaches in consistency and aesthetic quality, supporting downstream tasks like multi-concept injection and video extension.

Conclusion: FilmWeaver opens up new possibilities for creating more consistent, controllable, and narrative-driven video content.
...
TL;DR: The paper proposes a recursive autoencoder (RcAE) with a Cross Recursion Detection (CRD) module and Detail Preservation Network (DPN) to improve anomaly detection in industrial settings. It outperforms traditional methods and matches diffusion models with fewer parameters and faster inference.


<details>
  <summary>Details</summary>
Motivation: Traditional autoencoder-based methods struggle with incomplete anomaly suppression and loss of fine details due to single-pass decoding.

Method: The paper introduces a recursive architecture for autoencoder (RcAE) that iteratively reconstructs data, a CRD module to track inconsistencies, and a DPN to preserve high-frequency textures.

Result: The method significantly outperforms existing non-diffusion methods and achieves performance comparable to diffusion models with only 10% of their parameters and faster inference.

Conclusion: The proposed approach is practical and efficient for real-world industrial anomaly detection applications.
...
TL;DR: ARVAE is an autoregressive video autoencoder that decouples temporal and spatial information for efficient video compression and reconstruction, achieving high quality with lightweight models.


<details>
  <summary>Details</summary>
Motivation: Existing video autoencoders entangle spatial and temporal information, limiting temporal consistency and performance.

Method: ARVAE compresses and reconstructs frames autoregressively, using a temporal-spatial decoupled representation with downsampled flow fields and spatial relative compensation.

Result: ARVAE achieves superior reconstruction quality with lightweight models and small-scale training data.

Conclusion: ARVAE demonstrates strong potential for downstream video generation tasks due to its efficient and high-quality performance.
