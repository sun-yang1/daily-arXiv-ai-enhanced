...
TL;DR: IndicVisionBench is introduced as the first large-scale benchmark for evaluating vision-language models (VLMs) in culturally diverse and multilingual settings, focusing on the Indian subcontinent. It reveals performance gaps in current VLMs.


<details>
  <summary>Details</summary>
Motivation: Most VLM evaluation benchmarks are Western-centric, leaving gaps in understanding performance in culturally diverse and multilingual contexts.

Method: The benchmark covers English and 10 Indian languages across 3 multimodal tasks (OCR, MMT, VQA) with ~5K images and 37K+ QA pairs. It includes a parallel corpus for analyzing biases.

Result: Evaluation of 8 models shows substantial performance gaps, highlighting limitations of current VLMs in diverse settings.
...
TL;DR: A novel knowledge-based anomaly detection method is introduced to identify network-induced shape artifacts in synthetic images, improving synthetic data quality for machine learning.


<details>
  <summary>Details</summary>
Motivation: Synthetic data can introduce artifacts and distortions, compromising model performance and clinical utility, necessitating quality assessment methods.
...
Result: High AUC values (0.97 and 0.91) and human agreement rates (66% and 68%) confirm effectiveness in detecting artifacts.

Conclusion: The method enhances responsible synthetic data use by evaluating anatomic constraints and improving dataset quality.
...
TL;DR: CPO enhances controllability in text-to-image generation by optimizing control conditions, outperforming ControlNet++ with significant error reductions.


<details>
  <summary>Details</summary>
Motivation: Existing methods like ControlNet++ optimize only low-noise timesteps, ignoring high-noise contributions and introducing approximation errors. Direct Preference Optimization (DPO) struggles with ensuring controllability differences without affecting other factors like image quality.

Method: CPO performs preference learning over control conditions (winning and losing control signals) instead of generated images, eliminating confounding factors and reducing training objective variance.

Result: CPO achieves over 10% error rate reduction in segmentation, 70–80% in human pose, and consistent 2–5% reductions in edge and depth maps compared to ControlNet++.

Conclusion: Condition Preference Optimization (CPO) significantly improves controllability in text-to-image generation, outperforming ControlNet++ across various control types with notable error rate reductions.
...
TL;DR: DARN introduces dynamic adaptive regularization for geospatial foundation models, outperforming standard methods with innovations like task complexity prediction and adaptive dropout.


<details>
  <summary>Details</summary>
Motivation: Standard adaptation methods for foundation models in geospatial analysis fail to account for the significant heterogeneity in satellite imagery, limiting their effectiveness.

Method: DARN integrates a Task Complexity Predictor (TCP), Adaptive Dropout Modulation (ADM), and Dynamic Capacity Gating (DCG) to dynamically adjust regularization and activation based on per-sample difficulty.

Result: DARN achieves state-of-the-art performance in both full fine-tuning (86.66% mIoU on GeoBench) and efficient adaptation (90.5% mIoU on Sen1Floods11), with superior OOD generalization, robustness, and minority class performance.

Conclusion: DARN provides a more intelligent, robust, and efficient approach to leveraging foundation models in geospatial applications, addressing the heterogeneity in satellite imagery through dynamic adaptation.
...
TL;DR: A new ML framework translates 2D satellite data into 3D cloud maps, improving tropical cyclone forecasting by reconstructing intense storm structures globally.


<details>
  <summary>Details</summary>
Motivation: Accurate forecasting of tropical cyclones is challenging due to limited satellite observations and difficulties in resolving cloud properties involved in intensification.

Method: A pre-training--fine-tuning pipeline is used to translate 2D satellite imagery into 3D cloud maps, leveraging multiple satellites with global coverage.

Result: The model generates global instantaneous 3D cloud maps and reconstructs 3D structures of intense storms, even when observations are missing.

Conclusion: The new framework successfully creates global 3D cloud maps and accurately reconstructs the 3D structure of intense tropical cyclones, advancing understanding and improving forecasts.
...
TL;DR: EETnet is a CNN for event-based eye tracking, deployable on microcontrollers, with two architecture versions: classification and regression models.


<details>
  <summary>Details</summary>
Motivation: Existing event-based eye tracking solutions often rely on powerful GPUs, limiting deployment on embedded devices.

Method: Developed EETnet, a CNN for event-based data, with training, evaluation, and quantization methodology. Proposed classification and regression models.

Result: EETnet is capable of running on resource-limited microcontrollers.

Conclusion: The paper presents a practical solution for efficient, low-power eye tracking on embedded devices using event-based cameras.
...
TL;DR: The paper introduces 3D Gaussian Point Encoders, an explicit geometric representation for 3D recognition tasks, outperforming implicit methods like PointNet in speed, memory efficiency, and computational cost.


<details>
  <summary>Details</summary>
Motivation: The motivation is to move from implicit representations (e.g., PointNet) to explicit geometric representations (e.g., 3D Gaussians) for 3D recognition tasks, leveraging computational geometry heuristics for efficiency.

Method: The method involves developing optimization techniques using natural gradients and distillation from PointNets to learn 3D Gaussian encoders, along with filtering techniques from 3D Gaussian Splatting for acceleration.

Result: Results show the 3D Gaussian Point Encoders are 2.7x faster, use 46% less memory, and require 88% fewer FLOPs than PointNet, while also improving efficiency in Mamba3D (1.27x faster, 42% less memory, 54% fewer FLOPs).

Conclusion: The 3D Gaussian Point Encoders offer a lightweight, efficient alternative to traditional implicit methods, achieving high performance on CPU-only devices.
...
TL;DR: The paper analyzes data redundancy and cross-domain transfer in biomedical image segmentation using Cellpose, proposing a dataset quantization strategy and demonstrating its effectiveness in reducing training data needs and mitigating catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: The study addresses underexplored challenges in generalist biomedical image segmentation models: training data redundancy and the impact of cross-domain transfer on model retention.

Method: The authors use Cellpose as a case study, introducing a dataset quantization (DQ) strategy to create compact training subsets and conducting cross-domain finetuning experiments to examine catastrophic forgetting. Latent space analysis and selective DQ-based replay are employed.

Result: Results show that segmentation performance saturates with 10% of data, indicating redundancy. Selective replay of 5-10% source data restores source performance, while full replay hinders adaptation. Training domain sequencing improves generalization.

Conclusion: The study emphasizes data-centric design in biomedical segmentation, advocating for compact training subsets, retention-aware learning strategies, and informed domain ordering to enhance efficiency.
...
TL;DR: A data-centric AI workflow combines active learning and pseudo-labeling to reduce manual annotation needs while maintaining performance in biomedical image segmentation.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of requiring large annotated datasets for nnU-Net and the limitations of large foundation models in specific biomedical tasks.

Method: Proposes a workflow using pseudo-labeling from foundation models for nnU-Net self-configuration, followed by active learning for minimal manual annotation.

Result: Significantly reduces manual annotation requirements while maintaining competitive segmentation performance.

Conclusion: Provides an accessible solution for biomedical researchers to apply advanced AI techniques in segmentation tasks.
...
TL;DR: A new geometry denoising method uses prior knowledge of surface normal vectors (label vectors) for segmentation and regularization, solved via split Bregman and second-order shape calculus.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve geometry denoising by incorporating prior knowledge of surface normal vectors, enabling simultaneous denoising and segmentation for more accurate results.

Method: The method involves using label vectors as prior knowledge for surface normal vectors, embedding segmentation into denoising, and applying a total variation term for regularization. A split Bregman (ADMM) approach is used to solve the optimization problem, with vertex updates based on second-order shape calculus.

Result: The proposed method effectively denoises geometry while segmenting based on normal vector similarity, demonstrating the utility of the split Bregman approach and second-order shape calculus.

Conclusion: The paper presents a novel geometry denoising method leveraging prior knowledge of surface normal vectors, achieving effective denoising and segmentation through a split Bregman approach and second-order shape calculus.
...
TL;DR: An implicit self-prior approach distills shape-specific priors from point clouds, integrating them with RIMLS to generate high-fidelity surfaces, outperforming other methods.


<details>
  <summary>Details</summary>
Motivation: Recovering high-quality surfaces from irregular point clouds is ill-posed without strong geometric priors. The method aims to distill shape-specific priors directly from the input point cloud.

Method: The approach involves training a small dictionary of learnable embeddings with an implicit distance field using self-supervised point cloud reconstruction losses. The trained field is sampled for dense points and normals, integrated into RIMLS.

Result: The method effectively preserves fine geometric details and regularizes sparse regions, outperforming existing approaches.

Conclusion: The hybrid strategy combining learned self-priors with RIMLS formulation outperforms classical and learning-based methods in generating high-fidelity surfaces, preserving details, and handling data degradations.
...
TL;DR: Clinical-ComBAT is a flexible method for harmonizing DW-MRI data in clinical settings, overcoming ComBAT's limitations with non-linear modeling and adaptability to small cohorts.


<details>
  <summary>Details</summary>
Motivation: The limitations of the widely used ComBAT method, such as reliance on linear covariate relationships and homogeneous populations, constrain its clinical use. Clinical-ComBAT is designed to address these issues for real-world clinical scenarios.

Method: Clinical-ComBAT independently harmonizes each site, uses a non-linear polynomial data model, site-specific harmonization referenced to a normative site, and adaptable variance priors. It also includes hyperparameter tuning and a goodness-of-fit metric.

Result: Clinical-ComBAT demonstrates improved alignment of diffusion metrics and enhanced applicability for normative modeling in both simulated and real data.

Conclusion: Clinical-ComBAT is a robust and flexible method for harmonizing DW-MRI data in clinical settings, overcoming the limitations of traditional ComBAT by addressing non-linear relationships, small cohorts, and dynamic site additions.
...
TL;DR: Swin transformers initially outperformed ResNet in diagnosing ear diseases but revealed data leakage issues, stressing the need for careful data preprocessing in medical ML.


<details>
  <summary>Details</summary>
Motivation: With a 27% misdiagnosis rate among otolaryngologists, the study aimed to improve diagnostic accuracy for ear diseases using vision transformer models.

Method: The research utilized Swin v1 and Swin v2 transformer models and ResNet on a real-world dataset of otoscopic videos, employing Laplacian and Shannon entropy thresholds for frame selection and removing blank frames.

Result: Initial accuracies were 100% (Swin v1) and 99.1% (Swin v2), outperforming ResNet (99.5%). After addressing data leakage, corrected accuracies dropped to 83% (Swin v1/v2) and 82% (ResNet).

Conclusion: The study underscores the importance of rigorous data handling in machine learning for medical applications, emphasizing the need to balance advanced model architectures with effective data preprocessing to ensure reliable diagnostic tools.
...
TL;DR: A novel geospatial deep learning framework uses satellite imagery to predict fatal crash risks with uncertainty-aware outputs, improving recall by 17-23% and aiding safer autonomous navigation and urban planning.


<details>
  <summary>Details</summary>
Motivation: Traditional traffic safety studies overlook spatial complexity and contextual interactions, while conventional Neural Network-based risk estimators lack model uncertainty, limiting their utility in critical decision-making.

Method: The framework leverages satellite imagery to capture nuanced spatial patterns and environmental risk factors, estimating a full Beta probability distribution over fatal crash risk for uncertainty-aware predictions.

Result: The model outperforms baselines with a 17-23% improvement in recall and superior calibration, providing accurate and uncertainty-aware predictions.

Conclusion: The proposed geospatial deep learning framework offers a scalable and cost-effective tool for urban planners and policymakers to enhance roadway safety by providing reliable and interpretable risk assessments from satellite imagery.
...
TL;DR: A novel network (IMDNet) addresses multi-degradation image restoration by decoupling degradation types and dynamically selecting restoration paths, outperforming single-degradation methods.


<details>
  <summary>Details</summary>
Motivation: Real-world images often suffer from multiple coexisting degradations, but most existing methods focus on single degradation types, limiting their practical use.

Method: The method involves a degradation ingredient decoupling block (DIDBlock) for separating degradation types, a fusion block (FBlock) for integrating degradation information, and a task adaptation block (TABlock) for dynamically selecting restoration paths.

Result: IMDNet achieves superior performance in multi-degradation restoration and maintains strong competitiveness in single-degradation tasks.

Conclusion: The proposed IMDNet demonstrates superior performance in multi-degradation image restoration while remaining competitive in single-degradation tasks, showcasing its practical effectiveness.
...
TL;DR: A comprehensive multimodal dataset for AI in oral healthcare was introduced, fine-tuned models outperformed baselines, and the dataset is publicly available for research.


<details>
  <summary>Details</summary>
Motivation: To address the lack of large-scale multimodal datasets in oral healthcare, which is crucial for advancing AI applications in clinical practice.

Method: The authors fine-tuned state-of-the-art large vision-language models (Qwen-VL 3B and 7B) and evaluated them on classification and diagnostic report generation tasks, comparing them with base models and GPT-4o.

Result: The fine-tuned models achieved significant improvements over baseline models, demonstrating the dataset's utility for AI-driven oro-dental healthcare solutions.

Conclusion: The dataset is publicly available, serving as a valuable resource for advancing AI-driven solutions in oro-dental healthcare.
...
TL;DR: A novel deep learning framework using latent space representations and MAARL improves deepfake watermarking, outperforming existing methods by 4.5-5.3% on benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing deepfake detectors, which struggle with generalization and balancing robustness against benign distortions with sensitivity to malicious tampering.

Method: The method involves a learnable watermark embedder operating in the latent space and the MAARL paradigm, where a watermarking agent interacts with adversarial manipulations to optimize robustness and sensitivity.

Result: The results show significant improvements over state-of-the-art methods, with performance gains of 4.5% on CelebA and 5.3% on CelebA-HQ under challenging scenarios.

Conclusion: The paper concludes that the proposed deep learning framework, utilizing high-dimensional latent space representations and the MAARL paradigm, effectively balances robustness and fragility in watermarking for deepfake detection, outperforming existing methods.
...
TL;DR: CLM optimizes 3DGS for large scenes on a single GPU by smart memory offloading and reducing communication overheads, achieving high-quality rendering.


<details>
  <summary>Details</summary>
Motivation: Scaling 3DGS to large or intricate scenes is challenging due to high memory requirements that exceed GPU capacity. CLM addresses this by enabling efficient rendering on a single GPU.

Method: CLM offloads Gaussians to CPU memory and loads them into GPU memory only when necessary, using a novel offloading strategy that exploits 3DGS's memory access patterns for pipelining and reduces communication volume.

Result: The implementation can render scenes requiring 100 million Gaussians on a single RTX4090, maintaining high-quality output.

Conclusion: CLM successfully enables 3DGS to render large scenes on a single consumer-grade GPU by optimizing memory offloading and reducing communication overheads, achieving state-of-the-art reconstruction quality.
...
TL;DR: PDS introduces a novel method for fMRI-dMRI synthesis, combining a pattern-aware dual-modal 3D diffusion framework and tissue refinement, achieving superior performance and clinical utility.


<details>
  <summary>Details</summary>
Motivation: Missing modalities in MRI (fMRI and dMRI) hinder clinical use, and existing GAN- and diffusion model-based approaches are limited due to signal differences and inadequate integration of disease-related neuroanatomical patterns.

Method: The paper introduces PDS, which includes a pattern-aware dual-modal 3D diffusion framework for cross-modality learning and a tissue refinement network with microstructure refinement to maintain structural fidelity.

Result: PDS achieves PSNR/SSIM scores of 29.83 dB/90.84% for fMRI synthesis and 30.00 dB/77.55% for dMRI synthesis, outperforming baselines. Clinical validation shows diagnostic accuracies of 67.92%/66.02%/64.15% for NC vs. MCI vs. AD.

Conclusion: The proposed PDS method effectively addresses the challenges of fMRI-dMRI synthesis by introducing a pattern-aware dual-modal 3D diffusion framework and a tissue refinement network, achieving state-of-the-art results and strong clinical diagnostic performance.
...
TL;DR: The paper introduces a framework to probe DNNs' geometric understanding, showing optimized shapes can classify, interpret, and deceive models, using Fourier series and winding number-based mapping.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the lack of focus on geometric understanding in DNNs, despite its fundamental role in visual recognition alongside texture.

Method: The method involves an end-to-end differentiable framework combining a Fourier series for shape parameterization, a winding number-based mapping for pixel grid translation, and signal energy constraints for efficient optimization and physical plausibility.

Result: The results demonstrate that optimized shapes can serve as semantic carriers, interpretability tools, and a new adversarial paradigm for deceiving downstream visual tasks.

Conclusion: The paper concludes that the proposed framework offers a versatile tool for exploring the geometric understanding of DNNs, enabling new ways to challenge and interpret machine perception.
...
TL;DR: The paper introduces a synthetic 3D dataset for TDA tasks using the Repulsive Surface algorithm, trains a genus estimator network, and highlights the role of geometric complexity in estimator accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the computational demands of traditional TDA methods like persistent homology and the lack of labeled 3D datasets tailored for supervised learning in TDA tasks. The paper aims to address these challenges by providing a novel dataset generation approach.

Method: The method involves using the Repulsive Surface algorithm to systematically generate labeled 3D datasets with controlled topological invariants. A 3D convolutional transformer architecture is then employed to train a genus estimator network on this dataset.

Result: The results show that the synthetic dataset is suitable for training and benchmarking neural network estimators. However, accuracy decreases as deformations increase, indicating the impact of geometric complexity on estimator performance.

Conclusion: The paper concludes that the introduced synthetic 3D dataset, generated using the Repulsive Surface algorithm, effectively fills the gap in labeled 3D datasets for TDA tasks. It highlights the importance of considering both topological and geometric complexity when training generalized estimators.
...
TL;DR: The paper introduces Triple-S, a benchmark for sticker semantic similarity, and proposes GSE, a model for robust sticker embeddings, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of understanding semantic relationships in stickers due to their diverse and symbolic content, which existing models struggle to capture.

Method: The authors propose the General Sticker Encoder (GSE), a lightweight model that learns robust sticker embeddings using the Triple-S benchmark and additional datasets.

Result: GSE achieves superior performance on unseen stickers and demonstrates strong results on downstream tasks like emotion classification and sticker-to-sticker retrieval.

Conclusion: The paper concludes by releasing the Triple-S benchmark and the General Sticker Encoder (GSE), providing standardized tools and robust embeddings for future research in sticker understanding and multimodal content generation.
...
TL;DR: The paper addresses LVLM's language bias by refining text embeddings with visual features, improving grounding and reducing hallucinations.


<details>
  <summary>Details</summary>
Motivation: The work identifies a bias in LVLM architectures favoring the language modality due to the common practice of appending visual embeddings to text sequences, leading to hallucinations and poor visual grounding.

Method: The proposed method integrates average-pooled visual features to refine textual embeddings, addressing the bias toward the language modality in LVLM architectures.

Result: The approach improves visual grounding and significantly reduces hallucinations on established benchmarks.

Conclusion: The paper concludes that refining textual embeddings with visual information effectively mitigates modality imbalance and reduces hallucinations, though advanced fusion methods could further enhance performance.
...
TL;DR: DRE-SLCL uses dynamic residual encoding and slide-level contrastive learning to efficiently train WSI representation models, overcoming GPU limitations and proving effective in cancer-related tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the difficulty of training end-to-end WSI representation models due to the large number of image tiles in gigapixel slides, which exceeds current GPU capabilities for gradient computation in a single mini-batch.

Method: The method involves using a memory bank to store tile features across WSIs, randomly sampling tiles for feature computation, and employing residual encoding to generate WSI representations. Slide-level contrastive loss is then computed based on these representations and histopathology reports.

Result: Experiments confirmed the effectiveness of DRE-SLCL in cancer subtyping, recognition, and mutation prediction, showcasing its practical utility.

Conclusion: The proposed DRE-SLCL method effectively addresses the challenge of training end-to-end WSI representation models by leveraging dynamic residual encoding and slide-level contrastive learning, as demonstrated by its success in cancer subtyping, recognition, and mutation prediction tasks.
...
TL;DR: Pressure2Motion is a novel algorithm that synthesizes human motion from ground pressure sequences and text prompts, eliminating the need for specialized equipment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable privacy-preserving, low-light, and low-cost motion capture without cameras or wearables, addressing the ill-posed nature of pressure signals to full-body motion.

Method: The method involves a generative model with a dual-level feature extractor for pressure data and a hierarchical diffusion model for motion generation, guided by text prompts.

Result: Experiments show the method generates high-fidelity, physically plausible motions, setting a new state-of-the-art.

Conclusion: Pressure2Motion is a pioneering work combining pressure data and linguistic priors for motion generation, with plans to release codes and benchmarks publicly.
...
TL;DR: NTP-MRISeg simplifies MRIS by reformulating it as an autoregressive next-token prediction task, introducing three novel strategies to improve performance, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Recent MRIS approaches involve complex multimodal fusion or multi-stage decoders, which are cumbersome. The goal is to simplify the model design while improving performance.

Method: The framework reformulates MRIS as an autoregressive next-token prediction task, eliminating complex multimodal fusion and external segmentation models. It introduces three novel strategies: Next-k Token Prediction (NkTP), Token-level Contrastive Learning (TCL), and a memory-based Hard Error Token (HET) optimization strategy.

Result: Extensive experiments on QaTa-COV19 and MosMedData+ datasets show NTP-MRISeg outperforms existing methods.

Conclusion: NTP-MRISeg achieves state-of-the-art performance on MRIS tasks, offering a streamlined and effective alternative to traditional pipelines.
...
TL;DR: PITTA, a novel TTA framework for MDE, uses pose-agnostic adaptation and instance-aware masking to outperform existing methods in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Existing TTA methods for MDE are ineffective in diverse and dynamic environments, prompting the need for a more robust solution.

Method: PITTA incorporates a pose-agnostic TTA paradigm and an instance-aware image masking strategy, along with an edge extraction methodology for input images and depth maps.

Result: Extensive experiments on DrivingStereo and Waymo datasets show PITTA's superior performance over current state-of-the-art methods.

Conclusion: The proposed PITTA framework significantly outperforms existing state-of-the-art techniques in monocular depth estimation (MDE) during test-time adaptation (TTA), demonstrating remarkable performance improvements across diverse and dynamic environments.
...
TL;DR: Role-SynthCLIP improves CLIP model training by using multi-perspective role-playing prompts to generate diverse captions, enhancing semantic diversity and alignment without increasing data volume.


<details>
  <summary>Details</summary>
Motivation: Existing synthetic data methods focus on volume, leading to limited semantic diversity and shallow captions. Role-SynthCLIP addresses this by generating diverse captions from distinct viewpoints.

Method: Role-SynthCLIP uses multi-perspective role-playing prompts (e.g., compositional analyst, interpreter) to guide MLLMs in generating diverse captions, improving semantic diversity and fine-grained alignment.

Result: A CLIP-B/16 model trained on 1M Role-SynthCLIP pairs achieves 64.1% Recall@1 on MS COCO, outperforming a baseline trained on 5M pairs by 2.8 percentage points.

Conclusion: Role-SynthCLIP effectively enhances caption diversity and alignment, improving CLIP model performance with fewer data pairs.
...
TL;DR: SurgiATM is a lightweight, plug-and-play module for surgical smoke removal, combining physics-based and deep learning models to enhance accuracy and generalizability without adding trainable weights.


<details>
  <summary>Details</summary>
Motivation: Surgical smoke during laparoscopic surgery degrades visual quality, risking errors and hindering clinical and computer-assisted analysis. Removing smoke is critical for safety and efficiency.

Method: SurgiATM bridges a physics-based atmospheric model with data-driven deep learning, offering generalizability and accuracy. It integrates seamlessly into existing desmoking architectures with minimal overhead.

Result: Experiments on three datasets show SurgiATM reduces restoration errors and enhances generalizability without additional trainable layers or weights.

Conclusion: SurgiATM is a convenient, low-cost, effective, and generalizable solution for surgical smoke removal, with code publicly available.
...
TL;DR: Adversarial examples are more sensitive to occlusion than clean samples. The study introduces SMCE and SWM-AED, a detection method with high accuracy, avoiding overfitting.


<details>
  <summary>Details</summary>
Motivation: The research aims to understand intrinsic differences between adversarial and clean samples to improve DNN robustness and detection against adversarial attacks.

Method: The study employs Sliding Mask Confidence Entropy (SMCE) to quantify model confidence fluctuation under occlusion, supported by Mask Entropy Field Maps and statistical distributions. It introduces SWM-AED for adversarial example detection.

Result: Results show adversarial examples have significantly higher confidence volatility under occlusion. SWM-AED achieves accuracy over 62% in most cases and up to 96.5% across classifiers and attacks on CIFAR-10.

Conclusion: The study concludes that adversarial examples exhibit higher confidence volatility under occlusion compared to clean samples, leading to the development of SWM-AED, a robust detection method that avoids overfitting and achieves high accuracy.
