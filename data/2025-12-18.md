...
TL;DR: SkyCap is a new optical-SAR dataset for change detection. Optical foundation models, with proper preprocessing, outperform SAR-specific models on SAR amplitude change detection, highlighting the importance of preprocessing alignment.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this research is to address the challenge of maintaining a regular acquisition cadence for linear infrastructure monitoring, as optical VHR imagery is often disrupted by clouds, while SAR data, though all-weather, is difficult to annotate. The study aims to explore the potential of foundation models for SAR amplitude change detection.

Method: The study introduces SkyCap, a bitemporal VHR optical-SAR dataset created by archive matching and co-registration of SkySat (optical) and Capella Space (SAR) scenes. Optical-to-SAR label transfer is used to generate SAR amplitude change detection labels without SAR-expert annotations. The research involves continued pretraining of SARATR-X on SAR data and benchmarking SAR-specific and optical foundation models on SkyCap under various preprocessing choices.

Result: The results show that MTP(ViT-B+RVSA), an optical foundation model with dB+Z-score preprocessing, achieves the best performance (F1$_c$ = 45.06) on SAR amplitude change detection, outperforming SAR-specific foundation models. The study also finds that preprocessing alignment with pretraining statistics is crucial, and the performance ranking of optical models on optical change detection does not directly apply to SAR ACD.

Conclusion: The study concludes that optical foundation models, when properly preprocessed, can outperform SAR-specific foundation models in SAR amplitude change detection tasks. The research highlights the importance of preprocessing alignment with pretraining statistics and introduces SkyCap, a novel dataset for optical-SAR change detection.
...
TL;DR: The paper proposes SocialNav-MoE, an efficient Mixture-of-Experts vision language model for socially compliant navigation, addressing computational overhead and real-time deployment challenges.


<details>
  <summary>Details</summary>
Motivation: Prior work emphasized safety but neglected social compliance in robot navigation. Large VLMs are computationally expensive, limiting real-time use.

Method: Investigates small VLMs, introduces SocialNav-MoE with reinforcement fine-tuning (RFT) and a semantic similarity reward (SSR). Evaluates model types, routing strategies, and vision encoders.

Result: SocialNav-MoE balances navigation accuracy and efficiency, with SSR outperforming other rewards. Validated on the SNEI dataset.

Conclusion: The work advances socially compliant navigation with efficient, deployable solutions, leveraging small VLMs and novel rewards.
...
TL;DR: A hybrid expert-system pipeline for converting printed Chinese Jianpu scores with lyrics into MusicXML and MIDI, achieving high precision without needing large annotated datasets.


<details>
  <summary>Details</summary>
Motivation: Large-scale OMR research has neglected Chinese Jianpu and its lyric resources, leaving them underexplored.

Method: A modular top-down expert-system design combining traditional computer-vision techniques (phrase correlation, skeleton analysis) with unsupervised deep-learning modules for image feature embeddings.

Result: Digitized over 5,000 melody-only songs (>300,000 notes) and 1,400 lyric-aligned songs (>100,000 notes), achieving note-wise F1 = 0.951 and character-wise F1 = 0.931.

Conclusion: The hybrid approach balances interpretability and accuracy, successfully addressing the gap in Jianpu OMR research.
...
TL;DR: AquaDiff is a diffusion-based underwater image enhancement framework that corrects chromatic distortions while preserving structural and perceptual fidelity, outperforming existing methods in color correction and image quality.


<details>
  <summary>Details</summary>
Motivation: Underwater images suffer from severe degradation due to wavelength-dependent light absorption and scattering, leading to color distortion, low contrast, and loss of fine details, which hinder vision-based applications.

Method: AquaDiff combines a chromatic prior-guided color compensation strategy with a conditional diffusion process, utilizing cross-attention for dynamic fusion of degraded inputs and noisy latent states. It features an enhanced denoising backbone with residual dense blocks and multi-resolution attention, along with a cross-domain consistency loss for pixel-level accuracy, perceptual similarity, structural integrity, and frequency-domain fidelity.

Result: Extensive experiments on challenging underwater benchmarks show that AquaDiff outperforms state-of-the-art traditional, CNN-, GAN-, and diffusion-based methods in color correction and overall image quality.

Conclusion: AquaDiff effectively addresses underwater image degradation by integrating a chromatic prior-guided color compensation strategy with a conditional diffusion process, achieving superior color correction and competitive overall image quality.
...
TL;DR: DAVR enhances VLM reliability by integrating Self-Reflection and Cross-Model Verification, achieving top performance in the Reliable VQA Challenge.


<details>
  <summary>Details</summary>
Motivation: VLMs often produce overconfident but incorrect answers, undermining reliability.

Method: DAVR uses a dual-pathway architecture: one assesses response reliability via fused VLM features and QA embeddings, while the other employs external models for factual cross-checking.

Result: DAVR scored 39.64 on Φ₁₀₀ and 97.22 on 100-AUC, securing first place in the challenge.

Conclusion: DAVR effectively mitigates hallucinations and enhances the trustworthiness of VLM responses.
...
TL;DR: HERBench is a new VideoQA benchmark testing multi-evidence integration, revealing current Video-LLMs' limitations in retrieval and fusion of temporal evidence.


<details>
  <summary>Details</summary>
Motivation: Current VideoQA benchmarks under-test reasoning requiring aggregation of multiple, temporally separated visual evidence. HERBench is designed to address this gap by making cross-time evidence unavoidable and quantifiable.

Method: The paper presents HERBench, a benchmark with 26K multiple-choice questions requiring aggregation of at least three non-overlapping evidential cues across video segments. It introduces the Minimum Required Frame-Set (MRFS) to quantify evidential demand.

Result: Evaluation of 13 state-of-the-art Video-LLMs on HERBench shows pervasive failures, with accuracies of 31-42%, barely above the 20% random-guess baseline. The failures are attributed to retrieval and fusion deficits.

Conclusion: HERBench is introduced as a VideoQA benchmark to assess multi-evidence integration across time, revealing significant limitations in current Video-LLMs through measurable metrics like MRFS.
...
TL;DR: A new model for ISLR integrates pose estimation, segmentation, and a ResNet-Transformer to efficiently classify ASL signs, addressing data scarcity, signer variability, and computational costs.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in accessibility for ASL users by addressing the limitations of ISLR, such as scarce per-sign data, high signer variability, and high computational costs.

Method: The approach combines (i) pose estimation for hand and face joint coordinates, (ii) a segmentation module to isolate relevant information, and (iii) a ResNet-Transformer backbone to model spatial and temporal dependencies.

Result: The model reduces computational requirements while maintaining robustness to signer variation, making ISLR more accessible.

Conclusion: The proposed model successfully integrates pose estimation, segmentation, and a ResNet-Transformer backbone to address the limitations of ISLR, offering a computationally efficient solution robust to signer variability.
...
TL;DR: The study introduces a dermatoglyphic text-image co-synthesis pipeline for animal re-identification, combining visual and textual descriptors to improve AI accuracy and explainability in ecological monitoring.


<details>
  <summary>Details</summary>
Motivation: Current AI tools for animal re-identification (Re-ID) are largely image-based and limited to species with distinctive morphological features. The study aims to overcome these limitations by integrating precise dermatoglyphic textual descriptors, a forensic approach new to ecology, to enhance cross-modal identity retrieval.

Method: The study extends Re-ID methodologies by incorporating dermatoglyphic textual descriptors, creating a visual-textual approach. A text-image co-synthesis pipeline generates 'virtual individuals' (life-like visuals paired with dermatoglyphic text) to optimize performance. The method was evaluated on 84,264 manually labelled minutiae across 3,355 images of 185 tigers.

Result: The dermatoglyphic text-image co-synthesis pipeline significantly boosts AI accuracy in cross-modal retrieval and alleviates data scarcity. The method demonstrates novel capabilities for cross-modal identity retrieval, improving Re-ID performance in real-world scenarios.

Conclusion: Dermatoglyphic language-guided biometrics can overcome vision-only limitations in animal re-identification (Re-ID), enabling textual-to-visual identity recovery with human-verifiable matchings. This approach represents a significant advance towards explainability in Re-ID and unifies descriptive modalities in ecological monitoring.
...
TL;DR: Vibe Blending introduces a hierarchical graph manifold (Vibe Space) to generate coherent visual hybrids by connecting distant concepts through shared attributes, outperforming current methods in creativity and coherence.


<details>
  <summary>Details</summary>
Motivation: Creating new visual concepts by connecting distinct ideas through shared attributes (vibes) is challenging for current methods, which struggle with nonlinear paths in latent space.
...
Conclusion: Vibe Space effectively addresses the challenge of generating meaningful visual hybrids by leveraging hierarchical geodesics, demonstrating superior performance in creative quality.
...
TL;DR: The paper introduces PANDA-PLUS-Bench, a benchmark dataset for evaluating AI foundation models in prostate cancer Gleason grading, highlighting variability in model robustness and the impact of tissue-specific training.


<details>
  <summary>Details</summary>
Motivation: To address the limitation of AI models learning specimen-specific artifacts rather than generalizable biological features in prostate cancer grading, which affects clinical utility.

Method: A curated benchmark dataset (PANDA-PLUS-Bench) with nine expert-annotated prostate biopsy slides, evaluated across eight augmentation conditions and two resolutions, testing seven foundation models.

Result: Substantial variation in model robustness: Virchow2 had low cross-slide accuracy (47.2%), while HistoEncoder (tissue-specific) achieved the highest cross-slide accuracy (59.7%) and slide-level encoding (90.3%). All models showed gaps between within-slide and cross-slide performance.

Conclusion: PANDA-PLUS-Bench provides a standardized resource to evaluate foundation models' robustness in Gleason grading, emphasizing the importance of tissue-specific training for clinical relevance.
...
TL;DR: Proposes adaptive post-processing techniques to refine glioma segmentations from large-scale pretrained models, improving accuracy and addressing systematic errors.


<details>
  <summary>Details</summary>
Motivation: Addresses poor generalization and systematic errors (e.g., false positives, label swaps) in deep learning models for glioma segmentation, along with GPU resource inequality and environmental costs.

Method: Adaptive post-processing techniques applied to outputs of large-scale pretrained models, evaluated in BraTS 2025 segmentation challenges.

Result: Improved ranking metric by 14.9% for sub-Saharan Africa challenge and 0.9% for adult glioma challenge.

Conclusion: Advocates for a shift from complex model architectures to efficient, clinically aligned post-processing strategies that are precise, computationally fair, and sustainable.
...
TL;DR: TalkVerse is a large-scale, open corpus for audio-driven talking video generation, offering 2.3 million high-resolution clips. It includes a reproducible 5B DiT baseline model with efficient inference and supports zero-shot video dubbing.


<details>
  <summary>Details</summary>
Motivation: To enable fair, reproducible comparison across methods in audio-driven talking video generation by providing a large, open dataset and a baseline model.

Method: Utilizes a video VAE with high downsampling ratio and sliding window mechanism for minute-long generation with low drift. Integrates an MLLM director for storytelling enhancement.

Result: Achieves comparable lip-sync and visual quality to a 14B model with 10x lower inference cost. Supports zero-shot video dubbing via controlled latent noise injection.

Conclusion: TalkVerse lowers barriers for research in audio-driven human video generation by open-sourcing the dataset, training recipes, and checkpoints.
...
TL;DR: PC-GRPO is a supervision-free reinforcement learning method for Vision Language Models (VLMs) that improves reasoning quality and training stability using self-supervised puzzle environments and a difficulty-aware curriculum.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations in current RL approaches like GRPO, such as reliance on costly annotations, flat/sparse rewards, and logical inconsistencies in reasoning chains.

Method: Introduces Puzzle Curriculum GRPO (PC-GRPO) with self-supervised puzzle environments (PatchFit, Rotation, Jigsaw) and a dynamic difficulty-aware curriculum.

Result: PC-GRPO enhances reasoning quality, training stability, and downstream accuracy across benchmarks and model backbones (Qwen-7B, Qwen-3B).

Conclusion: PC-GRPO provides a scalable, verifiable, and interpretable RL post-training solution for VLMs, mitigating key issues in existing methods.
...
TL;DR: A Trimodal person identification framework robust to missing modalities achieves high accuracy using multi-task learning and dynamic fusion strategies.


<details>
  <summary>Details</summary>
Motivation: Real-world person recognition systems often face challenges due to missing or degraded modalities, necessitating a robust solution that can adapt to such conditions.

Method: The approach integrates voice, face, and gesture modalities using multi-task learning, cross-attention, gated fusion mechanisms, and a confidence-weighted fusion strategy to handle missing or low-quality data.

Result: The system achieves 99.18% Top-1 accuracy on the CANDOR dataset and 99.92% accuracy on the VoxCeleb1 dataset in Bimodal mode, outperforming conventional methods.

Conclusion: The proposed Trimodal person identification framework demonstrates robustness to missing or degraded modalities, achieving high accuracy in both Trimodal and Bimodal scenarios. The system's adaptability and performance make it suitable for real-world applications.
...
TL;DR: A new post-hoc image watermarking method using localized embedding and wavelet transforms offers interpretable detection maps, robust performance against transformations, and high imperceptibility.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this research is the lack of transparency in existing image watermarking schemes, which operate as black boxes and provide no insight into how or where the watermark is present. This impacts user trust and makes it difficult to interpret the effects of tampering.

Method: The method involves embedding watermark signals in the discrete wavelet transform domain using a statistical block-wise strategy. This generates detection maps that reveal which regions of an image are likely watermarked or altered.

Result: The results show that the method is robust against common image transformations (e.g., cropping up to half the image) and remains sensitive to semantic manipulations. The watermark is also highly imperceptible.

Conclusion: The paper concludes that their post-hoc image watermarking method, which combines localized embedding with region-level interpretability, achieves strong robustness against common image transformations while remaining sensitive to semantic manipulations. The watermark is highly imperceptible and offers more interpretable detection compared to prior methods.
...
TL;DR: A pose-based framework for classifying livestock social interactions outperforms proximity-based methods by leveraging anatomical keypoints and motion signatures, achieving 77.51% accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of existing approaches that use static proximity thresholds, which cannot differentiate between affiliative and agonistic behaviors in complex barn environments, thereby improving the interpretability of automated social network analysis.

Method: The method involves a computer vision pipeline integrating YOLOv11 for object detection, supervised individual identification, ByteTrack for multi-object tracking, ZebraPose for 27-point anatomical keypoint estimation, and a support vector machine classifier trained on pose-derived distance dynamics.

Result: The framework achieved 77.51% accuracy in distinguishing affiliative and agonistic behaviors using pose information alone, showing substantial gains over proximity-only baselines.

Conclusion: The paper concludes that the proposed pose-based computational framework significantly improves the ability to distinguish between affiliative and agonistic behaviors in livestock, offering a more nuanced and accurate approach to automated social network analysis in precision livestock farming.
...
TL;DR: The paper introduces a protocol to evaluate question-generation models by simulating expert communication, using a novel dataset (EgoExoAsk) to measure question quality in eliciting expert knowledge.


<details>
  <summary>Details</summary>
Motivation: To assess what makes questions effective in extracting information from experts, focusing on question quality rather than just answerability.

Method: Proposes a protocol using question-to-answer retrieval, leveraging the EgoExoAsk dataset (27,666 QA pairs) to train and evaluate models.

Result: The protocol aligns with question-generation settings, showing models with richer context perform better.

Conclusion: The proposed method effectively evaluates question quality for eliciting expert knowledge, supported by experimental results.
...
TL;DR: MAPO is a model-agnostic framework for medical image segmentation that improves boundary adherence and optimization stability without direct ground-truth supervision.


<details>
  <summary>Details</summary>
Motivation: Preference optimization offers a scalable supervision paradigm based on relative preference signals, yet prior attempts in medical image segmentation remain model-specific and rely on low-diversity prediction sampling.

Method: MAPO (Model-Agnostic Preference Optimization), a training framework that utilizes Dropout-driven stochastic segmentation hypotheses to construct preference-consistent gradients without direct ground-truth supervision.

Result: Comprehensive evaluations across diverse medical datasets reveal that MAPO consistently enhances boundary adherence, reduces overfitting, and yields more stable optimization dynamics.

Conclusion: MAPO consistently enhances boundary adherence, reduces overfitting, and yields more stable optimization dynamics compared to conventional supervised training.
...
TL;DR: MVGSR improves 3D Gaussian Splatting Super-Resolution by integrating multi-view information with an Auxiliary View Selection Method and an epipolar-constrained attention mechanism, achieving top performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing 3DGS SR methods, which either lack cross-view consistency (single-image SR networks) or require sequential frames (video-based SR). MVGSR aims to enable high-resolution rendering from low-resolution inputs for unstructured multi-view datasets.

Method: The paper proposes MVGSR, a framework combining an Auxiliary View Selection Method based on camera poses and an epipolar-constrained multi-view attention mechanism. This approach selectively aggregates consistent information from auxiliary views, improving geometric consistency and detail fidelity.

Result: Extensive experiments show that MVGSR achieves state-of-the-art performance on both object-centric and scene-level 3DGS SR benchmarks, demonstrating superior consistency and detail fidelity.

Conclusion: MVGSR introduces a novel framework for 3D Gaussian Splatting Super-Resolution, addressing limitations of previous methods by integrating multi-view information. It achieves state-of-the-art performance by incorporating an Auxiliary View Selection Method and an epipolar-constrained multi-view attention mechanism, enhancing consistency and detail fidelity in 3DGS representations.
...
TL;DR: A novel method using event cameras and LED markers accurately measures high-frequency deformations, overcoming limitations of traditional high-speed cameras.


<details>
  <summary>Details</summary>
Motivation: Traditional high-speed camera-based measurement methods are limited by harsh lighting conditions and high equipment costs, necessitating an alternative approach.

Method: The method filters observation noise based on event stream characteristics and spatiotemporal correlation, differentiates motion-induced events from LED blinking events, and extracts high-speed moving LED markers to measure deformations.

Result: Experimental results confirm the accuracy of the method in measuring high-frequency planar deformations.

Conclusion: The proposed method using an event camera and LED markers effectively measures high-frequency planar deformations, confirmed by experimental results.
...
TL;DR: The paper proposes a memory bank-based wavelet filtering and fusion network (MWNet) for high-fidelity lesion area and target organ segmentation in ultrasound videos, addressing challenges like low contrast, noisy backgrounds, and small object losses.


<details>
  <summary>Details</summary>
Motivation: Medical ultrasound videos are crucial for inspections, diagnosis, and surgical planning, but face issues like missegmentation due to low contrast and noisy backgrounds. Object tracking in long videos is also challenging.

Method: The method uses an encoder-decoder structure with memory-based wavelet convolution, cascaded wavelet compression, and a long short-term memory bank with cross-attention. It also includes an HF-aware feature fusion module for boundary-sensitive details.

Result: Benchmark tests on four ultrasound datasets show marked improvements in segmentation metrics, especially for small thyroid nodules.

Conclusion: MWNet effectively addresses segmentation challenges in ultrasound videos, demonstrating superior performance for small objects and long videos.
...
TL;DR: PMMD is a diffusion framework for generating photorealistic human images with controllable pose and appearance, addressing issues like occlusions and garment style drift.


<details>
  <summary>Details</summary>
Motivation: Current methods struggle with occlusions, garment style drift, and pose misalignment, limiting their effectiveness in applications like virtual try-on and digital human creation.

Method: Pose-guided Multi-view Multimodal Diffusion (PMMD) uses a multimodal encoder to model visual views, pose features, and text prompts, reducing cross-modal discrepancy. It includes a ResCVA module for detail enhancement and a cross-modal fusion module for integrating image semantics with text.

Result: Experiments on the DeepFashion MultiModal dataset show PMMD outperforms baselines in consistency, detail preservation, and controllability.

Conclusion: PMMD effectively synthesizes high-quality, controllable human images, advancing applications in virtual try-on and digital human creation.
...
TL;DR: Uni-Parser is a scalable, modular document parsing engine for scientific literature and patents, achieving high throughput and efficiency for diverse applications.


<details>
  <summary>Details</summary>
Motivation: The paper introduces Uni-Parser to address the need for a high-throughput, accurate, and cost-efficient document parsing engine tailored for scientific literature and patents.

Method: Uni-Parser employs a modular, loosely coupled multi-expert architecture that preserves fine-grained cross-modal alignments across text, equations, tables, figures, and chemical structures. It incorporates adaptive GPU load balancing, distributed inference, dynamic module orchestration, and configurable modes.

Result: The system achieves a processing rate of up to 20 PDF pages per second on 8 x NVIDIA RTX 4090D GPUs, enabling cost-efficient inference across billions of pages.

Conclusion: Uni-Parser's scalability facilitates a broad spectrum of downstream applications, from literature retrieval to training next-generation AI models.
...
TL;DR: Nano Banana Pro shows promise as a zero-shot solver for low-level vision tasks, excelling in subjective quality but struggling with pixel-level consistency required by conventional metrics.


<details>
  <summary>Details</summary>
Motivation: To explore whether Nano Banana Pro, a commercial text-to-image generation model, can serve as a generalist solver for traditional low-level vision challenges, given its underexplored potential in this domain.

Method: The study conducted a comprehensive zero-shot evaluation across 14 distinct low-level tasks using 40 diverse datasets, benchmarking Nano Banana Pro against state-of-the-art specialist models with simple textual prompts.

Result: Nano Banana Pro demonstrates superior subjective visual quality, often hallucinating plausible high-frequency details, but lags behind in traditional reference-based quantitative metrics due to its stochastic nature.

Conclusion: Nano Banana Pro is identified as a capable zero-shot contender for low-level vision tasks, though it falls short of matching the high fidelity of domain-specific models due to its inherent stochasticity.
...
TL;DR: A lightweight 3D animation framework decouples geometric control from appearance synthesis, enabling efficient, high-fidelity animation with interactive control, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the trade-off between rendering quality and 3D control in single-image 3D animation generation, addressing the limitations of traditional labor-intensive pipelines and recent AIGC-based approaches.

Method: The method uses a 2D-3D aligned proxy representation, leveraging a coarse 3D estimate for structural control while relying on learned image-space generative priors for high-fidelity appearance and view synthesis.

Result: The framework achieves efficient animation generation on low-power platforms, outperforming video-based methods in identity preservation, consistency, and interactive control.

Conclusion: The proposed lightweight 3D animation framework successfully decouples geometric control from appearance synthesis, offering efficient animation generation with high-fidelity results and interactive control, outperforming existing video-based methods.
...
TL;DR: GENIE is a Generalizable Instance Editing framework that addresses semantic entanglement in reference-guided instance editing by disentangling intrinsic and extrinsic attributes. It uses Spatial Alignment Module (SAM), Adaptive Residual Scaling Module (ARSM), and Progressive Attention Fusion (PAF) to achieve high fidelity and robustness.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitation of semantic entanglement in reference-guided instance editing, where intrinsic and extrinsic attributes are intertwined, making it challenging to borrow and apply information appropriately.

Method: GENIE employs SAM for spatial alignment, ARSM to learn what to borrow by amplifying intrinsic cues and suppressing extrinsic attributes, and PAF to learn how to render the appearance onto the target while preserving its structure.

Result: Extensive experiments on the AnyInsertion dataset show that GENIE achieves state-of-the-art fidelity and robustness.

Conclusion: GENIE sets a new standard for disentanglement-based instance editing by effectively addressing semantic entanglement through its modular approach.
...
TL;DR: The paper introduces a new Human Action Form Assessment (AFA) task, the CoT-AFA dataset with multi-level annotations, and the Explainable Fitness Assessor framework for detailed feedback. Results show significant improvements in explanation generation, action classification, and quality assessment.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of current video understanding methods, which focus on identifying actions (what and where) but fail to assess action standardization or provide explainable feedback. Existing datasets also lack labels for action standardization and detailed feedback, necessitating a new task and dataset.

Method: The paper introduces the Explainable Fitness Assessor framework, which employs two parallel processing streams (visual and semantic) and a dynamic gating mechanism to fuse information for comprehensive action analysis. It also introduces the CoT-AFA dataset, enriched with a Chain-of-Thought explanation paradigm for detailed feedback.

Result: The method achieves improvements in explanation generation (e.g., +16.0% in CIDEr), action classification (+2.7% in accuracy), and quality assessment (+2.1% in accuracy), showcasing the potential of the CoT-AFA dataset and the Explainable Fitness Assessor framework.

Conclusion: The paper concludes by highlighting the success of the proposed Explainable Fitness Assessor framework and the CoT-AFA dataset in addressing the Human Action Form Assessment (AFA) task. The framework's ability to provide detailed feedback and explanations, along with the dataset's multi-level annotations, demonstrates significant improvements in action classification, quality assessment, and explanation generation. The availability of the dataset and source code is noted for future research.
...
TL;DR: EagleVision introduces a dual-stage framework for spatial cognition, combining macro perception (keyframe selection) and micro verification (pose querying), achieving top performance in spatial understanding.


<details>
  <summary>Details</summary>
Motivation: To address challenges in spatial Chain-of-Thought (CoT) such as weak spatial consistency, limited viewpoint diversity, and untraceable evidence chains in existing approaches.

Method: EagleVision employs a dual-stage framework: macro perception (using SPF-DPP to select keyframes) and micro verification (formalizing spatial CoT as BEV-grounded pose querying with reinforcement learning).

Result: EagleVision achieves state-of-the-art performance on VSI-Bench among open-source vision-language models.

Conclusion: EagleVision demonstrates strong and generalizable spatial understanding, achieving state-of-the-art performance among open-source vision-language models.
...
TL;DR: CMUS-Net, a cross-modal ultra-scale learning network, bridges the scale gap between TEM and OM/IM images for multi-glomerular disease classification, achieving high accuracy and outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The substantial scale difference between TEM (nanoscale) and OM/IM (microscale) images challenges existing multi-modal models in achieving effective feature fusion and improving classification accuracy.

Method: CMUS-Net introduces a sparse multi-instance learning module to aggregate TEM image features and a cross-modal scale attention module for feature interaction. Multiple loss functions are combined to weigh the importance of different modalities.

Result: On an in-house dataset, CMUS-Net achieves an ACC of 95.37+/-2.41%, an AUC of 99.05+/-0.53%, and an F1-score of 95.32+/-2.41%, outperforming other methods.

Conclusion: The proposed CMUS-Net effectively bridges the scale difference between nanometer and micrometer images, achieving high accuracy in the classification of multiple glomerular diseases. It outperforms existing multi-modal and multi-scale methods and demonstrates generalization capability in staging MN.
...
TL;DR: Analysis of criticality metrics for safety evaluation in object detection systems, proposing novel strategies for improved accuracy.


<details>
  <summary>Details</summary>
Motivation: Ensuring safety in automated driving requires accurate perception and evaluation of object detection systems.

Method: Comprehensive review of existing metrics, empirical validation using the DeepAccident dataset, and proposal of bidirectional criticality rating and multi-metric aggregation.

Result: Up to 100% improvement in criticality classification accuracy.

Conclusion: The proposed strategies significantly advance the safety evaluation of object detection systems in automated vehicles.
