...
TL;DR: Gaussian See, Gaussian Do enables rig-free 3D motion transfer between objects using motion embeddings and 3D Gaussian Splatting with anchor-based view-aware mechanisms.


<details>
  <summary>Details</summary>
Motivation: To achieve semantic 3D motion transfer from multiview video without requiring rigs, enabling cross-category motion transfer with meaningful correspondence.

Method: Extracts motion embeddings via condition inversion from source videos, applies to static target shapes, supervises dynamic 3D Gaussian Splatting reconstruction with anchor-based view-aware embedding for consistency.

Result: Establishes first benchmark for semantic 3D motion transfer, shows superior motion fidelity and structural consistency compared to adapted baselines.

Conclusion: The approach successfully enables cross-category 3D motion transfer with improved fidelity and consistency, with code and data publicly available.
...
TL;DR: This study benchmarks 12 deep learning architectures for dental caries segmentation in panoramic radiographs, finding that CNN-based DoubleU-Net outperforms transformer and Mamba models despite their theoretical advantages.


<details>
  <summary>Details</summary>
Motivation: Automated dental caries segmentation is challenging due to low lesion contrast, morphological variability, and limited annotated data, requiring comprehensive evaluation of modern architectures.

Method: Twelve state-of-the-art architectures (including VMUnet, MambaUNet, DoubleU-Net) were trained identically on the DC1000 dataset for dental caries segmentation in panoramic radiographs.

Result: CNN-based DoubleU-Net achieved the highest performance (Dice: 0.7345, mIoU: 0.5978, Precision: 0.8145), with top 3 results across all metrics from CNN architectures, while transformer/Mamba models underperformed due to data limitations.

Conclusion: Architecture-task alignment is more critical than model complexity for domain-specific medical image segmentation, as CNNs demonstrated superior performance over theoretically advanced architectures in this data-limited scenario.
...
TL;DR: Novel CAD geometric representation using volumetric distance functions (BR-DF) enables 100% successful conversion to watertight B-Rep models via extended Marching Cubes, with multi-branch diffusion generation matching SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Need robust geometric representation for CAD B-Rep that guarantees successful conversion to watertight models, overcoming failures in existing methods.

Method: BR-DF encodes surface mesh as SDF and B-Rep elements as per-face UDFs. Extended Marching Cubes converts BR-DF to faceted B-Rep. Multi-branch latent diffusion with 3D U-Net generates SDF and UDFs.

Result: Achieves comparable CAD generation performance to SOTA methods with unprecedented 100% success rate in producing watertight B-Rep models.

Conclusion: BR-DF provides a reliable volumetric representation for CAD, ensuring failure-proof conversion and effective generation, advancing robust CAD model creation.
...
TL;DR: GeoSceneGraph introduces a method for generating 3D indoor scenes from text prompts by leveraging scene graph structure and geometric symmetries without predefined relationship classes, achieving performance comparable to methods using ground-truth annotations.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-3D scene methods either require resource-intensive VLMs (unsuitable for constrained devices) or overlook scene graph structures, limiting coherence. Methods using scene graphs need inconvenient user-provided graphs or ground-truth relationships, restricting object interaction variety.

Method: Uses equivariant graph neural networks (EGNNs) enhanced with a novel text conditioning strategy to handle complex text modalities, synthesizing scenes based on geometric symmetries and inferred graph structures without predefined relationship classes.

Result: Achieves performance comparable to methods using ground-truth relationships, validated through ablation studies showing the effectiveness of the text conditioning approach for EGNNs.

Conclusion: GeoSceneGraph provides a practical solution for generating coherent 3D scenes from text on resource-constrained devices, balancing performance and flexibility by avoiding dependency on predefined annotations.
...
TL;DR: Unsupervised bidirectional MRI synthesis between ultra-low field (ULF) and high-field (HF) images using physics-inspired forward modeling and implicit neural representation.


<details>
  <summary>Details</summary>
Motivation: Existing MRI synthesis models lack physics-driven approaches for contrast adaptation between different field strengths.

Method: Forward model simulates HF-to-ULF transformation using tissue-type SNR estimation; INR network handles super-resolution for HF synthesis without training data.

Result: 52% WM-GM contrast improvement in synthetic ULF images, 37% in real 64mT images; model shows robustness to contrast/noise variations.

Conclusion: Physics-based approach enables effective bidirectional MRI synthesis with significant contrast improvements and robustness.
...
TL;DR: I-Mix2Mix is a novel framework for multi-view image editing that uses a modified Score Distillation Sampling approach with a multi-view diffusion student to achieve consistent edits across different viewpoints by leveraging a pretrained model's 3D prior.


<details>
  <summary>Details</summary>
Motivation: Existing methods for multi-view image editing from sparse input views often produce artifacts and inconsistent results when modifying scenes based on text instructions, creating a need for a more robust solution.

Method: The proposed method, InstructMix2Mix (I-Mix2Mix), distills a 2D diffusion model's editing ability into a pretrained multi-view diffusion model. Key innovations include replacing the neural field consolidator in Score Distillation Sampling with a multi-view diffusion student, featuring incremental student updates, a specialized teacher noise scheduler, and an attention modification to enhance cross-view coherence.

Result: Experiments show that I-Mix2Mix significantly improves multi-view consistency in edited scenes while maintaining high quality in individual frames, outperforming existing approaches.

Conclusion: I-Mix2Mix effectively addresses the challenge of coherent multi-view editing by integrating 2D diffusion capabilities with a multi-view model's 3D prior, offering a practical solution for sparse input view scenarios.
...
TL;DR: SkinR1 is a dermatological vision-language model that uses textbook-based reasoning and reinforcement learning to improve diagnostic accuracy and generalization.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models for dermatology face limitations in data heterogeneity, absence of diagnostic rationales, and poor scalability.

Method: SkinR1 employs a textbook-based reasoning generator, supervised fine-tuning, and a novel reinforcement learning paradigm with disease hierarchy.

Result: Extensive experiments show superior diagnostic accuracy across multiple dermatology datasets.

Conclusion: SkinR1 effectively addresses key limitations in dermatological VLMs through integrated reasoning and RL-driven generalization.
...
TL;DR: Paper introduces FarSLIP, a fine-grained CLIP variant for remote sensing that addresses limitations in spatial awareness through patch-to-patch distillation and multi-granular dataset MGRS-200k, achieving SOTA in segmentation and image-level tasks.


<details>
  <summary>Details</summary>
Motivation: Current RS-specific CLIP variants inherit limited spatial awareness from CLIP's global alignment, failing to capture fine-grained details due to underutilized object-level supervision and performance degradation from direct application of region-text alignment methods.

Method: Constructs MGRS-200k dataset with multi-granularity supervision; proposes FarSLIP framework using patch-to-patch distillation for local-global alignment and CLS token-based region-category alignment instead of explicit patch-level methods to preserve semantic coherence.

Result: FarSLIP achieves state-of-the-art performance on RS open-vocabulary semantic segmentation, zero-shot classification, and image-text retrieval, demonstrating improved fine-grained vision-language alignment.

Conclusion: FarSLIP effectively enhances spatial awareness in RS CLIP models through novel alignment strategies and dataset, setting new benchmarks while maintaining semantic coherence.
...
TL;DR: nnMIL is a multiple-instance learning framework that enhances pathology AI by aggregating patch-level features from foundation models into robust slide-level predictions through random sampling and ensemble methods.


<details>
  <summary>Details</summary>
Motivation: Current methods for aggregating patch-level features from pathology foundation models into slide-level predictions have design limitations affecting generalizability and reliability in computational pathology.

Method: nnMIL uses random sampling at patch and feature levels for large-batch optimization, task-aware sampling, and scalable training, with a lightweight aggregator for sliding-window inference and uncertainty estimation.

Result: Outperformed existing MIL methods across 40,000 WSIs, 35 clinical tasks, and 4 foundation models in disease diagnosis, subtyping, biomarker detection, and prognosis, with strong generalization and reliability.

Conclusion: nnMIL provides a practical, generalizable solution for translating pathology foundation models into clinically meaningful predictions, advancing reliable AI deployment in real-world settings.
...
TL;DR: X-WIN is a chest X-ray world model that learns 3D anatomical knowledge from CT scans to improve 2D CXR analysis through projection prediction and contrastive alignment.


<details>
  <summary>Details</summary>
Motivation: Chest X-rays (CXRs) have limitations due to 2D projection causing structural superposition, making 3D anatomy representation and disease diagnosis challenging.

Method: Proposes X-WIN model that distills volumetric knowledge from CT by predicting 2D projections in latent space, using affinity-guided contrastive alignment, masked image modeling with real CXRs, and a domain classifier for representation similarity.

Result: X-WIN outperforms existing foundation models in downstream tasks with linear probing and few-shot fine-tuning, and can render 2D projections for 3D CT reconstruction.

Conclusion: X-WIN effectively bridges 2D and 3D medical imaging, enhancing CXR analysis by incorporating 3D structural knowledge from CT, showing strong performance and reconstruction capability.
...
TL;DR: CPSL is a compact 2.5D video representation using depth and saliency to decompose frames into layers, enabling efficient novel-view synthesis with low storage and rendering costs.


<details>
  <summary>Details</summary>
Motivation: Traditional volumetric video representations are computationally expensive, limiting scalability for on-demand video and real-time communication.

Method: Decomposes frames into geometry-consistent layers with soft alpha bands and edge-depth cache, using depth-weighted warping and alpha compositing for view synthesis, with motion-guided propagation for temporal coherence.

Result: Achieves superior perceptual quality and boundary fidelity compared to baselines, while reducing storage and rendering costs by several times.

Conclusion: CPSL offers a practical solution to bring volumetric benefits to 2D content, enabling scalable immersive media.
...
TL;DR: A new benchmark for analyzing long-term periodic human workflows with low-contrast patterns, introducing three evaluation tasks and a training-free baseline method.


<details>
  <summary>Details</summary>
Motivation: Long-term periodic workflows with subtle patterns are common in real-world scenarios but remain understudied compared to simpler short-term activities. Current methods struggle with these complex, low-contrast sequences.

Method: Collection of 580 multimodal human activity sequences showing long-term periodic workflows. A lightweight, training-free baseline is designed to model diverse periodic patterns using predefined logic instead of learning from annotations.

Result: The benchmark challenges existing unsupervised and LLM-based methods. The proposed baseline outperforms competitors across all tasks (unsupervised detection, task tracking, anomaly detection) and shows deployment advantages similar to supervised approaches without needing annotations.

Conclusion: This work establishes the first comprehensive benchmark for long-term periodic workflows, demonstrating that a simple training-free method can effectively address complex patterns and perform comparably to supervised methods in real applications.
...
TL;DR: A low-cost, general-purpose method for millisecond-level temporal synchronization of multi-view video streams using a custom LED Clock device, applicable to heterogeneous camera systems including RGB and IR modalities.


<details>
  <summary>Details</summary>
Motivation: Synchronizing multiple cameras is challenging in heterogeneous setups without hardware sync capabilities, especially in real-world environments where controlled conditions aren't feasible.

Method: Uses a custom LED Clock with red and infrared LEDs to encode time, allowing visual decoding of exposure windows from recorded frames for precise synchronization.

Result: Achieves 1.34ms RMSE residual error compared to hardware sync, outperforms light-, audio-, and timecode-based methods, and improves downstream tasks like pose estimation and 3D reconstruction.

Conclusion: The method simplifies synchronization pipelines and enables advanced vision-based sensing in unconstrained environments like industrial and clinical applications.
...
TL;DR: Deep learning enables adaptive control of CO2 laser suction pumps, cutting energy use by 20-50% via material classification and smoke detection.


<details>
  <summary>Details</summary>
Motivation: Current laser cutting systems lack adaptive control, operating suction pumps in open-loop mode leading to unnecessary energy consumption and environmental impact.

Method: Closed-loop system using material classification (via lens-less speckle sensing with custom CNN or USB camera with VGG16 transfer learning) and separate DL model for smoke level detection to dynamically adjust pump power.

Result: Experimental results show 20% to 50% reduction in smoke suction pump energy consumption.

Conclusion: The integrated DL approach significantly improves energy efficiency in laser cutting, contributing to sustainable manufacturing.
...
TL;DR: Proposes Edge-Guided Spatial Attention (EGSA) fusion mechanism and multi-modal progressive training to improve transparent object perception, showing better depth accuracy than MODEST while maintaining segmentation performance.


<details>
  <summary>Details</summary>
Motivation: Transparent objects challenge computer vision as they confuse depth estimation and semantic segmentation; multi-task learning often suffers from negative cross-task interactions that limit performance.

Method: Introduces EGSA to fuse semantic and geometric features using boundary information, and a training strategy that transitions from RGB-derived edges to depth-predicted edges without needing ground-truth depth.

Result: EGSA improved depth accuracy on Syn-TODD and ClearPose benchmarks over MODEST, with largest gains in transparent regions, while keeping segmentation competitive.

Conclusion: Edge-guided fusion is a robust approach for enhancing transparent object perception, effectively mitigating destructive task interactions and leveraging geometric learning progressively.
...
TL;DR: Proposes a feature-based knowledge distillation method using only feature losses without logit-based losses, introducing a knowledge quality metric for selecting effective teacher layers, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Traditional KD methods use both logit-based and feature-based losses, but this work explores distillation using exclusively feature-based losses to simplify and potentially improve efficiency.

Method: Uses feature-based losses only, introduces a knowledge quality metric based on latent representation geometry to identify the most effective teacher layers for distillation.

Result: Achieves state-of-the-art performance on three image classification datasets with four student-teacher pairs, delivering up to 15% top-1 accuracy improvement over standard approaches.

Conclusion: Demonstrates that feature-only distillation with a knowledge quality metric can achieve superior performance, with publicly shared code for future research.
...
TL;DR: Kandinsky 5.0 is a state-of-the-art family of foundation models for high-resolution image and 10-second video synthesis, consisting of three model variants with varying sizes and capabilities, trained through a multi-stage pipeline with advanced optimization techniques.


<details>
  <summary>Details</summary>
Motivation: To advance high-quality generative AI by providing scalable, publicly available models for image and video synthesis that leverage comprehensive data curation and cutting-edge training methods to achieve superior performance and speed.

Method: Uses a multi-stage training pipeline with extensive pre-training, self-supervised fine-tuning (SFT), and reinforcement learning (RL)-based post-training. Includes novel architectural, training, and inference optimizations for efficiency.

Result: Achieves high generation speeds and state-of-the-art performance in human evaluations, demonstrated across various generative tasks with models ranging from 2B to 19B parameters.

Conclusion: Kandinsky 5.0 significantly advances generative model development and accessibility through open-source release, enabling broad research and application adaption.
...
TL;DR: FinCriticalED is a visual benchmark for evaluating OCR/VLM models on financial documents at the fact level, focusing on detecting critical errors in numerical/temporal data.


<details>
  <summary>Details</summary>
Motivation: Financial documents have dense layouts where small OCR errors (e.g., sign inversions) can cause major misinterpretations, but current metrics like ROUGE don't address factual correctness.

Method: Provides 500 image-HTML pairs with expert-annotated financial facts, uses an LLM-as-Judge pipeline for structured fact extraction and verification.

Result: Benchmarks show proprietary models have highest accuracy but still make substantial errors in complex visual contexts.

Conclusion: FinCriticalED establishes a rigorous foundation for improving factual precision in financial and other precision-critical domains.
...
TL;DR: This paper introduces CKDA, a method for Visible-Infrared Lifelong Re-ID that disentangles modality-common and modality-specific knowledge to prevent collaborative forgetting during sequential training.


<details>
  <summary>Details</summary>
Motivation: Existing lifelong Re-ID methods suffer from collaborative forgetting due to the mutual interference between acquiring new modality-specific knowledge and preserving old modality-common knowledge.

Method: CKDA uses Modality-Common Prompting (MCP) and Modality-Specific Prompting (MSP) modules to disentangle knowledge, and a Cross-modal Knowledge Alignment (CKA) module to align new and old knowledge using dual-modality prototypes.

Result: Extensive experiments on four benchmark datasets demonstrate that CKDA outperforms state-of-the-art methods in Visible-Infrared Lifelong Re-ID.

Conclusion: The proposed CKDA effectively addresses collaborative forgetting by explicitly balancing and aligning modality-common and modality-specific knowledge, achieving superior performance in lifelong person re-identification across visible and infrared modalities.
...
TL;DR: A new hologram representation using structured complex-valued 2D Gaussian primitives significantly reduces parameter space, improves VRAM efficiency and optimization speed, and achieves higher-fidelity reconstructions with effective noise suppression.


<details>
  <summary>Details</summary>
Motivation: To reduce the parameter search space and computational demands of traditional per-pixel hologram representations for more scalable computer-generated holography systems.

Method: Proposes a hologram representation based on structured complex-valued 2D Gaussian primitives, a differentiable rasterizer for end-to-end training, and a GPU-optimized light propagation kernel.

Result: Achieves up to 2.5x lower VRAM usage, 50% faster optimization, higher-fidelity reconstructions, and effective noise suppression in converted phase-only holograms.

Conclusion: The representation enables more scalable hologram estimation in next-generation systems by efficiently reducing computational requirements while maintaining high quality.
...
TL;DR: Computer vision models like ResNet-50 show developmental alignment with children's mathematical thinking progression, matching human-like learning patterns in geometry and number concepts during training.


<details>
  <summary>Details</summary>
Motivation: Prior research showed CV models learn latent representations of math concepts similar to adults; this study investigates if their training progress mimics children's developmental trajectories.

Method: Case study of ResNet-50 model analyzing performance improvements during training compared to developmental progressions in children for geometry/topology and number concepts.

Result: Developmental alignment found for some geometry classes (Euclidean Geometry, Geometrical Figures, etc.) and number line representation, but not others (Chiral Figures, Symmetrical Figures).

Conclusion: CV models show promise for understanding human mathematical development, suggesting future research on different architectures and larger benchmarks.
...
TL;DR: UniHOI is a unified model that jointly handles HOI detection and generation using a shared token space, achieving SOTA results with significant improvements in accuracy and generation metrics.


<details>
  <summary>Details</summary>
Motivation: Traditional approaches treat HOI detection and generation as separate tasks, which limits comprehensive interaction understanding and knowledge sharing between the two.

Method: Proposes a symmetric interaction-aware attention module and a unified semi-supervised learning paradigm to enable bidirectional mapping between images and interaction semantics, effective even with limited annotations.

Result: UniHOI achieves state-of-the-art performance: 4.9% improvement in long-tailed HOI detection accuracy and 42.0% boost in interaction metrics for open-vocabulary generation tasks.

Conclusion: Jointly modeling HOI detection and generation through a unified framework enhances generalization and interaction understanding, demonstrating the effectiveness of the proposed UniHOI approach.
...
TL;DR: Proposes DLRRF model for hyperspectral-multispectral image fusion that handles inter-image variability through degradation modeling and low-rank decomposition with residual components


<details>
  <summary>Details</summary>
Motivation: Existing fusion methods struggle with spectral variability and spatially localized changes between HSI and MSI, which degrade fusion quality when using direct image transformations

Method: Models spectral variability as degradation operator changes, decomposes target HSI into low-rank and residual components, uses dimensionality reduction and Plug-and-Play framework with Proximal Alternating Optimization

Result: DLRRF achieves superior performance in fusing HSI and MSI with inter-image variability compared to existing methods

Conclusion: The proposed degradation-based approach effectively handles inter-image variability and improves hyperspectral image spatial resolution enhancement
...
TL;DR: CellGenNet uses a student-teacher knowledge distillation framework with hybrid loss to improve nuclei segmentation in WSIs under limited supervision, showing better accuracy and generalization than baselines.


<details>
  <summary>Details</summary>
Motivation: Accurate nuclei segmentation in microscopy whole slide images is difficult due to staining, imaging, and tissue variability, requiring robust methods that work with sparse annotations.

Method: A student-teacher architecture where the teacher trains on sparse labels to generate pseudo-labels; the student uses a joint loss combining ground truth, teacher targets, binary cross-entropy, and Tversky loss, with consistency regularization and dropout.

Result: Experiments on diverse cancer tissue WSIs demonstrate improved segmentation accuracy and generalization over supervised and semi-supervised methods.

Conclusion: CellGenNet enables scalable and reproducible histopathology analysis by effectively leveraging limited supervision.
...
TL;DR: ProPL is a universal semi-supervised framework for ultrasound image segmentation that handles multiple organs/tasks using a prompt-guided dual decoder and uncertainty-driven pseudo-label calibration, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing ultrasound segmentation methods are specialized for specific tasks, limiting clinical utility. There's a need for a universal approach that works across multiple anatomical structures.

Method: Uses shared vision encoder with prompt-guided dual decoders for task adaptation, plus uncertainty-driven pseudo-label calibration module for reliable self-training with labeled/unlabeled data.

Result: ProPL outperforms state-of-the-art methods across metrics on a comprehensive ultrasound dataset spanning 5 organs and 8 segmentation tasks.

Conclusion: ProPL establishes a new benchmark for universal ultrasound image segmentation, demonstrating superior performance and practical utility in clinical settings.
...
TL;DR: Evaluation of MLLMs on vertically written Japanese text shows poor performance, which improves after fine-tuning with a synthesized Japanese OCR dataset containing both horizontal and vertical writing.


<details>
  <summary>Details</summary>
Motivation: MLLMs need to process Japanese documents, some of which are written vertically, but research on vertical writing support is limited.

Method: Generate a synthetic Japanese OCR dataset with horizontal and vertical writing for fine-tuning and evaluation, plus real-world document images for evaluation.

Result: Existing MLLMs perform worse on vertical text than horizontal text; fine-tuning with the synthesized dataset improves performance on vertical writing.

Conclusion: Training on datasets containing vertical text enhances MLLM capability for vertical Japanese document understanding, with datasets and code made available.
...
TL;DR: This paper introduces VR-Bench, a benchmark for evaluating video models' spatial reasoning abilities through maze-solving tasks, showing that video generation models outperform vision-language models in spatial perception and generalization.


<details>
  <summary>Details</summary>
Motivation: Comparing text-based reasoning in language models, the authors investigate if video models can perform reasoning through video generation, leveraging video's spatial and temporal properties.

Method: The authors created VR-Bench with 7,920 procedurally generated videos across five maze types and visual styles, grounded in maze-solving tasks requiring spatial planning and multi-step reasoning. Fine-tuning (SFT) was used to elicit reasoning capabilities.

Result: Video models demonstrated superior spatial perception over leading vision-language models, generalized well across scenarios, and showed a test-time scaling effect where diverse sampling improved reasoning reliability by 10-20%.

Conclusion: Video generation holds unique potential for scalable spatial reasoning tasks, as evidenced by the effectiveness of the VR-Bench framework.
...
TL;DR: BokehFlow is a depth-free framework using flow matching for controllable bokeh rendering from all-in-focus images via text prompts, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Rendering controllable bokeh without depth inputs is challenging; existing methods rely on depth maps or have limited controllability and efficiency.

Method: Uses flow matching with a cross-attention mechanism to synthesize bokeh effects directly from images, allowing semantic control via text prompts.

Result: Achieves compelling bokeh effects with precise control, superior to depth-dependent and generative methods in quality and efficiency, validated on four datasets.

Conclusion: BokehFlow effectively addresses depth-free controllable bokeh rendering, offering a robust solution with enhanced performance.
...
TL;DR: MambaTrack3D is a novel 3D object tracking framework using Mamba state space model for efficient handling of high temporal variation environments, achieving superior accuracy-efficiency trade-off.


<details>
  <summary>Details</summary>
Motivation: Existing memory-based trackers struggle with quadratic complexity, temporal redundancy, and insufficient geometric modeling in dynamic outdoor environments with high temporal variation.

Method: Uses Mamba-based Inter-frame Propagation module for near-linear complexity and explicit spatial modeling, plus Grouped Feature Enhancement Module to separate foreground/background semantics.

Result: Outperforms both HTV-oriented and normal trackers on benchmarks (up to 6.5% success, 9.5% precision improvements), maintains competitiveness on standard datasets.

Conclusion: MambaTrack3D achieves robust performance across both specialized HTV and conventional scenarios with superior accuracy-efficiency balance.
...
TL;DR: TiCAL is a novel framework for Multimodal Emotion Recognition (MER) that addresses inter-modal emotion conflicts by using pseudo unimodal labels and typicality estimation, with hyperbolic embeddings to improve fine-grained emotion distinctions, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing MER approaches use unified emotion labels but ignore inter-modal emotion conflicts, where different modalities in a sample show divergent emotional tendencies, limiting accuracy.

Method: The TiCAL framework dynamically assesses sample consistency using pseudo unimodal emotion labels and typicality estimation, and employs hyperbolic space embeddings to capture fine-grained emotional distinctions.

Result: Extensive experiments on CMU-MOSEI and MER2023 show TiCAL mitigates inter-modal conflicts and improves recognition accuracy, e.g., by about 2.6% over the state-of-the-art DMD method.

Conclusion: TiCAL effectively handles inter-modal emotion conflicts through consistency-aware learning and hyperbolic embeddings, enhancing MER performance and robustness on inconsistent samples.
