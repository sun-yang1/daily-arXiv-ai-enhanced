...
TL;DR: V-Agent is a multi-agent platform for video search and interactive conversations, using a fine-tuned vision-language model and retrieval vector to improve multimodal retrieval. It outperforms traditional text-based systems and achieves state-of-the-art zero-shot performance.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of traditional text-based retrieval systems in multimodal scenarios by integrating visual and spoken content for context-aware video search.

Method: Fine-tuning a vision-language model with a small video preference dataset and enhancing it with a retrieval vector from an image-text retrieval model. The system includes three agents (routing, search, chat) for collaborative intent handling.

Result: Demonstrates state-of-the-art zero-shot performance on the MultiVENT 2.0 benchmark.

Conclusion: V-Agent shows strong potential for both academic research and real-world applications in multimodal video retrieval and interactive systems.
...
TL;DR: A study developed a deep learning system using CNN and VGG-16 models to detect pornographic content in websites. CNN proved more effective, achieving 94.87% accuracy.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the observation that despite government blocking of pornographic websites in Indonesia, VPNs allow public access, highlighting the need for an efficient system to identify such content.

Method: The research employs a deep learning approach, specifically comparing the performance of convolutional neural network (CNN) and visual geometry group 16 (VGG-16) models for detecting pornographic image content.

Result: The CNN model outperformed the VGG-16 model, achieving an accuracy of 94.87% in the eighth experiment with an epoch value of 50 and a learning rate of 0.001.

Conclusion: The study concludes that the CNN model is more effective than the VGG-16 model for quickly and accurately detecting pornographic content in websites, achieving an accuracy of 94.87% under optimal conditions.
...
TL;DR: AVM introduces a modular framework for condition-aware neural response modeling, improving generalization and interpretability in visual encoding.


<details>
  <summary>Details</summary>
Motivation: Deep learning models often fail to separate stable visual encoding from condition-specific adaptation, limiting generalization across stimuli and individuals.

Method: AVM uses a frozen Vision Transformer-based encoder for stable visual features and independently trained modulation paths for condition-specific adaptation.

Result: AVM outperforms the state-of-the-art V1T model by ~2% in predictive correlation and achieves a 9.1% improvement in explained variance under cross-dataset adaptation.

Conclusion: AVM provides a unified, scalable framework for adaptive neural modeling, offering robust generalization and interpretable condition-wise modulation. Its design may inform future cortical modeling in neuroscience and AI.
...
TL;DR: The study uses Finer-CAM and YOLOv8 models on TLS data to analyze tree species classification, finding crown features dominate for some species and stem features for others, highlighting the need for clearer model decision processes.


<details>
  <summary>Details</summary>
Motivation: The motivation behind the research is to address the lack of clarity in the decision processes of state-of-the-art tree species classification methods, despite their high accuracy. The study aims to systematically evaluate which structural features of trees drive species discrimination.

Method: The study proposes a novel method linking Finer-CAM explanations to segments of TLS projections representing structural tree features. It involves training and validating five YOLOv8 models using TLS data from 2,445 trees across seven European species, achieving a mean accuracy of 96%. The analysis includes 630 saliency maps to evaluate feature contributions.

Result: The results show that the models primarily rely on crown features for species like Silver Birch, European Beech, English oak, and Norway spruce, while stem features are more critical for European ash, Scots pine, and Douglas fir. Finer branches also contribute significantly to model decisions.

Conclusion: The study concludes that while deep learning models achieve high accuracy in tree species classification using TLS data, their decision processes are primarily driven by crown features for some species and stem features for others. The findings emphasize the need for better understanding of model decision processes to address dataset and model limitations, biases, and to enhance confidence in predictions.
...
TL;DR: A multi-stage pipeline using LLMs and text-to-image models improves character consistency in video generation, with visual anchoring being crucial. Cultural biases in current models are also identified.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of generating long, cohesive video stories with consistent characters, which current text-to-video AI struggles with.

Method: The method involves a pipeline that first uses a large language model to generate a detailed production script, then a text-to-image model to create consistent visuals for characters, and finally a video generation model to synthesize scenes individually.

Result: The results show that visual anchoring significantly improves character consistency (from 0.55 to 7.99 in scores) and reveal cultural biases in subject consistency and dynamic degree between Indian vs Western-themed generations.

Conclusion: The paper concludes that a multi-stage decomposition approach, including visual anchoring, is essential for maintaining character consistency in long video story generation. It also highlights cultural biases in current models.
...
TL;DR: InfoTok is an adaptive video tokenization framework that optimizes token allocation based on information density, achieving superior compression and performance.


<details>
  <summary>Details</summary>
Motivation: Current video tokenizers compress content at a fixed rate, leading to redundancy or information loss due to varying information density in videos.

Method: The paper introduces InfoTok, a framework using an ELBO-based algorithm for adaptive video tokenization, implemented via a transformer-based adaptive compressor.

Result: InfoTok saves 20% tokens without performance loss and achieves 2.3x compression rates while outperforming prior heuristic approaches.

Conclusion: InfoTok provides a more efficient and accurate method for video tokenization by adaptively allocating tokens based on informational richness, outperforming existing methods.
...
TL;DR: Endo-SemiS is a semi-supervised segmentation framework for endoscopic video frames that uses cross-supervision, uncertainty-guided pseudo-labels, joint supervision, and mutual learning to improve performance with limited labeled data. It outperforms existing methods on two clinical datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of limited annotated data in endoscopic video segmentation by developing a semi-supervised framework that effectively utilizes unlabeled data to improve segmentation performance.

Method: Endo-SemiS employs a semi-supervised framework with four key strategies: cross-supervision between two networks, uncertainty-guided pseudo-labels, joint pseudo-label supervision, and mutual learning. It also includes a corrective network leveraging spatiotemporal information. The framework is evaluated on kidney stone laser lithotomy and polyp screening datasets.

Result: Endo-SemiS achieves superior results compared to state-of-the-art segmentation methods on both kidney stone laser lithotomy and polyp screening datasets, particularly with limited labeled data.

Conclusion: Endo-SemiS demonstrates superior performance in semi-supervised segmentation of endoscopic video frames, outperforming state-of-the-art methods with limited labeled data. The framework's innovative strategies (cross-supervision, uncertainty-guided pseudo-labels, joint pseudo-label supervision, and mutual learning) effectively utilize unlabeled data, and the inclusion of a corrective network further enhances segmentation accuracy.
...
TL;DR: LongShOTBench is a new benchmark for evaluating long-form multimodal video understanding, featuring open-ended questions and graded rubrics. It reveals significant performance gaps among models, with LongShOTAgent achieving 44.66% accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for long-form multimodal video understanding lack both temporal length and multimodal richness, often relying on single-score accuracy, which obscures failure modes. The authors aim to address these limitations.

Method: The authors introduce LongShOTBench, a diagnostic benchmark with open-ended, intent-driven questions, single- and multi-turn dialogues, and tasks requiring multimodal reasoning. They also present LongShOTAgent, an agentic system for analyzing long videos via preprocessing, search, and iterative refinement.

Result: State-of-the-art MLLMs show large performance gaps on LongShOTBench: Gemini-2.5-Flash achieves 52.95%, open-source models remain below 30%, and LongShOTAgent attains 44.66%.

Conclusion: LongShOTBench provides a practical and reproducible foundation for evaluating and improving Multimodal Large Language Models (MLLMs) in long-form video understanding, highlighting significant performance gaps among existing models.
...
TL;DR: A new MLLM (4D-RGPT) and benchmark (R4D-Bench) improve 4D perception in video QA, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current MLLMs in reasoning over 3D structures and temporal dynamics due to weak 4D perception and temporal understanding.

Method: The paper introduces 4D-RGPT, a specialized MLLM for 4D representations, along with Perceptual 4D Distillation (P4D) for training and R4D-Bench as a new benchmark.

Result: 4D-RGPT shows notable improvements on existing 4D VQA benchmarks and the proposed R4D-Bench.

Conclusion: The 4D-RGPT model demonstrates significant improvements in 4D VQA tasks, validated by both existing benchmarks and the newly introduced R4D-Bench.
...
TL;DR: FORMSpoT-$Δ$ provides high-resolution forest monitoring, outperforming existing tools, and highlights the need for sustained high-resolution satellite missions.


<details>
  <summary>Details</summary>
Motivation: The recent decline of the European forest carbon sink highlights the need for spatially explicit and frequently updated forest monitoring tools capable of detecting changes at the scale of individual trees.

Method: Canopy heights were derived from annual SPOT-6/7 composites using a hierarchical transformer model (PVTv2) trained on high-resolution airborne laser scanning (ALS) data. A dedicated post-processing pipeline combining co-registration and spatio-temporal total variation denoising was developed for robust change detection.

Result: FORMSpoT-$Δ$ substantially outperforms existing disturbance products, achieving an F1-score of 0.44 in mountainous forests, an order of magnitude higher than existing benchmarks.

Conclusion: The results underscore the critical importance of sustaining very high-resolution satellite missions like SPOT and open-data initiatives such as DINAMIS for monitoring forests under climate change.
...
TL;DR: InfCam introduces a depth-free, camera-controlled video generation framework using infinite homography warping and data augmentation, improving pose fidelity and visual quality over existing methods.


<details>
  <summary>Details</summary>
Motivation: To overcome the challenges of ensuring camera-pose fidelity and maintaining view consistency in dynamic scenes, while avoiding errors from inaccurate depth estimation and limited dataset diversity.

Method: The framework integrates infinite homography warping to encode 3D camera rotations in the 2D latent space and a data augmentation pipeline to enhance trajectory diversity. It predicts a residual parallax term through end-to-end training.

Result: InfCam outperforms baseline methods in camera-pose accuracy and visual fidelity, generalizing well from synthetic to real-world data.

Conclusion: InfCam is a novel framework for camera-controlled video-to-video generation that addresses the limitations of existing methods by using infinite homography warping and a data augmentation pipeline, achieving superior camera-pose accuracy and visual fidelity.
...
TL;DR: Proposes Interpretable Utility Similarity (IUS), a novel measure to assess synthetic vs. real image similarity for DL-based CDS systems, showing up to 54.6% improvement in classification performance.


<details>
  <summary>Details</summary>
Motivation: Current methods for assessing synthetic vs. real image similarity rely on user studies or inception-based measures, lacking interpretability for clinical relevance.

Method: Introduces IUS, inspired by generalized neural additive models, to quantitatively and interpretably compare synthetic and real image utility for CDS systems.

Result: Experiments across multiple medical imaging modalities demonstrate IUS's effectiveness, with synthetic datasets selected via IUS improving classification performance by up to 54.6%.

Conclusion: IUS provides a clinically interpretable and effective measure for synthetic data assessment, enhancing the utility of synthetic images in DL-based CDS systems.
...
TL;DR: DGH is a learning-based framework for photorealistic dynamic hair modeling, using a coarse-to-fine motion model and strand-guided optimization for appearance, outperforming physics-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for dynamic hair modeling rely on static capture and physics-based models, which are limited by manual parameter tuning and high computational costs, making them unsuitable for diverse hairstyles and motions.

Method: The framework includes a coarse-to-fine model for learning temporally coherent hair motion dynamics and a strand-guided optimization module for learning a dynamic 3D Gaussian representation of hair appearance, supporting differentiable rendering.

Result: DGH achieves promising geometry and appearance results, generalizing across various hairstyles and head motions, and can be integrated into a 3D Gaussian avatar framework.

Conclusion: The Dynamic Gaussian Hair (DGH) framework presents a scalable, data-driven solution for photorealistic dynamic hair modeling, overcoming the limitations of physics-based methods by leveraging learning-based techniques.
...
TL;DR: The paper reviews transformer architectures for maritime radar frame prediction, finding a gap in research despite radar's importance for all-weather navigation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the unexplored application of transformer architectures in maritime radar frame prediction, given radar's reliability in all-weather conditions, which is critical for maritime autonomous systems.

Method: The paper systematically reviews predictive modeling approaches for maritime radar, focusing on transformer architectures for spatiotemporal sequence forecasting. It analyzes existing methods based on data type, architecture, and prediction horizon.

Result: The review reveals that no prior work has addressed transformer-based maritime radar frame prediction, highlighting a research gap.

Conclusion: The paper concludes that while transformer architectures have been successfully applied to AIS-based trajectory prediction and sonar frame forecasting, there is a significant gap in their application to maritime radar frame prediction, which is crucial for all-weather navigation. This gap presents a clear research opportunity.
...
TL;DR: SDUM is a universal MRI reconstruction framework that scales predictably with model depth, outperforming specialized methods across diverse protocols without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current deep learning reconstructions in clinical MRI are typically protocol-specific, hindering generalization and deployment. SDUM aims to provide a universal framework for diverse MRI protocols.

Method: SDUM combines a Restormer-based reconstructor, a learned coil sensitivity map estimator (CSME), sampling-aware weighted data consistency (SWDC), universal conditioning (UC) on cascade index and protocol metadata, and progressive cascade expansion training.

Result: SDUM achieves state-of-the-art results across all four CMRxRecon2025 challenge tracks without task-specific fine-tuning, surpassing specialized baselines by up to +1.0 dB, and outperforms the winning method PromptMR+ by +0.55 dB on CMRxRecon2024.

Conclusion: SDUM is established as a practical path toward universal, scalable MRI reconstruction, demonstrating predictable performance gains with model depth and achieving state-of-the-art results across diverse clinical MRI protocols without task-specific fine-tuning.
...
TL;DR: The paper presents a method to turn ordinary photos into professional portraits using UV space transformation and multi-image fine-tuning, preserving identity and enhancing quality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the gap between amateur and professional photography by transforming ordinary photos into high-quality, posed portraits while preserving the subject's unique identity.

Method: The method involves transforming the input photo into a canonical UV space, coupled with reposing techniques to handle occlusions and synthesize novel views. Multi-image fine-tuning is used to personalize the output.

Result: The approach achieves high-quality, reposed portraits with strong qualitative and quantitative performance on real-world images.

Conclusion: The paper concludes that the proposed method effectively transforms ordinary photos into professional-quality portraits by leveraging UV space and multi-image fine-tuning, preserving identity while enhancing quality.
...
TL;DR: The paper introduces a training-free framework for document-centric background generation with multi-page editing, ensuring readability and thematic continuity through latent masking, Automated Readability Optimization, and summarization-based consistency.


<details>
  <summary>Details</summary>
Motivation: The motivation behind the paper is to address the challenge of generating document-centric backgrounds with multi-page editing while ensuring text readability and thematic continuity. The authors aim to automate the process of maintaining aesthetic harmony and readability without human intervention.

Method: The method involves a training-free framework that uses latent masking to softly attenuate updates in the diffusion space for readability. It introduces Automated Readability Optimization (ARO) to automatically place semi-transparent backing shapes behind text regions for perceptual contrast. Multi-page consistency is maintained through a summarization-and-instruction process, treating documents as structured compositions with separate layers for text, figures, and backgrounds.

Result: The framework produces visually coherent, text-preserving, and thematically aligned documents. It ensures readability through ARO and maintains multi-page consistency, allowing for flexible customization while adhering to perceptual contrast standards.

Conclusion: The paper concludes that the proposed framework successfully bridges generative modeling with natural design workflows by ensuring visually coherent, text-preserving, and thematically aligned documents. It achieves this through latent masking, Automated Readability Optimization (ARO), and multi-page consistency mechanisms.
...
TL;DR: PhysFire-WM integrates physics-informed world modeling with cross-task collaborative training to improve fire spread prediction accuracy.


<details>
  <summary>Details</summary>
Motivation: Current methods for fire prediction are limited by binary mask modeling and physical inconsistencies in world models, failing to capture complex fire dynamics.

Method: The paper introduces PhysFire-WM, which encodes structured priors from a physical simulator and employs a Cross-task Collaborative Training strategy (CC-Train) to integrate thermal radiation dynamics and spatial boundary delineation.

Result: Extensive experiments show superior accuracy in fire spread prediction, validated on a fine-grained multimodal fire dataset.

Conclusion: The study highlights the importance of physical priors and cross-task collaboration, offering new insights for physics-informed world models in disaster prediction.
...
TL;DR: LGCLIP introduces a lightweight framework for zero-shot image classification by using LLM-generated prompts and diffusion models to create visual prototypes, eliminating the need for annotated image-text pairs.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on annotated text-to-image pairs for aligning visual and textual modalities, which introduces substantial cost and accuracy requirements in preparing high-quality datasets. Additionally, processing data from two modes requires dual-tower encoders, hindering their lightweight.

Method: LGCLIP leverages a Large Language Model (LLM) to generate class-specific prompts that guide a diffusion model in synthesizing reference images. These generated images serve as visual prototypes, and the visual features of real images are extracted and compared with the visual features of these prototypes to achieve comparative prediction.

Result: Experimental results validate the feasibility and efficiency of LGCLIP.

Conclusion: LGCLIP demonstrates great performance in zero-shot classification tasks and establishes a novel paradigm for classification by eliminating the need for manually annotated image-text pairs and extra pre-processing.
...
TL;DR: ABE-CLIP enhances attribute-object binding in CLIP-like models without additional training, using semantic refinement and token-patch alignment.


<details>
  <summary>Details</summary>
Motivation: CLIP struggles with compositional image-text matching due to overlooked fine-grained semantics in global representations.

Method: ABE-CLIP employs Semantic Refinement Mechanism and Local Token-Patch Alignment to improve attribute binding.

Result: ABE-CLIP significantly outperforms existing methods in attribute-object binding, even without training.

Conclusion: ABE-CLIP addresses CLIP's limitations in compositional tasks, offering a training-free solution with superior performance.
...
TL;DR: Study finds global consistency in discrepancies between objective and subjective urban greenery assessments, with location being the primary influencer, highlighting cultural and environmental impacts.


<details>
  <summary>Details</summary>
Motivation: Urban greenery is vital for climate and well-being, yet objective and subjective assessments often differ. Understanding these discrepancies can improve urban planning and development.

Method: The research combines objective measures (e.g., Green View Index) with subjective scores from a visual perception survey of 1,000 people across five countries, analyzing discrepancies through human, geographic, and spatial dimensions.

Result: Discrepancies between objective and subjective greenery assessments are comparable worldwide. Demographics and personality have minimal impact, while location (where people live) is a key factor, suggesting cultural and environmental influences.

Conclusion: The study highlights that discrepancies between objective and subjective greenery assessments are consistent globally and are primarily influenced by where people live, rather than demographics or personality. Cultural, environmental, and experiential factors significantly shape perceptions of urban greenery.
...
TL;DR: A globally optimal solver using affine correspondences is proposed for accurate relative pose estimation in multi-camera systems, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the accuracy of relative pose estimation in multi-camera systems, which is crucial for applications like self-driving cars, by utilizing visual and inertial data effectively.

Method: The method involves decoupling the rotation matrix and translation vector, minimizing algebraic error from affine correspondences, converting the problem into polynomial equations, and solving for the relative rotation angle and translation vector. A linear solution is also introduced for small rotations.

Result: Experiments on synthetic and real-world datasets confirm that the solver achieves higher accuracy compared to existing methods.

Conclusion: The proposed solver significantly improves the accuracy of relative pose estimation in multi-camera systems by leveraging affine correspondences and global optimization techniques, outperforming existing state-of-the-art methods.
...
TL;DR: ARCD introduces a plug-and-play strategy to reduce hallucinations in MedVLMs by using anatomical masks for targeted guidance, improving diagnostic reliability across multiple medical imaging datasets.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing strategies for mitigating hallucinations in MedVLMs, such as reliance on costly expert annotations or untargeted corrections, which hinder reliability in clinical settings.

Method: Anatomical Region-Guided Contrastive Decoding (ARCD) leverages an anatomical mask to dynamically re-weight at the token, attention, and logits levels, directing the model's focus onto specified regions.

Result: ARCD demonstrates effectiveness across diverse datasets (chest X-ray, CT, brain MRI, and ocular ultrasound) by improving regional understanding, reducing hallucinations, and enhancing diagnostic accuracy.

Conclusion: ARCD effectively mitigates hallucinations in MedVLMs by providing targeted, region-specific guidance through a three-tiered contrastive decoding process, improving diagnostic accuracy and reliability.
...
TL;DR: Fose combines one-step diffusion and E2E models via a four-stage training strategy, improving pansharpening performance and speed (7.42x faster than baseline DM).


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models (DM) for pansharpening are computationally intensive due to multi-step processes, while end-to-end (E2E) models suffer from limited performance due to lack of prior and simple structure. The goal is to combine the strengths of both approaches efficiently.

Method: The paper introduces a four-stage training strategy to create Fose, a lightweight network. It involves one-step distillation of an enhanced diffusion model (DM) and fusion with an end-to-end (E2E) model using lightweight ensemble blocks.

Result: Fose achieves significant performance improvements on three benchmarks and a 7.42 speedup ratio compared to the baseline DM. The code and model are publicly available.

Conclusion: The proposed Fose model significantly improves pansharpening performance by combining one-step diffusion and an E2E model, achieving a 7.42 speedup ratio compared to the baseline DM while maintaining superior results.
...
TL;DR: Reasoning Palette introduces a latent-modulation framework to enhance diversity and efficiency in (vision-) language models' reasoning, outperforming standard RL methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the redundancy in reasoning paths and lack of high-level diversity in stochastic sampling for large (vision-) language models during inference and RL training.

Method: The paper proposes Reasoning Palette, a latent-modulation framework using a stochastic latent variable inferred via a VAE. This latent context is decoded into token prefixes to modulate the model's reasoning trajectory. A supervised fine-tuning phase adapts the model to latent conditioning.

Result: Experiments across multiple reasoning benchmarks show that Reasoning Palette enables interpretable and controllable strategic behavior, leading to consistent performance improvements.

Conclusion: The paper concludes that Reasoning Palette significantly enhances exploration efficiency and sustained learning capability in (vision-) language models, achieving consistent performance gains over standard RL methods.
...
TL;DR: CheXPO-v2 reduces hallucinations in Medical VLMs via process supervision and knowledge graph rewards, achieving high accuracy with minimal data.


<details>
  <summary>Details</summary>
Motivation: Medical VLMs are prone to hallucinations, and existing methods like GRPO encourage verbose, unverifiable reasoning, posing safety risks.

Method: Proposes CheXPO-v2, a novel alignment framework with a Knowledge Graph Consistency Reward mechanism and hard-example mining strategy.

Result: Significantly outperforms GRPO and state-of-the-art models on benchmarks like MIMIC-CXR-VQA, using only 5k samples.

Conclusion: CheXPO-v2 achieves state-of-the-art accuracy with high data efficiency and clinically sound reasoning, addressing hallucinations in Medical VLMs.
...
TL;DR: DAVE is a vision encoder designed for VLMs to address the lack of robust structural and spatial information in low-level features. It uses self-supervised and supervised pretraining, model-merging, and ensemble training to improve performance on document and web tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs lack robust structural and spatial information for document understanding and web agents.

Method: Self-supervised pretraining on unlabeled data, followed by supervised pretraining with tasks like parsing and localization. Model-merging and ensemble training are used to enhance alignment.

Result: DAVE shows strong performance on document tasks, VQAs, web localization, and agent-based benchmarks.

Conclusion: DAVE is an effective vision encoder tailored for document and web applications, overcoming limitations of current VLMs.
...
TL;DR: AOM is a universal Remote Sensing Foundation Model that handles arbitrary band compositions, sensor types, and resolutions, outperforming existing models in diverse and challenging scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing RSFMs are limited by fixed band configurations and resolutions, making them vulnerable to real-world scenarios like missing bands, cross-sensor fusion, and unseen spatial scales, which hinder their generalization and practical deployment.

Method: AOM introduces a spectrum-independent tokenizer for encoding spectral identity, a multi-scale adaptive patch embedding mechanism for capturing texture and contextual patterns, and a multi-scale semantic alignment mechanism with a self-supervised masking and reconstruction pretraining strategy.

Result: AOM consistently achieves state-of-the-art performance on over 10 public datasets, including Sentinel-2, Landsat, and HLS, under challenging conditions such as band missing, cross-sensor, and cross-resolution settings.

Conclusion: The proposed Any Optical Model (AOM) successfully addresses the limitations of existing Remote Sensing Foundation Models (RSFMs) by accommodating arbitrary band compositions, sensor types, and resolution scales, achieving state-of-the-art performance across diverse datasets and challenging conditions.
...
TL;DR: A novel aggregator module combines geometric and visual cues for robust global descriptors, improving localization accuracy without manual labels.


<details>
  <summary>Details</summary>
Motivation: Existing visual localization methods rely solely on geometric cues for global descriptors, limiting discriminative power and robustness against noisy geometric constraints.

Method: The method introduces an aggregator module that learns global descriptors by combining geometric and visual cues, using a batch-mining strategy based on overlap scores and a modified contrastive loss.

Result: Experiments demonstrate substantial localization improvements in challenging benchmarks, with the method generalizing well across diverse environments.

Conclusion: The proposed aggregator module, which integrates geometric structure and visual similarity, significantly improves localization accuracy in large-scale environments while maintaining computational efficiency.
...
TL;DR: A curriculum-based framework disentangles abstract reasoning and strategic visual perception in MLLMs, addressing 'visual forgetting' through a two-stage approach: SFT curriculum and reinforcement learning for perception timing.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the observed 'visual forgetting' in MLLMs during complex, long-chain visual reasoning tasks, where models lose visual grounding as reasoning extends. The authors attribute this to the premature entanglement of abstract logical reasoning and strategic visual perception in current training paradigms.

Method: The method involves a two-stage approach: (1) a disentangled Supervised Fine-Tuning (SFT) curriculum to build abstract reasoning on text-only data, followed by anchoring it to vision with Perception-Grounded Chain-of-Thought (PG-CoT), and (2) resolving the strategic perception deficit by formulating timing as a reinforcement learning problem with a Pivotal Perception Reward.

Result: The results demonstrate that the proposed framework effectively addresses the foundational cold-start deficiency and strategic perception deficit, enabling models to autonomously learn when to perceive and improve visual grounding.

Conclusion: The paper concludes that disentangling abstract reasoning from strategic visual perception through a curriculum-based framework significantly improves the robustness of Multimodal Large Language Models (MLLMs) in long-chain visual reasoning tasks. The proposed two-stage framework transforms models into strategic, grounded reasoners.
