...
TL;DR: A real-time maritime object detection system using RT-DETR, synthetic data, and multi-scale fusion improves small vessel detection and handles extreme conditions.


<details>
  <summary>Details</summary>
Motivation: Challenges in maritime object detection include small target size and limited labeled real RGB data, motivating the need for an enhanced real-time detection system.

Method: The method combines RT-DETR with multi-scale feature fusion, uncertainty-minimizing query selection, and a weighting strategy for synthetic and real data. Data augmentation is used to balance classes.

Result: The system achieves real-time performance, improves small vessel detection, and handles extreme conditions, with each module's contribution quantified.

Conclusion: The study presents a robust real-time maritime object detection system using RT-DETR, enhanced by synthetic data and multi-scale feature fusion, which maintains performance under practical constraints.
...
TL;DR: DynamicEval addresses gaps in T2V evaluation by focusing on dynamic camera motion and video-level assessment, introducing new metrics for background and foreground consistency that outperform existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing T2V evaluation benchmarks like VBench and EvalCrafter lack focus on dynamic camera motion and video-level evaluation, aggregating scores into model-level rankings. This overlooks the need for selecting better videos for specific prompts.

Method: The authors introduce DynamicEval, a benchmark with curated prompts emphasizing dynamic camera motion, paired with 45k human annotations. They propose two new metrics: one for background scene consistency (using object error maps to correct Vbench failures) and another for foreground object consistency (tracking points within object instances).

Result: DynamicEval's metrics achieve stronger correlations with human preferences, improving alignment by over 2% points at both video and model levels. The benchmark is validated through extensive experiments.

Conclusion: DynamicEval is introduced as a more comprehensive benchmark for evaluating text-to-video (T2V) models under dynamic camera motion, addressing limitations of existing benchmarks by focusing on video-level evaluation and dynamic motion. The proposed metrics for background and foreground consistency show stronger correlations with human preferences, improving evaluation accuracy.
...
TL;DR: RISP extends RED with restarting inertia and score-based priors, achieving faster convergence and high-quality reconstructions without requiring convexity.


<details>
  <summary>Details</summary>
Motivation: Existing methods like RED focus on image priors for quality but lack principled convergence acceleration. RISP bridges this gap by combining fast convergence with high-quality reconstruction.

Method: RISP incorporates restarting inertia for fast convergence and score-based image priors for high-quality reconstruction. Theoretical analysis includes proving faster stationary-point convergence and deriving the associated continuous-time dynamical system.

Result: Experiments show RISP enables fast convergence and high-quality reconstructions across various imaging inverse problems.

Conclusion: RISP achieves faster convergence and high-quality reconstructions in imaging inverse problems, outperforming RED without requiring convexity of the image prior.
...
TL;DR: A novel denoising framework using Image Purification and Frequency-domain Flow Matching addresses data mismatch in uLDCT, achieving SOTA results in structure preservation.


<details>
  <summary>Details</summary>
Motivation: Ultra-low dose CT (uLDCT) reduces radiation exposure but introduces severe noise, artifacts, and spatial misalignment with normal dose CT (NDCT), making existing denoising networks ineffective.

Method: The paper proposes an Image Purification (IP) strategy to generate structurally aligned uLDCT-NDCT image pairs and a Frequency-domain Flow Matching (FFM) model to preserve anatomical structure integrity.

Result: Experiments show the IP strategy enhances denoising model performance, and the FFM model combined with IP achieves SOTA results in anatomical structure preservation.

Conclusion: The study provides an effective solution to the data mismatch problem in real-world uLDCT denoising by combining the Image Purification strategy and the Frequency-domain Flow Matching model, achieving state-of-the-art results in anatomical structure preservation.
...
TL;DR: D2RA is a training-free attack that weakens watermarks by projecting images onto natural priors, exposing flaws in current watermarking schemes.


<details>
  <summary>Details</summary>
Motivation: The growing use of generative models necessitates robust watermarking methods for content attribution and provenance. However, existing semantic watermarking schemes are vulnerable even under resource-constrained adversarial settings.

Method: The authors propose D2RA, a training-free, single-image attack that projects watermarked images onto natural priors across complementary representations to suppress watermark signals while preserving visual fidelity.

Result: Experiments show that D2RA consistently reduces watermark detectability across diverse watermarking schemes, highlighting vulnerabilities in current designs.

Conclusion: The paper reveals fundamental weaknesses in current watermarking schemes and demonstrates the effectiveness of D2RA in removing or weakening watermarks without requiring access to the underlying model.
...
TL;DR: PickStyle is a video-to-video style transfer framework using diffusion models, leveraging paired image data and low-rank adapters for efficient motion-style transfer. It introduces CS-CFG for preserving context while transferring style, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The lack of paired video data for supervision in video style transfer tasks motivates the need for a framework that can utilize paired still image data and adapt diffusion models effectively.

Method: PickStyle augments pretrained video diffusion models with style adapters and uses synthetic training clips from paired images. It introduces CS-CFG for independent control of style and context.

Result: The approach achieves temporally coherent, style-faithful, and content-preserving video translations, outperforming baselines qualitatively and quantitatively.

Conclusion: PickStyle effectively bridges the gap between static image supervision and dynamic video, offering a robust solution for video style transfer with diffusion models.
...
TL;DR: The paper introduces TRAVL and ImplausiBench to improve and evaluate physical plausibility in video generative models, addressing limitations in VLMs' temporal and causal reasoning.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of robust methods for quantitatively assessing physical realism in videos generated by modern models, which often violate intuitive physical laws.

Method: The authors introduce TRAVL, a fine-tuning recipe combining a balanced dataset with a trajectory-aware attention module, and propose ImplausiBench, a benchmark for evaluating physical reasoning.

Result: The results show that existing Video-Language Models (VLMs) struggle with identifying physics violations, but TRAVL improves motion encoding and discrimination. ImplausiBench provides a rigorous evaluation framework.

Conclusion: The paper concludes that TRAVL and ImplausiBench provide a unified framework for evaluating and improving physical plausibility in video generative models, addressing a critical gap in visual-temporal understanding.
...
TL;DR: The paper introduces S3FN, a Semantic Spectral-Spatial Fusion Network for HSI classification, leveraging textual descriptions from LLMs to enhance model performance by aligning features with label semantics.


<details>
  <summary>Details</summary>
Motivation: HSI classification models often overfit due to limited training samples and high data dimensionality, and most rely solely on spectral-spatial data, ignoring contextual semantics.

Method: S3FN uses LLMs to generate class-specific textual descriptions, embeds them via pre-trained text encoders (e.g., BERT), and fuses these with spectral-spatial data for improved classification.

Result: S3FN significantly boosts performance on three HSI benchmark datasets (Hyperspectral Wood, HyperspectralBlueberries, DeepHS-Fruit), demonstrating the value of textual semantics.

Conclusion: The synergy of textual semantics and spectral-spatial data in S3FN advances HSI classification, with potential for broader applications in semantically augmented models.
...
TL;DR: The paper introduces CAGUL, a lightweight framework for unlearning sensitive data in Vision-Language Models (VLMs) for VQA tasks, using cross-modal attention to retain model behavior without retraining.


<details>
  <summary>Details</summary>
Motivation: VLMs may memorize and leak private data during inference, and existing unlearning methods are computationally expensive. The paper addresses this by focusing on visual tokens and cross-modal attention.

Method: The authors propose CAGUL, which uses external modules to encode unlearning information in low-importance visual tokens, leveraging cross-modal attention for efficient unlearning.

Result: CAGUL performs comparably to finetuning-based methods without altering model parameters or requiring retraining, effectively preventing data leakage.

Conclusion: CAGUL is a practical and effective unlearning solution for VLMs, balancing privacy and model performance without computational overhead.
...
TL;DR: MaSC is an automated maize stand counting algorithm using YOLOv9 on UAV-captured RGB imagery, offering scalable, low-cost, and accurate results for crop management.
...
Method: MaSC uses a lightweight YOLOv9 model trained to detect maize seedlings from V2-V10 growth stages, distinguishing maize from weeds and performing row and range segmentation for precise counts. It operates in two modes: mosaic images divided into patches and raw video frames aligned using homography matrices.

Result: MaSC showed strong agreement with ground truth (R^2= 0.616 for mosaics, R^2 = 0.906 for raw frames) and processed 83 full-resolution frames in 60.63 s, demonstrating potential for real-time operation.
...
TL;DR: QCN speeds up CapsNet by reducing capsules, achieving 5x faster inference with minimal accuracy loss, suitable for real-time use.


<details>
  <summary>Details</summary>
Motivation: CapsNet, while accurate, suffers from slow training and testing, limiting its use in fast real-time applications.

Method: QCN reduces the number of capsules to speed up the network and employs a more powerful decoder for further enhancement.

Result: QCN achieves 5x faster inference on multiple datasets (MNIST, F-MNIST, SVHN, Cifar-10) with a slight accuracy trade-off.

Conclusion: Quick-CapsNet (QCN) is introduced as a faster alternative to CapsNet, with a marginal loss in accuracy, making it suitable for real-time applications.
...
TL;DR: Rectified-CFG++ is an adaptive predictor-corrector guidance method for rectified flow models, addressing off-manifold drift issues in standard CFG and improving text-to-image generation performance.


<details>
  <summary>Details</summary>
Motivation: Standard classifier-free guidance (CFG) causes off-manifold drift in rectified flow (RF) models, leading to visual artifacts and text misalignment. This paper aims to mitigate these issues.

Method: Rectified-CFG++ combines conditional RF updates with weighted conditional corrections, ensuring trajectories stay near the data manifold. It interpolates between conditional and unconditional velocity fields.

Result: The method outperforms standard CFG on text-to-image models (Flux, Stable Diffusion 3/3.5, Lumina) across datasets like MS-COCO, LAION-Aesthetic, and T2I-CompBench.

Conclusion: Rectified-CFG++ provides stable and efficient guidance for RF models, enhancing text-to-image generation while avoiding off-manifold drift.
...
TL;DR: PIT-QMM, a novel LMM, excels in NR-PCQA by leveraging text, images, and point clouds, outperforming benchmarks and enabling distortion localization.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the under-explored potential of LMMs in 3D asset quality assessment, particularly for NR-PCQA, where no reference is available. The complementary nature of different data modalities for assessing point cloud quality drives the research.

Method: The method involves constructing PIT-QMM, an LMM that processes text, images, and point clouds end-to-end to predict quality scores. It leverages complementary information from different data modalities (text, 2D projections, 3D views) for improved performance.

Result: Results show that PIT-QMM outperforms state-of-the-art methods on popular benchmarks with fewer training iterations. It also enables distortion localization and identification, adding value beyond quality prediction.

Conclusion: The paper concludes that PIT-QMM, a novel Large Multimodal Model (LMM), significantly outperforms existing methods in No-Reference Point Cloud Quality Assessment (NR-PCQA) and enables additional capabilities like distortion localization and identification, enhancing model explainability and interactivity.
...
TL;DR: DSA Net introduces a hybrid quantum-classical framework for action segmentation, using two-stream feature alignment and achieves state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing single-stream approaches for action segmentation lack the ability to capture both action and action-transition cues effectively. The shift toward two-stream methods inspired the development of DSA Net to enhance segmentation performance.

Method: The proposed DSA Net incorporates a second stream of learned action features, facilitated by a Temporal Context (TC) block and Quantum-based Action-Guided Modulation (Q-ActGM). Feature alignment is encouraged via the Dual-Stream Alignment Loss, which includes relational consistency, cross-level contrastive, and cycle-consistency reconstruction losses.

Result: DSA Net outperforms existing methods on benchmark datasets (GTEA, Breakfast, 50Salads, and EgoProcel) and demonstrates the effectiveness of its components through ablation studies.

Conclusion: The Dual-Stream Alignment Network (DSA Net) achieves state-of-the-art performance in action segmentation by leveraging a hybrid quantum-classical framework and feature alignment between frame-wise and action-wise streams.
...
TL;DR: OIE is a novel video virtual try-on method that replaces clothing in the first frame and synthesizes the rest, achieving efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Current dual-branch architectures for diffusion models face challenges in adapting to Diffusion Transformer-based models, requiring excessive parameters and lacking temporal characteristics in latent space features.

Method: The approach involves using an image-based clothing transfer model for the first frame and leveraging pose and mask information to guide the synthesis of subsequent frames.

Result: Experiments demonstrate that the OIE method is efficient in terms of parameters and computation while delivering high performance.

Conclusion: The proposed OIE method achieves superior parameter and computational efficiency while maintaining leading performance in video virtual try-on tasks.
...
TL;DR: A method to improve personalization in diffusion models by using automatically generated masks to restrict image tokens to the subject, enhancing text prompt alignment.


<details>
  <summary>Details</summary>
Motivation: Personalization in diffusion models often leads to recreating the subject image while ignoring the text prompt, limiting control.

Method: Uses IP-Adapter's automatically generated masks to segment the subject during inference, then applies these masks in a second pass to restrict image tokens to the subject, allowing the text prompt to influence the background.

Result: Produces images that accurately depict the subject while matching the text prompt, outperforming other test-time personalization methods in prompt and source image alignment.

Conclusion: The proposed method effectively balances subject depiction and text prompt alignment, offering improved control in personalized image generation.
...
TL;DR: Transformer-based models outperform VLMs in automated text box placement, but challenges remain with small text or dense layouts, emphasizing the need for task-specific architectures.


<details>
  <summary>Details</summary>
Motivation: The research aims to balance visual appeal and communication efficiency in layout design for advertisements and web pages by examining automated text box placement in incomplete layouts.

Method: The study compares a standard Transformer-based method, a small Vision and Language Model (Phi3.5-vision), a large pretrained VLM (Gemini), and an extended Transformer processing multiple images. Evaluations are conducted on the Crello dataset.

Result: Standard Transformer-based models generally outperform VLM-based approaches, especially when incorporating richer appearance information. All methods struggle with very small text or densely populated layouts.

Conclusion: The study highlights the benefits of task-specific architectures for automated layout design and identifies challenges with small text or dense layouts, suggesting areas for future improvement.
...
TL;DR: TCIP combines FERM and TCI to improve medical image registration accuracy by addressing misalignment propagation and adaptive iteration control, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing pyramid networks for deformable medical image registration propagate anatomical misalignments and lack adaptive iteration control, leading to suboptimal performance.

Method: The paper introduces the Feature-Enhanced Residual Module (FERM) for mitigating anatomical misalignments and the dual-stage Threshold-Controlled Iterative (TCI) strategy for adaptive iteration control. These are integrated into the Threshold-Controlled Iterative Pyramid (TCIP) model.

Result: TCIP achieves superior accuracy compared to SOTA methods, with comparable inference speed and model size. FERM and TCI also enhance other registration networks.

Conclusion: The proposed TCIP model, integrating FERM and TCI, outperforms SOTA registration networks in accuracy while maintaining efficiency and compactness. FERM and TCI also demonstrate strong generalizability when integrated with other networks.
...
TL;DR: A video synthesis method is developed to enhance controllability and diversity by leveraging variational inference and multiple video generation backbones, outperforming prior works in controllability, diversity, and 3D consistency.


<details>
  <summary>Details</summary>
Motivation: Existing video generative models lack flexibility for mixed user controls, necessitating a method that balances controllability and diversity.

Method: The approach uses variational inference to approximate a composed distribution, employing step-wise KL divergence minimization and context-conditioned factorization to optimize the solution.

Result: The method achieves improved controllability, diversity, and 3D consistency in generated video samples.

Conclusion: The proposed method successfully addresses the need for flexible video synthesis, outperforming existing models in key metrics.
...
TL;DR: A hybrid CNN-BYOL method, including the new BYOL-IMNet model, achieves high accuracy (99.89%) and fast inference for fault detection in induction motors using thermal images.


<details>
  <summary>Details</summary>
Motivation: Induction motors are prone to faults that can cause overheating and operational failures. Early fault detection is crucial to prevent damage and extend motor lifespan. The paper aims to improve accuracy in fault detection using thermal images.

Method: The paper introduces a hybrid method combining BYOL (Bootstrap Your Own Latent) with CNNs for classifying thermal images of induction motors. It employs multiple DL models (ResNet-50, DenseNet-121, etc.) and proposes a new lightweight CNN model, BYOL-IMNet, tailored for fault classification.

Result: The proposed BYOL-IMNet achieves 99.89% test accuracy and an inference time of 5.7 ms per image, outperforming existing state-of-the-art models.

Conclusion: The study demonstrates the effectiveness of the CNN-BYOL hybrid method, particularly the proposed BYOL-IMNet, in achieving high accuracy (99.89%) and fast inference time (5.7 ms per image) for fault detection in induction motors. This method offers a robust solution for online monitoring in industrial applications.
...
TL;DR: MLH combines center-based and pairwise-based hashing methods through mutual learning and a mixture-of-hash-experts module, achieving better performance in image retrieval.


<details>
  <summary>Details</summary>
Motivation: Center-based methods often underutilize local similarity information, while pairwise-based methods excel in preserving local relationships but lack global structure modeling. MLH aims to bridge this gap.

Method: MLH introduces a weak-to-strong framework with two branches: a strong center-based branch and a weaker pairwise-based branch, coupled with a mixture-of-hash-experts module for cross-branch interaction.

Result: MLH outperforms state-of-the-art hashing methods across multiple benchmark datasets.

Conclusion: The proposed Mutual Learning for Hashing (MLH) framework effectively combines the strengths of center-based and pairwise-based methods, achieving superior performance in large-scale image retrieval.
...
TL;DR: Repainter, a reinforcement learning framework, improves e-commerce image inpainting by integrating spatial-matting and GRPO, outperforming existing methods and supported by a new dataset and benchmark.


<details>
  <summary>Details</summary>
Motivation: Intrusive elements like watermarks and promotional text in e-commerce product images hinder user engagement. Existing diffusion-based inpainting methods struggle with unreliable object removal and domain-specific adaptation.

Method: The proposed Repainter framework combines spatial-matting trajectory refinement with Group Relative Policy Optimization (GRPO), modulating attention mechanisms to emphasize background context. A composite reward mechanism balances global, local, and semantic constraints.

Result: Repainter significantly outperforms state-of-the-art methods, particularly in complex scenes, and reduces visual artifacts and reward hacking. The framework is supported by the EcomPaint-100K dataset and EcomPaint-Bench benchmark.

Conclusion: Repainter, a reinforcement learning framework integrating spatial-matting trajectory refinement and GRPO, outperforms state-of-the-art methods in e-commerce inpainting, as demonstrated by extensive experiments. The release of EcomPaint-100K and EcomPaint-Bench further supports future research.
...
TL;DR: SyncHuman combines 2D and 3D generative models for high-quality 3D human reconstruction from single images, outperforming baselines in accuracy and fidelity.


<details>
  <summary>Details</summary>
Motivation: The task of photorealistic 3D full-body human reconstruction from a single image is challenging due to ambiguities and self-occlusions, with existing methods struggling with inaccurate 3D priors and difficult poses.

Method: SyncHuman integrates a 2D multiview generative model and a 3D native generative model, using pixel-aligned 2D-3D synchronization attention and a feature injection mechanism to enhance details.

Result: Extensive experiments show SyncHuman achieves high-quality clothed human mesh reconstruction from single-view images, even under challenging poses, with improved geometric accuracy and visual fidelity.

Conclusion: SyncHuman demonstrates robust and photo-realistic 3D human reconstruction, outperforming baseline methods in geometric accuracy and visual fidelity, and presents a promising direction for future 3D generation models.
...
TL;DR: ComGS introduces Surface Octahedral Probes (SOPs) for efficient relightable object reconstruction and simplifies lighting estimation for 3D object-scene composition, achieving real-time rendering at 28 FPS.


<details>
  <summary>Details</summary>
Motivation: Realistic 3D object-scene composition in Gaussian Splatting (GS) is challenging due to baked appearance and shadow inconsistencies. Existing methods are inefficient or struggle with complex lighting.

Method: Uses SOPs for efficient lighting and occlusion storage, avoiding ray tracing. Simplifies lighting estimation by focusing on object placement and fine-tuning a diffusion model for environment lighting.

Result: Achieves high-quality, real-time rendering (~28 FPS), harmonious results with vivid shadows, and fast editing (36 seconds).

Conclusion: ComGS provides an efficient and effective solution for 3D object-scene composition in GS, addressing key challenges in relightable reconstruction and lighting estimation.
...
TL;DR: UltraLED uses a single short-exposure RAW image to reconstruct UHDR scenes, avoiding ghosting and excelling in detail recovery, supported by a novel dataset and framework.


<details>
  <summary>Details</summary>
Motivation: UHDR scenes with extreme exposure disparities pose challenges in preserving both highlight and shadow details. RGB-based methods suffer from misalignment and ghosting, while RAW images offer higher potential due to their bit depth and noise characteristics.

Method: The study introduces UltraLED, a two-stage framework involving exposure correction via a ratio map and a brightness-aware RAW denoiser. A 9-stop bracketing pipeline synthesizes realistic UHDR images for dataset creation.

Result: UltraLED significantly outperforms existing single-frame approaches in UHDR reconstruction, as demonstrated by extensive experiments.

Conclusion: UltraLED, a two-stage framework using a single short-exposure RAW image, effectively addresses UHDR reconstruction by avoiding ghosting and motion blur, outperforming existing single-frame methods.
...
TL;DR: A novel framework combines low-framerate RGB and high-framerate event data to optimize Dynamic 3DGS, using event motion priors and geometry-aware data association, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Reconstructing Dynamic 3DGS from low-framerate RGB videos is challenging due to large inter-frame motions, which increase solution uncertainty. Event cameras provide high-framerate motion data but lack color information, motivating their combination with RGB data.

Method: The framework jointly optimizes dynamic 3DGS using RGB and event modalities by extracting motion priors from event streams via the LoCM unsupervised fine-tuning framework and employing geometry-aware data association for event-Gaussian motion correspondence.

Result: Extensive experiments show the method outperforms existing approaches in synthetic and real scenes, proving the effectiveness of event data in optimizing dynamic 3DGS.

Conclusion: The paper demonstrates that combining low-temporal-resolution RGB images with high-framerate event streams, guided by event motion priors, effectively optimizes Dynamic 3D Gaussian Splatting (3DGS), outperforming existing methods.
...
TL;DR: The study used XAI to enhance brain tumor segmentation in MRI images, finding ResUNet as the best-performing model. Grad-CAM and attention-based visualization improved interpretability, aiding clinical decision-making.


<details>
  <summary>Details</summary>
Motivation: The research aimed to improve the accuracy of brain tumor segmentation in MRI images using Explainable AI (XAI) to assist physicians in clinical decision-making and increase their trust in AI models.

Method: The study evaluated three deep learning models (UNet, ResUNet, and AttUNet) for brain tumor segmentation using the BraTS2020 dataset. XAI techniques like Grad-CAM and attention-based visualization were applied to enhance model interpretability. Performance was assessed based on training, validation, and inference times, segmentation similarity coefficients, loss functions, and classification metrics.

Result: ResUNet outperformed UNet and AttUNet in terms of Dice and Jaccard similarity scores, accuracy, recall, and F1 scores. Grad-CAM and attention-based visualization provided insights into model decisions and mechanisms.

Conclusion: The study recommends the use of ResUNet for automated brain tumor segmentation in clinical assessments due to its superior performance in terms of Dice and Jaccard similarity scores, accuracy, recall, and F1 scores. The source code and checkpoints are made available for future use.
...
TL;DR: GTR-Bench introduces a novel benchmark for geographic temporal reasoning in VLMs, highlighting their deficiencies in spatial-temporal intelligence compared to humans.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to assess VLMs' geographic spatial-temporal intelligence with both images/video and graphics context, crucial for applications like traffic management and emergency response.

Method: The authors propose GTR-Bench, a challenge requiring perspective switches between maps and videos, joint reasoning across non-overlapping videos, and inference over unobserved regions.

Result: Evaluations show top VLMs (e.g., Gemini-2.5-Pro at 34.9%) significantly lag behind humans (78.61%) and reveal three key deficiencies: imbalanced context utilization, weak temporal forecasting, and poor map-video alignment.

Conclusion: GTR-Bench provides insights and opportunities for improving spatial-temporal intelligence in VLMs, with benchmark and code to be released.
...
TL;DR: The paper introduces MM-COF, a motion representation integrating both micro-expression phases, and FMANet, a neural network for adaptive motion fusion, outperforming existing methods in recognition tasks.


<details>
  <summary>Details</summary>
Motivation: Micro-expression recognition is challenging due to subtle facial movements, and existing methods often overlook motion information in the apex-to-offset phase.

Method: Proposes MM-COF for comprehensive motion representation and FMANet, an end-to-end network with learnable modules for adaptive motion fusion and classification.

Result: Outperforms existing methods on standard benchmarks (MMEW, SMIC, CASME-II, SAMM).

Conclusion: The dual-phase framework and learnable modules in FMANet advance micro-expression recognition, demonstrating superior performance.
...
TL;DR: A framework for depth estimation from monocular 360° indoor panoramas using room geometry constraints, improving accuracy by integrating layout prediction and background segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing methods focus on pixel-level accuracy but oversmooth room corners and are noise-sensitive, prompting the need for a geometry-aware approach.

Method: A shared feature encoder followed by task-specific decoders for layout estimation, depth estimation, and background segmentation, enhanced by room-geometry-based depth resolving and segmentation-guided fusion.

Result: Superior performance on Stanford2D3D, Matterport3D, and Structured3D datasets compared to open-source methods.

Conclusion: The proposed framework effectively leverages room geometry and segmentation to improve depth estimation accuracy in indoor panoramas.
