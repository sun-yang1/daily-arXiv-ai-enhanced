...
TL;DR: WorldVQA is a new benchmark that evaluates MLLMs' atomic visual world knowledge by decoupling memorization from reasoning, testing their ability to ground and name visual entities across a stratified taxonomy from common to rare objects.


<details>
  <summary>Details</summary>
Motivation: Current evaluations conflate visual knowledge retrieval with reasoning, making it difficult to measure what models actually memorize versus what they reason about. There's a need for a benchmark that strictly assesses the atomic visual world knowledge and factuality of MLLMs.

Method: WorldVQA is designed to decouple visual knowledge retrieval from reasoning capabilities. It assesses atomic capabilities of grounding and naming visual entities across a stratified taxonomy that spans from common head-class objects to long-tail rarities.

Result: The benchmark provides a rigorous test for visual factuality in MLLMs, allowing for measurement of encyclopedic breadth and hallucination rates across models.

Conclusion: WorldVQA establishes a standard for assessing the visual knowledge and hallucination rates of current and future frontier models by focusing on atomic memorization capabilities rather than conflated reasoning tasks.
...
TL;DR: AdaptMMBench is a new benchmark for evaluating adaptive multimodal reasoning in VLMs, focusing on dynamic difficulty assessment and meta-cognition abilities across five domains, with special metrics for mode selection rationality and process evaluation.


<details>
  <summary>Details</summary>
Motivation: Current evaluations for adaptive multimodal reasoning rely on static difficulty labels and simplistic metrics that fail to capture dynamic difficulty relative to model capabilities, obscuring the distinction between adaptive mode selection and general performance while neglecting fine-grained process analysis.

Method: Proposes AdaptMMBench benchmark covering five domains (real-world, OCR, GUI, knowledge, math) with tasks ranging from direct perception to complex reasoning. Uses Matthews Correlation Coefficient (MCC) to evaluate selection rationality of reasoning modes by dynamically identifying task difficulties based on models' capability boundaries. Enables multi-dimensional process evaluation across key step coverage, tool effectiveness, and computational efficiency.

Result: Evaluation reveals that adaptive mode selection scales with model capacity but decouples from final accuracy. Key step coverage aligns with performance, but tool effectiveness remains highly inconsistent across different model architectures.

Conclusion: AdaptMMBench provides a comprehensive framework for evaluating adaptive multimodal reasoning, addressing limitations of existing benchmarks by focusing on dynamic difficulty assessment, meta-cognition abilities, and multi-dimensional process analysis to better understand VLM capabilities in adaptive reasoning scenarios.
...
TL;DR: SVD-ViT uses singular value decomposition to help Vision Transformers focus on foreground features while suppressing background noise and artifacts, improving classification accuracy.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers lack an explicit mechanism to distinguish foreground from background, causing them to learn unnecessary background features and artifacts that degrade classification performance.

Method: SVD-ViT leverages singular value decomposition with three components: SPC module, SSVA, and ID-RSVD to extract and aggregate singular vectors capturing object foreground information while suppressing task-irrelevant factors like background noise and artifacts.

Result: The method improves classification accuracy and effectively learns informative foreground representations while reducing the impact of background noise, as demonstrated in experimental results.

Conclusion: SVD-ViT successfully addresses ViT's foreground/background discrimination limitation through SVD-based feature prioritization, leading to better foreground representation learning and classification performance.
...
TL;DR: A self-supervised framework for OR video anonymization avoids manual annotation and calibration, achieving high recall and real-time performance.


<details>
  <summary>Details</summary>
Motivation: Existing anonymization methods require manual annotations and camera calibration, limiting scalability in multi-camera OR setups.

Method: The framework combines whole-body person detection and pose estimation, leveraging temporal and multi-view context to retrieve false negatives and self-supervised domain adaptation.

Result: The approach achieves over 97% recall on simulated and real surgery datasets, with a real-time detector showing comparable performance.

Conclusion: The proposed self-supervised multi-view video anonymization framework effectively addresses scalability bottlenecks in privacy preservation for OR research, achieving high recall and practical applicability.
...
TL;DR: A novel two-stage token pruning framework called Nüwa that preserves spatial integrity for better visual grounding performance in Vision Language Models, achieving state-of-the-art results on VQA and significant improvements on VG tasks.


<details>
  <summary>Details</summary>
Motivation: Existing vision token pruning methods work well for VQA tasks but suffer substantial degradation on visual grounding tasks. The authors discovered that current pruning strategies lose global spatial reference frames derived from tokens' positional interactions, which is crucial for grounding tasks that require spatial understanding.

Method: A two-stage pruning framework: 1) After the vision encoder, apply three operations (separation, alignment, aggregation) inspired by swarm intelligence algorithms to retain information-rich global spatial anchors, 2) Within the LLM, perform text-guided pruning to keep task-relevant visual tokens.

Result: Achieves state-of-the-art performance on multiple VQA benchmarks (improving from 94% to 95%) and yields substantial improvements on visual grounding tasks (from 7% to 47%).

Conclusion: Nüwa effectively addresses the spatial reference loss problem in existing pruning methods by maintaining spatial integrity through swarm intelligence-inspired operations, enabling efficient feature aggregation while significantly improving performance on visual grounding tasks.
...
TL;DR: This paper develops analytical formulas to calculate depth and range errors in fisheye stereo vision systems, with particular focus on accuracy at wide viewing angles.


<details>
  <summary>Details</summary>
Motivation: Fisheye stereo vision systems have applications requiring wide field of view, but their accuracy, especially at large angles, has not been well characterized mathematically. Understanding error behavior is crucial for applications like robotics, surveillance, and navigation.

Method: The study derives analytical expressions (mathematical formulas) for depth and range error as functions of object distance, specifically focusing on the error characteristics at large angles in fisheye stereo vision systems.

Result: The paper presents closed-form analytical expressions that quantify depth and range errors in fisheye stereo systems, showing how these errors vary with object distance and are particularly pronounced at large viewing angles.

Conclusion: Analytical error models for fisheye stereo vision provide valuable tools for system design, calibration, and performance prediction, especially for applications requiring wide-angle coverage where traditional pinhole models are insufficient.
...
TL;DR: SceneLinker is a framework that generates compositional 3D scenes from RGB sequences using semantic scene graphs, addressing the need for Mixed Reality content that adapts to individual user spaces by capturing real-world layout and object relationships.


<details>
  <summary>Details</summary>
Motivation: To enable adaptive Mixed Reality (MR) content that reflects real-world layouts, there's a need to generate 3D scenes that capture semantic cues from surroundings. Prior methods struggled to fully capture contextual relationships between objects or focused too much on shape synthesis, making it difficult to generate 3D scenes aligned with object arrangements in real environments.

Method: SceneLinker uses a graph network with cross-check feature attention for scene graph prediction, combined with a graph-variational autoencoder (graph-VAE) that includes a joint shape and layout block for 3D scene generation. The system takes RGB sequences as input and outputs compositional 3D scenes through semantic scene graph understanding.

Result: Experiments on 3RScan/3DSSG and SG-FRONT datasets show that SceneLinker outperforms state-of-the-art methods in both quantitative and qualitative evaluations, even in complex indoor environments and challenging scene graph constraints. The framework demonstrates strong performance in generating consistent 3D spaces from physical environments.

Conclusion: SceneLinker successfully enables users to generate consistent 3D spaces from their physical environments via scene graphs, allowing creation of spatial MR content. The framework addresses key limitations of prior works by better capturing contextual relationships between objects and generating scenes aligned with real-world object arrangements.
...
TL;DR: IVC-Prune is a training-free, prompt-aware pruning method that identifies and retains implicit visual coordinate tokens and semantically relevant foreground tokens, reducing visual tokens by ~50% while maintaining ≥99% of original LVLM performance.


<details>
  <summary>Details</summary>
Motivation: LVLMs face prohibitive inference costs with high-resolution visual inputs. Existing token pruning methods focus on semantic relevance but discard tokens crucial for spatial reasoning, creating a gap in spatial reasoning capabilities.

Method: IVC-Prune identifies implicit visual coordinate tokens by analyzing RoPE's mathematical properties (finding positions where rotation matrices approximate identity or 90° rotation matrices). It also identifies foreground tokens through a two-stage process: semantic seed discovery followed by contextual refinement via value-vector similarity.

Result: The method reduces visual tokens by approximately 50% while maintaining ≥99% of original performance across four LVLMs and twenty benchmarks, with some benchmarks even showing performance improvements.

Conclusion: IVC-Prune successfully addresses the spatial reasoning gap in LVLM token pruning by retaining both IVC tokens and semantically relevant foreground tokens, providing an effective solution for reducing inference costs while preserving performance.
