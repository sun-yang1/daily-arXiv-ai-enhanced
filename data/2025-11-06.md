...
TL;DR: Geospatial embeddings simplify cropland mapping, achieving high accuracy and supporting climate impact assessments.


<details>
  <summary>Details</summary>
Motivation: Accurate and up-to-date land cover maps are essential for understanding land use change, a key driver of climate change. Geospatial embeddings offer a more efficient and accessible way to map landscape features.

Method: Produced cropland maps using embeddings from Presto and AlphaEarth.

Result: Geospatial embeddings can simplify workflows and achieve high-accuracy cropland classification.

Conclusion: Geospatial embeddings simplify workflows and achieve high-accuracy cropland classification, supporting better assessments of land use change and its climate impacts.
...
TL;DR: Generative hints, a semi-supervised training method using virtual examples from a generative model, outperforms standard data augmentation by enforcing invariances across the input space, improving accuracy on fine-grained classification tasks.


<details>
  <summary>Details</summary>
Motivation: Standard data augmentation alone fails to fully capture invariant properties, as it only learns from transformations of training data. Generative hints aim to enforce these invariances more effectively.

Method: The approach uses a generative model to create virtual examples (unlabeled images) from the training set. These examples are used to enforce functional properties (hints) in a semi-supervised manner, combining classification and hint objectives.

Result: Generative hints consistently outperform data augmentation, achieving up to 1.78% top-1 accuracy improvement on fine-grained benchmarks and an average boost of 1.286% on the CheXpert dataset.

Conclusion: Generative hints provide a more effective way to learn invariant properties compared to standard data augmentation, demonstrating significant accuracy improvements across diverse datasets and architectures.
...
TL;DR: ProM3E is a probabilistic masked multimodal embedding model for ecology, enabling any-to-any generation of multimodal representations, superior cross-modal retrieval, and effective modality fusion.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of any-to-any generation of multimodal representations for ecology, focusing on learning what to fuse and enabling cross-modal retrieval.

Method: ProM3E employs masked modality reconstruction in the embedding space to learn missing modalities from context modalities, supporting modality inversion and probabilistic fusion analysis.

Result: The model achieves superior performance in cross-modal retrieval tasks and demonstrates strong representation learning capabilities through linear probing.

Conclusion: ProM3E demonstrates superior performance in cross-modal retrieval and representation learning, with its probabilistic and masked multimodal embedding approach enabling effective modality fusion and inversion.
...
TL;DR: EvtSlowTV, a large-scale event camera dataset from YouTube, enhances event-based depth estimation by providing diverse, unconstrained data for self-supervised learning, improving model generalization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the constraints of small-scale annotated datasets in event-based depth estimation, which limit generalizability to real-world scenarios.

Method: The paper introduces EvtSlowTV, a dataset curated from publicly available YouTube footage, containing over 13B events across diverse conditions. It leverages a self-supervised learning framework to utilize the HDR potential of raw event streams without frame-based annotations.

Result: Training with EvtSlowTV improves model generalization to complex scenes and motions, demonstrating its suitability for self-supervised learning.

Conclusion: The introduction of EvtSlowTV, a large-scale event camera dataset, successfully addresses the limitations of small-scale annotated datasets, enhancing the generalizability of event-based depth estimation models to complex real-world scenarios.
...
TL;DR: A hybrid CNN-ViT search space for NAS is introduced, enabling efficient tinyML architectures that outperform ResNet models in accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: Hybrid CNN-ViT architectures, while effective, are computationally expensive and unsuitable for tinyML. This work aims to find efficient hybrid architectures for deployment in resource-constrained environments.

Method: A new hybrid CNN-ViT search space for NAS is introduced, incorporating hybrid blocks for local and global information learning and a novel Pooling block for efficient feature map reduction.

Result: Experiments on CIFAR10 demonstrate that the proposed search space yields hybrid CNN-ViT architectures with better accuracy and inference speed than ResNet-based tinyML models.

Conclusion: The proposed hybrid CNN-ViT search space for NAS successfully produces efficient architectures for tinyML, outperforming ResNet-based models in accuracy and speed under tight constraints.
...
TL;DR: SCALE-VLP enhances vision-language models for volumetric data by integrating spatial and clinical semantics, achieving superior performance in retrieval, classification, and report generation, with strong cross-domain generalization.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for vision-language models (VLMs) often treat volumetric scans as independent 2D slices, compromising spatial coherence and underutilizing clinical semantics. The paper aims to address these limitations by preserving anatomical structure and leveraging rich clinical knowledge.

Method: The paper proposes SCALE-VLP, a soft-weighted contrastive vision-language pre-training framework that integrates volumetric spatial semantics and domain-aware, knowledge-infused semantics (e.g., radiological ontologies) to guide alignment.

Result: SCALE-VLP achieves up to 4.3x higher top-1 CT-report retrieval, improves abnormality classification by 10 points, and reaches ROUGE-L 0.44 and BERT-F1 0.89 for report generation. It also shows consistent gains in zero-shot evaluations on out-of-domain datasets.

Conclusion: SCALE-VLP demonstrates strong cross-task and cross-domain generalization, achieving significant improvements in CT-report retrieval, abnormality classification, and report generation, even in zero-shot evaluations.
...
TL;DR: A label-efficient method using self-supervised learning achieves high accuracy for 1-m land cover classification with minimal annotated data.


<details>
  <summary>Details</summary>
Motivation: The challenge of collecting large training datasets for high-resolution land cover mapping limits widespread adoption of deep learning models.

Method: Utilizes self-supervised pre-training with unlabeled images, transfers weights to segmentation models, and fine-tunes with small annotated datasets.

Result: Achieved 87.14% overall accuracy and 75.58% macro F1 score for 8-class land cover mapping over Mississippi.

Conclusion: Self-supervised learning effectively reduces the need for large annotated datasets, enabling scalable high-resolution land cover mapping.
...
TL;DR: A foundation model for brain MRI uses a single encoder with learnable modality embeddings and masked autoencoding to handle missing or unseen imaging sequences, trained on 60,000 MRIs for flexible representation learning.


<details>
  <summary>Details</summary>
Motivation: The need for separate models for each MRI modality and the challenge of missing or unseen sequences motivate a unified approach.

Method: The model employs learnable modality embeddings, conditional layer normalization, and a masked autoencoding objective with a variance-covariance regularizer for stable and diverse feature learning.
...
Conclusion: The model offers a flexible, unified solution for brain MRI analysis, with code and pretrained models publicly available.
...
TL;DR: SLIP introduces structural pretraining by modeling relationships between image-text pairs in graphs, outperforming CLIP on cross-modal tasks.


<details>
  <summary>Details</summary>
Motivation: Current VLP methods treat image-text pairs as isolated examples, ignoring rich relational structures present in domains like e-commerce and social networks. Human knowledge encoding through relationship cognitive maps inspired this approach.

Method: SLIP integrates a structural contrastive loss to align modalities while modeling relationships between neighboring entities in a structured graph. A large-scale Amazon Product Co-purchase Multimodal Graph Dataset was constructed for supervision.
...
Conclusion: Relational supervision provides significant value for cross-modal alignment, demonstrating that structural awareness improves vision-language pretraining performance.
...
TL;DR: A learning-based framework for MLS point clouds reduces reliance on ground truth by using geometric features, with XGBoost outperforming Random Forest in efficiency.


<details>
  <summary>Details</summary>
Motivation: Ground truth (GT) for uncertainty evaluation is often costly and infeasible, prompting the need for a GT-independent method.

Method: The framework integrates optimal neighborhood estimation with geometric feature extraction, using XGBoost and Random Forest models for evaluation.

Result: The XGBoost model achieves comparable accuracy to Random Forest but is about 3 times faster, validating the feasibility of using geometric features to predict point-level uncertainty.

Conclusion: The study demonstrates that uncertainty in Mobile Laser Scanning (MLS) point clouds is learnable, introducing a novel learning-based approach for uncertainty evaluation.
...
TL;DR: A computational imaging framework combining CS and LSM with advanced denoising techniques effectively reconstructs dynamic cellular structures in the beating heart, validated by zebrafish experiments.


<details>
  <summary>Details</summary>
Motivation: Traditional optical imaging struggles with the trade-off between spatial and temporal resolution in capturing dynamic cellular structures in the beating heart. This work aims to overcome these limitations for efficient, low-phototoxic cardiac imaging.

Method: The framework integrates Compressive Sensing (CS) with Light-Sheet Microscopy (LSM), using a Digital Micromirror Device (DMD) for compressed acquisition. A Plug-and-Play (PnP) framework, solved via ADMM, incorporates advanced denoisers (Tikhonov, TV, BM3D) and temporal regularization for smoothness between z-slices.

Result: Experimental results on zebrafish heart imaging under high compression ratios show successful reconstruction of cellular structures with high denoising performance and clarity.

Conclusion: The proposed computational imaging framework successfully reconstructs cellular structures in the beating heart with excellent denoising performance and image clarity, proving its effectiveness and robustness in high-speed, low-light biological imaging.
...
TL;DR: ISC-Perception is a hybrid dataset for ISC component detection, combining synthetic and real images, reducing human effort by 81.7%, and improving detection accuracy for construction robotics.


<details>
  <summary>Details</summary>
Motivation: The absence of a dedicated image corpus for ISC component detection hampers dependable perception in robotic steel-frame assembly, due to logistical and safety challenges in collecting real photographs on construction sites.

Method: The paper introduces ISC-Perception, a hybrid dataset combining procedurally rendered CAD images, game-engine photorealistic scenes, and curated real photographs, with fully automatic labelling for the synthetic portion.

Result: Detectors trained on ISC-Perception achieved a mean Average Precision at IoU 0.50 of 0.756, outperforming models trained on synthetic-only or photorealistic-only data. Bench test results showed mAP@0.50/mAP@[0.50:0.95] of 0.943/0.823.

Conclusion: ISC-Perception bridges the data gap for construction-robotics perception, enabling rapid development of custom object detectors and improving worker safety by eliminating manual assembly.
...
TL;DR: DentalSplat improves 3D reconstruction from sparse orthodontic images using prior-guided initialization, pruning, and optical flow, achieving better results than existing methods.


<details>
  <summary>Details</summary>
Motivation: Orthodontic cases often involve only three sparse images, making 3D reconstruction challenging due to limited viewpoints and lack of camera pose information.

Method: The framework combines a prior-guided dense stereo reconstruction model for point cloud initialization, a scale-adaptive pruning strategy, and optical flow with gradient regularization for enhanced rendering fidelity.

Result: Experimental validation on 950 clinical cases and 195 video-based test cases shows superior performance in handling sparse inputs and improving reconstruction quality.

Conclusion: DentalSplat effectively addresses the challenges of 3D reconstruction from sparse orthodontic imagery, outperforming existing methods in novel view synthesis for dental occlusion visualization.
...
TL;DR: IC DefectNCD is a novel framework for defect detection and classification in IC manufacturing, using intrinsic image priors and adaptive strategies to outperform traditional methods.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of supervised methods (requiring extensive human annotation) and clustering-based unsupervised methods (unstable performance due to missing priors) in detecting and classifying defects in IC manufacturing.

Method: The method involves Self Normal Information Guided IC Defect Detection for coarse defect localization, an adaptive binarization strategy for stable subimages, and Self Defect Information Guided IC Defect Classification with a soft mask guided attention mechanism for enhanced sensitivity to defects.

Result: Experimental results show robust performance in defect detection and classification of unseen defects on a real-world dataset covering three fabrication stages and 15 defect types.

Conclusion: The paper concludes that IC DefectNCD, a support set-free framework leveraging Image Intrinsic Priors, effectively addresses defect detection and novel class discovery in IC SEM images. It demonstrates robust performance on real-world datasets across multiple fabrication stages and defect types.
...
TL;DR: \sysname accelerates vision-guided physical property reasoning from minutes to seconds using optimizations like 3D reconstruction and parallel encoding, achieving high speed and accuracy in real-world tests.


<details>
  <summary>Details</summary>
Motivation: The motivation is to accelerate vision-guided physical property reasoning for augmented visual cognition, addressing the high latency (10--20 minutes) of existing pipelines.

Method: The system employs algorithmic and systematic optimizations, including rapid geometric 3D reconstruction, efficient semantic feature fusion, and parallel view encoding, to minimize run-time latency.

Result: \sysname reduces end-to-end latency to less than 6 seconds, achieving a 62.9×--287.2× speedup. It matches or surpasses SOTA baselines in accuracy for object-level physical property estimation, material segmentation, and voxel-level inference. Real-world case studies confirm its robustness.

Conclusion: The paper concludes that \sysname significantly reduces latency in vision-guided physical property reasoning while maintaining or improving accuracy. It also demonstrates robust performance in real-world scenarios, such as cluttered environments, and integrates effectively with gaze-tracking for practical applications like smart glasses.
...
TL;DR: This paper introduces the first operational AI/ML system for automating building damage assessment in sUAS imagery, addressing data overload in disaster response. The model, trained on a large dataset, was successfully deployed during two hurricanes, assessing 415 buildings in 18 minutes.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the overwhelming volume of sUAS imagery collected during disasters, which exceeds the capacity for manual analysis, delaying response efforts. Automated damage assessment is needed to address this challenge.

Method: The method involved training models on the largest known dataset of post-disaster sUAS imagery (21,716 building damage labels) and operational training of 91 disaster practitioners. The best-performing model was deployed during Hurricanes Debby and Helene.

Result: The deployed model assessed 415 buildings in approximately 18 minutes during Hurricanes Debby and Helene, showcasing its efficiency and practicality.

Conclusion: The paper successfully establishes the state of practice for AI/ML-based building damage assessment using sUAS imagery, demonstrating operational deployment during disasters and contributing valuable lessons to the AI/ML and disaster response communities.
...
TL;DR: A fine-tuning-free method for personalizing text-to-image diffusion models using Hypernetworks to predict LoRA-adapted weights, enhanced by Hybrid-Model Classifier-Free Guidance (HM-CFG) for better compositional generalization.


<details>
  <summary>Details</summary>
Motivation: Traditional methods like DreamBooth are computationally expensive and slow. Adapter- and encoder-based methods still require fine-tuning or large models. This work aims to reduce overhead while maintaining performance.

Method: Uses Hypernetworks to predict LoRA-adapted weights directly from subject images, stabilized by output regularization. Introduces HM-CFG for improved compositional generalization during inference.

Result: Achieves strong personalization performance on CelebA-HQ, AFHQ-v2, and DreamBench without per-subject optimization.

Conclusion: Demonstrates hypernetworks as a scalable and effective solution for open-category personalization, balancing subject fidelity and prompt alignment.
...
TL;DR: A depth-guided liver landmark segmentation framework using SAM2 and DA2 encoders, enhanced by SRFT-GaLore for efficient fine-tuning, improves accuracy and robustness in surgical settings.


<details>
  <summary>Details</summary>
Motivation: Accurate detection and delineation of anatomical structures in laparoscopic liver surgery are challenging due to limited depth perception in 2D video streams. Existing methods struggle with fusing RGB and depth features and adapting large-scale vision models efficiently.

Method: The framework integrates semantic and geometric cues using SAM2 and DA2 encoders, employs SRFT-GaLore for efficient fine-tuning, and uses a cross-attention fusion module to combine RGB and depth features.

Result: The method achieves a 4.85% improvement in Dice Similarity Coefficient and an 11.78-point reduction in Average Symmetric Surface Distance on the L3D dataset, with competitive performance on the new LLSD dataset.

Conclusion: The proposed SRFT-GaLore-enhanced dual-encoder framework demonstrates scalable and precise segmentation in real-time, depth-constrained surgical settings, with strong cross-dataset robustness.
...
TL;DR: The paper introduces PitVQA-Anticipation, the first VQA dataset for forward-looking surgical reasoning, and SurgAnt-ViVQA, a model that advances surgical VQA by incorporating temporal awareness and gated cross-attention, outperforming baselines and highlighting the importance of temporal modeling.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of existing VQA systems in surgical settings, which focus on static vision-language alignment and current scenes rather than anticipating future events. The need for proactive assistance in endonasal transsphenoidal pituitary surgery, where visibility is limited and workflow changes rapidly, drives the development of a forward-looking VQA dataset and model.

Method: The authors propose SurgAnt-ViVQA, a video-language model that adapts a large language model using a GRU Gated Temporal Cross-Attention module. This includes a bidirectional GRU for encoding frame-to-frame dynamics and an adaptive gate for injecting visual context into the language stream at the token level. Parameter-efficient fine-tuning customizes the language backbone for the surgical domain.

Result: SurgAnt-ViVQA outperforms strong image and video-based baselines on the PitVQA-Anticipation and EndoVis datasets. Ablation studies reveal that temporal recurrence and gated fusion contribute significantly to performance gains. A frame budget study shows a trade-off between fluency (maximized at 8 frames) and numeric time estimation (improved at 32 frames).

Conclusion: The paper concludes that SurgAnt-ViVQA, with its temporally aware encoder and fine-grained gated cross-attention, significantly advances surgical VQA from retrospective description to proactive anticipation. The PitVQA-Anticipation dataset serves as a comprehensive benchmark, emphasizing the importance of targeted temporal modeling for reliable future-aware surgical assistance.
...
TL;DR: PETWB-REP is a curated dataset of 490 patients' PET/CT scans and reports, supporting AI and clinical research in oncology.


<details>
  <summary>Details</summary>
Motivation: Large-scale medical imaging datasets combining functional and anatomical imaging with detailed clinical reports are scarce, hindering AI model development and retrospective clinical research.

Method: The dataset includes whole-body 18F-FDG PET/CT scans and corresponding radiology reports from 490 patients with various malignancies, along with structured clinical metadata.

Result: PETWB-REP offers paired PET/CT images, de-identified reports, and clinical metadata for cancers like lung, liver, breast, prostate, and ovarian cancer.

Conclusion: The PETWB-REP dataset addresses the scarcity of combined functional and anatomical imaging with clinical reports, providing a valuable resource for AI model development and clinical research in oncology.
...
TL;DR: The paper introduces QG-CoC, a zero-shot prompting method for MLLMs, improving multi-image reasoning by addressing gaps in perception and integration.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs lack fine-grained perception and reasoning capabilities in multi-image contexts, creating a gap in addressing complex tasks.

Method: The study investigates current prompting methods and introduces a zero-shot prompting method called Question-Guided Chain-of-Captions (QG-CoC) for handling multi-image tasks.

Result: QG-CoC outperforms existing methods in multi-image benchmarks and shows robust improvements in challenging scenarios.

Conclusion: The proposed QG-CoC method demonstrates competitive performance and robust improvements in multi-image reasoning tasks, addressing the limitations of existing prompting methods.
...
TL;DR: The paper proposes MvBody, a multi-view-based Transformer network, to predict cesarean section (CS) risk using self-reported medical data and 3D body scans, achieving high accuracy and AUC-ROC.


<details>
  <summary>Details</summary>
Motivation: Early and reliable CS risk prediction is crucial in resource-limited settings, but existing models rely on unavailable parameters. This study explores affordable alternatives using 3D body shape.

Method: The authors develop MvBody, incorporating metric learning loss for efficiency and generalizability, and use Integrated Gradients for explainability.

Result: MvBody outperforms other models with 84.62% accuracy and 0.724 AUC-ROC, identifying key predictors like pre-pregnancy weight, maternal age, and body shape.

Conclusion: The study demonstrates the feasibility of using 3D body shape for CS risk prediction, offering a promising tool for resource-limited settings.
...
TL;DR: A novel augmentation method (MCPMix + RLA) combines label-preserving mixing and diffusion-driven diversity, achieving top segmentation results by mitigating domain shift and soft label issues.


<details>
  <summary>Details</summary>
Motivation: Current augmentation methods for dense prediction, such as sample mixing or generative synthesis, have limitations like soft label ambiguity or synthetic-real domain shift. The authors aim to combine the strengths of both while mitigating their weaknesses.

Method: The paper introduces Mask-Consistent Paired Mixing (MCPMix) and Real-Anchored Learnable Annealing (RLA). MCPMix generates synthetic counterparts for real images under the same mask, mixing appearance while preserving hard masks. RLA adaptively adjusts mixing strength and loss weights to re-anchor optimization to real data.

Result: The approach achieves state-of-the-art segmentation performance on datasets like Kvasir-SEG, PICCOLO, CVC-ClinicDB, a private NPC-LES cohort, and ISIC 2017, with consistent gains over baselines.

Conclusion: The proposed approach, combining label-preserving mixing with diffusion-driven diversity and adaptive re-anchoring, achieves state-of-the-art segmentation performance and robust generalizability across multiple datasets.
...
TL;DR: T-PMambaSR combines window-based self-attention and Progressive Mamba for efficient super-resolution, outperforming existing methods with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing Mamba-based super-resolution methods lack fine-grained transitions across modeling scales, limiting feature representation efficiency.

Method: The paper proposes T-PMambaSR, integrating window-based self-attention with Progressive Mamba to enable fine-grained modeling across scales. It also introduces an Adaptive High-Frequency Refinement Module (AHFRM) to recover lost high-frequency details.

Result: Extensive experiments show T-PMambaSR enhances receptive field and expressiveness progressively, achieving better performance with lower computational cost.

Conclusion: T-PMambaSR outperforms recent Transformer- and Mamba-based methods with lower computational cost, demonstrating its effectiveness in super-resolution tasks.
...
TL;DR: DMPO decouples low-level and high-level features in early stages via a bypass module and decoupled optimization, improving inference efficiency.


<details>
  <summary>Details</summary>
Motivation: The challenge of early stages providing both low-level fundamental features to deep stages and high-level discriminative features to early-stage predictors remains unresolved.

Method: The DMPO method introduces a lightweight bypass module for functional decomposition of shallow features and a high-order statistics-based predictor for early stages. It employs a decoupled optimization strategy with two-phase loss weights during model tuning.

Result: Experiments show that DMPO outperforms its counterparts in reducing computational cost while maintaining performance.

Conclusion: The proposed DMPO method effectively decouples low-level representative and high-level discriminative abilities in early stages, achieving superior performance in inference efficiency across various datasets and pre-trained backbones.
...
TL;DR: A generative method for translating ultrasound CFD-greyscale videos produces realistic synthetic videos, indistinguishable from real ones in DL tasks and clinical evaluations, enhancing dataset utility and design.


<details>
  <summary>Details</summary>
Motivation: Ultrasound data often suffers from imbalance and missingness, particularly with sub-modalities like greyscale and color flow doppler (CFD). Image translation can help balance datasets, but this is challenging for ultrasound sub-modalities.

Method: The method leverages pixel-wise, adversarial, and perceptual losses, utilizing two networks: one for reconstructing anatomic structures and another for denoising to achieve realistic ultrasound imaging. It was trained on 54,975 videos and tested on 8,368.

Result: The synthetic videos achieved an average pairwise SSIM of 0.91±0.04 with ground truth. They performed indistinguishably from real videos in DL classification and segmentation tasks (F1 score: 0.9 vs. 0.89; Dice score: 0.97). Clinicians had 54±6% accuracy in distinguishing real vs. synthetic videos, indicating high realism. The model also generalized well to other clinical domains (average SSIM 0.91±0.05).

Conclusion: The study demonstrates that the generative method for ultrasound CFD-greyscale video translation produces realistic synthetic videos, indistinguishable from real ones in DL tasks and clinical evaluations. This expands the utility of retrospectively collected imaging and enhances dataset design for medical imaging.
...
TL;DR: A hybrid U-Mamba model with Heat Conduction Equation improves medical image segmentation by efficiently modeling global context and long-range dependencies, outperforming baselines on CT and MRI datasets.


<details>
  <summary>Details</summary>
Motivation: Existing models struggle with efficient global context modeling and long-range dependency reasoning under practical computational budgets, prompting the need for a novel hybrid architecture.

Method: The model integrates Mamba-based state-space modules for long-range reasoning and Heat Conduction Operators (HCOs) in bottleneck layers to simulate frequency-domain thermal diffusion, enhancing semantic abstraction.

Result: Experimental results on multimodal abdominal CT and MRI datasets show that the proposed model consistently outperforms strong baselines, demonstrating its effectiveness and generalizability.

Conclusion: The proposed hybrid architecture, combining U-Mamba with Heat Conduction Equation, offers a scalable and interpretable solution for medical image segmentation by blending state-space dynamics with heat-based global diffusion.
...
TL;DR: The paper introduces IEC3D-AD, a high-fidelity 3D anomaly detection dataset for industrial components, and GMANet, a novel method that enhances detection accuracy through geometric analysis and spatial optimization.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of existing 3D datasets in capturing real industrial complexities, which hinder precise anomaly detection for critical industrial components.

Method: The method involves creating a high-fidelity point cloud dataset (IEC3D-AD) and introducing a novel 3D-AD paradigm (GMANet) that uses geometric morphological analysis and spatial discrepancy optimization to enhance anomaly detection.

Result: Experimental results show that GMANet effectively improves anomaly detection on IEC3D-AD and other datasets, demonstrating superior performance.

Conclusion: The paper concludes that the proposed IEC3D-AD dataset and GMANet paradigm significantly improve 3D anomaly detection for industrial equipment components, outperforming existing methods.
...
TL;DR: A novel method for long video inpainting/outpainting using LoRA and temporal co-denoising achieves high fidelity and controllability, outperforming baselines in quality and realism.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of generating long videos with high controllability in inpainting and outpainting, the paper aims to overcome limitations of prior work, such as fixed-length clips and stitching artifacts.

Method: The approach leverages LoRA for fine-tuning a pre-trained video diffusion model (Wan 2.1) and employs an overlap-and-blend temporal co-denoising strategy with high-order solvers for consistency in long sequences.

Result: The method demonstrates superior performance in inpainting/outpainting tasks over hundreds of frames, outperforming baseline models (Wan 2.1 and VACE) in quality (PSNR/SSIM) and perceptual realism (LPIPS).

Conclusion: The proposed method successfully achieves controllable long video inpainting and outpainting with high fidelity, outperforming baseline methods in quality and realism while maintaining efficiency.
...
TL;DR: The paper introduces Diffusion-SDPO, a safeguarded update rule for diffusion-based Direct Preference Optimization (DPO) to address issues where enlarging preference margins degrades generation quality. It ensures non-increasing error for preferred outputs and shows consistent improvements in benchmarks.


<details>
  <summary>Details</summary>
Motivation: Aligning text-to-image diffusion models with human preferences is challenging, as standard Diffusion-DPO can degrade both preferred and less-preferred outputs.

Method: The authors propose Diffusion-SDPO, which adaptively scales the loser gradient to preserve the winner's quality, backed by a first-order analysis for guaranteed error control.

Result: Diffusion-SDPO outperforms baselines in automated preference, aesthetic, and prompt alignment metrics.

Conclusion: Diffusion-SDPO is a simple, model-agnostic solution that improves alignment without significant computational overhead, validated by benchmarks.
