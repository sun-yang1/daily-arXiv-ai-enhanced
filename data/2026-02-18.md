...
TL;DR: GRAFNet, a biologically inspired neural network, addresses polyp segmentation challenges in colonoscopy by mimicking the human visual system, achieving state-of-the-art performance with improved accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Polyp segmentation in colonoscopy is challenging due to morphological variability, visual similarity to normal structures, and multi-scale detection needs, which current deep learning methods inadequately address.

Method: GRAFNet integrates three modules: GAAM for boundary emphasis, MSRM for multi-scale feature analysis, and GCAFM for iterative refinement, unified in a Polyp Encoder-Decoder Module (PEDM).

Result: Achieves 3-8% Dice improvement and 10-20% higher generalization on five benchmarks (Kvasir-SEG, CVC-300, CVC-ColonDB, CVC-Clinic, PolypGen), with interpretable decision pathways.

Conclusion: GRAFNet bridges AI accuracy with clinically trustworthy reasoning, setting a new paradigm in polyp segmentation.
...
TL;DR: Proposes a decoupled framework for zero-shot HOI detection, using MLLMs and deterministic generation for better generalization and integration with any object detector.


<details>
  <summary>Details</summary>
Motivation: Existing methods for HOI detection struggle with unseen interactions due to tight coupling with specific detectors and coarse-grained VLM features.

Method: Decouples object detection from IR, leverages MLLMs, deterministic generation, and introduces spatial-aware pooling and one-pass matching.

Result: Achieves superior zero-shot performance and strong cross-dataset generalization on HICO-DET and V-COCO.

Conclusion: Our method offers flexibility and state-of-the-art performance in zero-shot HOI detection.
...
TL;DR: A novel AI approach using contrastive and prototype learning improves ovarian cancer subtype classification and localization while maintaining scalability.


<details>
  <summary>Details</summary>
Motivation: The study addresses the challenge of increasing diagnostic workloads in UK pathology departments and the trade-off between accuracy and scalability in traditional AI approaches.

Method: The approach leverages contrastive and prototype learning with pre-computed, frozen features via feature-space augmentations.

Result: The method achieves improvements of 70.4% and 15.3% in F1 score for instance- and slide-level classification, respectively, and AUC gains of 16.9% for instance localization and 2.3% for slide classification over DSMIL.

Conclusion: The proposed method demonstrates significant improvements in subtype classification and localization for ovarian cancer histopathology images while maintaining scalability by using frozen patch features.
...
TL;DR: A model-agnostic method detects annotation errors in video datasets using Cumulative Sample Loss (CSL) to identify mislabeled or disordered frames, improving dataset reliability.


<details>
  <summary>Details</summary>
Motivation: High-quality video datasets are essential for training robust models, but annotation errors (mislabeling, disordering) hinder performance, especially in tasks requiring temporal consistency.

Method: Proposes analyzing CSL—average loss per frame across training epochs—to flag mislabeled or disordered frames. Frames with consistently high CSL are identified as likely errors.

Result: Effectively detects subtle annotation errors like mislabeling and disordering in datasets (EgoPER, Cholec80) without needing ground truth on errors.

Conclusion: The CSL-based method offers a reliable, generalizable tool for auditing video datasets and enhancing training reliability in machine learning.
...
TL;DR: A distributional learning model enhances super-resolution for 4D Flow MRI, addressing domain shift by training on CFD simulations and fine-tuning on real data.


<details>
  <summary>Details</summary>
Motivation: Conventional super-resolution methods often fail due to domain shift when applied to real-world clinical data with acquisition mechanisms differing from simple downsampling.

Method: A distributional deep learning framework is developed, initially trained on high-resolution CFD simulations and downsampled counterparts, then fine-tuned on paired 4D Flow MRI and CFD samples.

Result: The framework significantly outperforms traditional deep learning approaches, demonstrating improved robustness and domain generalization.

Conclusion: The proposed distributional deep learning framework effectively addresses domain shift and enhances super-resolution performance in clinical settings, outperforming traditional deep learning methods.
...
TL;DR: The paper proposes a neural volume rendering method for camera virtualization, addressing challenges in dynamic scene rendering and enabling time-archival for live events.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome limitations of current methods (e.g., dependency on accurate 3D point clouds, inability to handle rapid motions) and provide efficient time-archival capabilities for applications like sports broadcasting.

Method: The method models dynamic scenes as rigid transformations across synchronized camera views, performing neural representation learning for improved rendering.

Result: The approach achieves photorealistic rendering with temporal coherence and supports time-archival, allowing users to revisit past instances of dynamic scenes for novel view synthesis.

Conclusion: The paper concludes that their neural volume rendering approach offers enhanced visual quality and supports time-archival, enabling retrospective rendering for live events, which is lacking in existing methods.
...
TL;DR: First large-scale study on training long-context vision language models (up to 344K), achieving SOTA on MMLongBenchDoc with insights on context matching, page indices, and bidirectional transfer.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in reproducible training recipes and data pipelines for long-context vision language models, focusing on long-document visual question answering and bidirectional context transfer.

Method: The study employs continued pretraining, supervised finetuning, and preference optimization for 24B and 32B parameter models, supported by extensive evaluations and ablations.

Result: State-of-the-art performance on MMLongBenchDoc, with additional findings on context length matching, page indices, synthetic data utility, and bidirectional transfer.

Conclusion: The study successfully trains long-context vision language models up to 344K context, achieving state-of-the-art performance on MMLongBenchDoc. Key findings include insights on context length matching, page indices, synthetic data pipelines, and bidirectional text-visual context transfer. MMLBD-C is released to improve benchmark quality.
...
TL;DR: E^2D introduces targeted updates for efficient large-scale dataset distillation, outperforming SOTA in speed and accuracy.


<details>
  <summary>Details</summary>
Motivation: Address the efficiency-accuracy trade-off in existing decoupling-based distillation methods.

Method: E^2D employs a two-phase optimization strategy: exploration (uniform updates) and exploitation (focused updates on high-loss regions).

Result: Achieves 18x faster performance on ImageNet-1K and 4.3x faster on ImageNet-21K with improved accuracy.

Conclusion: E^2D bridges the gap between accuracy and efficiency in large-scale dataset distillation through targeted updates, outperforming state-of-the-art methods in speed and performance.
...
TL;DR: A framework for studying visual preferences of VLMs through controlled image-based choice tasks and systematic input perturbations.


<details>
  <summary>Details</summary>
Motivation: Understanding the visual preferences of vision-language models (VLMs) to improve auditing and governance of image-based AI agents.

Method: Using visual prompt optimization and systematically edited images to infer VLMs' latent visual utility through revealed preference.

Result: Optimized edits significantly shift choice probabilities, revealing consistent visual themes driving VLM selection.

Conclusion: The approach offers a proactive way to identify visual vulnerabilities and safety concerns in VLMs.
...
TL;DR: Proposes a joint-sampling framework for diverse and temporally consistent text-to-video generation without costly backpropagation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of generating diverse yet temporally consistent videos efficiently without expensive backpropagation.

Method: The proposed method involves diversity-driven updates followed by filtering out components that harm temporal consistency, using lightweight latent-space models to avoid costly video decoding.

Result: Experiments demonstrate comparable diversity to baselines while significantly improving temporal consistency and naturalness.

Conclusion: The paper concludes with a joint-sampling framework that enhances batch diversity in text-to-video generation while maintaining temporal consistency, validated by experiments showing improved results.
...
TL;DR: A training-free framework extends zero-shot anomaly detection to 3D brain MRI by aggregating 2D slices into volumetric tokens, enabling robust and practical anomaly detection without supervision.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of extending zero-shot anomaly detection to 3D medical images, which existing methods fail to do effectively due to reliance on slice-wise features and vision-language models.

Method: The method involves constructing localized volumetric tokens by aggregating multi-axis slices processed by 2D foundation models. These 3D patch tokens restore cubic spatial context and integrate directly with distance-based, batch-level anomaly detection pipelines.

Result: Results demonstrate that the proposed framework successfully extends zero-shot anomaly detection from 2D to 3D MRI volumes, offering practical computation on standard GPUs without requiring fine-tuning or supervision.

Conclusion: The paper concludes that training-free, batch-based zero-shot anomaly detection can be effectively extended from 2D encoders to full 3D MRI volumes, providing a simple and robust approach for volumetric anomaly detection.
...
TL;DR: Sparrow speeds up Vid-LLMs by addressing attention dilution and visual redundancy, using hidden state reuse and intermediate-layer training.


<details>
  <summary>Details</summary>
Motivation: Address performance collapse in Vid-LLMs caused by attention dilution, negative visual gain, and context window mismatches.

Method: Utilizes visually-aware text-anchored window attention and intermediate-layer visual state bridging, along with a multi-token prediction strategy.

Result: Achieves an average speedup of 2.82x even with 25k visual tokens.

Conclusion: Sparrow effectively resolves performance degradation in long sequences for Vid-LLMs, offering a practical solution for real-time long video tasks.
...
TL;DR: EventMemAgent: An active online video agent with hierarchical memory and Agentic RL, outperforming passive methods.


<details>
  <summary>Details</summary>
Motivation: Addresses the conflict between unbounded streaming media input and MLLMs' limited context window, improving trade-offs between long-range context and fine-grained details.

Method: Uses a hierarchical memory module with short-term memory for event boundary detection and reservoir sampling, and long-term memory for structured archiving. Integrates multi-granular perception and Agentic RL.

Result: Achieves competitive results on online video benchmarks.

Conclusion: EventMemAgent demonstrates competitive performance on online video benchmarks, leveraging hierarchical memory and Agentic RL to overcome challenges in continuous perception and long-range reasoning.
...
TL;DR: New MAIL and Robust-MAIL networks address limitations in multimodal fusion learning, improving performance, efficiency, and robustness.


<details>
  <summary>Details</summary>
Motivation: Existing Multimodal Fusion Learning (MFL) methods suffer from limited generalizability, high computational costs, and vulnerability to adversarial attacks.

Method: The authors introduce Multi-Attention Integration Learning (MAIL) with residual learning attention blocks and multimodal cross-attention modules. Robust-MAIL adds random projection filters and modulated attention noise.

Result: MAIL and Robust-MAIL achieve performance gains of up to 9.34% and reduce computational costs by up to 78.3% across 20 public datasets.

Conclusion: The proposed MAIL and Robust-MAIL networks outperform existing methods by improving performance, reducing computational costs, and enhancing adversarial robustness.
...
TL;DR: The CREMD dataset explores how different presentation modes and annotator characteristics affect dog emotion recognition, revealing key insights on context, audio, and annotator demographics.


<details>
  <summary>Details</summary>
Motivation: Accurate dog emotion recognition is essential for improving human-animal interactions, veterinary care, and automated monitoring, but it is hindered by subjective assessments and lack of standardized methods.

Method: The study uses the CREMD dataset, featuring 923 video clips in three modes (no context/audio, context/no audio, context/audio) and analyzes annotations from diverse participants.

Result: Visual context improves agreement, audio effects are inconclusive, non-owners/males show higher agreement, and audio boosts confidence in identifying anger and fear.

Conclusion: Context and annotator demographics significantly impact dog emotion recognition, with audio enhancing confidence for specific emotions, though audio's role requires further study.
...
TL;DR: DAV-GSWT uses diffusion models and active sampling to create high-quality Gaussian Splatting Wang Tiles from sparse inputs, reducing data needs.


<details>
  <summary>Details</summary>
Motivation: Existing methods for photorealistic neural rendering are limited by dense sample requirements. DAV-GSWT aims to address this inefficiency.

Method: The framework integrates diffusion priors, active view sampling, and hierarchical uncertainty quantification to synthesize Gaussian Splatting Wang Tiles from minimal observations.

Result: The system significantly reduces data volume needs without compromising visual quality or interactive performance.

Conclusion: DAV-GSWT effectively reduces the reliance on dense input data while maintaining high-fidelity rendering and seamless transitions in large-scale environments.
...
TL;DR: The paper introduces GMAIL, a framework that treats generated images as a separate modality from real ones, using multi-modal learning to align them in latent space. This approach improves performance in various vision-language tasks.


<details>
  <summary>Details</summary>
Motivation: To address the modality discrepancies between real and synthetic images and avoid mode collapse, while leveraging generative models for training machine learning models.

Method: Fine-tune a model on generated images with cross-modality alignment loss, then use it to train vision-language models.

Result: Improved performance in image captioning, zero-shot tasks, and scaling trends, with notable enhancements in LLaVA's captioning.

Conclusion: GMAIL effectively bridges real and synthetic modalities, enhancing vision-language model performance with generated images.
...
TL;DR: Novel framework detects and suppresses semantic hallucinations in day-to-night image translation, improving performance significantly.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of large appearance shifts and semantic hallucinations in day-to-night image translation, which degrade downstream task performance.

Method: Utilizes a dual-head discriminator for hallucination detection and class-specific prototypes to suppress hallucinations within a Schrodinger Bridge-based translation model.

Result: Outperforms existing methods, achieving a 15.5% mAP improvement on BDD100K dataset and a 31.7% gain for hallucination-prone classes like traffic lights.

Conclusion: The proposed framework effectively suppresses semantic hallucinations in day-to-night unpaired image translation, significantly improving downstream task performance.
...
TL;DR: ASBM improves diffusion models by learning optimal trajectories via Schrödinger Bridge dynamics, achieving straighter paths and better image generation.


<details>
  <summary>Details</summary>
Motivation: Addresses the inefficiency and noise in diffusion models due to memoryless forward processes, aiming for straighter and more optimal trajectories.

Method: Proposes Adjoint Schrödinger Bridge Matching (ASBM), a two-stage framework: first learns the SB forward dynamic via data-to-energy sampling, then trains the backward generative dynamic using a matching loss.

Result: ASBM achieves straighter sampling paths, better stability, and higher efficiency in high-dimensional tasks like image generation, outperforming prior works.

Conclusion: ASBM successfully improves the efficiency and fidelity of diffusion models by learning optimal trajectories in a non-memoryless regime, showcasing its potential for high-dimensional data generation.
...
TL;DR: Open-source MLLMs show strong zero-shot morphing attack detection, with LLaVA1.6-Mistral-7B outperforming task-specific models by 23%.


<details>
  <summary>Details</summary>
Motivation: To explore the underexplored potential of MLLMs in biometric forensics, particularly for detecting morphing attacks without task-specific training.

Method: Systematic zero-shot evaluation of MLLMs using public weights and standardized protocols.

Result: LLaVA1.6-Mistral-7B outperforms task-specific MAD baselines by 23% in EER, showing MLLMs' ability to detect morphing artifacts without fine-tuning.

Conclusion: Open-source MLLMs demonstrate strong zero-shot MAD performance, suggesting their potential as reproducible and interpretable tools for biometric security.
...
TL;DR: RPT-SR improves infrared image super-resolution by encoding scene layout into attention, outperforming general-purpose models with a dual-token framework.


<details>
  <summary>Details</summary>
Motivation: General-purpose super-resolution models, including Vision Transformers, are inefficient for infrared imaging scenarios with fixed viewpoints, failing to exploit persistent spatial priors. This leads to redundant learning and suboptimal performance.

Method: The paper proposes a Regional Prior attention Transformer (RPT-SR) that uses a dual-token framework to fuse regional prior tokens (for global structure) with local tokens (for frame-specific content). This allows dynamic modulation of the reconstruction process.

Result: Extensive experiments show RPT-SR achieves state-of-the-art performance across diverse datasets covering both Long-Wave (LWIR) and Short-Wave (SWIR) spectra.

Conclusion: RPT-SR introduces a novel dual-token framework that effectively leverages persistent spatial priors in infrared imaging, achieving state-of-the-art performance across diverse datasets.
...
TL;DR: LEADER is a lightweight, end-to-end deep learning model for fingerprint minutiae extraction, achieving top accuracy and efficiency with innovative architecture.


<details>
  <summary>Details</summary>
Motivation: To address the scarcity of truly end-to-end minutiae extraction methods in fingerprint recognition, eliminating separate preprocessing and postprocessing steps.

Method: LEADER integrates non-maximum suppression, angular decoding, and a dual-autoencoder structure with attention-gating. It uses a novel ground-truth encoding called "Castle-Moat-Rampart."

Result: LEADER achieves a 34% higher F1-score on NIST SD27, outperforms commercial software in efficiency (15ms on GPU, 322ms on CPU), and ranks first in 47% of samples.

Conclusion: LEADER achieves state-of-the-art accuracy and computational efficiency in minutiae extraction, outperforming specialized methods and demonstrating robust cross-domain generalization.
...
TL;DR: A semantic filtering framework using vision-language models effectively removes transient objects in 3DGS reconstructions, improving quality without significant memory overhead.


<details>
  <summary>Details</summary>
Motivation: Transient objects in multi-view captures cause ghosting artifacts in 3DGS, and existing methods are either memory-intensive or unreliable due to parallax ambiguity.

Method: The framework uses CLIP similarity scores to classify and remove transient Gaussians through opacity regularization and pruning.

Result: Experiments on RobustNeRF showed consistent quality improvement over vanilla 3DGS, with minimal memory overhead and real-time rendering.

Conclusion: Semantic filtering is a practical solution for transient removal in scenarios with predictable distractor categories.
...
TL;DR: The paper introduces a comprehensive evaluation measure, 'advanced acceptance score,' to assess biometric gesture scores, outperforming existing methods in selecting optimal scores and showing reliability through correlation with other measures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of measures for evaluating the quality of biometric gesture scores, as existing methods rely on error rates, which do not indicate score goodness.

Method: The method involves identifying ranking order and relevance of scores, considering rank deviation, rewards for high/low-ranked gestures, and compensating for score trends. It integrates these elements with weighting to form the advanced acceptance score.

Result: Experiments on three datasets with five SOTA models show the proposed measure selects more appropriate optimal scores and correlates with existing measures, validating its reliability.

Conclusion: The advanced acceptance score provides a holistic and reliable evaluation of biometric gesture scores, addressing limitations of existing methods.
...
TL;DR: A dynamic training-free framework for LoRA fusion improves subject-style synthesis by leveraging KL divergence and gradient-based corrections, outperforming static methods.


<details>
  <summary>Details</summary>
Motivation: Existing LoRA fusion methods use static heuristics, deviating from LoRA's adaptive learning purpose and ignoring input randomness, prompting the need for a dynamic solution.

Method: A dynamic fusion framework that computes KL divergence for feature-level selection and applies gradient-based corrections using CLIP and DINO scores during the generation process.

Result: The method consistently outperforms state-of-the-art LoRA fusion techniques qualitatively and quantitatively across diverse subject-style combinations.

Conclusion: The proposed dynamic training-free fusion framework effectively achieves coherent subject-style synthesis without retraining, outperforming existing LoRA fusion methods.
...
TL;DR: PADE, a training-free attention intervention, leverages internal Positive Attention Dynamics in LVLMs to reduce hallucinations and improve visual grounding, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the issue of hallucinations in LVLMs, where outputs are inconsistent with visual inputs or user instructions, and the limitations of existing training-free methods that are computationally expensive or prone to interference.

Method: The proposed method, PADE, constructs a PAD map to identify semantically core visual regions, uses per-head Median Absolute Deviation Scaling for adaptive intervention strength, and employs System-Token Compensation to maintain consistency with complex user instructions.

Result: Experiments on multiple LVLMs and benchmarks demonstrate that PADE enhances visual grounding and reduces hallucinations.

Conclusion: The paper concludes that Positive Attention Dynamics Enhancement (PADE) effectively improves visual grounding and reduces hallucinations in LVLMs by leveraging internal attention dynamics, validating its utility for reliable multimodal reasoning.
...
TL;DR: A fully automated pipeline for OCT vessel segmentation and classification achieves high accuracy using machine learning.


<details>
  <summary>Details</summary>
Motivation: Challenges like noise, artifacts, and complex structures in OCT images necessitate automated solutions for accurate vessel segmentation and classification.

Method: The pipeline includes image preprocessing, guidewire artifact removal, polar-to-Cartesian transformation, K-means clustering, and local feature extraction for training classifiers.

Result: High performance metrics (precision, recall, F1-score up to 1.00, 99.68% accuracy) demonstrate the method's effectiveness.

Conclusion: The proposed method offers a reliable and efficient solution for automated OCT image analysis, with potential clinical applications.
...
TL;DR: IRIS-v2 dataset aids alignment of schematics with 2D/3D scenes in industrial facilities, improving efficiency via segmentation and graph matching.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of manually aligning schematics with scene data in industrial facilities, which is tedious, complex, and lacks scalable solutions.

Method: The paper introduces IRIS-v2, a dataset with images, point clouds, annotations, CAD models, and P&ID diagrams, and experiments with segmentation and graph matching for alignment.

Result: The result is IRIS-v2, a comprehensive dataset, and a practical case study showing reduced alignment time using segmentation and graph matching.

Conclusion: The paper concludes that IRIS-v2 provides a valuable dataset for advancing research in aligning functional schematics with 2D and 3D scene acquisitions, demonstrating practical improvements through segmentation and graph matching.
...
TL;DR: CEMRAG improves RRG by combining interpretable clinical concepts with multimodal RAG, enhancing both accuracy and transparency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address limitations in Radiology Report Generation (RRG) through VLMs, such as lack of interpretability and hallucination of findings, by unifying interpretability and factual grounding into a single framework.

Method: The study introduces Concept-Enhanced Multimodal RAG (CEMRAG), a framework that decomposes visual representations into interpretable clinical concepts and integrates them with multimodal RAG.

Result: Experiments demonstrate consistent improvements in clinical accuracy and NLP measures over traditional RAG and concept-only baselines across multiple datasets and configurations.

Conclusion: The paper concludes that transparent visual concepts can enhance diagnostic accuracy in medical VLMs without compromising interpretability, challenging the assumed trade-off between the two.
...
TL;DR: A new strawberry ripeness dataset was created and tested with YOLO models, showing YOLOv8s performs best overall, aiding smart agriculture.


<details>
  <summary>Details</summary>
Motivation: Traditional visual assessments for strawberry ripeness are subjective and error-prone. The lack of comprehensive datasets in literature hinders comparison and advancement in computer-assisted systems.

Method: A new publicly available dataset of 566 images with 1,201 labeled objects was created under variable conditions. Comparative tests were conducted using YOLOv8, YOLOv9, and YOLO11-based models.

Result: YOLOv9c achieved the highest precision (90.94%), YOLO11s the highest recall (83.74%), and YOLOv8s the best overall performance (86.09% mAP@50).

Conclusion: The study establishes a fundamental reference point for smart agriculture applications, demonstrating that small and medium-sized YOLO models perform efficiently and balancedly on the strawberry ripeness dataset.
