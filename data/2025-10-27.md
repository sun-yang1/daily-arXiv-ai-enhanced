...
TL;DR: Shortcut-Rerouted Adapter Training improves text-to-image synthesis by routing confounding factors through auxiliary modules, enhancing generation quality and prompt adherence.


<details>
  <summary>Details</summary>
Motivation: Adapters in image generation often entangle target attributes with incidental ones, limiting generalization and prompt adherence.

Method: Proposes Shortcut-Rerouted Adapter Training, where confounding factors are routed through auxiliary modules (e.g., ControlNet or LoRA) during training and removed during inference.

Result: Improves generation quality, diversity, and prompt adherence in tasks like facial and full-body identity injection.

Conclusion: The approach suggests a design principle for disentangled representations: establish shortcuts for what should not be learned.
...
TL;DR: Video-As-Prompt (VAP) introduces a unified, generalizable approach for semantic-controlled video generation by using reference videos as prompts, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for semantic control in video generation suffer from artifacts or lack generalizability, prompting the need for a more robust solution.

Method: VAP leverages a reference video as a semantic prompt, guiding a frozen Video Diffusion Transformer via a Mixture-of-Transformers expert, with temporally biased position embedding for robust context retrieval.

Result: VAP achieves a 38.7% user preference rate, rivaling commercial models, and demonstrates strong zero-shot generalization.

Conclusion: VAP represents a significant advance toward general-purpose, controllable video generation, supported by the VAP-Data dataset for future research.
...
TL;DR: FM-BFF-Net combines CNNs and transformers with focal modulation and bidirectional fusion, improving medical image segmentation performance across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation is critical for clinical applications, but existing methods struggle with global context and long-range dependencies, especially for complex structures.

Method: The proposed FM-BFF-Net integrates convolutional and transformer components, using focal modulation attention and bidirectional feature fusion to enhance context awareness and feature interaction.

Result: FM-BFF-Net achieves higher Jaccard index and Dice coefficient compared to recent methods, proving its robustness across various medical imaging tasks.

Conclusion: FM-BFF-Net demonstrates superior performance in medical image segmentation, outperforming state-of-the-art methods across diverse datasets, confirming its effectiveness and adaptability.
...
TL;DR: GenPT introduces a generative framework for multi-modal point tracking, outperforming discriminative models in occluded scenarios while maintaining competitive accuracy on visible points.


<details>
  <summary>Details</summary>
Motivation: Current discriminative models for point tracking fail to capture multi-modality in trajectories, especially under uncertainty like occlusions.

Method: GenPT uses a flow matching formulation with iterative refinement, a window-dependent prior, and a tuned variance schedule for point coordinates. It employs best-first search on generated samples during inference.

Result: GenPT achieves state-of-the-art tracking accuracy on occluded points and remains competitive on visible points.

Conclusion: GenPT effectively models multi-modality in point trajectories, offering improved performance in occluded scenarios without sacrificing accuracy on visible points.
...
TL;DR: The paper introduces 3DReasonKnee, a dataset for 3D grounded reasoning in medical images, addressing VLMs' limitations in anatomical grounding and step-by-step reasoning, validated by clinicians and benchmarked with five VLMs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in current VLMs' ability to ground anatomical regions in 3D medical images and reason step-by-step, which is crucial for aligning AI outputs with clinical workflows.

Method: The authors introduce 3DReasonKnee, a dataset with 494k high-quality quintuples derived from 7,970 3D knee MRI volumes, including diagnostic questions, 3D bounding boxes, clinician-generated reasoning steps, and severity assessments. They also establish ReasonKnee-Bench to evaluate VLMs.

Result: The results include the creation of 3DReasonKnee, validated with over 450 hours of expert clinician input, and benchmarking of five state-of-the-art VLMs on ReasonKnee-Bench.

Conclusion: The paper concludes that 3DReasonKnee is a valuable resource for advancing multimodal medical AI systems by providing expert-annotated 3D reasoning pathways, enabling clinically aligned and localized decision-making.
...
TL;DR: A novel method for 3D shape reconstruction using thermal polarization cues, independent of illumination and material properties, outperforming existing techniques.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D shape reconstruction are often dependent on illumination and material properties, limiting their effectiveness.

Method: The paper formulates a theory of polarization observation using LWIR polarimetric imaging and proposes a method for 3D shape recovery from multi-view thermal polarimetric images.

Result: The approach successfully reconstructs fine details in transparent, translucent, and heterogeneous objects, outperforming current techniques.

Conclusion: The proposed method offers a robust and illumination-independent solution for detailed 3D shape reconstruction, particularly for challenging materials.
...
TL;DR: VESSA is a self-supervised adaptation method for vision models, using multi-view videos to improve performance without labels, outperforming existing approaches.


<details>
  <summary>Details</summary>
Motivation: Current vision foundation models underperform in domains with distribution shifts and scarce labels, where supervised fine-tuning is infeasible.

Method: VESSA employs a self-distillation paradigm, leveraging multi-view object-centric videos for adaptation, while carefully tuning prediction heads and using parameter-efficient techniques to retain pretrained knowledge.

Result: VESSA outperforms base models and previous adaptation methods in downstream classification tasks, validated through experiments on 3 vision foundation models and 2 datasets.

Conclusion: VESSA introduces a novel self-supervised fine-tuning method for vision foundation models, demonstrating consistent improvements in downstream tasks without requiring annotations.
...
TL;DR: A plug-in pipeline for 2D detection in industrial settings enhances accuracy for 6D pose estimation by addressing domain shift and false positives, validated on real-world benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing pipelines for 6D pose estimation degrade under challenging conditions like clutter, poor lighting, and complex backgrounds, making detection a critical bottleneck.

Method: The approach combines low-light image enhancement, background removal, and open-vocabulary detection with foundation models to reduce domain shift and suppress false positives in raw SAM outputs.

Result: Extensive experiments on real-world industrial benchmarks show a significant boost in detection accuracy.

Conclusion: The proposed method significantly improves detection accuracy for 6D pose estimation in industrial settings, demonstrating effectiveness and practicality with negligible inference overhead.
...
TL;DR: Deep learning models automate post-earthquake structural damage detection, using labeled datasets and YOLOv11 to assess exposed steel reinforcement and damage levels.


<details>
  <summary>Details</summary>
Motivation: Timely assessment of structural integrity after seismic events is critical for public safety and emergency response, necessitating automated and reliable damage detection methods.

Method: The study employs deep learning, specifically YOLOv11 models, enhanced with fine-tuning and data augmentation, to detect exposed steel reinforcement and classify structural damage levels. A hybrid framework integrates these models for automated damage assessment.

Result: The proposed framework successfully detects exposed steel bars and classifies structural damage levels, proving effective across diverse damage contexts.

Conclusion: The research demonstrates that rapid and automated damage detection post-disasters is feasible using image data and deep learning, providing a reliable tool for assessing structural integrity.
...
TL;DR: ZING-3D is a zero-shot framework for open-vocabulary 3D scene graph generation, enabling incremental updates and geometric grounding, validated on Replica and HM3D datasets.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D scene graph generation lack incremental updates, geometric grounding, and multi-view support, which are essential for embodied scenarios.

Method: The framework leverages pretrained VLMs to generate 2D scene graphs, which are grounded in 3D using depth information. Nodes represent objects with features, 3D locations, and semantic context, while edges capture spatial and semantic relations.

Result: Experiments on Replica and HM3D datasets demonstrate ZING-3D's effectiveness in capturing spatial and relational knowledge without task-specific training.

Conclusion: ZING-3D effectively captures spatial and relational knowledge in 3D environments without task-specific training, making it suitable for robotics applications.
...
TL;DR: WaveSeg introduces a novel decoder architecture for semantic segmentation, optimizing feature refinement in spatial and wavelet domains to balance semantic context and detail preservation.


<details>
  <summary>Details</summary>
Motivation: Current semantic segmentation networks rely on powerful encoders but use simplistic decoders, leading to suboptimal trade-offs between semantic context and fine-grained detail preservation.

Method: WaveSeg employs high-frequency component learning, Dual Domain Operation (DDO), Spectrum Decomposition Attention (SDA) blocks, and reparameterized convolutions to refine features in both spatial and wavelet domains.

Result: WaveSeg outperforms state-of-the-art methods on standard benchmarks, achieving efficient and precise segmentation.

Conclusion: WaveSeg's innovative approach, combining wavelet-domain frequency priors with Mamba-based attention, effectively enhances segmentation performance.
...
TL;DR: A novel framework integrating expert-derived textual concepts into a vision-language model improves Hirschsprung's disease diagnosis, outperforming CNNs with 83.9% accuracy.


<details>
  <summary>Details</summary>
Motivation: Deep learning approaches like CNNs, while effective, are often opaque and may not align with physician decision-making. The study aims to improve interpretability and clinical relevance by incorporating expert knowledge.

Method: The proposed framework integrates expert-derived textual concepts into a Contrastive Language-Image Pre-training-based vision-language model to guide plexus classification. Expert prompts, generated by large language models and reviewed, are encoded with QuiltNet to align clinically relevant semantic cues with visual features.

Result: The model outperformed CNN-based models (VGG-19, ResNet-18, ResNet-50), achieving an accuracy of 83.9%, precision of 86.6%, and specificity of 87.6%.

Conclusion: The study highlights the potential of multi-modal learning in histopathology and emphasizes the value of integrating expert knowledge for clinically relevant model outputs.
...
TL;DR: HistRetinex, a histogram-based Retinex model, offers faster low-light image enhancement with better results, saving significant time compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing Retinex-based methods are time-consuming for large-sized images, prompting the need for a faster solution.

Method: The paper extends the Retinex model to the histogram domain, defining histogram location and count matrices. A two-level optimization model is constructed, and iterative formulas for illumination and reflectance histograms are derived. The low-light image is enhanced by histogram matching.

Result: HistRetinex demonstrates superior performance in visibility and metrics, with notable speed improvements.

Conclusion: The proposed HistRetinex method outperforms existing enhancement methods in visibility and performance metrics, achieving significant time savings (1.86 seconds for 1000*664 resolution images, saving at least 6.67 seconds).
...
TL;DR: The paper introduces Active Visual Reasoning (AVR) to address limitations of static visual reasoning in MLLMs by enabling agents to interact with partially observable environments, integrating perception, reasoning, and action. It presents CLEVR-AVR as a benchmark and PhysVLM-AVR as a state-of-the-art model.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs are limited to static, fully observable settings, which hinders their effectiveness in real-world scenarios where information is incomplete. Humans, however, actively explore environments to gather information, inspiring the need for active visual reasoning.

Method: The authors introduce the AVR task, requiring agents to actively acquire information, integrate observations, and adjust decisions dynamically. They develop CLEVR-AVR, a simulation benchmark, and AVR-152k, a dataset with Chain-of-Thought annotations. PhysVLM-AVR is proposed as a solution.

Result: PhysVLM-AVR achieves state-of-the-art performance on CLEVR-AVR and other benchmarks. Analysis shows current MLLMs struggle with active information acquisition and integration.

Conclusion: The work highlights the importance of active reasoning in MLLMs and demonstrates the effectiveness of PhysVLM-AVR, while identifying gaps in current models' capabilities.
...
TL;DR: An object-centric, uncertainty-aware LiDAR pipeline for city-scale change detection outperforms existing methods, achieving high accuracy and handling splits/merges effectively.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D city map maintenance (e.g., DSM differencing, point-based neural models) are sensitive to biases, require large memory, and fail to handle splits/merges or enforce class consistency. The goal is to develop a robust, object-centric solution for accurate change detection.

Method: The method involves multi-resolution NDT alignment, point-to-plane ICP, height normalization, and a class-constrained bipartite assignment for cross-epoch associations. It uses semantic and instance segmentation, along with geometry proxies, to refine associations while preserving class counts.

Result: The method outperforms Triplet KPConv, with significant improvements in IoU (7.6 points) for decreased objects, demonstrating robustness under partial overlap and sampling variation.

Conclusion: The proposed object-centric, uncertainty-aware pipeline for city-scale LiDAR achieves high accuracy (95.2%), mF1 (90.4%), and mIoU (82.6%), outperforming existing methods like Triplet KPConv, particularly in handling splits, merges, and partial overlaps.
...
TL;DR: Controllable-LPMoE introduces a dynamic priors-based fine-tuning method for efficient segmentation tasks, outperforming 31 SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Large-scale foundation models are computationally expensive to fine-tune, and existing prompt-based methods lack semantic priors, limiting adaptability.

Method: The approach uses a dynamic mixed local priors extractor and a gating network to capture and modulate priors, along with a bi-directional interaction adapter for feature restructuring.

Result: The method demonstrates superior segmentation performance and adaptability across multiple tasks.

Conclusion: Controllable-LPMoE offers an efficient and adaptable solution for fine-tuning large models in segmentation tasks.
...
TL;DR: SafetyPairs is a framework for generating counterfactual image pairs to isolate safety-relevant features, improving evaluation and training of models for fine-grained image safety distinctions.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenge of systematically differentiating between benign and problematic images, as subtle changes can drastically alter safety implications. Existing datasets are coarse and lack specificity in isolating features that drive safety differences.

Method: The paper introduces SafetyPairs, a scalable framework that uses image editing models to generate counterfactual image pairs. These pairs differ only in safety-relevant features, flipping their safety labels while preserving irrelevant details.

Result: The results demonstrate that SafetyPairs effectively creates a benchmark for evaluating vision-language models' weaknesses in distinguishing subtle image differences. It also improves the sample efficiency of training lightweight guard models.

Conclusion: The paper concludes by highlighting the effectiveness of SafetyPairs as both an evaluation benchmark and a data augmentation tool, improving the training of lightweight guard models. It also emphasizes the release of a benchmark with 3,020 SafetyPair images across 9 safety categories, providing a systematic resource for fine-grained image safety analysis.
...
TL;DR: NoisyGRPO, a multimodal RL framework, enhances MLLMs' CoT reasoning by injecting noise into visual inputs and using Bayesian advantage estimation, improving generalization and robustness, particularly in small-scale models.


<details>
  <summary>Details</summary>
Motivation: Existing RL frameworks for improving general CoT reasoning in MLLMs often fail to generalize beyond the training distribution, limiting their effectiveness.

Method: NoisyGRPO introduces a systematic multimodal RL framework with two key components: (1) a noise-injected exploration policy that perturbs visual inputs with Gaussian noise to encourage broader exploration, and (2) Bayesian advantage estimation, which models advantage estimation as a Bayesian inference problem to robustly guide MLLMs.

Result: Experiments show that NoisyGRPO improves generalization and robustness in CoT quality, general capability, and hallucination benchmarks, especially for small-scale MLLMs.

Conclusion: NoisyGRPO significantly enhances the generalization and robustness of multimodal large language models (MLLMs) in Chain-of-Thought (CoT) reasoning tasks, particularly in small-scale settings like Qwen2.5-VL 3B.
...
TL;DR: A CycleGAN-based method generates contrast-enhanced CTPA from NCCT scans, reducing risks of contrast agents while improving vascular visualization and downstream clinical tasks.


<details>
  <summary>Details</summary>
Motivation: CTPA's reliance on iodinated contrast agents poses risks like nephrotoxicity and allergic reactions, especially in high-risk patients. The study aims to mitigate these risks by generating contrast-enhanced images from NCCT scans.

Method: A cascaded synthesizer based on CycleGAN was used to generate DCCTPA from NCCT scans. The model was trained and validated on 249 paired images and tested on 161 paired images.

Result: The method achieved superior performance in quantitative metrics (MAE, PSNR, SSIM) and qualitative visualization. Downstream tasks like vessel segmentation and quantification also showed significant improvements (e.g., Dice scores and ICC values).

Conclusion: The proposed DCCTPA method effectively enhances vascular visualization from NCCT scans, outperforming SOTA methods in both quantitative and qualitative evaluations. It also improves downstream clinical tasks like pulmonary vessel segmentation and vascular quantification.
...
TL;DR: Spatial Intelligence Grid (SIG) is introduced as a structured, grid-based schema to explicitly encode spatial information, improving foundation models' visual-spatial intelligence (VSI) evaluation and performance.


<details>
  <summary>Details</summary>
Motivation: Current methods for evaluating VSI in foundation models rely on textual prompts and VQA-style scoring, which obscure geometry and weaken spatial skill attribution.

Method: SIG is proposed as a complementary channel to text, encoding object layouts, inter-object relations, and grounded priors. SIG-informed metrics quantify intrinsic VSI, separating spatial capability from language priors.

Result: SIG yields larger, more stable gains in VSI metrics compared to VQA-only methods, demonstrating its effectiveness. SIGBench, a benchmark with 1.4K driving frames, is also released.

Conclusion: SIG shows promise for enhancing VSI in foundation models, offering a faithful representation of scene structure and improved evaluation metrics.
...
TL;DR: BFM improves Flow Matching by using specialized blocks for temporal segments, enhancing efficiency and quality, with 2.1x-4.9x faster inference.


<details>
  <summary>Details</summary>
Motivation: Current Flow Matching models struggle with capturing distinct signal characteristics across timesteps and incur high inference costs due to iterative evaluation of large networks.

Method: BFM partitions the generative trajectory into temporal segments, each modeled by smaller, specialized velocity blocks. It includes a Semantic Feature Guidance module and a Feature Residual Approximation strategy to enhance fidelity and reduce costs.

Result: BFM achieves 2.1x to 4.9x faster inference complexity at comparable performance on ImageNet 256x256, establishing a superior Pareto frontier.

Conclusion: Blockwise Flow Matching (BFM) significantly improves inference efficiency and sample quality by partitioning the generative trajectory into specialized temporal segments, outperforming existing Flow Matching methods.
...
TL;DR: TokenCLIP improves anomaly detection by dynamically aligning visual tokens with customized textual subspaces, outperforming single-space methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on a single textual space for alignment, which hinders accurate capture of varied anomaly semantics. TokenCLIP addresses this by enabling dynamic alignment for better semantic representation.

Method: TokenCLIP introduces a token-wise adaptation framework that dynamically aligns visual tokens with learnable textual subspaces using optimal transport, ensuring fine-grained anomaly learning.

Result: Extensive experiments show TokenCLIP's effectiveness in anomaly detection, achieving better performance than existing approaches.

Conclusion: TokenCLIP demonstrates superior performance in anomaly detection by dynamically aligning visual tokens with customized textual subspaces, outperforming existing methods.
...
TL;DR: KBE is a dynamic evaluation framework for MLLMs that evolves static benchmarks using multimodal knowledge, mitigating data contamination and saturation while enabling controllable difficulty assessment.


<details>
  <summary>Details</summary>
Motivation: Existing static benchmarks for MLLMs are prone to data contamination and saturation, leading to unreliable performance evaluations. A dynamic, evolving evaluation framework is needed to address these issues.

Method: KBE formulates VQA samples using Graph representation, integrates multimodal knowledge to expand static benchmarks, and reconstructs or expands questions with visual and textual knowledge. It allows difficulty-controllable evaluation by adjusting question exploration.

Result: Extensive experiments show that KBE reduces data contamination and saturation risks, providing a more thorough evaluation of MLLM capabilities.

Conclusion: The Knowledge-enhanced Benchmark Evolution (KBE) framework effectively addresses the limitations of static benchmarks by providing a dynamic, controllable evaluation method for multimodal large language models (MLLMs). It mitigates risks like data contamination and saturation while offering a more comprehensive assessment of MLLM capabilities.
...
TL;DR: The paper presents the 3rd place solution in the ICCV LargeFineFoodAI Retrieval Competition, using ensemble models, TTA, and a novel reranking method to achieve high mAP@100 scores.


<details>
  <summary>Details</summary>
Motivation: To improve feature representation and retrieval performance in the ICCV LargeFineFoodAI competition.

Method: Four models trained with ArcFace and Circle loss, enhanced with TTA, ensemble, and a new diffusion-based reranking method.

Result: Achieved 0.81219 and 0.81191 mAP@100 on public and private leaderboards.

Conclusion: The proposed ensemble and reranking methods effectively improved retrieval performance, securing a top position in the competition.
...
TL;DR: Combining Arcface and Circle loss with careful tuning and ensembling led to a 3rd-place win in a fine-grained food recognition challenge.


<details>
  <summary>Details</summary>
Motivation: The motivation was to address the fine-grained food recognition task, a key area in food analysis, by improving performance through innovative loss function combinations.

Method: The method involved training a model with a combination of Arcface and Circle loss, along with carefully tuned configurations and ensembling techniques.

Result: The solution won the 3rd place in the competition, indicating its success in enhancing recognition performance.

Conclusion: The paper's solution, combining Arcface and Circle loss with careful tuning and ensembling, achieved 3rd place in the LargeFineFoodAI-ICCV Workshop-Recognition challenge, demonstrating its effectiveness in fine-grained food recognition.
...
TL;DR: The paper introduces iSM, a framework resolving five key issues in shortcut models, improving their performance and making them competitive in generative modeling.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the critical performance bottlenecks of shortcut models, which include compounding guidance flaws, inflexible fixed guidance, frequency bias, divergent self-consistency, and curvy flow trajectories, hindering their widespread adoption.

Method: The paper proposes iSM, a unified training framework with four key improvements: Intrinsic Guidance for dynamic control, Multi-Level Wavelet Loss to mitigate frequency bias, Scaling Optimal Transport (sOT) for stable generative paths, and Twin EMA strategy for training stability and self-consistency.

Result: The results show significant FID improvements over baseline shortcut models across one-step, few-step, and multi-step generation on ImageNet 256 x 256.

Conclusion: The paper concludes that the introduced iSM framework successfully addresses the five core issues of shortcut models, making them a viable and competitive class of generative models, as demonstrated by substantial FID improvements on ImageNet 256 x 256.
...
TL;DR: TSSR introduces a parallel generation method for high-quality 3D artist-style meshes using Discrete Diffusion Models, achieving efficiency and realism through decoupled training, improved architecture, and topological constraints.


<details>
  <summary>Details</summary>
Motivation: The primary motivation is to achieve highly accurate token prediction with parallel generation, overcoming limitations of sequential autoregressive methods.

Method: TSSR employs Decoupled Training and Hybrid Inference, an Improved Hourglass Architecture with RoPE, and a novel Connection Loss to achieve parallel generation and high fidelity in 3D mesh creation.

Result: Extensive experiments show TSSR generates high-quality meshes with intricate local topology and global shape, achieving remarkable spatial resolution.

Conclusion: TSSR successfully generates high-quality 3D artist-style meshes with up to 10,000 faces at a resolution of $1024^3$, demonstrating its effectiveness in parallel generation and topological refinement.
...
TL;DR: SAGE-3D upgrades 3DGS with fine-grained semantics and physical executability for VLN, improving performance by 31% and introducing new datasets and benchmarks.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) lacks fine-grained semantics and physical executability for Visual-Language Navigation (VLN), limiting its practical utility.

Method: SAGE-3D comprises two components: (1) Object-Centric Semantic Grounding for fine-grained annotations and (2) Physics-Aware Execution Jointing for embedding collision objects and constructing physical interfaces.

Result: Experiments demonstrate that 3DGS scene data is challenging to converge but exhibits strong generalizability, improving baseline performance by 31% on the VLN-CE Unseen task.

Conclusion: The proposed SAGE-3D paradigm successfully upgrades 3DGS into an executable, semantically and physically aligned environment, improving VLN performance by 31% on the VLN-CE Unseen task. The data and code will be released soon.
...
TL;DR: \textsc{FineRS} is a two-stage MLLM framework for high-resolution image analysis, excelling in small object localization and segmentation.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with high-resolution images due to restricted input resolutions, especially in localizing extra-small objects in cluttered contexts.

Method: \textsc{FineRS} employs a two-stage pipeline: Global Semantic Exploration (GSE) for coarse reasoning and Localized Perceptual Refinement (LPR) for precise segmentation. A locate-informed retrospective reward couples the stages.

Result: Experimental results on \textsc{FineRS}-4k and public datasets show superior performance in instruction-guided segmentation and visual reasoning tasks.

Conclusion: The proposed \textsc{FineRS} framework, with its two-stage coarse-to-fine pipeline and locate-informed retrospective reward, consistently outperforms state-of-the-art MLLM-based approaches in handling high-resolution images with extra-small objects.
...
TL;DR: VL-SAE, a sparse autoencoder, interprets and enhances vision-language alignment by mapping representations to a unified concept set, improving downstream task performance.


<details>
  <summary>Details</summary>
Motivation: The interpretability of vision-language alignment is uninvestigated due to the challenge of mapping multi-modal semantics into a unified concept set.

Method: The paper proposes VL-SAE, a sparse autoencoder with a distance-based encoder and modality-specific decoders, trained self-supervised to ensure activation consistency for semantically similar representations.

Result: VL-SAE demonstrates superior capability in interpreting and enhancing alignment across VLMs like CLIP and LLaVA, validated by downstream task improvements.

Conclusion: VL-SAE successfully interprets and enhances vision-language alignment by mapping representations into a unified concept set, improving performance in tasks like zero-shot image classification and hallucination elimination.
