...
TL;DR: This paper investigates privacy attacks on multimodal vision-language models (VLMs) and proposes a neuroscience-inspired topological regularization (tau) method to enhance their resilience.


<details>
  <summary>Details</summary>
Motivation: Multimodal models are increasingly deployed but vulnerable to privacy attacks like membership inference, unlike unimodal models where neuroscience-inspired approaches have shown improved resilience.

Method: The authors introduce a topological regularization framework and test it on three VLMs (BLIP, PaliGemma 2, ViT-GPT2) across three datasets (COCO, CC3M, NoCaps), comparing baseline models to neuro-inspired variants.

Result: NEURO VLMs showed a 24% decrease in MIA attack success (ROC-AUC) on BLIP with COCO, while maintaining similar model utility (MPNet, ROUGE-2 scores). Results were consistent across other models and datasets.

Conclusion: Neuro-inspired VLMs are more resilient to privacy attacks without significantly compromising utility, contributing to understanding and mitigating privacy risks in multimodal AI systems.
...
TL;DR: Inferix is an inference engine designed for world models, using semi-autoregressive decoding to generate coherent, high-quality videos with efficient KV Cache management. It supports interactive streaming and benchmarking.


<details>
  <summary>Details</summary>
Motivation: World models need to generate realistic, interactive videos for AI applications, but current methods have limitations in coherence and efficiency. Semi-autoregressive decoding combines diffusion and autoregressive strengths to address this.

Method: Inferix employs semi-autoregressive (block-diffusion) decoding, generating video tokens in blocks with diffusion within each block while conditioning on prior blocks. It uses LLM-style KV Cache for efficient, variable-length generation.

Result: The approach produces more coherent and stable video sequences, overcoming standard video diffusion limitations. It enables immersive world synthesis with real-time interaction and supports benchmarking via LV-Bench.

Conclusion: Inferix represents a step forward for world models, distinct from high-concurrency systems and classic diffusion models. Community collaboration is encouraged to advance world model exploration.
...
TL;DR: A hybrid approach combining edge-based object detection with local tracking, optimized through reinforcement learning for resource-constrained video analytics


<details>
  <summary>Details</summary>
Motivation: Video object recognition on resource-constrained devices like traffic cameras requires efficient processing due to limited computational power

Method: Proposes LTED-Ada algorithm using deep reinforcement learning to adaptively switch between local tracking and edge detection based on frame rate, accuracy, and delay requirements

Result: Demonstrates superior performance through hardware-in-the-loop experiments using Raspberry Pi devices and edge servers

Conclusion: LTED-Ada provides an effective solution for adaptive video analytics in edge computing environments
...
TL;DR: DeeAD: Training-free early-exit framework that accelerates Vision-Language Action models by terminating inference when trajectories meet physical feasibility criteria.


<details>
  <summary>Details</summary>
Motivation: Reduce significant inference latency in VLA models caused by deep transformer stacks while maintaining planning quality.

Method: Uses action-guided early-exit with physical feasibility checks and multi-hop controller to skip redundant layers based on score change rates.

Result: Achieves up to 28% transformer-layer sparsity and 29% latency reduction on Bench2Drive benchmark.

Conclusion: DeeAD effectively accelerates VLA planning without retraining while preserving safety and performance.
...
TL;DR: FMD compresses large SSL foundation models into efficient proxies that retain general-purpose capabilities, with Foundry implementation for 3D point clouds showing strong performance across multiple tasks.


<details>
  <summary>Details</summary>
Motivation: Foundation models are too large for edge devices, and existing compression methods sacrifice generality.

Method: Trains students to learn compressed SuperTokens that reconstruct teacher's token representations.

Result: Single distilled model maintains strong transferability across classification, segmentation, and few-shot tasks with fewer tokens/FLOPs.

Conclusion: FMD enables practical deployment of foundation models on resource-constrained hardware while preserving performance.
...
TL;DR: DinoLizer: A DINOv2-based model detects manipulated regions in generative inpainting via patch classification and sliding-window aggregation, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Need for robust detection of manipulated regions in generative inpainting, especially with common post-processing operations applied.

Method: Uses DINOv2 pretrained model with added linear classification head on ViT patch embeddings; applies sliding-window strategy for larger images and refines binary masks via post-processing.

Result: DinoLizer achieves 12% higher IoU than next best model, maintains robustness to resizing, noise, JPEG compression.

Conclusion: DinoLizer demonstrates Vision Transformers' strong representation power for manipulation localization; ablation studies confirm superiority over successors like DINOv3.
...
TL;DR: CANVAS benchmark evaluates VLMs' ability to perform UI design tasks using tools in design software.


<details>
  <summary>Details</summary>
Motivation: No existing benchmark evaluates tool-based design performance of VLMs, limiting understanding of their potential to collaborate with designers.

Method: Created benchmark with 598 tool-based design tasks from 3.3K mobile UI designs across 30 categories, with two task types: design replication and design modification.

Result: Leading models show strategic tool invocations improving design quality, with identified error patterns for future improvement.

Conclusion: CANVAS provides a framework to assess and enhance VLMs' tool-based UI design capabilities.
...
TL;DR: TIE proposes a text-guided image encoder that generates image representations conditioned on text queries, outperforming conventional VLMs on multiple benchmarks while improving efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard vision-language models use image encoders trained agnostically without considering downstream tasks or text queries, limiting performance.

Method: Text-Guided Semantic Image Encoder (TIE) generates image representations conditioned on input text queries during training.

Result: VLMs with TIE achieve +1.5 and +1.3 points improvement on average across nine benchmarks at 1B and 3B scales, with up to 6 points gain on specific tasks.

Conclusion: TIE enables more efficient and effective VLMs by focusing on query-relevant visual features, demonstrating better interpretability and task-specific performance.
...
TL;DR: SMARC is a unified model that reconstructs full RGB surfaces and classifies materials using only a single 10% image patch, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing methods require dense or full-scene observations, making them ineffective in constrained or partial view environments where minimal visual input is available.

Method: SMARC combines a Partial Convolutional U-Net with a classification head to enable spatial inpainting and semantic understanding from extremely sparse observations.

Result: SMARC outperforms five models (convolutional autoencoders, ViT, MAE, Swin Transformer, DETR) with a PSNR of 17.55 dB and material classification accuracy of 85.10%.

Conclusion: The model demonstrates the effectiveness of partial convolution for spatial reasoning with missing data, providing a strong foundation for minimal-vision surface understanding.
...
TL;DR: LongVT is an agentic framework that enables multimodal Chain-of-Tool-Thought reasoning for long videos, using global skimming followed by local zooming to reduce hallucinations and improve answer grounding.


<details>
  <summary>Details</summary>
Motivation: Current LMMs struggle with hallucinations when processing long-form videos due to sparse and temporally dispersed evidence, unlike human comprehension which uses global skimming followed by detailed clip examination.

Method: Exploits LMMs' temporal grounding as a native cropping tool for iterative global-to-local reasoning, supplemented by the VideoSIAH data suite (247.9K SFT, 1.6K RL, 15.4K RFT samples) and a three-stage training strategy.

Result: LongVT consistently outperforms strong baselines across four challenging long-video understanding benchmarks.

Conclusion: The framework effectively addresses hallucinations in long-video reasoning, with publicly available code, data, and models facilitating further research.
...
TL;DR: Lightweight reproduction of KRISP that achieves 75% of original performance with fewer parameters, revealing design flaws while enabling edge device deployment.


<details>
  <summary>Details</summary>
Motivation: The original KRISP model is computationally demanding and designed for industrial-scale training, making it unsuitable for resource-constrained environments.

Method: Reexamine KRISP architecture with systematic ablation studies, using low parameter setup constrained by external Knowledge graph domain to prevent hallucinations.

Result: Created a lightweight model that functions on edge devices like smartphones and AR-VR, improves offline visual reasoning, and maintains domain-constrained outputs.

Conclusion: The reproduction uncovers design flaws in original KRISP while demonstrating knowledge-enhanced VQA architectures can be effectively scaled down for resource-constrained applications.
...
TL;DR: This paper presents a unified mathematical framework called "warping" that generalizes dynamic sampling methods in deep learning, analyzes their statistical properties, and reveals training asymmetries between forward/backward passes.


<details>
  <summary>Details</summary>
Motivation: To unify theoretical analysis of various dynamic sampling methods in computer vision which currently lack a common analytical framework.

Method: Develops a warping operator that generalizes existing methods, conducts statistical analysis using IID variables and random fields, and introduces gradient-based loss landscape visualization.

Result: Shows dynamic sampling represents a class of orthogonal operators different from convolutional operators, identifies conditions for stable training, and analyzes discretization effects.

Conclusion: Warping provides a minimal implementation amenable to theoretical analysis while unifying deformable convolutions, active convolutional units, and spatial transformer networks under one framework.
...
TL;DR: Δ-NeRF introduces a modular residual framework with incremental refinement techniques for Neural Radiance Fields, achieving comparable performance to joint training with reduced training time and data requirements.


<details>
  <summary>Details</summary>
Motivation: Existing NeRF frameworks require complete retraining for new views, limiting sequential data applications like satellite terrain analysis where catastrophic forgetting occurs without past data.

Method: Proposes Δ-NeRF with: (1) residual controller for layer corrections on frozen base NeRF, (2) uncertainty-aware gating to prevent overcorrection, (3) view selection reducing training data by 47%, and knowledge distillation for model compression.

Result: Experiments show Δ-NeRF reduces training time by 30-42% while matching joint training performance, outperforming baselines by up to 43.5% PSNR improvement over naive fine-tuning.

Conclusion: Δ-NeRF enables efficient incremental NeRF refinement suitable for sequential data scenarios like satellite imagery, addressing catastrophic forgetting and resource limitations.
...
TL;DR: Split-then-Merge (StM) framework for video composition using unlabeled videos, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Address data scarcity and enhance control in generative video composition without relying on annotated datasets.

Method: Splits videos into dynamic foreground/background layers, self-composes them with transformation-aware training and identity-preservation loss.

Result: Outperforms SoTA methods in quantitative benchmarks and qualitative evaluations.

Conclusion: StM effectively learns compositional dynamics for realistic video generation from unlabeled data.
...
TL;DR: Sphinx is a synthetic benchmark for visual reasoning with 25 task types, where state-of-the-art models like GPT-5 achieve only 51.1% accuracy, but RLVR significantly improves performance.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack precise evaluation and large-scale datasets for core cognitive primitives in visual perception and reasoning.

Method: Sphinx procedurally generates puzzles using motifs, tiles, charts, icons, and geometric primitives, each with verifiable ground-truth solutions.

Result: Evaluation shows GPT-5 attains 51.1% accuracy, well below human performance, but RLVR substantially improves accuracy and generalizes to external benchmarks.

Conclusion: Sphinx enables precise evaluation and RLVR shows promise for advancing multimodal reasoning.
...
TL;DR: Replaces computational diffusion priors with Optimization-based Visual Inversion (OVI), a training-free and data-free alternative for text-to-image generation.


<details>
  <summary>Details</summary>
Motivation: Traditional prior networks are computationally expensive and require massive training data.

Method: Uses OVI with random pseudo-token initialization and iterative cosine similarity optimization, regularized by Mahalanobis and Nearest-Neighbor losses.

Result: Constrained OVI methods improve visual fidelity, with Nearest-Neighbor achieving comparable/better scores than data-efficient priors on Kandinsky 2.2.

Conclusion: OVI is a viable prior alternative; benchmarks like T2I-CompBench++ contain flaws, and the approach merits further investigation.
...
TL;DR: RefTr - a 3D image-to-graph model for vascular tree centerline generation using recurrent refinement of confluent trajectories with Producer-Refiner Transformer architecture


<details>
  <summary>Details</summary>
Motivation: Accurate detection of tubular tree centerlines with correct topology is critical for clinical applications, where high recall is essential to avoid fatal mistakes from missing small branches

Method: Producer-Refiner Transformer architecture where Producer proposes initial confluent trajectories that are recurrently refined by Refiner to form centerline graph; includes efficient non-maximum suppression for spatial tree graphs

Result: Achieves superior recall and comparable precision to SOTA across multiple datasets, with 2.4x fewer decoder parameters, faster inference, and substantially fewer parameters

Conclusion: RefTr demonstrates potential as new SOTA framework for vascular tree analysis in 3D medical imaging
...
TL;DR: Introduction of a high-resolution stereo DSLR dataset with systematic optical variations to address the lack of real-world data for depth estimation and related tasks.


<details>
  <summary>Details</summary>
Motivation: Lack of large-scale, high-fidelity real stereo DSLR datasets limits generalization and evaluation of models trained on synthetic data for depth estimation and optical effects.

Method: Created a dataset with 18000 high-resolution images (5472x3648px) using two identical DSLR cameras, varying focal length (28-70mm) and aperture (f/2.8-f/22) across 9 scenes, including calibration sets for each configuration.

Result: Dataset enables controlled analysis of geometric and optical effects for various vision tasks and reveals challenges in current state-of-the-art methods.

Conclusion: The dataset bridges the realism gap between synthetic and real data, supporting reproducible research on optical generalization; data and code are publicly released.
...
TL;DR: This paper introduces a large-scale unsupervised dataset for visual memorability using Reddit ToT queries, enabling better recall generation and retrieval than GPT-4o.


<details>
  <summary>Details</summary>
Motivation: Existing memorability datasets rely on expensive human annotations, limiting diversity and capturing only aggregate scores rather than nuanced recall signals.

Method: Leverage tip-of-the-tongue queries from Reddit to create a dataset of 82K videos with recall descriptions; fine-tune vision-language models and use contrastive training for ToT retrieval.

Result: Fine-tuned models outperform GPT-4o in generating memorability descriptions, and the first multimodal ToT retrieval model is created.

Conclusion: The dataset and models offer a scalable, unsupervised approach to advance visual memorability research by capturing rich recall signals.
...
TL;DR: This paper introduces a method for estimating fog parameters from stereo images simultaneously rather than sequentially, proposes a new dataset SDIRF, and shows superior performance over prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing approaches estimate fog parameters sequentially, leading to error propagation. Real-world fog is often inhomogeneous, requiring better handling.

Method: The algorithm estimates all fog parameters simultaneously by solving a novel optimization problem, assuming fog is only locally homogeneous. It can be integrated into SLAM/odometry systems.

Result: Extensive experiments on synthetic data and the new SDIRF dataset demonstrate the method's superiority, providing more accurate estimates and better adaptation to real fog.

Conclusion: The proposed method effectively handles real-world fog, advances fog parameter estimation, and the publicly released code and SDIRF dataset support further research.
...
TL;DR: V^2-SAM adapts SAM2 for cross-view object correspondence using two prompt generators (V^2-Anchor and V^2-Visual) with a multi-expert selector, achieving SOTA on multiple benchmarks.
...
Method: V^2-SAM uses two complementary prompt generators: V^2-Anchor (geometry-aware via DINOv3 features) enables coordinate-based prompting, while V^2-Visual (appearance-guided) aligns representations via feature and structural matching. A multi-expert design with Post-hoc Cyclic Consistency Selector (PCCS) adaptively chooses the best output.

Result: The framework achieves state-of-the-art performance on Ego-Exo4D, DAVIS-2017, and HANDAL-X benchmarks, validating its effectiveness.

Conclusion: V^2-SAM successfully extends SAM2 to cross-view scenarios, offering a unified solution for robust object correspondence across viewpoints.
...
TL;DR: Null-TTA aligns diffusion models by optimizing the unconditional embedding in classifier-free guidance to prevent reward hacking and enable semantic coherence.


<details>
  <summary>Details</summary>
Motivation: Existing TTA methods tend to either under-optimize or over-optimize (reward hack) target reward functions during inference.

Method: Optimizes the unconditional embedding in classifier-free guidance rather than manipulating latent or noise variables, leveraging semantic embedding space structure.

Result: Achieves state-of-the-art target test-time alignment while maintaining strong cross-reward generalization.

Conclusion: Establishes semantic-space optimization as an effective and principled paradigm for TTA.
...
TL;DR: GaINeR: A geometry-aware implicit neural representation that combines Gaussian distributions with INR for enhanced image modeling with local editing capabilities.


<details>
  <summary>Details</summary>
Motivation: Traditional INRs lack explicit geometric structure and local editing capabilities, limiting their use in dynamic/interactive settings.

Method: Retrieves K nearest Gaussians for image coordinates, aggregates distance-weighted embeddings, and predicts RGB values through a neural network.

Result: A framework enabling continuous image representation with interpretable geometric structure and flexible local editing.

Conclusion: GaINeR provides a foundation for physically aware and interactive image manipulation, addressing limitations of traditional INRs.
...
TL;DR: A deep learning model effectively restores standard-dose MRI quality from low-dose (10%-30%) contrast-enhanced images for cerebellopontine angle imaging, improving both image metrics and diagnostic utility.


<details>
  <summary>Details</summary>
Motivation: To reduce contrast agent dose in MRI scans for vestibular schwannoma patients while maintaining diagnostic image quality, addressing concerns about contrast agent safety and cost.

Method: Used multi-center retrospective data to simulate low-dose T1ce images, trained DL models to restore standard-dose quality, and evaluated results through quantitative metrics and radiologist assessments.

Result: DL restoration improved structural similarity and signal-to-noise ratios; at 10% dose, segmentation accuracy increased (Dice from 0.673 to 0.734) and images were rated excellent by radiologists.

Conclusion: DL enables reliable lesion detection and characterization with only 10%-30% of standard contrast dose, offering a safe and effective low-dose MRI solution.
...
TL;DR: Smooth regularization using Gaussian Random Walk improves temporal modeling in lightweight video recognition models, achieving 3.8-6.4% accuracy gains on Kinetics-600.


<details>
  <summary>Details</summary>
Motivation: Lightweight video models struggle to capture temporal coherence effectively; natural videos exhibit smooth temporal transitions that should be reflected in model representations.

Method: Proposes smooth regularization that models changes in intermediate-layer embeddings of consecutive frames as a Gaussian Random Walk, penalizing abrupt representational shifts.

Result: Improves accuracy by 3.8-6.4% on Kinetics-600; MoViNets achieve 3.8-6.1% improvement within FLOP constraints; MobileNetV3 and MoViNets-Stream gain 4.9-6.4% over prior SOTA with comparable memory.

Conclusion: The GRW-based smooth regularization effectively instills temporal inductive bias, enabling lightweight models to better capture video dynamics while maintaining efficiency.
...
TL;DR: Introduces a framework for generating compositional explanations of neurons using open vocabulary semantic segmentation, overcoming limitations of human-annotated data.


<details>
  <summary>Details</summary>
Motivation: Existing compositional explanations rely on human-annotated datasets, restricting their applicability to specific domains and predefined concepts.

Method: Proposes a three-step framework: specifying arbitrary concepts, generating semantic segmentation masks using open vocabulary models, and deriving compositional explanations from these masks.

Result: The framework is compared with previous methods quantitatively and in terms of human interpretability, showing flexibility in handling arbitrary concepts and datasets.

Conclusion: The framework enables more flexible and scalable compositional explanations for neuron analysis by leveraging model-generated annotations instead of human-annotated data.
...
TL;DR: Introduction of UruDendro4 dataset for automated tree-ring detection and volumetric modeling, with performance benchmarks using DeepCS-TRD method.


<details>
  <summary>Details</summary>
Motivation: Manual measurement of tree-ring growth is time-consuming and imprecise; need for automated algorithms and more comprehensive datasets for accurate ring delineation.

Method: Creation of UruDendro4 dataset with 102 annotated Pinus taeda L. samples from multiple stem heights; use of DeepCS-TRD for automatic ring detection with ablation experiments.

Result: DeepCS-TRD achieved mean Average Precision of 0.838, mean Average Recall of 0.782, and Adapted Rand Error of 0.084; training with this dataset improves model generalization.

Conclusion: UruDendro4 enables volumetric modeling of annual growth and provides a robust baseline for automated tree-ring detection, enhancing accuracy and efficiency in silvicultural research.
...
TL;DR: BUSTR is a multitask vision-language framework that generates breast ultrasound reports without paired image-report data, using structured descriptors and radiomics features with improved performance on clinical metrics.


<details>
  <summary>Details</summary>
Motivation: Automated radiology report generation for breast ultrasound is limited by lack of paired datasets and hallucination risks from large language models.

Method: BUSTR constructs reports from structured descriptors, uses a multi-head Swin encoder with multitask loss for descriptor-aware visual representations, and aligns visual-textual tokens with dual-level objective combining cross-entropy and cosine similarity.

Result: BUSTR consistently improves standard NLG and clinical efficacy metrics across two BUS datasets, particularly for key targets like BI-RADS category and pathology.

Conclusion: The descriptor-aware vision model with combined token-level and alignment loss improves report metrics and clinical efficacy without requiring paired image-report data.
...
TL;DR: This paper introduces expressive composition as a new image composition task focused on artistic/stylistic placement rather than realism, and presents StickerNet - a two-stage framework trained on real editing data that outperforms baselines.


<details>
  <summary>Details</summary>
Motivation: Traditional image composition research focuses on realism, but modern content creators often aim for artistic/playful compositions to gain community recognition on online platforms.

Method: StickerNet: a two-stage framework that first identifies composition type, then predicts placement parameters (opacity, mask, location, scale). Dataset built from 1.8M real editing actions from an online platform.

Result: StickerNet outperforms common baselines and closely matches human placement behavior in both user studies and quantitative evaluations.

Conclusion: The work introduces a new direction in visual understanding emphasizing expressiveness over realism, demonstrating effectiveness of learning from real-world editing patterns despite task ambiguity.
...
TL;DR: TrafficLens is a sequential algorithm that optimizes multi-camera traffic analysis by reusing outputs and skipping redundant processing, reducing conversion time by 4x while maintaining accuracy


<details>
  <summary>Details</summary>
Motivation: Multi-camera traffic analysis faces challenges with large video data volumes and time-consuming video-to-text conversion needed for LLM integration

Method: Uses overlapping camera coverage with iterative VLM application, previous outputs as prompts, and object-level similarity detection to skip redundant processing

Result: Experimental results show TrafficLens reduces video-to-text conversion time by up to 4 times while preserving information accuracy

Conclusion: Proposed algorithm enables efficient multi-camera traffic analysis by significantly reducing processing time without compromising output quality
