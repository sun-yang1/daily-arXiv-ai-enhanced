...
TL;DR: This paper improves 3D lunar map quality by reducing noise in Kaguya TC images, enhancing safety for missions like NASA's Endurance.


<details>
  <summary>Details</summary>
Motivation: High-quality 3D lunar maps are critical for long-distance missions like NASA's Endurance, but Kaguya TC images suffer from altitude inaccuracies due to stereo matching errors and JPEG-based compression artifacts.

Method: The method involves analyzing compression behavior of Kaguya TC imagery, identifying systematic disparity noise patterns, and reducing residual noise in disparity images derived from compressed images.

Result: Experimental results demonstrate that the approach effectively reduces elevation noise in disparity maps.

Conclusion: The proposed approach successfully reduces elevation noise in 3D lunar maps, improving the safety and reliability of terrain data for future missions.
...
TL;DR: CLIP's advantage in VLMs comes from language supervision, enabling high-level semantic understanding, while DINO focuses on low-level features. Controlled experiments show CLIP excels in text tasks, DINO in vision tasks, with limited gains from language variants.


<details>
  <summary>Details</summary>
Motivation: The motivation was to determine whether CLIP's superior performance in VLMs is due to language supervision or larger training data, and to understand the distinct roles of high-level and low-level features in vision encoders.

Method: The researchers pre-trained CLIP and DINO under controlled settings (same architecture, dataset, and training configuration) to isolate the impact of language supervision and training data size. Embedding analysis and integration into VLMs were used to evaluate performance.

Result: CLIP outperforms DINO in text-intensive VQA tasks due to its high-level semantic understanding, while DINO slightly excels in vision-centric tasks. Variants of language supervision showed limited gains.

Conclusion: The study concludes that CLIP's advantage in vision-language models (VLMs) primarily stems from its language supervision, as it captures high-level semantics, while DINO excels in low-level features. Language supervision variants offer limited improvements, providing insights for VLM encoder design.
...
TL;DR: MammoDINO is a novel SSL framework for mammography, pretrained on 1.4M images, using domain-specific augmentations and contrastive learning to improve breast cancer screening tasks.


<details>
  <summary>Details</summary>
Motivation: SSL is underutilized in medical imaging due to limited data and domain-specific biases. MammoDINO aims to address this gap for mammography.

Method: The framework includes a breast tissue aware data augmentation sampler for image-level and patch-level supervision, along with a cross-slice contrastive learning objective leveraging 3D DBT structure for 2D pretraining.
...
Conclusion: MammoDINO provides a scalable, annotation-free foundation for multipurpose CAD tools in mammography, improving diagnostic efficiency and reducing radiologists' workload.
...
TL;DR: A dual-model framework using VideoLLaMA and Qwen2.5-VL improves traffic safety analysis by optimizing captioning and VQA tasks separately, achieving strong performance in temporal reasoning and visual understanding.


<details>
  <summary>Details</summary>
Motivation: To improve traffic safety analysis by capturing fine-grained behavioral patterns and generating comprehensive descriptions for accident prevention.

Method: A dual-model framework with task-specific optimization, separating training for captioning and VQA tasks to minimize interference and enhance specialization.

Result: VideoLLaMA excels in temporal reasoning (CIDEr score: 1.1001), while Qwen2.5-VL performs well in visual understanding (VQA accuracy: 60.80%). The method achieves an S2 score of 45.7572 in the 2025 AI City Challenge Track 2.

Conclusion: The dual-model framework, leveraging VideoLLaMA and Qwen2.5-VL, effectively addresses traffic safety analysis by minimizing task interference and optimizing performance in captioning and VQA tasks.
...
TL;DR: PanoTPS-Net, a novel model using CNN and TPS transformation, accurately estimates 3D room layouts from single panorama images, excelling in both cuboid and non-cuboid layouts with high 3DIoU scores.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve 3D room layout estimation from single panorama images, addressing applications in robotics, augmented reality, and interior design. The challenge includes handling both cuboid and non-cuboid layouts.

Method: The proposed method, PanoTPS-Net, uses a two-stage architecture: a CNN extracts high-level features to learn TPS transformation parameters, followed by a TPS spatial transformation layer to warp a reference layout to the predicted layout.

Result: Results show high accuracy with 3DIoU values of 85.49, 86.16, 81.76, and 91.98 on PanoContext, Stanford-2D3D, Matterport3DLayout, and ZInD datasets, respectively, outperforming state-of-the-art methods.

Conclusion: The paper concludes that PanoTPS-Net is an effective model for accurately estimating room layouts from single panorama images, demonstrating robustness in handling both cuboid and non-cuboid layouts. The compatibility between TPS transformation and panorama images is highlighted, supported by high 3DIoU values across multiple datasets.
...
TL;DR: A spatial reasoning framework for warehouses improves vision-language systems by embedding bounding box coordinates into prompts, achieving 4th place in the AI City Challenge.


<details>
  <summary>Details</summary>
Motivation: Existing models struggle with spatial reasoning in cluttered 3D environments due to reliance on local appearance and lack of explicit spatial grounding.

Method: The framework embeds bounding box coordinates into prompts and fine-tunes across four question categories with task-specific supervision. Normalized answers are appended to GPT responses for consistency.

Result: The pipeline achieves a score of 73.0606, ranking 4th on the public leaderboard.

Conclusion: Structured prompt enrichment and targeted optimization effectively advance spatial reasoning in real-world industrial settings.
...
TL;DR: The study finds DINO with Grad-CAM offers the best explainability for Vision Transformers in medical imaging, enhancing model transparency for clinical use.


<details>
  <summary>Details</summary>
Motivation: Interpretability of model decisions is crucial in medical imaging to ensure clinical trust and adoption, especially given the complex attention mechanisms of Vision Transformers.

Method: The study evaluates the explainability of ViT, DeiT, DINO, and Swin Transformer using Gradient Attention Rollout and Grad-CAM through quantitative and qualitative analyses on peripheral blood cell and breast ultrasound image classification tasks.

Result: DINO combined with Grad-CAM provides the most faithful and localized explanations, with Grad-CAM producing class-discriminative and spatially precise heatmaps, while Gradient Attention Rollout results in more scattered activations.

Conclusion: The research supports the reliable and explainable integration of Vision Transformers (ViTs) into medical diagnostic workflows by identifying DINO with Grad-CAM as the most effective combination for providing faithful and localized explanations.
...
TL;DR: APGNet improves underwater camouflaged object detection by combining MSRCR for image enhancement, ERF and MPD for feature refinement, and a prior-guided mechanism, outperforming 15 existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges of underwater image degradation (low contrast, color distortion) and the natural camouflage of marine organisms, which existing methods fail to address adequately.

Method: The method includes the Multi-Scale Retinex with Color Restoration (MSRCR) algorithm for data augmentation, an Extended Receptive Field (ERF) module combined with a Multi-Scale Progressive Decoder (MPD), and an adaptive prior-guided mechanism for hierarchical fusion of position and boundary priors.

Result: APGNet outperforms 15 state-of-the-art methods on two public MAS datasets under widely used evaluation metrics.

Conclusion: APGNet, an Adaptive Prior-Guided Network, effectively addresses the challenges of underwater image degradation and camouflaged object detection by integrating a Siamese architecture with a novel prior-guided mechanism. It outperforms existing methods on public MAS datasets.
...
TL;DR: VidMP3 introduces a novel approach for motion-preserved video editing, using pose and position priors to maintain original motion while allowing structural and semantic changes, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with temporal inconsistency, subject identity drift, and human intervention in structure-variable video editing. VidMP3 aims to address these challenges.

Method: VidMP3 leverages pose and position priors to learn a generalized motion representation from source videos, enabling new video generation with original motion preserved.

Result: Qualitative and quantitative evaluations show VidMP3 outperforms existing methods in maintaining motion consistency and flexibility.

Conclusion: VidMP3 demonstrates superior performance in motion-preserved video editing, offering structural and semantic flexibility while maintaining original motion. The code will be publicly available.
...
TL;DR: Domain Adaptation helps overcome labeled data scarcity by adapting models trained on one domain to perform well on another, reducing reliance on costly labeled datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the high cost and difficulty of obtaining labeled data in computer vision, particularly for tasks like image classification.

Method: The paper discusses various methods of Domain Adaptation, such as leveraging models trained on one dataset to predict on data from a different but related domain.

Result: The results suggest that Domain Adaptation can produce comparable performance to benchmark results despite limited labeled data.

Conclusion: The paper concludes that Domain Adaptation is a viable solution to address the scarcity of labeled data in computer vision, enabling models trained on one domain to perform well on another.
...
TL;DR: A DiT-based framework improves audio-driven human video generation with better lip-sync, temporal coherence, and multi-character support, using LoRA training and Mask-CFG for efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with lip-sync accuracy, long video coherence, and multi-character animation.

Method: Uses a diffusion transformer (DiT) with LoRA-based training, position shift inference, partial parameter updates, and Mask-CFG for multi-character animation.

Result: Outperforms state-of-the-art in quality, coherence, and multi-character support.

Conclusion: The proposed method is simple, efficient, and cost-effective for high-quality audio-driven video generation.
...
TL;DR: IL3D is a large-scale dataset for LLM-driven 3D scene generation, improving performance and generalization through diverse, annotated data and multimodal capabilities.


<details>
  <summary>Details</summary>
Motivation: Addressing the need for diverse, high-quality training data in indoor layout design for LLM-driven 3D scene generation.

Method: The study introduces IL3D, a large-scale dataset with 27,816 indoor layouts and 29,215 3D object assets, enriched with natural language annotations. It employs supervised fine-tuning (SFT) of LLMs and establishes benchmarks for evaluation.

Result: SFT on IL3D improves generalization and outperforms SFT on other datasets. The dataset supports multimodal learning and various visual tasks.

Conclusion: IL3D significantly advances research in 3D scene generation and embodied intelligence by providing high-fidelity, diverse data, enhancing generalization and performance in LLM-driven tasks.
...
TL;DR: Proposed EG-Restormer and ADNet leverage edge priors in QR codes for effective deblurring, achieving top performance with adaptive network selection.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for QR code deblurring do not explicitly exploit the structured patterns and sharp edges of QR codes, which are robust priors for restoration.

Method: The paper introduces Edge-Guided Attention Block (EGAB) embedded in a Transformer architecture, leading to EG-Restormer for severe blur and LENet for mild blur, combined into Adaptive Dual-network (ADNet).

Result: EG-Restormer and ADNet significantly improve decoding rates for severely blurred QR codes and offer fast deblurring for mildly blurred inputs, with competitive speed.

Conclusion: The proposed EG-Restormer and ADNet achieve state-of-the-art performance in QR code deblurring, balancing effectiveness and efficiency, especially for mobile devices.
...
TL;DR: The paper introduces a method to enhance 3D scene reconstruction by using accurate geometry and generative models, addressing limitations in existing approaches and achieving superior results in both observed and unobserved regions.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D scene reconstruction using generative priors lack reliable geometric supervision and mechanisms to mitigate multi-view inconsistencies, leading to poor quality reconstructions and shape-appearance ambiguities.

Method: The method leverages planar structures to derive accurate metric-scale depth maps, providing reliable supervision. It incorporates geometry guidance to improve visibility mask estimation, novel view selection, and multi-view consistency when inpainting with video diffusion models.

Result: Extensive experiments on Replica, ScanNet++, and DeepBlending show the method outperforms baselines in geometry and appearance reconstruction, especially for unobserved regions. It also supports single-view inputs and unposed videos with strong generalizability.

Conclusion: The proposed method significantly improves 3D scene reconstruction by leveraging accurate geometry and generative models, outperforming existing baselines in both observed and unobserved regions. It also supports single-view inputs and unposed videos, demonstrating strong generalizability across indoor and outdoor scenarios.
...
TL;DR: The DRL framework, with IPA and DAS, tackles CIL challenges by ensuring smooth representation shifts and aligned feature spaces, outperforming state-of-the-art methods efficiently.


<details>
  <summary>Details</summary>
Motivation: The challenges in non-rehearsal CIL include large model complexity, non-smooth representation shifts, and inconsistency between stage-wise optimization and global inference. The DRL framework aims to address these issues.

Method: The proposed DRL framework includes the IPA network, which learns lightweight adapters with minimal parameter overhead, and DAS, which decouples constraints of positive and negative samples using virtual anchors.

Result: Extensive experiments on six benchmarks show that DRL consistently outperforms other state-of-the-art methods in CIL while remaining efficient in training and inference.

Conclusion: The DRL framework, incorporating the Incremental Parallel Adapter (IPA) network and Decoupled Anchor Supervision (DAS), effectively addresses the challenges of non-rehearsal Class-Incremental Learning (CIL) by ensuring smooth representation shifts and aligning feature spaces across stages. It outperforms state-of-the-art methods while maintaining efficiency.
...
TL;DR: SSDiff is a self-supervised selective-guided diffusion method for old-photo face restoration, outperforming existing methods in quality and controllability.


<details>
  <summary>Details</summary>
Motivation: Old-photo face restoration is challenging due to compounded degradations like breakage, fading, and blur. Existing methods struggle with localized artifacts or face color.

Method: SSDiff leverages pseudo-reference faces generated by a pre-trained diffusion model under weak guidance, using staged supervision (structural guidance and color refinement) and selective restoration via face parsing maps and scratch masks.

Result: SSDiff achieves superior restoration by structurally aligned contours and natural colors, avoiding identity mismatch.

Conclusion: SSDiff outperforms existing GAN-based and diffusion-based methods in perceptual quality, fidelity, and regional controllability, as demonstrated on the VintageFace benchmark.
...
TL;DR: ImageSentinel is a framework to protect visual datasets in RAIG systems by generating sentinel images that verify unauthorized use while maintaining generation quality.


<details>
  <summary>Details</summary>
Motivation: Concerns about unauthorized use of private image datasets in RAIG systems, with traditional watermarking methods failing due to complex feature extraction.

Method: Proposes ImageSentinel, which synthesizes sentinel images using vision-language models, embedding retrieval keys for verification.

Result: ImageSentinel effectively detects unauthorized dataset usage without compromising generation quality.

Conclusion: ImageSentinel provides a viable solution for protecting visual datasets in RAIG systems, balancing protection and utility.
...
TL;DR: A constrained optimization approach improves compressive single-photon 3D imaging by jointly optimizing illumination and coding matrices under hardware constraints, outperforming traditional designs.


<details>
  <summary>Details</summary>
Motivation: Single-photon cameras face performance limitations due to hardware constraints like bandwidth, laser power, and sensor data rates, especially under real-world illumination conditions.

Method: The paper introduces a gradient descent-based constrained optimization approach to design practical coding functions for compressive single-photon 3D imaging, adhering to hardware constraints.

Result: The optimized coding functions outperform traditional designs, especially under peak power constraints, and adapt well to non-ideal impulse responses.

Conclusion: The proposed method effectively addresses hardware limitations in compressive single-photon 3D imaging, offering superior performance under real-world constraints.
...
TL;DR: CapFlow and MetaCaptioner bridge the gap between open-source and commercial visual captioning models, offering high-quality, cost-effective solutions.


<details>
  <summary>Details</summary>
Motivation: To address the performance gap between open-source and commercial visual captioning models, enabling cost-effective, high-quality captioning for various applications.

Method: The paper proposes CapFlow, a multi-agent collaboration workflow, and MetaCaptioner, a generalist visual captioner fine-tuned using CapFlow-synthesized data.

Result: CapFlow achieves caption quality comparable to GPT-4.1 with an 89.5% cost reduction, while MetaCaptioner matches commercial models and excels in multimodal performance.

Conclusion: CapFlow and MetaCaptioner provide a strong, cost-effective solution for generalist visual captioning, bridging the performance gap between open-source and commercial models.
...
TL;DR: FedHUG, a federated learning framework, tackles privacy and unlabeled data challenges in remote physiological measurement with dynamic aggregation and learning control, achieving top performance.


<details>
  <summary>Details</summary>
Motivation: Remote physiological measurement faces challenges due to privacy-sensitive data and reliance on labeled client data, making updates to deployed models difficult with unlabeled user data.

Method: The FedHUG framework includes (1) Minimal Bias Aggregation for handling heterogeneous non-IID features and (2) Global Distribution-aware Learning Controller to mitigate label distribution skew and long-tail issues.

Result: FedHUG outperforms state-of-the-art techniques in estimation using RGB video or mmWave radar.

Conclusion: The proposed FedHUG framework demonstrates superior performance in remote physiological measurement, addressing challenges like privacy concerns and unlabeled data through innovative modules like Minimal Bias Aggregation and Global Distribution-aware Learning Controller.
...
TL;DR: KFF introduces class-aware domain knowledge fusion and fission to improve continual test-time adaptation, outperforming existing methods on ImageNet-C.


<details>
  <summary>Details</summary>
Motivation: Existing CTTA methods suffer from insufficient learning of new knowledge and interference from harmful historical knowledge, causing performance degradation. The KFF method aims to mitigate these issues by adaptively managing domain knowledge.

Method: The KFF method includes a domain Knowledge Fission (KFI) module to separate new domain knowledge and a domain Knowledge Fusion (KFU) module to merge new knowledge efficiently. A greedy knowledge dynamic merging strategy is used to enhance compatibility and efficiency.

Result: Extensive experiments on the ImageNet-C dataset demonstrate the effectiveness of KFF compared to other methods.

Conclusion: The proposed KFF method effectively addresses the challenges of continual test-time adaptation by dynamically fusing and fissioning class-aware domain knowledge, leading to improved performance and reduced computational overhead.
...
TL;DR: DPL uses diffusion-based learning to enhance prototype diversity in one-shot medical image segmentation, achieving state-of-the-art results on MRI and CT datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of traditional prototype-based methods, which fail to capture intra-class diversity due to deterministic averaging, leading to brittle representations in one-shot medical image segmentation.

Method: DPL introduces a diffusion-based framework with three core innovations: (1) diffusion-based prototype enhancement, (2) spatial-aware conditioning, and (3) conservative fusion. It models prototypes as learnable distributions for diverse yet coherent variants.

Result: Experiments on abdominal MRI and CT datasets show DPL outperforms existing methods, setting new benchmarks in one-shot segmentation.

Conclusion: The paper concludes that Diffusion Prototype Learning (DPL) significantly improves one-shot medical image segmentation, achieving state-of-the-art performance on abdominal MRI and CT datasets.
...
TL;DR: SSP enhances video understanding by combining intra-frame and inter-frame prompts, outperforming SOTA methods by 2.76% with reduced parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Existing prompt learning methods for pre-trained state space models fail to capture spatial and temporal contextual information in videos, limiting effective information propagation and discriminative feature extraction.

Method: The SSP method combines intra-frame and inter-frame prompts to aggregate and propagate key spatiotemporal information in videos. It includes an Intra-Frame Gathering (IFG) module for spatial aggregation and an Inter-Frame Spreading (IFS) module for temporal information spread.

Result: Extensive experiments on four video benchmark datasets show SSP's superior performance over existing methods.

Conclusion: The proposed State Space Prompting (SSP) method significantly outperforms existing state-of-the-art methods by 2.76% on average while reducing fine-tuning parameter overhead.
...
TL;DR: UniGS is a unified framework for high-fidelity multimodal 3D reconstruction using 3D Gaussian Splatting, offering photo-realistic rendering, geometric accuracy, and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: The need for a unified and differentiable framework to achieve high-fidelity multimodal 3D reconstruction with geometric consistency and efficiency.

Method: Integration of a CUDA-accelerated rasterization pipeline, differentiable ray-ellipsoid intersection for depth rendering, analytic gradient formulation for surface normals, and a learnable attribute for Gaussian pruning.

Result: State-of-the-art reconstruction accuracy across all modalities, validated through quantitative and qualitative experiments.

Conclusion: UniGS effectively combines geometric awareness and computational efficiency, with plans to release source code and a multimodal viewer.
...
TL;DR: BEEP3D introduces an end-to-end method for 3D instance segmentation using box-level supervision, avoiding the need for costly point-level annotations. It employs a student-teacher framework with novel losses to improve pseudo-mask generation and achieves competitive performance.


<details>
  <summary>Details</summary>
Motivation: Fully supervised 3D instance segmentation requires expensive point-level annotations. Box-level supervision is more scalable but introduces ambiguity in overlapping regions. Existing two-stage methods increase complexity and hinder optimization.

Method: BEEP3D uses a student-teacher framework with Exponential Moving Average updates. It includes instance center-based query refinement and two novel losses (query consistency and masked feature consistency) to align predictions with pseudo-masks.

Result: BEEP3D achieves competitive or superior performance on ScanNetV2 and S3DIS datasets compared to state-of-the-art weakly supervised methods, while remaining computationally efficient.

Conclusion: BEEP3D provides an efficient and effective solution for 3D instance segmentation with box-level supervision, addressing ambiguity and computational challenges of existing methods.
...
TL;DR: CompoDistill aligns student-teacher visual attention in MLLMs, improving visual perception distillation and performance on reasoning tasks while maintaining VQA results.


<details>
  <summary>Details</summary>
Motivation: Existing knowledge distillation methods fail to effectively transfer the teacher MLLM's rich visual perception abilities to the student, primarily due to visual attention misalignment.

Method: The paper proposes CompoDistill, a knowledge distillation framework that explicitly aligns the student's visual attention with the teacher's to enhance visual perception abilities.

Result: CompoDistill significantly improves performance on compositional reasoning tasks requiring visual perception and maintains strong performance on visual question answering tasks. It also shows generalizability with advanced backbones.

Conclusion: CompoDistill effectively addresses the challenge of distilling visual perception abilities from teacher to student MLLMs by aligning visual attention, demonstrating improved performance on compositional reasoning tasks and maintaining strong results on visual question answering tasks.
...
TL;DR: A hierarchical VLM framework improves incident report generation for autonomous driving, ranking 2nd in the 2COOOL challenge.


<details>
  <summary>Details</summary>
Motivation: Addressing the gap in autonomous driving models' performance in out-of-distribution scenarios by improving hazard understanding and generating human-interpretable incident reports.

Method: A hierarchical reasoning framework integrating frame-level captioning, incident frame detection, and fine-grained reasoning within VLMs, enhanced by model ensembling and a Blind A/B Scoring protocol.

Result: Ranked 2nd among 29 teams on the 2COOOL leaderboard, achieving the best CIDEr-D score for accurate and coherent incident narratives.

Conclusion: Hierarchical reasoning with VLMs shows promise for accident analysis and understanding safety-critical traffic events.
...
TL;DR: The paper explores using synthetic data for fine-tuning object detection models in warehouse logistics, comparing its performance to real-world data. Findings suggest a balanced mix of synthetic and real data enhances model robustness.


<details>
  <summary>Details</summary>
Motivation: The lack of expertise and resources for domain-specific data collection in AI applications motivates the use of synthetic data as a cost-effective alternative.

Method: The study investigates synthetic data's impact on object detection models, using NVIDIA Omniverse Replicator for synthetic data generation and comparing it with real-world data in pallet detection experiments.

Result: A balanced integration of synthetic and real data improves the robustness and efficiency of object detection models in warehouse logistics.

Conclusion: Synthetic data is a viable and effective alternative to real-world data for fine-tuning object detection models, especially when combined with real data for optimal performance.
...
TL;DR: DIANet, a dual-stream framework using phase-aware dynamic images and cross-attention fusion, improves micro-expression recognition by explicitly modeling temporal phases, outperforming single-phase methods.


<details>
  <summary>Details</summary>
Motivation: Micro-expression recognition (MER) is challenging due to subtle and transient facial cues and limited annotated data. Conventional dynamic image (DI) methods overlook distinct temporal phases within micro-expressions.

Method: The paper introduces DIANet, a dual-stream framework that leverages phase-aware dynamic images (one for onset-to-apex and another for apex-to-offset phases). Each stream is processed by a dedicated CNN, and a cross-attention fusion module integrates features adaptively.

Result: Extensive experiments on CASME-II, SAMM, and MMEW datasets show DIANet consistently outperforms single-phase DI-based approaches.

Conclusion: The proposed DIANet framework, which explicitly models temporal phase information, outperforms conventional single-phase DI-based methods, suggesting a promising direction for advancing micro-expression recognition (MER).
...
TL;DR: The paper introduces data curation strategies for VL reasoning, leading to the HoneyBee dataset, which improves model performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the lack of understanding in constructing performant VL reasoning training datasets, despite the effectiveness of vision-language models in reasoning tasks.

Method: The study introduces data curation approaches, including context source analysis, targeted data interventions (e.g., auxiliary signals from captions and text-only reasoning), and scaling data dimensions (images, questions, chain-of-thought solutions).

Result: Results show that context source strategies, targeted interventions, and scaling data dimensions significantly enhance VL reasoning. HoneyBee-trained models outperform SOTA models, e.g., by 7.8% on MathVerse, and a test-time scaling strategy reduces decoding costs by 73%.

Conclusion: The paper concludes with improved strategies for vision-language (VL) reasoning dataset curation, demonstrated by the success of the HoneyBee dataset and its superior performance over state-of-the-art models.
