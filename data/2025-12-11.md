...
TL;DR: The paper explores VLMMs' limitations in temporal event order understanding, introduces the VECTOR benchmark for assessment, and proposes MECOT for improvement.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored ability of VLMMs in capturing temporal order of events despite their strong performance in video understanding.

Method: Proposes VECTOR benchmark for temporal order assessment and MECOT, which uses detailed event descriptions and chain-of-thought prompts.

Result: MECOT outperforms prior methods on VECTOR and improves performance on existing benchmarks.

Conclusion: MECOT effectively enhances VLMMs' temporal understanding, with released code, model, and datasets.
...
TL;DR: IMAgent is an open-source vision agent for multi-image QA tasks, leveraging multi-agent reinforcement learning and specialized tools for visual reflection and confirmation.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitation of current VLM-based agents in handling multi-image QA tasks by proposing a more robust solution.

Method: End-to-end reinforcement learning with a multi-agent system and specialized tools for visual attention reallocation.

Result: IMAgent achieves stable tool use and strong performance on both single-image benchmarks and multi-image datasets.

Conclusion: IMAgent demonstrates significant improvements in multi-image QA tasks, offering insights for future research.
...
TL;DR: Proposes UTIE, a method to reduce demographic bias in face recognition by enriching embeddings with cross-demographic text-derived features, leveraging VLMs.


<details>
  <summary>Details</summary>
Motivation: Addressing demographic bias in face recognition systems, which can cause disparities in verification performance across groups.

Method: Unified Text-Image Embedding (UTIE) uses VLMs to enrich facial embeddings with text-derived features from other demographic groups.

Result: UTIE reduces bias metrics while maintaining or improving face verification accuracy across benchmarks.

Conclusion: UTIE effectively mitigates demographic bias in face recognition by leveraging cross-modal alignment in VLMs.
...
TL;DR: Consist-Retinex adapts consistency modeling to Retinex-based low-light enhancement, achieving state-of-the-art performance with single-step sampling.


<details>
  <summary>Details</summary>
Motivation: Addressing the impractical deployment of diffusion models due to iterative sampling steps in low-light image enhancement.

Method: Introduces dual-objective consistency loss and adaptive noise-emphasized sampling strategy for stable convergence and effective conditional enhancement.

Result: Achieves PSNR: 25.51 vs. 23.41, FID: 44.73 vs. 49.59 compared to Diff-Retinex++, with 1/8 of the training budget.

Conclusion: Consist-Retinex provides a practical and efficient solution for low-light image enhancement, outperforming existing methods.
...
TL;DR: HSCP, a Hierarchical Spectral Clustering Pruning framework, optimizes UAV identification by reducing model complexity and improving accuracy and robustness for edge deployment.


<details>
  <summary>Details</summary>
Motivation: Traditional UAV identification methods and existing deep learning approaches struggle with real-time requirements and deployment on resource-constrained devices due to large model sizes and high computational demands.

Method: Hierarchical Spectral Clustering Pruning (HSCP) framework combining layer pruning with channel pruning, guided by Centered Kernel Alignment (CKA), and employing noise-robust fine-tuning.

Result: HSCP achieves 86.39% parameter reduction and 84.44% FLOPs reduction on ResNet18, with a 1.49% accuracy improvement over the unpruned baseline, and maintains robustness in low signal-to-noise ratio environments.

Conclusion: HSCP demonstrates superior performance in UAV identification by achieving significant parameter and FLOPs reduction while improving accuracy and robustness, making it suitable for resource-constrained edge devices.
...
TL;DR: RAG-HAR is a training-free retrieval-augmented framework for HAR that leverages LLMs, achieving top performance without training or fine-tuning, and recognizing unseen activities.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for Human Activity Recognition (HAR) require dataset-specific training, large labeled corpora, and significant computational resources, limiting their practicality.

Method: RAG-HAR computes lightweight statistical descriptors, retrieves semantically similar samples from a vector database, and uses this contextual evidence for LLM-based activity identification. It further enhances performance through prompt optimization and an LLM-based activity descriptor.

Result: RAG-HAR attains state-of-the-art performance across six diverse HAR benchmarks without the need for model training or fine-tuning.

Conclusion: RAG-HAR achieves state-of-the-art performance across six diverse HAR benchmarks without requiring model training or fine-tuning, emphasizing its robustness and practical applicability. It also enables the recognition and meaningful labeling of multiple unseen human activities.
...
TL;DR: The paper proposes a method called Verifier-Threshold to optimize test-time compute allocation in image generation models, achieving 2-4x efficiency improvements.


<details>
  <summary>Details</summary>
Motivation: Current methods for allocating non-uniform inference-compute budgets in image generation models rely on greedy algorithms, leading to ineffective compute budget allocation.

Method: The Verifier-Threshold method automatically reallocates test-time compute to improve efficiency.

Result: The method achieves a 2-4x reduction in computational time for the same performance on the GenEval benchmark.

Conclusion: The proposed Verifier-Threshold method effectively addresses inefficiencies in compute budget allocation for image generation models, offering significant computational savings.
...
TL;DR: Proposes a quality-control framework for diabetic retinopathy (DR) diagnosis using AI, focusing on high-standard data through image filtering, enhancement, and annotator agreement.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of manual errors in DR diagnosis by ensuring high-quality annotated datasets for AI training.

Method: Uses a feature-based classifier for image filtering, contrastive learning for feature extraction, deep-learning-based enhancement, and annotator agreement formulas.

Result: A framework that improves the reliability of DR diagnosis by ensuring high-quality data and annotations.

Conclusion: The proposed framework enhances the accuracy and efficiency of DR diagnosis, supporting clinicians with better AI tools.
...
TL;DR: A novel 3D Inverse Design framework combines latent representation and physics-aware optimization to outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: Current methods approximate 3D design spaces with 2D projections or fine-tune existing shapes, limiting true 3D design exploration.

Method: The framework uses a two-stage strategy: gradient-guided diffusion sampling for global exploration and objective-driven refinement for detailed sculpting.

Result: 3DID generates high-fidelity 3D geometries, demonstrating superior performance in solution quality and design versatility.

Conclusion: The proposed 3D Inverse Design (3DID) framework successfully navigates the 3D design space by combining continuous latent representation with physics-aware optimization, outperforming existing methods in quality and versatility.
...
TL;DR: CKI framework improves HSI classification in heterogeneous settings by addressing spectral and semantic inconsistencies, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations in existing methods for hyperspectral image (HSI) classification, particularly in fully heterogeneous settings where label spaces do not overlap.

Method: Cross-scene Knowledge Integration (CKI) includes Alignment of Spectral Characteristics (ASC), Cross-scene Knowledge Sharing Preference (CKSP), and Complementary Information Integration (CII).

Result: Extensive experiments verify CKI's superior performance.

Conclusion: CKI achieves state-of-the-art performance with strong stability across diverse cross-scene HSI scenarios.
...
TL;DR: Proposes a Deterministic World Model (DWM) for precise vision-based control system verification by eliminating stochastic latent variables, achieving tighter reachable sets.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of verifying vision-based control systems due to high-dimensional images and unreliable generative models with stochastic latent variables.

Method: Introduces DWM, trained with a dual-objective loss function for pixel-level accuracy and control consistency, integrated with StarV and conformal prediction for verification.

Result: Experiments show DWM yields significantly tighter reachable sets and better verification performance than latent-variable baselines.

Conclusion: DWM effectively eliminates uninterpretable latent variables, ensuring precise input bounds and superior verification performance.
...
TL;DR: A novel generative model for 3D dose prediction in radiotherapy planning, customizable to user preferences, outperforms Varian RapidPlan in adaptability and quality.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for 3D dose prediction rely on reference plans, which may bias models toward specific planning styles or institutional preferences.

Method: Introduces a generative model that predicts 3D dose distributions based on user-defined preference flavors, allowing prioritization of trade-offs between OARs and PTVs.

Result: The method surpasses the Varian RapidPlan model in adaptability and plan quality in some scenarios.

Conclusion: The approach offers greater flexibility and personalization, enabling efficient generation of high-quality plans integrated with clinical treatment planning systems.
...
TL;DR: A novel diffusion model regularized implicit neural representation framework for metal artifact reduction (MAR) in CT images, addressing limitations of supervised and unsupervised methods.


<details>
  <summary>Details</summary>
Motivation: Existing MAR methods suffer from performance instability due to reliance on limited paired data (supervised) or fail to incorporate CT physical geometry and prior knowledge effectively (unsupervised).

Method: Proposes a framework combining implicit neural representation (for physical constraints and data fidelity) with a pre-trained diffusion model (for prior knowledge regularization).

Result: Demonstrated effectiveness and generalization on simulated and clinical data, showing potential for clinical application.

Conclusion: The proposed method overcomes key challenges in MAR, offering a robust solution with clinical applicability.
...
TL;DR: A novel method generates large-scale, physically valid defect datasets for AI in semiconductor fabrication, showing significant performance gains with Mask R-CNN.


<details>
  <summary>Details</summary>
Motivation: To address the scarcity of high-quality and physically grounded training data for defect inspection in micro/nano manufacturing, particularly in the semiconductor industry.

Method: The framework synthesizes defect layouts using physics-constrained mathematical morphology operations, fabricates physical samples via DMD-based lithography, and creates consistent defect delineation annotations by comparing optical micrographs.

Result: A comprehensive dataset of 3,530 optical micrographs containing 13,365 annotated defect instances was constructed. Mask R-CNN achieved significant AP@0.5 improvements over Faster R-CNN (mean ~34% for bridge, burr, pinch; ~42% for contamination).

Conclusion: The proposed methodology for generating large-scale, physically valid defect datasets with pixel-level annotations is feasible for robust AI-based Measurement/Inspection (MI) in semiconductor fabrication, as demonstrated by the significant performance improvements of Mask R-CNN over Faster R-CNN.
...
TL;DR: A survey on body and face motion generation, covering concepts, techniques, datasets, and future directions for enhancing avatar realism and expressiveness.


<details>
  <summary>Details</summary>
Motivation: Body and face motion are crucial in communication, but generating expressive and coherent dynamics is challenging due to complex verbal/non-verbal cues and individual traits.

Method: Reviews core concepts, representation techniques, generative approaches, datasets, and evaluation metrics.

Result: Identifies gaps and future directions to improve realism, coherence, and expressiveness in avatar motion generation.

Conclusion: This is the first comprehensive review covering both body and face motion, with detailed resources provided.
...
TL;DR: LUVC framework compresses visual tokens iteratively, integrating pruning and merging to enhance VLM efficiency without significant accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Addressing redundancy in token representations of high-resolution images/videos and limitations of current compression algorithms (position bias, class imbalance, poor generalization to shallow LLM layers).

Method: Extends token compression to visual encoder via iterative merging orthogonal in spatial axes; integrates spectrum pruning unit (low-pass filter) into LLM for gradual token pruning, compatible with FlashAttention.

Result: Achieves 2× speedup in inference with negligible accuracy degradation; training-free, enabling immediate deployment across VLMs.

Conclusion: LUVC effectively balances computational efficiency and accuracy, offering a scalable solution for visual token compression in VLMs.
...
TL;DR: The paper presents a deep learning approach for detecting specific characters in video sequences, achieving efficient results compared to state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: The study aims to enhance entity detection in videos, particularly for applications like national security in Angola.

Method: The approach uses supervised learning algorithms with artificial neural networks to analyze video sequences.

Result: The method efficiently locates target individuals in video databases, outperforming existing techniques.

Conclusion: The proposed classifier has potential applications in security systems, leveraging video data for target identification.
...
TL;DR: E-Deflare framework removes lens flare from event camera data through physics-based modeling and achieves state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Lens flare causes severe degradation in event camera data but has been largely overlooked despite event cameras' potential in vision systems.

Method: Developed a physics-grounded forward model, created E-Flare-2.7K simulated dataset and E-Flare-R real-world test set, then designed E-DeflareNet model.

Result: Achieved state-of-the-art restoration performance validated by extensive experiments.

Conclusion: E-Deflare provides an effective solution for lens flare removal in event cameras, benefiting downstream tasks with publicly available code/datasets.
...
TL;DR: ConceptPose is a training-free, model-free framework for object pose estimation using vision-language models to create open-vocabulary 3D concept maps, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Bridging the gap between object pose estimation methods requiring extensive training and zero-shot capabilities of vision-language models.

Method: Leverages a vision-language model to create 3D concept maps with concept vectors from saliency maps, establishing 3D-3D correspondences for pose estimation.

Result: Achieves state-of-the-art results on zero-shot relative pose estimation benchmarks, outperforming existing methods by over 62% in ADD(-S) score.

Conclusion: ConceptPose demonstrates the potential of vision-language models in training-free, model-free object pose estimation, significantly advancing the field.
...
TL;DR: SIP (Site in Pieces) is a dataset designed to address the gap in LiDAR data for construction sites, featuring real-world constraints like radial density decay and fragmented geometry. It includes annotated indoor/outdoor scenes and supports robust benchmarking for 3D vision tasks.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR datasets lack realism for construction sites, which are cluttered, dynamic, and constrained by safety/access. SIP fills this gap by capturing field data with practical limitations.

Method: SIP uses terrestrial LiDAR scans, annotated with a construction-specific taxonomy (Built Environment, Construction Operations, Site Surroundings), and includes quality control procedures.

Result: The dataset provides realistic, annotated LiDAR data for construction, enabling better benchmarking and advancement in 3D vision tasks.

Conclusion: SIP addresses the underrepresentation of real-world LiDAR challenges in construction, offering a valuable resource for research and practical applications in 3D scene interpretation.
...
TL;DR: KD-OCT compresses a high-performance teacher model into a lightweight student model for efficient AMD screening, achieving near-teacher performance with reduced computational demands.


<details>
  <summary>Details</summary>
Motivation: To address the computational demands of deploying state-of-the-art deep learning models like ConvNeXtV2-Large in clinical settings, the study aims to develop efficient models that maintain high diagnostic performance while enabling real-time deployment.

Method: The study proposes a novel knowledge distillation framework, KD-OCT, which employs real-time distillation with a combined loss balancing soft teacher knowledge transfer and hard ground-truth supervision. The framework enhances the teacher model with advanced augmentations, stochastic weight averaging, and focal loss.

Result: KD-OCT outperforms comparable multi-scale or feature-fusion OCT classifiers in efficiency-accuracy balance, achieving near-teacher performance with substantial reductions in model size and inference time. The student model exceeds most existing frameworks.

Conclusion: The study demonstrates that KD-OCT effectively compresses the high-performance ConvNeXtV2-Large teacher model into a lightweight EfficientNet-B2 student model, achieving near-teacher performance with significant reductions in model size and inference time. This facilitates edge deployment for AMD screening.
...
TL;DR: The paper proposes an automatic threshold selection method for Visual Place Recognition (VPR) by analyzing 'negative' Gaussian mixture statistics, improving robustness across diverse visual scenarios.


<details>
  <summary>Details</summary>
Motivation: VPR is challenging due to varying conditions like seasonal changes and illumination. Manual threshold setting is unreliable for diverse scenarios.

Method: The approach uses 'negative' Gaussian mixture statistics (indicating non-matching places) to automatically determine optimal thresholds.

Result: The method effectively selects thresholds that perform well across various image databases and descriptors.

Conclusion: Automated threshold selection via negative statistics enhances VPR reliability, eliminating the need for manual tuning.
...
TL;DR: AgentComp enhances text-to-image models' compositionality by training them to differentiate between similar prompts using LLM-aided dataset construction and preference optimization, achieving SOTA results without quality loss.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with fine-grained details and object relationships despite high visual quality, due to lack of explicit training on compositional variations.

Method: AgentComp uses LLMs with image tools to autonomously build datasets for compositional variations, then applies agentic preference optimization to fine-tune models.

Result: AgentComp achieves state-of-the-art performance on T2I-CompBench, improving compositional generation without sacrificing image quality, and shows generalization to untrained capabilities like text rendering.

Conclusion: AgentComp effectively addresses compositionality challenges in text-to-image generation by leveraging LLM reasoning and preference optimization, setting a new benchmark while maintaining image quality.
...
TL;DR: MDSE is a vision-language framework for generating detailed explanations of underground disaster scenes, improving situational awareness with innovations in cross-attention, visual encoding, and efficient language modeling.


<details>
  <summary>Details</summary>
Motivation: Underground mining disasters create challenging environments with poor visibility, making situational awareness difficult for humans and conventional systems.

Method: Proposes MDSE with Context-Aware Cross-Attention, Segmentation-aware dual pathway visual encoding, and a Resource-Efficient Transformer-Based Language Model.

Result: MDSE outperforms state-of-the-art captioning models, providing more accurate and contextually relevant descriptions in obscured environments.

Conclusion: MDSE enhances situational awareness in underground emergencies, demonstrated by extensive experiments on the Underground Mine Disaster dataset.
...
TL;DR: The paper proposes FoCULR to improve generative models' handling of multi-noun food categories by integrating domain knowledge and refining layouts, addressing misinterpretations in image generation.


<details>
  <summary>Details</summary>
Motivation: Generative models often misinterpret multi-noun food categories, producing incorrect ingredients or layouts due to insufficient domain knowledge and semantic misunderstandings.

Method: FoCULR incorporates food domain knowledge and introduces core concepts early in the generation process to refine spatial layouts and correct misinterpretations.

Result: Experimental results show that FoCULR improves image generation performance for multi-noun food categories.

Conclusion: FoCULR effectively addresses challenges in generating realistic food images for multi-noun categories by enhancing domain knowledge and layout understanding.
...
TL;DR: SuperF is a test-time optimization approach for multi-image super-resolution using neural fields, achieving high-quality upsampling without high-resolution training data.


<details>
  <summary>Details</summary>
Motivation: High-resolution imagery is often limited by sensor technology, atmospheric conditions, and costs. Single-image super-resolution methods can introduce unrealistic structures, while MISR aims to improve resolution by using multiple views with sub-pixel shifts.

Method: SuperF leverages coordinate-based neural networks (neural fields) to represent continuous signals with an implicit neural representation (INR). It shares an INR for multiple shifted low-resolution frames and jointly optimizes frame alignment with the INR, parameterizing sub-pixel alignment as optimizable affine transformation parameters.

Result: Experiments show compelling results on simulated bursts of satellite imagery and ground-level images from handheld cameras, with upsampling factors of up to 8.

Conclusion: SuperF demonstrates a compelling approach for multi-image super-resolution (MISR) without relying on high-resolution training data, achieving upsampling factors of up to 8.
...
TL;DR: AngioAI-QFR is an automated, efficient pipeline for coronary artery disease assessment, combining deep learning and angiography-derived QFR, showing strong correlation with FFR.


<details>
  <summary>Details</summary>
Motivation: To improve the assessment of coronary artery disease by offering a wire-free, automated, and efficient alternative to traditional methods like FFR.

Method: The system combines deep learning stenosis detection, lumen segmentation, centreline and diameter extraction, per millimetre Relative Flow Capacity profiling, and virtual stenting with automatic recomputation of angiography-derived QFR.

Result: AngioAI-QFR correlated strongly with FFR (r = 0.89, MAE 0.045), with an AUC of 0.93 for detecting FFR <= 0.80, sensitivity 0.88, and specificity 0.86. The pipeline completed fully automatically in 93% of vessels, with a median time to result of 41 seconds.

Conclusion: AngioAI-QFR provides a practical, near real-time pipeline that unifies computer vision, functional profiling, and virtual PCI with automated angiography-derived physiology.
...
TL;DR: A method combining 2D Gaussian Splatting and UV texture mapping for editable, relightable head avatars from monocular video.


<details>
  <summary>Details</summary>
Motivation: To address the lack of intuitive editability in Gaussian Splatting-based head avatars while maintaining their accuracy and fidelity.

Method: The method embeds each canonical Gaussian primitive's local frame into a UV patch on a template mesh efficiently, reconstructing continuous editable material textures. It also uses a physically based reflectance model for relighting and editing intrinsic material maps.

Result: The method achieves accurate reconstructions, high-quality relighting results, and intuitive controls for modifying avatar appearance and geometry via texture mapping.

Conclusion: The proposed method successfully combines the accuracy of 2D Gaussian Splatting with the intuitiveness of UV texture mapping, enabling editable material head textures from monocular video while supporting relighting and appearance modifications without additional optimization.
...
TL;DR: WonderZoom introduces a novel approach for generating multi-scale 3D scenes from a single image, addressing limitations of existing single-scale synthesis methods.


<details>
  <summary>Details</summary>
Motivation: Existing 3D world generation models are limited to single-scale synthesis and lack the ability to produce coherent scene contents at varying granularities.

Method: WonderZoom uses scale-adaptive Gaussian surfels and a progressive detail synthesizer to generate and render multi-scale 3D scenes.

Result: Experiments show WonderZoom outperforms state-of-the-art models in quality and alignment, enabling multi-scale 3D world creation from a single image.

Conclusion: WonderZoom successfully addresses the challenge of multi-scale 3D scene generation, offering significant improvements over existing methods.
