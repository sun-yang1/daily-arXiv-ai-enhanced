...
TL;DR: SO-Bench benchmark evaluates MLLMs' capability for structured visual information extraction across multiple domains using diverse JSON schemas.


<details>
  <summary>Details</summary>
Motivation: There's no existing benchmark that systematically evaluates schema-grounded information extraction and reasoning over visual inputs for multimodal language models.

Method: Created SO-Bench benchmark with 6.5K diverse JSON schemas and 1.8K curated image-schema pairs covering four visual domains (UI screens, natural images, documents, charts). Conducted benchmarking and training experiments.

Result: Revealed persistent gaps in models' ability to produce accurate, schema-compliant outputs from visual inputs.

Conclusion: Highlights the need for better multimodal structured reasoning capabilities in MLLMs.
...
TL;DR: Score-based generative models need guidance for quality samples, but current methods require labeled data or extra training. SFG uses positive curvature in saddle regions for guidance without these requirements, achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: Existing guidance methods like Classifier-Free Guidance and Auto-Guidance depend on labeled data or training additional models, limiting their applicability when such resources are unavailable.

Method: SFG leverages the positive curvature of log density estimates in saddle regions to guide score-based models, requiring no extra training and working with off-the-shelf models like diffusion and flow matching.

Result: SFG achieves SOTA FID and FD-DINOv2 metrics for unconditional ImageNet-512 generation. Combined with Auto-Guidance, it sets SOTA in FD-DINOv2. It also boosts diversity in FLUX.1-dev and Stable Diffusion v3.5 while maintaining prompt adherence and fidelity.

Conclusion: SFG provides an effective, resource-efficient guidance method for score-based models, outperforming existing techniques without needing labeled data or additional training.
...
TL;DR: UniArt is a diffusion framework that generates fully articulated 3D objects from a single image using a unified latent representation for geometry, texture, segmentation, and kinematics.


<details>
  <summary>Details</summary>
Motivation: Manual creation of articulated 3D objects is costly and not scalable, creating a need for automated methods to support simulation and robotics.

Method: Uses a diffusion-based framework with a reversible joint-to-voxel embedding to align articulation features with geometry, and treats articulation type prediction as an open-set problem.

Result: Achieves state-of-the-art mesh quality and articulation accuracy on the PartNet-Mobility benchmark.

Conclusion: UniArt provides an effective end-to-end solution for generating articulated 3D assets, generalizing well to novel objects and joint types.
...
TL;DR: PathReasoning is a multi-modal reasoning agent that iteratively navigates Whole Slide Images through reasoning chains to efficiently identify diagnostically relevant regions, outperforming existing methods on cancer analysis tasks.


<details>
  <summary>Details</summary>
Motivation: The challenge of navigating extremely large Whole Slide Images (up to 10 billion pixels) makes it difficult and time-consuming to find relevant regions for clinical analysis, while pathologists effectively navigate through sampling and reasoning.

Method: PathReasoning starts with random sampling, then iteratively reviews selections through self-reflection that reasons about visual-clinical question correspondence, proposing new regions to explore across multiple rounds to build reasoning chains.

Result: PathReasoning outperforms strong ROI-selection methods by 6.7% and 3.1% AUROC on subtyping and longitudinal analysis, and improves breast cancer report accuracy by 10% over GPT-4o.

Conclusion: PathReasoning enables efficient slide review, interpretable reasoning chains, and supports accurate diagnostics and evidence traceability in digital pathology by prioritizing question-specific regions.
...
TL;DR: PRISM algorithm enables adaptive contactless heart rate monitoring using RGB cameras without training, achieving state-of-the-art unsupervised performance comparable to supervised methods.


<details>
  <summary>Details</summary>
Motivation: Existing rPPG methods rely on fixed parameters optimized for specific conditions, limiting adaptability across diverse environments.

Method: PRISM algorithm jointly optimizes photometric detrending and color mixing through online parameter adaptation based on signal quality assessment.

Result: MAE of 0.77 bpm on PURE and 0.66 bpm on UBFC-rPPG, with accuracy of 97.3% and 97.5% at 5 bpm threshold respectively.

Conclusion: Adaptive time series optimization significantly improves rPPG performance across diverse conditions, achieving performance equivalent to supervised methods while maintaining real-time CPU operation without training.
...
TL;DR: A flexible multimodal prototyping framework integrating histology images with potentially incomplete genomics data for precision oncology, featuring biological prototyping, multiview alignment, bipartite fusion, and semantic genomics imputation.


<details>
  <summary>Details</summary>
Motivation: Current multimodal approaches struggle with phenotypic/genotypic heterogeneity and fail to handle real-world scenarios where genomics data may be partially or completely missing.

Method: Four-component framework: 1) Biological Prototyping with text prompting and prototype-wise weighting; 2) Multiview Alignment through sample- and distribution-wise alignments; 3) Bipartite Fusion for shared and modality-specific information; 4) Semantic Genomics Imputation for missing data handling.

Result: Extensive experiments show consistent superiority over state-of-the-art methods on multiple downstream tasks.

Conclusion: The proposed framework effectively addresses multimodal integration challenges in precision oncology, particularly handling incomplete genomics data while demonstrating strong performance improvements.
...
TL;DR: AmodalGen3D is a generative framework that reconstructs complete 3D objects from sparse, unposed views using 2D amodal completion and multi-view stereo geometry, outperforming traditional methods in occlusion-heavy scenarios.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D reconstruction methods fail with sparse, occluded views, leading to incomplete results. Real-world applications need complete object models despite limited visibility.

Method: Integrates 2D amodal completion with multi-view stereo conditioning via View-Wise Cross Attention for feature fusion and Stereo-Conditioned Cross Attention for inferring hidden structures.

Result: Achieves superior fidelity and completeness on synthetic and real datasets under heavy occlusion and sparse views, producing consistent 3D models.

Conclusion: AmodalGen3D effectively addresses sparse-view 3D reconstruction challenges, benefiting robotics, AR/VR, and embodied AI with plausible occluded region hallucinations.
...
TL;DR: WalkCLIP is a multimodal framework combining satellite imagery, street views, and population data to better predict urban walkability.


<details>
  <summary>Details</summary>
Motivation: Existing methods use only single data sources (satellite, street view, or population data), each missing key perspectives needed for accurate walkability assessment.

Method: Integrates multiple data sources via walkability-aware vision-language representations from GPT-4o captions, spatial aggregation for neighborhood context, and fusion with population dynamics features.

Result: Outperforms unimodal and multimodal baselines in predictive accuracy and spatial alignment when evaluated on 4,660 Minneapolis-Saint Paul locations.

Conclusion: Combining visual and behavioral data yields more reliable predictions of the walking environment than single-source approaches.
...
TL;DR: Comparative analysis of deep learning models on 4,000 endoscopic images shows VGG16 and MobileNetV2 achieving 96.5% accuracy, with explainable AI via Grad-CAM for clinical interpretability.


<details>
  <summary>Details</summary>
Motivation: To address challenges in endoscopic imaging (variable lighting, camera angles, artifacts) and advance automated disease classification for gastrointestinal conditions like Diverticulosis, Neoplasm, Peritonitis, and Ureters.

Method: Utilized state-of-the-art deep learning techniques (VGG16, MobileNetV2, Xception) on a dataset of 4,000 endoscopic images across four disease classes, incorporating Grad-CAM visualization for explainable AI.

Result: VGG16 and MobileNetV2 achieved 96.5% test accuracy, Xception reached 94.24%, with robust benchmarks and explainable predictions via Grad-CAM highlighting influential image regions.

Conclusion: The study establishes strong baselines for automated gastrointestinal disease classification, emphasizing the value of diverse datasets and model explainability in medical AI for accurate, interpretable diagnostics.
...
TL;DR: PAT3D is a physics-augmented text-to-3D scene generation framework that integrates vision-language models with physics simulation to create physically plausible, intersection-free 3D scenes.


<details>
  <summary>Details</summary>
Motivation: Current text-to-3D generation methods often produce scenes with physical implausibilities like object intersections and unstable configurations, limiting their practical utility for downstream applications.

Method: Generates 3D objects from text prompts, infers spatial relations, organizes them into hierarchical scene trees, and uses differentiable rigid-body simulation to achieve static equilibrium without interpenetrations through simulation-in-the-loop optimization.

Result: PAT3D substantially outperforms prior approaches in physical plausibility, semantic consistency, and visual quality, producing simulation-ready scenes.

Conclusion: PAT3D enables high-quality, physically stable 3D scene generation suitable for downstream tasks like scene editing and robotic manipulation, with code and data to be released.
...
TL;DR: A new large-scale dataset RPM-10K and vision-language model MRLM are proposed for robust pointer meter reading recognition by integrating physical relations.


<details>
  <summary>Details</summary>
Motivation: Existing pointer meter reading methods are fragile due to challenges like reflections, occlusions, and ambiguous scale-pointer distinctions, lacking large-scale datasets.

Method: Develop RPM-10K dataset with 10730 images, then build MRLM model that encodes geometric and causal relations between pointer and scale using cross-attention and expert selection.

Result: Extensive experiments validate MRLM's effectiveness on the new benchmark.

Conclusion: The proposed dataset and model advance robust meter reading by aligning perception with physical reasoning.
...
TL;DR: PPBoost transforms weak text prompts into precise visual bounding boxes for medical image segmentation, improving accuracy over text- and visual-prompted baselines without requiring labeled data.


<details>
  <summary>Details</summary>
Motivation: Text-prompted models lack spatial precision and degrade under domain shifts, while visual-prompted models require costly precise bounding boxes that are challenging to obtain clinically.

Method: PPBoost uses a vision-language model to generate initial pseudo-bounding boxes from text, filters unreliable predictions with uncertainty-aware criteria, trains a pseudo-labeled detector, and refines bounding boxes during inference to tightly cover targets.

Result: PPBoost consistently improves Dice and Normalized Surface Distance across three diverse medical datasets, outperforming text- and visual-prompted baselines and even few-shot segmentation models without labeled data.

Conclusion: PPBoost effectively bridges the gap between text and visual prompts for medical image segmentation, generalizing to multiple backbones and offering a zero-shot solution that enhances spatial precision without requiring annotations.
...
TL;DR: Introduces Qualcomm Interactive Cooking benchmark and LiveMamba model for real-time interactive step-by-step guidance with mistake detection in instructional tasks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-modal LLMs lack real-time interactive guidance capabilities, particularly in detecting and alerting mistakes during task execution, which is crucial for future AI assistants.

Method: Created a new benchmark and dataset based on CaptainCook4D with dense annotations, timed instructions, and precisely timestamped mistake alerts. Developed LiveMamba, a streaming multi-modal LLM designed for interactive instructional guidance.

Result: Provides the first dedicated benchmark and strong baseline for live situated coaching, with evaluation of state-of-the-art models showing improved capabilities for real-time interactive guidance.

Conclusion: This work addresses a critical gap in multi-modal LLMs by introducing specialized datasets and models for live interactive coaching, paving the way for more responsive AI assistants.
...
TL;DR: Proposes a comprehensive acceleration pipeline for Rectified Flow models using batch processing, vectorization, and TensorRT compilation, achieving 611% speedup vs existing 18% acceleration.


<details>
  <summary>Details</summary>
Motivation: Existing acceleration methods cannot be directly applied to Rectified Flow models due to theoretical and design differences from diffusion models.

Method: Developed acceleration pipeline with novel velocity field batch processing, heterogeneous time-step vectorization, and dynamic TensorRT compilation tailored for flow models.

Result: Achieved 611% acceleration on 512x512 image generation, significantly outperforming existing methods that typically achieve only 18% speedup.

Conclusion: The proposed specialized acceleration approach effectively addresses Rectified Flow model limitations and dramatically improves generation efficiency beyond current methods.
...
TL;DR: MedEyes is a reinforcement learning framework that improves medical VQA by mimicking clinician diagnostic reasoning through gaze-guided exploration and dual-stream optimization.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models using RLVR tend to produce clinically inaccurate reasoning paths despite appearing coherent. Medical diagnosis requires progressive visual focusing and iterative reasoning similar to clinical workflows.

Method: Proposes MedEyes framework with Gaze-guided Reasoning Navigator (GRN) for dual-mode exploration (scanning and drilling), Confidence Value Sampler (CVS) for balanced exploration, and dual-stream GRPO optimization to decouple on-policy/off-policy learning.

Result: Achieves +8.5% average performance improvement across multiple medical VQA benchmarks compared to existing methods.

Conclusion: MedEyes validates the potential for building interpretable medical AI systems by better aligning with clinical diagnostic reasoning patterns.
...
TL;DR: Training-free uncertainty estimation method for VLMs detects erroneous predictions using visual feature consistency and multivariate Gaussians, achieving SOTA performance with minimal training data.


<details>
  <summary>Details</summary>
Motivation: VLMs like CLIP show high misclassification confidence, limiting reliability in safety-critical applications, requiring better uncertainty estimation.

Method: Measure visual feature consistency within classes using feature projection and multivariate Gaussians to create class-specific probabilistic embeddings, without fine-tuning.

Result: Extensive experiments on ImageNet, Flowers102, Food101, EuroSAT, and DTD demonstrate state-of-the-art error detection performance, outperforming baseline methods.

Conclusion: The proposed VLM-agnostic method is robust, requires minimal training data, and effectively improves reliability for safety-critical applications.
...
TL;DR: Direct audio-visual object grounding without text transcription can outperform transcription-based methods, especially for robustness to linguistic variability.


<details>
  <summary>Details</summary>
Motivation: Question transcription-based pipelines' efficiency and robustness; explore direct audio-visual alignment.

Method: Simplify task to single-word spoken instructions; introduce new audio-based grounding dataset; adapt/benchmark audio-visual models.

Result: Direct grounding from audio is feasible and outperforms transcription-based methods in some cases.

Conclusion: Findings encourage renewed interest in direct audio grounding for more robust multimodal systems.
...
TL;DR: Simple UDA method using frequency-domain style adaptation for domain-adaptive object detection, removing need for adversarial training or complex architectures.


<details>
  <summary>Details</summary>
Motivation: Most state-of-the-art UDA approaches are overly complex with adversarial training or auxiliary models, creating practical deployment challenges.

Method: Learns to adapt image styles in frequency domain with lightweight pre-processing module during training only, no inference overhead.

Result: Achieves substantial performance gains on multiple DAOD benchmarks without computational cost increase.

Conclusion: Proposed frequency-domain style adaptation provides practical and effective UDA solution with minimal complexity.
...
TL;DR: The paper proposes ICM (Image-Conditioned Manifold Regularization) for Real-ISR, addressing issues with text-conditioned manifolds by using sparse structural information from images.


<details>
  <summary>Details</summary>
Motivation: Existing Real-ISR methods use text-conditioned manifolds from diffusion models, which are conceptually misaligned with the task and cause color distortions and blurred edges.

Method: ICM regularizes output towards a manifold conditioned on colormap and Canny edges, providing task-aligned and stable regularization.

Result: Experiments show ICM significantly enhances super-resolution performance, especially in perceptual quality.

Conclusion: ICM offers a more suitable manifold for Real-ISR, improving stability and output quality for real-world applications.
...
TL;DR: TPCNet enhances low-light images using physics-based constraints derived from Kubelka-Munk theory, outperforming existing methods on 10 datasets without adding parameters.


<details>
  <summary>Details</summary>
Motivation: Prior Retinex-based models ignore specular reflection and use image-space constraints, limiting generalization. This paper addresses these limitations by incorporating specular reflection and reformulating constraints.

Method: Proposes Triple Physical Constraints (TPCs) theory based on Kubelka-Munk theory, preserving specular reflection coefficients and constructing constraints in feature space. Implements TPCNet with these physical constraints.

Result: TPCNet achieves superior performance in quantitative metrics and visual quality on 10 benchmark datasets, validated through comprehensive experiments and ablations.

Conclusion: The TPCs theory and TPCNet effectively enhance low-light images by integrating physical constraints into deep learning, demonstrating improved generalization and state-of-the-art results.
...
TL;DR: OralGPT-Omni is a multimodal LLM specialized for dentistry, addressing data scarcity and reliability issues. It uses a novel Chain-of-Thought dataset and training method to excel in dental image analysis across multiple modalities.


<details>
  <summary>Details</summary>
Motivation: Dentistry is underexplored by MLLMs due to limited domain data, scarce expert annotations, insufficient modality modeling, and reliability challenges.

Method: Constructed TRACE-CoT dataset mimicking dental radiologists' reasoning; implemented four-stage training paradigm for dental image analysis.

Result: Achieved overall score of 51.84 on MMOral-Uni benchmark and 45.31 on MMOral-OPG benchmark, significantly outperforming GPT-5.

Conclusion: OralGPT-Omni advances intelligent dentistry and enables future progress in dental image analysis; all resources will be publicly available.
...
TL;DR: TL;DR: DNA model enables online handwriting generation for unseen characters/writer styles using dual-branch adaptation


<details>
  <summary>Details</summary>
Motivation: Existing OHG methods struggle with unseen characters in glyph-based languages like Chinese

Method: Dual-branch Network with Adaptation (DNA) with adaptive style branch (stroke attributes) and adaptive content branch (structural/texture decomposition)

Result: Achieves state-of-the-art performance for unseen OHG setting

Conclusion: DNA is well-suited for generating handwriting with unseen characters/styles
...
TL;DR: WorldWander is a novel framework for translating between first-person and third-person video perspectives using in-context learning and collaborative position encoding.


<details>
  <summary>Details</summary>
Motivation: Bridging egocentric and exocentric perspectives is crucial for applications in filmmaking, embodied AI, and world models.

Method: WorldWander integrates In-Context Perspective Alignment and Collaborative Position Encoding built upon video diffusion transformers.

Result: Experiments show superior perspective synchronization, character consistency, and generalization capabilities.

Conclusion: WorldWander sets a new benchmark for egocentric-exocentric video translation tasks.
...
TL;DR: Proposes using supervised contrastive learning with Rank-N-Contrast loss for brain age estimation from T1w MRI, achieving better performance than conventional methods and demonstrating potential as a neurodegenerative disease biomarker.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning approaches for brain age estimation fail to capture continuous neuromorphological changes, leading to suboptimal feature representation and results.

Method: Uses supervised contrastive learning with Rank-N-Contrast loss for regression, along with Grad-RAM for visual explanation of results, applied to T1w structural MRI data.

Result: Achieves MAE of 4.27 years and R² of 0.93 with limited training data, outperforming conventional deep regression and competing with state-of-the-art methods using larger datasets. Grad-RAM revealed more nuanced age-related features.

Conclusion: The method shows promise as a biomarker for neurodegenerative diseases, with exploratory studies revealing correlations between brain age gap and disease severity in Alzheimer's and Parkinson's patients.
...
TL;DR: MoE3D integrates Mixture of Experts into multi-modal 3D learning, using specialized experts for different modalities and cross-modal interactions, achieving state-of-the-art performance on various 3D understanding tasks.


<details>
  <summary>Details</summary>
Motivation: Previous multi-modal fusion methods use dense fusion networks that struggle with modality heterogeneity and complexity, leading to suboptimal performance.

Method: Proposes MoE3D with MoE-based transformer, expert networks for specific modalities/interactions, information aggregation module, top-1 gating for efficiency, and progressive pre-training strategy.

Result: Achieves competitive performance across four 3D understanding tasks, surpassing top-performing counterpart by 6.1 mIoU on Multi3DRefer.

Conclusion: MoE3D effectively handles modality heterogeneity through specialized experts and advanced fusion techniques, demonstrating superior performance in multi-modal 3D understanding.
...
TL;DR: HyperST predicts gene expression from histology images using hyperbolic space to model hierarchical structure and align multi-level representations across modalities.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with incomplete image-gene alignment due to ignoring hierarchical gene expression structure and information asymmetry between rich molecular data and limited visual correlates in images.

Method: Proposes HyperST with Multi-Level Representation Extractors to capture spot-level and niche-level features, and Hierarchical Hyperbolic Alignment to unify representations while preserving hierarchy.

Result: Achieves state-of-the-art performance on four public datasets from different tissues, demonstrating improved cross-modal prediction accuracy.

Conclusion: HyperST provides a scalable framework for spatial transcriptomics prediction by effectively bridging modality gaps through hierarchical hyperbolic alignment.
...
TL;DR: PROMPTMINER is a black-box framework for recovering text prompts from AI-generated images, achieving superior performance through RL-based subject reconstruction and fuzzing-driven style recovery.


<details>
  <summary>Details</summary>
Motivation: High-quality prompts face security and IP risks from unauthorized extraction, while existing methods have limitations in practicality and adaptability.

Method: Two-phase approach: (1) RL-based optimization for primary subject reconstruction, (2) fuzzing-driven search for stylistic modifiers.

Result: Achieves CLIP similarity up to 0.958 and SBERT alignment up to 0.751, outperforming baselines with better generalization and robustness.

Conclusion: PROMPTMINER demonstrates effective black-box prompt recovery with strong performance across different datasets and defensive conditions.
...
TL;DR: GoPrune is an accelerated structured pruning method using  ℓ₂,ₚ-norm and proximal alternating minimization for efficient CNN compression, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: CNNs face high storage and computational costs on edge devices, and existing  ℓₚ-norm pruning methods are inefficient and limited to unstructured pruning with p ∈ (0,1).

Method: Proposes GoPrune using ℓ₂,ₚ-norm for sparse learning (p ∈ [0,1)) and an efficient optimization algorithm based on proximal alternating minimization with closed-form solutions.

Result: Experiments on CIFAR datasets with ResNet and VGG show superior performance in network pruning compared to existing approaches.

Conclusion: GoPrune effectively accelerates structured pruning, enhances computational efficiency, and is suitable for deployment on resource-constrained devices.
