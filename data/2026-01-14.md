...
TL;DR: An autonomous roadside AI node for traffic violation detection and safety event dissemination, achieving high accuracy and energy efficiency in real-time.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the inability of traditional surveillance and manual ticketing to scale with the rapid motorization in emerging economies, leading to severe enforcement asymmetries.

Method: The method involves integrating YOLOv8 Nano for multi-object detection, DeepSORT for vehicle tracking, and a rule-guided OCR engine for license plate recognition, deployed on an NVIDIA Jetson Nano with TensorRT optimization.

Result: The system achieves 97.7% violation detection accuracy and 84.9% OCR precision, sustaining 28-30 FPS at 9.6 W, with a 10.7% mAP gain and 1.4x accuracy per watt improvement over benchmarks.

Conclusion: The paper concludes that the proposed autonomous roadside perception node effectively addresses enforcement asymmetries in rapidly motorizing economies by providing high-accuracy, energy-efficient traffic violation detection and safety event dissemination within a connected vehicle ecosystem.
...
TL;DR: The paper introduces the ROAD benchmark for evaluating LiDAR-based object classification under domain and label shifts, revealing limitations of current methods and setting baselines for future robust 3D perception research.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the underexplored area of continual and transfer learning in 3D point cloud perception, particularly under simultaneous domain and label shifts, which is crucial for real-world applications like autonomous driving and embodied AI.

Method: The proposed method involves the RObust Autonomous driving under Dataset shifts (ROAD) benchmark, which evaluates LiDAR-based object classification under domain shifts and three forms of label evolution: class split, class expansion, and class insertion. The evaluation includes zero-shot transfer, linear probe, and continual learning (CL), with an analysis of backbone architectures, training objectives, and CL methods.

Result: The results reveal limitations of existing approaches under realistic dataset shifts, providing insights into the effectiveness of various backbone architectures, training objectives, and CL methods.

Conclusion: The paper concludes by highlighting the limitations of existing approaches under realistic dataset shifts and establishes strong baselines for future research in robust 3D perception.
...
TL;DR: A high-quality, diverse aesthetic dataset generated by the Lunara model, released under Apache 2.0 for unrestricted use.


<details>
  <summary>Details</summary>
Motivation: To create a first-of-its-kind dataset with high aesthetic scores and stylistic diversity, surpassing existing aesthetics-focused and general-purpose datasets.

Method: Images are generated using the Moonworks Lunara model, accompanied by human-refined prompts and structured annotations describing objects, attributes, relationships, and stylistic cues.

Result: The dataset yields substantially higher aesthetic scores and includes diverse artistic styles from various regions and general categories like sketch and oil painting.

Conclusion: The Lunara Aesthetic Dataset is released under the Apache 2.0 license to support unrestricted academic and commercial use, prioritizing aesthetic quality and stylistic diversity.
...
TL;DR: LWMSCNN-SE, a lightweight CNN, achieves high accuracy (96.63%) with low computational costs, making it ideal for maize disease classification on edge devices.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of deploying traditional disease detection models in resource-constrained environments like smartphones and drones due to high computational costs.

Method: The model integrates multi-scale feature extraction, depthwise separable convolutions, and squeeze-and-Excitation (SE) attention mechanisms.

Result: The model achieves 96.63% classification accuracy with only 241,348 parameters and 0.666 GFLOPs, making it suitable for real-time deployment.

Conclusion: The LWMSCNN-SE model demonstrates significant potential for efficient maize disease diagnosis in resource-constrained environments, achieving high accuracy with low computational costs.
...
TL;DR: 3DGS-Drag is a point-based 3D editing framework enabling intuitive drag manipulation for real 3D scenes, combining deformation and 2D-editing methods with innovations in 3D Gaussian Splatting and diffusion guidance.


<details>
  <summary>Details</summary>
Motivation: The gap between deformation-based and 2D-editing-based 3D editing methods for geometry-related content editing remains a challenge, which 3DGS-Drag aims to address.

Method: Utilizes 3D Gaussian Splatting for deformation guidance and diffusion guidance for content correction and visual quality enhancement, supported by a progressive editing strategy.

Result: Enables a wide range of edits (motion change, shape adjustment, inpainting, content extension) with state-of-the-art performance, taking 10-20 minutes on an RTX 4090 GPU.

Conclusion: 3DGS-Drag effectively bridges the gap in 3D editing, offering efficient and intuitive manipulation for real 3D scenes with high-quality results.
...
TL;DR: An open-source sesame plant segmentation dataset with 292 images was created for AI applications, achieving strong detection and segmentation performance using YOLOv8.


<details>
  <summary>Details</summary>
Motivation: To support the development of AI models for sesame plant analysis in real-world farm settings, addressing the lack of precise segmentation datasets for this crop.

Method: The dataset includes 206 training, 43 validation, and 43 test images annotated using the Segment Anything Model version 2 with farmer supervision. Data were collected using a high-resolution mobile camera in Nigeria. Model evaluation was performed using the Ultralytics YOLOv8 framework.

Result: The model achieved strong performance metrics: for detection (recall: 79%, precision: 79%, mAP@0.50: 84%, mAP@0.50-0.95: 58%) and segmentation (recall: 82%, precision: 77%, mAP@0.50: 84%, mAP@0.50-0.95: 52%).

Conclusion: The Sesame Plant Segmentation Dataset is a valuable open-source resource for AI-driven agricultural applications, particularly for sesame plants, offering precise pixel-level segmentation and strong model performance metrics.
...
TL;DR: AKT, a novel transformer model with PKAN modules and PAA, outperforms SOTA methods in maize localization using UAV imagery, offering computational efficiency and accuracy for agricultural remote sensing.


<details>
  <summary>Details</summary>
Motivation: Address challenges in point-level maize localization in UAV imagery, including small object-to-pixel ratios, computational costs of high-resolution images, and agricultural scene complexities.

Method: Proposed the Additive Kolmogorov-Arnold Transformer (AKT), which replaces MLPs with PKAN modules for enhanced feature extraction and introduces PKAN Additive Attention (PAA) for efficient multiscale spatial dependency modeling. Also presented the PML dataset for evaluation.

Result: AKT achieves an average F1-score of 62.8%, outperforming state-of-the-art methods by 4.2%, reduces FLOPs by 12.6%, and improves inference throughput by 20.7%. For downstream tasks, it attains a mean absolute error of 7.1 in stand counting and a root mean square error of 1.95-1.97 cm in interplant spacing estimation.

Conclusion: The AKT model, integrating Kolmogorov-Arnold representation theory with efficient attention mechanisms, provides an effective framework for high-resolution agricultural remote sensing, demonstrated by superior performance in maize localization and downstream tasks.
...
TL;DR: A new statistical ideal observer model for visual search reduces free parameters via feature thresholds, applicable in medical imaging, computer vision, and defense.


<details>
  <summary>Details</summary>
Motivation: The paper aims to develop a new statistical ideal observer model for holistic visual search (gist) processing, with applications in medical image perception, computer vision, benchmarking, and defense/security.

Method: The model places thresholds on minimum extractable image features to reduce free parameters and simplify the system.

Result: The framework is applicable in optimizing imaging systems, algorithms, target detection, recognition, and evaluating sensors/detectors.

Conclusion: The proposed model offers a novel approach to visual search processing with broad applications across multiple fields.
...
TL;DR: Combining Gabor and GLCM features improves modeling of human visual search, showing strong correlation and consistency with actual gaze behavior.


<details>
  <summary>Details</summary>
Motivation: To understand the relationship between Gabor-based and GLCM-based texture features in modeling early-stage visual search behavior, with implications for attention allocation in location-unknown search tasks.

Method: Two feature-combination pipelines integrating Gabor and GLCM features were proposed and evaluated using simulated digital breast tomosynthesis images. The pipelines were compared with a threshold-based model observer.

Result: Qualitative agreement was found between predicted fixation candidates and the model observer. A strong correlation between GLCM mean and Gabor feature responses was observed. Eye-tracking data confirmed consistency between predicted fixation regions and early-stage gaze behavior.

Conclusion: The study highlights the value of combining structural (Gabor-based) and texture-based (GLCM) features for modeling visual search behavior, supporting the development of perceptually informed observer models.
...
TL;DR: The paper introduces CASHEW and CASHEW-RL to stabilize multi-step reasoning in vision-language models, achieving significant performance gains on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Vision-language models exhibit unstable multi-step reasoning, leading to divergent trajectories and inconsistent predictions.

Method: Two approaches: (1) CASHEW, an inference-time framework aggregating trajectories with visual verification; (2) CASHEW-RL, a learned variant trained with GSPO and a composite reward.

Result: Performance improvements up to +23.6pp on ScienceQA and +8.1pp on EgoSchema across 13 benchmarks.

Conclusion: The proposed methods effectively stabilize reasoning and improve performance, demonstrating the value of test-time scaling and adaptive training objectives.
...
TL;DR: TP-Blend is a training-free framework for simultaneous object and style editing in diffusion models, using dual attention processors for superior results.


<details>
  <summary>Details</summary>
Motivation: Existing text-conditioned diffusion editors fail when both a new object and a new style need to be introduced simultaneously.

Method: TP-Blend uses two attention processors: Cross-Attention Object Fusion (CAOF) for object blending and Self-Attention Style Fusion (SASF) for style injection, combining them in a single denoising trajectory.

Result: TP-Blend achieves high-resolution, photo-realistic edits with precise control over content and appearance, surpassing recent baselines.

Conclusion: TP-Blend effectively addresses the challenge of simultaneously introducing a new object and style in text-conditioned diffusion editing, outperforming existing methods in fidelity, quality, and speed.
...
TL;DR: A novel decoder-based deep learning framework generates manufacturable 3D structures for additive manufacturing, respecting constraints like overhang angles and wall thickness, and shows improved manufacturability.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of generating 3D structures that are not only geometrically valid but also manufacturable using additive manufacturing techniques, overcoming limitations of naive generation approaches.

Method: The method involves a deep learning framework that decodes latent representations into geometrically valid, printable objects while adhering to manufacturing constraints like overhang angles, wall thickness, and structural integrity.

Result: The results show that the neural decoders can learn complex mapping functions to produce valid 3D geometries, with significantly improved manufacturability compared to naive approaches. The approach was validated on diverse object categories and successfully demonstrated in practical 3D printing.

Conclusion: The paper concludes that the proposed decoder-based approach effectively generates manufacturable 3D structures optimized for additive manufacturing, demonstrating improved manufacturability and practical applicability.
...
TL;DR: The paper demonstrates early image-text alignment in vision-language models using a synthesis-based method, contradicting the belief that alignment only occurs in late layers.


<details>
  <summary>Details</summary>
Motivation: To challenge the established view that image-text alignment in vision-language models only appears in late layers and to provide a simple, fast method for measuring multimodal alignment without requiring auxiliary models or datasets.

Method: A new synthesis-based method inspired by DeepDream is introduced. Given a textual concept, its concept vector is extracted at a given layer, and optimization is used to synthesize an image whose representation aligns with that vector. This method is applied to hundreds of concepts across seven layers in Gemma 3.

Result: The synthesized images often depict salient visual features of the targeted textual concepts, with more than 50% of images at layer 1 depicting recognizable features of animals, activities, or seasons.

Conclusion: The paper provides direct, constructive evidence of early image-text alignment in adapter-based vision-language models, challenging the established view that such alignment only appears in late layers. It also offers a new method for model interpretability by visualizing the representation space.
...
TL;DR: A training-free, vision-only ZSAD framework using DDIM inversion and reconstruction achieves top performance on VISA, improving anomaly localization without auxiliary modalities or fine-grained prompts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of vision-only ZSAD methods, which lack spatial precision and rely on fine-grained prompts. The goal is to develop a training-free, vision-only framework that improves localization without auxiliary modalities.

Method: The method involves inverting an input image to obtain latent representations using a pretrained DDIM, then initiating the denoising process from a fixed intermediate timestep to reconstruct the image. The discrepancy between the input and reconstructed image highlights anomalies.

Result: The method achieves state-of-the-art performance on the VISA dataset, showcasing strong anomaly localization capabilities without the need for additional modalities or fine-grained prompts.

Conclusion: The paper concludes that the proposed training-free vision-only ZSAD framework, leveraging DDIM inversion and reconstruction, achieves state-of-the-art performance on the VISA dataset. It demonstrates strong anomaly localization capabilities without relying on auxiliary modalities, reducing dependence on fine-grained prompts.
...
TL;DR: The paper introduces Concept-Based Diversity (CBD), a highly efficient metric for image inputs using Vision-Language Models (VLM), which correlates strongly with Geometric Diversity (GD) but requires less computation time. A hybrid approach combining CBD with Margin (an uncertainty metric) is proposed and shown to outperform existing methods in improving DNN performance, while maintaining high efficiency and scalability.


<details>
  <summary>Details</summary>
Motivation: The high cost and time consumption of labeling new inputs for fine-tuning DNNs, and the limitations of existing diversity-based selection methods in terms of computational intensity and scalability.

Method: Introduction of Concept-Based Diversity (CBD) leveraging VLM, and a hybrid approach combining CBD with Margin for input selection. Comprehensive evaluation across various DNN models, input sets, and selection budgets.

Result: CBD-based selection consistently outperforms existing baselines in improving DNN performance, with high efficiency and scalability, even on large input sets like ImageNet.

Conclusion: The CBD-based approach is effective, computationally advantageous, and scalable, making it suitable for repetitive and extensive input selection scenarios.
...
TL;DR: FigEx2 is a visual-conditioned framework for panel localization and captioning in scientific figures, using noise-aware fusion and RL for superior performance and zero-shot transferability.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of panel-level understanding in scientific compound figures due to missing or figure-level captions.

Method: The framework employs a noise-aware gated fusion module for stable detection query space and a staged optimization strategy combining supervised learning with reinforcement learning (RL) using CLIP-based alignment and BERTScore-based semantic rewards.

Result: FigEx2 achieves 0.726 mAP@0.5:0.95 for detection and outperforms Qwen3-VL-8B by 0.51 in METEOR and 0.24 in BERTScore.

Conclusion: FigEx2 demonstrates superior performance in panel detection and captioning, with notable zero-shot transferability to out-of-distribution scientific domains.
...
TL;DR: A vision-language framework (Integscan) and benchmark (Rescind) are introduced for detecting and generating biomedical image forgeries, achieving top performance in ensuring scientific integrity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the growing threat of scientific image manipulation in biomedical publications, which undermines research integrity and reproducibility. The challenge lies in the domain-specific artifacts, complex textures, and unstructured figure layouts unique to biomedical images.

Method: The method involves a vision-language guided framework that combines diffusion-based synthesis with vision-language prompting for generating and detecting forgeries. It uses Rescind, a large-scale benchmark, and Integscan, a structured state space modeling framework with attention-enhanced visual encoding and prompt-conditioned semantic alignment. A vision-language model verification loop ensures semantic fidelity.

Result: Extensive experiments on Rescind and existing benchmarks show that Integscan achieves state-of-the-art performance in both detection and localization of biomedical image forgeries.

Conclusion: The paper concludes that the proposed Integscan framework, combined with the Rescind benchmark, establishes a strong foundation for automated scientific integrity analysis by achieving state-of-the-art performance in detecting and localizing biomedical image forgeries.
...
TL;DR: Adding controlled noise to training data (10%) improves CNN robustness to real-world corruptions without harming clean-data performance, as demonstrated on CIFAR-10 with ResNet-18.


<details>
  <summary>Details</summary>
Motivation: The motivation was to explore whether deliberately adding noise to training data could improve the robustness of CNNs for image classification tasks, given that real-world inputs often contain noise and distortions.

Method: The study used the CIFAR-10 dataset and evaluated the impact of three types of noise (Gaussian noise, Salt-and-Pepper noise, and Gaussian blur) at varying intensities and pollution levels during training. A ResNet-18 model was employed for these experiments.

Result: The results showed that incorporating just 10% noisy data during training significantly reduced test loss and improved accuracy under fully corrupted test conditions, with minimal negative effects on clean-data performance.

Conclusion: The paper concludes that strategically introducing controlled noise into training data can enhance the robustness of CNNs without significantly compromising performance on clean data.
...
TL;DR: DINO-AugSeg leverages DINOv3 features with wavelet-based augmentation and contextual fusion for few-shot medical image segmentation, outperforming existing methods across multiple modalities.


<details>
  <summary>Details</summary>
Motivation: Few-shot medical image segmentation is challenging due to limited annotated data. Existing self-supervised models like DINOv3 show potential but face domain adaptation issues in medical imaging.

Method: Proposes DINO-AugSeg with WT-Aug (wavelet-based feature augmentation) and CG-Fuse (contextual fusion module) to enhance DINOv3 features for medical images.

Result: Outperforms existing methods on six benchmarks spanning MRI, CT, ultrasound, endoscopy, and dermoscopy under limited-sample conditions.

Conclusion: DINO-AugSeg demonstrates effectiveness through wavelet-domain augmentation and contextual fusion, advancing few-shot medical image segmentation.
...
TL;DR: An automated pipeline for generating domain-specific synthetic datasets using diffusion models, addressing distribution shifts between pre-trained models and real-world deployment.


<details>
  <summary>Details</summary>
Motivation: To reduce reliance on extensive real-world data collection by creating high-quality synthetic datasets.

Method: A three-stage framework involving controlled inpainting for synthesis, multi-modal validation (object detection, aesthetic scoring, vision-language alignment), and user-preference classification.

Result: Efficient construction of deployable datasets with minimized distribution shift.

Conclusion: The pipeline successfully bridges the gap between synthetic data and real-world deployment needs.
...
TL;DR: PathoGen is a diffusion-based generative model for realistic lesion inpainting in histopathology images, outperforming existing methods and enhancing downstream segmentation performance.


<details>
  <summary>Details</summary>
Motivation: The scarcity of expert-annotated lesion data for rare pathologies and underrepresented disease subtypes limits AI model development in histopathology.

Method: PathoGen uses diffusion models to synthesize lesions with natural tissue boundaries, preserved cellular structures, and authentic staining characteristics.

Result: PathoGen outperforms state-of-the-art generative baselines in image fidelity and distributional similarity, and enhances downstream segmentation performance.

Conclusion: PathoGen offers a scalable solution to overcome the manual annotation bottleneck and develop generalizable medical AI systems with limited expert-labeled data.
...
TL;DR: The paper introduces SSP, a framework for AVSS that uses optical flow and textual prompts to improve segmentation accuracy, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of traditional audio-visual segmentation (AVS) by extending it to audio-visual semantic segmentation (AVSS), which requires deeper semantic understanding beyond just identifying sound-emitting objects.

Method: The authors propose a novel framework called Stepping Stone Plus (SSP), which decomposes AVSS into two subtasks using a pre-mask technique with optical flow and textual prompts. They also introduce a visual-textual alignment module (VTA) for cross-modal integration and a post-mask technique for training.

Result: SSP outperforms existing AVS methods, providing efficient and precise segmentation results, especially in scenarios with moving or stationary sound-emitting objects.

Conclusion: The SSP framework successfully advances AVSS by integrating motion dynamics and semantic prompts, offering a robust solution for complex audio-visual scenes.
...
TL;DR: SubTTA enhances TTA for VLMs by aligning semantic subspaces and filtering noise, improving performance by 2.24%.


<details>
  <summary>Details</summary>
Motivation: Vision-language models (VLMs) are vulnerable to distribution shifts, and existing TTA methods rely on unreliable zero-shot predictions due to modality gaps and visual nuisance.

Method: SubTTA aligns the semantic subspaces of visual and textual modalities by minimizing their chordal distance and projects aligned visual features onto the task-specific textual subspace to filter out noise. Standard TTA is then performed on the purified space.

Result: Extensive experiments show SubTTA improves performance by an average of 2.24% over state-of-the-art TTA methods.

Conclusion: SubTTA effectively addresses the limitations of existing TTA methods by aligning semantic subspaces and filtering out task-irrelevant noise, achieving an average improvement of 2.24% over state-of-the-art TTA methods.
...
TL;DR: A study on MLLMs reveals layer-specific visual-text fusion dynamics and introduces a training-free framework to improve multimodal reasoning by modeling attention shifts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how MLLMs internally integrate visual and textual information, as this process remains poorly understood despite their progress in vision-language tasks.

Method: The research employs a systematic layer-wise masking analysis across multiple MLLM architectures to investigate visual-text fusion dynamics. It also introduces a contrastive attention framework to model attention shifts.

Result: The results reveal that fusion occurs at specific layers, exhibits late-stage visual signal reactivation, and shows persistent noise in attention. The proposed framework enhances performance.

Conclusion: The study concludes that a training-free contrastive attention framework, informed by layer-wise analysis, effectively improves multimodal reasoning performance in MLLMs.
...
TL;DR: The paper introduces instance-aligned captions for explainable video anomaly detection (VAD) to address limitations in spatial grounding and verifiability, particularly in multi-entity interactions. It presents VIEW360+, an extended dataset, and demonstrates the effectiveness of the proposed framework.


<details>
  <summary>Details</summary>
Motivation: Current explainable VAD methods lack spatial grounding and verifiability, especially in complex multi-entity scenarios, reducing trustworthiness.

Method: The authors propose instance-aligned captions that link textual claims to specific object instances with appearance and motion attributes, enabling verifiable reasoning.

Result: The framework successfully grounds explanations spatially, addressing limitations in existing methods, and is validated through experiments on VIEW360+.

Conclusion: The work provides a robust benchmark for future research in trustworthy and interpretable anomaly detection, highlighting the importance of spatially grounded explanations.
...
TL;DR: A hardware-algorithm co-design framework using a custom SVE sensor and physics-aware dehazing algorithm effectively addresses extreme imaging challenges in rocket launches, enabling accurate recovery of visual data and extraction of critical mechanical parameters.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges of quantitatively measuring critical mechanical parameters (e.g., plume flow fields, shock wave structures, nozzle oscillations) during rocket launches due to extreme imaging conditions. Intense combustion causes dense particulate haze and high luminance variations, degrading image quality and undermining photogrammetric and velocimetric analyses.

Method: The study employs a hardware-algorithm co-design framework that integrates a custom Spatially Varying Exposure (SVE) sensor with a physics-aware dehazing algorithm. The SVE sensor captures multi-exposure data in a single shot, while the algorithm dynamically estimates haze density, optimizes illumination adaptively, and applies multi-scale entropy-constrained fusion to separate haze from scene radiance.

Result: The framework demonstrates superior performance in recovering physically accurate visual information of the plume and engine region, validated on real launch imagery and controlled experiments. It enables reliable extraction of key mechanical parameters such as particle velocity, flow instability frequency, and structural vibration.

Conclusion: The proposed hardware-algorithm co-design framework, combining a custom Spatially Varying Exposure (SVE) sensor with a physics-aware dehazing algorithm, effectively recovers accurate visual information of plume and engine regions under extreme conditions. This enables reliable extraction of critical mechanical parameters, supporting precise quantitative analysis in aerospace environments.
...
TL;DR: SISTA enhances medical VLP by aligning image-report and patch-word levels, eliminating false negatives, and improving transfer performance in tasks like classification and segmentation.


<details>
  <summary>Details</summary>
Motivation: Traditional contrastive learning in medical VLP treats all unpaired samples as negatives, which can disrupt semantic structure due to similarities between images or reports from different patients. This paper aims to address this limitation.

Method: The framework employs semantic-aware instance and sparse token alignments at two levels: image-report and patch-word. It improves contrastive learning by incorporating inter-report similarity to eliminate false negatives and aligns image patches with relevant word tokens.

Result: Experimental results show significant improvements in transfer performance on downstream tasks like image classification, segmentation, and object detection, particularly in fine-grained tasks with limited labeled data.

Conclusion: The proposed multi-level alignment framework, SISTA, effectively improves transfer performance across different datasets and tasks by addressing the limitations of traditional contrastive learning in medical VLP. It achieves notable improvements, especially in fine-grained tasks with limited labeled data.
...
TL;DR: Award-winning solution for cross-platform 3D object detection using PVRCNN++ with data augmentation and self-training, achieving high AP scores for Car and Pedestrian categories.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of cross-platform 3D object detection by narrowing domain gaps for better generalization.

Method: Built upon PVRCNN++, integrating point-based and voxel-based features, with improvements through tailored data augmentation and a self-training strategy using pseudo-labels.

Result: Achieved a 3D AP of 62.67% for Car (phase-1 target domain), and 58.76% (Car) and 49.81% (Pedestrian) on the phase-2 target domain.

Conclusion: The approach secured 3rd place in the RoboSense2025 Challenge, demonstrating strong performance in cross-platform 3D object detection with notable AP scores for Car and Pedestrian categories.
...
TL;DR: CogniMap3D is a bioinspired framework for dynamic 3D scene understanding and reconstruction, emulating human cognitive processes. It integrates motion cues, cognitive mapping, and optimization for efficient scene storage and retrieval.


<details>
  <summary>Details</summary>
Motivation: The framework aims to enhance dynamic 3D scene understanding by mimicking human cognitive processes, enabling efficient spatial knowledge storage and rapid retrieval.

Method: CogniMap3D uses a multi-stage motion cue framework for dynamic object identification, a cognitive mapping system for static scene storage and updates, and factor graph optimization for refining camera poses.

Result: The model achieves state-of-the-art performance in video depth estimation, camera pose reconstruction, and 3D mapping tasks, supporting continuous scene understanding across extended sequences and multiple visits.

Conclusion: CogniMap3D effectively integrates cognitive processes with technical capabilities, demonstrating robust performance in dynamic 3D scene understanding and reconstruction.
...
TL;DR: A new framework for text-driven facial expression transitions in 3D avatars, achieving state-of-the-art performance on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: To simulate realistic emotional variation in 3D avatars by enabling transitions between arbitrary facial expressions based on text instructions.

Method: The framework includes the Instruction-driven Facial Expression Decomposer (IFED) module for multimodal data learning, the Instruction to Facial Expression Transition (I2FET) method for refining semantic comprehension, and the Facial Expression Transition model for smooth transitions.

Result: The proposed model outperforms state-of-the-art methods on CK+ and CelebV-HQ datasets, demonstrating its ability to generate accurate and smooth facial expression transitions.

Conclusion: The study presents a novel framework for generating facial expression transitions driven by textual instructions, enabling realistic and diverse emotional simulations.
