...
TL;DR: A new cylindrical fiducial marker called PuzzlePole is introduced, derived from PuzzleBoard, allowing 360° recognition and accurate pose estimation for autonomous systems.


<details>
  <summary>Details</summary>
Motivation: Reliable environment perception is crucial for autonomous systems, which often depend on robust visual markers for calibration and localization.

Method: Developed PuzzlePole, a cylindrical marker based on the PuzzleBoard pattern, utilizing its combinatorial structure for reliable recognition from any viewing angle.

Result: PuzzlePoles enable high-accuracy localization and orientation, with robustness to occlusions, suitable for various applications like robot navigation and SLAM.

Conclusion: The PuzzlePole marker offers flexible, reliable performance for autonomous systems, enhancing tasks requiring precise environmental perception.
...
TL;DR: PIGReward is a personalized reward model that evaluates text-to-image generation using user-specific preferences and Chain-of-Thought reasoning, enabling better alignment with individual visual tastes without requiring extensive user data.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods for text-to-image models fail to capture personal visual preferences, as they rely on general metrics rather than individual user tastes.

Method: PIGReward uses a self-bootstrapping strategy to construct rich user contexts from limited reference data and employs CoT reasoning to dynamically generate user-conditioned evaluation dimensions.

Result: PIGReward outperforms existing methods in accuracy and interpretability, and it enables personalized prompt optimization for better image-user intent alignment.

Conclusion: PIGReward establishes a scalable, reasoning-based approach for personalized T2I evaluation and optimization, advancing individually aligned image generation.
...
TL;DR: SG-OIF introduces a stability-guided online framework for efficient training-point influence estimation in deep learning vision models, overcoming computational and calibration limitations of prior methods.


<details>
  <summary>Details</summary>
Motivation: Current influence function implementations for deep learning vision models face challenges with expensive inverse-curvature computations, non-stationary training dynamics, and fragile confidence calibration that misidentifies critical examples.

Method: Proposes a Stability-Guided Online Influence Framework (SG-OIF) that uses algorithmic stability as a real-time controller, featuring lightweight anchor IHVPs via stochastic Richardson and preconditioned Neumann methods, plus modular curvature backends with stability-guided residual thresholds, anomaly gating, and confidence modulation.

Result: SG-OIF achieves SOTA performance on noise-label and out-of-distribution detection tasks across multiple datasets, including 91.1% accuracy in top 1% prediction samples on CIFAR-10 (20% asymmetric noise) and 99.8% AUPR on MNIST.

Conclusion: The framework demonstrates practical effectiveness for online influence estimation in deep-learning vision models, addressing key limitations of previous approaches through stability-guided control.
...
TL;DR: Pistachio is a new VAD/VAU benchmark created using AI video generation to overcome limitations of current datasets, offering controlled scenes, anomalies, and temporal narratives.


<details>
  <summary>Details</summary>
Motivation: Current Video Anomaly Detection (VAD) benchmarks lack scene diversity, balanced anomaly coverage, and temporal complexity. Video Anomaly Understanding (VAU) requires deeper reasoning but is hard to benchmark due to manual annotation needs.

Method: A generation-based pipeline using video generation models for scene-conditioned anomaly assignment, multi-step storyline generation, and temporally consistent synthesis to create 41-second videos with minimal human input.

Result: Pistachio provides large-scale, diverse, and complex benchmarks, exposing new challenges for existing methods and advancing dynamic, multi-event anomaly research.

Conclusion: Pistachio effectively addresses VAD/VAU benchmarking gaps through controlled generation, enabling more reliable assessment and motivating future work on anomaly understanding.
...
TL;DR: SATA is a universal framework that unifies various tracking and segmentation subtasks across different modalities using a Decoupled Mixture-of-Expert mechanism and Task-aware Multi-object Tracking pipeline.


<details>
  <summary>Details</summary>
Motivation: Existing approaches use specialized architectures for different tracking/segmentation tasks and modalities, creating distribution gaps between modalities and feature representation gaps between tasks that limit cross-task/modal knowledge sharing.

Method: Proposes Decoupled Mixture-of-Expert (DeMoE) to separate unified representation learning into cross-modal shared knowledge and specific information modeling, and Task-aware Multi-object Tracking (TaMOT) to unify task outputs as instances with calibrated IDs.

Result: SATA achieves superior performance on 18 challenging tracking and segmentation benchmarks.

Conclusion: SATA provides a novel perspective for generalizable video understanding by effectively addressing cross-modal and cross-task gaps while maintaining model flexibility.
...
TL;DR: Novel DRaM approach solving exact 3D-3D (EnP) and 3D-2D orthographic (OnP) pose estimation problems, placing existing QR/pseudoinverse methods in unified framework and generalizing to N-dimensional Euclidean spaces.


<details>
  <summary>Details</summary>
Motivation: Address the lack of closed-form solutions for noisy 3D-2D orthographic pose estimation (OnP) and unify previous QR/pseudoinverse methods under a coherent mathematical framework.

Method: Determinant ratio matrix (DRaM) approach for least-squares solutions of error-free EnP and OnP problems, with rotation correction for noisy data.

Result: Provides exact solutions for EnP and new DRaM-class solutions for OnP, showing these methods generalize to N-dimensional Euclidean pose estimation.

Conclusion: DRaM framework offers both practical solutions and deeper mathematical insights, revealing connections that could have been discovered centuries earlier.
...
TL;DR: LatentDreamer is a framework for fast, high-fidelity 3D object generation from a single image using a pre-trained VAE and a sequential pipeline.


<details>
  <summary>Details</summary>
Motivation: Automatic 3D generation often fails to achieve fast, detailed, and high-fidelity generation simultaneously.

Method: Uses a pre-trained variational autoencoder to map 3D geometries to latent features, then a pipeline generates coarse geometries, refined geometries, and textures sequentially.

Result: Generates 3D objects with high fidelity to input images in about 70 seconds, showing competitive performance with minimal training.

Conclusion: LatentDreamer effectively addresses the challenge of simultaneous speed and quality in 3D generation.
...
TL;DR: This paper analyzes the relationship between model capacity and visual tokens needed for semantic preservation, proposing Orthogonal Filtering to reduce redundancy and discovering that larger models require fewer tokens.


<details>
  <summary>Details</summary>
Motivation: Inspired by the Minimum Description Length principle, the study aims to determine the minimal set of visual tokens necessary to preserve image semantics by reinterpreting tokens as vectors in a semantic space.

Method: The authors propose Orthogonal Filtering, a lightweight module that adaptively clusters redundant tokens into a compact set of orthogonal bases, and validate it through experiments on various Vision Transformer models.

Result: Experiments reveal a consistent scaling law: larger models need significantly fewer tokens to span the visual semantic space. Additionally, a visual long-context dataset is contributed.

Conclusion: The findings demonstrate that model capacity inversely correlates with the number of tokens required for semantic representation, offering insights for efficient visual processing.
...
TL;DR: GroundingAgent is a zero-shot visual grounding framework that uses pretrained models without fine-tuning, achieving 65.1% accuracy on benchmarks through iterative reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing visual grounding methods require extensive task-specific annotations and fine-tuning, limiting generalization to novel scenarios.

Method: Integrates pretrained open-vocabulary object detectors, MLLMs, and LLMs in an iterative reasoning mechanism for joint semantic and spatial analysis.

Result: Achieves 65.1% zero-shot grounding accuracy on RefCOCO benchmarks; accuracy reaches ~90% when using MLLM-generated captions instead of original queries.

Conclusion: GroundingAgent demonstrates strong zero-shot performance and interpretability, highlighting the importance of LLM reasoning in visual grounding tasks.
...
TL;DR: InfoPrune is an information-theoretic framework for compressing vision-language models that achieves 3.2x FLOP reduction with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Current VLMs face deployment challenges due to large scale, and existing compression methods lack theoretical guarantees about information preservation.

Method: Uses Information Bottleneck principle with entropy-based eRank and KS distance to prune attention heads, plus training-free FFN compression via low-rank approximation.

Result: Achieves up to 3.2x FLOP reduction and 1.8x acceleration on VQAv2, TextVQA, and GQA with negligible performance degradation.

Conclusion: InfoPrune provides a theoretically grounded and practical approach for efficient multimodal models.
...
TL;DR: The paper introduces Eyelid Angle (ELA), a novel 3D-based metric for measuring eye openness that improves drowsiness detection. It enables accurate blink analysis and synthetic dataset generation for better driver monitoring systems.


<details>
  <summary>Details</summary>
Motivation: Current drowsiness detection methods using 2D metrics like EAR are sensitive to camera angle variations, creating reliability issues. There's also a shortage of natural drowsiness data due to collection difficulties and risks.

Method: Developed ELA from 3D facial landmarks as a stable geometric measure of eyelid motion. Created blink detection framework extracting temporal characteristics. Used ELA to animate avatars in Blender 3D for generating realistic synthetic datasets with controllable parameters.

Result: ELA showed lower variance under viewpoint changes compared to EAR and achieved accurate blink detection. Synthetic data augmentation successfully expanded training diversity for drowsiness recognition.

Conclusion: ELA serves as both a reliable biometric measure for drowsiness detection and a powerful tool for creating scalable synthetic datasets, advancing driver state monitoring systems.
...
TL;DR: VideoChat-M1 is a multi-agent system using collaborative policy planning to dynamically invoke tools for improved video understanding, achieving SOTA results on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent systems use static tool invocation, limiting their ability to discover diverse clues needed for complex video understanding.

Method: Collaborative Policy Planning (CPP) paradigm with multiple policy agents that generate unique policies, execute them with tools, and communicate to update policies during execution. Enhanced with Multi-Agent Reinforcement Learning for joint optimization.

Result: SOTA performance across eight benchmarks, outperforming Gemini 2.5 pro by 3.6% and GPT-4o by 15.6% on LongVideoBench.

Conclusion: VideoChat-M1 demonstrates that dynamic, collaborative multi-agent planning significantly advances video understanding capabilities.
...
TL;DR: This paper introduces Perceptual Taxonomy, a benchmark for physically grounded visual reasoning that evaluates vision-language models' ability to perform structured scene understanding beyond surface-level recognition.


<details>
  <summary>Details</summary>
Motivation: Current vision-language benchmarks lack comprehensive evaluation of human-like perceptual reasoning that involves recognizing objects, their spatial configurations, and inferring task-relevant properties (material, affordance, function, physical attributes) for goal-directed reasoning.

Method: Created a benchmark by annotating 3173 objects with 84 fine-grained attributes across 4 property families. Constructed 28033 template-based questions across synthetic and real domains (5802 images) covering 4 reasoning types, plus 50 expert-crafted questions.

Result: Leading vision-language models perform well on recognition tasks but degrade by 10-20% on property-driven questions, especially those requiring multi-step reasoning. Models relying heavily on pattern matching show persistent gaps in structured visual understanding.

Conclusion: Perceptual Taxonomy reveals significant limitations in current models' structured reasoning abilities. Providing in-context reasoning examples from simulated scenes improves performance, demonstrating the effectiveness of perceptual-taxonomy-guided prompting for advancing physically grounded visual reasoning.
...
TL;DR: MapRF is a weakly supervised framework that constructs 3D HD maps using only 2D image labels, avoiding costly 3D annotations through a self-training approach with NeRF-based pseudo labels and Map-to-Ray Matching.


<details>
  <summary>Details</summary>
Motivation: Existing online HD map construction methods require expensive 3D annotations for training, limiting generalization and scalability across diverse driving environments.

Method: Proposes MapRF with a NeRF module conditioned on map predictions to generate view-consistent 3D pseudo labels, refined iteratively via self-training, and a Map-to-Ray Matching strategy to align predictions with 2D labels.

Result: Experiments on Argoverse 2 and nuScenes show MapRF achieves ~75% of fully supervised baseline performance, surpassing other 2D-label-only methods.

Conclusion: MapRF enables scalable, cost-effective online HD map construction for autonomous driving by reducing annotation dependency.
...
TL;DR: Vidi2 is a next-generation video understanding model that advances beyond temporal retrieval to include fine-grained spatio-temporal grounding and video question answering. It introduces new benchmarks (VUE-STG and VUE-TR-V2) and outperforms proprietary systems.


<details>
  <summary>Details</summary>
Motivation: The growing demand for scalable, high-quality video production drives the need for advanced video understanding capabilities that go beyond basic temporal retrieval to enable complex multimodal reasoning.

Method: Vidi2 extends video understanding with end-to-end spatio-temporal grounding, allowing it to identify both timestamps and bounding boxes of objects given text queries. It introduces improved benchmarks with longer video durations, refined queries, and enhanced evaluation metrics.

Result: Vidi2 substantially outperforms leading proprietary systems (Gemini 3 Pro, GPT-5) on VUE-TR-V2 and VUE-STG benchmarks and achieves competitive results with open-source models on video QA tasks.

Conclusion: Vidi2 represents a significant advancement in video understanding, enabling practical applications in complex editing scenarios through its comprehensive spatio-temporal grounding capabilities and robust benchmark performance.
...
TL;DR: A multimodal LLM outperforms traditional CV models in cross-domain generalization for global photovoltaic assessment, showing robustness under domain shift.


<details>
  <summary>Details</summary>
Motivation: The rapid expansion of distributed PV systems poses grid management challenges due to undocumented installations. Satellite imagery offers coverage but traditional CV models like CNNs and U-Nets require extensive labeled data and fail to generalize across regions.

Method: The study investigates cross-domain generalization using a multimodal large language model. It employs structured prompts and fine-tuning to integrate detection, localization, and quantification within a unified schema.

Result: Cross-regional evaluation using the ΔF1 metric shows the proposed model achieves the smallest performance degradation across unseen regions, outperforming conventional CV and transformer baselines.

Conclusion: The results highlight the robustness of multimodal LLMs under domain shift and their potential for scalable, transferable, and interpretable global PV mapping.
...
TL;DR: This research develops computational methods to analyze cartographic heritage at scale, using semantic segmentation and object detection on nearly 1 million historical maps to uncover cultural and political insights about map production over six centuries.


<details>
  <summary>Details</summary>
Motivation: Current automated map analysis methods lack engagement with cartographic history and cultural perspectives. Maps are semantic-symbolic systems reflecting political and epistemic expectations, requiring cultural analysis.

Method: Leverages a corpus of 771,561 map records and 99,715 digitized images from 38 catalogs. Uses semantic segmentation and object detection models trained on annotated/synthetic data for recognizing land classes and cartographic signs.

Result: Revealed spatial focus linked to political dynamics (Atlantic charting, triangular trade, colonialism). Documented progression of national focus and military impacts. Analysis showed maps emphasize features through framing/composition. Identified figurative shifts like hachure-to-contour replacement. Found signs form locally consistent systems.

Conclusion: Large-scale computational analysis reveals maps as cultural artifacts reflecting historical contexts. Collaborative patterns show legitimacy, major actors, and cities drive figurative norm diffusion. The approach enables new cultural perspectives on cartographic heritage.
...
TL;DR: SpLap is a novel proxy-free deformation method for Gaussian splats that uses a Laplacian operator from a surface-aware splat graph, outperforming existing approaches while preserving details and topology.


<details>
  <summary>Details</summary>
Motivation: Existing GS deformation methods rely on deformation proxies (cages/meshes) which depend on proxy quality and add computational overhead. Direct Laplacian-based methods treating splats as point clouds often fail to capture surface structure due to lack of explicit topology.

Method: Constructs a surface-aware splat graph where neighboring splats are defined by intersections rather than just center distances. Uses a Laplacian operator derived from this graph and introduces Gaussian kernel adaptation to preserve surface structure during deformation.

Result: Superior performance compared to both proxy-based and proxy-free baselines, demonstrated on 50 challenging objects from ShapeNet, Objaverse, Sketchfab, and NeRF-Synthetic datasets.

Conclusion: SpLap provides more plausible deformations that preserve details and topology while being proxy-free, addressing limitations of existing methods.
...
TL;DR: Proposes ThiFAN-VQA, a two-stage VQA framework combining CoT reasoning and answer selection for disaster damage assessment, achieving better accuracy and interpretability than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current AI methods for disaster damage assessment face limitations: costly data annotation, restricted answer spaces in classification models, and unreliable outputs from generative models.

Method: Two-stage framework: 1) Generate reasoning traces using chain-of-thought prompting and in-context learning, 2) Answer selection module evaluates responses for coherence and accuracy.

Result: Superior performance on FloodNet and RescueNet-VQA datasets, demonstrating improved accuracy, interpretability, and adaptability for real-world damage assessment tasks.

Conclusion: ThiFAN-VQA effectively bridges zero-shot and supervised methods, offering flexible yet consistent disaster damage assessment with practical applicability.
...
TL;DR: HunyuanOCR is a lightweight (1B parameter) vision-language model that unifies OCR tasks (spotting, parsing, IE, VQA, translation) in an end-to-end architecture, achieving SOTA performance and beating larger models while being commercially viable.


<details>
  <summary>Details</summary>
Motivation: To address limitations of narrow OCR expert models and inefficient general VLMs by creating a unified, efficient model that eliminates error propagation from traditional pipeline approaches.

Method: Uses a Native Vision Transformer (ViT) connected to a lightweight LLM via MLP adapter in a pure end-to-end architecture, trained with high-quality data and reinforcement learning strategies.

Result: Outperforms commercial APIs, traditional pipelines, and larger models; achieves 1st place in ICDAR 2025 DIMT Challenge (Small Model Track) and SOTA on OCRBench for models under 3B parameters.

Conclusion: HunyuanOCR successfully demonstrates that lightweight models can achieve superior OCR performance through unified architecture and advanced training strategies, providing a solid foundation for industrial applications.
...
TL;DR: Semi-supervised GAN method for early ischemic stroke segmentation using limited labeled and abundant unlabeled NCCT scans.


<details>
  <summary>Details</summary>
Motivation: NCCT often fails to detect subtle early ischemic changes, delaying critical interventions.

Method: Adversarial framework with Dice loss, cross-entropy loss, feature matching loss, and self-training loss.

Result: Demonstrated potential on AISD dataset to improve diagnostic accuracy and reduce annotation burden.

Conclusion: The proposed method can enhance stroke diagnosis and support more efficient clinical decision-making.
...
TL;DR: This paper introduces MSVQ-VAE, a multiscale VAE model for generating synthetic WCE images with abnormalities, showing comparable performance to real data in CDS classification.


<details>
  <summary>Details</summary>
Motivation: Manual screening of WCE images is time-consuming, and deep learning CDS systems need large, diverse datasets which are scarce due to privacy and annotation costs. Current SDG methods struggle with training stability and visual diversity for abnormal findings.

Method: Proposes Multiscale Vector Quantized Variational Autoencoder (MSVQ-VAE), which extends VQ-VAE to multiscale for stable training. It generates synthetic WCE images by seamlessly introducing abnormalities into normal images and allows conditional generation of various abnormality types like polyps and vascular conditions.

Result: Experiments show that training a CDS classifier with synthetic abnormal images from MSVQ-VAE yields classification results comparable to using only real data.

Conclusion: MSVQ-VAE effectively addresses data scarcity for WCE imaging, with potential applicability to other medical multimedia domains, enhancing CDS development.
...
TL;DR: SkillSight introduces gaze-only skill assessment achieving high accuracy with 73x less power consumption than video-based methods.


<details>
  <summary>Details</summary>
Motivation: Automatic skill assessment remains challenging for egocentric perception on smart glasses, while continuous video processing consumes excessive power.

Method: Two-stage framework: teacher model jointly learns from gaze and video data, then distills knowledge to gaze-only student model for inference.

Result: Achieves state-of-the-art performance on three datasets (cooking, music, sports) and maintains high accuracy while reducing power consumption by 73x.

Conclusion: Gaze provides valuable skill indicators across diverse settings, enabling practical AI-supported skill learning applications.
...
TL;DR: Vision-language foundation models enhance MRI reconstruction by aligning semantic representations with target distributions.


<details>
  <summary>Details</summary>
Motivation: To leverage high-level contextual information from foundation models beyond conventional MRI reconstruction priors.

Method: A semantic distribution-guided framework using contrastive learning to align reconstructed images with target semantic distributions from vision-language models.

Result: Superior preservation of anatomical structures and perceptual quality compared to conventional methods, with image-language information enabling reconstruction attribute control.

Conclusion: Vision-language models effectively improve MRI reconstruction through semantic-space optimization.
...
TL;DR: GIANT enables LMMs to navigate whole-slide pathology images agentically, achieving expert-level accuracy on clinical questions.


<details>
  <summary>Details</summary>
Motivation: General-purpose large multimodal models underperform in medical image diagnosis due to inadequate input representation (low-res thumbnails or random patches).

Method: Developed GIANT framework for iterative navigation of gigapixel WSIs and MultiPathQA benchmark with 934 WSI-level questions across clinical tasks.

Result: GPT-5 with GIANT achieved 62.5% accuracy on pathologist-authored questions, surpassing specialist models like TITAN (43.8%) and SlideChat (37.5%).

Conclusion: Agentic navigation enables LMMs to approach specialist performance in pathology, revealing both capabilities and limitations of foundation models for medical reasoning.
...
TL;DR: CodeV is a code-based visual agent trained with Tool-Aware Policy Optimization (TAPO), which improves faithfulness in visual tool use while maintaining high accuracy on visual search and other reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models achieve high final-answer accuracy but exhibit unfaithful visual reasoning, such as using tools on irrelevant regions or ignoring tool outputs.

Method: Proposes TAPO, a process-level RL framework that assigns step-wise rewards based on visual tool inputs/outputs rather than chain-of-thought tokens, and introduces CodeV which represents tools as executable Python code.

Result: CodeV achieves competitive or superior accuracy while substantially increasing faithful tool-use rates on visual search benchmarks, and performs well on multimodal reasoning and math tasks.

Conclusion: Explicitly supervising intermediate tool behavior is crucial for building trustworthy, agentic visual reasoning systems.
...
TL;DR: OncoVision is a multimodal AI system that combines mammography images and clinical data to improve breast cancer diagnosis through joint segmentation and clinical feature prediction.


<details>
  <summary>Details</summary>
Motivation: To enhance diagnostic precision, reduce inter-observer variability, and provide scalable breast cancer screening solutions for underprivileged areas.

Method: Uses attention-based encoder-decoder architecture for joint segmentation of four ROIs and prediction of ten clinical features; implements two late-fusion strategies to combine imaging and clinical data.

Result: Achieves state-of-the-art accuracy in segmentation and robust clinical feature prediction; produces structured reports with dual-confidence scoring and attention-weighted visualizations.

Conclusion: OncoVision sets a new standard for AI-based mammography by offering an accurate, scalable, and equitable solution for early breast cancer detection that can be integrated into clinical workflows worldwide.
...
TL;DR: INTERLACE is a new framework that prunes redundant layers in Vision-Language Models (VLMs) while maintaining performance through efficient finetuning.


<details>
  <summary>Details</summary>
Motivation: Existing layer pruning methods cause significant performance drops when applied to VLMs, creating a need for a more effective approach.

Method: The method analyzes triplets of consecutive layers to identify local redundancy, removes the most redundant layer from the first two, finetunes the remaining layer to compensate, and freezes the third layer as an anchor during finetuning.

Result: INTERLACE achieves 88.9% average performance retention after pruning 25% of the network when finetuned on only 1% of the FineVision dataset for one epoch.

Conclusion: The interleaved finetune-freeze design enables rapid convergence with minimal data, establishing state-of-the-art performance for VLM layer pruning.
...
TL;DR: Introduces IndEgo, a multimodal dataset for industrial tasks featuring both egocentric and exocentric recordings with rich annotations and benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address the need for comprehensive multimodal datasets that capture collaborative industrial work with detailed context like eye gaze and narration.

Method: Collection of 3,460 egocentric and 1,092 exocentric recordings, annotated with actions, summaries, mistakes, and processed outputs including eye gaze and hand pose.

Result: Baseline evaluations indicate the dataset poses challenges for state-of-the-art multimodal models in tasks like Mistake Detection and Question Answering.

Conclusion: IndEgo provides a valuable resource for advancing multimodal understanding in industrial settings, with data publicly available.
...
TL;DR: A novel prototype-based method that achieves interpretable cell counting via density map estimation without compromising performance, validated by biologists.


<details>
  <summary>Details</summary>
Motivation: Interpretability of deep learning models in cell counting remains a significant challenge for clinical adoption.

Method: Integrates a prototype layer into density estimation network to learn representative visual patterns for cells and background artifacts.

Result: Extensive experiments on two public datasets show achieved interpretability without compromising counting effectiveness.

Conclusion: Provides researchers and clinicians with a transparent and reliable tool for cell counting, increasing trust in deep learning for biomedical applications.
