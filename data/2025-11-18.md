...
TL;DR: The paper develops an analytic framework to test conditions for unbounded AI capability growth, providing measurable criteria and safety controls to prevent runaway scenarios.


<details>
  <summary>Details</summary>
Motivation: The study aims to replace speculation about AI singularity with precise, testable conditions and practical safety measures.

Method: An analytic framework links capability growth to resource build-out and deployment policies, using physical and information-theoretic limits to define a service envelope. Decision rules map observable data into yes/no certificates for runaway behavior.

Result: The framework provides falsifiable tests and safety controls (e.g., power caps, throughput throttling) to certify or preclude unbounded AI growth. Analytical case studies illustrate when limits bind.

Conclusion: The results offer testable conditions and deployable controls to address AI singularity concerns, though limitations like dependence on capability metrics remain for future work.
...
TL;DR: CAR-Scenes is a dataset for autonomous driving, annotating 5,192 images with 350+ attributes using GPT-4o and human verification. It supports vision-language models and includes tools for semantic retrieval and risk-aware mining.


<details>
  <summary>Details</summary>
Motivation: To enable interpretable, scene-level understanding in autonomous driving by providing a detailed, annotated dataset for training and evaluating vision-language models.

Method: Annotations are created using a GPT-4o-assisted vision-language pipeline with human verification. The dataset includes attribute co-occurrence graphs, JSONL records, and reproducible baselines like a LoRA-tuned Qwen2-VL-2B model.

Result: The dataset provides 350+ leaf attributes, tools for semantic retrieval, and baseline model performance metrics (accuracy, F1, MAE/RMSE).

Conclusion: CAR-Scenes facilitates explainable, data-centric workflows for autonomous driving research, with publicly released scripts and annotations.
...
TL;DR: A scalable and efficient method for data attribution in text-to-image models, reducing computation time by 2,500x - 400,000x compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing data attribution methods are computationally expensive, making them impractical for real-world applications.

Method: Distills a slow, unlearning-based attribution method into a feature embedding space for efficient retrieval of influential training images, combined with indexing and search methods.

Result: Achieves better or competitive performance in seconds, significantly faster than existing methods.

Conclusion: The work advances the practical application of data attribution methods in large-scale models like Stable Diffusion.
...
TL;DR: ColoWorkflow, a new video-based assessment tool for minimally invasive colorectal surgery, was developed and validated, showing broad applicability and moderate reliability, aiming to standardize training and improve surgical quality.


<details>
  <summary>Details</summary>
Motivation: Minimally invasive colorectal surgery suffers from procedural variability, a steep learning curve, and complications affecting outcomes. VBA can provide data-driven insights to address these challenges, but existing tools lack standardization.

Method: A Delphi process was used to achieve consensus on workflow descriptors, leading to the development of ColoWorkflow. Independent raters applied it to a multicentre video dataset of laparoscopic and robotic CRS, evaluating applicability and inter-rater reliability.

Result: Consensus was achieved for 10 procedure-agnostic phases and 34 procedure-specific steps. ColoWorkflow showed broad applicability (used in 54 videos) and moderate inter-rater reliability (Cohen's K of 0.71 for phases, 0.66 for steps).

Conclusion: ColoWorkflow is the first consensus-based, validated VBA tool for comprehensive workflow analysis in minimally invasive CRS, offering a reproducible framework for performance assessment and potential for AI-driven workflow recognition.
...
TL;DR: The paper introduces FVMGN, a frequency-aware vision-language network for RS image classification, addressing multimodality generalization through innovative modules like DTAug, MWDis, SFIE, and MSFFA.


<details>
  <summary>Details</summary>
Motivation: The need to overcome data heterogeneity and lack of proprietary linguistic knowledge in RS vision-language models drives the development of FVMGN.

Method: FVMGN employs DTAug for multimodal land-cover reconstruction, MWDis for frequency-domain feature learning, SFIE for image encoding, and MSFFA for semantic space alignment.

Result: FVMGN outperforms SOTA methods in multimodality generalization for RS image classification.

Conclusion: FVMGN effectively addresses RS multimodality generalization, demonstrating superior performance through its novel modules.
...
TL;DR: GFT introduces a point-cloud-specific PEFT method using dynamic graphs and lightweight modules, reducing parameters without performance loss.


<details>
  <summary>Details</summary>
Motivation: General PEFT approaches are suboptimal for point cloud data, prompting the need for a tailored solution to further reduce trainable parameters without compromising performance.

Method: The proposed GFT learns a dynamic graph from initial tokenized inputs using a lightweight graph convolution network and integrates these features into deeper layers via skip connections and efficient cross-attention modules.

Result: Extensive experiments demonstrate that GFT rivals existing methods in performance while significantly reducing the number of trainable parameters.

Conclusion: GFT, a point-cloud-specific PEFT method, effectively reduces trainable parameters while maintaining competitive performance in object classification and segmentation tasks.
...
TL;DR: A new pruning method using LRP improves pruning rates and accuracy preservation without fine-tuning, outperforming existing approaches.


<details>
  <summary>Details</summary>
Motivation: Existing LRP-based pruning methods suffer from significant accuracy degradation, limiting their practical usability. The study aims to address this issue.

Method: The method leverages Layer-wise Relevance Propagation (LRP) for pruning without requiring fine-tuning, making it suitable for scenarios with limited data.

Result: The proposed method achieves a higher pruning rate while preserving better model accuracy compared to existing methods.

Conclusion: The study proposes a pruning method that achieves higher pruning rates while better preserving model accuracy, outperforming existing LRP-based approaches.
...
TL;DR: A Short-Window Sliding Learning framework for real-time violence detection in CCTV footage achieves high accuracy by using 1-2 second clips and LLM-based auto-caption labeling.


<details>
  <summary>Details</summary>
Motivation: To improve real-time violence detection by addressing the limitations of conventional long-video training approaches.

Method: Divides videos into 1-2 second clips, applies LLM-based auto-caption labeling for fine-grained datasets, and utilizes all frames for temporal continuity.
...
Conclusion: The proposed framework is effective for real-time violence detection, offering high accuracy and improved performance in intelligent surveillance systems.
...
TL;DR: MCN-CL, a novel multimodal emotion recognition method, uses cross-attention and contrastive learning to address modal heterogeneity and imbalance, outperforming state-of-the-art approaches.


<details>
  <summary>Details</summary>
Motivation: The need for an efficient cross-modal fusion framework arises from challenges like unbalanced category distribution, dynamic facial action unit complexity, and modal heterogeneity in multimodal emotion recognition.

Method: The paper introduces Multimodal Cross-Attention Network and Contrastive Learning (MCN-CL), utilizing a triple query mechanism and hard negative mining strategy for feature redundancy reduction and emotional cue preservation.

Result: Experiments on IEMOCAP and MELD datasets show MCN-CL improves Weighted F1 scores by 3.42% and 5.73%, respectively, over existing methods.

Conclusion: The proposed MCN-CL method effectively addresses challenges in multimodal emotion recognition, such as modal heterogeneity and category imbalance, and outperforms state-of-the-art methods on benchmark datasets.
...
TL;DR: A competitive and efficient method for probabilistic rainfall nowcasting using a V-JEPA Vision Transformer and lightweight probabilistic head outperforms 3D-UNET baselines.


<details>
  <summary>Details</summary>
Motivation: To improve the accuracy and computational efficiency of probabilistic rainfall nowcasting.

Method: Uses a V-JEPA Vision Transformer with a lightweight probabilistic head attached to a pre-trained satellite vision encoder (DINOv3-SAT493M) to map encoder tokens into a discrete eCDF over 4-hour rainfall. Optimized end-to-end over CRPS.

Result: Achieved a CRPS of 3.5102, showing ≈26% effectiveness gain over the best 3D-UNET baseline.

Conclusion: The proposed method is promising for probabilistic rainfall nowcasting, outperforming traditional approaches.
...
TL;DR: YOLO-Drone, an enhanced YOLOv11 model with GhostHead Network, improves accuracy and speed in drone-based object detection, outperforming other YOLO variants.


<details>
  <summary>Details</summary>
Motivation: The challenge of identifying objects in high-altitude drone images necessitates an improved object detection model to enhance accuracy and speed.

Method: The paper introduces an enhancement to the Head network of YOLOv11, called the GhostHead Network, and evaluates the improved model (YOLO-Drone) using the VisDrone dataset. Comparative experiments are conducted against YOLOv8, YOLOv9, and YOLOv10.

Result: YOLO-Drone achieves improvements in Precision (0.4%), Recall (0.6%), F1-Score (0.5%), and mAP (0.5%) over YOLOv11, and outperforms YOLOv8, YOLOv9, and YOLOv10 in mAP (0.5).

Conclusion: The proposed YOLO-Drone model, enhanced with the GhostHead Network, outperforms the original YOLOv11 and other high-performance models like YOLOv8, YOLOv9, and YOLOv10 in key accuracy metrics and inference speed, making it a superior solution for drone-based object detection.
...
TL;DR: PhaseWin is a scalable, high-faithfulness attribution method for object-level models, combining phased search and pruning to achieve near-greedy performance with much lower computational cost.


<details>
  <summary>Details</summary>
Motivation: The efficiency limitations of recent submodular subset selection methods hinder their practical deployment, despite their high faithfulness. PhaseWin aims to address this by enabling faithful region attribution with near-linear complexity.

Method: PhaseWin introduces a phased coarse-to-fine search algorithm, combining adaptive pruning, windowed fine-grained selection, and dynamic supervision mechanisms to approximate greedy behavior with near-linear complexity.

Result: PhaseWin achieves over 95% of greedy attribution faithfulness using only 20% of the computational budget and consistently outperforms other baselines in object detection and visual grounding tasks.

Conclusion: PhaseWin establishes a new state of the art in scalable, high-faithfulness attribution for object-level multimodal models, outperforming other baselines in tasks like object detection and visual grounding.
...
TL;DR: A novel Positive and Negative Prompt Supervision method improves OOD detection by refining prompt-based supervision and using a graph-based architecture, achieving superior results on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current negative prompts in OOD detection often capture overlapping or misleading information due to their broad range of non-ID features, leading to suboptimal results. The proposed method aims to address this by refining prompt supervision.

Method: The method involves initializing class-specific positive and negative prompts using LLMs, optimizing them to focus on intra-class and inter-class features, and employing a graph-based architecture to propagate semantic-aware supervision to the visual branch.

Result: Extensive experiments on CIFAR-100 and ImageNet-1K across eight OOD datasets and five LLMs show the method outperforms existing baselines.

Conclusion: The proposed Positive and Negative Prompt Supervision method significantly enhances OOD detection performance by leveraging optimized prompts and a graph-based architecture, outperforming state-of-the-art baselines on multiple benchmarks.
...
TL;DR: Lightweight YOLO models (YOLOv11n and YOLOv12n) are evaluated for facial expression recognition. YOLOv12n shows higher sensitivity, while YOLOv11n offers better precision, making them suitable for real-time, resource-constrained applications.


<details>
  <summary>Details</summary>
Motivation: Facial Expression Recognition is challenging in real-world environments. The study aims to investigate the performance of lightweight YOLO models to address this challenge.

Method: The study evaluates YOLOv11n and YOLOv12n within a unified detection and classification framework for FER. Performance is assessed using mAP 0.5, precision, recall, and confusion matrices on converted FER2013 and KDEF datasets.

Result: YOLOv12n achieves higher mAP (95.6 on KDEF, 63.8 on FER2013), showing better sensitivity. YOLOv11n has higher precision (65.2 on FER2013), indicating fewer false positives. Both models struggle with visually similar expressions on FER2013 but perform better on KDEF.

Conclusion: The study highlights the adaptability of lightweight YOLO models (YOLOv11n and YOLOv12n) in facial expression recognition, balancing sensitivity and precision. They are strong candidates for real-time, resource-constrained applications.
...
TL;DR: HCD is a simple yet effective framework for heterogeneous knowledge distillation, using complementary features and novel techniques like SDD and OL to outperform current methods.


<details>
  <summary>Details</summary>
Motivation: Traditional KD methods struggle with heterogeneous architectures due to differences in spatial feature representations, high computational costs, or over-reliance on logit alignment. HCD aims to address these limitations.

Method: HCD integrates complementary teacher and student features, processes them through a Complementary Feature Mapper (CFM), and uses SDD and OL to ensure diverse knowledge transfer.

Result: HCD demonstrates superior performance on CIFAR-100, Fine-grained (e.g., CUB200), and ImageNet-1K datasets compared to existing KD methods.

Conclusion: Heterogeneous Complementary Distillation (HCD) is an effective solution for heterogeneous knowledge distillation, outperforming state-of-the-art methods by leveraging complementary features and introducing Sub-logit Decoupled Distillation (SDD) and Orthogonality Loss (OL).
...
TL;DR: FedBCS addresses feature heterogeneity in federated learning for medical data by aligning domain-invariant contextual prototypes and recalibrating style biases, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Feature heterogeneity in federated learning due to diverse scanners or protocols limits model performance, with existing methods failing to capture multi-level cues and accumulating style biases.

Method: FedBCS introduces frequency-domain adaptive style recalibration and context-aware dual-level prototype alignment to decouple content-style representations and align domain-invariant prototypes across layers.

Result: Extensive experiments on public datasets show FedBCS achieves remarkable performance.

Conclusion: FedBCS effectively bridges feature representation gaps and enhances model robustness in federated learning for medical data.
...
TL;DR: SandboxVLM bridges the modality gap in VLMs for 3D tasks using abstract bounding boxes, improving spatial intelligence without extra training.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with 3D tasks due to a modality gap between 2D training and 3D requirements, limiting real-world applications like robotics.

Method: Introduces SandboxVLM, a framework with a 4-stage pipeline: multi-view priors, proxy elevation, multi-view voting/clustering, and 3D-aware reasoning.

Result: Achieves an 8.3% gain on SAT Real benchmark, enhancing 3D reasoning in zero-shot settings.

Conclusion: SandboxVLM demonstrates that 3D abstraction significantly boosts VLM performance in 3D tasks, enabling broader embodied intelligence applications.
...
TL;DR: DEFT-LLM improves micro expression recognition by aligning motion semantics and using multimodal large language models, achieving top results and interpretability.


<details>
  <summary>Details</summary>
Motivation: The entanglement of static appearance and dynamic motion cues, along with the semantic gap between text supervision and physical motion in existing datasets, hinders accurate micro expression recognition.

Method: The paper proposes DEFT-LLM, which uses multi-expert disentanglement to decouple facial dynamics into structure, dynamic textures, and motion-semantics. It introduces Uni-MER, a motion-driven instruction dataset aligned with local facial motion using optical flow and Action Unit labels.

Result: Experiments on multiple MER benchmarks show DEFT-LLM's superior performance and interpretable modeling of local facial motion.

Conclusion: DEFT-LLM achieves state-of-the-art performance in micro expression recognition by addressing the challenges of motion-semantic alignment and leveraging multimodal large language models for interpretable modeling.
...
TL;DR: LGRLN, a novel video summarization method, uses structured graphs and language guidance to improve performance and efficiency, outperforming benchmarks with significant reductions in inference time and parameters.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of video content on social media necessitates efficient summarization methods. Existing approaches struggle with capturing global dependencies, multimodal customization, and semantic proximity in video frames.

Method: The paper introduces a Language-guided Graph Representation Learning Network (LGRLN) with a video graph generator, intra-graph relational reasoning module, and language-guided cross-modal embedding module. The summary generation is modeled as a mixture of Bernoulli distribution and solved with the EM algorithm.

Result: LGRLN outperforms existing methods on multiple benchmarks, reducing inference time by 87.8% and model parameters by 91.7%.

Conclusion: The proposed LGRLN method effectively addresses challenges in video summarization by leveraging structured graph representations and language-guided cross-modal embedding, achieving superior performance and efficiency.
...
TL;DR: TG-DFER is a text-guided weakly supervised framework for DFER that uses semantic guidance and temporal modeling to improve performance.


<details>
  <summary>Details</summary>
Motivation: Address the challenges of visual diversity and temporal complexity in MIL-based DFER by incorporating semantic guidance and coherent temporal modeling.

Method: The framework integrates a vision-language pre-trained model for semantic guidance, visual prompts for alignment, and a multi-grained temporal network for capturing facial dynamics.

Result: TG-DFER achieves enhanced performance in DFER, demonstrating improved generalization, interpretability, and temporal sensitivity.

Conclusion: TG-DFER improves generalization, interpretability, and temporal sensitivity in DFER under weak supervision by leveraging semantic guidance and coherent temporal modeling.
...
TL;DR: ERMoE improves sparse MoE models by using eigenbasis-based routing, eliminating load-balancing losses, and achieving better performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: To address challenges in Mixture-of-Experts (MoE) architectures, such as misalignment between router logits and expert structures leading to unstable routing and load imbalances.

Method: ERMoE reparameterizes each expert in a learned orthonormal eigenbasis and replaces learned gating logits with an "Eigenbasis Score," defined as the cosine similarity between input features and an expert's basis.

Result: ERMoE achieves state-of-the-art accuracy on ImageNet classification and cross-modal image-text retrieval benchmarks, improves brain age prediction accuracy by over 7%, and yields interpretable expert specializations.

Conclusion: ERMoE introduces a new architectural principle for sparse expert models, addressing routing instabilities and enabling improved performance with scalable, interpretable specialization.
...
TL;DR: DMC and DMC-OT are frameworks for CLIP-based class-incremental learning, addressing bias and drift issues. DMC-OT enhances performance with optimal-transport calibration and task-specific prompts, achieving top results.


<details>
  <summary>Details</summary>
Motivation: Extending vision-language models like CLIP to continual learning settings is challenging due to classifier bias and distributional drift. The goal is to preserve cross-modal alignment and improve inter-task separability.

Method: DMC decouples the adaptation of the vision encoder and the optimization of textual soft prompts, training each stage with the other frozen. DMC-OT adds an optimal-transport guided calibration strategy to align memory statistics and a task-specific prompting design.

Result: DMC and DMC-OT achieve state-of-the-art performance on benchmarks like CIFAR-100, Imagenet-R, CUB-200, and UCF-101, with DMC-OT improving accuracy by an average of 1.80%.

Conclusion: The proposed DMC and DMC-OT frameworks effectively address the challenges of class-incremental learning (CIL) in vision-language models like CLIP. DMC-OT, with its optimal-transport guided calibration and task-specific prompting, further enhances performance, achieving state-of-the-art results.
...
TL;DR: PAS stabilizes Video LLM attention by smoothing RoPE's temporal kernel, improving performance without extra training.


<details>
  <summary>Details</summary>
Motivation: Address temporal inconsistency in Video LLMs caused by multimodal RoPE, which disrupts attention due to frame-scale ripples.

Method: Phase Aggregated Smoothing (PAS), which applies small opposed phase offsets across heads and aggregates outputs to smooth the temporal kernel.

Result: Experiments show consistent improvements in video understanding benchmarks with negligible overhead.

Conclusion: PAS offers a plug-and-play solution to enhance temporal encoding in Video LLMs, improving robustness without significant computational cost.
...
TL;DR: A training-free, binary verification workflow for zero-shot vision using VLMs, involving quantization to MCQs and binarization to True/False questions, improves accuracy across tasks.


<details>
  <summary>Details</summary>
Motivation: To enhance zero-shot vision performance without task-specific training by leveraging quantization and binarization of queries.

Method: Quantization converts open-ended queries into MCQs, followed by binarization into True/False questions, resolved deterministically.

Result: The workflow significantly improves accuracy across tasks like REC, spatial reasoning, and BLINK-Jigsaw.

Conclusion: The proposed workflow offers a practical, unified approach for stronger zero-shot vision with VLMs, emphasizing inference-time design.
...
TL;DR: The paper introduces HPAC, a lightweight AR model with hierarchical parallelism and optimizations (CSI, AFC), achieving SOTA lossless compression. SARP-FT enables instance-level fine-tuning, proving AR models can be practical and efficient.


<details>
  <summary>Details</summary>
Motivation: The motivation is to re-establish autoregressive models as practical and top-performing solutions for lossless image compression, addressing their perceived impracticality due to high computational costs.

Method: The method involves the Hierarchical Parallel Autoregressive ConvNet (HPAC), which uses hierarchical parallelism, content-aware convolutional gating, and two key optimizations: Cache-then-Select Inference (CSI) and Adaptive Focus Coding (AFC). Additionally, Spatially-Aware Rate-Guided Progressive Fine-tuning (SARP-FT) is introduced for instance-level adaptation.

Result: The results demonstrate state-of-the-art compression performance across diverse datasets (natural, satellite, medical), with a small parameter count and competitive coding speeds.

Conclusion: The paper concludes that a carefully designed autoregressive (AR) framework, embodied in HPAC, can achieve state-of-the-art lossless image compression with practical computational costs, outperforming existing methods in efficiency and performance.
...
TL;DR: CLUE is a generative model framework for stable and diverse image synthesis in specialized fields like medicine, using fixed-format prompts without extra data. It outperforms baselines in metrics like FID and F1 scores.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle in specialized fields with limited data. CLUE aims to achieve diverse and stable image generation without requiring additional data.

Method: CLUE uses a Style Encoder and a new second attention layer in U-Net, with Kullback-Leibler divergence for continuous latent space representation.

Result: CLUE reduced FID to 9.30, improved recall to 70.29%, and achieved high F1 scores (83.21% synthetic-only, 94.76% combined with real data).

Conclusion: CLUE enables diverse and stable image generation from limited datasets and serves as an effective data augmentation tool for domain-specific applications.
...
TL;DR: PROMISE is a novel multimodal framework using prompt-attention and hierarchical contrastive learning to improve cross-modal representation when modalities are missing.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal models degrade in real-world scenarios with missing modalities due to inconsistent representation learning.

Method: PROMISE integrates multimodal prompt learning and a prompt-attention mechanism within a hierarchical contrastive learning framework.

Result: PROMISE outperforms state-of-the-art methods on benchmark datasets, demonstrating robust performance with missing modalities.

Conclusion: PROMISE effectively bridges the representational gap between complete and incomplete multimodal data, enhancing robustness.
...
TL;DR: EmoVid is a new multimodal, emotion-annotated video dataset for creative media, enabling emotion-conditioned video generation with improved quality.


<details>
  <summary>Details</summary>
Motivation: Existing video generation systems neglect affective dimensions, lacking resources to bridge emotion understanding with generative tasks, especially in stylized contexts.

Method: Introduces EmoVid, a dataset with emotion labels, visual attributes, and captions, and develops an emotion-conditioned video generation technique by fine-tuning the Wan2.1 model.

Result: Significant improvement in quantitative metrics and visual quality for text-to-video and image-to-video tasks.

Conclusion: EmoVid sets a new benchmark for affective video computing, offering insights into visual emotion analysis and practical methods for enhancing emotional expression in video generation.
...
TL;DR: MeCaMIL introduces causality-aware MIL for fair and interpretable WSI analysis, outperforming existing methods in performance and fairness while addressing demographic biases.
...
Method: MeCaMIL employs causality-aware MIL, using structured causal graphs, do-calculus, and collider structures to disentangle disease-relevant signals from demographic confounders.

Result: MeCaMIL achieves state-of-the-art performance on benchmarks (e.g., CAMELYON16, TCGA-Lung, TCGA-Multi) and reduces demographic disparity variance by over 65%. It also generalizes to survival prediction with improved C-index.

Conclusion: MeCaMIL is established as a principled framework for fair, interpretable, and clinically actionable AI in digital pathology, with superior performance and fairness metrics compared to existing methods.
...
TL;DR: The paper introduces Draft and Refine (DnR), a framework to improve visual grounding in LVLMs by quantifying and enhancing their reliance on visual evidence, reducing hallucinations.


<details>
  <summary>Details</summary>
Motivation: LVLMs often produce ungrounded responses due to over-reliance on linguistic priors, lacking a measure of visual information usage.

Method: DnR uses a question-conditioned utilization metric to localize visual cues and measure dependence, refining responses with feedback from visual experts.

Result: Experiments show accuracy gains and reduced hallucination in VQA and captioning tasks.

Conclusion: Measuring visual utilization improves interpretability and grounding in multimodal systems without retraining.
