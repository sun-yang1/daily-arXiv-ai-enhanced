...
TL;DR: VL4Gaze is a new benchmark to evaluate and improve VLMs' gaze understanding, showing that task-specific training is crucial for performance.


<details>
  <summary>Details</summary>
Motivation: Gaze understanding is unexplored in VLMs, and there's no benchmark to evaluate or train them for this purpose.

Method: VL4Gaze benchmark with 489K question-answer pairs across 124K images, evaluated under in-context learning and fine-tuning settings.

Result: Large-scale VLMs struggle with gaze semantics and spatial localization without task-specific supervision, but VL4Gaze training brings consistent improvements.

Conclusion: Training on VL4Gaze significantly improves gaze understanding in VLMs, emphasizing the need for targeted multi-task supervision.
...
TL;DR: A hardware-aware NAS framework creates efficient trash detectors (TrashDets) for TinyML, boosting accuracy and cutting energy/latency significantly.


<details>
  <summary>Details</summary>
Motivation: To address trash detection on the TACO dataset under strict TinyML constraints for edge and IoT devices, improving accuracy and efficiency.

Method: An iterative hardware-aware neural architecture search framework constructs a Once-for-All-style ResDets supernet, optimizing backbone and neck/head alternately with a population passthrough mechanism and accuracy predictor.

Result: TrashDet-l achieves 19.5 mAP50 with 30.5M parameters, outperforming prior detectors. Specialized variants on MAX78002 microcontroller reduce energy by up to 88%, latency by 78%, and power by 53%.

Conclusion: The TrashDet family offers scalable, efficient detectors for TinyML deployments, significantly improving accuracy, energy efficiency, and latency over existing methods.
...
TL;DR: OccuFly is a camera-based aerial SSC benchmark addressing LiDAR limitations in UAVs, automating 3D annotation via 2D masks, and benchmarking state-of-the-art methods for elevated viewpoints.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the lack of SSC research in aerial scenarios and the challenges posed by LiDAR sensors in UAVs due to regulations, mass, energy constraints, and sparse point clouds from elevated views.

Method: The method involves creating OccuFly, a real-world benchmark dataset captured at various altitudes and seasons, using camera modality. It automates label transfer via traditional 3D reconstruction and annotated 2D masks, reducing manual 3D annotation.

Result: The results include the successful creation of OccuFly, a comprehensive benchmark for aerial SSC, and a framework that minimizes manual 3D annotation, facilitating research in aerial 3D perception.

Conclusion: The paper concludes with the introduction of OccuFly, a camera-based aerial SSC benchmark, and a LiDAR-free data generation framework, addressing the limitations of LiDAR in UAVs. It benchmarks state-of-the-art methods and highlights challenges specific to elevated viewpoints, advancing aerial 3D scene understanding.
...
TL;DR: NullBUS, a multimodal framework with nullable prompts, improves BUS segmentation by handling missing text prompts, achieving high performance metrics.


<details>
  <summary>Details</summary>
Motivation: The lack of reliable metadata or reports in many public BUS datasets limits training to small multimodal subsets, reducing robustness. NullBUS aims to improve segmentation performance and tumor delineation by learning from images with and without prompts in a single model.

Method: NullBUS is a multimodal mixed-supervision framework that uses nullable prompts (learnable null embeddings with presence masks) to handle missing text prompts, allowing fallback to image-only evidence when metadata is absent.

Result: NullBUS achieves a mean IoU of 0.8568 and a mean Dice of 0.9103 on a unified pool of three public BUS datasets.

Conclusion: NullBUS demonstrates state-of-the-art performance in BUS segmentation under mixed prompt availability, achieving high mean IoU and Dice scores.
...
TL;DR: A task-driven co-design framework optimizes optics, sensors, and networks for autonomous driving, improving segmentation accuracy and robustness with a compact, deployable model.


<details>
  <summary>Details</summary>
Motivation: Traditional autonomous driving pipelines decouple camera design from downstream perception, relying on fixed optics and handcrafted ISPs that prioritize human viewable imagery rather than machine semantics. This separation discards information during demosaicing, denoising, or quantization, while forcing models to adapt to sensor artifacts.

Method: The paper presents a task-driven co-design framework that unifies optics, sensor modeling, and lightweight semantic segmentation networks into a single end-to-end RAW-to-task pipeline. It integrates realistic cellphone-scale lens models, learnable color filter arrays, Poisson-Gaussian noise processes, and quantization, all optimized directly for segmentation objectives.

Result: Evaluations on KITTI-360 show consistent mIoU improvements over fixed pipelines, with optics modeling and CFA learning providing the largest gains, especially for thin or low-light-sensitive classes. These robustness gains are achieved with a compact ~1M-parameter model running at ~28 FPS, demonstrating edge deployability.

Conclusion: The paper establishes full-stack co-optimization of optics, sensors, and networks as a principled path toward efficient, reliable, and deployable perception in autonomous systems.
...
TL;DR: Adaptive visual preprocessing for VLMs cuts inference time by 50% and reduces visual tokens by 55%, enhancing efficiency without model changes.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs face high inference latency and computational costs, especially with high-resolution inputs, due to static visual preprocessing leading to redundant computations.

Method: The approach combines content-aware image analysis, adaptive resolution selection, and content-aware cropping to dynamically adjust input resolution and spatial coverage based on image content characteristics.

Result: Experimental results show over 50% reduction in per-image inference time, lower mean full generation time, and more than 55% reduction in visual token count compared to the baseline.

Conclusion: The proposed adaptive visual preprocessing method significantly improves the efficiency of Vision-Language Models (VLMs) by reducing inference time and computational costs without modifying the model architecture or requiring retraining.
...
TL;DR: ALIVE transforms passive lectures into interactive experiences using local AI, avatars, and content-aware retrieval, enhancing learning without cloud reliance.


<details>
  <summary>Details</summary>
Motivation: Traditional lecture videos lack real-time clarification mechanisms, forcing learners to seek external help. Existing interactive systems often lack lecture awareness, rely on cloud services, or fail to integrate retrieval and avatar explanations in a unified, privacy-preserving pipeline.

Method: ALIVE integrates avatar-delivered lectures (ASR transcription, LLM refinement, neural talking-head synthesis), content-aware retrieval (semantic similarity + timestamp alignment), and real-time multimodal interaction (text/voice questions, avatar/text responses). It uses lightweight embedding models, FAISS-based retrieval, and segmented avatar synthesis with progressive preloading.

Result: ALIVE provides accurate, content-aware, and engaging real-time support, as demonstrated in a medical imaging course, with evaluated retrieval accuracy, latency, and user experience.

Conclusion: ALIVE demonstrates the potential of multimodal AI combined with content-aware retrieval and local deployment to enhance the pedagogical value of recorded lectures, paving the way for next-generation interactive learning environments.
...
TL;DR: A 3D GPR-based pipeline detection framework improves accuracy and robustness by integrating multi-view analysis, enhanced YOLO, and spatial feature matching, achieving high performance in complex scenarios.


<details>
  <summary>Details</summary>
Motivation: To address weak correlation between multi-view features, low recognition accuracy of small-scale targets, and insufficient robustness in complex scenarios in underground pipeline detection using 3D GPR.

Method: The paper proposes a 3D pipeline intelligent detection framework involving a B/C/D-Scan three-view joint analysis strategy, DCO-YOLO framework with DySample, CGLU, and OutlookAttention mechanisms, and a 3D-DIoU spatial feature matching algorithm.

Result: The method achieves accuracy, recall, and mean average precision of 96.2%, 93.3%, and 96.7%, respectively, outperforming the baseline model by 2.0%, 2.1%, and 0.9%.

Conclusion: The study integrates deep learning optimization strategies with the physical characteristics of 3D GPR, providing an efficient and reliable technical framework for intelligent underground pipeline recognition and localization.
...
TL;DR: NeRV360 improves 360-degree video compression by decoding only the user-selected viewport, reducing memory usage by 7x and speeding up decoding by 2.5x compared to prior work.


<details>
  <summary>Details</summary>
Motivation: Applying NeRV to high-resolution 360-degree videos results in high memory usage and slow decoding, making real-time applications impractical.

Method: NeRV360 integrates viewport extraction into decoding and introduces a spatial-temporal affine transform module for conditional decoding based on viewpoint and time.

Result: NeRV360 achieves a 7-fold reduction in memory consumption and a 2.5-fold increase in decoding speed compared to HNeRV, while delivering better image quality.

Conclusion: NeRV360 effectively addresses the limitations of NeRV for high-resolution 360-degree videos by focusing on user-selected viewports, significantly reducing memory usage and improving decoding speed while maintaining better image quality.
...
TL;DR: Proposes DRI, a feature-space PEFT strategy for Cross-Modality Ship Re-ID, achieving SOTA performance with minimal parameters by injecting domain-specific representations into a frozen Vision Foundation Model.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing Parameter-Efficient Fine-Tuning (PEFT) methods and the dependency on large-scale paired datasets for modality alignment in Cross-Modality Ship Re-Identification.

Method: A lightweight Offset Encoder extracts domain-specific representations, and a Modulator adaptively transforms these for injection into intermediate layers via additive fusion.

Result: Achieves 57.9% and 60.5% mAP on the HOSS-ReID dataset with only 1.54M and 7.05M parameters, respectively.

Conclusion: The proposed Domain Representation Injection (DRI) method achieves SOTA performance in Cross-Modality Ship Re-Identification with minimal trainable parameters, demonstrating its effectiveness in bridging modality gaps without altering pre-trained weights.
...
TL;DR: DGSAN enhances lung nodule classification via advanced multimodal fusion, validated on new datasets.


<details>
  <summary>Details</summary>
Motivation: Addressing the inefficiency of current fusion methods (vector concatenation, simple mutual attention) in early lung cancer detection.

Method: Developed a Global-Local Feature Encoder, Dual-Graph Construction, and Hierarchical Cross-Modal Graph Fusion Module for effective multimodal data fusion.

Result: DGSAN outperforms state-of-the-art methods on NLST-cmst and CSTL-derived datasets.

Conclusion: The Dual-Graph Spatiotemporal Attention Network (DGSAN) significantly improves pulmonary nodule classification accuracy and computational efficiency, outperforming existing methods.
...
TL;DR: A benchmark evaluates VLMs on compressed images, identifies performance gaps, and introduces a universal adaptor boosting performance by 10%-30%.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore and improve VLMs' ability to interpret low-bitrate compressed images, addressing a gap in existing research.

Method: The method involves creating a comprehensive benchmark with over one million compressed images, analyzing performance gaps, and proposing a universal VLM adaptor to enhance performance.

Result: The result shows that the proposed universal VLM adaptor improves performance by 10%-30% across varying codecs and bitrates.

Conclusion: The paper concludes that a single universal VLM adaptor can significantly improve model performance across various codecs and bitrates, providing valuable insights for bridging the gap between VLMs and compressed images.
...
TL;DR: PanoGrounder combines panoramic representations with 2D VLMs for superior 3D visual grounding, achieving SOTA results and better generalization.


<details>
  <summary>Details</summary>
Motivation: Traditional supervised models have limited generalization due to scarce 3D vision-language datasets and weaker reasoning compared to modern VLMs.

Method: The approach uses a three-stage pipeline: selecting panoramic viewpoints, grounding text queries on each rendering with a VLM, and fusing predictions into a 3D bounding box.

Result: PanoGrounder outperforms existing methods, showing strong vision-language reasoning and generalization.

Conclusion: PanoGrounder achieves state-of-the-art results on ScanRefer and Nr3D, demonstrating superior generalization to unseen datasets and text rephrasings.
...
TL;DR: SMC-Mamba, a self-supervised framework, enhances general image fusion via MAFE and MCCM modules, outperforming SOTA methods in various tasks.


<details>
  <summary>Details</summary>
Motivation: General image fusion needs to address a wide range of tasks while improving performance without increasing complexity, unlike task-specific techniques.

Method: The framework includes the Modality-Agnostic Feature Enhancement (MAFE) module for detail preservation and global representation enhancement, and the Multiplex Consensus Cross-modal Mamba (MCCM) module for dynamic collaboration among experts. It also uses a Bi-level Self-supervised Contrastive Learning Loss (BSCL).

Result: Extensive experiments show superior performance in infrared-visible, medical, multi-focus, and multi-exposure fusion, as well as downstream tasks.

Conclusion: SMC-Mamba outperforms SOTA image fusion algorithms across various tasks, demonstrating its effectiveness in integrating complementary information from multiple modalities.
...
TL;DR: TL;DR: Q-Render and GS-Net improve 3D open-vocabulary segmentation by efficiently rendering high-dimensional features, achieving faster and better results than current methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inefficiency and information loss in existing methods for rendering high-dimensional features in open-vocabulary 3D segmentation.

Method: The method involves Quantile Rendering (Q-Render), which sparsely samples dominant 3D Gaussians, and integrates it into a Gaussian Splatting Network (GS-Net) for generalizable feature prediction.

Result: Results show a ~43.7x speedup on 512-D feature maps and superior performance on ScanNet and LeRF datasets.

Conclusion: The paper concludes that the proposed Q-Render and GS-Net framework significantly outperforms existing methods in open-vocabulary 3D segmentation, offering both high fidelity and real-time rendering capabilities.
...
TL;DR: TVP, a transductive visual programming framework, builds tools from experience, outperforming existing methods and achieving superior generalization in spatial reasoning tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on fixed toolsets or speculative tool induction, leading to suboptimal programs and poor tool utilization. TVP aims to improve by building tools from experience.

Method: TVP first solves problems using basic tools, accumulates experiential solutions into an Example Library, and abstracts recurring patterns into reusable higher-level tools for an evolving Tool Library.

Result: TVP achieves state-of-the-art performance on Omni3D-Bench, outperforming GPT-4o by 22% and the previous best system by 11%. Its tools are used 5x more frequently and generalize well to unseen tasks.

Conclusion: TVP establishes experience-driven transductive tool creation as a powerful paradigm for self-evolving visual programming agents, effectively tackling challenging spatial reasoning tasks.
...
TL;DR: A multi-agent framework improves amodal completion by decoupling planning and synthesis, introducing self-correction and diversity, validated by a new metric.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of prior approaches in semantic consistency and structural integrity during amodal completion.

Method: A Collaborative Multi-Agent Reasoning Framework decoupling Semantic Planning from Visual Synthesis, with a self-correcting Verification Agent and Diverse Hypothesis Generator.

Result: The framework significantly outperforms existing methods across multiple datasets, with human-aligned evaluation via MAC-Score.

Conclusion: The proposed Collaborative Multi-Agent Reasoning Framework outperforms state-of-the-art methods by addressing semantic and structural challenges in amodal completion, validated by the novel MAC-Score metric.
...
TL;DR: REM improves synthetic image detection by modeling real image distributions, outperforming existing methods by 7.5% and generalizing well on degraded images.


<details>
  <summary>Details</summary>
Motivation: Existing detectors overfit to generator-specific artifacts and are sensitive to real-world degradations, becoming obsolete as generative architectures evolve and images undergo multi-round degradations.

Method: Proposes Real-centric Envelope Modeling (REM), which shifts detection from learning generator artifacts to modeling the robust distribution of real images. Introduces feature-level perturbations in self-reconstruction and employs an envelope estimator with cross-domain consistency.

Result: REM achieves an average improvement of 7.5% over state-of-the-art methods across eight benchmark evaluations and maintains exceptional generalization on the severely degraded RealChain benchmark.

Conclusion: REM establishes a solid foundation for synthetic image detection under real-world conditions by modeling the robust distribution of real images.
...
TL;DR: SPOT, a map-guided LLM agent, predicts vehicle trajectories in CCTV blind spots using spatial info and beam search, outperforming existing methods in tests.


<details>
  <summary>Details</summary>
Motivation: Addressing the limitations of CCTV-based vehicle tracking systems, such as blind spots and object ID switching, to improve real-time path prediction reliability.

Method: SPOT uses map-guided LLM agents, 2D spatial coordinates for road structures and CCTV placement, chunking techniques for real-time querying, and beam search at intersections to predict vehicle entry points.

Result: Experiments in a virtual city environment (CARLA simulator) confirmed SPOT's accuracy in predicting next CCTV locations and maintaining trajectories.

Conclusion: The proposed SPOT method effectively maintains continuous vehicle trajectories in multi-CCTV environments by predicting the next CCTV location after blind spots, outperforming existing techniques.
...
TL;DR: XGrid-Mapping combines sparse and dense grids for efficient neural LiDAR mapping, using geometric priors and submap organization to outperform current methods.


<details>
  <summary>Details</summary>
Motivation: Addressing the inefficiency of voxel-guided methods and the underutilization of geometric structure in neural LiDAR mapping.

Method: The approach uses a sparse grid for geometric priors and an implicit dense grid for scene representation, coupled with a VDB structure and submap-based organization. A distillation-based overlap alignment strategy ensures consistency, and a dynamic removal module enhances robustness.

Result: Extensive experiments demonstrate superior mapping quality and efficiency, overcoming limitations of existing methods.

Conclusion: XGrid-Mapping, a hybrid grid framework combining explicit and implicit representations, achieves superior mapping quality and efficiency, outperforming existing state-of-the-art methods.
...
TL;DR: Novel data synthesis pipeline using normal X-rays and diffusion models, enhanced by LKG and PIL, improves diagnosis of rare lung lesions, achieving top performance on public datasets.


<details>
  <summary>Details</summary>
Motivation: Addressing the diagnostic challenges of long-tailed pulmonary anomalies by overcoming the scarcity of rare lesion exemplars in current diffusion-based methods.

Method: A diffusion model is trained on normal X-rays to generate images, then used to inpaint head lesions in diseased X-rays, augmented by LKG and PIL for stable fine-tuning.

Result: The method outperforms existing approaches, demonstrating superior performance on MIMIC and CheXpert datasets.

Conclusion: The proposed method, integrating a novel data synthesis pipeline with LKG and PIL strategies, significantly enhances diagnostic precision for long-tailed pulmonary anomalies, setting a new performance benchmark on public datasets.
...
TL;DR: PUFM++ enhances point cloud upsampling via improved flow matching, achieving state-of-the-art results in accuracy and fidelity.


<details>
  <summary>Details</summary>
Motivation: To improve geometric fidelity, robustness to imperfect input, and consistency with downstream surface-based tasks in point cloud upsampling.

Method: A two-stage flow-matching strategy with a data-driven adaptive time scheduler and on-manifold constraints, enhanced by a recurrent interface network (RIN).

Result: Extensive experiments show PUFM++ outperforms existing methods in both synthetic benchmarks and real-world scans.

Conclusion: PUFM++ sets a new state of the art in point cloud upsampling, offering superior visual fidelity and quantitative accuracy.
...
TL;DR: A feed-forward multi-view inverse rendering framework predicts materials and lighting from RGB images, ensuring cross-view consistency and real-world generalization via consistency-based finetuning.


<details>
  <summary>Details</summary>
Motivation: Existing single-view methods lack cross-view consistency, while multi-view optimization is slow and hard to scale. Real-world generalization is also limited by synthetic training data.

Method: Alternates attention across views to capture intra-view lighting and inter-view material consistency, with consistency-based finetuning on unlabeled real-world videos.

Result: Achieves state-of-the-art performance in multi-view consistency, material/normal estimation, and real-world generalization.

Conclusion: The framework effectively balances efficiency, consistency, and robustness for multi-view inverse rendering.
...
TL;DR: NExT-Vid is an autoregressive visual generative pretraining framework that improves semantic localization and generation quality, outperforming existing methods in downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive visual pretraining methods suffer from inaccurate semantic localization and poor generation quality, which NExT-Vid aims to address.

Method: The framework utilizes masked next-frame prediction, a context-isolated autoregressive predictor, and a conditioned flow-matching decoder to model images and videos.

Result: The approach achieves strong representations and consistently outperforms previous methods in large-scale experiments.

Conclusion: NExT-Vid demonstrates superior performance in visual representation learning, outperforming previous generative pretraining methods through attentive probing in downstream classification tasks.
...
TL;DR: FluencyVE introduces Mamba for efficient video editing, reducing costs while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Extending text-to-image diffusion models to video editing is challenging due to temporal inconsistency and high computational overheads.

Method: FluencyVE replaces temporal attention layers with the Mamba module and employs low-rank approximation matrices and weighted averaging to update attention scores.

Result: The approach achieves promising results in editing various attributes, subjects, and locations in real-world videos.

Conclusion: FluencyVE successfully integrates the Mamba module into a video editing model, preserving generative power while reducing computational costs, as evidenced by promising experimental results.
...
TL;DR: A novel video defense framework protects against 3D-field TFG methods by perturbing 3D info acquisition, ensuring high fidelity and efficiency.


<details>
  <summary>Details</summary>
Motivation: Address privacy concerns by protecting portrait videos from misuse via 3D-field TFG methods, overcoming limitations of existing image-based defenses.

Method: Introduces a similarity-guided parameter sharing mechanism and a multi-scale dual-domain attention module to perturb 3D information acquisition.

Result: Achieves strong defense capability, 47x acceleration over baselines, and robustness against attacks, maintaining high fidelity.

Conclusion: The proposed framework effectively protects portrait videos against 3D-field TFG methods with high fidelity and computational efficiency, validated by extensive experiments.
...
TL;DR: A latent diffusion model with multi-attribute classifier and Self-attn Mamba improves infrared-to-visible face image generation, achieving top performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods for infrared face recognition suffer from domain shifts and feature loss, prompting the need for better generative approaches.

Method: A novel latent diffusion-based model with a multi-attribute classifier and Self-attn Mamba module for global feature modeling.

Result: The proposed method outperforms benchmarks in image quality and identity preservation on two datasets.

Conclusion: The paper introduces a latent diffusion-based model and Self-attn Mamba module that significantly improve infrared-to-visible image generation, achieving state-of-the-art performance in quality and identity preservation.
...
TL;DR: NSP, a new self-supervised method, effectively balances noise removal and detail preservation in image denoising, outperforming current techniques.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of decorrelating spatially structured noise while preserving high-frequency details, which existing BSN methods struggle with due to aggressive or mild downsampling.

Method: Introduces Next-Scale Prediction (NSP), a self-supervised paradigm that decouples noise decorrelation from detail preservation by using cross-scale training pairs.

Result: NSP significantly outperforms existing methods on real-world benchmarks and naturally supports super-resolution of noisy images.

Conclusion: NSP achieves state-of-the-art self-supervised denoising performance, resolving the conflict between noise decorrelation and detail preservation.
