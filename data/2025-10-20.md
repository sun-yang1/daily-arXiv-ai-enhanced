...
TL;DR: GAZE pipeline automates video-to-dataset conversion for world-model training, saving time and reducing human review by 80%.


<details>
  <summary>Details</summary>
Motivation: Manual annotation for world-model training is slow and expensive, necessitating an automated solution.

Method: GAZE normalizes video formats, applies AI models for pre-annotation, and consolidates signals for human validation.

Result: Efficiency gains (~19 minutes saved per hour) and >80% reduction in human review volume.

Conclusion: GAZE provides a scalable, privacy-aware method for high-quality dataset generation.
...
TL;DR: PC-UNet with PVMC-Loss improves PET image fidelity by handling Poisson noise better than current methods, ensuring physical consistency.


<details>
  <summary>Details</summary>
Motivation: Current denoising methods for PET images fail to handle increased Poisson noise from lower doses, leading to distortions and artifacts. The study aims to improve image fidelity by integrating physical data into the denoising process.

Method: The paper proposes a Poisson Consistent U-Net (PC-UNet) model with a novel Poisson Variance and Mean Consistency Loss (PVMC-Loss), which incorporates physical data and ensures statistical unbiasedness in variance and gradient adaptation.

Result: Tests on PET datasets confirm that PC-UNet enhances physical consistency and image fidelity, validating its effectiveness in integrating physical information.

Conclusion: The PC-UNet model, enhanced with PVMC-Loss, effectively integrates physical data to improve PET image fidelity, demonstrating robustness against minor data mismatches.
...
TL;DR: DeLeaker is a lightweight, optimization-free method that mitigates semantic leakage in T2I models by dynamically adjusting attention maps, outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: Existing mitigation strategies for semantic leakage in T2I models are optimization-based or rely on external inputs, prompting the need for a lightweight, optimization-free approach.

Method: DeLeaker dynamically reweights attention maps during the diffusion process to suppress cross-entity interactions and strengthen entity identity.

Result: DeLeaker outperforms all baselines, including those with external information, and introduces SLIM, the first dataset for semantic leakage evaluation.

Conclusion: DeLeaker effectively mitigates semantic leakage in T2I models without compromising fidelity or quality, highlighting the value of attention control for more semantically precise models.
...
TL;DR: UrbanVerse is a data-driven system that converts city-tour videos into realistic, interactive urban simulations for training AI agents, outperforming prior methods in realism and performance.


<details>
  <summary>Details</summary>
Motivation: Training urban AI agents requires scalable, high-fidelity environments, but existing solutions lack realism or scalability.

Method: UrbanVerse uses crowd-sourced videos to create physics-aware simulations, including a repository of 100k+ annotated 3D assets and an automated pipeline for scene generation.

Result: UrbanVerse achieves human-evaluated realism comparable to manual scenes and improves AI agent performance by +6.3% in simulation and +30.1% in sim-to-real transfer.

Conclusion: UrbanVerse provides a scalable, realistic solution for training urban AI agents, demonstrating strong generalization and real-world applicability.
...
TL;DR: Nano3D is a training-free framework for precise 3D object editing, integrating FlowEdit and region-aware merging strategies to improve consistency and visual quality. It also introduces a large-scale dataset, Nano3D-Edit-100k.


<details>
  <summary>Details</summary>
Motivation: Current 3D object editing methods are inefficient, inconsistent, and often damage unedited regions, limiting practicality.

Method: Nano3D integrates FlowEdit into TRELLIS for localized edits guided by front-view renderings and introduces Voxel/Slat-Merge for adaptive structural preservation.

Result: Nano3D achieves superior 3D consistency and visual quality compared to existing methods and provides a large-scale dataset.

Conclusion: Nano3D addresses key challenges in 3D editing, improving generality and reliability, and sets the stage for future feed-forward models.
...
TL;DR: ECHO is a framework for creating benchmarks from real-world social media posts, uncovering new tasks and improving model evaluation for image generation.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lag behind rapid advancements in image generation, failing to capture new use cases and community perceptions.

Method: The ECHO framework constructs benchmarks from social media posts, curating over 31,000 prompts from GPT-4o Image Gen use cases.

Result: ECHO identifies creative tasks absent in current benchmarks, better distinguishes top models, and informs metric design based on community feedback.

Conclusion: ECHO successfully bridges the gap between emerging model capabilities and formal evaluation by leveraging real-world evidence from social media. It not only uncovers novel tasks but also improves model comparison and metric design.
...
TL;DR: The paper proposes a submodular framework to efficiently select diverse LoRA models from a large database, addressing user challenges in navigation and selection.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges users face in navigating and selecting suitable LoRA adapters due to their sheer volume, diversity, and lack of structured organization.

Method: The paper frames the task as a combinatorial optimization problem and introduces a novel submodular framework to select the most relevant and diverse LoRA models.

Result: The method successfully generates diverse outputs across a wide range of domains, as evidenced by experiments.

Conclusion: The paper concludes that the proposed submodular framework effectively addresses the challenge of selecting relevant and diverse LoRA models from a vast database, as demonstrated by both quantitative and qualitative experiments.
...
TL;DR: MOBIUS is a family of efficient foundation models for universal instance segmentation, reducing computational costs without sacrificing performance, suitable for diverse devices.


<details>
  <summary>Details</summary>
Motivation: The high computational cost of existing foundation models limits their adoption on resource-constrained platforms, prompting the need for efficient alternatives without performance compromise.

Method: MOBIUS introduces a bottleneck pixel decoder for efficient multi-scale fusion, a language-guided uncertainty calibration loss for adaptive pruning, and a unified training strategy.

Result: MOBIUS reduces pixel and transformer decoder FLOPs by up to 55% and 75%, respectively, while maintaining top performance in just a third of the training iterations.

Conclusion: MOBIUS establishes a new benchmark for efficient segmentation, achieving state-of-the-art performance while significantly reducing computational costs, making it suitable for both high-performance and mobile platforms.
...
TL;DR: COGS is a data-efficient framework that enhances MLLMs' reasoning in artificial image domains by generating synthetic QA pairs from seed questions, improving performance and generalizability.


<details>
  <summary>Details</summary>
Motivation: Addressing the lack of large-scale annotated reasoning datasets for artificial image domains like charts and webpages.

Method: COGS decomposes seed questions into primitive factors, recomposes them with new images to generate synthetic QA pairs, and uses factor-level process rewards for reinforcement learning.

Result: COGS improves performance on unseen questions, especially reasoning-heavy and compositional ones, and shows better transfer across datasets.

Conclusion: COGS enhances MLLMs' reasoning abilities efficiently, generalizes across domains, and reduces dataset-specific overfitting.
...
TL;DR: GDGen unifies rigid, articulated, and soft body dynamics using potential energy, offering a versatile framework for interactive virtual environments and robotic training.


<details>
  <summary>Details</summary>
Motivation: To develop generalist embodied agents in scannable environments with complex physical behaviors by unifying diverse dynamics into a single framework.

Method: GDGen integrates rigid, articulated, and soft body dynamics into a unified system using a potential energy perspective. It introduces directional stiffness and employs a neural field for geometry-agnostic deformation representation.

Result: Extensive experiments show GDGen robustly unifies simulation paradigms, enabling realistic interactive dynamics.

Conclusion: GDGen successfully unifies diverse simulation paradigms, providing a versatile foundation for interactive virtual environments and robotic agent training in complex scenarios.
...
TL;DR: The paper introduces COLIPRI, a vision-language pre-training method for 3D medical images, leveraging additional data and inductive biases to achieve top performance in tasks like report generation and classification.


<details>
  <summary>Details</summary>
Motivation: The lack of data in the 3D medical image domain limits the capabilities of current vision-language encoders (VLEs). This paper aims to address this limitation by enhancing data utilization through additional inductive biases.

Method: The methodology involves introducing a report generation objective and pairing vision-language pre-training with vision-only pre-training to leverage additional data sources. Best practices of the 3D medical imaging domain are also incorporated.

Result: COLIPRI encoders achieve state-of-the-art performance in report generation, classification probing, and zero-shot classification, and remain competitive for semantic segmentation.

Conclusion: The COLIPRI encoder family, developed by injecting additional inductive biases and leveraging both image-only and paired image-text datasets, achieves state-of-the-art performance in report generation, classification probing, and zero-shot classification, while remaining competitive in semantic segmentation.
...
TL;DR: DRIFT efficiently transfers reasoning knowledge to MLLMs, outperforming naive merging and supervised fine-tuning with lower resource use.


<details>
  <summary>Details</summary>
Motivation: Existing methods to enhance reasoning in MLLMs are resource-intensive, and naive model merging is inconsistent across model families.

Method: DRIFT precomputes a reasoning prior as the parameter-space difference between reasoning and multimodal variants, then biases gradients during multimodal fine-tuning.

Result: DRIFT improves reasoning performance on benchmarks like MathVista and MathVerse, matching or surpassing training-heavy methods at lower cost.

Conclusion: DRIFT is a lightweight and efficient method for transferring reasoning knowledge to MLLMs, outperforming naive merging and supervised fine-tuning while being cost-effective.
...
TL;DR: Infants generalize object categories from limited visual experiences due to the 'lumpy' similarity structure in their daily visual input, which also improves machine learning generalization.


<details>
  <summary>Details</summary>
Motivation: Understanding how infants generalize visual object categories from limited experiences despite the typical need for large datasets in learning.

Method: Analysis of egocentric images from 14 infants (7-11 months) to study visual input structure and computational experiments mimicking this structure in machine learning.

Result: Infant visual input shows a lumpy similarity structure, aiding category learning and generalization, and improves machine learning performance with small datasets.

Conclusion: The natural lumpiness of infant visual experiences supports early category learning and offers principles for efficient learning in various contexts.
...
TL;DR: SaLon3R is a novel framework for efficient, long-term 3D Gaussian Splatting reconstruction, reducing redundancies and inconsistencies through compact anchor primitives and a 3D Point Transformer.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D Gaussian Splatting (3DGS) suffer from redundancies and inconsistencies in long-duration video sequences, prompting the need for a more efficient and consistent approach.

Method: The method introduces compact anchor primitives for redundancy removal via differentiable saliency-aware Gaussian quantization and employs a 3D Point Transformer to refine anchor attributes and saliency, ensuring geometric fidelity.

Result: SaLon3R successfully reconstructs over 50 views at over 10 FPS with 50% to 90% redundancy removal, outperforming existing methods in efficiency and accuracy.

Conclusion: SaLon3R achieves state-of-the-art performance in novel view synthesis and depth estimation, offering superior efficiency, robustness, and generalization for long-term 3D reconstruction.
...
TL;DR: TGT improves text-to-video generation by using trajectories and localized text for better control over subject composition and motion, outperforming prior methods in quality and alignment.


<details>
  <summary>Details</summary>
Motivation: Standard text-to-video generation methods lack precise control over subject composition, especially in complex or multi-object scenarios. Prior localized control signals (e.g., bounding boxes) are limited in precision and scalability. TGT aims to address these limitations by using intuitive motion handles (trajectories) paired with text.

Method: The paper proposes Text-Grounded Trajectories (TGT), which uses trajectories and localized text descriptions for video generation. It introduces Location-Aware Cross-Attention (LACA) and a dual-CFG scheme for integrating signals and modulating text guidance. A data processing pipeline is developed to produce annotated trajectories for training.

Result: TGT outperforms prior approaches in visual quality, text alignment, and motion controllability, as demonstrated through extensive experiments. The framework successfully controls appearance and motion using point trajectories and localized text.

Conclusion: TGT introduces a novel framework for text-to-video generation, leveraging trajectories paired with localized text descriptions to enhance control over subject composition and motion. The proposed LACA and dual-CFG scheme, along with a robust data pipeline, enable superior visual quality, text alignment, and motion controllability compared to prior methods.
...
TL;DR: Diffusion models are combined with domain knowledge to solve medical imaging inverse problems, achieving state-of-the-art performance without paired training data.


<details>
  <summary>Details</summary>
Motivation: The challenge of integrating data-driven diffusion models with domain knowledge in neuroimaging, where Bayesian inverse problems traditionally rely on classical priors that lack anatomical complexity.

Method: A score-based diffusion prior trained on diverse brain MRI data, paired with flexible forward models for tasks like super-resolution and inpainting.

Result: State-of-the-art performance on heterogeneous MRI data, producing high-quality solutions without paired datasets.

Conclusion: Diffusion priors are versatile tools for brain MRI analysis, enhancing anatomical fidelity and performance.
...
TL;DR: FFT-MIL enhances WSI classification by integrating frequency-domain learning with MIL, improving global dependency capture and boosting performance metrics.


<details>
  <summary>Details</summary>
Motivation: Existing MIL methods for WSI classification struggle to capture global dependencies due to the immense size of WSIs and the local nature of patch embeddings, limiting robust diagnostic prediction.

Method: The proposed FFT-MIL framework augments MIL with a frequency-domain branch, using Fast Fourier Transform to extract low-frequency crops and process them through an FFT-Block. This global feature is fused with spatial patch features via lightweight integration strategies.

Result: FFT-MIL improved macro F1 scores by an average of 3.51% and AUC by 1.51% across six MIL methods on three datasets (BRACS, LUAD, and IMP), demonstrating consistent gains.

Conclusion: The integration of frequency-domain learning through FFT-MIL enhances the scalability and accuracy of MIL-based computational pathology by effectively capturing global dependencies in WSI classification.
...
TL;DR: The paper introduces XModBench, a tri-modal benchmark, to evaluate OLLMs' cross-modal consistency, finding current models like Gemini 2.5 Pro still lack modality-invariant reasoning due to biases and imbalances.


<details>
  <summary>Details</summary>
Motivation: To address the lack of clarity on whether OLLMs achieve modality-invariant reasoning or exhibit modality-specific biases, the authors aim to measure cross-modal consistency systematically.

Method: The study introduces XModBench, a large-scale tri-modal benchmark with 60,828 multiple-choice questions spanning five task families and covering all six modality compositions in question-answer pairs.

Result: Experiments reveal that even the strongest model, Gemini 2.5 Pro, struggles with spatial and temporal reasoning (less than 60% accuracy), shows persistent modality disparities (worse performance with audio than text), and exhibits directional imbalance (lower consistency with vision as context).

Conclusion: Current OLLMs, including Gemini 2.5 Pro, fall short of achieving truly modality-invariant reasoning, highlighting the need for further improvement. XModBench serves as a crucial diagnostic tool for evaluating and enhancing cross-modal competence.
...
TL;DR: UniFilter, a multimodal data quality classifier, improves MLLM performance by filtering high-quality image-text data using a semi-synthetic approach, with released resources for community use.


<details>
  <summary>Details</summary>
Motivation: High-quality data filtering for image-text interleaved document data is under-explored, limiting MLLM performance.

Method: A semi-synthetic approach generates labeled multimodal data across four quality levels, enabling efficient training of UniFilter, a Unified Multimodal Data Quality Classifier.

Result: MLLMs pre-trained on UniFilter-curated data show enhanced zero-shot reasoning, in-context learning, and superior performance on benchmarks after visual supervised fine-tuning.

Conclusion: The UniFilter method significantly enhances MLLM capabilities by filtering high-quality multimodal data, leading to improved zero-shot reasoning and in-context learning. The release of synthetic training data, UniFilter checkpoints, and OBELICS-HQ supports community reproduction and development.
...
TL;DR: Reproducibility in histopathology foundation model training is challenged by randomness and inconsistent hyperparameters. Key findings include optimal RandomResizedCrop values (0.7-0.8), benefits of distributed training, and learning rate thresholds (<5.0e-5). The LC25000 (Colon) dataset was the most reproducible.


<details>
  <summary>Details</summary>
Motivation: To address reproducibility challenges in foundation model training for histopathology, caused by software randomness, hardware non-determinism, and inconsistent hyperparameter reporting.

Method: Trained a CLIP model on the QUILT-1M dataset, systematically evaluating hyperparameter settings and augmentation strategies across three downstream histopathology datasets (PatchCamelyon, LC25000-Lung, LC25000-Colon).

Result: Identified trends: RandomResizedCrop values of 0.7-0.8 performed best, distributed training improved stability, and learning rates below 5.0e-5 degraded performance. LC25000 (Colon) was the most reproducible dataset.

Conclusion: Reproducibility in computational pathology requires transparent documentation and careful experimental configurations. Practical rules are provided to guide future reproducible foundation model development.
...
TL;DR: A framework for personalized image generation improves fidelity and diversity in data augmentation, enhancing downstream model performance.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of balancing fidelity and diversity in generative data augmentation by preserving essential image details while aligning with varied text prompts.

Method: A personalized image generation framework using a salient concept-aware image embedding model to reduce irrelevant visual details and align image and text inputs intuitively.

Result: Outperforms state-of-the-art augmentation methods with averaged classification accuracy improvements by 0.73% and 6.5% under conventional and long-tail settings, respectively.

Conclusion: The proposed framework demonstrates superior performance in enhancing dataset diversity and improving downstream model robustness, outperforming state-of-the-art methods with significant accuracy improvements.
...
TL;DR: The paper introduces CARDIUM, a multimodal dataset for prenatal CHD detection, and a transformer-based model that outperforms single-modality methods, with public release of data and code.


<details>
  <summary>Details</summary>
Motivation: The rarity of CHDs leads to imbalanced and low-quality datasets, and the lack of integrated multimodal data limits AI model performance in prenatal diagnosis.

Method: A robust multimodal transformer architecture with a cross-attention mechanism is introduced to fuse feature representations from fetal ultrasound and echocardiographic images along with maternal clinical records.

Result: The proposed method improves CHD detection by 11% and 50% over image and tabular single-modality approaches, respectively, achieving an F1 score of 79.8 ± 4.8%.

Conclusion: The CARDIUM dataset and the proposed multimodal transformer architecture significantly improve prenatal CHD detection, achieving an F1 score of 79.8 ± 4.8%. The public release of the dataset and code aims to foster further research in this field.
...
TL;DR: The paper explores demographic biases in text-to-image models for ads, assessing persuasiveness disparities based on gender/race and testing a country-targeting technique.


<details>
  <summary>Details</summary>
Motivation: To investigate potential biases in text-to-image models for ads and their impact on persuasiveness, as well as to explore targeted ad customization.

Method: Examined demographic bias in ads across topics, compared persuasiveness of ads with varying gender/race portrayals, and experimented with country-specific targeting.

Result: Identified biases in ad persuasiveness linked to demographic portrayals and demonstrated feasibility of country-targeted ads.

Conclusion: Text-to-image models exhibit demographic biases affecting ad effectiveness, but targeted customization is achievable.
...
TL;DR: DriveGen3D is a framework for generating high-quality, controllable 3D driving scenes, combining efficient video synthesis and dynamic 3D reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for driving scene synthesis are limited by computational demands, lack of 3D representation, or static scene reconstruction. DriveGen3D aims to bridge these gaps.

Method: The framework integrates FastDrive-DiT for video synthesis under text and BEV guidance, and FastRecon3D for dynamic 3D reconstruction using 3D Gaussian representations.

Result: DriveGen3D achieves real-time generation of high-resolution driving videos (424×800 at 12 FPS) with SSIM 0.811 and PSNR 22.84 for novel view synthesis.

Conclusion: DriveGen3D successfully addresses limitations in existing methodologies, enabling efficient and high-quality dynamic 3D driving scene generation.
...
TL;DR: cuSfM, a GPU-accelerated offline SfM system, outperforms COLMAP in speed and accuracy, released as open-source for computer vision and robotics.


<details>
  <summary>Details</summary>
Motivation: Efficient and accurate camera pose estimation is crucial for dense reconstruction in autonomous navigation, robotic perception, and virtual simulation systems.

Method: The system employs GPU parallelization to efficiently use computationally intensive feature extractors, supporting pose optimization, mapping, prior-map localization, and extrinsic refinement.

Result: cuSfM achieves significantly improved accuracy and processing speed compared to COLMAP, with high precision and global consistency.

Conclusion: cuSfM, a CUDA-accelerated offline Structure-from-Motion system, outperforms COLMAP in accuracy and speed while maintaining high precision and global consistency. It is released as open-source (PyCuSfM) to aid research and applications in computer vision and robotics.
...
TL;DR: A new MRI inpainting pipeline combining model ensembling, post-processing, and U-Net enhancement improves accuracy and robustness for clinical use.


<details>
  <summary>Details</summary>
Motivation: Automated MRI analysis tools often fail with large lesions like tumors. Image inpainting can synthesize healthy tissues in tumor regions, enabling reliable use of general-purpose tools.

Method: The methodology involves combining model ensembling with post-processing strategies (median filtering, histogram matching, pixel averaging) and a lightweight U-Net enhancement stage.

Result: The pipeline outperforms baseline models in accuracy and robustness, achieving higher anatomical plausibility and visual fidelity.

Conclusion: The proposed pipeline, combining model ensembling with post-processing and a U-Net enhancement stage, improves anatomical plausibility and visual fidelity in MRI inpainting, supporting broader clinical deployment.
...
TL;DR: QCFace introduces a hard-margin strategy to decouple recognizability from identity in face recognition, improving performance and stability.


<details>
  <summary>Details</summary>
Motivation: Current face recognition methods inadequately capture recognizability due to soft margin constraints and overlapping gradients, leading to poor generalization and entangled representations.

Method: QCFace employs a hard-margin-based loss function with a guidance factor for hypersphere planning, optimizing for both recognition and recognizability.

Result: QCFace achieves state-of-the-art performance in verification and identification benchmarks, providing robust recognizability encoding.

Conclusion: QCFace effectively addresses the limitations of current methods by decoupling recognizability and identity, enhancing face recognition performance.
...
TL;DR: The paper introduces a hyperbolic classification framework for Single Positive Multi-Label Learning (SPMLL), using hyperbolic balls to model label relationships, achieving competitive performance and interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing SPMLL methods lack explicit geometric definitions for label relationships, limiting their ability to capture hierarchical and co-occurrence patterns.

Method: Proposes a hyperbolic ball-based framework with a temperature-adaptive classifier and double-well regularization to model label relationships (inclusion, overlap, separation).

Result: Demonstrates competitive performance on benchmark datasets (MS-COCO, PASCAL VOC, NUS-WIDE, CUB-200-2011) with superior interpretability and correlation to real-world patterns.

Conclusion: Hyperbolic geometry is a robust paradigm for structured classification under incomplete supervision, validated by empirical results.
...
TL;DR: SVG replaces VAEs in diffusion models with self-supervised features, improving efficiency and generative quality by leveraging semantically structured latent spaces.


<details>
  <summary>Details</summary>
Motivation: The VAE+diffusion paradigm suffers from limited training efficiency, slow inference, and poor transferability due to the lack of clear semantic separation in VAE latent spaces.

Method: SVG constructs a semantically structured latent space using frozen DINO features and a lightweight residual branch for fine-grained details, training diffusion models directly on this space.

Result: SVG enables accelerated diffusion training, supports few-step sampling, and improves generative quality while preserving semantic discriminability.

Conclusion: SVG provides a principled pathway toward task-general, high-quality visual representations by leveraging self-supervised features, improving generative quality and efficiency.
...
TL;DR: CoMe introduces a progressive pruning framework with concatenation-based merging and hierarchical distillation, addressing limitations of current methods and retaining 83% accuracy when pruning 30% of LLaMA-2-7b's parameters.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of current structured pruning methods, which often lead to performance degradation, incompetent weight layer aggregation, and lack of post-training recovery mechanisms.

Method: The proposed method, CoMe, includes a progressive layer pruning framework with concatenation-based merging and hierarchical distillation. It introduces a channel sensitivity metric for fine-grained channel selection and a layer merging method to fuse critical channels across adjacent layers.

Result: Experiments on seven benchmarks demonstrate that CoMe retains 83% of the original average accuracy when pruning 30% of LLaMA-2-7b's parameters, achieving state-of-the-art performance.

Conclusion: The paper concludes that CoMe, a progressive layer pruning framework with concatenation-based merging and hierarchical distillation, effectively addresses the limitations of existing structured pruning methods, achieving state-of-the-art performance while retaining significant model accuracy.
