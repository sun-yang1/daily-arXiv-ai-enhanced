...
TL;DR: MA-LipNet introduces multi-attention modules (CA, JSTA, SSTA) to refine lipreading features, achieving lower error rates and better generalization than existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of existing lipreading methods, which suffer from poor feature discriminability and generalization due to subtle articulatory gestures. The paper aims to enhance these aspects by purifying visual features across temporal, spatial, and channel dimensions.

Method: The proposed method, MA-LipNet, sequentially applies three attention modules: Channel Attention (CA) for channel-wise feature recalibration, Joint Spatial-Temporal Attention (JSTA) for coarse-grained spatio-temporal filtering, and Separate Spatial-Temporal Attention (SSTA) for fine-grained refinement.

Result: Experimental results on the CMLR and GRID datasets show that MA-LipNet significantly reduces Character Error Rate (CER) and Word Error Rate (WER), outperforming state-of-the-art methods.

Conclusion: The paper concludes that multi-dimensional feature refinement, as demonstrated by MA-LipNet, is crucial for robust visual speech recognition, significantly improving performance metrics like CER and WER.
...
TL;DR: The paper addresses the challenge of non-Markov conversational image generation, where models must reference long-range history. It introduces data construction, training, and inference methods to improve multi-round consistency and instruction compliance.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal large language models (MLLMs) often rely on Markovian shortcuts, ignoring long-range chat history. The work aims to tackle non-Markov scenarios where users reference earlier states or entities.

Method: The paper proposes (i) non-Markov data construction (e.g., rollback-style editing, name-based personalization), (ii) a history-conditioned framework with token-level caching, and (iii) techniques like a DiT detokenizer and multi-stage fine-tuning.

Result: The approach improves multi-round consistency and instruction compliance while maintaining single-round editing and personalization performance.

Conclusion: Training explicitly for non-Markov interactions enhances model performance in multi-turn conversational image generation, addressing limitations of Markovian approaches.
...
TL;DR: A novel text-guided denoising method using a pretrained CLIP model with U-Net improves PET image quality across various count levels, showing potential for reducing acquisition time.


<details>
  <summary>Details</summary>
Motivation: PET images often suffer from noise that obscures diagnostic information, influenced by factors like scanner hardware and acquisition time.

Method: The study proposes a text-guided denoising method combining a pretrained CLIP model with a U-Net based denoising model.

Result: The model significantly improves PET image quality in both qualitative and quantitative assessments.

Conclusion: The method's flexibility suggests potential for addressing complex denoising needs or reducing acquisition time in PET imaging.
...
TL;DR: Proposes a synthetic data-based dual-domain method to correct CT detector artifacts without real clinical data, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To address the high cost of collecting dedicated datasets for training and the limitations of existing methods that ignore intrinsic CT geometry correlations.

Method: An unrolled dual-domain method leveraging synthetic data from natural images to exploit intrinsic correlations between sinogram and image domains.

Result: Outperforms state-of-the-art approaches in correcting LPP artifacts, especially with 1-2% detector defects near the isocenter.

Conclusion: The proposed unrolled dual-domain method effectively corrects LPP artifacts without needing real clinical data, offering adaptability across different scanner settings.
...
TL;DR: An AI-based model trained on prostate biopsy slides predicts biochemical recurrence (BCR) post-radical prostatectomy, showing generalizability across external cohorts and incremental improvement over CAPRA-S.


<details>
  <summary>Details</summary>
Motivation: Current prognostic tools for BCR after radical prostatectomy are imprecise, necessitating better predictive methods.

Method: The study used foundation models and attention-based multiple instance learning on biopsy slides from the STHLM3 cohort (n=676) and validated across three external cohorts (LEOPARD, CHIMERA, TCGA-PRAD).

Result: The model achieved 5-year AUCs of 0.64, 0.70, and 0.70 in external cohorts. Integrating clinical variables improved risk stratification, outperforming CAPRA-S.

Conclusion: Biopsy-trained AI can generalize across specimen types for preoperative/postoperative decision-making, but further scrutiny is needed for multimodal AI approaches.
...
TL;DR: BadDet+ is a penalty-based framework for backdoor attacks in object detection, addressing weaknesses in existing methods by unifying Region Misclassification Attacks (RMA) and Object Disappearance Attacks (ODA) with improved physical robustness and position/scale invariance.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor attacks in object detection rely on unrealistic assumptions and lack physical validation, leaving a gap in understanding their impact.

Method: BadDet+ uses a log-barrier penalty to suppress true-class predictions for triggered inputs, ensuring position/scale invariance and physical robustness.

Result: BadDet+ outperforms existing RMA and ODA baselines in synthetic-to-physical transfer while maintaining clean performance. Theoretical analysis confirms its reliability.

Conclusion: The study reveals significant vulnerabilities in object detection, emphasizing the need for specialized defenses against backdoor attacks.
...
TL;DR: ActionVLM balances vision and language in TAL by dynamically reweighting language and treating it as a refinement, achieving 3.2% higher mAP on THUMOS14.


<details>
  <summary>Details</summary>
Motivation: Existing approaches in Temporal Action Localization overemphasize linguistic priors, causing a modality bias that undermines visual performance. The goal is to balance vision and language modalities more effectively.

Method: The paper introduces ActionVLM, featuring a debiasing reweighting module to dynamically adjust language modality based on its incremental benefit over vision, and a residual aggregation strategy that treats language as a complementary refinement.

Result: ActionVLM outperforms state-of-the-art methods by up to 3.2% mAP on THUMOS14, demonstrating reduced modality bias and improved temporal reasoning.

Conclusion: ActionVLM effectively mitigates modality bias in Temporal Action Localization by prioritizing visual signals and adaptively leveraging language, achieving superior performance with a 3.2% mAP improvement over state-of-the-art methods on THUMOS14.
...
TL;DR: An AI framework is proposed for automated assessment of microanastomosis instrument handling skills, addressing limitations of traditional subjective evaluations.


<details>
  <summary>Details</summary>
Motivation: Traditional microsurgical skill assessment relies on subjective expert judgment, which suffers from variability, bias, and inefficiency.

Method: The framework includes instrument detection (YOLO), tracking (DeepSORT), tip localization (shape descriptors), and a classification module trained on expert-labeled data.

Result: The system achieves 97% precision in instrument detection and 96% mAP50-95.

Conclusion: The proposed AI framework offers a reliable, scalable, and objective alternative to traditional microsurgical skill assessment methods.
...
TL;DR: SDCI introduces a dual-branch framework with cross-model attention fusion and bidirectional diffusion to improve training-free open-vocabulary semantic segmentation in high-resolution remote sensing imagery.


<details>
  <summary>Details</summary>
Motivation: High-resolution remote sensing imagery's dense objects and complex boundaries challenge existing methods, which use simplistic fusion strategies.

Method: SDCI uses cross-model attention fusion (CAF), bidirectional cross-graph diffusion refinement (BCDR), and superpixel collaborative prediction (CSCP) for better segmentation.

Result: SDCI outperforms existing methods on benchmarks, with ablation studies confirming the effectiveness of superpixel structures in deep learning.

Conclusion: SDCI effectively addresses the limitations of current training-free OVSS methods, demonstrating superior performance and validating the role of traditional superpixel techniques in modern frameworks.
...
TL;DR: GeoDiff-LF, a diffusion-based framework, enhances underwater 4-D light field imaging with geometric adaptations, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The challenge of acquiring high-quality underwater images via 4-D light field imaging drives the need for improved methods.

Method: The framework includes a modified U-Net with adapters, a geometry-guided loss function, and an optimized sampling strategy.

Result: GeoDiff-LF effectively mitigates color distortion and enhances underwater imaging, demonstrating superior performance in experiments.

Conclusion: GeoDiff-LF advances underwater 4-D light field imaging by integrating diffusion priors and LF geometry, outperforming existing methods in visual fidelity and quantitative performance.
...
TL;DR: FRISM introduces a fine-grained reasoning injection framework for VLMs, using subspace-level model merging to enhance reasoning without losing visual capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing methods for merging VLMs and LRMs operate at a coarse-grained layer level, leading to trade-offs between reasoning and visual capabilities.

Method: FRISM decomposes LRM task vectors via SVD, adaptively tunes subspace scaling coefficients, and employs a label-free self-distillation learning strategy.

Result: FRISM achieves state-of-the-art performance across diverse visual reasoning benchmarks while preserving visual capabilities.

Conclusion: FRISM effectively balances reasoning and visual capabilities in VLMs through fine-grained subspace-level merging.
...
TL;DR: GRDR improves two-stage TVR by enhancing generative recall with multiple semantic IDs and joint training, achieving accuracy comparable to dense retrievers while significantly reducing storage and speeding up retrieval.


<details>
  <summary>Details</summary>
Motivation: The need for efficient and scalable two-stage TVR systems due to limitations of dense retrieval in large-scale applications.

Method: GRDR introduces a query-guided multi-view tokenizer for multiple semantic IDs per video and joint training with a shared codebook.

Result: GRDR matches dense retrievers in accuracy, reduces storage by an order of magnitude, and speeds up retrieval by up to 300x.

Conclusion: GRDR effectively addresses semantic ambiguity and cross-modal misalignment in generative recall, making it a viable solution for large-scale TVR.
...
TL;DR: Thinker is a vision-language model for robotics, addressing perspective and temporal reasoning issues with a new dataset and key-frame input method, achieving top results on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Large vision-language models struggle with human-simple tasks like perspective confusion and overlooking video endings in robotics.

Method: Constructed a specialized dataset for robotic perception and reasoning, and introduced a key-frame and full-video input approach.

Result: Achieved state-of-the-art performance on two benchmark datasets for task planning.

Conclusion: Thinker effectively addresses key challenges in robotics by combining a tailored dataset and improved video comprehension.
...
TL;DR: LAMP introduces a black-box method for generating Universal Adversarial Perturbations (UAPs) targeting multi-image MLLMs, outperforming existing methods by disrupting cross-image information aggregation and spreading adversarial effects.


<details>
  <summary>Details</summary>
Motivation: The vulnerabilities of multi-image MLLMs are unexplored, and existing adversarial attacks focus on single-image settings with impractical white-box assumptions.

Method: LAMP uses an attention-based constraint to disrupt information aggregation, a cross-image contagious constraint to spread adversarial effects, and an index-attention suppression loss for position-invariant attacks.

Result: LAMP achieves the highest attack success rates across multiple vision-language tasks and models, outperforming state-of-the-art baselines.

Conclusion: LAMP effectively exposes vulnerabilities in multi-image MLLMs, demonstrating the need for robust defenses against such attacks.
...
TL;DR: PTQ4ARVG introduces a training-free quantization method for ARVG models, addressing outliers and distribution issues, achieving 8/6-bit quantization with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Quantization for AutoRegressive Visual Generation (ARVG) models is underexplored, and existing methods fail to generalize effectively due to challenges like outliers, dynamic activations, and distribution mismatches.

Method: The paper proposes PTQ4ARVG, a training-free post-training quantization framework addressing channel-wise outliers (via Gain-Projected Scaling), token-wise variance (via Static Token-Wise Quantization), and sample-wise distribution mismatch (via Distribution-Guided Calibration).

Result: PTQ4ARVG successfully quantizes ARVG models to 8-bit and 6-bit with competitive performance.
...
TL;DR: NFCDS improves diffusion sampling by controlling noise frequency, balancing fidelity and perception without extra training.


<details>
  <summary>Details</summary>
Motivation: Address the trade-off between perceptual quality and data fidelity in diffusion sampling-based PnP methods caused by reverse diffusion noise.

Method: Introduces Noise Frequency-Controlled Diffusion Sampling (NFCDS), a spectral modulation mechanism using a Fourier-domain filter to suppress low-frequency noise and preserve high-frequency content.

Result: NFCDS enhances fidelity-perception balance in zero-shot tasks, enabling high-fidelity and perceptually convincing results.

Conclusion: NFCDS effectively mitigates the fidelity-perception conflict in diffusion-based restoration by controlling noise frequency, offering a plug-and-play solution.
...
TL;DR: Hypersolid reinterprets representation learning as a discrete packing problem, using local hard-ball repulsion to prevent collapse, excelling in fine-grained and low-resolution tasks.


<details>
  <summary>Details</summary>
Motivation: To address representation collapse in self-supervised learning without relying on global regularization methods.

Method: Reinterprets representation learning as a discrete packing problem, using short-range hard-ball repulsion (Hypersolid) to maintain injectivity and prevent local collisions.

Result: Achieves high-separation geometric regime, preserving augmentation diversity and excelling in fine-grained and low-resolution classification tasks.

Conclusion: Hypersolid offers a novel, effective approach to prevent representation collapse by focusing on local geometric constraints, outperforming global regularization methods in specific tasks.
...
TL;DR: A lightweight framework for high-fidelity 3D talking face compression using FLAME-based modeling and 3DGS neural rendering, achieving superior performance at low bitrates.


<details>
  <summary>Details</summary>
Motivation: The demand for immersive 3D video conferencing requires high-fidelity face representation at low bitrates, which traditional 2D compression and implicit neural rendering methods fail to address efficiently.

Method: Integrates FLAME-based parametric modeling with 3DGS neural rendering, transmitting essential facial metadata and using Gaussian attribute compression and MLP optimization for efficiency.

Result: Achieves superior rate-distortion performance, enabling high-quality facial rendering at extremely low bitrates.

Conclusion: The proposed framework is effective for real-time 3D video conferencing, balancing fidelity and efficiency.
...
TL;DR: VLMs are good at geolocation but bad at explaining why. This paper introduces a benchmark showing human experts outperform VLMs in reasoning chains.


<details>
  <summary>Details</summary>
Motivation: VLMs excel at geolocation prediction but often fail to explain their reasoning, hallucinating scene attributes. This paper aims to assess and improve VLM reasoning capabilities.

Method: The paper introduces the first benchmark for geolocation reasoning chains, collaborating with expert GeoGuessr players to produce 800 ground truth reasoning chains for 500 query scenes. It evaluates LLM-as-a-judge and VLM-as-a-judge strategies.

Result: Large, closed-source VLMs like Gemini and GPT 5 match human experts in prediction but lag in reasoning chains. Open-weight VLMs perform poorly, barely surpassing hallucinations.

Conclusion: The benchmark highlights a significant gap between human experts and VLMs in producing auditable reasoning chains for geolocation tasks, pointing to VLM limitations in extracting fine-grained visual attributes.
...
TL;DR: A novel multi-modal training framework with TER improves antenna identification by aligning images and PCI signals, outperforming manual methods.


<details>
  <summary>Details</summary>
Motivation: The need for accurate antenna identification in communication networks is hindered by manual inspections, prompting a shift to multi-modal classification using video footage, geometric features, and PCI signals.

Method: The paper introduces a training framework that aligns antenna images with PCI signals, along with a Token Entropy Regularization module during pretraining to enhance cross-modal alignment.

Result: Experiments show that TER accelerates convergence and improves performance, with the first token's entropy being modality-dependent.

Conclusion: The proposed method successfully addresses the challenges of antenna affiliation identification by introducing a novel training framework and TER module, leading to improved performance and convergence.
...
TL;DR: WorldBench is a new benchmark for disentangled evaluation of video-based world models, revealing SOTA models' shortcomings in physical reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks suffer from entanglement, limiting diagnostic capability for assessing physical fidelity in generative models.

Method: Introduces WorldBench, a video-based benchmark designed for disentangled, concept-specific evaluation of physical laws and concepts at two levels: intuitive understanding and low-level properties.

Result: SOTA models show failure patterns in specific physics concepts, lacking physical consistency for real-world interactions.

Conclusion: WorldBench provides a scalable and nuanced framework for evaluating video-based world models, revealing specific failure patterns in physical reasoning and paving the way for more reliable models.
...
TL;DR: GBPN, a hybrid deep learning and probabilistic graphical model framework, excels in depth completion by dynamically constructing MRFs and using adaptive message passing, achieving SOTA results on NYUv2 and KITTI.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of handling sparse and irregular depth data in deep networks, which limits performance, especially under high sparsity.

Method: Introduces Gaussian Belief Propagation Network (GBPN), a hybrid framework combining deep learning with probabilistic graphical models. It dynamically constructs a scene-specific MRF via GMCN and infers dense depth distribution using GBP with adaptive non-local edges and a novel message passing scheme.

Result: GBPN outperforms existing methods, showcasing robustness and adaptability to different sparsity conditions and datasets.

Conclusion: GBPN achieves state-of-the-art performance on NYUv2 and KITTI benchmarks, demonstrating superior robustness and generalizability across varying sparsity levels and patterns.
...
TL;DR: Mam-App, a lightweight Mamba-based model, achieves high accuracy (~99%) for apple leaf disease detection with minimal parameters, suitable for low-resource platforms.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between efficiency and performance in deep learning models for apple leaf disease detection, especially for resource-constrained applications.

Method: The study introduces Mam-App, a parameter-efficient Mamba-based model for feature extraction and leaf disease classification.

Result: Mam-App achieves high accuracy (99.58%), precision (99.30%), recall (99.14%), and F1-score (99.22%) on the PlantVillage Apple Leaf Disease dataset, with only 0.051M parameters. It also performs well on corn and potato datasets.

Conclusion: The proposed Mam-App model effectively balances efficiency and performance, achieving high accuracy with minimal parameters, making it suitable for low-resource platforms.
...
TL;DR: LANE and AdaGraph enhance 3D mesh generation by optimizing autoregressive processes and decoupling strategies.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with slow inference and limited expressible details due to insufficient resource utilization.

Method: LANE introduces compact autoregressive dependencies, and AdaGraph employs spatiotemporal decoupling.

Result: LANE achieves a $6\times$ sequence length improvement and superior performance in speed, detail, and consistency.

Conclusion: LANE, combined with AdaGraph, significantly improves the efficiency and quality of 3D mesh generation.
...
TL;DR: A framework using OT-induced singular boundaries to mitigate OOD overconfidence, achieving better calibration than existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the issue of DNNs producing overconfident predictions on OOD inputs, which reduces their reliability in open-world environments. The authors identify singularities in semi-discrete OT as regions of semantic ambiguity where classifiers often make unwarranted high-confidence predictions.

Method: The authors formulate an OT problem between a continuous base distribution and latent embeddings of training data to identify singular boundaries. They construct OTIS (optimal transport-induced OOD samples) near these boundaries and apply a confidence suppression loss during training.

Result: Experimental results demonstrate that the proposed method significantly reduces OOD overconfidence and surpasses state-of-the-art techniques.

Conclusion: The paper concludes that leveraging the geometry of OT-induced singular boundaries effectively mitigates OOD overconfidence and improves model calibration in uncertain regions, outperforming existing methods.
...
TL;DR: Vision foundation models implicitly learn continuous disease progression from static images, validated by trajectory fidelity and predictive performance, suggesting broader applicability beyond pathology.


<details>
  <summary>Details</summary>
Motivation: The motivation is to determine if vision foundation models, despite being trained on discrete images, can encode continuous processes like disease progression. This is particularly relevant in computational pathology for better reflecting biology and enabling robust generalization.

Method: The paper employs diffusion pseudotime, a method adapted from single-cell transcriptomics, to analyze whether foundation models organize disease states along continuous progression directions in representation space. Four cancer progressions and six models are evaluated.

Result: Results show that pathology-specific models recover trajectory orderings significantly better than null baselines, with vision-only models achieving the highest fidelity (τ > 0.78). Model rankings by trajectory fidelity predict few-shot classification performance (ρ = 0.92), and cell-type composition varies smoothly along inferred trajectories.

Conclusion: The study concludes that vision foundation models can implicitly capture continuous disease progression processes from static images, as demonstrated by their ability to recover coherent trajectory orderings and their predictive power for few-shot classification. This suggests trajectory fidelity as a complementary measure of representation quality.
...
TL;DR: SR$^{2}$-Net boosts HSI-SR with spectral fidelity via a plug-and-play rectifier, achieving better results without architecture changes.


<details>
  <summary>Details</summary>
Motivation: Existing HSI-SR methods often neglect spectral consistency, leading to artifacts. The goal is to improve spatial resolution without sacrificing spectral fidelity or flexibility.

Method: SR$^{2}$-Net uses a plug-and-play rectifier with Hierarchical Spectral-Spatial Synergy Attention (H-S$^{3}$A) and Manifold Consistency Rectification (MCR), along with a degradation-consistency loss.

Result: SR$^{2}$-Net consistently improves spectral fidelity and reconstruction quality across benchmarks and backbones.

Conclusion: The proposed SR$^{2}$-Net effectively enhances spatial resolution while maintaining spectral fidelity and physical plausibility, outperforming existing methods with minimal computational overhead.
...
TL;DR: DAF dynamically fuses adapters in CIL using PAC-Bayes and Taylor expansion, achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: Addresses challenges of destructive interference and catastrophic forgetting in CIL by moving beyond task-specific adapters to a single robust global adapter.

Method: Proposes Dynamical Adapter Fusion (DAF) grounded in PAC-Bayes theorem, utilizing Taylor expansion for optimal fusion coefficients and Robust Initialization for global knowledge capture.

Result: DAF outperforms existing methods on multiple CIL benchmarks, demonstrating superior performance.

Conclusion: DAF effectively balances stability and plasticity in Class-Incremental Learning by dynamically fusing task-specific and global adapters, achieving SOTA performance across benchmarks.
...
TL;DR: SGDS improves plasticity in CIL by dynamically sparsifying activation subspaces, balancing knowledge transfer and interference prevention.


<details>
  <summary>Details</summary>
Motivation: Addressing the plasticity issue caused by rigid orthogonal constraints in lightweight adapters for CIL.

Method: Semantic-Guided Dynamic Sparsification (SGDS) governs activation subspaces through targeted sparsification, promoting knowledge transfer and preventing interference.

Result: SGDS outperforms existing methods on benchmark datasets.

Conclusion: SGDS effectively mitigates interference in Class-Incremental Learning without rigid parameter constraints, achieving state-of-the-art performance.
