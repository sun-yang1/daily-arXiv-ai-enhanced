...
TL;DR: The paper shows that T-YOLOv5 and its CBAM-augmented version outperform standard YOLOv5 in underwater object detection, especially in dynamic and challenging environments.


<details>
  <summary>Details</summary>
Motivation: The study aims to improve underwater object detection by integrating spatio-temporal modeling and spatial attention mechanisms, addressing challenges like dynamic marine environments and partial occlusions.

Method: The research evaluates the performance of T-YOLOv5 and an augmented version with CBAM, comparing them against the standard YOLOv5. Testing involves assessing detection accuracy under various conditions like sudden movements and occlusions.

Result: T-YOLOv5 and T-YOLOv5 with CBAM outperformed YOLOv5, achieving mAP@50-95 scores of 0.813 and 0.811, respectively, demonstrating superior accuracy and generalization in complex scenarios.

Conclusion: The study concludes that T-YOLOv5 significantly enhances detection reliability over the standard YOLOv5, and the addition of CBAM further improves performance in challenging scenarios, albeit with a slight trade-off in simpler cases.
...
TL;DR: MIRO trains text-to-image models with multiple reward models to directly learn user preferences, improving quality, diversity, and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models, trained on uncurated datasets, do not align well with user preferences, and post-hoc reward-based selection harms diversity and efficiency.

Method: MIRO conditions the model on multiple reward models during training to directly learn user preferences, avoiding post-hoc selection.

Result: MIRO dramatically improves visual quality and speeds up training, outperforming existing methods on benchmarks like GenEval and user-preference metrics.

Conclusion: The proposed method, MIRO, achieves state-of-the-art performance on benchmarks and user-preference scores, improving both visual quality and training efficiency.
...
TL;DR: The paper introduces a LiDAR segmentation approach for bicycle safety using the 'SenseBike' platform and the BikeScenes dataset, showing significant improvement over automotive-focused models.


<details>
  <summary>Details</summary>
Motivation: The vulnerability of cyclists, especially with faster e-bikes, drives the need for adapted perception technologies to enhance bicycle safety.

Method: A 3D LiDAR segmentation approach is developed and evaluated using the 'SenseBike' platform and the novel BikeScenes-lidarseg dataset, which includes 3021 annotated LiDAR scans.

Result: Fine-tuning on the BikeScenes dataset achieves a mIoU of 63.6%, a significant improvement over the 13.8% from SemanticKITTI pre-training alone.

Conclusion: The study highlights the importance of domain-specific training for bicycle-mounted perception systems and contributes the BikeScenes dataset to advance cyclist-centric LiDAR segmentation research.
...
TL;DR: ML-based image repair and super-resolution for STM reduces tip-conditioning needs and speeds up data acquisition.


<details>
  <summary>Details</summary>
Motivation: STM's utility is limited by tip degradation, slow data acquisition, and the complexity of tip fabrication. The study aims to address these challenges using ML.

Method: A physics-informed synthetic data generation pipeline was used to train flow-matching and diffusion models on a dataset of 36 pristine experimental images of Si(001):H.

Result: The models effectively restored images and reduced acquisition time by two- to fourfold, as validated by metrics like CMMD score and structural similarity.

Conclusion: The proposed ML framework can significantly enhance STM experimental throughput by reducing tip-conditioning frequency and improving image acquisition speed.
...
TL;DR: A new inversion-free framework decomposes and aggregates flows for better image editing, improving semantic fidelity and disentanglement over existing methods.


<details>
  <summary>Details</summary>
Motivation: Rectified flow models struggle with inaccurate inversion and gradient entanglement in image editing, leading to outputs that do not faithfully reflect the target prompt. Existing ODE-based approaches also yield suboptimal editing quality.

Method: The method involves semantically decomposing the target prompt into sub-prompts, computing independent flows for each, and aggregating them with a projection and soft-aggregation mechanism to ensure diversity and consistency.

Result: Experimental results show the method outperforms existing zero-shot editing approaches in semantic fidelity and attribute disentanglement.

Conclusion: The proposed flow decomposition-and-aggregation framework effectively addresses the limitations of rectified flow models in image editing by enhancing semantic fidelity and attribute disentanglement, outperforming existing zero-shot editing approaches.
...
TL;DR: Brain-IT uses a Brain Interaction Transformer (BIT) to reconstruct images from fMRI data, improving faithfulness and efficiency over current methods.


<details>
  <summary>Details</summary>
Motivation: Current methods for reconstructing images from fMRI lack faithfulness to the actual seen images, prompting the need for a more effective approach.

Method: Brain-IT employs a Brain Interaction Transformer (BIT) to facilitate interactions between functionally-similar brain-voxel clusters, predicting high-level semantic and low-level structural image features to guide reconstruction.

Result: The method achieves faithful image reconstructions, outperforms state-of-the-art approaches, and works efficiently with limited data (1-hour fMRI recordings).

Conclusion: Brain-IT advances fMRI-based image reconstruction by leveraging brain-inspired interactions and localized feature prediction, offering improved performance and data efficiency.
...
TL;DR: The paper fine-tuned SAM 2.1 for real-time tumor tracking in MRI, achieving a Dice score of 0.8794 and ranking 6th in the TrackRAD2025 challenge.
...
Method: The method involved fine-tuning SAM 2.1 with mask-based prompts from the first annotated slice, using a small labeled subset. Training minimized overfitting with 1024x1024 patches, standard augmentations, and a balanced Dice + IoU loss. A low uniform learning rate (0.0001) was applied across all modules.

Result: The model achieved a Dice score of 0.8794 on the hidden test set, ranking 6th in the TrackRAD2025 challenge.

Conclusion: The study demonstrates the strong potential of foundation models, specifically SAM 2.1, for accurate and real-time tumor tracking in MRI-guided radiotherapy, achieving a competitive Dice score of 0.8794 in the TrackRAD2025 challenge.
...
TL;DR: The paper introduces a Hilbert Selective Scan mechanism to enhance Mamba's scanning pattern, improving low-light image enhancement with better efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: To address information inconsistencies and improve coverage in feature space while maintaining long-range dependency handling.

Method: A novel Hilbert Selective Scan mechanism is introduced to increase the Hausdorff dimension of the scanning pattern, refining spatial locality and capturing fine-scale details.

Result: Significant improvements in quantitative metrics and qualitative visual fidelity, with reduced computational resource consumption and shorter inference time.

Conclusion: The proposed Hilbert Selective Scan mechanism enhances the Mamba framework, improving low-light image enhancement and showing potential for broader Mamba-based applications.
...
TL;DR: CAVE is the first benchmark for real-world visual anomalies, introducing tasks like anomaly description, explanation, and justification. It highlights the limitations of current Vision-Language Models (VLMs) in anomaly perception and reasoning.


<details>
  <summary>Details</summary>
Motivation: Current anomaly detection in computer vision is limited to industrial defects or synthetic anomalies, lacking the richness of real-world scenarios. CAVE aims to bridge this gap.

Method: CAVE provides a benchmark with fine-grained annotations for visual grounding and anomaly categorization, inspired by cognitive science research.

Result: State-of-the-art VLMs perform poorly in visual anomaly perception and commonsense reasoning, even with advanced prompting.

Conclusion: CAVE offers a realistic, cognitively grounded benchmark to advance research in anomaly detection and commonsense reasoning for VLMs.
...
TL;DR: A lightweight CNN model predicts coastal flooding under SLR scenarios, outperforming existing methods by reducing MAE by ~20%, offering a scalable tool for flood management.


<details>
  <summary>Details</summary>
Motivation: Climate change and sea-level rise (SLR) threaten coastal cities, necessitating efficient and accurate flood hazard prediction methods. Traditional physics-based simulators are computationally expensive, while existing DL techniques face challenges like data scarcity and high-dimensional outputs.

Method: A novel, lightweight Convolutional Neural Network (CNN)-based model is developed to predict coastal flooding under variable sea-level rise (SLR) projections and shoreline adaptation scenarios. The model leverages a vision-based, low-resource DL framework and is tested on datasets from Abu Dhabi and San Francisco.

Result: The model outperforms state-of-the-art methods, reducing the mean absolute error (MAE) in predicted flood depth maps by nearly 20% on average. It also generalizes well across diverse geographical contexts.

Conclusion: The proposed lightweight CNN-based model demonstrates significant potential as a scalable and practical tool for coastal flood management, aiding decision-makers in developing effective mitigation strategies against climate change impacts.
...
TL;DR: A new Video-LLM architecture with temporal attention in the vision encoder improves video understanding, outperforming existing models in benchmarks by up to +5.5%.


<details>
  <summary>Details</summary>
Motivation: Current Video-LLM architectures struggle with understanding complex temporal dynamics in videos, particularly in tasks requiring detailed comprehension of action sequences and temporal progression.

Method: The work introduces stacked temporal attention modules within the vision encoder to enhance the model's ability to capture action progression and frame relationships.

Result: The approach improves performance in video question answering tasks, especially in action recognition, demonstrating superior temporal reasoning capabilities.

Conclusion: The proposed Video-LLM architecture with stacked temporal attention modules significantly improves temporal reasoning in video understanding, outperforming existing models in benchmarks like VITATECS, MVBench, and Video-MME by up to +5.5%.
...
TL;DR: FlexICL, a flexible in-context learning framework, improves segmentation of bony regions in pediatric elbow and wrist ultrasound images with limited labeled data, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Automatic segmentation of musculoskeletal structures in ultrasound can enhance diagnostic accuracy and treatment planning, but expert annotations for training are costly and time-consuming.

Method: FlexICL uses in-context learning for intra-video segmentation, requiring only 5% of labeled frames. It incorporates novel image concatenation techniques and augmentation strategies to boost performance.

Result: FlexICL outperforms state-of-the-art models by 1-27% Dice coefficient on 1,252 US sweeps, demonstrating robust segmentation with minimal labeled data.

Conclusion: FlexICL offers an efficient, scalable solution for ultrasound image segmentation, particularly valuable in medical imaging with scarce labeled data.
...
TL;DR: A dynamic negative prompting method using VLMs adaptively generates context-aware negative prompts during denoising, outperforming fixed-prompt approaches.


<details>
  <summary>Details</summary>
Motivation: Traditional negative prompting methods use fixed prompts, which lack contextual adaptability during the denoising process.

Method: The approach generates intermediate image predictions during denoising, queries a VLM for contextually appropriate negative prompts, and adapts them dynamically.

Result: The method shows improved performance on benchmark datasets, balancing negative guidance strength and text-image alignment.

Conclusion: The proposed dynamic negative prompting method using VLMs improves text-image alignment and offers flexibility in negative guidance strength compared to traditional fixed-prompt approaches.
...
TL;DR: PReMA is a novel attack exploiting misalignment in diffusion models by modifying input images to manipulate outputs, posing risks in fixed-prompt applications.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the underexplored susceptibility of multi-modal diffusion models to adversarial inputs, particularly the misalignment between textual and image modalities, which risks generating inappropriate content.

Method: The authors introduce PReMA, a novel attack that manipulates generated content by modifying input images without altering the prompt. This method is evaluated on image inpainting and style transfer tasks across various models.

Result: Comprehensive evaluations demonstrate PReMA's efficacy in manipulating model outputs by creating adversarial images, highlighting its threat to model integrity.

Conclusion: The paper concludes that the proposed PReMA attack effectively exploits the misalignment between textual and image modalities in diffusion models, posing a novel threat to their integrity, especially in applications with fixed prompts.
...
TL;DR: The paper introduces EgoExo-Con to evaluate Video-LLMs' consistency across viewpoints, finds current models lacking, and proposes View-GRPO, a reinforcement learning method, to improve cross-view consistency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether Video-LLMs can achieve consistent temporal understanding when videos capture the same event from different viewpoints, addressing limitations in current models.

Method: The study introduces EgoExo-Con, a benchmark for synchronized egocentric and exocentric video pairs, and evaluates Video-LLMs on Temporal Verification and Temporal Grounding tasks. The proposed method, View-GRPO, uses reinforcement learning to enhance view-specific temporal reasoning and cross-view consistency.

Result: The analysis reveals that existing Video-LLMs fail to maintain consistency across viewpoints, and naive finetuning with synchronized videos underperforms single-view training. View-GRPO outperforms naive SFT and GRPO in improving cross-view consistency.

Conclusion: The paper concludes that existing Video-LLMs struggle with maintaining consistency across viewpoints and proposes View-GRPO, a reinforcement learning framework, to improve cross-view consistency.
...
TL;DR: OracleAgent is an agent system designed to manage and retrieve Oracle Bone Script (OBS) information, integrating multimodal tools and a knowledge base to outperform existing models and reduce research time.


<details>
  <summary>Details</summary>
Motivation: Current OBS research faces challenges in interpretation workflow complexity and inefficient information organization and retrieval.

Method: OracleAgent integrates OBS analysis tools with LLMs, orchestrates components flexibly, and uses a multimodal knowledge base (1.4M images, 80K texts) for retrieval tasks.

Result: OracleAgent outperforms leading MLLMs (e.g., GPT-4o) in multimodal tasks and reduces OBS research time for experts.

Conclusion: OracleAgent advances OBS research by improving efficiency and practical deployment of automated interpretation systems.
...
TL;DR: A unified framework jointly optimizes 3D Gaussian points and camera poses, outperforming COLMAP-free methods and COLMAP baselines in novel view synthesis.


<details>
  <summary>Details</summary>
Motivation: Traditional methods rely on external camera pose estimation tools like COLMAP, which introduce computational bottlenecks and propagate errors, limiting their effectiveness in scenarios with large viewpoint variations and sparse feature distributions.

Method: The framework employs a novel co-optimization strategy with two interleaved phases: updating 3D Gaussian parameters via differentiable rendering and refining camera poses using a customized 3D optical flow algorithm with geometric and photometric constraints.

Result: Extensive evaluations show the approach significantly improves reconstruction quality and pose accuracy, particularly in challenging scenarios, outperforming COLMAP-free techniques and COLMAP-based baselines.

Conclusion: The proposed unified framework successfully addresses the limitations of traditional novel view synthesis methods by jointly optimizing 3D Gaussian points and camera poses, outperforming existing COLMAP-free techniques and even surpassing COLMAP-based baselines in reconstruction quality.
...
TL;DR: The paper introduces WOD-E2E, a dataset for challenging long-tail driving scenarios, and proposes RFS, a novel evaluation metric for end-to-end driving performance.


<details>
  <summary>Details</summary>
Motivation: Current E2E driving benchmarks lack challenging scenarios and effective evaluation metrics, limiting the testing of system potential.

Method: The authors curate the WOD-E2E dataset with rare driving scenarios and introduce RFS, a metric based on rater-annotated trajectory preferences.

Result: WOD-E2E includes 4,021 segments of challenging scenarios, and RFS provides a better evaluation of driving performance.

Conclusion: The work aims to advance research in robust and generalizable end-to-end autonomous driving by addressing gaps in benchmarks and metrics.
...
TL;DR: The paper proposes integrating gradient-based attention with CNN features for RGB-D indoor SLAM, showing improved performance, especially in large environments.


<details>
  <summary>Details</summary>
Motivation: Explicit use of gradient-based attention information in CNN representations for semantic object understanding is limited, despite its potential benefits for tasks like SLAM.

Method: The proposed method integrates layer-wise attention information derived from network gradients with CNN feature representations to enhance performance.

Result: Experimental results show improved performance over baseline methods, particularly in large environments.

Conclusion: The integration of gradient-based attention information with CNN feature representations improves frame association performance in RGB-D indoor SLAM, especially in large environments.
...
TL;DR: FullPart combines implicit and explicit paradigms for 3D part generation, using implicit diffusion for bounding boxes and full-resolution voxel grids for detailed parts, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Previous methods either lack geometric detail (implicit tokens) or degrade quality for small parts (shared voxel grids). FullPart addresses these limitations.

Method: FullPart uses implicit box vector-set diffusion for layouts and full-resolution voxel grids for parts, with center-point encoding for alignment.

Result: Achieves state-of-the-art results in 3D part generation, validated by extensive experiments.

Conclusion: FullPart advances 3D part generation by combining implicit and explicit methods, supported by the new PartVerse-XL dataset.
...
TL;DR: BasicAVSR introduces a robust baseline for AVSR with adaptive priors, flow-guided propagation, motion compensation, and hyper-upsampling, outperforming existing methods in quality and speed while offering adaptable variants for different needs.


<details>
  <summary>Details</summary>
Motivation: AVSR faces challenges in spatial detail reproduction, temporal consistency, and computational complexity. The authors aim to address these by developing a robust and adaptable baseline model.

Method: The paper proposes BasicAVSR, incorporating adaptive multi-scale frequency priors, flow-guided propagation, second-order motion compensation, and hyper-upsampling. Three propagation variants (unidirectional RNN, limited lookahead RNN, and bidirectional RNN) are instantiated for different scenarios.

Result: BasicAVSR significantly outperforms existing methods in super-resolution quality, generalization, and inference speed, demonstrating effectiveness across diverse scenarios.

Conclusion: BasicAVSR advances the state-of-the-art in AVSR by integrating key components for spatial detail reproduction, temporal consistency, and computational efficiency. It outperforms existing methods in quality, generalization, and speed, and its adaptable variants cater to diverse application needs.
...
TL;DR: The paper introduces a Multi-View Mammography and Language Model (MV-MLM) for breast cancer classification and risk prediction, leveraging synthetic radiology reports and multi-view supervision to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Large annotated datasets for breast cancer CAD models are costly and time-consuming to acquire. VLMs like CLIP offer a solution by improving robustness and data efficiency.

Method: MV-MLM uses multi-view supervision and cross-modal self-supervision on paired mammogram images and synthetic radiology reports. A joint visual-textual learning strategy enhances generalization.

Result: The model achieves state-of-the-art performance in malignancy classification, subtype classification, and cancer risk prediction, with strong data efficiency.

Conclusion: MV-MLM demonstrates the potential of leveraging synthetic data and VLMs to improve breast cancer diagnosis and risk prediction without requiring extensive annotated datasets.
...
TL;DR: A YOLOv8-based system detects auto-rickshaws in traffic images with high accuracy, addressing challenges in monitoring due to vehicle similarity and manual analysis inefficiency.


<details>
  <summary>Details</summary>
Motivation: Monitoring auto-rickshaws is challenging due to their similarity to non-auto rickshaws and the inefficiency of manual video analysis. An automated solution is needed to enforce traffic rules effectively.

Method: The study employs the YOLOv8 model for real-time object detection, trained on a dataset of 1,730 annotated images captured under various traffic conditions.

Result: The model achieves an mAP50 of 83.447% and binary precision and recall values above 78%, demonstrating strong performance in both dense and sparse traffic scenarios.

Conclusion: The proposed machine learning-based approach using YOLOv8 effectively detects auto-rickshaws in traffic images, achieving high accuracy and performance in diverse traffic conditions. The dataset has been made publicly available for further research.
...
TL;DR: CRAG-MM is a comprehensive benchmark for multi-modal multi-turn conversations in wearable scenarios, featuring diverse datasets and tasks, revealing significant room for improvement in RAG approaches.


<details>
  <summary>Details</summary>
Motivation: The lack of a comprehensive benchmark for multi-modal retrieval-augmented generation (MM-RAG) in wearable scenarios motivated the creation of CRAG-MM.

Method: CRAG-MM includes 6.5K (image, question, answer) triplets and 2K multi-turn conversations across 13 domains, with tasks like single-source augmentation, multi-source augmentation, and multi-turn conversations.

Result: Current RAG approaches achieve only 32%-43% truthfulness, with state-of-the-art solutions showing similar performance, indicating substantial room for improvement.

Conclusion: CRAG-MM serves as a valuable benchmark, already impacting the field through the KDD Cup 2025, with winning solutions improving baselines by 28%.
...
TL;DR: MoTDiff is a high-resolution motion trajectory estimation framework using diffusion models, outperforming existing methods in blind image deblurring and coded exposure photography.


<details>
  <summary>Details</summary>
Motivation: Existing motion representations from single blurred images are often low-quality (coarse-grained and inaccurate), necessitating a high-resolution solution.
...
Result: MoTDiff outperforms state-of-the-art methods in blind image deblurring and coded exposure photography.

Conclusion: The proposed MoTDiff framework effectively estimates high-quality motion trajectories from single blurred images, advancing motion representation quality.
...
TL;DR: ConceptScope is a scalable framework for identifying and quantifying dataset biases using Sparse Autoencoders and vision foundation models, validated on diverse visual concepts and biases.


<details>
  <summary>Details</summary>
Motivation: Dataset bias is common but hard to identify without costly annotations. ConceptScope aims to automate this process.

Method: Uses Sparse Autoencoders on vision foundation model representations to categorize concepts (target, context, bias) and evaluate dataset robustness.

Result: Validated on various visual concepts and biases, including known (e.g., Waterbirds background bias) and unannotated ones (e.g., ImageNet co-occurring objects).

Conclusion: ConceptScope is a practical tool for dataset auditing and model diagnostics, offering scalable bias detection.
...
TL;DR: A novel 'learn from synthesis' approach using a diffusion model and synthetic dataset (SKEP-120K) improves 3D human pose estimation from sketches, outperforming previous methods in accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: The task of 3D human pose estimation from sketches is challenging due to the abstract and disproportionate nature of sketches, and previous methods were limited by the lack of large-scale annotated data and reliance on time-consuming optimization with heuristic rules.

Method: The approach uses a 'learn from synthesis' strategy, involving a diffusion model to synthesize sketches from 2D poses, creating a synthetic dataset (SKEP-120K). An end-to-end framework combines 2D pose detectors, generative diffusion priors, and a feed-forward neural network for efficient 2D pose estimation, with heuristic loss functions ensuring geometric coherence and accurate self-contacts.

Result: The model achieves superior performance in estimation accuracy and speed, validated through comprehensive evaluations.

Conclusion: The proposed model significantly outperforms previous methods in accuracy and speed for sketch-to-pose tasks, as demonstrated by qualitative, quantitative, and subjective evaluations.
...
TL;DR: A novel deep learning network (Ch-EGN) improves supply chain sustainability and risk management with high accuracy, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: To enhance supply chain sustainability and performance efficiency by addressing risk management and product classification challenges using advanced deep learning techniques.

Method: A hybrid convolutional and geometric deep learning network (Ch-EGN) is introduced to analyze supply chain datasets, leveraging information dependencies to derive hidden states.

Result: High accuracy rates are achieved: 98.95% for risk management, 100% and 98.07% for product group and relation classification, and 92.37% for company relation classification.

Conclusion: The proposed Chebyshev ensemble geometric network (Ch-EGN) demonstrates significant improvements in accuracy for risk management and sustainability tasks in supply chains, outperforming state-of-the-art methods.
...
TL;DR: The paper introduces OmniLayout-1M, a diverse document layout dataset, and OmniLayout-LLM, a model that outperforms existing methods in layout generation, addressing the lack of diversity in current research.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the underrepresentation of diverse document layouts in existing research, particularly for open-world genres like newspapers and magazines, and the limitations of current methods in handling complex domains and long sequences.

Method: The paper introduces OmniLayout-LLM, a 0.5B model with a two-stage Coarse-to-Fine learning paradigm: 1) learning universal layout principles from the OmniLayout-1M dataset, and 2) transferring this knowledge to specific domains with fine-grained annotations.

Result: The results show that OmniLayout-LLM achieves strong performance across multiple domains in the M$^{6}$Doc dataset, surpassing existing layout generation experts and general-purpose LLMs.

Conclusion: The paper concludes that OmniLayout-LLM, a 0.5B model with a two-stage learning paradigm, significantly outperforms existing methods in document layout generation, as demonstrated by extensive experiments on the M$^{6}$Doc dataset. The authors also commit to publicly releasing their code, models, and dataset.
...
TL;DR: VLMs struggle with temporal and causal understanding in videos, performing poorly compared to humans on the AoT-PsyPhyBENCH benchmark.


<details>
  <summary>Details</summary>
Motivation: The motivation was to address the under-evaluated and weak temporal information grasp in modern VLMs, particularly in judging the arrow of time (AoT) in videos.

Method: The authors introduced AoT-PsyPhyBENCH, a psychophysically validated benchmark, to evaluate VLMs' ability to infer temporal direction in videos. They tested both open-weight and proprietary VLMs, comparing their performance to human baselines.

Result: Most VLMs performed near chance, significantly lagging behind human accuracy, especially in recognizing physically irreversible processes and causal manual actions.

Conclusion: The study reveals a fundamental gap in current vision-language models (VLMs) regarding temporal and causal understanding, despite their strong visual-semantic correlations. The release of AoT-PsyPhyBENCH aims to spur progress in these areas.
