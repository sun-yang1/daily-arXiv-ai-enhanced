...
TL;DR: Thermal imaging estimates EDA, HR, and BR via signal-processing pipelines, achieving moderate accuracy with potential for improvement.


<details>
  <summary>Details</summary>
Motivation: Thermal infrared imaging offers a contactless method to estimate EDA, HR, and BR, addressing limitations of visible-light methods that cannot access EDA.

Method: The paper employs a signal-processing pipeline involving anatomical region tracking, spatial aggregation, and separation of slow sudomotor trends from faster cardiorespiratory components. Specific techniques include OMIT decomposition for HR and spectral peak detection for BR.

Result: The best EDA configuration achieved a mean absolute correlation of 0.40 ± 0.23, with some sessions reaching 0.89. BR estimation had an MAE of 3.1 ± 1.1 bpm, while HR estimation had an MAE of 13.8 ± 7.5 bpm.

Conclusion: The study establishes baseline performance bounds and design guidance for thermal contactless biosignal estimation, highlighting the potential and limitations of thermal imaging for HR, BR, and EDA measurement.
...
TL;DR: LLaMo introduces a unified framework using Mixture-of-Transformers to enable scalable multimodal adaptation without compromising LLMs' linguistic capabilities, achieving real-time motion generation and high-fidelity motion-language tasks.


<details>
  <summary>Details</summary>
Motivation: Current approaches for unifying motion-language generation and understanding often suffer from catastrophic forgetting of linguistic capabilities due to limited text-motion pairs and jitter artifacts from discrete tokenization.

Method: The proposed LLaMo framework extends pretrained LLMs using a modality-specific Mixture-of-Transformers (MoT) architecture. It encodes motion into a causal continuous latent space and employs a lightweight flow-matching head for real-time streaming motion generation (>30 FPS).

Result: LLaMo demonstrates superior performance in high-fidelity text-to-motion generation and motion-to-text captioning, excelling in zero-shot motion generation.

Conclusion: LLaMo represents a significant advancement towards a unified motion-language large model, achieving high-fidelity text-to-motion generation and motion-to-text captioning, particularly in zero-shot scenarios.
...
TL;DR: CLIP-based SID detectors excel but struggle with generalization; SynthCLIC dataset reveals reliance on high-level attributes, not artifacts.


<details>
  <summary>Details</summary>
Motivation: Addresses the generalization challenges of synthetic image detection (SID) methods and explores CLIP's potential for accurate SID.

Method: Uses SynthCLIC dataset with interpretable linear head and text-grounded concept-model to analyze CLIP-based detectors.

Result: CLIP-based detectors perform well (0.96 mAP on GANs, 0.92 on SynthCLIC) but generalize poorly across generator families (as low as 0.37 mAP).

Conclusion: CLIP-based detectors perform well but generalize unevenly across generative architectures, emphasizing the need for continual updates and broader training for more universal SID.
...
TL;DR: Reproducibility study confirms DragDiffusion's claims but highlights hyperparameter sensitivity.


<details>
  <summary>Details</summary>
Motivation: To verify and understand the reproducibility of DragDiffusion's claims for interactive point-based image editing.

Method: Reproducibility study using the authors' implementation and DragBench benchmark, including ablation studies on timestep selection, LoRA-based fine-tuning, and other components.

Result: Close agreement with original trends, with sensitivity to hyperparameters like optimized timestep and feature level. Multi-timestep optimization does not improve accuracy.

Conclusion: Our findings support DragDiffusion's central claims while clarifying the conditions for reliable reproducibility.
...
TL;DR: RL refines mid-to-late transformer layers, enhancing vision-to-reasoning alignment, unlike uniform perception boosts.


<details>
  <summary>Details</summary>
Motivation: To clarify the specific capabilities RL improves compared to supervised fine-tuning in visual reasoning.

Method: Frankenstein-style analysis framework with causal probing, parameter comparison, and model merging.

Result: RL induces consistent inference-time shifts in mid-to-late layers, which are transferable and necessary for gains.

Conclusion: RL's contribution in visual reasoning is not a uniform enhancement but a systematic refinement of mid-to-late transformer computation, improving vision-to-reasoning alignment.
...
TL;DR: ZeroDiff++ enhances ZSL with diffusion-based methods, improving correlations and performance despite scarce data.


<details>
  <summary>Details</summary>
Motivation: To address spurious visual-semantic correlations and the bottleneck of unadaptive fully noised generators in generative ZSL.

Method: ZeroDiff++ uses diffusion augmentation, supervised contrastive representations, multi-view discriminators, Diffusion-based Test time Adaptation (DiffTTA), and Diffusion-based Test time Generation (DiffGen).

Result: Extensive experiments show ZeroDiff++ outperforms existing ZSL methods and handles data scarcity effectively.
...
TL;DR: MonoLoss efficiently boosts monosemanticity and interpretability in sparse autoencoders with significant speedups and performance gains.


<details>
  <summary>Details</summary>
Motivation: Existing monosemanticity metrics are inefficient, and standard training objectives weakly encourage interpretable feature decomposition.

Method: Introduces a single-pass MonoScore algorithm and MonoLoss objective to encourage monosemantic features.

Result: Achieves up to 1200x evaluation speedup, improves MonoScore, and enhances class purity and accuracy.

Conclusion: MonoLoss improves monosemanticity and class purity in SAEs, achieving notable efficiency gains and accuracy improvements in benchmarks.
...
TL;DR: PathoSpatial integrates WSIs and ST for interpretable prognostic modeling, demonstrating strong performance and biological insights.


<details>
  <summary>Details</summary>
Motivation: As paired WSI-ST cohorts grow, leveraging their complementary spatial signals for prognosis is crucial, but cross-modal fusion strategies are limited.

Method: PathoSpatial integrates co-registered WSIs and ST using task-guided prototype learning within a multi-level experts architecture, combining unsupervised within-modality discovery with supervised cross-modal aggregation.

Result: PathoSpatial achieves strong performance across five survival endpoints, outperforming or matching leading unimodal and multimodal methods, with enhanced interpretability.

Conclusion: PathoSpatial serves as a scalable and interpretable proof-of-concept for multimodal learning in spatial omics-pathology fusion, offering biologically grounded prognostic insights.
...
TL;DR: SAFT enhances CLIP's adversarial robustness by fine-tuning with semantic-aware adversarial examples generated using refined textual descriptions, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the insufficiency of cosine similarity between a single image and a hand-crafted template to measure image-text similarity, leading to vulnerabilities in adversarial robustness.

Method: The authors propose Semantic-aware Adversarial Fine-Tuning (SAFT), which involves generating semantic-aware adversarial examples by minimizing the average similarity between the original image and refined textual descriptions. These descriptions are generated by a foundation model to capture core semantic features.

Result: Extensive experiments demonstrate that SAFT achieves substantial improvements in zero-shot adversarial robustness across 16 datasets.

Conclusion: The paper concludes that SAFT, which fine-tunes CLIP's image encoder with semantic-aware adversarial examples, significantly improves zero-shot adversarial robustness compared to current methods.
...
TL;DR: Optimized DenseNet 121 outperforms baseline CNNs in grape leaf disease detection, offering high accuracy and interpretability for sustainable vineyard management.
...
Method: The study employs Optimized DenseNet 121 with domain-specific preprocessing and Grad-CAM for interpretability, comparing it with baseline CNN models.

Result: The model achieves 99.27% accuracy, 99.28% F1 score, 99.71% specificity, and a Kappa of 98.86%, with a 9-second inference time.

Conclusion: The proposed framework using Optimized DenseNet 121 is scalable, precise, and computationally efficient for detecting grape leaf diseases, with high accuracy and interpretability.
...
TL;DR: Segmentation models align with human-like coarse object representations best at intermediate training/sizes, suggesting resource constraints drive this alignment.


<details>
  <summary>Details</summary>
Motivation: To understand whether and when segmentation models acquire human-like coarse, volumetric representations of objects, which trade fine visual details for efficient physical predictions.

Method: A time-to-collision (TTC) behavioral paradigm was used to compare segmentation models with human behavior. The study varied model training time, size, and effective capacity via pruning.

Result: Alignment with human behavior follows an inverse U-shaped curve: small/briefly trained/pruned models under-segment, large/fully trained models over-segment, and intermediate models best match human-like coarse bodies.

Conclusion: Human-like coarse object representations emerge from resource constraints rather than specialized biases, and can be elicited through simple adjustments like early checkpoints, modest architectures, or light pruning.
...
TL;DR: A novel method for slice-level navigation in 3D volumes uses contextual learning and attention, reducing localization errors by 35.7%.


<details>
  <summary>Details</summary>
Motivation: The technique addresses the need for accurate slice-level content navigation in 3D volumes, aiding diagnostic tasks, registration, and segmentation pipelines.

Method: The approach trains a network to insert slices from one sequence into another using contextual representations and a slice-to-slice attention mechanism.

Result: The insertion network reduces slice localization errors from 8.4 mm to 5.4 mm in supervised settings.

Conclusion: The proposed method significantly improves slice localization accuracy by leveraging contextual sequence information, outperforming traditional body part regression techniques.
...
TL;DR: NAST improves VLMs' negation sensitivity in clinical reporting using causal interpretability-guided training, without affecting general alignment.


<details>
  <summary>Details</summary>
Motivation: The study aims to address VLMs' frequent failure to distinguish affirmative from negated medical statements, a critical issue in clinical reporting.

Method: The authors introduce NAST, a method using causal tracing effects to modulate layer-wise gradient updates during fine-tuning, scaling updates based on causal contributions to negation processing.

Result: Experiments show enhanced discrimination of affirmative and negated clinical statements while maintaining general vision-language alignment.

Conclusion: The paper concludes that NAST improves negation discrimination in VLMs without harming general vision-language alignment, showcasing the value of causal interpretability for medical model adaptation.
...
TL;DR: A new method for matching optical and SAR images by transforming them into a shared modality, enabling the use of pre-trained models like RoMa for high-quality co-registration.


<details>
  <summary>Details</summary>
Motivation: The fundamental differences between optical and SAR images make precise co-registration challenging, necessitating a novel approach.

Method: Proposes transforming images into a common modality with equal channels and preserved features, then using the RoMa model for matching.

Result: Superior performance over alternatives on the MultiSenGE dataset, with high-quality matching and versatility.

Conclusion: The approach effectively bridges the gap between optical and SAR images, leveraging pre-trained models without retraining.
...
TL;DR: A self-supervised method using 3D LiDAR to improve 2D image encoders' robustness in adverse conditions, outperforming competitors.


<details>
  <summary>Details</summary>
Motivation: Pre-trained 2D image encoders lack robustness in noisy and adverse weather conditions, necessitating a method to enhance visual perception under such scenarios.

Method: Collaborative Distillation, leveraging 3D LiDAR as self-supervision to improve 2D image encoders' robustness.

Result: The method outperforms competing approaches in diverse downstream tasks and demonstrates strong generalization ability.

Conclusion: The proposed Collaborative Distillation method effectively enhances the robustness of 2D image encoders in noisy and adverse weather conditions while preserving their original capabilities and improving 3D awareness.
...
TL;DR: The paper stratifies P3P singular configurations based on camera center multiplicity, linking them to geometric features like the danger cylinder and circumcircle, and extends this to complementary configurations.


<details>
  <summary>Details</summary>
Motivation: The research aims to comprehensively understand and classify singular configurations in the P3P problem, which are critical for solving camera pose estimation issues in computer vision.

Method: The study employs a local dual space approach within an algebraic-computational framework to systematically analyze singular configurations.

Result: For camera center multiplicity μ ≥ 2, singular configurations lie on the danger cylinder; for μ ≥ 3, on specific generatrices or the circumcircle; and for μ ≥ 4, infinite solutions occur on the circumcircle. Complementary configurations O′ lie on deltoidal surfaces for μ ≥ 2 and cuspidal curves for μ ≥ 3.

Conclusion: The paper provides a detailed geometric stratification of singular configurations in the P3P problem, highlighting the relationship between camera center multiplicity and specific geometric loci like the danger cylinder, Morley triangle, and circumcircle. It also extends this analysis to complementary configurations, showing their deltoidal surface and cuspidal curve dependencies.
...
TL;DR: AD-LiST-JEPA is a self-supervised world model for autonomous driving, using LiDAR data and JEPA framework to predict spatiotemporal evolution, showing improved performance in occupancy completion and forecasting tasks.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving requires scalable, self-supervised world models to capture environmental evolution for long-term planning, avoiding costly human annotations.

Method: Proposes AD-LiST-JEPA, a JEPA-based framework for learning world models from LiDAR data, evaluated via occupancy completion and forecasting (OCF) tasks.

Result: Experiments demonstrate better OCF performance with the pretrained encoder from JEPA-based world model learning.

Conclusion: AD-LiST-JEPA effectively leverages self-supervised learning for autonomous driving world models, showing promise for scalable, annotation-free solutions.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods rely on supervised training with paired shape-program data, which is often unavailable.

Method: PLLM iteratively samples candidate programs, selects high-fidelity executions, and augments programs to construct synthetic program-shape pairs for fine-tuning.

Result: Experiments show consistent improvements in geometric fidelity and program diversity on the ABC dataset.

Conclusion: PLLM enables CAD program synthesis from unlabeled 3D shapes, reducing dependency on paired data.
...
TL;DR: The paper introduces navdream, a benchmark for testing autonomous driving robustness under appearance shifts, and proposes DINOv3-based features for better generalization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the fragility of autonomous driving algorithms under Out-of-Distribution (OOD) conditions by distinguishing between appearance-based shifts and structural scene changes.

Method: The authors establish the navdream benchmark using generative pixel-aligned style transfer to isolate the impact of appearance shifts on driving performance. They then leverage DINOv3 for extracting appearance-invariant features.

Result: The results show that existing planning algorithms degrade significantly under OOD appearance conditions. The proposed solution achieves consistent performance across extreme appearance shifts.

Conclusion: The paper concludes by proposing a universal perception interface using DINOv3 to extract appearance-invariant features, which significantly improves zero-shot generalization across diverse planning paradigms without needing further fine-tuning.
...
TL;DR: Proposes unbiased gradient estimation for event-based vision, improving accuracy and convergence in tasks like optical flow and SLAM.


<details>
  <summary>Details</summary>
Motivation: Current event-based vision methods suffer from biased gradient estimation due to discontinuities in binning operations, limiting learning efficiency. The paper aims to overcome this issue.

Method: The authors introduce a framework for unbiased gradient estimation by synthesizing weak derivatives during backpropagation, leveraging integration by parts and cotangent function reconstruction.

Result: The method reduces RMS error in egomotion estimation by 3.2%, achieves faster convergence (1.57×), lowers EPE in optical flow by 9.4%, and reduces SLAM error by 5.1%.

Conclusion: The proposed framework successfully addresses gradient estimation challenges in event-based vision, demonstrating improved accuracy and convergence in various tasks like egomotion estimation, optical flow, and SLAM.
...
TL;DR: QuEPT introduces an efficient post-training quantization scheme for large language models, enabling dynamic bit-width adaptation and real-time switching between uniform and mixed precision quantization without repeated optimization.


<details>
  <summary>Details</summary>
Motivation: The high storage and optimization costs of Transformer architectures limit research on elastic quantization, especially for large language models. QuEPT addresses this gap.

Method: QuEPT reconstructs block-wise multi-bit errors with one-shot calibration, uses MB-ToMe for dynamic token fusion, and MB-CLoRA for bit-width group correlation.

Result: QuEPT matches or outperforms state-of-the-art post-training quantization methods.

Conclusion: QuEPT offers a flexible and efficient solution for multi-bit quantization in large language models, enhancing both accuracy and robustness.
...
TL;DR: ADSC compresses vision tokens in MLLMs using the LLM's attention mechanism, reducing computational costs significantly while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Addressing the computational cost of processing vision tokens in MLLMs, prior methods are limited by diverse designs or incompatibility with FlashAttention. ADSC avoids heuristics by using the LLM itself as a guide for compression.

Method: Attention-Driven Self-Compression (ADSC) progressively reduces vision tokens using the LLM's attention mechanism without additional score computation or auxiliary modules. It applies uniform token downsampling at selected layers.

Result: ADSC reduces FLOPs by 53.7% and peak KV-cache memory by 56.7%, preserving 98.2% of original performance. It outperforms prior pruning methods in efficiency and accuracy, especially under high compression.

Conclusion: ADSC introduces a novel approach to token compression in MLLMs, leveraging the LLM's natural attention mechanism. It achieves significant efficiency gains (53.7% FLOPs reduction, 56.7% KV-cache memory reduction) while maintaining high performance (98.2% of original performance), outperforming prior methods.
...
TL;DR: ImageRAGTurbo improves few-step diffusion models by using retrieval augmentation to enhance prompt alignment and image quality without increasing latency.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of current few-step diffusion models, such as compromised image quality, poor prompt alignment, and computationally expensive training, the authors propose retrieval augmentation to reduce denoising steps while maintaining quality.

Method: The approach involves retrieving relevant text-image pairs to condition the generation process, editing the denoiser's latent space ($ℎ$-space), and augmenting the UNet denoiser with a trainable adapter in the $ℎ$-space using cross-attention.

Result: Experiments show that ImageRAGTurbo produces high-fidelity images efficiently, matching or surpassing existing methods in prompt fidelity without additional latency.

Conclusion: The proposed ImageRAGTurbo method successfully enhances the efficiency and quality of few-step diffusion models by leveraging retrieval augmentation, offering high-fidelity image generation without increasing latency.
...
TL;DR: AddUNet uses additive skip fusion to stabilize U-Nets for denoising and MTL, showing task-aware skip weights and robust performance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve training stability and controlled information flow in U-Net architectures for image denoising and multi-task learning.

Method: The proposed Additive U-Net (AddUNet) replaces concatenative skips with gated additive fusion to constrain shortcut capacity while maintaining feature dimensionality.

Result: AddUNet achieves competitive reconstruction performance with improved stability, and learned skip weights show task-aware redistribution in MTL.

Conclusion: The study concludes that additive skip fusion in U-Nets acts as an effective architectural regularizer, enabling stable and scalable multi-task learning without added model complexity.
...
TL;DR: The paper introduces CloudyBigEarthNet, a dataset for cloud-robust training, showing that adapting methods to cloudy data improves performance significantly.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitation of excluding cloudy images in machine learning datasets, which reduces applicability in time-sensitive scenarios like natural disasters.

Method: The authors assembled a dataset (CloudyBigEarthNet) of paired optical and radar images with cloud occlusion and adapted state-of-the-art methods to train on cloudy optical data.

Result: Performance drops of 23-33 percentage points were observed for state-of-the-art methods evaluated on cloudy images. Adapting these methods improved performance by 17.2-28.7 percentage points.

Conclusion: The paper concludes that adapting existing methods to handle cloudy optical data during training significantly improves performance on cloudy test cases, demonstrating the feasibility and benefits of cloud-robust approaches.
...
TL;DR: IndicFairFace dataset balances Indian geographical diversity in VLMs, reducing bias with minimal performance impact.


<details>
  <summary>Details</summary>
Motivation: VLMs misrepresent Indian diversity by treating it as a single category, ignoring intra-national diversity and causing geographical bias.

Method: Developed IndicFairFace with 14,400 images, sourced ethically and balanced across states and gender. Used Iterative Nullspace Projection debiasing to quantify and reduce bias in CLIP-based VLMs.

Result: Reduced geographical bias in VLMs with less than 1.5% drop in retrieval accuracy on benchmark datasets.

Conclusion: IndicFairFace addresses geographical bias in VLMs for India, providing a balanced dataset and reducing bias without significantly impacting performance.
...
TL;DR: Motion Prior Distillation (MPD) improves temporal coherence in image-to-video models by aligning forward and backward paths, reducing visual artifacts.


<details>
  <summary>Details</summary>
Motivation: Existing inference-time sampling methods cause temporal discontinuities due to misalignment between forward and backward paths.

Method: Proposes Motion Prior Distillation (MPD), an inference-time technique that distills motion residuals from the forward path into the backward path.

Result: Achieves more temporally coherent inbetweening results, validated through benchmarks and user studies.

Conclusion: MPD effectively suppresses bidirectional mismatch, enhancing the quality of generated frames.
...
TL;DR: CAP improves MCI encoder performance by leveraging channel diversity with IFE and DCP, matching fine-tuning from scratch and reducing the gap to full fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing MCI probing methods underperform, and transferring strategies from other domains is ineffective, prompting the need for a specialized approach.

Method: Proposes Channel-Aware Probing (CAP) using Independent Feature Encoding (IFE) and Decoupled Pooling (DCP) to manage inter-channel diversity.

Result: CAP outperforms default probing, matches training from scratch, and narrows the gap to full fine-tuning across three benchmarks.

Conclusion: CAP effectively addresses the challenge of leveraging fixed MCI representations for downstream tasks, offering a robust solution.
...
TL;DR: ART3mis is a user-friendly, general-purpose tool for annotating 3D cultural heritage objects, designed for non-technical users and supporting real-time interaction and JSON storage.


<details>
  <summary>Details</summary>
Motivation: Archaeologists and cultural heritage experts require advanced tools for annotating 3D objects, but existing solutions are often domain-specific.

Method: ART3mis uses a user-driven, direct-on-surface approach for segmenting and annotating 3D artefacts, supporting real-time handling and JSON data storage.

Result: The tool enables non-technical users to easily annotate and segment detailed 3D cultural objects.

Conclusion: ART3mis provides a versatile solution for 3D annotation in cultural heritage, bridging the gap for non-technical practitioners.
...
TL;DR: VimRAG is a multimodal RAG framework using dynamic graphs and graph-modulated memory encoding, achieving top performance.


<details>
  <summary>Details</summary>
Motivation: Traditional RAG methods struggle with long-context tasks involving multimodal data, particularly visual information, necessitating a more robust framework like VimRAG.

Method: The paper proposes VimRAG, which models reasoning as a dynamic directed acyclic graph and employs a Graph-Modulated Visual Memory Encoding mechanism. It also introduces Graph-Guided Policy Optimization for fine-grained credit assignment.

Result: VimRAG outperforms existing methods on multimodal RAG benchmarks, demonstrating its effectiveness.

Conclusion: VimRAG introduces a novel framework for multimodal retrieval-augmented reasoning, achieving state-of-the-art performance by leveraging dynamic directed acyclic graphs and graph-modulated visual memory encoding.
