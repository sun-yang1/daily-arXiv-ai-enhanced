...
TL;DR: MeCSAFNet is a multi-branch encoder-decoder model for land cover segmentation in multispectral images, outperforming existing methods like U-Net, SegFormer, and DeepLabV3+ with significant mIoU gains.


<details>
  <summary>Details</summary>
Motivation: To improve land cover segmentation accuracy by effectively combining visible and non-visible spectral channels while maintaining computational efficiency.

Method: Uses dual ConvNeXt encoders for separate channel processing, individual decoders for spatial reconstruction, and a fusion decoder with CBAM attention and ASAU activation for enhanced feature integration.

Result: Achieves +19.21% mIoU over U-Net on FBP and +6.48% over DeepLabV3+ on Potsdam. Compact variants also perform well with lower resource usage.

Conclusion: MeCSAFNet is highly effective for multispectral land cover segmentation, offering superior performance and adaptability for resource-constrained environments.
...
TL;DR: A survey on MLLM-based chart analysis, organizing the field's challenges, tasks, datasets, and methodologies while identifying future research directions.


<details>
  <summary>Details</summary>
Motivation: Motivated by the fragmented landscape of MLLM-based chart analysis, the survey aims to provide a systematic organization and comprehensive roadmap for the field.

Method: The paper structures the domain by analyzing challenges, categorizing tasks and datasets, and tracing methodological evolution from classic deep learning to state-of-the-art MLLM paradigms.

Result: The survey highlights the progression of methodologies and categorizes benchmarks, revealing perceptual and reasoning deficits in current models.

Conclusion: The survey concludes by identifying future directions like advanced alignment techniques and reinforcement learning to enhance cognitive capabilities in MLLMs, aiming to foster robust and reliable systems for chart understanding.
...
TL;DR: MPA, a multimodal FSL framework, enhances few-shot learning with semantic, feature, and uncertainty augmentation, achieving significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot learning methods focus primarily on visual modality and lack multimodal information, prompting the development of MPA to address these gaps.

Method: The proposed MPA framework includes LMSE for semantic enhancement, HMA for feature diversity, and AUCA for handling uncertainty.

Result: MPA achieves superior results, outperforming the second-best method by 12.29% and 24.56% in single-domain and cross-domain settings, respectively.

Conclusion: The MPA framework significantly outperforms existing state-of-the-art methods in few-shot learning tasks, demonstrating its effectiveness in both single-domain and cross-domain settings.
...
TL;DR: The paper identifies VER Heads in VLMs, proposes VERA for better long-context understanding, and shows significant performance gains.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges VLMs face in handling long-context and complex reasoning tasks, prompting an investigation into their internal mechanisms and performance bottlenecks.

Method: The paper employs attention analysis to identify VER Heads and introduces VERA, a training-free framework that leverages model uncertainty to trigger explicit verbalization of visual evidence.

Result: VERA yields an average relative improvement of 21.3% on Qwen3-VL-8B-Instruct and 20.1% on GLM-4.1V-Thinking across five benchmarks.

Conclusion: The paper concludes by demonstrating that VERA significantly enhances the long-context understanding capabilities of VLMs, achieving notable improvements on benchmark tasks.
...
TL;DR: RVMS-Bench and RACLO improve video retrieval benchmarks by simulating real-world fuzzy memory searches, revealing MLLM limitations.


<details>
  <summary>Details</summary>
Motivation: Traditional benchmarks fail to reflect real-world video searches characterized by fuzzy, multi-dimensional memories. RVMS-Bench aims to bridge this gap.

Method: RVMS-Bench includes 1,440 samples across 20 categories and four duration groups, using a hierarchical description framework. RACLO employs abductive reasoning to mimic human cognitive processes for fuzzy memory searches.

Result: Experiments show existing MLLMs struggle with real-world video retrieval and moment localization based on fuzzy memories.

Conclusion: RVMS-Bench and RACLO address the limitations of traditional video retrieval benchmarks by simulating real-world fuzzy memory searches, highlighting gaps in existing MLLMs for such tasks.
...
TL;DR: This paper evaluates adversarial robustness of autonomous driving systems, identifies severe vulnerabilities to three attack types, and proposes an effective lightweight detection model (AD^2).


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the underexplored adversarial robustness of end-to-end autonomous driving systems, despite their significant progress. The study aims to evaluate vulnerabilities and propose solutions.

Method: The authors conducted closed-loop evaluations of state-of-the-art autonomous driving agents (Transfuser and Interfuser) under black-box adversarial threat models in CARLA. They tested three attack vectors: physics-based blur, electromagnetic interference, and digital ghost object attacks. They also proposed AD^2, an attack detector based on attention mechanisms.

Result: The results revealed severe vulnerabilities, with driving scores dropping by up to 99% under attacks. The proposed AD^2 detector outperformed existing approaches in detection capability and computational efficiency.

Conclusion: The paper concludes by raising valid safety concerns regarding the adversarial robustness of end-to-end autonomous driving systems and proposes AD^2, a lightweight attack detection model, as a mitigation strategy.
...
TL;DR: Interactive AI-driven tools for 3D Gaussian Splat selection and segmentation, enabling flexible editing without extra optimization.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges of extracting usable objects from in-the-wild captures and the lack of controllable editing techniques for 3D Gaussian Splats.

Method: The paper introduces an interactive suite of tools featuring AI-driven propagation of 2D selection masks to 3DGS selections, coupled with manual selection and segmentation tools.

Result: The results demonstrate improved Gaussian Splat selection and segmentation, enabling user-guided local editing with a custom Video Diffusion Model.

Conclusion: The paper concludes by highlighting the effectiveness of the proposed interactive toolset for Gaussian Splat selection and segmentation, emphasizing its flexibility and user control without requiring additional optimization.
...
TL;DR: Paper exposes visual jailbreak attacks in image editing models, introduces VJA and IESBench, and proposes a training-free defense for improved safety.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored safety risk in vision-prompt editing models, where visual inputs can be exploited for malicious purposes.

Method: The authors introduce Vision-Centric Jailbreak Attack (VJA) and IESBench to study and mitigate visual-to-visual jailbreak attacks. They also propose a defense based on introspective multimodal reasoning.

Result: VJA achieves high attack success rates (up to 80.9% and 70.1% on commercial models). The proposed defense improves safety significantly without additional computational cost.

Conclusion: The paper highlights the vulnerability of vision-prompt editing models to visual jailbreak attacks and proposes both a benchmark (IESBench) and a training-free defense to improve model safety.
...
TL;DR: The paper enhances DDPMs by introducing geometric key feature extraction and network equivariance using group morphological convolutions in Riemannian manifolds, improving performance on datasets like MNIST and CIFAR-10.


<details>
  <summary>Details</summary>
Motivation: Address limitations in DDPMs related to geometric feature extraction and equivariance, as the U-net architecture only supports translation equivariance.

Method: Proposes group morphological convolutions derived from Hamilton-Jacobi PDEs, adding a convection term solved via characteristics to capture nonlinearities and symmetries.

Result: Demonstrates improved performance on MNIST, RotoMNIST, and CIFAR-10 datasets compared to baseline DDPM.

Conclusion: The geometric approach and equivariance property enhance DDPMs, enabling better feature extraction and symmetry incorporation.
...
TL;DR: XSPLAIN introduces interpretability for 3DGS classification via voxel-aggregated PointNet and invertible transformations, preserving accuracy and improving user trust.


<details>
  <summary>Details</summary>
Motivation: The lack of interpretability in 3DGS models hinders adoption in critical domains, prompting the need for a prototype-based explainability framework.

Method: The framework uses a voxel-aggregated PointNet backbone and an invertible orthogonal transformation to disentangle features while preserving decision boundaries.

Result: User studies show XSPLAIN is preferred 48.4% of the time, significantly outperforming baselines (p<0.001).

Conclusion: XSPLAIN significantly improves interpretability in 3DGS classification, outperforming baselines in user trust and transparency.
...
TL;DR: New PMMA dataset for detecting pedestrians with mobility aids; YOLOX, Deformable DETR, Faster R-CNN perform best.


<details>
  <summary>Details</summary>
Motivation: To address the lack of datasets focusing on pedestrians using mobility aids, this study introduces PMMA, a comprehensive outdoor dataset with nine categories of pedestrians.

Method: Seven object detection models and three tracking algorithms were implemented under the MMDetection framework to evaluate performance on the PMMA dataset.

Result: YOLOX, Deformable DETR, and Faster R-CNN achieved the best detection performance, while differences among trackers (ByteTrack, BOT-SORT, OC-SORT) were minimal.

Conclusion: The PMMA dataset provides a valuable resource for object detection and tracking research focused on pedestrians using mobility aids, with YOLOX, Deformable DETR, and Faster R-CNN emerging as top-performing models.
...
TL;DR: A neural network model for estimating skin tone in dermatoscopic images is developed, validated against colorimeter measurements, and shown to improve fairness auditing by addressing annotation gaps in datasets.


<details>
  <summary>Details</summary>
Motivation: The study aims to address performance disparities in neural-network-based dermatoscopic diagnosis across different skin tones by providing reliable skin-tone annotations, which are lacking in public datasets.

Method: The researchers developed neural networks for predicting Fitzpatrick skin type via ordinal regression and Individual Typology Angle (ITA) via color regression, using in-person Fitzpatrick labels and colorimeter measurements. They leveraged pretraining on synthetic and real dermatoscopic and clinical images.

Result: The Fitzpatrick model matched human crowd agreement, and ITA predictions outperformed pixel-averaging methods, showing high concordance with colorimeter-derived ITA. Application to datasets revealed minimal representation of darker skin tones (Fitzpatrick types V and VI).

Conclusion: The study introduces a validated tool for skin-tone annotation and bias auditing, highlighting performance gaps across skin-tone groups in dermatoscopic diagnosis.
...
TL;DR: ERGO is an adaptive optimization framework for 3D reconstruction that handles supervision noise by decomposing losses into excess risk and Bayes error, improving geometric and textural fidelity.


<details>
  <summary>Details</summary>
Motivation: The challenge of generating 3D content from a single image is exacerbated by geometric inconsistencies and textural misalignments in synthesized auxiliary views.

Method: ERGO decomposes optimization losses in 3D Gaussian splatting into excess risk and Bayes error, dynamically adjusting loss weights. It also introduces geometry-aware and texture-aware objectives for adaptive optimization.

Result: ERGO outperforms state-of-the-art methods on the Google Scanned Objects and OmniObject3D datasets.

Conclusion: ERGO demonstrates robustness against supervision noise and consistently enhances both geometric fidelity and textural quality in 3D reconstruction, outperforming existing methods.
...
TL;DR: LoRD defends Latent Diffusion Models against adversarial attacks using low-rank adaptation, ensuring robust image generation.


<details>
  <summary>Details</summary>
Motivation: Prevent misuse of adversarial attacks on diffusion models to protect their practical applications.

Method: Low-Rank Defense (LoRD) integrates merging and balance parameters with LoRA modules to detect and defend adversarial samples.

Result: Experiments show LoRD outperforms baselines in defense performance on facial and landscape images.

Conclusion: LoRD effectively defends adversarial attacks on Latent Diffusion Models, ensuring high-quality image generation even when fine-tuned on adversarial samples.
...
TL;DR: UA-Flow enhances flow matching by predicting uncertainty, improving sample reliability and generation quality.


<details>
  <summary>Details</summary>
Motivation: To address inconsistent or degraded sample quality in sampling-based generative models like flow matching.

Method: UA-Flow extends flow matching by predicting velocity fields with heteroscedastic uncertainty and propagating velocity uncertainty through flow dynamics.

Result: UA-Flow produces uncertainty signals more correlated with sample fidelity and improves generation quality via uncertainty-guided sampling.

Conclusion: UA-Flow effectively improves sample quality in flow matching models by incorporating uncertainty-aware mechanisms.
...
TL;DR: This paper explores uncertainty-aware deepfake detection using stochastic CNNs, showing improved reliability for political contexts.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the operational limitation of existing deepfake detectors, which lack reliability indicators, especially in high-stakes political contexts.

Method: The paper employs stochastic convolutional neural networks within a decision-oriented reliability framework. It evaluates uncertainty through observable criteria like calibration quality and proper scoring rules. Two pretrained CNN backbones (ResNet-18 and EfficientNet-B4) are fine-tuned, comparing deterministic inference with stochastic prediction methods.

Result: Results show that calibrated probabilistic outputs and uncertainty estimates improve risk-aware moderation policies, with systematic confidence-band analysis clarifying their operational value.

Conclusion: The study concludes that calibrated probabilistic outputs and uncertainty estimates enhance risk-aware moderation policies in political deepfake detection, while also highlighting the operational benefits and limitations of uncertainty-aware methods.
...
TL;DR: PGD-MC is a scalable MLE-based method for speckle mitigation in digital holography, avoiding costly matrix inversions by using randomized linear algebra and conjugate gradient, outperforming prior methods in accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: Speckle noise in coherent imaging like digital holography complicates reconstruction. MLE is principled but hindered by high computational costs of matrix inversion, limiting accurate aperture modeling.

Method: Proposes PGD-MC, combining randomized linear algebra and conjugate gradient for scalable MLE optimization without explicit matrix inversions, enabling accurate aperture models.

Result: PGD-MC improves reconstruction quality and computational efficiency, scales to high resolutions, and outperforms prior methods in accuracy and speed across various denoisers.

Conclusion: PGD-MC offers a flexible, effective MLE framework for digital holography, robust to diverse aperture models and superior to existing Plug-and-Play methods.
...
TL;DR: Open-source FDA-cleared deep learning pipelines (AAQ and BMD) for CT scan analysis demonstrate clinical accuracy, addressing transparency and validation gaps.


<details>
  <summary>Details</summary>
Motivation: To address the lack of validated open-source tools and transparency in commercial solutions for opportunistic imaging analysis.

Method: Deep learning pipelines (AAQ and BMD) were developed and validated using external datasets enriched for relevant conditions (AAA and osteoporosis).

Result: AAQ achieved a mean absolute error of 1.57 mm (95% CI 1.38-1.80 mm); BMD showed sensitivity of 81.0% (95% CI 74.0-86.8%) and specificity of 78.4% (95% CI 72.3-83.7%).

Conclusion: The Comp2Comp AAQ and BMD pipelines are validated as accurate for clinical use, offering transparency and open-source solutions for FDA-cleared medical imaging analysis.
...
TL;DR: The paper introduces a pipeline to synthesize Hallucination-Inducing Images (HIIs), revealing scene-conditioned hallucination patterns in VLMs. It proposes the MOH benchmark for evaluation and uses HIIs for alignment, achieving a 38% improvement in hallucination mitigation.


<details>
  <summary>Details</summary>
Motivation: Large VLMs are prone to hallucinations due to language bias, but existing methods overlook underlying patterns. Understanding and mitigating these hallucinations is crucial for model reliability.

Method: The authors design a pipeline to synthesize HIIs, identify hallucination patterns (scene-conditioned object mentions), and create the MOH benchmark. They use HIIs to build preference datasets for fine-grained alignment.

Result: The approach achieves up to a 38% improvement in hallucination mitigation on standard benchmarks while maintaining general model capabilities.

Conclusion: The study successfully identifies and mitigates language-bias-driven hallucinations in VLMs using synthesized HIIs and fine-grained alignment, significantly improving model reliability.
...
TL;DR: ChangeTitans leverages Titans architecture for efficient, accurate remote sensing change detection, outperforming benchmarks with novel backbone and fusion modules.


<details>
  <summary>Details</summary>
Motivation: Addresses limitations in capturing long-range dependencies and computational inefficiency in existing remote sensing change detection methods.

Method: Proposes VTitans (Titans-based vision backbone with neural memory and segmented local attention), hierarchical VTitans-Adapter for multi-scale feature refinement, and TS-CBAM for cross-temporal fusion.

Result: Achieves 84.36% IoU and 91.52% F1-score on LEVIR-CD, with competitive computational performance across four datasets.

Conclusion: ChangeTitans achieves state-of-the-art performance in remote sensing change detection, balancing computational efficiency and accuracy by integrating VTitans backbone, hierarchical VTitans-Adapter, and TS-CBAM fusion module.
...
TL;DR: Proposes adaptive LiDAR sensing for dynamic parameter adjustment, improving registration accuracy and efficiency over fixed-parameter methods.


<details>
  <summary>Details</summary>
Motivation: Conventional LiDAR registration uses fixed configurations, leading to suboptimal data collection and high computational overhead.

Method: An adaptive framework dynamically adjusts LiDAR and registration hyperparameters using registration feedback.

Result: Outperforms fixed-parameter baselines in CARLA simulations, balancing point density, noise, and sparsity.

Conclusion: Adaptive LiDAR shows promise for autonomous perception and robotics by optimizing data acquisition and registration.
...
TL;DR: This paper analyzes Multi-Resolution Hash Encoding (MHE) using physical principles, revealing insights about its spatial resolution and proposing Rotated MHE (R-MHE) for improved performance.


<details>
  <summary>Details</summary>
Motivation: The spatial behavior of MHE lacks rigorous physical understanding, leading to heuristic-based hyperparameter selection. This work aims to provide analytical insights and improve MHE's performance.

Method: The work analyzes MHE through its Point Spread Function (PSF), deriving a closed-form approximation for the collision-free PSF and investigating the impact of finite hash capacity. It introduces R-MHE to address limitations.

Result: Key findings include the discovery that effective resolution is governed by the broadened empirical FWHM rather than the finest resolution, and the development of R-MHE to mitigate anisotropy.

Conclusion: The study provides a rigorous methodology based on physical principles to characterize and optimize Multi-Resolution Hash Encoding (MHE), introducing Rotated MHE (R-MHE) to mitigate anisotropy while maintaining efficiency.
...
TL;DR: GD dataset aids waste segregation research with 13,348 labeled images. EfficientNetV2S performed best (96.19% accuracy), but class imbalance and carbon costs are challenges.


<details>
  <summary>Details</summary>
Motivation: To advance automated waste segregation by providing a diverse, publicly available image dataset (GD) covering 10 household waste categories.

Method: The study employed rigorous validation, class imbalance analysis, visual separability checks (PCA/t-SNE), and background complexity assessment. Benchmarking used deep learning models (EfficientNetV2M, EfficientNetV2S, MobileNet, ResNet50, ResNet101).

Result: EfficientNetV2S achieved the highest performance (96.19% accuracy, 0.96 F1-score) with moderate carbon emissions. Dataset characteristics like class imbalance and brightness variations were identified.

Conclusion: The GD dataset serves as a valuable benchmark for waste classification research, emphasizing challenges like class imbalance, background complexity, and environmental trade-offs in model selection.
...
TL;DR: Med-SegLens introduces interpretable latent features to diagnose and fix segmentation model failures, improving performance without retraining.


<details>
  <summary>Details</summary>
Motivation: Modern segmentation models are opaque, limiting failure diagnosis, understanding of dataset shift, and principled intervention.

Method: Med-SegLens decomposes segmentation model activations into interpretable latent features using sparse autoencoders trained on SegFormer and U-Net. Cross-architecture and cross-dataset latent alignment is performed.

Result: Targeted latent-level interventions correct errors and improve cross-dataset adaptation, recovering performance in 70% of failure cases and boosting Dice score from 39.4% to 74.2%.

Conclusion: Latent-level model diffing offers a practical tool for diagnosing failures and mitigating dataset shift in segmentation models.
...
TL;DR: CoLin introduces a low-rank complex adapter with 1% parameters, outperforming full fine-tuning and delta-tuning in vision tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional full fine-tuning is costly and inefficient for vision foundation models, and delta-tuning's advantages don't directly apply. CoLin aims to improve adaptation efficiency.

Method: Proposes a low-rank complex adapter (CoLin) with a tailored loss to address convergence issues, introducing only 1% parameters.

Result: CoLin outperforms full fine-tuning and delta-tuning in tasks like object detection, segmentation, and image classification, even with minimal parameters.

Conclusion: CoLin provides an efficient and novel solution for deploying vision foundation models, validated by extensive experiments.
...
TL;DR: 3DXTalker is a novel framework for expressive 3D talking avatars, addressing data scarcity and controllability through advanced techniques like disentangled representations and flow-matching transformers.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges in audio-driven 3D talking avatar generation, including limited training data, narrow audio representations, and restricted controllability, which hinder achieving expressivity in avatars.

Method: The paper proposes 3DXTalker, which includes a 2D-to-3D data curation pipeline, disentangled representations for identity modeling, frame-wise amplitude and emotional cues for nuanced expression, and a flow-matching-based transformer for coherent facial dynamics. It also supports prompt-based conditioning for stylized control.

Result: Extensive experiments demonstrate that 3DXTalker excels in lip synchronization, emotional expression, and head-pose dynamics, outperforming existing methods.

Conclusion: 3DXTalker successfully integrates identity modeling, audio-rich representations, and spatial dynamics controllability into a unified framework, achieving superior performance in generating expressive 3D talking avatars.
...
TL;DR: MapVerse is a real-world map benchmark for evaluating VLMs, showing they excel in classification but struggle with complex spatial reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for benchmarking VLMs on map-based reasoning are narrow, domain-specific, and rely on artificial content, limiting evaluation of genuine geospatial reasoning.

Method: The authors present MapVerse, a large-scale benchmark built on real-world maps with 11,837 human-authored QA pairs across 1,025 maps, spanning ten diverse categories. They evaluate ten state-of-the-art models to establish baselines and analyze performance across multiple dimensions.

Result: Models perform competitively on classification tasks but fall short on advanced tasks requiring complex spatial reasoning.

Conclusion: Current VLMs perform well on classification tasks but struggle with complex spatial reasoning, highlighting a significant gap in geospatial reasoning capabilities.
...
TL;DR: A large-scale dataset and noise entropy-based method improve AI-generated image detection, outperforming existing solutions.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations in existing datasets (limited generalization, low quality, simple prompts, lack of diversity) to enhance AI-generated image detection.

Method: A high-quality, large-scale dataset of over 730,000 images (real and AI-generated) is created using advanced methods like text-to-image generation, inpainting, refinement, and face swapping. A detection method based on image noise entropy is also introduced.

Result: Models trained on the dataset show superior generalization, and the proposed detection method performs competitively.

Conclusion: The proposed dataset and lightweight detection method significantly improve the generalization and performance of AI-generated image detection, setting a strong foundation for future research.
...
TL;DR: A novel text-guided framework improves video anomaly detection by enhancing text feature extraction and multimodal fusion, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Text modality's potential in anomaly detection is under-explored due to challenges in feature extraction and fusion redundancy.

Method: The framework includes a multi-stage text augmentation mechanism for fine-tuning text features and a multi-scale bottleneck Transformer fusion module for balanced multimodal integration.

Result: Experiments on UCF-Crime and XD-Violence show superior performance.

Conclusion: The proposed text-guided framework effectively leverages text modality for anomaly detection, achieving state-of-the-art performance through innovative augmentation and fusion methods.
...
TL;DR: C^2RoPE improves RoPE for 3D LMMs by enhancing spatial continuity and causal relationships via hybrid positional indices and Chebyshev masking, validated on benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from identified limitations in RoPE for multimodal processing, specifically disruptions in visual feature continuity and long-term decay in attention allocation due to 1D temporal indices.

Method: The proposed C^2RoPE introduces a hybrid positional index combining 1D temporal positions and Cartesian spatial coordinates, along with Chebyshev Causal Masking based on Chebyshev distance for determining causal dependencies in 2D space.

Result: Evaluation on benchmarks shows C^2RoPE's effectiveness in improving 3D scene reasoning and visual question answering tasks.

Conclusion: The paper concludes that C^2RoPE effectively addresses the limitations of RoPE by improving spatial continuity and causal relationships in visual processing, as demonstrated by superior performance on benchmarks like 3D scene reasoning and visual question answering.
