...
TL;DR: UI-Venus-1.5 introduces Mid-Training, Online RL, and Model Merging to unify GUI tasks, achieving SOTA benchmarks and robust real-world performance.


<details>
  <summary>Details</summary>
Motivation: Achieving broad generality and strong task performance in GUI automation remains challenging, prompting the development of UI-Venus-1.5.

Method: The model employs Mid-Training, Online Reinforcement Learning with full-trajectory rollouts, and Model Merging to unify domain-specific models (grounding, web, mobile).

Result: UI-Venus-1.5 achieves state-of-the-art performance on benchmarks (ScreenSpot-Pro: 69.6%, VenusBench-GD: 75.0%, AndroidWorld: 77.6%) and demonstrates robust real-world navigation capabilities.

Conclusion: UI-Venus-1.5 sets a new benchmark in GUI Agent performance, excelling in diverse benchmarks and real-world applications while unifying domain-specific models into a single checkpoint.
...
TL;DR: Agent Banana introduces a hierarchical framework for high-fidelity image editing, addressing over-editing and multi-turn challenges with Context Folding and Image Layer Decomposition, and validates performance on HDD-Bench.


<details>
  <summary>Details</summary>
Motivation: The paper is motivated by challenges in instruction-based image editing, such as over-editing, lack of multi-turn consistency, and misalignment with real high-resolution workflows.

Method: The paper proposes Agent Banana, a hierarchical agentic planner-executor framework featuring Context Folding for long-horizon control and Image Layer Decomposition for localized edits.

Result: Agent Banana achieves superior multi-turn consistency and background fidelity on HDD-Bench (e.g., IC 0.871, SSIM-OM 0.84, LPIPS-OM 0.12) and performs well on standard single-turn benchmarks.

Conclusion: The paper concludes that Agent Banana provides a reliable solution for professional-grade agentic image editing by addressing key challenges like over-editing, multi-turn consistency, and high-resolution evaluation. It also introduces a new benchmark (HDD-Bench) for rigorous evaluation.
...
TL;DR: Proposes SemanticMoments for motion-centric video retrieval, showing it beats existing methods by focusing on semantic temporal statistics.


<details>
  <summary>Details</summary>
Motivation: Existing video representation methods focus on static appearance and scene context, lacking semantic grounding for motion dynamics.

Method: Proposes SemanticMoments, which computes higher-order temporal moments over features from pre-trained semantic models.

Result: SemanticMoments consistently outperforms RGB, flow, and text-supervised methods on SimMotion benchmarks.

Conclusion: SemanticMoments, a training-free method leveraging temporal statistics in semantic feature space, outperforms existing approaches in motion-centric video understanding.
...
TL;DR: A modular pipeline for extracting names from news videos outperforms generative methods in transparency, achieving high accuracy and traceability.


<details>
  <summary>Details</summary>
Motivation: The increasing volume of video-based news content necessitates reliable methods to extract on-screen information due to impracticality of manual indexing.

Method: The work introduces a modular extraction pipeline designed for deterministic and auditable conditions, evaluated against generative multimodal methods.

Result: The pipeline achieves robust performance (95.8% mAP@0.5) for graphical element localization, with balanced precision (79.9%) and recall (74.4%), outperforming generative methods in transparency.

Conclusion: The paper establishes a methodologically rigorous and interpretable baseline for hybrid multimodal information extraction in news media, emphasizing transparency and reliability.
...
TL;DR: CNNs analyze WSIs of low-grade adenomas to predict colorectal cancer risk, improving early detection and patient-specific care.


<details>
  <summary>Details</summary>
Motivation: Despite screening efforts, some low-grade adenoma patients develop colorectal cancer later in life. Identifying high-risk patients early is crucial for tailored surveillance and therapy.

Method: The study employs convolutional neural networks (CNNs) to analyze whole-slide images (WSIs) of low-grade tubular adenomas for histological features indicative of malignant potential.

Result: The findings suggest that CNNs can detect predictive features in WSIs, addressing a critical gap in current screening and surveillance methods.

Conclusion: Machine learning, specifically CNNs, can identify subtle histological features in low-grade tubular adenomas that predict long-term colorectal cancer risk, offering potential for improved patient surveillance and preventative strategies.
...
TL;DR: TL;DR: A novel ASQL Conditioner improves text-to-image synthesis by using scene graphs for soft guidance, enhancing coherence and diversity without rigid constraints.


<details>
  <summary>Details</summary>
Motivation: The motivation arises from the challenge of maintaining semantic fidelity and structural coherence in text-to-image synthesis for complex prompts, where current models fall short despite photorealistic outputs.

Method: The method involves a zero-shot, scene graph-based conditioning mechanism using the ASQL Conditioner, which generates soft visual guidance via a lightweight language model and guides diffusion-based generation during inference.

Result: The result is a model that maintains text-image alignment while enabling lightweight, coherent, and diverse image synthesis, outperforming rigid pre-defined layout approaches.

Conclusion: The paper concludes that their proposed ASQL Conditioner framework enhances text-to-image synthesis by improving semantic fidelity and structural coherence without limiting flexibility, supported by inference-time optimization.
...
TL;DR: A lightweight CNN-RNN model predicts foot COP and TOI from visual data, showing feasibility for assistive systems.


<details>
  <summary>Details</summary>
Motivation: The ability to predict foot contact with changing environments is underexplored, despite its potential to improve assistive system control.

Method: A CNN-RNN model was trained to forecast COP and TOI within a 250ms window before foot-strike. Data from eight subjects wearing an RGB-D camera and instrumented insoles was used.

Result: COP MAE was 29.42mm, 26.82mm, and 23.72mm at 150ms, 100ms, and 50ms FH, respectively. TOI MAE was 21.14ms, 20.08ms, and 17.73ms at the same FHs. Faster toe-swing speeds improved COP accuracy but not TOI.

Conclusion: The study demonstrates the feasibility of forecasting foot center-of-pressure (COP) and time-of-impact (TOI) using a lightweight CNN-RNN model from visual data, which could enhance anticipatory control in assistive systems.
...
TL;DR: VLM-UQBench evaluates UQ methods in VLMs, revealing gaps in detecting fine-grained uncertainty and weak signals against hallucinations.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of localizing uncertainty in VLMs to its source (image, text, or misalignment) for safer and more reliable model behavior.

Method: Introduces VLM-UQBench, a benchmark with 600 real-world samples and a perturbation pipeline (visual, textual, cross-modal). Proposes two metrics to evaluate UQ methods across VLMs and datasets.

Result: Existing UQ methods show modality-specific specialization and dependence on VLMs, weak correlation with hallucinations, and failure to detect subtle ambiguity.

Conclusion: Current UQ methods fall short in providing fine-grained, modality-aware uncertainty signals necessary for reliable VLM deployment, highlighting a significant gap in addressing subtle, instance-level ambiguity.
...
TL;DR: IR-SIS introduces a language-driven surgical image segmentation system with adaptive refinement and clinician interaction, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing surgical image segmentation methods lack adaptability to new categories, one-shot refinement, and clinician interaction mechanisms.

Method: The system uses fine-tuned SAM3 for initial segmentation, a Vision-Language Model for instrument detection and quality assessment, and an agentic workflow for adaptive refinement. Clinician interaction is supported via natural language feedback.

Result: IR-SIS demonstrates superior performance on in-domain and out-of-distribution data, with further improvements from clinician feedback.

Conclusion: The paper establishes IR-SIS as the first language-based surgical segmentation framework with adaptive self-refinement and clinician interaction capabilities, achieving state-of-the-art performance.
...
TL;DR: Modulation-based text conditioning is optional but valuable for controllable shifts in diffusion models.


<details>
  <summary>Details</summary>
Motivation: To determine if modulation-based text conditioning is necessary and if it offers performance advantages over attention-only approaches.

Method: Analysis of pooled embedding's impact and its alternative use as guidance for controllable shifts.

Result: Pooled embedding contributes little conventionally but provides significant gains when used as guidance for controllable shifts.

Conclusion: Modulation-based text conditioning can provide significant gains when used as guidance for controllable shifts, making it a valuable training-free addition to diffusion models.
...
TL;DR: X-Mark is a robust watermarking method for chest x-rays, ensuring copyright protection while preserving diagnostic quality.


<details>
  <summary>Details</summary>
Motivation: Unauthorized use of high-quality medical imaging datasets raises copyright and ethical issues, necessitating a specialized watermarking solution.

Method: X-Mark uses a conditional U-Net to generate unique perturbations in salient regions and incorporates Laplacian regularization for scale-invariance.

Result: Achieves 100% WSR, reduces false positives by 12%, and resists adaptive attacks.

Conclusion: X-Mark effectively addresses copyright and ethical concerns in medical imaging by providing a robust and scalable watermarking method.
...
TL;DR: A deep multi-modal approach predicts wound hospitalization risk using transfer learning, improving early detection.


<details>
  <summary>Details</summary>
Motivation: High hospitalization costs due to wound deterioration motivate the need for early risk prediction using wound data.

Method: The authors developed a transfer learning-based solution to predict wound variables and healing trajectories from wound images.

Result: The proposed method can predict wound variables and healing trajectories, aiding in early complexity detection.

Conclusion: The paper proposes a deep multi-modal method to predict hospitalization risk by integrating wound variables and images, aiming to improve early detection and reduce clinician diagnosis time.
...
TL;DR: GAFR-Net, a Graph Attention and FuzzyRule Network, improves breast cancer histopathology image classification with scarce supervision and offers interpretable diagnostic logic.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of conventional deep learning architectures in histopathology image classification, particularly their performance degradation under limited annotations and lack of interpretability.

Method: GAFRNet integrates a Graph Attention and FuzzyRule Network, utilizing similarity-driven graph representations and multihead graph attention mechanisms alongside differentiable fuzzy-rule modules for interpretability.

Result: GAFR-Net outperforms state-of-the-art methods across multiple benchmark datasets (BreakHis, Mini-DDSM, and ICIAR2018) in various magnifications and classification tasks.

Conclusion: The study validates GAFR-Net as a reliable decision-support tool for weakly supervised medical image analysis, showcasing its superiority over existing methods.
...
TL;DR: Study evaluates deep models for bladder cancer classification, finding ConvNext less generalizable and ViTs better calibrated. Interpretability varies by model type.


<details>
  <summary>Details</summary>
Motivation: Existing deep models (ViT and CNN) may not perform well in medical imaging due to small abnormal regions, prompting investigation into bladder cancer classification tasks.

Method: Standard classification using 13 models (four CNNs and eight transformer-based models), calibration analysis, and GradCAM++ for interpretability evaluation. Test time augmentation was also used.

Result: ConvNext series showed limited generalization (~60% accuracy), while ViTs exhibited better calibration. Test time augmentation improved interpretability.

Conclusion: ConvNext series are suitable for in-distribution samples, while ViT and its variants are better for interpreting out-of-distribution samples. No single model provides a universal solution for interpretable bladder cancer classification.
...
TL;DR: Kyrtos automates chart curve recognition and analysis in technical documents, preserving structural features and converting them into graphs and natural language.


<details>
  <summary>Details</summary>
Motivation: Technical documents contain valuable knowledge, and understanding them requires analyzing their modalities (e.g., charts) and their associations.

Method: Kyrtos uses clustering for middle-point recognition and segment parsing to analyze curves. It converts segments into attributed graphs and NL sentences, and further into SPN graphs.

Result: Kyrtos successfully recognizes and analyzes chart curves, capturing behavioral features and converting them into attributed graphs and NL sentences.

Conclusion: The extensive evaluation results confirm the accuracy of Kyrtos' methods in recognizing and analyzing chart curves, preserving structural characteristics, and converting them into natural language and SPN graphs.
...
TL;DR: Domain adaptation boosts performance, interpretability, and calibration in medical imaging using deep learning models, with notable gains in noisy, multi-modality, and federated settings.


<details>
  <summary>Details</summary>
Motivation: The study aims to explore and demonstrate the effectiveness of domain adaptation techniques in improving model performance and robustness across diverse medical imaging scenarios, addressing challenges like noise and limited labeled data.

Method: The research employs 10 deep learning models to simulate common DA techniques, tested on four medical image datasets. Scenarios include multi-modality, noisy data, federated learning, interpretability analysis, and classifier calibration.

Result: Experimental results show DA improves performance (e.g., 4.7% boost with ResNet34 on brain tumor data), reduces noise impact (~3% accuracy increase), enhances interpretability via gradcam++, and lowers calibration error (~2% ECE reduction).

Conclusion: The study concludes that domain adaptation (DA) significantly enhances model performance, interpretability, and calibration across various medical image datasets, especially in scenarios involving noise, multi-modality, and federated learning.
...
TL;DR: Proposes DBiSL, a bidirectional synergistic learning framework for semi-supervised medical image segmentation, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the high annotation costs and need for clinical expertise in medical image analysis, coupled with limitations in current semi-supervised learning methodologies that lack bidirectional interaction mechanisms.

Method: The authors introduce the DBiSL framework, integrating supervised learning, consistency regularization, pseudo-supervised learning, and uncertainty estimation seamlessly.

Result: Experimental results on two benchmark datasets demonstrate the state-of-the-art performance of the DBiSL framework.

Conclusion: The paper concludes that the proposed DBiSL framework effectively addresses the limitations of current dual-task collaborative learning methods by enabling online bidirectional cross-task collaboration, achieving state-of-the-art performance on benchmark datasets.
...
TL;DR: Study evaluates 5 image-to-3D models on medical data, finding depth ambiguity limits single-slice reconstruction; SAM3D performs best. Multi-view approaches are recommended.


<details>
  <summary>Details</summary>
Motivation: To address the high cost and long wait times of volumetric imaging by exploring the effectiveness of image-to-3D foundation models for medical data, given their success with natural images.

Method: Five state-of-the-art image-to-3D models (SAM3D, Hunyuan3D-2.1, Direct3D, Hi3DGen, and TripoSG) are evaluated across six medical datasets using voxel-based and point cloud distance metrics.

Result: Voxel-based overlap is moderate across models, indicating depth reconstruction challenges. SAM3D performs best in topological similarity, while other models tend to oversimplify reconstructions.

Conclusion: The study highlights the limitations of single-slice medical image-to-3D reconstruction and emphasizes the need for multi-view approaches to improve reliability in medical 3D inference.
...
TL;DR: K-Sort Eval proposes a scalable, efficient, and reliable VLM-based framework for evaluating visual generative models, addressing limitations of costly human evaluations and static VLM approaches.


<details>
  <summary>Details</summary>
Motivation: Current human-based evaluation methods (e.g., crowdsourced Arena) are costly and time-consuming, while VLMs as substitutes suffer from hallucinations and biases, compromising reliability.

Method: K-Sort Eval integrates posterior correction and dynamic matching, using a high-quality dataset from human votes and (K+1)-wise comparisons. It corrects VLM predictions based on human alignment and optimizes comparisons for efficiency.

Result: K-Sort Eval aligns with human evaluations (K-Sort Arena) and requires fewer than 90 model runs, proving efficient and reliable.

Conclusion: The framework successfully combines the strengths of human judgments and VLMs, offering a scalable and accurate evaluation method for visual generative models.
...
TL;DR: LARV enhances model merging via layer-wise adaptive rescaling, improving performance and robustness without extra training or data.


<details>
  <summary>Details</summary>
Motivation: Existing task-vector merging methods treat layers uniformly, ignoring layer-wise heterogeneity in vision transformers. LARV aims to address this by optimizing layer-specific scaling.

Method: LARV employs a deterministic schedule to assign per-layer scales to task vectors before aggregation, suppressing shallow-layer interference and amplifying deeper-layer alignment. It uses simple data-free layer proxies and lightweight rules.

Result: LARV consistently improves baseline merging methods across different task settings, achieving high performance (e.g., 85.9% on ViT-B/32, 89.2% on ViT-B/16, and 92.6% on ViT-L/14).

Conclusion: LARV introduces a layer-wise adaptive rescaling approach that enhances model merging by addressing layer-wise heterogeneity, improving robustness and performance across diverse merging rules without additional training or data.
...
TL;DR: A unified operator-theoretic framework for stability and concentration in nonlinear inverse problems, with applications to imaging and rendering, showing a stability-resolution tradeoff.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide a unified theoretical foundation for stability and statistical concentration in nonlinear inverse problems with block-structured parameters, applicable to modern imaging and differentiable rendering.

Method: The authors employ an operator-theoretic framework, combining blockwise Lipschitz geometry, local identifiability, and sub-Gaussian noise analysis to establish deterministic stability inequalities and global Lipschitz bounds.

Result: The results include deterministic stability inequalities, nonasymptotic concentration estimates, and high-probability parameter error bounds intrinsic to the forward operator. The Gaussian Splatting rendering operator is validated as a concrete example.

Conclusion: The paper concludes by highlighting a fundamental stability-resolution tradeoff in nonlinear inverse problems, emphasizing that estimation error is inherently constrained by the balance between image resolution and model complexity.
...
TL;DR: A framework using Vision-Language Models (VLMs) for fine-grained truck classification from roadside LiDAR data without fine-tuning, achieving competitive accuracy with minimal labeled examples.


<details>
  <summary>Details</summary>
Motivation: Address scalability challenges in LiDAR-based truck classification by leveraging VLMs to bridge the modality gap between sparse 3D point clouds and dense 2D imagery.

Method: A depth-aware image generation pipeline transforms LiDAR scans into 2D visual proxies, adapting VLMs for classification without parameter fine-tuning.

Result: Achieves competitive accuracy with 16-30 examples per class, with a 75% correct rate for specific drayage categories, and demonstrates a 'Semantic Anchor' effect.

Conclusion: The framework offers a scalable, low-cost solution for ITS applications, reducing reliance on manual annotation and intensive training.
...
TL;DR: SceneReVis introduces a vision-grounded self-reflection framework to resolve spatial conflicts in 3D scene synthesis, leveraging iterative feedback and a novel dataset.


<details>
  <summary>Details</summary>
Motivation: Current one-pass 3D scene synthesis methods suffer from spatial hallucinations like collisions due to a lack of deliberative reasoning.

Method: The framework employs an iterative ``diagnose-and-act'' loop using multi-modal feedback and is trained via a two-stage recipe transitioning from Supervised Fine-Tuning to Agentic Reinforcement Learning.

Result: Extensive experiments show improved performance in high-fidelity generation and goal-oriented optimization, with strong generalization.

Conclusion: SceneReVis achieves state-of-the-art performance in high-fidelity generation and goal-oriented optimization, with robust generalization to long-tail domains.
...
TL;DR: The paper introduces Fine-T2I, a high-quality, open dataset for text-to-image fine-tuning, addressing the lack of diverse, high-resolution, and well-aligned datasets. It combines synthetic and real images, rigorously filtered, and shows consistent performance improvements across models.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the bottleneck of low-quality, limited-diversity datasets for text-to-image fine-tuning, which hinders open research models from matching enterprise-grade performance.

Method: The authors create Fine-T2I, a dataset combining synthetic images from modern models and curated real images. It spans multiple tasks, categories, styles, and templates, with rigorous filtering for alignment and quality.

Result: Fine-T2I contains 6M text-image pairs (~2TB), improving generation quality and instruction adherence across diverse models, as validated by human and automated evaluations.

Conclusion: The release of Fine-T2I under an open license aims to bridge the data gap in text-to-image fine-tuning, fostering progress in the open research community.
...
TL;DR: Proposed training-free latent-trajectory adjustments (Look-Ahead/Look-Back) reduce errors and outperform SOTA models in image generation.


<details>
  <summary>Details</summary>
Motivation: Existing flow matching methods introduce errors by modifying velocity fields, whereas latent trajectory adjustments reduce error accumulation by leveraging pretrained networks.

Method: Two complementary training-free approaches—Look-Ahead (curvature-gated averaging) and Look-Back (exponential moving average)—are introduced to refine latent trajectories.

Result: The methods substantially outperform state-of-the-art models on datasets like COCO17, CUB-200, and Flickr30K.

Conclusion: The proposed training-free latent-trajectory adjustment methods (Look-Ahead and Look-Back) effectively reduce error accumulation and outperform state-of-the-art models across multiple datasets.
...
TL;DR: ArtifactLens, a system using pretrained VLMs with minimal labeled data, achieves state-of-the-art artifact detection in synthetic images, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Current detectors for synthetic image artifacts require extensive labeled data, which is costly and impractical as generators evolve. The need for a more efficient solution drives this work.

Method: ArtifactLens leverages pretrained VLMs with a multi-component architecture, incorporating in-context learning and text instruction optimization, requiring only a few hundred labeled examples per artifact type.

Result: ArtifactLens outperforms existing methods on five human artifact benchmarks and generalizes to other artifact types and AIGC detection.

Conclusion: The study demonstrates that pretrained VLMs, with minimal scaffolding, can effectively detect synthetic image artifacts, reducing reliance on large labeled datasets.
...
TL;DR: FD-DB, a frequency-decoupled dual-branch model, improves synthetic-to-real translation by separating appearance transfer into interpretable low-frequency editing and high-frequency residual compensation, enhancing photorealism and structural stability.


<details>
  <summary>Details</summary>
Motivation: Address domain shift between synthetic and real data in vision tasks by balancing photorealism and structural stability in unpaired synthetic-to-real translation.

Method: Propose FD-DB, a model with dual branches: one for interpretable low-frequency editing (white balance, exposure, etc.) and another for high-frequency residual details. Uses gated fusion and two-stage training.

Result: Improves appearance consistency and boosts semantic segmentation performance on the YCB-V dataset while preserving geometric and semantic structures.

Conclusion: FD-DB effectively reduces domain gap in synthetic-to-real translation, enhancing downstream task performance.
...
TL;DR: Proposes WeakSupCon, a weakly supervised contrastive learning framework for digital histopathology image analysis, improving downstream MIL performance without instance-level labels.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of limited training labels in digital histopathology by leveraging bag-level labels, avoiding costly manual annotations.

Method: Introduces WeakSupCon, a feature representation learning framework that uses bag-level labels to separate patches with different labels in feature space, without pseudo-labeling.

Result: WeakSupCon outperforms self-supervised contrastive learning in three datasets, enhancing downstream MIL performance.

Conclusion: WeakSupCon effectively improves feature representation for MIL in digital histopathology, demonstrating practical utility with available code.
...
TL;DR: Align-TI, a KD framework focusing on token interactions, outperforms existing methods and larger models in MLLM compression.


<details>
  <summary>Details</summary>
Motivation: Existing KD methods for MLLMs neglect dynamic token interactions, which are crucial for multimodal understanding and generation. Align-TI addresses this gap.

Method: The framework includes IVA for aligning instruction-relevant visual information and TPA for capturing dynamic generative logic through token-to-token transition probabilities.

Result: Align-TI achieves a 2.6% improvement over Vanilla KD and outperforms larger models like LLaVA-1.5-7B by 7.0%.

Conclusion: Align-TI introduces a novel KD framework focusing on dynamic token interactions, achieving significant improvements over existing methods and larger models, establishing a new state-of-the-art for efficient MLLMs.
