...
TL;DR: BFD, a two-stage analytical framework, reveals that Vision Transformers' attention mechanisms primarily rely on content-based interactions, with specialized attention heads, and explains DINOv2's advanced shape processing capabilities.


<details>
  <summary>Details</summary>
Motivation: The paper aims to understand the information exchanged between tokens in Vision Transformers, as attention maps alone do not reveal whether queries and keys trade position, content, or both.

Method: Bi-orthogonal Factor Decomposition (BFD) is introduced, involving an ANOVA-based decomposition to disentangle token activations into orthogonal positional and content factors, followed by SVD of the query-key interaction matrix to analyze how these factors mediate communication.

Result: BFD reveals that attention operates primarily through content, with content-content interactions dominating. Attention heads specialize into content-content, content-position, and position-position operators, and DINOv2's superior shape processing stems from intermediate layers preserving positional structure while enriching semantic content.

Conclusion: BFD provides a principled understanding of token interactions in Vision Transformers, revealing the dominance of content-based communication and the specialization of attention heads. It offers practical insights into the mechanisms of vision transformers, particularly in models like DINOv2.
...
TL;DR: VLMs demonstrate strong high-level understanding of complex systems in images but struggle with fine details.


<details>
  <summary>Details</summary>
Motivation: To explore VLMs' ability to recognize and simulate systems depicted in images using Im2Sim methodology.

Method: VLMs analyze real-world images, describe systems, and generate synthetic images via code execution for comparison.

Result: VLMs show deep understanding of multi-component systems but limited replication of fine details.

Conclusion: VLMs combine high-level visual understanding with constrained perception of fine details, revealing an asymmetry in their capabilities.
...
TL;DR: Introduction of STResNet and STYOLO models optimized for edge devices, achieving competitive accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Addressing the trade-off between accuracy and latency in lightweight neural networks for edge hardware.

Method: Development of STResNet for image classification and STYOLO for object detection, optimized for resource-constrained platforms.

Result: STResNetMilli achieves 70.0% Top 1 accuracy with 3M parameters; STYOLOMicro and STYOLOMilli show superior performance on MS COCO.

Conclusion: The proposed models outperform existing solutions like MobileNetV1, ShuffleNetV2, YOLOv5n, and YOLOX Nano in accuracy and efficiency.
...
TL;DR: MOSAIC-GS is a novel method for dynamic scene reconstruction from monocular videos, using geometric cues and efficient motion encoding to achieve fast and high-quality results.


<details>
  <summary>Details</summary>
Motivation: Monocular reconstruction is inherently ill-posed due to lack of multiview constraints, making accurate recovery of object geometry and temporal coherence challenging. The goal is to reduce reliance on ambiguous motion inference from visual appearance alone.

Method: The approach involves decomposing the scene into static and dynamic components, using geometric cues like depth, optical flow, and point tracking for preliminary 3D scene dynamics estimation. Dynamic Gaussians are assigned time-dependent Poly-Fourier curves for efficient motion encoding.

Result: MOSAIC-GS achieves substantially faster optimization and rendering compared to existing methods while maintaining reconstruction quality on par with state-of-the-art approaches.

Conclusion: MOSAIC-GS provides an efficient and high-fidelity solution for dynamic scene reconstruction from monocular videos, leveraging multiple geometric cues and a novel motion encoding method to achieve fast optimization and rendering while maintaining reconstruction quality.
...
TL;DR: Ensemble methods combining DL and radiomics outperform individual techniques in breast cancer detection.


<details>
  <summary>Details</summary>
Motivation: Early diagnosis of breast cancer is crucial for improving survival rates. Radiomics and DL have shown potential in assisting radiologists.

Method: Used RSNA 2023 and TecSalud datasets. Trained ConvNeXtV1-small DL model on RSNA and validated on TecSalud. Radiomics models were developed and validated with a leave-one-year-out approach. Ensemble method combined predictions.

Result: Ensemble achieved AUC of 0.87, DL 0.83, and radiomics 0.80.

Conclusion: Ensemble methods significantly enhance breast cancer diagnosis from mammograms.
...
TL;DR: EdgeLDR introduces a framework combining quaternion neural networks with block-circulant parameter structure, enabling efficient FFT-based computation for edge devices, achieving significant compression with competitive accuracy.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the limitations of deploying deep neural networks on edge devices, which are often constrained by memory traffic and compute cost. Quaternion neural networks improve parameter efficiency but retain unstructured dense weights, while structured matrices enable fast computation but are usually applied in the real domain.

Method: EdgeLDR combines quaternion channel mixing with block-circulant parameter structure and enables FFT-based evaluation through the complex adjoint representation. It includes reference implementations and compares FFT-based computation against naive spatial-domain realization.

Result: FFT evaluation yields large empirical speedups over naive implementation, keeping latency stable as block size increases. EdgeLDR layers integrated into CNN and Transformer backbones show significant compression with competitive accuracy on various classification tasks.

Conclusion: EdgeLDR layers provide significant compression with competitive accuracy, making them a practical solution for deploying deep neural networks on edge devices.
...
TL;DR: A hybrid Gaussian representation (Sketch and Patch) with hierarchical clustering improves 3D content efficiency, achieving better metrics and smaller model sizes.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of efficiently representing and streaming 3D content, particularly in bandwidth-constrained networks and resource-limited devices, by leveraging the distinct roles of Gaussians in capturing high-frequency features (edges, contours) and low-frequency regions (smooth areas).

Method: The authors propose a hybrid representation categorizing Gaussians into Sketch Gaussians (high-frequency features) and Patch Gaussians (low-frequency regions). They introduce a hierarchical adaptive categorization framework using multi-criteria density-based clustering and adaptive quality-driven refinement, eliminating dependency on external 3D line primitives.

Result: The method achieves significant improvements: up to 1.74 dB in PSNR, 6.7% in SSIM, and 41.4% in LPIPS compared to uniform pruning baselines. For indoor scenes, it maintains visual quality with only 0.5% of the original model size.

Conclusion: The proposed structure-aware representation enables efficient storage, adaptive streaming, and high-fidelity 3D content rendering, making it suitable for constrained networks and devices.
...
TL;DR: The paper proposes a multi-task learning framework to fine-tune BiomedCLIP for improved CXR image-text retrieval, achieving better clinical relevance and diagnostic sensitivity.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models like CLIP and BiomedCLIP are not optimized for fine-grained medical retrieval tasks, such as retrieving clinically relevant radiology reports using CXR image queries.

Method: A multi-task learning framework incorporating a lightweight MLP projector head trained with a composite loss function (binary cross-entropy, supervised contrastive loss, and CLIP loss) is used to fine-tune BiomedCLIP.

Result: The fine-tuned model achieves more balanced and clinically meaningful performance in image-to-text and text-to-image retrieval tasks, with clearer semantic clustering of normal and abnormal cases.

Conclusion: Domain-adaptive, multi-task learning enhances cross-modal retrieval in biomedical applications, demonstrating improved diagnostic sensitivity.
...
TL;DR: The paper proposes a map-based reasoning approach for image geolocalization, using a two-stage optimization (RL + TTS), and demonstrates superior performance over existing models.


<details>
  <summary>Details</summary>
Motivation: Existing large vision-language models overlook the human strategy of using maps for geolocalization. This work aims to bridge this gap by equipping the model with map-based reasoning capabilities.

Method: The paper introduces a two-stage optimization scheme: agentic reinforcement learning (RL) followed by parallel test-time scaling (TTS). This approach enhances the model's sampling efficiency and enables exploration of multiple candidate paths before final prediction.

Result: The method achieves a notable improvement in accuracy, with Acc@500m increasing from 8.0% to 22.1% compared to Gemini-3-Pro with Google Search/Map grounded mode.

Conclusion: The proposed method, which integrates map-based reasoning and a two-stage optimization scheme, significantly outperforms existing models in image geolocalization, as demonstrated by the substantial improvement in Acc@500m.
...
TL;DR: TAPM-Net proposes a novel approach for infrared small target detection by modeling spatial diffusion behavior of target-induced feature disturbances, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current models lack mechanisms to trace directional perturbations in feature space, which is crucial for distinguishing targets from noise in infrared scenes.

Method: TAPM-Net introduces a Perturbation-guided Path Module (PGM) and Trajectory-Aware State Block (TASB) to model feature trajectories and dynamic propagation.

Result: Experiments on NUAA-SIRST and IRSTD-1K show TAPM-Net achieves state-of-the-art performance in infrared small target detection.

Conclusion: TAPM-Net effectively addresses the limitations of existing models by incorporating directional perturbation analysis, leading to superior detection performance.
...
TL;DR: ROAP is a pipeline that optimizes attention in Layout Transformers by modeling reading order and reducing visual noise, improving performance on document understanding tasks without altering pre-trained backbones.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of Multimodal Transformers in visually-rich document understanding (VrDU), specifically the lack of explicit modeling for logical reading order and the interference of visual tokens that dilutes attention on textual semantics.

Method: The paper introduces ROAP, a lightweight and architecture-agnostic pipeline that optimizes attention distributions in Layout Transformers without altering their pre-trained backbones. It uses Adaptive-XY-Gap (AXG-Tree) to extract hierarchical reading sequences and integrates them via Reading-Order-Aware Relative Position Bias (RO-RPB). It also introduces Textual-Token Sub-block Attention Prior (TT-Prior) to suppress visual noise and enhance text-text interactions.

Result: Extensive experiments on FUNSD and CORD benchmarks show that ROAP consistently improves the performance of representative backbones like LayoutLMv3 and GeoLayoutLM.

Conclusion: The paper concludes that explicitly modeling reading logic and regulating modality interference are critical for robust document understanding, offering a scalable solution for complex layout analysis.
...
TL;DR: A novel MISR framework improves underground root imaging, enabling better phenotypic analysis for agricultural research.


<details>
  <summary>Details</summary>
Motivation: Understanding plant root systems is critical for advancing research in soil-plant interactions, nutrient uptake, and overall plant health. However, accurate imaging of roots in subterranean environments remains a persistent challenge due to adverse conditions such as occlusion, varying soil moisture, and inherently low contrast, which limit the effectiveness of conventional vision-based approaches.

Method: In this work, we propose a novel underground imaging system that captures multiple overlapping views of plant roots and integrates a deep learning-based Multi-Image Super Resolution (MISR) framework designed to enhance root visibility and detail. To train and evaluate our approach, we construct a synthetic dataset that simulates realistic underground imaging scenarios, incorporating key environmental factors that affect image quality.

Result: Our proposed MISR algorithm leverages spatial redundancy across views to reconstruct high-resolution images with improved structural fidelity and visual clarity. Quantitative evaluations show that our approach outperforms state-of-the-art super resolution baselines, achieving a 2.3 percent reduction in BRISQUE, indicating improved image quality with the same CLIP-IQA score, thereby enabling enhanced phenotypic analysis of root systems.

Conclusion: The proposed framework presents a promising direction for robust automatic underground plant root imaging and trait quantification for agricultural and ecological research.
...
TL;DR: Study on gray matter loss in Alzheimer's disease (AD) and mild cognitive impairment (MCI) using MRI scans, focusing on hippocampal atrophy and its predictive value.


<details>
  <summary>Details</summary>
Motivation: To investigate the association between gray matter loss, particularly in medial temporal structures, and AD/MCI progression.

Method: Applied CAT12/SPM12 voxel-based morphometry to baseline T1-weighted MRI scans from 249 ADNI participants. Analyzed gray matter volume using a general linear model.

Result: Significant hippocampal atrophy in AD compared to CN and MCI. Hippocampal volume showed moderate predictive value for MCI-to-AD conversion (AUC = 0.66). No significant genetic effects from APOE4 status.

Conclusion: Medial temporal degeneration is a key feature of AD progression, with hippocampal atrophy as a predictive biomarker. Genetic influences were not significant in this study.
...
TL;DR: MMViR introduces a multi-modal, multi-grained structured representation for long video understanding, improving performance by 19.67% and reducing latency to 45.4%.


<details>
  <summary>Details</summary>
Motivation: Long videos present challenges for MLLMs due to complex events, diverse scenes, and long-range dependencies. Direct encoding is computationally expensive, and simple video-to-text conversion leads to redundancy or fragmentation.

Method: MMViR segments videos by identifying key turning points and constructs a three-level description combining global narratives with fine-grained visual details.

Result: MMViR outperforms prior methods, achieving a 19.67% improvement in understanding hour-long videos and reducing processing latency to 45.4%.

Conclusion: MMViR effectively addresses the limitations of current methods for long video understanding, offering a scalable and efficient solution.
...
TL;DR: A multi-task deep learning framework using SAM embeddings improves breast ultrasound lesion segmentation and classification, achieving high accuracy and DSC scores on the PRECISE dataset.


<details>
  <summary>Details</summary>
Motivation: The challenges in accurate tumor segmentation and classification in breast ultrasound (BUS) imaging, such as low contrast, speckle noise, and diverse lesion morphology, motivate the development of this method.

Method: A multi-task deep learning framework is presented, which jointly performs lesion segmentation and diagnostic classification using embeddings from the Segment Anything Model (SAM) vision encoder. The approach employs a prompt-free, fully supervised adaptation with a lightweight convolutional head or a UNet-inspired decoder for segmentation and mask-guided attention for classification.

Result: The proposed method achieves a Dice Similarity Coefficient (DSC) of 0.887 and an accuracy of 92.3% on the PRECISE 2025 breast ultrasound dataset.

Conclusion: The study demonstrates that SAM-based representations combined with segmentation-guided learning significantly enhance lesion delineation and diagnostic prediction in breast ultrasound imaging, achieving top performance on the PRECISE challenge leaderboard.
...
TL;DR: HieroSA is a framework enabling MLLMs to derive stroke-level structures from character bitmaps, bypassing language-specific priors and enhancing graphematics analysis.


<details>
  <summary>Details</summary>
Motivation: Current LLMs and MLLMs fail to model the underlying logic of character strokes, and existing structural analysis methods are script-specific and labor-intensive.

Method: Proposes Hieroglyphic Stroke Analyzer (HieroSA), a novel framework that transforms character images into interpretable line-segment representations in a normalized coordinate space.

Result: Extensive experiments show HieroSA effectively captures character-internal structures and semantics, enabling cross-lingual generalization.

Conclusion: HieroSA demonstrates potential as a graphematics analysis tool for deeper understanding of hieroglyphic scripts by capturing character-internal structures and semantics without language-specific priors.
...
TL;DR: GaussianSwap is a novel video face swapping framework using 3D Gaussian Splatting for high-fidelity avatars, enabling dynamic control and interactive applications.


<details>
  <summary>Details</summary>
Motivation: Conventional pixel-based video face swapping lacks animation and interactive capabilities. GaussianSwap addresses this by creating structured avatars.

Method: The framework preprocesses target videos for FLAME parameters, camera poses, and masks, rigs 3D Gaussian splats to FLAME, and uses a compound identity embedding for finetuning.

Result: GaussianSwap achieves superior identity preservation, visual clarity, and temporal consistency, enabling interactive applications.

Conclusion: GaussianSwap represents a paradigm shift from pixel-based to structured avatar-based face swapping, offering enhanced control and interactivity.
...
TL;DR: SAS-VPReID proposes a scale-adaptive framework with shape priors for VPReID, integrating MEVB, MGTM, and PRSD modules to address extreme far-distance challenges, achieving top performance on VReID-XFD.


<details>
  <summary>Details</summary>
Motivation: VPReID is challenging at extreme far distances due to resolution degradation, viewpoint variation, and appearance noise. The paper aims to overcome these issues.

Method: The framework consists of three modules: Memory-Enhanced Visual Backbone (MEVB) for feature extraction, Multi-Granularity Temporal Modeling (MGTM) for motion cue emphasis, and Prior-Regularized Shape Dynamics (PRSD) for body structure dynamics.

Result: The framework ranks first on the VReID-XFD challenge leaderboard, demonstrating the effectiveness of each module.

Conclusion: The proposed SAS-VPReID framework effectively addresses the challenges of VPReID at extreme far distances by integrating three complementary modules (MEVB, MGTM, PRSD), achieving state-of-the-art performance on the VReID-XFD benchmark.
...
TL;DR: DIFF-MF is a novel multi-modal image fusion method that uses feature discrepancy maps and state space modeling to balance infrared and visible details, achieving better results than current methods.


<details>
  <summary>Details</summary>
Motivation: Existing state space model-based approaches either prioritize infrared intensity at the cost of visible details or preserve visible structure while diminishing thermal target salience, necessitating a more balanced and effective fusion method.

Method: DIFF-MF uses a difference-driven channel-spatial state space model, incorporating a channel-exchange module for adaptive feature reweighting and a spatial-exchange module for comprehensive spatial fusion.

Result: Experimental results on driving scenarios and low-altitude UAV datasets show that DIFF-MF outperforms existing approaches in both visual quality and quantitative evaluation.

Conclusion: DIFF-MF effectively integrates complementary multi-modal features by leveraging feature discrepancy maps and achieves superior performance in visual quality and quantitative evaluation compared to existing methods.
...
TL;DR: MoGen is a user-friendly multi-object image generation method that uses RSA and AMG modules to achieve precise semantic alignment and flexible control, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing multi-object image generation methods struggle with precise alignment between localized image regions and their corresponding semantics, leading to inconsistent object quantities and attribute aliasing. Dependency on external control signals makes input formats rigid and incompatible with diverse user requirements.

Method: MoGen introduces a Regional Semantic Anchor (RSA) module to anchor phrase units in language descriptions to image regions and an Adaptive Multi-modal Guidance (AMG) module to integrate multi-source control signals for dynamic fine-grained control.

Result: MoGen achieves better generation quality, quantity consistency, and fine-grained control compared to existing methods, as demonstrated by experimental results.

Conclusion: MoGen significantly outperforms existing methods in generation quality, quantity consistency, and fine-grained control, while offering superior accessibility and control flexibility.
...
TL;DR: VIB-Probe, a novel framework using Variational Information Bottleneck theory, detects and mitigates hallucinations in Vision-Language Models by analyzing internal attention heads.


<details>
  <summary>Details</summary>
Motivation: Existing hallucination detection methods overlook internal mechanisms, focusing instead on output logits or external tools.

Method: Proposes VIB-Probe to extract discriminative patterns from attention heads while filtering noise via the information bottleneck principle, and introduces an inference-time intervention strategy.

Result: VIB-Probe significantly outperforms existing baselines in hallucination detection and mitigation across diverse benchmarks.

Conclusion: The study successfully leverages internal attention mechanisms for more truthful text generation in VLMs, with code to be made publicly available.
...
TL;DR: UniADet is a simple, efficient, and effective framework for universal visual anomaly detection, decoupling tasks and features to surpass state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Current methods for universal visual anomaly detection struggle with complex prompt engineering, elaborate adaptation modules, and challenging training strategies, limiting their flexibility and generality.

Method: The paper proposes an embarrassingly simple method to decouple classification and segmentation tasks, as well as cross-level features, by learning independent weights for different tasks and hierarchical features.

Result: UniADet outperforms state-of-the-art zero-/few-shot methods by a large margin and even full-shot AD methods for the first time on 14 real-world AD benchmarks.

Conclusion: UniADet presents a highly simple, parameter-efficient, general, and effective framework for universal visual anomaly detection, surpassing state-of-the-art methods across 14 real-world benchmarks.
...
TL;DR: A semi-supervised facial expression recognition algorithm using DTA and SNL achieves state-of-the-art results by effectively leveraging labeled and unlabeled data.


<details>
  <summary>Details</summary>
Motivation: The high cost of acquiring labeled facial expression data necessitates the development of semi-supervised algorithms that can leverage both labeled and unlabeled data efficiently.

Method: The algorithm employs local attention enhancement and random dropout of feature maps during feature extraction, along with dynamic thresholding and selective negative learning strategies to utilize both labeled and unlabeled data effectively.

Result: The method achieves impressive results, outperforming fully supervised methods on the RAF-DB and AffectNet datasets even without using the entire dataset.

Conclusion: The proposed semi-supervised facial expression recognition algorithm, incorporating Dynamic Threshold Adjustment (DTA) and Selective Negative Learning (SNL), achieves state-of-the-art performance on RAF-DB and AffectNet datasets, surpassing fully supervised methods without using the entire dataset.
...
TL;DR: The paper introduces a method to detect and correct misleading social-media news previews by developing a benchmark and proposing OMGuard, which improves detection accuracy and content correction.


<details>
  <summary>Details</summary>
Motivation: Social-media news previews can mislead by omitting crucial context, a covert harm underexplored compared to explicit misinformation.

Method: A multi-stage pipeline to disentangle preview-based vs. context-based understanding, leading to the MM-Misleading benchmark. OMGuard integrates Interpretation-Aware Fine-Tuning and Rationale-Guided Misleading Content Correction.

Result: OMGuard improves an 8B model's detection accuracy to match a 235B LVLM and enhances end-to-end correction. Misleadingness often stems from local narrative shifts, requiring visual interventions in image-driven scenarios.

Conclusion: The study highlights the necessity of visual interventions and systematic approaches to address misleadingness in social-media news previews.
...
TL;DR: A scalable multi-image editing framework for Unified Multimodal Models (UMMs) with innovations in latent separators and sinusoidal index encoding, improving consistency and generalization.


<details>
  <summary>Details</summary>
Motivation: Addressing limitations of UMMs in maintaining visual consistency and disambiguating visual cues across multiple input images.

Method: Introduces learnable latent separators and sinusoidal index encoding to differentiate image identities and generalize to variable input counts.

Result: Clear improvements in semantic consistency, visual fidelity, and cross-image integration over prior baselines.

Conclusion: The proposed framework validates advantages in consistency and generalization ability for multi-image editing tasks.
...
TL;DR: Orient Anything V2 improves orientation and rotation understanding for objects with diverse symmetries, using generative 3D assets, efficient annotation, symmetry-aware fitting, and multi-frame architecture, achieving top performance on 11 benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enhance the capability of orientation estimation beyond the limitations of Orient Anything V1, which was restricted to objects with a single unique front face, by addressing diverse rotational symmetries and enabling direct relative rotation estimation.

Method: The paper introduces four key innovations: 1) Scalable 3D assets synthesized by generative models, 2) An efficient model-in-the-loop annotation system, 3) A symmetry-aware, periodic distribution fitting objective, and 4) A multi-frame architecture for predicting relative object rotations.

Result: Orient Anything V2 achieves state-of-the-art zero-shot performance on orientation estimation, 6DoF pose estimation, and object symmetry recognition across 11 benchmarks, demonstrating strong generalization.

Conclusion: Orient Anything V2 significantly advances the field of object orientation and rotation understanding by introducing innovations that handle diverse rotational symmetries and direct relative rotation estimation, achieving state-of-the-art performance across multiple benchmarks.
...
TL;DR: A novel three-stage continual learning framework for detecting evolving AI-generated images, achieving superior performance with 92.20% accuracy and +5.51% improvement over baselines.


<details>
  <summary>Details</summary>
Motivation: To combat the threat of AI-generated images to online information authenticity, given the limitations of current detection methods in generalizing to unseen models and adapting to rapid generative technique evolution.

Method: The framework includes: (1) parameter-efficient fine-tuning for a transferable offline detection model, (2) continual learning with data augmentation and K-FAC to mitigate overfitting and forgetting, and (3) linear interpolation based on Linear Mode Connectivity to capture commonalities across models.

Result: The framework surpasses leading baselines by +5.51% in mean average precision for offline detection and achieves 92.20% average accuracy in continual learning, outperforming state-of-the-art methods.

Conclusion: The proposed three-stage domain continual learning framework effectively addresses the challenge of detecting AI-generated images by continuously adapting to evolving generative models, achieving superior performance compared to existing methods.
...
TL;DR: GS-DMSR improves 3D dynamic scene reconstruction by adaptive gradient focusing and multi-scale manifold enhancement, achieving high frame rates and efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of balancing model convergence rate and rendering quality in high-precision modeling of scenes with complex dynamic motions.

Method: The GS-DMSR method involves adaptive gradient focusing by analyzing Gaussian attributes dynamically and applying differentiated optimization strategies. It also integrates a multi-scale manifold enhancement module combining implicit nonlinear decoder and explicit deformation field.

Result: The method achieves up to 96 FPS on synthetic datasets while reducing storage overhead and training time.

Conclusion: The GS-DMSR method effectively balances model convergence rate and rendering quality in 3D dynamic scene reconstruction, achieving high efficiency and performance.
...
TL;DR: A data-driven metric and adaptation method to quantify and enhance shape bias in CNNs, improving performance on shape-dominant datasets with minimal training.


<details>
  <summary>Details</summary>
Motivation: CNNs inherently favor texture over shape, which harms performance on shape-dominant datasets like illustrations and sketches. Existing shape-biased models lack a quantitative metric to identify datasets that would benefit from such modifications.

Method: The paper introduces a data-driven metric to quantify the shape-texture balance of a dataset using SSIM between images and their L0-smoothed counterparts. It also proposes a computationally efficient adaptation method that modifies max-pooling dilation to promote shape bias without retraining convolutional weights.

Result: Experimental results demonstrate consistent accuracy improvements on shape-dominant datasets, particularly in low-data regimes where full fine-tuning is impractical.

Conclusion: The proposed method effectively improves classification accuracy on shape-dominant datasets, especially in low-data scenarios, by promoting shape bias through modified max-pooling dilation while keeping convolutional weights frozen.
...
TL;DR: SceneAlign uses scene graphs and targeted perturbations to enhance the visual grounding and reasoning faithfulness of multimodal models, showing improved performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Multimodal large language models often fail in faithful reasoning in complex visual scenes due to issues like hallucinated entities and mis-grounded relations. Existing preference-based approaches are insufficient as they allow models to bypass visual grounding.

Method: The paper proposes SceneAlign, a framework that uses scene graphs to perform controllable structural interventions, creating hard negative rationales for contrastive learning via Direct Preference Optimization.

Result: SceneAlign consistently improves answer accuracy and reasoning faithfulness across seven visual reasoning benchmarks.

Conclusion: SceneAlign effectively improves the faithfulness and accuracy of multimodal large language models in visual reasoning by leveraging scene graphs and targeted structural interventions.
