...
TL;DR: This study evaluates TerraMind Geospatial Foundation Model for flood mapping using multimodal satellite data, finding that fine-tuning with FloodsNet dataset improves recall while maintaining accuracy compared to baseline models.


<details>
  <summary>Details</summary>
Motivation: Floods are major weather hazards, and while EO satellites provide critical mapping capabilities, operational accuracy depends on labeled datasets and model generalization. The performance of recent GFMs like TerraMind on diverse global flood events needs better understanding.

Method: Fine-tuned TerraMind GFM using FloodsNet dataset (85 global flood events with Sentinel-1 SAR and Sentinel-2 optical imagery). Tested four model configurations (base/large, frozen/unfrozen backbones) and compared against TerraMind Sen1Floods11 example and U-Net baseline.

Result: Base-unfrozen configuration provided best balance of accuracy/precision/recall at lower computational cost. Large unfrozen model achieved highest recall. FloodsNet-trained models outperformed Sen1Floods11 in recall with similar accuracy. U-Net had higher recall than GFMs but slightly lower accuracy/precision.

Conclusion: Integrating multimodal optical/SAR data and fine-tuning GFMs can enhance near-real-time flood mapping. This provides one of the first global-scale GFM evaluations for flood segmentation, highlighting potential and limitations for climate adaptation.
...
TL;DR: A context-enriched contrastive loss function improves learning efficiency and reduces distortion, outperforming 16 methods on 8 benchmarks and showing a 22.9% gain on BiasedMNIST.


<details>
  <summary>Details</summary>
Motivation: Contrastive learning often suffers from information distortion due to overreliance on identical-label samples and neglect of positive pairs from the same image. This paper aims to mitigate this issue.

Method: The paper introduces a context-enriched contrastive loss function with two convergence targets: one sensitive to label contrast for efficiency, and another to align augmented samples from the same source image.

Result: The method outperforms 16 state-of-the-art contrastive learning methods on 8 benchmark datasets, with notable improvements in generalization and convergence speed. It excels in systematic distortion tasks, achieving a 22.9% improvement on BiasedMNIST.

Conclusion: The proposed context-enriched contrastive loss function effectively addresses information distortion in contrastive learning, improving both generalization performance and convergence speed. It shows significant promise for equitable downstream training, as evidenced by a 22.9% improvement on the BiasedMNIST dataset.
...
TL;DR: Proposes a hierarchical evaluation framework to jointly test Text-to-Image (T2I) models and Vision Language Models (VLMs) by having VLMs identify 27 specific failure modes in images generated from challenging prompts.


<details>
  <summary>Details</summary>
Motivation: T2I models often fail to accurately capture specific attributes in prompts, and current VLM benchmarks haven't kept pace with the complexity of scenes that VLMs are used to annotate, necessitating better evaluation methods.

Method: Created a dataset with prompts and images from 5 T2I models (Flux, SD3 variants) and corresponding annotations from 3 VLMs (Molmo, InternVL3, Pixtral) using Llama3 to test if VLMs correctly identify failure modes in generated images.

Result: Analysis revealed systematic errors in attribute fidelity and object representation, showing that current metrics are insufficient to capture nuanced errors in T2I models.

Conclusion: Targeted benchmarks are crucial for advancing generative model reliability and interpretability, as they expose specific failure modes that standard evaluation methods miss.
...
TL;DR: A deep latent variable model (LLOST) predicts somatic mutation profiles from medical images, showing promise for early cancer diagnosis and treatment.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve cancer treatment efficacy by enabling early diagnosis and treatment through predicting somatic mutation profiles from medical images.

Method: The authors propose LLOST, a model with dual variational autoencoders and a shared latent space, using conditional normalizing flow priors to handle diverse distributions.

Result: The model demonstrates predictive performance for mutation counts and occurrence, identifying shared patterns between imaging and somatic mutation domains that reflect cancer type.

Conclusion: The paper concludes with suggestions for model improvements and future research directions, including expanding to other genetic domains.
...
TL;DR: SplatSuRe selectively applies super-resolution only to undersampled regions using camera pose and scene geometry information, achieving sharper and more consistent 3D Gaussian Splatting renders than uniform SR approaches.


<details>
  <summary>Details</summary>
Motivation: Existing super-resolution methods for 3D Gaussian Splatting uniformly apply SR across all images, causing multi-view inconsistencies and blurry renders. The authors observed that close-up low-resolution views may contain high-frequency information for regions captured in distant views.

Method: Proposed SplatSuRe method that uses camera pose relative to scene geometry to identify undersampled regions lacking high-frequency supervision, then selectively applies super-resolution content only to those specific areas rather than uniformly across all images.

Result: Across Tanks & Temples, Deep Blending and Mip-NeRF 360 datasets, SplatSuRe surpasses baselines in both fidelity and perceptual quality, with most significant gains in localized foreground regions where higher detail is desired.

Conclusion: Selective super-resolution based on camera geometry and scene information yields superior results compared to uniform SR application, providing sharper and more consistent high-resolution renders for 3D Gaussian Splatting.
...
TL;DR: RobustSurg improves surgical scene segmentation generalization across centers and modalities using style-content separation with instance normalization and feature covariance mapping, plus a restitution module to preserve task-relevant features. Achieves 23% improvement on unseen data.


<details>
  <summary>Details</summary>
Motivation: Current surgical scene segmentation methods fail to generalize to unseen distributions and modalities due to limited visual cues and diverse scenarios compared to natural scenes. Need for methods that handle domain gaps in surgical data.

Method: Proposes RobustSurg with style-content separation using instance normalization and feature covariance mapping for robust features. Includes restitution module in ResNet backbone to retain task-relevant features. Also provides new multiclass multicentre dataset.

Result: RobustSurg achieved nearly 23% improvement over baseline DeepLabv3+ and 10-32% improvement over SOTA on unseen HeiCholSeg dataset. Also 22% improvement over baseline and 11% improvement over SOTA on EndoUDA polyp dataset.

Conclusion: Style-content separation with restitution module effectively improves surgical scene segmentation generalization across centers and modalities, demonstrating significant performance gains on unseen data.
...
TL;DR: The paper introduces multifractal recalibration methods for deep learning, showing significant improvements in medical image segmentation tasks over existing channel-attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing multifractal methods in deep learning rely on heavy pooling or feature-space decimation, which constrain tasks like semantic segmentation. The authors aim to overcome these limitations.

Method: Two inductive priors: Monofractal and Multifractal Recalibration, which leverage relationships between probability mass of exponents and multifractal spectrum to form statistical descriptions of encoder embeddings, implemented as channel-attention functions in convolutional networks using a U-Net framework.

Result: Multifractal recalibration yields substantial gains over baseline with other channel-attention mechanisms. Validated on three medical imaging datasets: ISIC18 (dermoscopy), Kvasir-SEG (endoscopy), and BUSI (ultrasound).

Conclusion: Multifractal recalibration is effective for medical image segmentation. Empirical analysis reveals that excitation responses don't become increasingly specialized with encoder depth due to skip connections, and their effectiveness relates to global statistics of instance variability.
...
TL;DR: Unified-VQA is a novel video quality assessment framework that uses a Diagnostic Mixture-of-Experts approach to provide both global quality scores and interpretable artifact analysis across multiple video formats without retraining.


<details>
  <summary>Details</summary>
Motivation: Current VQA methods are monolithic, format-specific, and lack diagnostic interpretability - they can't explain why video quality is degraded or handle diverse perceptual domains effectively.

Method: Uses multiple perceptual experts for different domains with multi-proxy expert training and a diagnostic multi-task head that generates quality scores and artifact vectors using weakly-supervised learning.

Result: Outperforms 18 benchmark methods across 17 databases with diverse streaming artifacts in HD, UHD, HDR, and HFR formats, using static parameters without retraining.

Conclusion: Represents a significant advancement towards practical, actionable, and interpretable video quality assessment that works across multiple formats and provides diagnostic feedback.
...
TL;DR: AV-SpeakerBench is a new benchmark for evaluating fine-grained audiovisual reasoning in MLLMs, focusing on speaker-centric tasks. It shows Gemini models outperform open-source systems, with Gemini 2.5 Pro leading.


<details>
  <summary>Details</summary>
Motivation: Existing video benchmarks lack fine-grained reasoning about human speech, limiting insights into MLLMs' ability to align speech with visual context.

Method: AV-SpeakerBench introduces 3,212 multiple-choice questions with speaker-centered formulation, fusion-grounded question design, and expert-curated annotations.

Result: Gemini models, especially Gemini 2.5 Pro, outperform open-source systems like Qwen3-Omni-30B, primarily due to better audiovisual fusion.

Conclusion: AV-SpeakerBench provides a rigorous foundation for advancing fine-grained audiovisual reasoning in future multimodal systems.
...
TL;DR: The paper proposes a novel Visual LIF (VLIF) neuron and spiking modules for image deraining, achieving top performance with 13% energy use of existing SNN methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore biologically plausible and energy-efficient frameworks like SNNs in low-level vision tasks, specifically addressing the lack of spatial contextual understanding and frequency-domain saturation in traditional spiking neurons.

Method: The paper introduces the Visual LIF (VLIF) neuron and leverages it to develop the Spiking Decomposition and Enhancement Module and the lightweight Spiking Multi-scale Unit for hierarchical multi-scale representation learning.

Result: Extensive experiments on five benchmark deraining datasets show the approach significantly outperforms state-of-the-art SNN-based deraining methods while using only 13% of their energy consumption.

Conclusion: The study establishes a strong foundation for deploying Spiking Neural Networks (SNNs) in high-performance, energy-efficient low-level vision tasks, demonstrating superior performance with significantly reduced energy consumption.
...
TL;DR: SPF introduces a hierarchical flow matching approach for efficient, parallel climate emulation across multiple timescales, outperforming baselines and scaling well with ClimateSuite, the largest Earth system simulation dataset.


<details>
  <summary>Details</summary>
Motivation: Current generative models for climate emulation are slow and unstable under nonstationary forcings, limiting their practical use.

Method: Spatiotemporal Pyramid Flows (SPF) hierarchically model data across spatial and temporal scales, enabling direct sampling at any temporal level and conditioning on physical forcings.

Result: SPF outperforms baselines on ClimateBench at yearly and monthly timescales, with fast sampling and good generalization to held-out scenarios.

Conclusion: SPF and ClimateSuite provide a foundation for accurate, efficient, and probabilistic climate emulation across temporal scales and future scenarios.
...
TL;DR: Fine-tuning CogVideo for progressive image restoration tasks by generating restoration trajectories instead of natural videos, achieving improved perceptual metrics and temporal coherence.


<details>
  <summary>Details</summary>
Motivation: Text-to-video models have strong temporal generation capabilities but their potential for image restoration remains underexplored.

Method: Construct synthetic datasets for super-resolution, deblurring, and low-light enhancement, fine-tune CogVideo to generate restoration trajectories, and compare uniform vs scene-specific prompting strategies using LLaVA and ChatGPT.

Result: The fine-tuned model learns to associate temporal progression with restoration quality, improving PSNR, SSIM, and LPIPS metrics across frames, and generalizes to real-world scenarios without additional training.

Conclusion: CogVideo can be effectively repurposed for progressive visual restoration, demonstrating strong zero-shot robustness and interpretability through temporal restoration trajectories.
...
TL;DR: MORP-Synth is a two-stage synthetic augmentation framework that improves SAR oil spill segmentation transfer from Mediterranean to Peruvian conditions by generating realistic geometric variations and SAR-like textures.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for SAR oil spill segmentation fail to generalize across regions due to differences in sea-state, backscatter statistics, and slick morphology, especially along the Peruvian coast where labeled data is scarce.

Method: Two-stage framework: Stage A applies Morphological Region Perturbation for geometric variations, and Stage B uses a conditional generative INADE model to render SAR-like textures from edited masks.

Result: Models pretrained on Mediterranean data degraded from 67.8% to 51.8% mIoU on Peruvian data; MORP-Synth improved performance by up to +6 mIoU and boosted minority-class IoU (+10.8 oil, +14.6 look-alike).

Conclusion: MORP-Synth effectively addresses domain adaptation challenges in SAR oil spill segmentation, significantly improving model generalization from Mediterranean to Peruvian conditions with limited labeled data.
...
TL;DR: Pre-trained video diffusion models inherently learn motion representations useful for tracking, enabling a self-supervised tracker that improves performance in distinguishing visually similar objects without labeled data.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenge of distinguishing visually similar objects by their motion, especially when visual cues are ambiguous, and the limitations of contemporary self-supervised trackers in such scenarios.

Method: The method leverages the denoising process of pre-trained video diffusion models, which isolates motion in early stages, to develop a self-supervised tracker. This approach avoids the need for extensive labeled data.

Result: The proposed self-supervised tracker achieves up to a 6-point improvement over recent self-supervised approaches on benchmarks, particularly in tracking visually similar items. Visualizations confirm the robustness of the method.

Conclusion: The paper concludes that pre-trained video diffusion models inherently learn motion representations suitable for tracking, enabling significant performance improvements in distinguishing visually similar objects without task-specific training.
...
TL;DR: The paper introduces a higher-DOF and long-term alignment framework using Thin Plate Spline for consistent 3D predictions in temporal settings, addressing limitations of existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for aligning 3D predictions over time have limitations in assumption validity, local alignment scope, and robustness under noisy geometry.

Method: The proposed framework uses Thin Plate Spline with globally propagated control points for long-term alignment and includes a point-agnostic submap registration design for robustness.

Result: The method achieves more coherent geometry and lower trajectory errors across diverse datasets, backbone models, and camera setups.

Conclusion: The framework is robust, generalizable, and compatible with various 3D foundation models and camera configurations, with publicly available code.
...
TL;DR: Proposes MS-CAM, a visual explanation method that enhances CNN interpretability for SAR images by combining channel-wise and element-wise weights to highlight regions of interest.


<details>
  <summary>Details</summary>
Motivation: The complexity and opacity of CNNs hinder their reliability in SAR applications, limiting deployment despite their success. Improving interpretability is crucial for development and deployment in SAR.

Method: MS-CAM matches SAR images with feature maps and gradients from CNNs, using both channel-wise and element-wise weights to visualize decision basis.

Result: Experiments on a SAR target classification dataset show MS-CAM accurately highlights regions of interest and captures detailed target features, enhancing interpretability. Validated for weakly-supervised object localization.

Conclusion: MS-CAM effectively improves CNN interpretability in SAR tasks, with potential for weakly-supervised localization. Future work will address factors like pixel thresholds.
...
TL;DR: A study on unified multimodal models reveals understanding components are compressible, but generation components are sensitive. MoE Adaptation improves efficiency by activating only half the parameters.


<details>
  <summary>Details</summary>
Motivation: To understand and mitigate inefficiencies in unified multimodal models, particularly the trade-offs between model capacity and task-specific requirements.

Method: The study uses training-free pruning (depth pruning and width reduction) to analyze unified multimodal model components and proposes MoE Adaptation to address generation component sensitivity.

Result: The understanding component is highly compressible, while generation components are sensitive to compression. MoE Adaptation restores generation quality with sparse activation.

Conclusion: The adapted BAGEL model achieves performance comparable to the full model while activating only about half of its parameters, demonstrating the effectiveness of sparse activation through Mixture-of-Experts (MoE) Adaptation.
...
TL;DR: Proposes WSCF-MVCC, a weakly-supervised calibration-free multi-view crowd counting method that uses crowd counts instead of density maps for supervision and leverages self-supervised ranking loss with multi-scale priors.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view crowd counting methods require expensive camera calibrations and crowd annotations. Even calibration-free methods still need image-level crowd annotations for training single-view counting modules.

Method: Uses crowd count as direct supervision for single-view counting module, employs self-supervised ranking loss with multi-scale priors, and leverages semantic information for more accurate view matching.

Result: Outperforms state-of-the-art methods on three widely used multi-view counting datasets under weakly supervised settings.

Conclusion: The proposed method is more suitable for practical deployment compared to calibrated methods as it reduces annotation requirements while maintaining high performance.
...
TL;DR: VACoT introduces a framework that dynamically applies image augmentations during inference to improve VLMs' robustness, especially in OCR-related adversarial scenarios, without requiring expensive retraining.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with basic perception tasks that conventional models handle well due to limited visual data augmentation, and retraining on augmented data is costly with diminishing returns.

Method: VACoT dynamically invokes post-hoc image augmentations (like denoising) during inference, using a structured collection of general visual augmentations and efficient agentic reinforcement learning with a conditional reward scheme.

Result: Substantial improvement in robustness on challenging and out-of-distribution inputs, demonstrated through extensive experiments on 13 perception benchmarks and introduction of AdvOCR for adversarial scenarios.

Conclusion: VACoT effectively enhances VLM robustness through post-hoc visual augmentations, providing a cost-efficient alternative to retraining while maintaining concise reasoning in perception tasks.
...
TL;DR: Comparison of ResNet-50 and SqueezeNet models for tuberculosis detection from chest X-rays, showing SqueezeNet outperforms ResNet-50 with 89% accuracy vs 73%.


<details>
  <summary>Details</summary>
Motivation: Traditional TB diagnosis methods like sputum microscopy are inefficient, especially in resource-limited settings, prompting exploration of deep learning alternatives.

Method: Used a dataset of 4,200 chest X-rays from Kaggle, applied data preprocessing (splitting, augmentation, resizing), and compared pretrained ResNet-50 with SqueezeNet models.

Result: SqueezeNet achieved superior performance: 89% accuracy, 98% precision, 80% recall, 87% F1 score. ResNet-50 performed worse: 73% accuracy, 88% precision, 52% recall, 65% F1 score.

Conclusion: Machine learning shows strong potential for TB detection, with possible mobile integration for resource-limited areas, but continued development of faster and more accurate models is needed.
...
TL;DR: A novel map-free trajectory prediction algorithm using Mixture of Experts for frequency component selection and selective attention for temporal/spatial filtering, achieving superior performance on complex interactive scenarios.
...
Method: Proposes a map-free algorithm that processes trajectory prediction across temporal, spatial, and frequency domains using MoE for frequency component selection, multi-scale temporal feature integration, selective attention module for redundancy filtering, and multimodal decoder with patch-level and point-level losses.

Result: Experiments on Nuscences datasets demonstrate the superiority of the algorithm, validating its effectiveness in handling complex interactive scenarios.

Conclusion: The proposed method effectively addresses challenges in trajectory prediction by efficiently processing information across multiple domains, showing promising results for autonomous driving systems.
...
<details>
  <summary>Details</summary>
Motivation: Traditional fine-tuning is hindered by privacy concerns and security constraints that prevent access to model parameters, creating demand for input-level strategies that enhance generalization without weight modification.

Method: Uses style transfer to construct diverse style representations from source domain, then adaptively fuses style cues based on visual context to form dynamic prompts that harmonize image appearance without modifying model internals.
...
Conclusion: SAGE effectively bridges the gap between frozen model invariance and unseen domain diversity through a closed-loop design that aligns feature distributions across styles via visual prompts.
...
TL;DR: On-the-fly Feedback SfM is an explore-and-exploit framework for real-time UAV photogrammetry that integrates online reconstruction, quality assessment, and predictive path planning to provide actionable feedback during data acquisition.


<details>
  <summary>Details</summary>
Motivation: Existing real-time UAV photogrammetry methods lack quality evaluation of on-the-go 3D reconstruction and guided feedback for image acquisition, which is crucial for time-critical applications like disaster response and digital-twin maintenance.

Method: Built upon SfM on-the-fly, the framework includes three modules: (1) online incremental coarse-mesh generation from sparse point clouds, (2) online mesh quality assessment with actionable indicators, and (3) predictive path planning for trajectory refinement.

Result: The method achieves in-situ reconstruction and evaluation in near real time, significantly reducing coverage gaps and re-flight costs through actionable feedback during the exploration process.

Conclusion: The integration of data collection, processing, reconstruction, assessment, and online feedback enables a transition from traditional passive photogrammetry to intelligent, adaptive exploration workflows for UAV applications.
...
TL;DR: FDTA introduces a feature refinement framework to enhance object discriminativeness in end-to-end MOT, addressing low association accuracy by integrating spatial, temporal, and identity adapters.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end MOT methods suffer from low association accuracy due to high inter-object similarity in embeddings, which lack instance-level distinction across frames.

Method: FDTA uses Spatial Adapter (SA) for depth-aware spatial cues, Temporal Adapter (TA) for historical temporal dependencies, and Identity Adapter (IA) for instance-level separability via contrastive learning.

Result: FDTA achieves state-of-the-art performance on MOT benchmarks like DanceTrack, SportsMOT, and BFT.

Conclusion: FDTA's discriminative embedding enhancement strategy effectively improves MOT performance by refining object embeddings across spatial, temporal, and identity perspectives.
...
TL;DR: This paper presents a camera-guided radar labeling pipeline that generates accurate labels for 4D radar point clouds without human annotations, enabling reproducible research in radar semantic segmentation.


<details>
  <summary>Details</summary>
Motivation: Progress in radar semantic segmentation is limited by scarce open source datasets and labels. Existing datasets like RaDelft only provide LiDAR annotations without public code for radar label generation, hindering reproducibility.

Method: A camera-guided radar labeling pipeline that projects radar point clouds into camera-based semantic segmentation and applies spatial clustering to generate accurate radar labels.

Result: The method significantly enhances radar label accuracy and establishes a reproducible framework for training and evaluating labeled 4D radar data. The study also quantifies how different fog levels affect radar labeling performance.

Conclusion: The proposed framework provides an effective solution for generating radar labels without human annotation, enabling broader research in radar semantic segmentation and studying environmental effects like fog on radar performance.
...
TL;DR: Skywork-R1V4 is a 30B parameter multimodal agentic model that unifies image manipulation, web search, and interleaved reasoning, achieving state-of-the-art results without reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat image manipulation and web search as disjoint capabilities, rely on costly reinforcement learning, and lack planning grounded in real tool-execution traces.

Method: Skywork-R1V4 is trained via supervised fine-tuning on 30,000 high-quality trajectories and validated through stepwise consistency filtering.

Result: The model scores 66.1 on MMSearch and 67.2 on FVQA, surpassing Gemini 2.5 Flash on all 11 metrics, and exhibits long-horizon reasoning.

Conclusion: Sophisticated agentic multimodal intelligence can be achieved through supervised learning alone, without reinforcement learning.
...
TL;DR: Nav-$R^2$ improves object-goal navigation by modeling target-environment and environment-action relationships with structured reasoning and a memory system, achieving better performance on unseen objects.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for object-goal navigation in open-vocabulary settings suffer from opaque decision-making and low success rates in locating unseen objects.

Method: The framework models target-environment relationships and environment-action planning via structured Chain-of-Thought reasoning and a Similarity-Aware Memory (SA-Mem), which compresses video frames and fuses historical observations without additional parameters.

Result: Nav-$R^2$ outperforms previous methods, achieving higher success rates in localizing unseen objects and maintaining real-time inference at 2Hz.

Conclusion: Nav-$R^2$ achieves state-of-the-art performance in localizing unseen objects through a streamlined and efficient pipeline, avoiding overfitting to seen object categories while maintaining real-time inference.
...
TL;DR: WISE, a modular MAD framework with heterogeneous experts, boosts accuracy by 2-7% on multimodal tasks by integrating Solvers and Reflectors and a modified Dawid-Skene algorithm.


<details>
  <summary>Details</summary>
Motivation: To explore the efficacy of multi-agent debate (MAD) on multimodal problems, leveraging complementary strengths of large language models (LLMs).

Method: The study introduces WISE, a framework partitioning agents into Solvers and Reflectors, and uses a modified Dawid-Skene algorithm for solution aggregation.

Result: WISE improves accuracy by 2-7% over existing MAD setups on datasets like SMART-840, VisualPuzzles, EvoChart-QA, and SMART-840++.

Conclusion: WISE, a generalized and modular MAD framework, consistently improves accuracy by 2-7% over state-of-the-art MAD setups across diverse multimodal tasks and LLM configurations.
...
TL;DR: MitUNet is a hybrid neural network that improves wall segmentation for 3D reconstruction by combining a Mix-Transformer encoder with a U-Net decoder and Tversky loss optimization, achieving better boundary accuracy and thin structure detection than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing wall segmentation methods struggle with detecting thin structural components and produce irregular boundaries that lack the geometric precision needed for 3D reconstruction vectorization.

Method: Proposed MitUNet architecture with hierarchical Mix-Transformer encoder for global context, U-Net decoder with scSE attention blocks for boundary recovery, and Tversky loss optimization to balance precision and recall.

Result: Experiments on CubiCasa5k and proprietary datasets show MitUNet generates structurally correct masks with high boundary accuracy, outperforming standard single-task models.

Conclusion: MitUNet provides a robust tool for automated 3D reconstruction pipelines by ensuring precise wall segmentation with improved boundary quality and thin structure detection.
...
TL;DR: GuiDG improves domain generalization in vision-language models by training domain-specific experts and using cross-modal attention, outperforming universal fine-tuning methods.


<details>
  <summary>Details</summary>
Motivation: Current fine-tuning methods for vision-language models compromise generalization to unseen domains by focusing on universal models. The paper addresses this gap by theoretically analyzing generalization and proposing a more effective approach.

Method: GuiDG employs a two-step approach: (1) prompt tuning to obtain source domain experts, and (2) a Cross-Modal Attention module to guide vision encoder fine-tuning via adaptive expert integration.

Result: GuiDG demonstrates superior performance on standard domain generalization benchmarks and the newly constructed ImageNet-DG dataset, surpassing state-of-the-art methods.

Conclusion: The proposed GuiDG framework, which leverages domain experts and cross-modal attention, outperforms existing fine-tuning methods in domain generalization while maintaining efficiency.
