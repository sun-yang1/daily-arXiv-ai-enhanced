...
TL;DR: MAT-Agent is a multi-agent framework for dynamic multi-label image classification, outperforming existing methods with adaptive training strategies and robust performance across datasets.


<details>
  <summary>Details</summary>
Motivation: Conventional static methods struggle in dynamic settings, prompting the need for adaptive training strategies in multi-label image classification.

Method: MAT-Agent uses autonomous agents to dynamically tune training parameters (e.g., data augmentation, optimizers) via non-stationary multi-armed bandit algorithms, enhanced with dual-rate smoothing and mixed-precision training.

Result: Achieves superior mAP, OF1, and CF1 scores on Pascal VOC, COCO, and VG-256 datasets compared to benchmarks.

Conclusion: MAT-Agent provides a scalable, intelligent solution for adaptive deep learning, improving convergence and cross-domain generalization.
...
TL;DR: CoIDO is a dual-objective framework for efficient data selection in MLLMs, achieving near-full performance with 20% of data by jointly optimizing importance and diversity.


<details>
  <summary>Details</summary>
Motivation: The high computational cost and suboptimal data selection in existing methods for MLLM instruction tuning motivate the need for a more efficient and effective solution.

Method: CoIDO employs a lightweight plug-in scorer trained on a small random sample to learn the candidate set distribution, balancing importance and diversity using a homoscedastic uncertainty-based formulation.

Result: CoIDO achieved 98.2% of full-data fine-tuning performance on the LLaVA-1.5-7B model across ten downstream tasks using only 20% of the data.

Conclusion: CoIDO successfully addresses the computational and suboptimal selection issues in existing methods by jointly optimizing data importance and diversity, achieving near-full performance with a fraction of the data.
...
TL;DR: The paper proposes a Latent Diffusion Model for early visual Treatment Response Prediction (TRP) in glioblastoma patients, using pre-treatment MRI and tumor localization to generate post-treatment MRI, enhancing personalized medicine.


<details>
  <summary>Details</summary>
Motivation: Glioblastoma (GBM) has highly heterogeneous therapeutic responses, and early prediction of treatment response is crucial for personalized medicine. Current methods require at least two months for visual impact, highlighting the need for faster prediction.

Method: The authors introduce a Latent Diffusion Model with concatenation-based conditioning from pre-treatment MRI and tumor localization, along with classifier-free guidance using survival information to improve generation quality.

Result: The model was trained and tested on a dataset of 140 GBM patients, utilizing pre- and post-treatment MRI, tumor localization, and survival data.

Conclusion: The proposed model offers a promising approach for early visual TRP, potentially improving personalized treatment strategies for GBM patients.
...
TL;DR: A framework using image embeddings and vector similarity effectively detects AI-generated images, validated by experiments showing robustness against perturbations.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of generative AI and LLMs has made it challenging to authenticate digital content, necessitating reliable methods to verify the integrity and origin of images.

Method: The methodology involves using image embeddings and vector similarity, processing a diverse dataset through five benchmark embedding models to validate the hypothesis that AI-generated images cluster closer to each other.

Result: Experiments confirm the framework's robustness, showing that perturbations minimally impact embedding signatures, with perturbed images maintaining similarity to their originals.

Conclusion: The proposed embedding-based AI image detection framework effectively distinguishes AI-generated images from human-created ones, maintaining robustness even with perturbations. It offers a generalizable solution balancing accuracy and computational efficiency.
...
TL;DR: CMIS-Net, a novel network, standardizes backchannel features across individuals and scales, improving detection of agreement cues in conversations.


<details>
  <summary>Details</summary>
Motivation: Backchannel behaviors are influenced by individual differences across multiple scales, presenting a complex pattern recognition problem. Existing methods often overlook the complementary nature of multi-scale cues.

Method: A Cascaded Multi-Scale Individual Standardization Network (CMIS-Net) is introduced, which extracts individual-normalized backchannel features by removing person-specific neutral baselines. It operates at both frame and sequence levels and includes an implicit data augmentation module.

Result: CMIS-Net demonstrates superior performance in handling individual differences and data imbalance, outperforming existing methods.

Conclusion: The proposed CMIS-Net effectively addresses individual differences and data imbalance in backchannel agreement detection, achieving state-of-the-art performance.
...
TL;DR: An ultra-efficient post-training method for shortcutting large-scale pre-trained flow matching diffusion models into few-step samplers, using novel velocity field self-distillation.


<details>
  <summary>Details</summary>
Motivation: Existing shortcutting methods require retraining from scratch, which is costly. The goal is to enable aggressive shortcutting without step-size embedding.

Method: Leverages velocity field self-distillation, working on the velocity field instead of sample space, and learns rapidly in an online manner.

Result: Produces efficient few-step samplers (e.g., 3-step Flux in less than one A100 day) and enables few-shot distillation for large models.

Conclusion: The method offers a cost-effective way to enhance pre-trained models, achieving state-of-the-art performance with minimal additional training.
...
TL;DR: A hybrid computer vision and pseudo-IMU method improves underwater diver safety by detecting medical emergencies, overcoming traditional limitations.


<details>
  <summary>Details</summary>
Motivation: Traditional human activity recognition methods (image analysis or wearable IMUs) are ineffective underwater due to signal attenuation. The goal is to improve diver safety by detecting medical emergencies like cardiac arrest.

Method: The method involves generating high-fidelity motion data (pseudo-IMU) from 3D human joint keypoints using computer vision, bypassing issues with wireless signal attenuation. This data is then used to classify anomalous diver behavior.

Result: The system successfully identifies anomalous diver behavior, demonstrating its utility for robotic monitoring and diver safety in simulated distress scenarios.

Conclusion: The hybrid approach combining computer vision and pseudo-IMU data effectively addresses the limitations of traditional methods in underwater environments, enhancing diver safety through real-time anomaly detection.
...
TL;DR: InsideOut extends 3D Gaussian splatting (3DGS) to fuse RGB and X-ray imaging, improving visualization and testing in medical, cultural, and industrial fields.


<details>
  <summary>Details</summary>
Motivation: The fusion of RGB and X-ray imaging is valuable for medical diagnostics, cultural heritage restoration, and manufacturing, but challenges include disparate data representations and limited paired datasets.

Method: The approach involves collecting paired RGB and X-ray data, hierarchical fitting to align RGB and X-ray radiative Gaussian splats, and an X-ray reference loss for consistent internal structures.

Result: InsideOut effectively bridges RGB and X-ray modalities, enhancing visualization, simulation, and non-destructive testing.

Conclusion: This work extends 3DGS applicability, addressing key challenges in multi-modal data fusion for diverse applications.
...
TL;DR: MUSE is a training-free framework for zero-shot 2D object detection and segmentation, using multi-view templates and joint similarity metrics to achieve top performance in benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of zero-shot 2D object detection and segmentation without requiring additional training or fine-tuning.

Method: MUSE integrates class and patch embeddings, normalizes patch embeddings with generalized mean pooling (GeM), and uses a joint similarity metric combining absolute and relative scores. It refines similarity scores with an uncertainty-aware object prior.

Result: MUSE ranks first in the BOP Challenge 2025 across the Classic Core, H3, and Industrial tracks.

Conclusion: MUSE provides a powerful and generalizable framework for zero-shot 2D object detection and segmentation, achieving state-of-the-art performance without additional training or fine-tuning.
...
TL;DR: A GAN-based method generates realistic handwritten musical scores to address data scarcity in OMR, showing high visual fidelity.


<details>
  <summary>Details</summary>
Motivation: The scarcity of annotated data in Optical Music Recognition (OMR), especially for historical handwritten scores, limits progress. Synthetic data has proven useful in similar fields like Handwritten Text Recognition.

Method: Implemented a music symbol-level Generative Adversarial Network (GAN) and assembled outputs into full scores using Smashcima engraving software.

Result: Generated symbols exhibit high visual realism, advancing synthetic score generation.

Conclusion: The study demonstrates significant progress in creating realistic synthetic handwritten scores, aiding OMR training.
...
TL;DR: The paper highlights biases in gender classification datasets, introduces BalancedFace to address these biases, and demonstrates its effectiveness in reducing disparities.


<details>
  <summary>Details</summary>
Motivation: Gender classification systems often amplify demographic imbalances in training data, leading to biased outcomes.

Method: The authors audit five datasets, train classifiers on UTKFace and FairFace, and create BalancedFace by blending and supplementing images to fill demographic gaps.

Result: BalancedFace reduces bias significantly, with a 50% decrease in True Positive Rate gaps and a 63% improvement in Disparate Impact scores, while maintaining accuracy.

Conclusion: Data-centric interventions like BalancedFace are valuable for fair gender classification, and the dataset is a resource for future research.
...
TL;DR: A 3D WSSS method using geometric priors and iterative label refinement achieves top performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address the low quality of pseudo-labels and insufficient use of 3D geometric priors in 3D weakly supervised semantic segmentation (3D WSSS).

Method: The method integrates 3D geometric priors into a class-aware guidance mechanism, using Class-Aware Label Refinement and Geometry-Aware Label Refinement modules, along with a Self-Training-based Label Update strategy.

Result: The methodology generates high-fidelity pseudo labels, improving segmentation accuracy and label coverage.

Conclusion: The proposed method achieves state-of-the-art performance on ScanNet and S3DIS benchmarks, demonstrating strong generalization in unsupervised settings.
...
TL;DR: The study evaluates bias in MRI segmentation models, finding race-matched training improves some models (ANTs, UNesT), while nnU-Net is robust. Biased models replicate sex but not race effects from manual segmentations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the growing concern of unfairness in deep-learning-based medical image segmentation, particularly performance disparities linked to sensitive attributes like race and sex. The study aims to quantify these biases and evaluate their impact on segmentation accuracy and derived volumes.

Method: The study evaluates three segmentation models (UNesT, nnU-Net, CoTr) and an atlas-based method (ANTs) on MRI images of the nucleus accumbens (NAc). It uses a dataset with four demographic subgroups (black female, black male, white female, white male) and manually labeled gold-standard segmentations. Performance and fairness are assessed using segmentation accuracy and volume measurements, with linear mixed models analyzing demographic impacts.

Result: Training on race-matched data improves segmentation accuracy for ANTs and UNesT, while nnU-Net performs robustly regardless of demographic matching. Biased models replicate sex effects seen in manual segmentations but often miss race effects.

Conclusion: The study highlights the importance of addressing intrinsic bias in deep-learning-based segmentation models, particularly concerning race and sex. It reveals that some models (ANTs and UNesT) perform better with race-matched training data, while others (nnU-Net) are robust across demographics. Additionally, biased models can replicate sex effects observed in manual segmentations but often fail to capture race effects.
...
TL;DR: ManzaiSet is a multimodal dataset of Japanese manzai comedy viewer responses, addressing Western bias in affective computing. It identifies three viewer types, shows a positive viewing order effect, and enables culturally aware AI development.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the Western-centric bias in affective computing by providing a large-scale dataset of Japanese manzai comedy viewer responses.

Method: The dataset includes facial videos and audio from 241 participants watching 10 performances. Analyses used k-means clustering, individual-level analysis, and automated humor classification.

Result: Key findings include three distinct viewer types, a positive viewing order effect, and no type-wise differences in humor classification after FDR correction.

Conclusion: ManzaiSet supports culturally aware emotion AI and personalized entertainment systems for non-Western contexts.
...
TL;DR: ViBED-Net is a deep learning framework for video-based engagement detection, combining facial and scene context with LSTM/Transformer temporal modeling, achieving 73.43% accuracy.


<details>
  <summary>Details</summary>
Motivation: Engagement detection in online learning is crucial for enhancing student outcomes and personalized instruction.

Method: ViBED-Net uses a dual-stream architecture with EfficientNetV2 for spatial feature extraction and employs LSTM and Transformer encoders for temporal modeling. Targeted data augmentation is applied to improve performance on underrepresented classes.

Result: ViBED-Net with LSTM achieves 73.43% accuracy on the DAiSEE dataset, outperforming existing state-of-the-art methods.

Conclusion: ViBED-Net advances video-based affective computing by providing a scalable and high-performing solution for engagement detection, with potential applications in education, user experience research, and content personalization.
...
TL;DR: SAVANT is a structured framework for detecting driving anomalies using VLMs, outperforming baselines and enabling cost-effective deployment.


<details>
  <summary>Details</summary>
Motivation: Autonomous driving systems struggle with rare, out-of-distribution scenarios. Existing Vision Language Models (VLMs) are unreliable and costly, limiting deployment.

Method: SAVANT employs a structured reasoning framework with a two-phase pipeline: structured scene description extraction and multi-modal evaluation across four semantic layers (Street, Infrastructure, Movable Objects, Environment).

Result: SAVANT achieves 89.6% recall and 88.0% accuracy, with a fine-tuned open-source model (Qwen2.5VL) reaching 90.8% recall and 93.8% accuracy. It also labels 9,640 real-world images accurately.

Conclusion: SAVANT provides a practical and cost-effective solution for detecting semantic anomalies in autonomous driving, addressing data scarcity and enabling reliable semantic monitoring.
...
TL;DR: The study introduced TriggerNet for classifying red palm mite-infested plants using deep learning and machine learning models, achieving effective detection and improved dataset reliability with Snorkel.


<details>
  <summary>Details</summary>
Motivation: The red palm mite infestation causes significant economic losses and reduced productivity in palm cultivation, necessitating accurate and early identification for effective management.

Method: The study utilized TriggerNet, integrating Grad-CAM, RISE, FullGrad, and TCAV for visual explanations, alongside deep learning models (CNN, EfficientNet, MobileNet, ViT, ResNet50, InceptionV3) and machine learning classifiers (Random Forest, SVM, KNN). Snorkel was used for efficient labeling of disease classes.

Result: TriggerNet and the employed models effectively classified plants and detected infestations, with Snorkel improving dataset reliability by reducing manual annotation time.

Conclusion: The study successfully applied TriggerNet, an interpretable AI framework, to classify and detect red palm mite infestations, demonstrating its effectiveness in improving agricultural productivity through advanced deep learning and machine learning models.
...
TL;DR: HouseTour generates 3D camera trajectories and natural language summaries from images, improving over existing VLMs by integrating geometric reasoning and 3D Gaussian splatting.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models struggle with geometric reasoning, prompting the need for a spatially-aware method for 3D camera trajectory and natural language summary generation.

Method: The approach generates smooth video trajectories via a diffusion process constrained by known camera poses and integrates this into a vision-language model (VLM) for 3D-grounded descriptions. The final video is synthesized using 3D Gaussian splatting.

Result: Experiments show that incorporating 3D camera trajectories into text generation improves performance over independent task handling. A new joint metric is introduced for evaluation.

Conclusion: HouseTour enables automated, professional-quality video creation for real estate and touristic applications without specialized expertise or equipment.
...
TL;DR: Chimera is a personalized image generation model that combines parts from multiple source images using textual instructions, outperforming baselines in accuracy and quality.
...
Method: The model is trained using a custom diffusion prior model with part-conditional guidance, leveraging a dataset of 37k prompts synthesized from 464 unique (part, subject) pairs (semantic atoms).

Result: Chimera achieves superior performance in part alignment, compositional accuracy, and visual quality compared to other baselines.

Conclusion: Chimera outperforms other baselines by 14% in part alignment and compositional accuracy and 21% in visual quality, demonstrating its effectiveness in generating personalized images with precise part composition.
...
TL;DR: The paper explores using SEM imaging and machine learning (YOLO models) for microplastic detection, highlighting potential, limitations, and preprocessing importance, while noting challenges like limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Microplastics (MPs) are pervasive pollutants, but their microscopic size makes detection and classification difficult, especially in large-scale studies. Current methods like SEM and AFM are manual and inefficient for screening.

Method: The study combines SEM imaging with machine learning-based object detection (YOLO models), focusing on a filtration scenario with symmetric, repetitive image backgrounds.

Result: Findings show varying performance of YOLO models for MP detection and emphasize the need for optimized preprocessing.

Conclusion: Machine learning shows promise for MP detection, but challenges like limited expert-labeled data for training must be addressed for broader application.
...
TL;DR: APT optimizes ViTs by varying patch sizes, boosting speed by 40-50% without performance loss, and reducing training/inference time in dense tasks.


<details>
  <summary>Details</summary>
Motivation: ViTs use uniform patch sizes, leading to long input sequences for high-resolution images, which is inefficient. APT aims to optimize this by adapting patch sizes based on image content.

Method: APT uses multiple patch sizes within the same image, allocating larger patches to homogeneous areas and smaller ones to complex regions, reducing input token count.

Result: APT achieves 40-50% speedup in ViT inference/training, maintains performance, and reduces training/inference time by up to 30% in dense visual tasks.

Conclusion: APT significantly improves the efficiency of Vision Transformers (ViTs) by dynamically adjusting patch sizes, achieving faster training and inference without compromising performance.
...
TL;DR: 3D Gaussian Splatting (3DGS) transforms 3D reconstruction with efficient rendering but faces memory and lighting challenges; this tutorial explores its pipeline and applications.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges of 3D reconstruction from posed images, leveraging the advancements in 3DGS for efficient rendering and novel view synthesis.

Method: The tutorial provides an overview of the 3DGS pipeline, starting with its splatting formulation and exploring efforts to address its limitations.

Result: The result is a comprehensive understanding of 3DGS, its capabilities, and ongoing efforts to mitigate its drawbacks, such as high memory footprint and limited secondary-ray effects.

Conclusion: The tutorial concludes by highlighting the versatility of 3D Gaussian Splatting (3DGS) in applications like surface reconstruction, avatar modeling, animation, and content generation, despite its limitations in memory usage and lighting effects.
...
TL;DR: The paper proposes In-Context Distillation (ICD) to help small VLMs perform better in low-resource settings by collaborating with a teacher model at inference time, achieving significant performance gains with minimal annotations.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the impracticality of deploying large VLMs in low-resource settings and the high cost of fine-tuning small VLMs to match larger models' performance. The goal is to enable small VLMs to thrive under budget constraints.

Method: The method involves an online In-Context Distillation (ICD) approach where a small VLM collaborates with a stronger teacher model at inference time. It includes a cross-modal demonstration selection strategy, teacher test-time scaling to reduce noise, and student uncertainty conditioning to dynamically manage demonstrations.

Result: ICD boosts small VLMs' performance by up to 33% using scarce teacher annotations (as low as 4%) and competes with the teacher's zero-shot performance. It also shows the advantage of in-context learning over fine-tuning under constrained compute budgets.

Conclusion: The proposed In-Context Distillation (ICD) method significantly enhances the performance of small VLMs in low-resource settings, achieving up to a 33% improvement with minimal teacher annotations (as low as 4%). It competes with the teacher model's zero-shot performance, demonstrating its effectiveness.
...
TL;DR: The paper introduces SafeCoop, a defense pipeline for natural-language-based collaborative driving, addressing vulnerabilities and improving safety and security in V2X systems.


<details>
  <summary>Details</summary>
Motivation: Traditional V2X systems face challenges like high bandwidth demands and semantic loss, while natural language offers semantic richness and lower bandwidth but introduces new vulnerabilities.

Method: The authors develop a comprehensive taxonomy of attack strategies and introduce SafeCoop, an agentic defense pipeline integrating semantic firewall, language-perception consistency checks, and multi-source consensus.

Result: SafeCoop achieves a 69.15% driving score improvement under malicious attacks and up to 67.32% F1 score for malicious detection in CARLA simulation.

Conclusion: The study provides guidance for advancing research on safe, secure, and trustworthy language-driven collaboration in transportation systems, demonstrating the effectiveness of SafeCoop in mitigating risks.
...
TL;DR: World-in-World is a platform benchmarking generative world models (WMs) for embodied tasks, revealing that visual quality alone doesn't ensure success; controllability and data scaling are key.


<details>
  <summary>Details</summary>
Motivation: To address the gap in evaluating WMs for embodied utility, focusing on task success rather than just visual quality.

Method: Introduces World-in-World, a closed-loop platform with unified planning and standardized APIs, testing WMs in four environments.

Result: Finds visual quality alone insufficient for task success; controllability, data scaling, and inference-time compute are crucial.

Conclusion: WMs must prioritize controllability and practical utility over visual realism for embodied decision-making.
...
TL;DR: LunarStereo is introduced as the first open dataset of photorealistic stereo image pairs of the Moon, enabling improved 3D reconstruction and pose estimation for lunar surfaces.


<details>
  <summary>Details</summary>
Motivation: Existing stereo vision methods and deep learning models trained on human-scale datasets struggle with lunar conditions due to lack of texture, lighting variations, and atypical trajectories.

Method: LunarStereo is created using ray tracing based on high-resolution topography and reflectance models. The MASt3R model is fine-tuned on this dataset for lunar domain adaptation.

Result: Extensive experiments on synthetic and real lunar data show significant improvements over zero-shot baselines in 3D reconstruction and pose estimation.

Conclusion: The approach paves the way for robust cross-scale generalization in extraterrestrial environments.
...
TL;DR: VelocityNet uses head detection and optical flow to track person-specific velocities, clustering them into motion classes and scoring anomalies for real-time detection in crowded scenes.


<details>
  <summary>Details</summary>
Motivation: Existing approaches struggle with occlusions, dynamic motion patterns, and lack interpretable anomaly indicators, especially in varying crowd densities.

Method: The framework combines head detection and dense optical flow to extract person-specific velocities, followed by hierarchical clustering into semantic motion classes and a percentile-based anomaly scoring system.

Result: Experiments show the framework's effectiveness in real-time detection of anomalous motion patterns in densely crowded environments.

Conclusion: VelocityNet effectively detects diverse anomalous motion patterns in crowded scenes by leveraging person-specific velocities and a percentile-based anomaly scoring system.
...
TL;DR: The paper introduces RadDiagSeg-D dataset and RadDiagSeg-M model for joint diagnostic text and segmentation mask generation, addressing a key clinical limitation.


<details>
  <summary>Details</summary>
Motivation: Current medical vision-language models lack the ability to generate diagnostic text and pixel-level segmentation masks simultaneously, limiting their clinical utility.

Method: The authors introduce RadDiagSeg-D, a unified dataset for abnormality detection, diagnosis, and segmentation, and propose RadDiagSeg-M, a model capable of joint text and mask generation.

Result: RadDiagSeg-M demonstrates strong performance in multi-target text-and-mask generation, providing clinically useful outputs.

Conclusion: The paper establishes RadDiagSeg-M as a robust baseline for joint text-and-mask generation in medical vision-language models, addressing a critical limitation in clinical applications.
...
TL;DR: EMA-SAM enhances SAM-2 for stable tumour tracking in ultrasound videos, improving segmentation accuracy and reducing false positives while maintaining real-time performance.


<details>
  <summary>Details</summary>
Motivation: Accurate lesion segmentation in ultrasound videos is challenging due to low contrast, probe-induced motion, and heat-related artifacts. SAM-2's frame-independent design leads to unstable predictions and temporal drift.

Method: EMA-SAM extends SAM-2 by incorporating a confidence-weighted exponential moving average pointer into the memory bank, ensuring temporal coherence and rapid adaptation to clear evidence.

Result: EMA-SAM improves maxDice from 0.82 to 0.86 and maxIoU from 0.72 to 0.76, reduces false positives by 29%, and achieves consistent gains of 2--5 Dice points on external benchmarks. It maintains real-time throughput of ~30 FPS.

Conclusion: EMA-SAM is a robust and efficient framework for stable tumour tracking in interventional ultrasound, bridging the gap between foundation models and clinical demands.
...
TL;DR: VLSU is a framework to evaluate multimodal safety, revealing models' poor joint image-text reasoning and alignment gaps, with performance dropping significantly for combined inputs.


<details>
  <summary>Details</summary>
Motivation: Existing safety evaluations for multimodal models treat vision and language inputs separately, missing risks from joint interpretation. Current approaches also fail to distinguish clearly unsafe content from borderline cases, leading to problematic over-blocking or under-refusal.

Method: The study employs a multi-stage pipeline with real-world images and human annotation to construct a large-scale benchmark of 8,187 samples across 15 harm categories. It evaluates eleven state-of-the-art models using fine-grained severity classification and combinatorial analysis.

Result: Models achieve over 90% accuracy on unimodal safety signals but degrade to 20-55% when joint image-text reasoning is required. Notably, 34% of errors occur despite correct unimodal classification. Instruction framing can reduce over-blocking but at the cost of under-refusing unsafe content.

Conclusion: The VLSU framework highlights significant gaps in joint image-text understanding and alignment in current models, providing a test bed for future research on robust vision-language safety.
