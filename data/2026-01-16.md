...
TL;DR: An end-to-end pipeline for generating adversarial patches to evade facial recognition systems, using FGSM and diffusion models, with applications in forensic analysis and security testing.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a robust pipeline for creating adversarial patches that can compromise facial biometric systems, with applications in forensic analysis and security testing. The goal is to ensure these patches evade recognition systems while appearing natural visually.

Method: The method involves generating adversarial noise using FGSM, refining it with a diffusion model (reverse diffusion, Gaussian smoothing, and adaptive brightness correction), and applying the patch to facial images. A Vision Transformer (ViT)-GPT2 model is used for captioning to aid forensic interpretation. The pipeline evaluates identity classification, captioning results, and vulnerabilities in facial verification and expression recognition.

Result: The results show the pipeline's ability to generate adversarial patches that effectively evade recognition systems (SSIM of 0.95). The adversarial patches maintain natural visual characteristics and are successfully analyzed using perceptual hashing and segmentation.

Conclusion: The paper concludes by demonstrating the effectiveness of the proposed pipeline in generating adversarial patches that evade facial recognition systems while maintaining visual naturalness. It also highlights the utility of the pipeline in forensic analysis and security testing, with successful detection and analysis of adversarial patches using perceptual hashing and segmentation.
...
TL;DR: LCF3D is a sensor fusion framework combining RGB and LiDAR data for improved 3D object detection, showing significant gains over LiDAR-only methods and robust domain generalization.


<details>
  <summary>Details</summary>
Motivation: Accurate 3D object localization is crucial for Autonomous Driving. Combining RGB cameras and LiDAR sensors is common, but effectively fusing these data sources for improved detection remains a challenge.

Method: LCF3D combines a 2D object detector on RGB images with a 3D object detector on LiDAR point clouds using late fusion and cascade fusion principles. Late fusion reduces LiDAR false positives by matching detections, while cascade fusion recovers missed objects by generating new 3D frustum proposals.

Result: LCF3D outperforms LiDAR-based methods, especially for pedestrians and cyclists in the KITTI dataset and motorcycles and bicycles in nuScenes. It also handles domain generalization well.

Conclusion: LCF3D demonstrates significant improvements in 3D object detection by effectively combining LiDAR and RGB data, particularly for challenging categories like pedestrians and cyclists. It shows robustness in domain generalization across different sensor configurations.
...
TL;DR: EfficientNet-B0 outperforms DenseNet121 in pediatric pneumonia detection from chest X-rays, with higher accuracy and explainability, making it suitable for clinical use.


<details>
  <summary>Details</summary>
Motivation: Pneumonia remains a leading cause of morbidity and mortality among children worldwide, emphasizing the need for accurate and efficient diagnostic support tools. Deep learning has shown strong potential in medical image analysis, particularly for chest X-ray interpretation.

Method: A publicly available dataset of 5,863 pediatric chest X-ray images was used. Images were preprocessed through normalization, resizing, and data augmentation to enhance generalization. DenseNet121 and EfficientNet-B0 were fine-tuned using pretrained ImageNet weights under identical training settings. Performance was evaluated using accuracy, F1-score, Matthews Correlation Coefficient (MCC), and recall. Model explainability was incorporated using Gradient-weighted Class Activation Mapping (Grad-CAM) and Local Interpretable Model-agnostic Explanations (LIME) to visualize image regions influencing predictions.

Result: EfficientNet-B0 outperformed DenseNet121, achieving an accuracy of 84.6%, F1-score of 0.8899, and MCC of 0.6849. DenseNet121 achieved 79.7% accuracy, an F1-score of 0.8597, and MCC of 0.5852. Both models demonstrated high recall values above 0.99, indicating strong sensitivity to pneumonia detection. Grad-CAM and LIME visualizations showed consistent focus on clinically relevant lung regions, supporting the reliability of model decisions.

Conclusion: EfficientNet-B0 provided a more balanced and computationally efficient performance compared to DenseNet121, making it a strong candidate for clinical deployment. The integration of explainability techniques enhances transparency and trustworthiness in AI-assisted pediatric pneumonia diagnosis.
...
TL;DR: NanoSD is a lightweight, Pareto-optimal diffusion model family distilled from Stable Diffusion 1.5, enabling real-time image restoration and generation on edge devices with superior performance.


<details>
  <summary>Details</summary>
Motivation: Existing lightweight variants of latent diffusion models disrupt the latent manifold and limit generalization beyond a single task, making them unsuitable for edge devices.

Method: Distillation from Stable Diffusion 1.5 through network surgery, feature-wise generative distillation, and structured architectural scaling applied to the U-Net and VAE encoder-decoder.

Result: NanoSD achieves real-time inference down to 20ms on mobile-class NPUs, with models ranging from 130M to 315M parameters, and outperforms prior models in perceptual quality and deployability.

Conclusion: NanoSD establishes a general-purpose diffusion foundation model family suitable for real-time visual generation and restoration on edge devices, outperforming prior lightweight models.
...
TL;DR: UniHash unifies pointwise and pairwise paradigms for balanced image retrieval performance across seen and unseen categories, validated by state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: To address the limitation of existing deep hashing methods confined to either pointwise or pairwise paradigms, which excel in seen or unseen categories respectively, but not both.

Method: UniHash integrates a center-based branch (pointwise paradigm) and a pairwise branch, with a novel hash code learning method for bidirectional knowledge transfer. It uses a mutual learning loss and a Split-Merge Mixture of Hash Experts (SM-MoH) module.

Result: UniHash achieves balanced retrieval performance across seen and unseen categories, outperforming existing methods on CIFAR-10, MSCOCO, and ImageNet.

Conclusion: UniHash demonstrates state-of-the-art performance in both seen and unseen image retrieval scenarios, validated by theoretical analysis and extensive experiments on benchmark datasets.
...
TL;DR: Proposes ViSIL, a metric to quantify information loss in multimodal video summaries, outperforming traditional metrics and improving VQA accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional metrics like BLEU or ROUGE fail to quantify information coverage across modalities in video summaries.

Method: Introduces Video Summary Information Loss (ViSIL) score, an information-theoretic framework using vision-language models (VLMs).

Result: ViSIL shows significant correlation with human and VLM performance on VQA tasks, optimizing trade-offs between information loss and processing speed.

Conclusion: ViSIL is a unified metric for multimodal summaries, enhancing VQA accuracy without additional processing load.
...
TL;DR: TuneCLIP is a self-supervised fine-tuning framework that improves CLIP's performance across various tasks without retraining from scratch. It includes a warm-up stage and a fine-tuning stage with a new contrastive loss.


<details>
  <summary>Details</summary>
Motivation: To enhance the performance of open-weight CLIP models across multiple downstream tasks using existing self-supervised datasets, avoiding the costly process of training from scratch.

Method: TuneCLIP introduces a warm-up stage to reduce cold-start bias and a fine-tuning stage with a new contrastive loss to mitigate penalization on false negative pairs.

Result: TuneCLIP consistently improves performance, achieving gains of up to +2.5% on ImageNet and +1.2% on DataComp benchmark.

Conclusion: TuneCLIP sets a new strong baseline for efficient post-pretraining adaptation of CLIP models.
...
TL;DR: VibrantSR is a super-resolution framework using Sentinel-2 data to estimate canopy heights, offering consistent monitoring with better accuracy than other satellite-based methods.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of aerial imagery-based approaches, which are constrained by infrequent and irregular acquisition schedules, by using globally available Sentinel-2 data for consistent monitoring.

Method: VibrantSR is a generative super-resolution framework that estimates 0.5 meter canopy height models (CHMs) from 10 meter Sentinel-2 imagery, leveraging globally available Sentinel-2 seasonal composites.

Result: VibrantSR achieves a Mean Absolute Error of 4.39 meters for canopy heights >= 2 m, outperforming Meta (4.83 m), LANDFIRE (5.96 m), and ETH (7.05 m) satellite-based benchmarks.

Conclusion: VibrantSR enables operational forest monitoring and carbon accounting at continental scales without relying on costly and infrequent aerial acquisitions, making it a practical solution for consistent seasonal-to-annual monitoring.
...
TL;DR: MedVL-SAM2 is a unified 3D medical multimodal model that combines report generation, VQA, and multi-paradigm segmentation, achieving state-of-the-art performance by integrating image-level reasoning and pixel-level perception through a cohesive architecture.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of achieving fine-grained visual grounding and volumetric spatial reasoning in 3D medical vision-language models (VLMs) within a single, generalizable framework.

Method: The method involves a multi-stage training pipeline: pre-training on 3D CT image-text pairs for feature alignment, followed by joint optimization with language-understanding and segmentation objectives using a comprehensive 3D CT segmentation dataset. The model integrates a SAM2-based volumetric segmentation module.

Result: MedVL-SAM2 achieves state-of-the-art performance in report generation, VQA, and multiple 3D segmentation tasks, providing reliable 3D visual grounding, controllable interactive segmentation, and robust cross-modal reasoning.

Conclusion: The paper concludes that MedVL-SAM2 successfully unifies high-level semantic reasoning and precise 3D localization within a single framework, demonstrating state-of-the-art performance across multiple tasks.
...
TL;DR: TMD distills video diffusion models into efficient few-step generators by matching denoising trajectories with lightweight conditional flows, improving speed without sacrificing quality.


<details>
  <summary>Details</summary>
Motivation: To address the inefficiency of multi-step sampling in large video diffusion models for real-time interactive applications.

Method: Transition Matching Distillation (TMD) decomposes the original diffusion backbone into a main backbone for semantic representation extraction and a flow head for conditional flow updates, enabling efficient distillation.

Result: TMD demonstrates superior performance in distilling large text-to-video models (e.g., Wan2.1 1.3B and 14B), achieving a balance between speed and quality.

Conclusion: TMD provides a flexible and strong trade-off between generation speed and visual quality, outperforming existing distilled models under comparable inference costs in terms of visual fidelity and prompt adherence.
...
TL;DR: OT-Drive is a novel Optimal Transport-driven multi-modal fusion framework for robust traversable area segmentation in autonomous driving, outperforming existing methods in OOD scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing data-driven approaches suffer from degraded segmentation performance in out-of-distribution (OOD) scenarios, impairing downstream driving tasks.

Method: Proposes OT-Drive, which formulates RGB and surface normal fusion as a distribution transport problem, using a Scene Anchor Generator (SAG) and Optimal Transport-based fusion module (OT Fusion).

Result: Achieves 95.16% mIoU on ORFD OOD scenarios (6.35% improvement) and 89.79% mIoU on cross-dataset transfer tasks (13.99% improvement).

Conclusion: OT-Drive demonstrates strong OOD generalization with limited training data, enhancing practicality for real-world deployment.
...
TL;DR: This paper highlights the lack of spatial awareness in current vision-language models (VLMs) and proposes using alternative image encoders and 2D positional encodings to improve spatial reasoning, showing positive results on benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the lack of spatial awareness in current VLMs, which is a bottleneck for applications requiring spatial grounding, such as robotics and embodied AI. The paper argues that flattening images into 1D patch sequences discards the necessary 2D structure.

Method: The study investigates two architectural choices: (i) image encoders trained with alternative objectives and (ii) 2D positional encodings. These methods aim to preserve the 2D structure of images, which is essential for spatial reasoning.

Result: The experiments demonstrate that the proposed architectural choices lead to improved spatial reasoning performance on several benchmarks.

Conclusion: The paper concludes that incorporating spatial awareness into vision-language models (VLMs) through alternative image encoders and 2D positional encodings can enhance their spatial reasoning capabilities, addressing a critical limitation in current VLM designs.
...
TL;DR: DR$^2$Seg is a self-rewarding framework that enhances reasoning efficiency and segmentation accuracy in vision-language tasks by decomposing reasoning segmentation into multimodal reasoning and referring segmentation, using a two-stage rollout strategy and self-rewards.


<details>
  <summary>Details</summary>
Motivation: Existing methods for reasoning segmentation suffer from overthinking, generating verbose reasoning chains that interfere with object localization in multimodal large language models (MLLMs).

Method: DR$^2$Seg employs a two-stage rollout strategy: first, generating a self-contained description of the target object, and second, using this description to verify self-containment. Two self-rewards are introduced to strengthen goal-oriented reasoning and suppress redundant thinking.

Result: Extensive experiments show that DR$^2$Seg consistently improves reasoning efficiency and segmentation performance across various MLLMs and segmentation models.

Conclusion: DR$^2$Seg effectively addresses the issue of overthinking in reasoning segmentation, offering a robust solution for improving both reasoning efficiency and segmentation accuracy without additional supervision.
...
TL;DR: DW-DGAT, a novel model integrating data fusion and dual graph attention, outperforms existing methods in early diagnosis of Parkinson's and Alzheimer's diseases.
...
Method: The model employs a dynamically weighted dual graph attention network (DW-DGAT) with a data fusion strategy, dual graph attention architecture for feature extraction, and a class weight generation mechanism with loss functions.

Result: The DW-DGAT model achieves state-of-the-art performance in diagnosing these diseases, as validated by the PPMI and ADNI studies.
...
TL;DR: The paper introduces VERHallu, a benchmark for evaluating event relation hallucination in VideoLLMs, and proposes the KFP strategy to improve multi-event understanding, showing its effectiveness in mitigating hallucinations without slowing inference.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the overlooked issue of event relation hallucination in VideoLLMs, which involves causal, temporal, and subevent relations between events. Existing research has focused on hallucinations involving objects and scenes but not event relations.

Method: The paper introduces the VERHallu benchmark, which evaluates event relation hallucination through tasks like relation classification, question answering, and counterfactual question answering. It also proposes the KFP strategy to enhance multi-event understanding by reallocating frame-level attention.

Result: The results show that current VideoLLMs perform poorly in dense-event relation reasoning due to reliance on prior knowledge and insufficient use of frame-level cues. The proposed KFP strategy effectively mitigates event relation hallucination without affecting inference speed.

Conclusion: The paper concludes that current VideoLLMs struggle with event relation hallucination, particularly in dense-event reasoning, and proposes the Key-Frame Propagating (KFP) strategy to mitigate this issue effectively without compromising inference speed.
...
TL;DR: The paper proposes DiCo, a framework for Text-to-image person re-identification (TIReID) that uses disentangled concept representation for better cross-modal alignment and fine-grained retrieval.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of TIReID due to the modality gap between visual and textual data and the need for fine-grained correspondence.

Method: Introduces DiCo with shared slot-based representation decomposed into concept blocks for disentangling attributes and maintaining part-level alignment.

Result: DiCo achieves competitive performance on CUHK-PEDES, ICFG-PEDES, and RSTPReid datasets.

Conclusion: DiCo enhances interpretability and retrieval performance through hierarchical and disentangled cross-modal alignment.
...
TL;DR: The paper introduces a synthetic underwater benchmark dataset for event-based optical flow, addressing challenges in underwater imaging.


<details>
  <summary>Details</summary>
Motivation: Underwater imaging faces issues like light attenuation, scattering, and turbidity, making ground-truth motion hard to obtain. Event cameras offer potential but lack suitable datasets.

Method: The authors create a synthetic dataset using ray-traced RGBD sequences and a video-to-event pipeline, providing dense ground-truth flow, depth, and camera motion.

Result: The dataset benchmarks state-of-the-art optical flow methods, revealing how underwater conditions affect event formation and motion estimation.

Conclusion: This work sets a baseline for future underwater event-based perception algorithms, with publicly available code and dataset.
...
TL;DR: CoF-T2I integrates Chain-of-Frame reasoning into text-to-image generation, improving quality and performance.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap in applying video models' CoF reasoning to T2I generation due to lack of defined starting points and interpretable states.

Method: Proposes CoF-T2I with progressive visual refinement, using intermediate frames as reasoning steps. Curates CoF-Evol-Instruct dataset and enables independent frame encoding.

Result: Outperforms base video model, achieving 0.86 on GenEval and 7.468 on Imagine-Bench.

Conclusion: Demonstrates promise of video models for advancing high-quality T2I generation.
...
TL;DR: ReaMIL is a multiple instance learning approach for histopathology that improves evidence efficiency without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: To enhance whole-slide histopathology analysis by providing small, spatially compact evidence sets.

Method: Adds a light selection head to a MIL backbone, trained with a budgeted-sufficiency objective.

Result: Matches or slightly improves baseline AUC, with quantitative evidence-efficiency diagnostics.

Conclusion: ReaMIL effectively balances performance and evidence efficiency, requiring no extra supervision.
...
TL;DR: A flow-guided geometric advection framework for 3D Gaussian Splatting transfers 2D artistic motion into 3D geometry, enabling expressive structural deformation aligned with Post-Impressionist principles.


<details>
  <summary>Details</summary>
Motivation: The motivation is to authentically reproduce Post-Impressionist stylization by embracing geometric abstraction as the primary vehicle of expression, addressing the limitation of existing 3D style transfer methods that treat geometry as a rigid substrate for surface-level texture projection.

Method: The method involves a flow-guided geometric advection framework for 3D Gaussian Splatting (3DGS), extracting directional flow fields from 2D paintings and back-propagating them into 3D space to form flow-aligned brushstrokes. It includes a projection-based, mesh-free flow guidance mechanism, a luminance-structure decoupling strategy, and a VLM-as-a-Judge evaluation framework.

Result: The results demonstrate expressive structural deformation driven directly by painterly motion, with contributions including a novel flow guidance mechanism, a decoupling strategy for geometric deformation, and an evaluation framework based on aesthetic judgment.

Conclusion: The paper concludes that their proposed flow-guided geometric advection framework successfully operationalizes the Post-Impressionist principle of geometric abstraction in 3D style transfer, achieving expressive structural deformation driven by painterly motion.
...
TL;DR: Proposes difficulty-guided sampling (DGS) and difficulty-aware guidance (DAG) to bridge the target gap in dataset distillation, improving performance and demonstrating broader potential for downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks have time and storage-consuming training processes. Dataset distillation aims to generate compact, high-quality datasets for efficient model training while maintaining downstream performance. Existing approaches overlook task-specific information, leading to a target gap between distillation objectives and downstream tasks.

Method: The paper proposes difficulty-guided sampling (DGS) as a plug-in post-stage sampling module and difficulty-aware guidance (DAG) to explore difficulty's effect in the generation process. The final distilled dataset is sampled from image pools generated by existing methods, following a specific target difficulty distribution.

Result: Extensive experiments demonstrate the effectiveness of DGS and DAG in improving dataset distillation performance. The methods successfully bridge the target gap and show broader potential for diverse downstream tasks.

Conclusion: The paper concludes that difficulty-guided sampling (DGS) and difficulty-aware guidance (DAG) effectively bridge the target gap between distillation objectives and downstream tasks, improving dataset distillation performance. It also highlights the broader potential of difficulty for diverse downstream tasks.
...
TL;DR: V-Zero is a self-improvement framework for VLMs using unlabeled data, with a Questioner and Solver trained via GRPO, achieving performance gains without human annotations.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitation of costly and time-consuming large-scale human-annotated datasets in enhancing the reasoning capabilities of vision-language models (VLMs).

Method: V-Zero introduces a co-evolutionary loop with a Questioner and a Solver, trained iteratively via Group Relative Policy Optimization (GRPO). The Questioner synthesizes questions using a dual-track reasoning reward, while the Solver is optimized using pseudo-labels from majority voting.

Result: V-Zero achieves performance gains of +1.7 on visual mathematical reasoning and +2.6 on general vision-centric tasks without human annotations.

Conclusion: V-Zero demonstrates the potential of self-improvement in multimodal systems without relying on human-annotated datasets, achieving consistent performance gains.
...
TL;DR: InfoSculpt introduces a novel information-theoretic framework for Generalized Category Discovery (GCD), using dual Conditional Mutual Information objectives to disentangle category signals from noise, achieving superior performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing GCD methods, which rely on pseudo-labeling or two-stage clustering and lack a principled mechanism to disentangle essential category signals from noise. The goal is to develop a more robust and principled approach.

Method: The method involves re-framing GCD using the Information Bottleneck principle and introducing InfoSculpt, a framework that minimizes a dual Conditional Mutual Information (CMI) objective. This includes Category-Level CMI for labeled data and Instance-Level CMI for all data to distill invariant features.

Result: Extensive experiments on 8 benchmarks demonstrate that InfoSculpt outperforms existing methods, validating the effectiveness of the information-theoretic approach.

Conclusion: The paper concludes that InfoSculpt, by leveraging the Information Bottleneck principle and dual Conditional Mutual Information objectives, effectively disentangles category-defining signals from noise, leading to superior performance in Generalized Category Discovery (GCD).
...
TL;DR: FlowAct-R1 is a real-time interactive humanoid video generation framework that balances high-fidelity synthesis with low-latency responsiveness, achieving 25fps at 480p resolution with a TTFF of ~1.5s.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between high-fidelity synthesis and real-time interaction in interactive humanoid video generation.

Method: Proposes FlowAct-R1, built on MMDiT architecture, using chunkwise diffusion forcing and self-forcing for temporal consistency.

Result: Achieves stable 25fps at 480p resolution with ~1.5s TTFF, demonstrating vividness and realism.

Conclusion: FlowAct-R1 effectively enables real-time, high-quality interactive humanoid video generation with robust generalization.
...
TL;DR: MathDoc is introduced as the first benchmark for document-level information extraction from noisy math exam papers, revealing that current MLLMs struggle with refusing illegible inputs despite good extraction performance.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of extracting structured questions from noisy paper-based mathematics exams, which existing benchmarks overlook by focusing on clean documents or generic layout analysis. The study aims to evaluate both structural integrity and the ability of models to reject incomplete inputs.

Method: The study introduces MathDoc, a benchmark containing 3,609 curated questions from authentic high school mathematics exams, including unrecognizable samples to evaluate active refusal behavior. A multi-dimensional evaluation framework is proposed, covering stem accuracy, visual similarity, and refusal capability.

Result: Experiments on SOTA MLLMs (e.g., Qwen3-VL, Gemini-2.5-Pro) show they achieve strong extraction performance but fail to refuse illegible inputs, producing confident but invalid outputs.

Conclusion: The MathDoc benchmark highlights a critical gap in current MLLMs, as they fail to refuse illegible inputs despite strong extraction performance. This establishes MathDoc as a valuable benchmark for assessing model reliability under degraded document conditions.
...
TL;DR: The paper introduces a novel framework for Visual In-Context Learning (VICL) that uses multi-combination collaborative fusion instead of single-prompt fusion, enhancing contextual information utilization.


<details>
  <summary>Details</summary>
Motivation: Current 'retrieve-then-prompt' approaches discard valuable contextual information by focusing on single best prompts, limiting reasoning capabilities.

Method: Proposes a framework with three contextual representation branches integrating top-quality prompts, fed into a MULTI-VQGAN architecture for collaborative fusion.

Result: Demonstrates strong cross-task generalization, effective contextual fusion, and more robust predictions in tasks like segmentation, detection, and colorization.

Conclusion: The multi-combination collaborative fusion approach outperforms existing methods by better leveraging diverse contextual information.
...
TL;DR: A new VICL framework improves inpainting models by fusing multiple prompts, using lightweight MLPs for layout priors, and bidirectional fine-tuning, achieving better performance and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing methods for Vision In-Context Learning (VICL) discard complementary cues from high-quality prompts and fail to exploit structured information from prompt arrangements, limiting their effectiveness.

Method: The method includes an adaptive Fusion Module to aggregate patterns from multiple prompts, arrangement-specific lightweight MLPs to decouple layout priors, and a bidirectional fine-tuning mechanism to enhance collaboration between modules.

Result: Experiments on tasks like foreground segmentation, single-object detection, and image colorization show superior results and strong cross-task generalization.

Conclusion: The proposed VICL framework, with its adaptive Fusion Module, arrangement-specific lightweight MLPs, and bidirectional fine-tuning mechanism, effectively addresses the limitations of existing methods and demonstrates superior performance and cross-task generalization.
...
TL;DR: VQ-Seg replaces dropout with a novel vector quantization-based perturbation module for semi-supervised medical image segmentation, achieving superior performance on the LC dataset and public benchmarks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of dropout-based perturbation methods in semi-supervised medical image segmentation, which require manual tuning of sensitive hyperparameters and often lead to suboptimal regularization.

Method: The method involves using vector quantization to discretize the feature space and introducing a Quantized Perturbation Module (QPM) that replaces dropout. A dual-branch architecture is employed to share the post-quantization feature space between image reconstruction and segmentation tasks, and a Post-VQ Feature Adapter (PFA) incorporates guidance from a foundation model.

Result: Extensive experiments on the LC dataset and other benchmarks show that VQ-Seg outperforms state-of-the-art approaches.

Conclusion: The proposed VQ-Seg method, which utilizes vector quantization and a novel Quantized Perturbation Module, effectively addresses the limitations of dropout-based perturbation in semi-supervised medical image segmentation. By introducing a dual-branch architecture and a Post-VQ Feature Adapter, the method mitigates information loss and enhances performance, as demonstrated on the LC dataset and other benchmarks.
...
TL;DR: LaViT is a framework aligning latent visual thoughts to bridge the Perception Gap in multimodal reasoning, enhancing visual grounding and outperforming larger models.


<details>
  <summary>Details</summary>
Motivation: Current multimodal reasoning relies on external supervision, ignoring intrinsic visual attention dynamics, leading to a Perception Gap where students mimic textual outputs without proper visual grounding.

Method: LaViT aligns latent visual thoughts by autoregressively reconstructing the teacher's visual semantics and attention trajectories before text generation, using a curriculum sensory gating mechanism.

Result: LaViT achieves up to +16.9% gains on complex reasoning tasks and enables a compact 3B model to outperform larger open-source variants and proprietary models like GPT-4o.

Conclusion: LaViT effectively bridges the Perception Gap by aligning visual and textual reasoning, significantly improving multimodal latent reasoning performance.
...
TL;DR: The paper introduces Video Anomaly Reasoning (VAR), a structured multi-stage reasoning task, supported by a new dataset and the Vad-R1-Plus model, enhancing MLLMs' anomaly understanding and outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM-based methods in Video Anomaly Detection and Understanding lack explicit reasoning processes, risk awareness, and decision-oriented interpretation, prompting the need for a structured, multi-stage reasoning approach.

Method: The study introduces the VAR task, a new dataset with structured annotations, and the Anomaly-Aware Group Relative Policy Optimization method to enhance reasoning reliability under weak supervision. The Vad-R1-Plus model is developed for adaptive hierarchical reasoning and risk-aware decision making.

Result: Extensive experiments show that the proposed benchmark and method outperform both open-source and proprietary baselines, advancing MLLMs' reasoning capabilities in VAR tasks.

Conclusion: The proposed VAR task and Vad-R1-Plus model significantly enhance the reasoning capabilities of MLLMs in video anomaly understanding, outperforming existing baselines.
