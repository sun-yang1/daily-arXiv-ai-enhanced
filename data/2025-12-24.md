...
TL;DR: PHANTOM framework crafts adversarial examples to exploit CAV vulnerabilities, achieving high attack success and network disruption.


<details>
  <summary>Details</summary>
Motivation: To address vulnerabilities in CAV systems to physical adversarial attacks, which current defenses fail to mitigate.

Method: PHANTOM crafts perspective-dependent adversarial examples using anamorphic art, exploiting geometric distortions that mislead object detectors.

Result: PHANTOM achieves over 90% attack success under optimal conditions and maintains 60-80% effectiveness in degraded environments, disrupting CAV networks.

Conclusion: PHANTOM exposes critical vulnerabilities in CAV ecosystems, highlighting risks in both perception and communication layers.
...
TL;DR: A new technique uses a pre-trained video diffusion model to recover complex scene dynamics from motion-blurred images, outperforming old methods and supporting tasks like camera trajectory recovery.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing methods that rely on handcrafted priors and struggle with complex scene dynamics, the paper explores leveraging large-scale datasets and video priors.

Method: The approach repurposes a pre-trained video diffusion model trained on large-scale datasets to recover videos from motion-blurred images, revealing past, present, and future scene dynamics.

Result: The proposed method outperforms previous techniques, generalizes to in-the-wild images, and supports tasks like recovering camera trajectories and dynamic 3D scene structure.

Conclusion: The paper introduces a novel technique using a pre-trained video diffusion model to recover complex scene dynamics from motion-blurred images, outperforming previous methods and supporting various downstream tasks.
...
TL;DR: A novel method using video diffusion models enables realistic post-capture refocusing from a single defocused image, outperforming existing methods and supported by a new dataset.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of autofocus systems, which often fail to capture the intended subject, and to allow users to adjust focus after capture.

Method: The method involves generating a perceptually accurate focal stack from a single defocused image using video diffusion models, represented as a video sequence for interactive refocusing.

Result: The method consistently outperforms existing approaches in perceptual quality and robustness across challenging scenarios. A large-scale focal stack dataset was also released to support future research.

Conclusion: The paper concludes that the introduced method for post-capture refocusing using video diffusion models outperforms existing approaches in perceptual quality and robustness, enabling advanced focus-editing capabilities in photography.
...
TL;DR: The paper revisits scoring functions for geometric model fitting, finds MAGSAC++ equivalent to simpler methods, and proposes a new evaluation methodology, concluding no superiority of complex methods over basic ones.


<details>
  <summary>Details</summary>
Motivation: To address the problem of assigning a quality of fit score to candidate geometric models, particularly in robust settings, and to critically evaluate existing methods like MAGSAC++.

Method: The study extends the geometric error to spherical noises, analyzes MAGSAC++'s assumptions and derivation, and proposes an experimental methodology for evaluating scoring functions.

Result: All scoring functions, including MAGSAC++, perform identically in experiments, with MAGSAC++ not outperforming simpler methods or being less sensitive to threshold choices.

Conclusion: The paper comprehensively revisits the state-of-the-art in scoring functions for geometric model fitting, highlighting that MAGSAC++ is not superior to simpler methods and emphasizing the need for future research to improve or apply these methods.
...
TL;DR: HyGE-Occ improves 3D panoptic occupancy prediction with hybrid view-transformation and edge priors, achieving better geometric consistency and performance than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing approaches struggle with precise geometry and spatial range of 3D instances, which are critical for robust panoptic separation. HyGE-Occ addresses these limitations by enhancing geometric consistency and boundary awareness.

Method: HyGE-Occ employs a hybrid view-transformation branch combining continuous Gaussian-based depth representation with discretized depth-bin formulation, alongside edge maps from BEV features for auxiliary edge cue learning.

Result: HyGE-Occ demonstrates superior 3D geometric reasoning, outperforming existing work on the Occ3D-nuScenes dataset.

Conclusion: HyGE-Occ introduces a hybrid view-transformation branch with 3D Gaussian and edge priors, significantly improving geometric consistency and boundary awareness in 3D panoptic occupancy prediction, outperforming existing methods on the Occ3D-nuScenes dataset.
...
TL;DR: The paper introduces Widget2Code, a benchmark for generating code from widget images, and develops a baseline method (WidgetFactory) to improve visual fidelity by combining perceptual understanding and structured code generation.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the underexplored area of app widgets in UI2Code, which lack accessible markup and hierarchical context, unlike web or mobile UIs.

Method: The method involves assembling atomic components into complete layouts, equipped with icon retrieval and reusable visualization modules, and designing an end-to-end infrastructure (WidgetFactory) with a widget-tailored domain-specific language (WidgetDSL) and a compiler for multiple front-end implementations.

Result: Benchmarking shows that generalized multimodal large language models (MLLMs) outperform specialized UI2Code methods but still produce unreliable and visually inconsistent code. The proposed baseline significantly improves visual fidelity.

Conclusion: The paper establishes a strong baseline and unified infrastructure for future Widget2Code research, enhancing visual fidelity through a joint approach of perceptual understanding and structured code generation.
...
TL;DR: NeurAlign is a deep learning framework for joint cortical and subcortical MRI registration, outperforming traditional methods in accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: Traditional methods treat volumetric and surface-based registration separately, leading to inconsistencies that limit downstream analyses.

Method: A deep learning framework, NeurAlign, that registers 3D brain MRI images by jointly aligning both cortical and subcortical regions through a unified volume-and-surface-based representation, leveraging an intermediate spherical coordinate space.

Result: NeurAlign outperforms both classical and machine learning-based methods, improving the Dice score by up to 7 points while maintaining regular deformation fields, and is orders of magnitude faster.

Conclusion: NeurAlign sets a new standard for joint cortical and subcortical registration with superior accuracy, fast inference, and ease of use.
...
TL;DR: VehicleMAE-V2 improves vehicle perception by using structured priors (symmetry, contour, semantics) in pre-training, outperforming existing methods on multiple tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack effective learning of vehicle-related knowledge during pre-training, leading to poor general vehicle perception representations.

Method: The approach includes the Symmetry-guided Mask Module (SMM), Contour-guided Representation Module (CRM), and Semantics-guided Representation Module (SRM) to incorporate symmetry, contour, and semantic priors into token reconstruction.

Result: VehicleMAE-V2 shows superior performance on five downstream tasks, supported by the Autobot4M dataset (4M vehicle images and 12,693 text descriptions).

Conclusion: VehicleMAE-V2 significantly enhances vehicle-centric perception by leveraging structured priors during pre-training, demonstrating superior performance across multiple downstream tasks.
...
TL;DR: ViTs exhibit a block-recurrent depth structure, allowing computation to be rewritten with fewer blocks. Raptor models validate this, showing high accuracy with minimal blocks and revealing dynamical interpretability insights.


<details>
  <summary>Details</summary>
Motivation: To provide a mechanistic account of Vision Transformers' computational phenomenology by interpreting Transformer depth as a well-characterized flow.

Method: Introduced the Block-Recurrent Hypothesis (BRH) and trained block-recurrent surrogates (Raptor) of pretrained ViTs.

Result: Demonstrated that stochastic depth and training promote recurrent structure, with Raptor models recovering 96% of DINOv2 ImageNet-1k accuracy in only 2 blocks. Also found directional convergence, token-specific dynamics, and low-rank updates in late depth.

Conclusion: A compact recurrent program emerges along ViT depth, enabling models to be studied through principled dynamical systems analysis.
...
TL;DR: SE360 introduces a novel framework for 360° panorama editing, using autonomous data generation and a Transformer-based diffusion model to outperform existing methods in quality and accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing methods for instruction-based image editing in 360° panoramas often produce implausible results in both equirectangular projections and perspective views.

Method: SE360 uses a coarse-to-fine autonomous data generation pipeline with a Vision-Language Model (VLM) and adaptive projection adjustment, followed by a two-stage data refinement strategy. A Transformer-based diffusion model is trained for flexible object editing.

Result: The proposed SE360 framework ensures semantically meaningful and geometrically consistent object editing, with improved realism and reduced overfitting artifacts.

Conclusion: SE360 outperforms existing methods in visual quality and semantic accuracy for 360° panorama editing, validated by experiments.
...
TL;DR: VidFMs pretrained on videos show strong 3D awareness, surpassing specialized 3D models, suggesting potential for scalable 3D modeling.


<details>
  <summary>Details</summary>
Motivation: To investigate whether global 3D understanding emerges naturally from training on large video data and to quantify the 3D awareness of existing VidFMs.

Method: A model-agnostic framework is proposed to measure 3D awareness in VidFMs by estimating multiple 3D properties from their features using shallow read-outs.

Result: State-of-the-art video generation models show strong 3D understanding, even outperforming expert 3D models, despite no explicit 3D training.

Conclusion: The study demonstrates that Video Foundation Models (VidFMs) exhibit strong 3D understanding, surpassing expert models trained specifically for 3D tasks, highlighting the potential for scalable 3D model development.
...
TL;DR: HistoWAS is a framework linking tissue spatial organization to clinical outcomes using GIS-inspired features and mass univariate regression, tested on KPMP data.


<details>
  <summary>Details</summary>
Motivation: The lack of tools to measure spatial interactions of tissue characteristics and their clinical relevance inspired the development of HistoWAS.

Method: HistoWAS augments conventional metrics with 30 topological and spatial features from GIS point pattern analysis and uses an association study engine for mass univariate regression.

Result: Applied to 385 WSIs from 206 KPMP participants, HistoWAS analyzed 102 features (72 conventional and 30 spatial), with code and data publicly released.

Conclusion: HistoWAS successfully links tissue spatial organization to clinical outcomes, demonstrating its potential for biomarker discovery and clinical relevance.
...
TL;DR: WSD-MIL enhances MIL in pathology by modeling varying tumor scales efficiently, achieving top performance with reduced memory usage.


<details>
  <summary>Details</summary>
Motivation: Existing MIL methods overlook complex semantic relationships among instances in WSIs, and Transformer-based MIL approaches face scalability issues due to quadratic computational complexity. Fixed-scale attention mechanisms also fail to capture local instance correlations effectively.

Method: WSD-MIL includes a window scale decay based attention module and a squeeze-and-excitation based region gate module to model tumor regions of varying scales efficiently.

Result: WSD-MIL outperforms existing methods, demonstrating improved diagnostic accuracy and computational efficiency.

Conclusion: WSD-MIL achieves state-of-the-art performance on CAMELYON16 and TCGA-BRCA datasets while reducing computational memory by 62%. The code will be publicly available.
...
TL;DR: The paper proposes a CNN-ML ensemble model for high-accuracy guava disease detection, achieving ~99.99% accuracy, aiding Bangladesh's agricultural economy.


<details>
  <summary>Details</summary>
Motivation: Enhancing guava production is crucial for Bangladesh's economy, and early disease detection can reduce losses and protect harvests.

Method: The study employs CNN alongside traditional machine learning techniques, proposing ensemble models combining CNNML with Gradient Boosting Machine.

Result: The proposed models achieve approximately 99.99% classification accuracy for the guava dataset.

Conclusion: The CNN-ML cascade framework demonstrates high accuracy in guava disease detection, suitable for real-time agricultural monitoring systems.
...
TL;DR: DDTM, a dual-branch framework, improves cross-resolution land cover mapping by decoupling local refinement and global reasoning, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing weakly supervised approaches struggle with alignment between fine-grained spatial structures and coarse labels, leading to noisy supervision and degraded accuracy.

Method: DDTM uses a dual-branch framework: a diffusion-based branch for local semantic refinement and a transformer-based branch for global contextual consistency, plus a pseudo-label confidence module to reduce noise.

Result: DDTM achieves 66.52% mIoU on the Chesapeake Bay benchmark, significantly outperforming prior weakly supervised methods.

Conclusion: DDTM establishes a new state-of-the-art on cross-resolution land cover mapping, achieving 66.52% mIoU on the Chesapeake Bay benchmark, outperforming prior methods.
...
TL;DR: MIVA, a lightweight adapter for DMs, enables efficient training on limited data and precise motion control, outperforming larger models.


<details>
  <summary>Details</summary>
Motivation: Addresses challenges of high dimensionality and data scarcity in video signals, and DMs' struggle with novel motion patterns.

Method: Proposes Modular Image-to-Video Adapter (MIVA), a lightweight sub-network attachable to pre-trained DMs, scalable via parallelization.

Result: MIVA can be trained efficiently on limited data (∼10 samples) and allows precise motion control without prompt engineering.

Conclusion: MIVA enables precise motion control and high-quality generation, outperforming models trained on larger datasets.
...
TL;DR: A standardized pavement defect dataset with 52,747 images and 135,277 annotations was created to improve model generalization and enable fair comparisons.


<details>
  <summary>Details</summary>
Motivation: To address the lack of standardized datasets in automated pavement defect detection, which limits integration and generalization across diverse conditions.

Method: The paper consolidates multiple publicly available sources into a standardized dataset of 52,747 images with 135,277 bounding box annotations covering 13 distress types, capturing diverse real-world variations.

Result: Benchmarking with state-of-the-art models (YOLOv8-YOLOv12, Faster R-CNN, DETR) showed competitive performance across diverse scenarios.

Conclusion: The dataset introduced provides a globally representative benchmark for pavement defect detection, enabling fair model comparisons and zero-shot transfer to new environments.
...
TL;DR: LaSeRS dataset and SegEarth-R2 model address complex geospatial segmentation challenges, outperforming existing benchmarks with improved spatial attention and flexible query mechanisms.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the failure of current models in complex geospatial scenarios, such as segmenting objects at various granularities and interpreting implicit user intent, by introducing a comprehensive dataset (LaSeRS) and a robust model.

Method: The paper proposes SegEarth-R2, an MLLM architecture with spatial attention supervision for small object localization and a flexible segmentation query mechanism for handling single and multi-target scenarios.

Result: Experimental results show that SegEarth-R2 achieves outstanding performance on LaSeRS and other benchmarks, demonstrating its effectiveness in complex language-guided segmentation tasks.

Conclusion: The paper concludes that SegEarth-R2, with its spatial attention supervision and flexible segmentation query mechanism, sets a strong baseline for complex geospatial segmentation tasks, addressing limitations of existing models.
...
TL;DR: Adding road-facing views to driver-facing footage doesn't always improve distraction detection; performance depends on the model architecture. SlowOnly-R50 improved by 9.8%, while SlowFast-R50 dropped by 7.2%. Fusion-aware design is crucial for future systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate whether incorporating road-facing views alongside driver-facing footage improves distraction detection accuracy in naturalistic driving conditions, as most existing models overlook environmental context.

Method: The study benchmarks three spatiotemporal action recognition architectures (SlowFast-R50, X3D-M, SlowOnly-R50) under two input configurations: driver-only and stacked dual-view, using synchronized dual-camera recordings from real-world driving.

Result: Results show that contextual inputs can improve detection in certain models (e.g., SlowOnly-R50 achieved a 9.8% improvement with dual-view inputs), but performance gains depend on the architecture (e.g., SlowFast-R50 experienced a 7.2% drop in accuracy).

Conclusion: The study concludes that simply adding visual context (road-facing views) to driver-facing footage is not sufficient for improving distraction detection accuracy. Performance gains depend on the underlying architecture, with some models experiencing drops in accuracy due to representational conflicts. Future multimodal driver monitoring systems need fusion-aware design.
...
TL;DR: MAPI-GNN improves medical diagnosis by dynamically building multiple graphs from disentangled features, outperforming current methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitation of relying on a single, static graph built from indiscriminate features, which hinders patient-specific pathological relationship modeling.

Method: The framework uses a multi-dimensional discriminator to uncover latent graph-aware patterns, dynamically constructs a stack of activation graphs, and aggregates them via a relational fusion engine.

Result: Extensive experiments on two diverse tasks with over 1300 patient samples show superior performance.

Conclusion: MAPI-GNN significantly outperforms state-of-the-art methods in multimodal medical diagnosis by dynamically constructing multifaceted graph profiles from semantically disentangled feature subspaces.
...
TL;DR: H2em introduces hyperbolic embeddings for CZSL, improving generalization with hierarchical learning objectives and cross-modal attention.


<details>
  <summary>Details</summary>
Motivation: Current CZSL methods fail to scale to large taxonomies due to Euclidean space's limitations in capturing hierarchical structures.

Method: H2em uses hyperbolic embeddings with Dual-Hierarchical Entailment Loss, Discriminative Alignment Loss, and Hyperbolic Cross-Modal Attention.

Result: H2em outperforms existing methods in both closed-world and open-world scenarios.

Conclusion: H2em leverages hyperbolic geometry to address the limitations of Euclidean space in CZSL, achieving state-of-the-art performance.
...
TL;DR: VALLR-Pin improves Mandarin lip-reading by combining visual features with phonetic context using a two-stage framework and LLM for error correction.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of Mandarin visual speech recognition due to ambiguous visemes and prevalent homophones by integrating phonetic and linguistic context.

Method: A two-stage framework with a shared video encoder and dual decoders for predicting Chinese characters and Pinyin, followed by LLM-based ambiguity resolution and fine-tuning on synthetic noisy examples.

Result: Improved Mandarin lip-reading performance through robust visual-semantic representations and LLM-assisted error correction.

Conclusion: VALLR-Pin synergizes visual features with phonetic and linguistic context to enhance Mandarin lip-reading performance by leveraging a two-stage framework and large language model (LLM) for error correction.
...
TL;DR: FlashLips is a fast, high-quality lip-sync system using a two-stage, mask-free approach with real-time performance.


<details>
  <summary>Details</summary>
Motivation: To achieve real-time lip-sync performance without compromising visual quality, decoupling lips control from rendering.

Method: The system uses a two-stage approach: Stage 1 is a latent-space editor trained with reconstruction losses, and Stage 2 is an audio-to-pose transformer trained with flow-matching. Self-supervision removes the need for explicit masks.

Result: FlashLips runs at over 100 FPS on a single GPU, matching the visual quality of larger state-of-the-art models.

Conclusion: FlashLips delivers high perceptual quality and faster-than-real-time speed by combining deterministic reconstruction with robust audio control in a simple, stable pipeline.
...
TL;DR: A multimodal pipeline augments image captions with external textual knowledge, producing richer, context-aware descriptions for real-world applications.


<details>
  <summary>Details</summary>
Motivation: Real-world image captions often lack contextual depth, limiting effectiveness in domains like journalism, education, and digital archives where richer descriptions are essential.

Method: The system retrieves semantically similar images using BEIT-3 and SigLIP So-384, reranks them with ORB and SIFT for geometric alignment, and extracts contextual information from related articles via semantic search. A fine-tuned Qwen3 model with QLoRA integrates this context with base captions from Instruct BLIP (Vicuna-7B).

Result: Evaluated on OpenEvents v1, the approach generates significantly more informative captions than traditional methods.

Conclusion: The proposed multimodal pipeline effectively enhances image captions by integrating external textual knowledge, demonstrating strong potential for real-world applications requiring deeper visual-textual understanding.
...
TL;DR: PICM-Net introduces a machine-oriented progressive image compression method using trit-plane coding and adaptive decoding, outperforming in downstream tasks.


<details>
  <summary>Details</summary>
Motivation: To address the unexplored area of progressive image compression with fine granular scalability for machine perception, analyzing differences in rate-distortion priorities between human and machine perception.

Method: The paper proposes PICM-Net, a progressive learned image compression codec based on trit-plane coding, with an adaptive decoding controller for dynamic decoding level determination.

Result: Extensive experiments show efficient and adaptive progressive transmission while maintaining high downstream classification performance.

Conclusion: PICM-Net establishes a new paradigm for machine-aware progressive image compression, maintaining high performance in downstream tasks.
...
TL;DR: IRSN enhances fashion style classification by analyzing item-specific features and their combinations, achieving significant accuracy improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: Fashion style classification is challenging due to large visual variations within the same style and visually similar styles. Styles are expressed by both global appearance and item attributes/combinations.

Method: The proposed IRSN uses item region pooling (IRP) to extract item-specific features, analyzes them separately, and combines them using gated feature fusion (GFF). It also employs a dual-backbone architecture combining domain-specific and general feature extractors.

Result: IRSN improved style classification accuracy by an average of 6.9% (max 14.5%) on FashionStyle14 and 7.6% (max 15.1%) on ShowniqV3. Visualization confirmed better capture of differences between similar styles.

Conclusion: IRSN effectively improves fashion style classification by leveraging item-specific features and their combinations, along with global features, outperforming baseline models significantly.
...
TL;DR: Study shows ConvLSTM with Adam/RMSprop excels in HAR, reaching 99% accuracy, while BiLSTM's performance varies widely.


<details>
  <summary>Details</summary>
Motivation: To analyze the under-explored impact of activation functions and optimizers on HAR system performance, especially in healthcare applications.

Method: The research evaluates three activation functions (ReLU, Sigmoid, Tanh) and four optimizers (SGD, Adam, RMSprop, Adagrad) using BiLSTM and ConvLSTM architectures on HMDB51 and UCF101 datasets.

Result: ConvLSTM outperforms BiLSTM, achieving up to 99.00% accuracy with Adam or RMSprop, while BiLSTM shows inconsistent performance across datasets.

Conclusion: The study highlights the importance of selecting appropriate activation functions and optimizers in HAR systems, with ConvLSTM combined with Adam or RMSprop achieving the best performance.
...
TL;DR: LiDARDraft uses 3D layouts and ControlNet to generate high-quality, controllable LiDAR point clouds from text, images, or sketches, improving simulation flexibility.


<details>
  <summary>Details</summary>
Motivation: To address the imbalance between complex LiDAR point cloud distributions and simple control signals, enabling better controllability and quality in generation.

Method: The approach uses 3D layouts as an intermediary, transforming inputs into semantic and depth control signals, and employs a rangemap-based ControlNet for pixel-level alignment.

Result: The method achieves excellent performance in controllable LiDAR point cloud generation, supporting 'simulation from scratch' from diverse inputs.

Conclusion: LiDARDraft successfully bridges the gap between simple control signals and complex LiDAR point cloud distributions, enabling high-quality, versatile generation from various inputs like text, images, and sketches.
...
TL;DR: Hybrid framework combines deterministic and stochastic methods for novel view synthesis, achieving high-quality renders with faster performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods either render quickly but blur unobserved areas (deterministic) or hallucinate plausible content at high computational cost (stochastic). The hybrid approach aims to unify these strengths.

Method: A bidirectional transformer encodes multi-view image tokens and Plucker-ray embeddings, followed by two lightweight heads: a feed-forward regression head for well-constrained geometry and a masked autoregressive diffusion head for occluded/unseen regions.

Result: The method attains state-of-the-art image quality while reducing rendering time by an order of magnitude compared to fully generative baselines.

Conclusion: The proposed hybrid framework achieves state-of-the-art image quality with significantly reduced rendering time, combining the strengths of deterministic and stochastic methods for novel view synthesis.
...
TL;DR: A multi-modal attention network fuses radar and thermal data for bridge deck delamination detection, outperforming single-modal methods in accuracy and AUC. It includes uncertainty quantification but struggles with extreme class imbalance.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of single-modal approaches (like Ground Penetrating Radar and Infrared Thermography) which face complementary constraints (e.g., radar struggles with moisture, thermography is weather-dependent). The goal is to detect subsurface defects more accurately by leveraging multi-modal fusion.

Method: The method involves a multi-modal attention network that fuses radar temporal patterns with thermal spatial signatures. It introduces temporal attention for radar, spatial attention for thermal features, and cross-modal fusion with learnable embeddings. Uncertainty quantification is incorporated via Monte Carlo dropout and learned variance estimation.

Result: Experiments on five bridge datasets show the approach outperforms baselines in accuracy and AUC, with meaningful improvements over single-modal and concatenation-based fusion. Cross-modal attention provides critical gains, and uncertainty quantification reduces calibration error, enabling selective prediction. However, extreme class imbalance reveals vulnerabilities.

Conclusion: The paper concludes that the proposed multi-modal attention network significantly improves bridge deck delamination detection, especially in balanced to moderately imbalanced datasets. However, it notes vulnerability to majority class collapse under extreme imbalance, suggesting specialized techniques for such cases. The system is efficient for real-time deployment with characterized capabilities and limitations.
