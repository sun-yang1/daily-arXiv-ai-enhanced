...
TL;DR: The paper introduces Complex Preference Optimization (CPO) to align diffusion models with fine-grained, hierarchical human expertise, improving image generation quality in domains like painting.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitation of current post-training alignment methods, which rely on simplified signals like scalar rewards or binary preferences, making them inadequate for aligning with complex, hierarchical human expertise.

Method: The method involves a two-stage alignment framework: 1) injecting domain knowledge into an auxiliary diffusion model via Supervised Fine-Tuning, and 2) introducing Complex Preference Optimization (CPO), which extends DPO to align the target diffusion model with non-binary, hierarchical criteria. CPO reformulates the alignment problem to maximize the probability of positive attributes and minimize the probability of negative attributes.

Result: Extensive experiments demonstrate that CPO significantly enhances generation quality and alignment with expertise, particularly in painting generation, validating the effectiveness of the proposed framework.

Conclusion: The paper concludes that the proposed Complex Preference Optimization (CPO) framework significantly improves the alignment of diffusion models with fine-grained, hierarchical human expertise, particularly in the domain of painting generation, opening new avenues for such alignment.
...
TL;DR: A novel method embeds text into images using quinary RGB pixel intensity combinations, achieving distortion-free results and higher efficiency compared to traditional LSB, MSB, and deep learning techniques.


<details>
  <summary>Details</summary>
Motivation: Existing methods like LSB, MSB, and deep learning approaches often introduce noise, require multiple pixels, or involve high computational overhead. The proposed method aims to address these limitations by providing a more efficient and distortion-free embedding technique.

Method: The method utilizes quinary combinations of pixel intensities in RGB space, where five controlled variations in each R, G, and B channel create 125 distinct combinations mapped to textual symbols. This allows embedding a complete textual symbol within a single RGB pixel.

Result: Evaluation using metrics such as MSE, MAE, SNR, PSNR, SSIM, and histogram comparison showed no significant distortion in the encoded images. The method achieves higher embedding efficiency by encoding a complete textual symbol in a single pixel.

Conclusion: The proposed quinary pixel intensity combination method offers an efficient and distortion-free approach for embedding textual data into images, outperforming traditional methods like LSB, MSB, and deep learning techniques.
...
TL;DR: A post-training approach for unified text-image generation improves performance across benchmarks by leveraging reward-weighted synthetic data and strategic modality coupling.


<details>
  <summary>Details</summary>
Motivation: Existing systems rely on explicit modality switching, limiting cross-modal coupling and prohibiting automatic multimodal generation. This work aims to achieve fully unified text-image generation within a single inference process.

Method: The work explores post-training to achieve fully unified text-image generation, examining the impact of joint text-image generation on T2I performance and the relative importance of each modality during post-training. It also investigates different post-training data strategies.

Result: The proposed method improves multimodal image generation across four diverse T2I benchmarks, showing superior results with targeted post-training data.

Conclusion: The approach demonstrates the effectiveness of reward-weighting both modalities and strategically designed post-training data, improving multimodal image generation across diverse benchmarks.
...
TL;DR: ReHyAt is a hybrid attention mechanism for video diffusion models that combines softmax and linear attention, reducing training costs and memory usage while maintaining high video quality.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the scalability limitations of quadratic attention complexity in transformer-based video diffusion models, enabling efficient long-duration and on-device video generation.

Method: The paper proposes a light-weight distillation and finetuning pipeline for ReHyAt, which combines softmax and linear attention mechanisms to reduce training costs and memory usage.

Result: ReHyAt achieves state-of-the-art video quality while reducing attention cost from quadratic to linear, as demonstrated by experiments on VBench and VBench-2.0 and human preference studies.

Conclusion: ReHyAt introduces a Recurrent Hybrid Attention mechanism that combines the fidelity of softmax attention with the efficiency of linear attention, enabling scalable and efficient video generation while maintaining high quality.
...
TL;DR: A new progressive codec using Residual Vector Quantization and an auto-regressive entropy model improves compression for 3D Gaussian Splatting, overcoming limitations of scalar quantization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the high storage requirements of 3D Gaussian Splatting models, which hinder their deployment over cloud and streaming services, by improving compression techniques beyond traditional scalar quantization.

Method: The method involves using Residual Vector Quantization to compress primitive features and an auto-regressive entropy model guided by a multi-resolution hash grid to predict conditional probabilities for efficient compression.

Result: The result is a novel progressive codec that achieves higher compression efficiency for 3D Gaussian Splatting models by leveraging Residual Vector Quantization and an auto-regressive entropy model.

Conclusion: The paper concludes that the proposed Residual Vector Quantization approach, combined with an auto-regressive entropy model, significantly improves the compression efficiency for 3D Gaussian Splatting models, addressing the limitations of traditional scalar quantization methods.
...
TL;DR: Transfer learning with pre-trained models (ResNet-18, VGG-16) outperforms custom CNNs and feature extraction, especially for complex tasks, while custom CNNs are better for simpler tasks.


<details>
  <summary>Details</summary>
Motivation: The research aims to provide practical insights for selecting deep learning approaches based on dataset characteristics, computational resources, and performance requirements.

Method: The study compares custom-built CNNs with pre-trained architectures (ResNet-18 and VGG-16) using feature extraction and transfer learning approaches across five image classification datasets from Bangladesh.

Result: Transfer learning with fine-tuning achieved accuracy improvements of 3% to 76% across datasets, with ResNet-18 achieving 100% accuracy on the Road Damage BD dataset. Custom CNNs were more efficient in model size and training for simpler tasks.

Conclusion: The study concludes that transfer learning with fine-tuning using pre-trained models like ResNet-18 and VGG-16 consistently outperforms custom-built CNNs and feature extraction methods, especially for complex tasks with limited data, while custom CNNs are more efficient for simpler tasks.
...
TL;DR: PackCache is a training-free method that optimizes KV-cache management in autoregressive models, significantly speeding up video generation by addressing spatiotemporal properties of tokens.


<details>
  <summary>Details</summary>
Motivation: The KV-cache mechanism in unified autoregressive models grows linearly with the number of generated tokens, becoming a bottleneck for inference efficiency and generative length, especially in video generation tasks.

Method: PackCache introduces three coordinated mechanisms: condition anchoring, cross-frame decay modeling, and spatially preserving position embedding to dynamically compact the KV-cache.

Result: PackCache accelerates end-to-end generation by 1.7-2.2x on 48-frame sequences, with the final four frames seeing a 2.6x and 3.7x speedup on A40 and H200, respectively.

Conclusion: PackCache significantly improves the efficiency of unified autoregressive models by dynamically managing the KV-cache, enabling faster and longer-sequence video generation.
...
TL;DR: The paper explores stress recognition using 3D facial geometry and proposes a Transformer-based model, achieving top performance with cross-modal fusion of EMOCA and physiological signals.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the underexplored role of disentangled 3D facial geometry in stress recognition, as most existing methods rely on Facial Action Units.

Method: The method involves analyzing stress during distracted driving using EMOCA-derived 3D expression and pose coefficients, followed by paired hypothesis tests. A Transformer-based temporal modeling framework is proposed, evaluating unimodal, early-fusion, and cross-modal attention strategies.

Result: The results show that 41 of 56 coefficients exhibit consistent stress responses. Cross-Modal Attention fusion of EMOCA and physiological signals achieves the best performance (AUROC 92%, Accuracy 86.7%).

Conclusion: The study concludes that temporal modeling and cross-modal attention are highly effective for stress recognition, with the best performance achieved by fusing EMOCA and physiological signals.
...
TL;DR: A flow-matching foundation model pre-trained on RGB is adapted using LoRA for cross-spectral translation to IR and SAR with minimal paired data. The synthetic data improves downstream object detection, showing promise for non-visible modality support.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in foundation models primarily trained on RGB data, while many safety-critical applications rely on non-visible modalities like IR and SAR. The goal is to repurpose RGB-trained models for cross-spectral translation using minimal paired data.

Method: The research adapts a pre-trained FLUX.1 Kontext model using low-rank adaptation (LoRA) modules. Fine-tuning is performed on just 100 paired images per domain (RGB to IR on KAIST and RGB to SAR on M4-SAR). The adapted model translates RGB images into pixel-aligned IR/SAR, enabling the reuse of existing bounding boxes for training object detection models.

Result: The adapted model successfully translates RGB images into IR/SAR, and the synthetic data improves downstream object detection performance. LPIPS on 50 held-out pairs serves as a strong proxy for downstream performance, with lower LPIPS predicting higher mAP for YOLOv11n and DETR on test data. Synthetic IR and SAR data enhance detection when combined with limited real data.

Conclusion: The study demonstrates that few-shot LoRA adaptation of flow-matching foundation models is a promising approach for enabling foundation-style support for non-visible modalities like IR and SAR. The synthetic data generated from RGB images significantly enhances downstream object detection tasks in these modalities.
...
TL;DR: Pre-trained CNNs (VGG-16, ResNet-50, MobileNet) outperform custom CNNs in accuracy and speed, but custom CNNs are more efficient computationally and in terms of parameters.


<details>
  <summary>Details</summary>
Motivation: The motivation was to address the practical consideration of choosing between designing a custom CNN from scratch or using established pre-trained architectures for image classification tasks.

Method: A comparative analysis was conducted between a custom-designed CNN and pre-trained architectures (VGG-16, ResNet-50, and MobileNet) using transfer learning. All models were evaluated under identical experimental settings with metrics like accuracy, precision, recall, and F1-score.

Result: Pre-trained CNN architectures consistently outperformed the custom CNN in classification accuracy and convergence speed, especially with limited training data. However, the custom CNN showed competitive performance with fewer parameters and reduced computational complexity.

Conclusion: The study highlights the trade-offs between model complexity, performance, and computational efficiency when choosing between custom-designed CNNs and pre-trained architectures for image classification tasks. It suggests that while pre-trained models like VGG-16, ResNet-50, and MobileNet outperform custom CNNs in accuracy and convergence speed, custom CNNs can be competitive with fewer parameters and lower computational demands.
...
TL;DR: Tri-MARF is a novel framework for 3D object annotation using tri-modal inputs (2D images, textual descriptions, 3D point clouds) and multi-agent collaboration, outperforming existing methods in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Challenges in 3D annotation for autonomous driving and augmented reality, such as spatial complexity and occlusion, are not effectively addressed by single-model approaches.

Method: Integrates tri-modal inputs via a multi-agent architecture (vision-language model agent, information aggregation agent, gating agent) to align textual semantics with 3D geometry.

Result: Achieves a CLIPScore of 88.7, retrieval accuracy of 45.2 and 43.8 on ViLT R@5, and throughput of 12,000 objects/hour on an NVIDIA A100 GPU.

Conclusion: Tri-MARF significantly enhances large-scale 3D annotation performance compared to state-of-the-art methods.
...
TL;DR: A hybrid self-supervised and weakly-supervised learning framework predicts mastoidectomy regions from preoperative CT scans, achieving a mean Dice score of 0.72.


<details>
  <summary>Details</summary>
Motivation: Accurate mastoidectomy shape prediction improves CI surgical planning but lacks labeled data.

Method: Proposes a hybrid self-supervised and weakly-supervised learning framework for prediction without human annotations.

Result: Achieves a mean Dice score of 0.72, surpassing state-of-the-art approaches.

Conclusion: This work offers a robust solution for CI surgical planning by integrating self-supervised and weakly-supervised learning.
...
TL;DR: CRUNet-MR-Univ is a foundation model for CMR reconstruction that generalizes across diverse scenarios, outperforming existing methods by leveraging spatio-temporal correlations and prompt-based priors.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for CMR reconstruction lack generalizability due to the wide variability in CMR scans (e.g., image contrast, sampling patterns, scanner vendors, anatomical structures, disease types). A unified model is needed to address these limitations.

Method: The paper introduces CRUNet-MR-Univ, a deep learning model that utilizes spatio-temporal correlations and prompt-based priors to handle the full diversity of CMR scans.

Result: CRUNet-MR-Univ consistently outperforms baseline methods across diverse CMR settings, demonstrating its effectiveness and potential for real-world clinical applications.

Conclusion: CRUNet-MR-Univ is proposed as a foundation model that effectively generalizes across diverse CMR scenarios by leveraging spatio-temporal correlations and prompt-based priors, outperforming baseline methods consistently.
...
TL;DR: GPRO, a meta-reasoning controller for LVLMs, dynamically routes computation to address overthinking and visual perception failures, improving accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: LVLMs often suffer from overthinking, leading to inefficiency and degraded accuracy. Prior adaptive reasoning strategies overlook visual perception failures, which are a fundamental bottleneck.

Method: The paper proposes GPRO, a meta-reasoning controller that dynamically routes computation among a fast path, a slow perception path, and a slow reasoning path. It uses large-scale failure attribution supervision and multi-objective reinforcement learning for training.

Result: Experiments on five benchmarks show GPRO improves accuracy and efficiency, generating shorter responses.

Conclusion: GPRO significantly enhances both accuracy and efficiency in LVLMs by dynamically routing computation among different decision paths, outperforming existing methods.
...
TL;DR: UniDrive-WM is a unified VLM-based world model for autonomous driving that integrates scene understanding, trajectory planning, and future image generation, outperforming previous methods in accuracy and safety metrics.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat perception, prediction, and planning as separate modules in autonomous driving, which may limit performance. UniDrive-WM aims to unify these tasks within a single VLM-based architecture to improve accuracy and safety.

Method: UniDrive-WM is a unified VLM-based world model that jointly performs driving-scene understanding, trajectory planning, and trajectory-conditioned future image generation within a single architecture. It uses a trajectory planner to predict future trajectories, which condition a VLM-based image generator to produce plausible future frames, enhancing scene understanding and refining trajectory generation iteratively.

Result: Experiments on the Bench2Drive benchmark show that UniDrive-WM improves planning performance by 5.9% in L2 trajectory error and 9.2% in collision rate over the previous best method, while also producing high-fidelity future images.

Conclusion: UniDrive-WM demonstrates the advantages of integrating vision-language models (VLMs) for joint driving-scene understanding, trajectory planning, and future image generation, leading to improved planning performance and reduced collision rates in autonomous driving.
...
TL;DR: An LLM-driven agent integrates vision-language models for forest change analysis, achieving strong performance on new datasets, improving interpretability and efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the gap in integrating large language models (LLMs) with vision-language models (VLMs) for remote sensing image change interpretation (RSICI) in forest monitoring.

Method: The system integrates a multi-level change interpretation (MCI) vision-language backbone with LLM-based orchestration to support natural language querying across multiple RSICI tasks.

Result: The system achieves mIoU and BLEU-4 scores of 67.10% and 40.17% on the Forest-Change dataset, and 88.13% and 34.41% on LEVIR-MCI-Trees.

Conclusion: The proposed LLM-driven agent for integrated forest change analysis demonstrates significant potential in improving accessibility, interpretability, and efficiency of forest change analysis, as evidenced by its performance on the Forest-Change dataset and LEVIR-MCI-Trees subset.
...
TL;DR: TokenSeg is a boundary-aware sparse token representation framework for efficient 3D medical volume segmentation, achieving high performance with reduced computational costs.


<details>
  <summary>Details</summary>
Motivation: To address the computational demands and redundancy in 3D medical image segmentation, particularly in processing voxels and homogeneous regions.

Method: Proposes a multi-scale hierarchical encoder, a boundary-aware tokenizer, and a sparse-to-dense decoder to efficiently capture and reconstruct segmentation details.

Result: Achieves 94.49% Dice and 89.61% IoU on a 3D breast DCE-MRI dataset, with 64% reduction in GPU memory and 68% reduction in inference latency.

Conclusion: TokenSeg demonstrates state-of-the-art performance and generalization across heterogeneous anatomical structures, highlighting the effectiveness of sparse representation.
...
TL;DR: FaceRefiner refines facial textures via multi-level style transfer and differentiable rendering, improving quality and identity consistency over current methods.


<details>
  <summary>Details</summary>
Motivation: Current facial texture generation methods lack generalization for in-the-wild images, leading to inconsistencies in details, structures, and identity. FaceRefiner addresses this limitation.

Method: FaceRefiner uses a style transfer-based approach, treating 3D sampled texture as style and generated texture as content, while integrating differentiable rendering to transfer low-level pixel information.

Result: Experiments on Multi-PIE, CelebA, and FFHQ datasets show FaceRefiner enhances texture quality and identity preservation compared to existing methods.

Conclusion: The proposed FaceRefiner method effectively improves facial texture quality and identity preservation by integrating multi-level style transfer with differentiable rendering, outperforming state-of-the-art methods.
...
TL;DR: RepMD detects harmful memes by reproducing their design concepts, achieving 81.1% accuracy and improving human efficiency.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of analyzing ever-shifting harmful memes by identifying their invariant principles (design concepts) to improve detection accuracy and efficiency.

Method: The method involves defining a Design Concept Graph (DCG) inspired by attack trees, deriving DCG from historical memes through design step reproduction and graph pruning, and using DCG to guide a Multimodal Large Language Model (MLLM) for detection.

Result: RepMD achieves 81.1% accuracy in detecting harmful memes and shows slight accuracy decreases for type-shifting and temporal-evolving memes. Human evaluation indicates it reduces discovery time to 15-30 seconds per meme.

Conclusion: The paper concludes that RepMD, a method based on reproducing the design concepts of harmful memes, effectively detects such memes with high accuracy and improves human efficiency in identifying them.
...
TL;DR: 3D conditional generative models (Pix2Pix GAN, SPADE-GAN, SPADE-LDM) synthesize LGE MRI volumes to improve LA segmentation, with SPADE-LDM showing the best performance.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of LA wall and endocardium from LGE MRI is crucial for atrial fibrosis quantification but is hindered by limited data and complex anatomy.

Method: A pipeline was developed to synthesize 3D LGE MRI volumes from composite semantic label maps using three 3D conditional generators.

Result: SPADE-LDM achieved the best realism (FID 4.063) and improved LA cavity segmentation Dice score from 0.908 to 0.936.

Conclusion: Label-conditioned 3D synthesis enhances segmentation of underrepresented cardiac structures, demonstrating significant potential.
...
TL;DR: MiLDEAgent introduces a reasoning-based framework for editing multi-layer design documents, outperforming existing models.


<details>
  <summary>Details</summary>
Motivation: Prior work overlooks multi-layer design document editing, focusing on single-layer tasks or multi-layer generation, lacking the reasoning needed for precise edits.

Method: The framework combines an RL-trained multimodal reasoner for layer-wise understanding with an image editor for targeted modifications.

Result: MiLDEAgent significantly outperforms open-source baselines and matches closed-source models in performance.

Conclusion: MiLDEAgent establishes a strong baseline for multi-layer document editing by combining layer-aware reasoning with precise modifications, outperforming existing models.
...
TL;DR: The paper addresses operational uncertainties in AI-enabled human-centric systems, proposing a framework for safety and security evaluation and demonstrating it with a technique for detecting meal non-announcement in diabetes control.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the operational uncertainties in AI-enabled human-centric cyber-physical systems, which can violate safety and security requirements, particularly in critical applications like medical monitoring and autonomous vehicles.

Method: The paper introduces a framework to evaluate strategies for ensuring safety and security in AI-enabled human-centric systems. It also presents a personalized image-based technique for detecting non-announcement of meals in closed-loop blood glucose control.

Result: The paper results in a framework for evaluating safety and security strategies and a novel technique for detecting operational deviations in blood glucose control systems.

Conclusion: The paper concludes that AI-enabled human-centric cyber-physical systems can face operational uncertainties that may compromise safety and security. It proposes a framework to evaluate strategies for mitigating these risks and demonstrates its application in detecting non-announcement of meals in blood glucose control for Type 1 diabetics.
...
TL;DR: The HUR-MACL model improves head and neck organ segmentation by focusing on high uncertainty regions with Vision Mamba and Deformable CNN, achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: Current hybrid architectures for organ segmentation in the head and neck often concatenate features without exploiting the unique strengths of each component, leading to functional overlap and limited accuracy, especially for small, complexly shaped organs.

Method: The proposed HUR-MACL model identifies high uncertainty regions using a convolutional neural network and employs Vision Mamba and Deformable CNN to improve segmentation accuracy in these regions. A heterogeneous feature distillation loss is introduced to enhance collaborative learning between the two architectures.

Result: The HUR-MACL model achieves state-of-the-art results on two public datasets and one private dataset, demonstrating superior segmentation accuracy.

Conclusion: The HUR-MACL model effectively addresses the limitations of existing hybrid architectures by adaptively focusing on high uncertainty regions and leveraging the strengths of Vision Mamba and Deformable CNN, leading to state-of-the-art segmentation accuracy for small and complexly shaped organs in the head and neck.
...
TL;DR: HyperAlign proposes a hyperbolic geometry-based framework for adaptive text-to-image alignment assessment, outperforming existing Euclidean-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on Euclidean space metrics, neglecting structured semantic alignment and lacking adaptability.

Method: Extracts CLIP features, maps to hyperbolic space, uses dynamic-supervision entailment modeling, and adaptive modulation regressor.

Result: Achieves competitive performance in single and cross-database evaluations.

Conclusion: HyperAlign validates the effectiveness of hyperbolic geometric modeling for image-text alignment assessment.
...
TL;DR: Agri-R1, a reasoning-enhanced model for agriculture, uses automated data generation and GRPO training to outperform conventional methods, achieving significant gains in accuracy, QA, and generalization.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges in agricultural disease diagnosis, where conventional fine-tuning methods require extensive labels, lack interpretability, and generalize poorly. Existing reasoning methods rely on costly expert annotations and fail to address the open-ended, diverse nature of agricultural queries.

Method: The method involves automating high-quality reasoning data generation through vision-language synthesis and LLM-based filtering, using only 19% of available samples. Training employs Group Relative Policy Optimization (GRPO) with a novel reward function that integrates domain-specific lexicons and fuzzy matching to evaluate correctness and linguistic flexibility in open-ended responses.

Result: The 3B-parameter Agri-R1 model achieves performance competitive with 7B- to 13B-parameter baselines, showing a +23.2% relative gain in disease recognition accuracy, +33.3% in agricultural knowledge QA, and a +26.10-point improvement in cross-domain generalization over standard fine-tuning. Ablation studies confirm the benefits of structured reasoning data and GRPO-driven exploration.

Conclusion: The paper concludes that the proposed Agri-R1 model, which leverages automated reasoning data generation and GRPO training, significantly outperforms conventional fine-tuning methods in agricultural disease diagnosis. The model achieves notable improvements in disease recognition accuracy, agricultural knowledge QA, and cross-domain generalization, demonstrating the effectiveness of the synergy between structured reasoning data and GRPO-driven exploration.
...
TL;DR: DB-MSMUNet, a novel encoder-decoder architecture for pancreatic segmentation, outperforms existing methods by combining multi-scale state space modeling and edge enhancement.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of the pancreas and its lesions in CT scans is challenging due to low tissue contrast, blurry boundaries, irregular shapes, and small lesion sizes.

Method: The proposed DB-MSMUNet uses a Multi-scale Mamba Module for global context and local deformation, a dual-decoder design for edge and area refinement, and Auxiliary Deep Supervision for multi-scale feature enhancement.

Result: Achieves Dice Similarity Coefficients of 89.47%, 87.59%, and 89.02% on NIH, MSD, and clinical datasets, outperforming state-of-the-art methods.

Conclusion: DB-MSMUNet demonstrates effectiveness and generalizability for real-world pancreatic CT segmentation tasks.
...
TL;DR: HATIR introduces a Heat-Aware Diffusion model for Turbulent InfraRed Video Super-Resolution, addressing turbulence and resolution degradation jointly.


<details>
  <summary>Details</summary>
Motivation: Infrared video suffers from turbulence and compression degradation, which existing methods fail to address adequately.

Method: HATIR uses heat-aware deformation priors and a Phasor-Guided Flow Estimator to model turbulence and resolution degradation. It also includes a Turbulence-Aware Decoder for structural recovery.

Result: The method is validated on FLIR-IVSR, a new dataset for turbulent infrared VSR, showing improved performance.

Conclusion: HATIR advances infrared VSR by jointly modeling turbulence and resolution degradation, encouraging future research in this area.
...
TL;DR: WebCryptoAgent is a novel trading framework for cryptocurrencies that uses modality-specific agents and a decoupled control architecture to improve decision-making and risk management, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing cryptocurrency trading systems struggle with integrating noisy multi-source web evidence and maintaining robustness to rapid price shocks at sub-second timescales. The need arises for a solution that can synthesize unstructured web content, social sentiment, and structured OHLCV signals into coherent trading decisions while ensuring risk control.

Method: The proposed WebCryptoAgent framework decomposes web-informed decision making into modality-specific agents and consolidates their outputs into a unified evidence document for confidence-calibrated reasoning. It also introduces a decoupled control architecture separating strategic hourly reasoning from real-time second-level risk models.
...
Conclusion: WebCryptoAgent effectively addresses the challenges of integrating heterogeneous web information and market microstructure signals for cryptocurrency trading by decomposing decision-making into modality-specific agents and using a decoupled control architecture. This approach improves trading stability, reduces spurious activity, and enhances tail-risk handling.
...
TL;DR: Forge-and-Quench is a unified framework that uses understanding models to refine image generation, enhancing fidelity and detail while maintaining accuracy and knowledge application, with efficient model migration.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore how understanding models can enhance image generation by improving fidelity and detail richness, rather than just leveraging reasoning or world knowledge.

Method: The paper proposes the Forge-and-Quench framework, where an MLLM refines text instructions into an enhanced version, which is then mapped to a virtual visual representation (Bridge Feature) via a Bridge Adapter. This feature guides the T2I backbone in generating images.

Result: Experiments show that Forge-and-Quench improves image fidelity and detail across multiple models, maintains instruction-following accuracy, and enhances world knowledge application, with efficient migration across different MLLM and T2I models.

Conclusion: The paper concludes that the Forge-and-Quench framework successfully integrates understanding models to enhance image generation, improving fidelity, detail richness, and instruction-following accuracy while maintaining the MLLM's multimodal understanding capabilities.
...
TL;DR: HuForDet is a holistic framework for detecting human image forgeries, combining face and full-body detection with a dual-branch architecture and a curated dataset.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of AI-generated content (AIGC) has escalated the threat of deepfakes, necessitating a unified detection method for both facial and full-body manipulations.

Method: HuForDet features a dual-branch architecture: a face forgery detection branch using RGB and frequency domains, and a contextualized forgery detection branch leveraging a Multi-Modal Large Language Model (MLLM).

Result: HuForDet achieves state-of-the-art forgery detection performance and superior robustness across diverse human image forgeries.

Conclusion: HuForDet provides a comprehensive solution for detecting human image forgeries, addressing the limitations of existing fragmented methods.
