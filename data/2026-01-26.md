...
TL;DR: A method for 3D relighting of room-scale scenes using diffusion models, avoiding inverse rendering problems.


<details>
  <summary>Details</summary>
Motivation: Existing solutions for 3D scene relighting are limited by under-determined or ill-conditioned inverse rendering problems and cannot handle complex real-world scenes effectively.

Method: Distilling outputs of a video-to-video relighting diffusion model into a 3D reconstruction.

Result: The approach faithfully renders novel views of scenes under new lighting conditions.

Conclusion: The method successfully enables high-quality 3D relighting of room-scale scenes without solving complex inverse rendering problems, validated on synthetic and real-world datasets.
...
TL;DR: Memory-V2V enhances multi-turn video editing by using memory to maintain cross-consistency, achieving 30% speedup without sacrificing performance.


<details>
  <summary>Details</summary>
Motivation: Current video editors struggle with cross-consistency in iterative, multi-turn video editing.

Method: Memory-V2V augments existing video-to-video models with explicit memory, using retrieval and dynamic tokenization strategies, and includes a learnable token compressor for efficiency.

Result: Memory-V2V achieves 30% speedup and produces more cross-consistent videos, outperforming state-of-the-art baselines.

Conclusion: Memory-V2V significantly improves cross-consistency in multi-turn video editing with minimal computational overhead, maintaining or enhancing task-specific performance.
...
TL;DR: FeTTL harmonizes multi-institutional medical imaging data in federated learning, outperforming baselines in segmentation and classification tasks.


<details>
  <summary>Details</summary>
Motivation: Domain shifts and data heterogeneity in federated learning degrade model performance, especially in medical imaging due to variations in acquisition protocols, scanner types, and patient populations.

Method: The paper introduces Federated Template and Task Learning (FeTTL), a framework that learns a global template and a task model to align data distributions among clients in federated environments.

Result: FeTTL significantly outperforms state-of-the-art federated learning baselines (p-values <0.002) in tasks like retinal fundus optical disc segmentation and histopathological metastasis classification.

Conclusion: : FeTTL provides a robust and extensible solution for addressing distribution shifts in federated learning, particularly in multi-institutional medical imaging tasks.
...
TL;DR: Study shows multimodal models perform poorly at identifying important sub-events in football games, calling for improved architectures and training.


<details>
  <summary>Details</summary>
Motivation: To assess the ability of models to identify important sub-events in videos, a prerequisite for narrating or summarizing multimodal events.

Method: The authors constructed a new dataset using human preferences from football game highlight reels and evaluated state-of-the-art multimodal models.

Result: Current multimodal models perform close to chance level, rely on a single dominant modality, and struggle to synthesize information from multiple sources.

Conclusion: The study highlights the limitations of current multimodal models in identifying important sub-events in football games and underscores the need for modular architectures and complementary training procedures to improve cross-modal synergy.
...
TL;DR: A coarse-to-fine non-rigid multi-modal registration method using sparse keypoints and thin-plate-splines for aligning historical panel painting images, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Manual alignment of multi-modal images in art technological investigations is laborious and imprecise, necessitating an automated, efficient solution.

Method: Proposes a one-stage non-rigid registration approach using CNNs for keypoint detection and GNNs for descriptor matching, with multi-level keypoint refinement for mixed-resolution images.

Result: Achieves the best registration results compared to competing methods, validated by a large multi-modal dataset and ablation studies.

Conclusion: The method significantly improves efficiency and precision in multi-modal image registration for historical paintings, leveraging craquelure patterns as features.
...
TL;DR: The paper introduces perspective tokens to improve spatial reasoning in multimodal language models, showing enhanced performance on perspective-taking tasks.


<details>
  <summary>Details</summary>
Motivation: Current multimodal language models (MLMs) perform well on semantic tasks but struggle with spatial reasoning and allocentric perspective-taking, highlighting a need for better spatial cognition mechanisms.

Method: The authors propose perspective tokens, specialized embeddings that encode orientation through body-keypoint cues or abstract representations for mental rotation, integrated into LLaVA-1.5-13B.

Result: Perspective tokens improve accuracy on synthetic and naturalistic benchmarks (Isle Bricks V2, COCO, 3DSRBench), with rotation-based tokens generalizing to non-human agents. Fine-tuning enhances latent orientation sensitivity.

Conclusion: Embedding cognitively grounded spatial structure into token space provides a lightweight, model-agnostic solution for improving perspective-taking and spatial reasoning in MLMs.
...
TL;DR: VTFusion, a vision-text fusion framework for FSAD, improves anomaly detection by addressing domain-specific semantics and cross-modal misalignment, achieving high accuracy on industrial datasets.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the neglect of granular, domain-specific semantics in FSAD and the superficial fusion strategies that compromise robustness against cross-modal interference.

Method: The study introduces VTFusion, a vision-text multimodal fusion framework with adaptive feature extractors and a dedicated multimodal prediction fusion module, including a fusion block and segmentation network.

Result: VTFusion achieves image-level AUROCs of 96.8% and 86.2% in the 2-shot scenario on MVTec AD and VisA datasets, respectively, and an AUPRO of 93.5% on a real-world industrial dataset.

Conclusion: VTFusion significantly advances FSAD performance by addressing domain-specific semantics and semantic misalignment between visual and textual modalities, achieving high accuracy in industrial inspection scenarios.
...
TL;DR: A novel RES framework, \model, integrates EBD and VBR to address limitations in existing methods, achieving SOTA performance with minimal prompts.


<details>
  <summary>Details</summary>
Motivation: Existing RES methods suffer from coarse bounding boxes leading to redundant point prompts and unreliable textual coordinate reasoning, which fails to distinguish targets from visually similar distractors.

Method: The framework combines Entropy-Based Point Discovery (EBD) for high-information point selection and Vision-Based Reasoning (VBR) for robust validation through visual-semantic alignment. It follows a coarse-to-fine workflow: bounding box initialization, entropy-guided point discovery, vision-based validation, and mask decoding.

Result: Extensive evaluations on RefCOCO, RefCOCO+, RefCOCOg, and ReasonSeg show that \model achieves new state-of-the-art performance across all benchmarks.

Conclusion: The proposed \textbf{\model} framework, integrating EBD and VBR, achieves state-of-the-art performance across four benchmark datasets, demonstrating its effectiveness in accurate and semantically grounded segmentation with minimal prompts.
...
TL;DR: CSRNet improves image super-resolution by using heterogeneous blocks and cosine annealing, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: To enhance image super-resolution by preserving structural information and overcoming the limitations of homologous information, while optimizing the training process.

Method: The paper introduces a cosine network (CSRNet) with odd and even heterogeneous blocks to extract complementary structural information, combining linear and non-linear features. A cosine annealing mechanism optimizes training.

Result: CSRNet achieves competitive results in image super-resolution, leveraging complementary structural information and optimized training.

Conclusion: The proposed CSRNet demonstrates competitive performance with state-of-the-art methods in image super-resolution, validated by experimental results.
...
TL;DR: DCCS-Det is a novel infrared small target detector that improves target-background discrimination and feature representation by combining localized perception, direction-aware context, and cross-scale feature extraction.


<details>
  <summary>Details</summary>
Motivation: Existing methods for infrared small target detection (IRSTD) struggle with inadequate local-global feature modeling and feature redundancy, degrading performance.

Method: DCCS-Det introduces a Dual-stream Saliency Enhancement (DSE) block for local-global feature integration and a Latent-aware Semantic Extraction and Aggregation (LaSEA) module for cross-scale feature extraction and noise suppression.

Result: The detector achieves state-of-the-art accuracy and efficiency across multiple datasets, with ablation studies confirming the effectiveness of its components.

Conclusion: DCCS-Det addresses key challenges in IRSTD by enhancing target perception and feature representation, validated through extensive experimentation.
...
TL;DR: AlphaFace improves face-swapping for extreme poses using CLIP embeddings, achieving better results and real-time performance.


<details>
  <summary>Details</summary>
Motivation: To address quality degradation in face-swapping under extreme poses without increasing computational cost or dependencies.

Method: Uses an open-source vision-language model and CLIP embeddings with visual and textual semantic contrastive losses.

Result: Surpasses state-of-the-art methods in pose-challenging cases, as shown in FF++, MPIE, and LPFF datasets.

Conclusion: AlphaFace outperforms existing methods in handling extreme facial poses by leveraging vision-language models and CLIP embeddings, maintaining real-time performance.
...
TL;DR: MDAFNet improves infrared small target detection by addressing edge degradation and frequency component issues via multi-scale edge and adaptive frequency modules, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods degrade target edge pixels with deeper networks, and traditional convolution fails to differentiate frequency components, leading to background interference and false detections.

Method: The proposed MDAFNet integrates the Multi-Scale Differential Edge (MSDE) module for edge extraction and enhancement and the Dual-Domain Adaptive Feature Enhancement (DAFE) module for frequency domain processing and spatial domain fusion.

Result: Experimental results on multiple datasets show MDAFNet's superior performance in detecting infrared small targets.

Conclusion: MDAFNet demonstrates superior detection performance by effectively compensating for edge information loss and enhancing high-frequency targets while suppressing noise, as validated across multiple datasets.
...
TL;DR: The paper evaluates face recognition models' performance with and without masks, finding r100 excels in standard tests and r100_mask_v2 in masked tests, with Vit-Small/Tiny also performing well masked.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenges posed by mask-wearing passengers in civil aviation security checks, evaluating how different backbone networks perform in face recognition with and without masks.

Method: The study conducts extensive comparative experiments to evaluate core backbone networks, including r100, r50, r34_mask_v1, r100_mask_v2, r50_mask_v3, and Vit-Small/Tiny, under standard and masked conditions.

Result: In standard tests, r100 models excelled (98%+ accuracy), while in masked tests, r100_mask_v2 led (90.07% accuracy). Vit-Small/Tiny showed strong gains in masked performance.

Conclusion: The paper concludes with specific deployment recommendations for face recognition models, highlighting the performance of various backbone networks in both standard and masked scenarios.
...
TL;DR: Emotion-LLaMAv2 and MMEVerse benchmark address limitations in multimodal emotion recognition by introducing an end-to-end pipeline, standardized evaluation, and improved training data.


<details>
  <summary>Details</summary>
Motivation: The field lacks large-scale datasets with high-quality emotion annotations and standardized benchmarks, limiting the capabilities of multimodal large language models in emotional reasoning.

Method: Emotion-LLaMAv2 introduces an end-to-end multiview encoder, Conv Attention pre-fusion module, and perception-to-cognition curriculum instruction tuning. MMEVerse aggregates and re-annotates twelve datasets for large-scale training.

Result: The framework produces 130k training clips and 36k testing clips across 18 benchmarks, enabling reproducible evaluation.

Conclusion: Emotion-LLaMAv2 and MMEVerse significantly advance multimodal emotion recognition and reasoning by addressing key limitations in data quality, fusion strategies, and evaluation standards.
...
TL;DR: VISTA-PATH is an interactive, class-aware pathology segmentation foundation model that outperforms existing models by incorporating visual context, semantic descriptions, and expert feedback. It improves clinical interpretation and supports human-in-the-loop refinement.


<details>
  <summary>Details</summary>
Motivation: Current segmentation foundation models lack alignment with pathology needs, treating segmentation as a static task. VISTA-PATH aims to resolve heterogeneous structures and incorporate expert feedback for clinically meaningful results.

Method: VISTA-PATH jointly conditions segmentation on visual context, semantic tissue descriptions, and optional expert spatial prompts. It uses a curated dataset of 1.6 million image-mask-text triplets across 9 organs and 93 tissue classes.

Result: VISTA-PATH consistently outperforms existing models on benchmarks and supports dynamic refinement. Its Tumor Interaction Score (TIS) shows strong associations with patient survival.

Conclusion: VISTA-PATH elevates pathology image segmentation to an interactive, clinically grounded representation, improving tissue microenvironment analysis and digital pathology.
...
TL;DR: The paper introduces PhysGame, a dataset using gameplay glitches for physical reasoning, and GameBench, a benchmark for evaluation, showing improved AI performance in physical understanding.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack human-level physical understanding, and existing datasets are either costly or lack realism. Gameplay glitches offer a scalable, rich supervision source.

Method: Leverages glitches in gameplay videos as supervision, creating PhysGame (140,057 QA pairs) and GameBench (880 expert-annotated videos). Uses metadata-guided prompting for QA accuracy.

Result: PhysGame improves Qwen2.5VL's real-world reasoning by 2.5% and general reasoning by 1.9%. Models show a 3.7% gain on GameBench, detecting physical implausibilities better.

Conclusion: Gameplay anomalies provide a scalable, effective way to enhance multimodal AI's physical world understanding, as demonstrated by improved performance on benchmarks.
...
TL;DR: WoundNeRF, a NeRF SDF-based method, offers accurate 3D wound segmentation from 2D images, improving automated wound care and outperforming existing techniques.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges in wound care, such as economic and logistical burdens, and the need for fast, automatic tissue assessment. Wound segmentation, especially in 3D, is seen as a solution for precise healing progress tracking.

Method: The paper proposes WoundNeRF, a NeRF SDF-based method for estimating robust wound segmentations from automatically generated annotations. It compares this method against Vision Transformer networks and conventional rasterisation-based algorithms.

Result: WoundNeRF shows potential in recovering accurate segmentations, outperforming state-of-the-art Vision Transformer networks and conventional rasterisation-based algorithms.

Conclusion: : WoundNeRF demonstrates a promising approach for accurate 3D wound segmentation from 2D images, outperforming state-of-the-art methods and paving the way for further advancements in automated wound care.
...
TL;DR: EKDC-Net enhances tree species classification by integrating expert knowledge, achieving top performance with minimal added parameters.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing methods in handling long-tailed distributions and high inter-class similarity in tree species classification, inspired by human knowledge dissemination where experts are consulted to overcome local thinking.

Method: The paper introduces EKDC-Net, which includes a Local Prior Guided Knowledge Extraction Module (LPKEM) for discriminative feature focus and an Uncertainty-Guided Decision Calibration Module (UDCM) for dynamic decision correction.

Result: EKDC-Net improves backbone accuracy by 6.42% and precision by 11.46% with only 0.08M additional parameters, outperforming existing methods.

Conclusion: EKDC-Net, a lightweight plug-and-play module, significantly improves backbone accuracy and precision with minimal additional parameters, demonstrating state-of-the-art performance on tree species classification.
...
TL;DR: SALAD combines sparse and linear attention with a gating mechanism for efficient, high-sparsity video generation.


<details>
  <summary>Details</summary>
Motivation: High computational latency from long input sequences in Diffusion Transformers due to quadratic complexity of full attention. Existing sparse attention methods either offer modest acceleration (training-free) or require extensive resources (training-based).

Method: Introduces a lightweight linear attention branch parallel to sparse attention, with an input-dependent gating mechanism.

Result: Attains 90% sparsity and 1.72x speedup while maintaining quality.

Conclusion: SALAD achieves 90% sparsity and 1.72x inference speedup with comparable generation quality to full attention, using efficient training (2,000 samples, 1,600 steps, batch size 8).
...
TL;DR: TangramPuzzle benchmark reveals MLLMs struggle with precise spatial reasoning, focusing on silhouettes over geometric accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the lack of rigorous evaluation for precise compositional spatial reasoning in MLLMs, which current benchmarks simplify or approximate.

Method: The authors introduce TangramPuzzle, a benchmark using Tangram Construction Expression (TCE) for exact coordinate specifications, and design two tasks: Outline Prediction and End-to-End Code Generation.

Result: Experiments show MLLMs favor silhouette matching, often distorting pieces due to overlooked geometric constraints.

Conclusion: The study highlights that MLLMs prioritize silhouette matching over geometric constraints, indicating a gap in precise spatial reasoning capabilities.
...
TL;DR: AnchoredDream is a zero-shot pipeline for generating 360° scenes from single-view images, using geometry and appearance mutual boosting, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Generating a complete 360° scene from a single image is highly ill-posed and challenging, with existing methods struggling to maintain appearance consistency and geometric plausibility under large viewpoint changes.

Method: The method involves appearance-guided geometry generation, followed by warp-and-inpaint, warp-and-refine, post-optimization, and a novel Grouting Block to ensure seamless transitions.

Result: AnchoredDream outperforms existing methods by a large margin in both appearance consistency and geometric plausibility, all in a zero-shot manner.

Conclusion: AnchoredDream demonstrates the potential of geometric grounding for high-quality, zero-shot single-view scene generation, outperforming existing methods significantly.
...
TL;DR: OnlineSI enables continuous spatial understanding for MLLMs by maintaining spatial memory and integrating 3D semantic data, validated by experiments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in existing methods that lack continuous spatial understanding and real-world deployment capabilities for MLLMs.

Method: The method involves maintaining a finite spatial memory to retain past observations and integrating 3D point cloud with semantic information for better object localization and identification.

Result: Experiments on two datasets using the Fuzzy $F_1$-Score demonstrate the method's effectiveness in improving spatial reasoning.

Conclusion: The paper concludes that OnlineSI effectively enables MLLMs to continuously improve spatial understanding in dynamic environments, paving the way for real-world embodied systems.
...
TL;DR: Proposed a teacher-student framework for semi-supervised hierarchical open-set classification with subtree pseudo-labels and age-gating, achieving strong performance with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: To extend hierarchical open-set classification to the semi-supervised setting, enabling the use of large-scale, uncurated datasets containing known and unknown classes.

Method: A teacher-student framework based on pseudo-labeling with subtree pseudo-labels and age-gating to mitigate overconfidence in pseudo-labels.

Result: Improved hierarchical open-set performance compared to baseline methods.

Conclusion: The framework outperforms self-supervised pretraining followed by supervised adaptation and matches fully supervised performance with only 20 labeled samples per class on iNaturalist19.
...
TL;DR: HA2F, a dual-module framework for remote sensing change detection, outperforms existing methods by addressing feature alignment and noise sensitivity.


<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from cross-temporal feature matching deviations and sensitivity to noise, which HA2F aims to address.

Method: The HA2F framework consists of two modules: DHFCM for dynamic hierarchical feature calibration and NAFRM for noise-adaptive feature refinement.

Result: HA2F achieves state-of-the-art performance on LEVIR-CD, WHU-CD, and SYSU-CD datasets, with improved precision and computational efficiency.

Conclusion: The proposed HA2F framework outperforms existing methods on multiple datasets, demonstrating its effectiveness in remote sensing change detection.
...
TL;DR: A novel CoVR framework with X-Aligner module and two-stage training achieves state-of-the-art performance in composed video retrieval and zero-shot generalization.


<details>
  <summary>Details</summary>
Motivation: Existing CoVR frameworks fuse multimodal inputs in a single stage, yielding marginal gains. The authors aim to improve this by leveraging VLMs and introducing a cross-attention module.

Method: The framework uses a novel cross-attention module (X-Aligner) to progressively fuse visual and textual inputs and align their representations with the target video. It is trained in two stages to preserve pretrained VLM representations.

Result: The framework achieves a Recall@1 of 63.93% on Webvid-CoVR-Test and performs well in zero-shot evaluations on CIR datasets like CIRCO and Fashion-IQ.

Conclusion: The proposed CoVR framework, leveraging VLMs and the X-Aligner module, achieves state-of-the-art performance on CoVR tasks and demonstrates strong zero-shot generalization on CIR tasks.
...
TL;DR: A lightweight hybrid classical-quantum framework for medical image analysis improves performance under resource constraints.


<details>
  <summary>Details</summary>
Motivation: Addresses challenges of scarce annotations, limited computational resources, and suboptimal model generalization in medical image analysis.

Method: Proposes a lightweight framework integrating self-supervised contrastive learning with quantum-enhanced feature modeling. Uses MobileNetV2 as a backbone and a quantum feature enhancement module.

Result: Outperforms classical baselines in Accuracy, AUC, and F1-score with only 2-3 million parameters and low computational cost.

Conclusion: The work provides a practical and forward-looking solution for high-performance medical AI under resource constraints, combining self-supervised learning and quantum enhancement.
...
TL;DR: BPIM framework improves small object detection in UAVs by integrating boundary, position, and scale info, outperforming baselines with efficient computation.


<details>
  <summary>Details</summary>
Motivation: Address challenges in UAV applications like imbalanced scale and blurred edges for small object detection.

Method: BPIM framework includes PIG, BIG, CSF, TFF, and AWF modules, utilizing attention mechanisms and cross-scale feature fusion for enhanced small object detection.

Result: BPIM outperforms Yolov5-P2 on VisDrone2021, DOTA1.0, and WiderPerson datasets, showing promising results with comparable computation.

Conclusion: BPIM demonstrates superior performance in small object detection by integrating boundary, position, and scale information, outperforming baseline methods like Yolov5-P2 and achieving state-of-the-art results with comparable computational efficiency.
...
TL;DR: SCHIGAND is a synthetic face generation pipeline that combines multiple models to create realistic, diverse, and identity-preserving facial datasets, offering a privacy-compliant solution for biometric applications.


<details>
  <summary>Details</summary>
Motivation: Addressing privacy regulations, data scarcity, and ethical concerns in facial dataset generation for biometric systems.

Method: SCHIGAND integrates StyleCLIP, HyperStyle, InterfaceGAN, and Diffusion models to generate realistic and controllable facial datasets.

Result: SCHIGAND achieves a balance between image quality and diversity, outperforming prior generative models in facial verification tests using ArcFace.

Conclusion: SCHIGAND effectively balances realism, diversity, and identity preservation in synthetic face generation, offering a viable alternative to real-world datasets for biometric applications.
...
TL;DR: A novel Structure Preservation Loss (SPL) is proposed to maintain edge structures in latent-diffusion-based image editing, achieving state-of-the-art results without additional training.


<details>
  <summary>Details</summary>
Motivation: Maintaining pixel-level edge structures in latent-diffusion-based image editing is challenging, especially for tasks like photorealistic style transfer or image tone adjustment.

Method: The method introduces a training-free approach with SPL, leveraging local linear models to quantify structural differences. It includes a post-processing step, masking strategy, and color preservation loss.

Result: Experiments confirm that SPL improves structural fidelity, outperforming existing methods.

Conclusion: The proposed Structure Preservation Loss (SPL) effectively enhances structural fidelity in latent-diffusion-based image editing, achieving state-of-the-art performance.
...
TL;DR: A reliable, energy-efficient SNN framework for 3D brain tumor segmentation with multi-view ensemble and FPTT reduces FLOPs by 87% while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the need for reliable and energy-efficient 3D brain tumor segmentation, leveraging the advantages of spiking neural networks (SNNs).

Method: The framework uses a multi-view ensemble of sagittal, coronal, and axial SNN models with voxel-wise uncertainty estimation. It employs Forward Propagation Through Time (FPTT) to reduce computational costs while maintaining efficiency.

Result: Experiments on BraTS 2017 and BraTS 2023 show competitive accuracy, well-calibrated uncertainty, and an 87% reduction in FLOPs.

Conclusion: The paper underscores the potential of SNNs for reliable, low-power medical IoT and Point-of-Care systems, demonstrated by competitive accuracy and reduced computational costs.
