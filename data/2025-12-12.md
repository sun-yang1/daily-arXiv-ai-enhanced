...
TL;DR: The paper proposes a neuromorphic approach for eye-tracking using SNNs, achieving near-ANN accuracy with significantly lower power and compute costs.


<details>
  <summary>Details</summary>
Motivation: Conventional frame-based eye-tracking systems face issues like motion blur and high compute costs, limiting their suitability for AR/VR applications. Neuromorphic sensors and SNNs offer a promising alternative but need performance improvements.

Method: The paper redesigns top-performing event-based eye-tracking models by replacing recurrent and attention modules with lightweight LIF layers and depth-wise separable convolutions.

Result: The models achieve 3.7-4.1px mean error (close to Retina's 3.24px) while reducing model size by 20x and compute by 850x compared to ANN variants. Estimated power is 3.9-4.9 mW with 3 ms latency.

Conclusion: High-performing event-based eye-tracking can be redesigned as SNNs with substantial efficiency gains, retaining accuracy for real-time wearable deployment.
...
TL;DR: ABBSPO introduces adaptive bounding box scaling and symmetry-prior-based orientation prediction to improve weakly supervised oriented object detection, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: To address limitations in previous HBox-supervised OOD methods, such as inaccurate scale estimation and learning collapse in orientation prediction.

Method: Proposes Adaptive Bounding Box Scaling (ABBS) and Symmetric Prior Angle (SPA) loss to optimize scale prediction and exploit object symmetry for self-supervised learning.

Result: ABBSPO outperforms existing methods, demonstrating state-of-the-art performance in WS-OOD.

Conclusion: ABBSPO effectively improves accuracy in weakly supervised oriented object detection by addressing key limitations of prior methods.
...
TL;DR: The paper proposes a hybrid approach combining diffusion models with autoregressive generation for improved performance in discrete domains, achieving State-of-the-Art results on COCO.


<details>
  <summary>Details</summary>
Motivation: Diffusion models underperform autoregressive solutions in discrete domains. The authors aim to bridge this gap by leveraging the strengths of both paradigms.

Method: A hybrid model (Show, Suggest, and Tell - SST) integrates diffusion models to provide suggestions to autoregressive generation, combining bidirectional refinement with linguistic structure.

Result: SST achieves 125.1 CIDEr-D on COCO, outperforming autoregressive and diffusion baselines by 1.5 and 2.5 points, respectively.

Conclusion: The hybrid approach demonstrates promise, with experiments validating the positive impact of the suggestion module on caption quality.
...
TL;DR: MetaVoxel is a joint diffusion framework unifying medical AI tasks, enabling flexible zero-shot inference without retraining, with performance matching task-specific models.


<details>
  <summary>Details</summary>
Motivation: Modern deep learning methods are often trained to model conditional distributions defined by a specific predictive direction with a specific set of input variables, limiting flexibility.

Method: MetaVoxel, a generative joint diffusion modeling framework that models the joint distribution over imaging data and clinical metadata by learning a single diffusion process spanning all variables.
...
Conclusion: The findings demonstrate that joint multimodal diffusion offers a promising direction for unifying medical AI models and enabling broader clinical applicability.
...
TL;DR: Proposes Independent Density Estimation (IDE) for compositional generalization in vision-language models, outperforming current methods.


<details>
  <summary>Details</summary>
Motivation: Large-scale vision-language models struggle with human-like compositional generalization.

Method: IDE learns word-image feature connections. Two models: one with fully disentangled visual inputs, another using a VAE for partial disentanglement. Includes entropy-based compositional inference.

Result: Superior generalization to unseen compositions compared to existing models.

Conclusion: IDE effectively addresses compositional generalization challenges in vision-language tasks.
...
TL;DR: TraceFlow introduces a novel framework for high-fidelity rendering of dynamic specular scenes, combining advanced representation and hybrid rendering for superior results.


<details>
  <summary>Details</summary>
Motivation: Addressing two key challenges in high-fidelity rendering of dynamic specular scenes: precise reflection direction estimation and physically accurate reflection modeling.

Method: Residual Material-Augmented 2D Gaussian Splatting representation and a hybrid rendering pipeline combining rasterization and ray tracing.

Result: TraceFlow demonstrates superior performance in dynamic scene benchmarks, achieving sharper and more realistic specular reflections.

Conclusion: TraceFlow outperforms prior methods both quantitatively and qualitatively, producing sharper and more realistic specular reflections in complex dynamic environments.
...
TL;DR: Proposes hierarchical instance tracking, introduces a benchmark dataset, and evaluates model variants, revealing the dataset's challenges.


<details>
  <summary>Details</summary>
Motivation: The motivation behind the paper is to address the need for tracking all instances of predefined categories of objects and parts while maintaining their hierarchical relationships, a task not previously benchmarked.

Method: The paper proposes a novel task of hierarchical instance tracking and introduces a benchmark dataset consisting of 2,765 unique entities tracked in 552 videos across 40 categories. It evaluates seven variants of four models tailored to this task.

Result: The results show that the new dataset is challenging, as evidenced by the evaluation of the seven model variants.

Conclusion: The paper concludes by highlighting the challenges revealed through the evaluation of seven variants of four models on the new dataset, emphasizing its utility for future research in hierarchical instance tracking.
...
TL;DR: A wavelet-based vectorization of persistent homology improves breast cancer detection accuracy across diverse mammography datasets.


<details>
  <summary>Details</summary>
Motivation: Breast cancer screening suffers from false negatives/positives and degraded model performance across scanners and populations.

Method: Uses topological data analysis to create stable, multi-scale spatial maps from wavelet persistence, integrated into a two-stage detection pipeline.

Result: Patient-level AUC increased from 0.55 to 0.75 on INbreast dataset when augmenting ConvNeXt Tiny with wavelet persistence channels.

Conclusion: The proposed method enhances external performance of mammography models, demonstrating robustness across varied datasets.
...
TL;DR: The paper introduces the Feature Coding Test Model (FCTM) for compressing intermediate features in DNNs, achieving 85.14% bitrate reduction while maintaining accuracy, addressing edge-cloud deployment challenges.


<details>
  <summary>Details</summary>
Motivation: Deploying DNNs on edge devices faces trade-offs in latency, bandwidth, and privacy. Splitting inference between edge and cloud introduces bandwidth challenges for intermediate features.

Method: The Moving Picture Experts Group (MPEG) initiated the Feature Coding for Machines (FCM) standard, designing the FCTM to compress intermediate features efficiently.

Result: FCTM achieves an average 85.14% bitrate reduction across multiple vision tasks without compromising accuracy.

Conclusion: FCM provides a scalable solution for efficient and interoperable deployment of intelligent features in bandwidth-limited and privacy-sensitive applications.
...
TL;DR: Latent-CoT-Drive (LCDrive) introduces latent language for chain-of-thought reasoning in autonomous driving, improving performance and safety over text-based reasoning.


<details>
  <summary>Details</summary>
Motivation: Text may not be the most efficient representation for reasoning in Vision-Language-Action models for autonomous driving.

Method: LCDrive uses action-proposal and world model tokens in a latent space, supervised by ground-truth future rollouts and post-trained with reinforcement learning.

Result: LCDrive achieves faster inference, better trajectory quality, and larger improvements from interactive reinforcement learning compared to baselines.

Conclusion: Latent reasoning unifies chain-of-thought and decision-making, outperforming text-based approaches in driving scenarios.
...
TL;DR: The paper introduces VCM and FCM for efficient machine-to-machine communication, showing FCM reduces bitrate without accuracy loss. HEVC and VVC perform similarly, while AVC is less efficient but still viable for tracking.


<details>
  <summary>Details</summary>
Motivation: Current machine-to-machine systems rely on human-optimized codecs, which are bandwidth-intensive, scale poorly, and expose raw images to third parties. The paper aims to redesign this pipeline for efficiency and privacy.

Method: The paper evaluates Video Coding for Machines (VCM) and Feature Coding for Machines (FCM), comparing their performance with traditional codecs (H.265/HEVC, H.266/VVC, and H.264/AVC) in terms of bitrate reduction and task accuracy.

Result: FCM significantly reduces bitrate while maintaining accuracy close to edge inference. H.265/HEVC and H.266/VVC perform similarly, with H.264/AVC showing higher bitrate increases. For tracking tasks, HEVC outperforms VVC, and AVC's impact is minimal.

Conclusion: Existing hardware for deployed codecs (like HEVC and AVC) can support machine-to-machine communication without degrading performance, especially for tasks like tracking where codec choice has minimal impact.
...
TL;DR: MCDPO resolves reward conflicts in DPO by introducing a disentangled Bradley-Terry objective and dimensional reward dropout, achieving superior performance in diffusion models.


<details>
  <summary>Details</summary>
Motivation: The standard DPO formulation aggregates diverse evaluation axes into a single scalar reward, causing reward conflicts where desirable features may be unlearned.

Method: Proposes Multi Reward Conditional DPO (MCDPO), which uses a disentangled Bradley-Terry objective and injects a preference outcome vector as a condition during training. Also introduces dimensional reward dropout.

Result: MCDPO achieves superior performance on benchmarks with Stable Diffusion 1.5 and SDXL, enabling dynamic multi-axis control at inference time.

Conclusion: MCDPO effectively addresses reward conflicts in DPO, allowing independent optimization across reward axes and dynamic control without additional training.
...
TL;DR: SWIFT (Stage-Wise Finetuning with Temperature Tuning) improves semi-supervised few-shot learning (SSFSL) by leveraging open-source Vision-Language Models (VLMs) and addressing their underperformance through classifier initialization and temperature tuning.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the underutilization of open-source VLMs in SSFSL, which contrasts with their successful application in few-shot learning (FSL). The goal is to enhance auto-annotation by improving the utilization of unlabeled data and supervision signals.

Method: The authors propose SWIFT, which involves classifier initialization and temperature tuning to boost the confidence scores of pseudo-labels. This method finetunes VLMs on limited labeled data, abundant unlabeled data, and noisy task-relevant data from the VLM's pretraining set.

Result: SWIFT outperforms recent FSL and SSL methods by approximately 5 accuracy points on five SSFSL benchmarks and rivals supervised learning performance.

Conclusion: SWIFT effectively bridges the gap between SSFSL and FSL by leveraging VLMs and simple yet powerful techniques, demonstrating significant performance improvements in semi-supervised few-shot learning.
...
TL;DR: RobustSora benchmark evaluates watermark robustness in AIGC video detection, revealing partial dependency and performance variations.


<details>
  <summary>Details</summary>
Motivation: Assess the influence of digital watermarks on AIGC video detection, overlooked by recent benchmarks.

Method: Constructed a dataset of 6,500 videos (A-C, A-S, G-W, G-DeW) and introduced two evaluation tasks (Task-I, Task-II). Tested ten models.

Result: Performance variations of 2-8pp under watermark manipulation; transformer-based models show moderate dependency (6-8pp), MLLMs exhibit diverse patterns (2-8pp).

Conclusion: Partial watermark dependency exists, highlighting the need for watermark-aware training strategies. RobustSora advances robust AIGC detection research.
...
TL;DR: THE-Pose introduces a hybrid graph fusion framework for category-level 6D pose estimation, combining topological and point-cloud features to outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing 3D graph convolution methods focus only on local geometry, making them vulnerable to complex objects and visual ambiguities.

Method: THE-Pose leverages surface embedding and hybrid graph fusion to integrate topological features from images with point-cloud features.

Result: THE-Pose achieves a 35.8% improvement over the 3D-GC baseline and surpasses the previous state-of-the-art by 7.2% on the REAL275 dataset.

Conclusion: THE-Pose effectively bridges 2D image context and 3D geometric structure, ensuring robustness for unseen or complicated objects under occlusions.
...
TL;DR: GDKVM introduces a novel architecture for echocardiography video segmentation, combining LKVA, GDR, and KPFF modules to improve accuracy and robustness while maintaining real-time performance.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation of cardiac chambers in echocardiography is crucial for clinical diagnosis, but existing methods struggle with balancing spatiotemporal dependencies and computational efficiency.

Method: GDKVM employs Linear Key-Value Association (LKVA) for inter-frame correlations, Gated Delta Rule (GDR) for memory states, and Key-Pixel Feature Fusion (KPFF) for multi-scale feature integration.

Result: GDKVM outperforms state-of-the-art methods on CAMUS and EchoNet-Dynamic datasets in segmentation accuracy and robustness, with real-time performance.

Conclusion: GDKVM provides an effective solution for echocardiography segmentation, addressing key challenges and achieving superior performance.
...
TL;DR: LLM-NCD is a multimodal framework for Novel Class Discovery (NCD) that fuses visual-textual semantics and prototype-guided clustering, achieving a 25.3% accuracy improvement for unknown classes on CIFAR-100.


<details>
  <summary>Details</summary>
Motivation: Existing NCD methods rely on visual features, which have limitations like insufficient discriminability and long-tail distribution issues.

Method: LLM-NCD jointly optimizes known class image and text features, models cluster centers and semantic prototypes, and uses a dual-phase discovery mechanism with semantic affinity thresholds and adaptive clustering.

Result: The method achieves up to 25.3% improvement in accuracy for unknown classes on CIFAR-100 and shows resilience to long-tail distributions.

Conclusion: LLM-NCD effectively addresses limitations of existing NCD methods by leveraging multimodal fusion and adaptive clustering, setting a new benchmark in the field.
...
TL;DR: Long-LRM++ combines implicit representation benefits with real-time performance, achieving high-quality rendering and scalability.


<details>
  <summary>Details</summary>
Motivation: To retain the benefits of implicit representations while enabling real-time performance, overcoming the speed limitations of prior implicit methods.

Method: Long-LRM++ adopts a semi-explicit scene representation combined with a lightweight decoder.

Result: Long-LRM++ matches the rendering quality of LaCT on DL3DV while achieving real-time 14 FPS rendering on an A100 GPU and scales to 64 input views at the $950×540$ resolution.

Conclusion: Long-LRM++ successfully combines the benefits of implicit representations with real-time performance, achieving high rendering quality and scalability to more input views.
...
TL;DR: SAAD improves adversarial robustness transfer by reweighting training examples based on adversarial transferability, outperforming prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial distillation methods often neglect state-of-the-art robust teachers and fail to explain why stronger teachers don't always yield more robust students.

Method: Proposes Sample-wise Adaptive Adversarial Distillation (SAAD), which reweights training examples by their adversarial transferability without extra computational cost.

Result: SAAD consistently improves AutoAttack robustness on CIFAR-10, CIFAR-100, and Tiny-ImageNet.

Conclusion: Adversarial transferability is a key factor in robustness transfer, and SAAD effectively leverages it to enhance student robustness.
...
TL;DR: MotionEdit introduces a dataset for motion-centric image editing and a benchmark (MotionEdit-Bench). MotionNFT, a post-training framework, improves motion editing quality and fidelity.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of motion-centric image editing, which is scientifically challenging and practically significant for applications like frame-controlled video synthesis and animation.

Method: MotionNFT (Motion-guided Negative-aware Fine Tuning), a post-training framework that computes motion alignment rewards based on motion flow matching.

Result: Benchmark results reveal that motion editing remains highly challenging for existing state-of-the-art diffusion-based editing models, but MotionNFT improves performance.

Conclusion: MotionNFT consistently improves editing quality and motion fidelity on the motion editing task without sacrificing general editing ability, demonstrating its effectiveness.
...
<details>
  <summary>Details</summary>
Motivation: Recent progress in video generation focuses on visual consistency but neglects intentional film-editing patterns, leading to incoherent narratives.

Method: The framework uses a camera control module (6-DoF poses and intrinsic settings) and a shot-aware mask mechanism for hierarchical prompts.

Result: The framework achieves controllable shot transitions, supported by the ShotWeaver40K dataset and new evaluation metrics.

Conclusion: ShotDirector effectively combines parameter-level conditions with semantic guidance, advancing controllable multi-shot video generation.
...
TL;DR: Disentangled360 integrates direction disentangled volume rendering with single-image 360° view synthesis for medical imaging and natural scene reconstruction, outperforming current techniques in SSIM and LPIPS metrics.


<details>
  <summary>Details</summary>
Motivation: Current techniques either oversimplify anisotropic light behavior or lack generalizability across various contexts.

Method: The framework uses a Gaussian Splatting backbone to differentiate isotropic and anisotropic contributions, implements a dual-branch conditioning framework, and employs a hybrid pose agnostic anchoring method.

Result: Superior SSIM and LPIPS performance on Mip-NeRF 360, RealEstate10K, and DeepDRR datasets, with confirmed viability for interactive applications.

Conclusion: Disentangled360 eliminates the need for scene-specific finetuning or expensive photon simulations, facilitating mixed-reality medical supervision, robotic perception, and immersive content creation.
...
TL;DR: Efficient-VLN is a training-efficient Vision-Language Navigation model that addresses computational overhead and exploration-efficiency trade-offs in DAgger through progressive and recursive memory mechanisms, achieving state-of-the-art performance with reduced training time.


<details>
  <summary>Details</summary>
Motivation: The substantial training overhead in MLLMs for VLN, caused by quadratic computational burden and exploration-efficiency trade-offs in DAgger, hinders practical development.

Method: Proposes Efficient-VLN with two memory mechanisms (progressive and recursive) and a dynamic mixed policy to balance exploration and efficiency.

Result: Achieves 64.2% SR on R2R-CE and 67.0% SR on RxR-CE, with only 282 H800 GPU hours.

Conclusion: Efficient-VLN significantly reduces training overhead while maintaining high performance, making it a practical solution for VLN tasks.
...
TL;DR: A prototype-driven framework for weakly supervised semantic segmentation (WSSS) in histopathology, leveraging vision-language alignment and multi-scale pyramid modules to improve region discovery and surpass state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of CAM-based supervision in WSSS, such as inter-class homogeneity, intra-class heterogeneity, and region-shrinkage effect.

Method: Integrates CoOp-style learnable prompt tuning for text-based prototypes and combines them with image prototypes, forming a dual-modal prototype bank. Also incorporates a multi-scale pyramid module to enhance spatial precision.

Result: Surpasses existing state-of-the-art methods on the BCSS-WSSS benchmark.

Conclusion: The approach effectively leverages textual semantics and visual prototype learning for WSSS in digital pathology, demonstrating the benefits of text description diversity, context length, and complementary behavior of text and image prototypes.
...
TL;DR: A prototype learning framework for weakly supervised semantic segmentation (WSSS) in histopathology integrates vision-language models (CONCH) and segmentation backbones (SegFormer) to improve localization completeness and semantic consistency without dense annotations.


<details>
  <summary>Details</summary>
Motivation: Existing WSSS methods in histopathology often localize only discriminative regions and struggle with full spatial extent. Combining complementary strengths of vision-language models and segmentation backbones remains challenging under weak supervision.

Method: Proposes a prototype learning framework with text-guided prototype initialization, structural distillation from SegFormer, and morphology-aware representations from CONCH.

Result: Outperforms existing WSSS methods on BCSS-WSSS datasets, improving localization completeness and semantic consistency.

Conclusion: The framework effectively leverages heterogeneous sources (text, morphology, structure) to produce high-quality pseudo-masks without pixel-level annotations, demonstrating computational efficiency.
...
TL;DR: EchoingPixels introduces a framework for token reduction in AV-LLMs by leveraging cross-modal synergies with the Cross-Modal Semantic Sieve (CS2) and Sync-RoPE, achieving efficiency gains.


<details>
  <summary>Details</summary>
Motivation: Token reduction in AV-LLMs is challenging due to the inability of unimodal methods to leverage cross-modal synergies and the dynamic information densities of audio and video.

Method: The framework uses CS2 for early audio-visual interaction and token reduction from a combined pool, alongside Sync-RoPE to preserve temporal relationships.

Result: EchoingPixels achieves comparable performance to baselines with only 5-20% of original tokens, offering 2-3x speedup and memory reduction.

Conclusion: The proposed framework effectively addresses the token reduction bottleneck in AV-LLMs by integrating cross-modal interaction and adaptive token allocation.
...
TL;DR: StainNet is a specialized foundation model for special stains in pathology, based on ViT architecture, trained with self-distillation SSL on over 1.4 million patch images. It addresses limitations of existing PFMs pre-trained on H&E-stained images.


<details>
  <summary>Details</summary>
Motivation: Existing pathology foundation models (PFMs) are pre-trained on H&E-stained images, limiting their application in clinical scenarios involving special stains like immunohistochemistry.

Method: StainNet uses a self-distillation SSL approach and is trained on patch images from 20,231 special staining WSIs in the HISTAI database.

Result: Experiments on slide-level liver malignancy classification and ROI-level datasets demonstrate StainNet's strong performance. Few-ratio learning and retrieval evaluations further highlight its advantages over larger PFMs.

Conclusion: StainNet effectively addresses the limitations of H&E-trained PFMs for special stains, showing robust performance in various evaluations, and its model weights are publicly available.
...
TL;DR: ISMVC selectively imputes missing multi-view data based on informativeness, avoiding noise and bias while leveraging variational autoencoders for robust clustering.


<details>
  <summary>Details</summary>
Motivation: Existing imputation-based methods introduce noise and bias, while imputation-free methods struggle with severe incompleteness. ISMVC addresses these limitations.

Method: ISMVC evaluates imputation-relevant informativeness, selectively imputes, and integrates a variational autoencoder with a mixture-of-Gaussians prior for clustering-friendly representations.

Result: ISMVC outperforms both imputation-based and imputation-free approaches in experiments under unbalanced missing scenarios.

Conclusion: ISMVC provides a lightweight, data-driven, and model-agnostic solution for robust multi-view clustering under incomplete data.
...
TL;DR: A Pix2Pix-based framework with a filament-aware structural loss generates realistic filaments to address data shortage, outperforming models without synthetic data.


<details>
  <summary>Details</summary>
Motivation: The dense distribution and geometric properties of filaments make manual annotation laborious and time-consuming, leading to a shortage of high-quality pixel-level annotated datasets for filament segmentation.

Method: A conditional generative framework based on the Pix2Pix architecture is proposed to generate realistic filaments in microscopy images from binary masks, along with a filament-aware structural loss to improve structure similarity.

Result: The experiments demonstrated the effectiveness of the approach, showing superior performance compared to existing models trained without synthetic data.

Conclusion: The proposed conditional generative framework and filament-aware structural loss effectively address the data shortage problem for filament segmentation, outperforming existing models trained without synthetic data.
